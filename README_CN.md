...
Xray 不使用任何形式的垃圾回收——既没有并发 tracing GC，也没有环收集器。普通 execution-local 引用值使用编译器插入的、按协程归属的引用计数；已发布的 const 根与受审计同步/运行时句柄使用其验证后的共享存储计划。引用计数就是全部机制，回收点因此是精确的：对象在丢掉最后一个引用的那条指令上销毁，不存在需要等待的延后扫描。代价是引用环不会被回收。环在它产生的地方处理，而不是事后清扫：编译器静态证明大多数对象图无环，证不了的用 `weak` 字段断边，剩下的那些以所属协程堆为上界、随协程结束整块释放。开发构建带一个检测器，会报出残留的环并点名该标注哪个字段。资源清理由 `defer` 显式表达。
