...
`ref` 实参是第四种、也是存活期最短的借用：一笔独占 place 借用，随调用进入而开始、随调用返回而结束。因为它绝不可能活过调用本身，所以不需要 loan 记录——它唯一的冲突面就是调用点自身，按同调用重叠检查（§5.2.4）。
...
**独占性在调用点强制执行。** 同一次调用内，`ref` place 不得与其他实参对同一所有权根的访问重叠——同名不行（`f(ref a, a)`），别名不行（`var b = a; f(ref a, b)`），方法调用的接收者也不行（`obj.m(ref obj)`）；重叠按 place 判定，所以 `f(ref h.left, h.right)` 合法（E0353 族）。嵌套实参如 `f(ref a, g(a))` 保持合法：实参先于调用求值，内层读在独占借用生效前就已完成。`ref` 指向接收者的真子 place（`this.m(ref this.field)`）同样在这个零误报的第一刀之外。
...
- **协程堆边界（L2）**：每个协程拥有独立的协程堆；协程结束时其堆整体释放。因此**未被引用计数回收的对象（引用环），其泄漏范围不超过所属协程的生命周期**。进程级的永久泄漏只可能来自 `MODULE_STATIC` / `SYNC_SHARED` 两个所有权域，以及主执行流自身的存活期。
- **`CONST_SHARED` 构造性无环**，因此从上面的名单里划掉：不可变图要成环，必须在构造期间就存在一条后向边，而 Xray 不提供任何造出它的方式——没有前向引用；发布进 `const` 存储要求对一张此前唯一的图做显式 `move`/`copy`；也没有能把仍可变（因此可能带环）的图偷偷定格的隐式 freeze。发布出的 const 图必为 DAG。
- **`SYNC_SHARED` 域的环是程序缺陷，语言把这句话说出来而不是藏起来**：共享域对象禁用 `weak`（W4），也没有任何协程堆拆除为该域设界，所以这样的环没有任何机制回收。工具链提供的是观测：开发期检测器在主执行流退出前扫描共享域（点名对象与闭环边；修复是重构回指或在释放前显式断开），`runtime.sharedBytes()` 是廉价的生产信号——熬过通道拆除仍单调上涨的读数就是共享泄漏。当前的带纪律还把已知的源级构造路径全部 fail-closed（图里捕获了自身通道的值，没有显式 transfer 计划就发不出去），实践中要撞上它得绕开不少防线。
...
- **用户可见 introspection**：`runtime.liveBytes()` / `runtime.liveObjects()` / `runtime.info()` 报告当前 coroutine heap（无当前 coroutine 时回退到 main coroutine）的 live-memory 视图；`runtime.sharedBytes()` / `runtime.staticBytes()` 报告协程拆除永远不会设界的那两个域（`import runtime`；`mem` 模块只承载裸内存能力）。

#### 16.3.0 泄漏工程学

纯 RC 模型留下的泄漏面由四条法则覆盖，每条都把一个域和它的观测手段配成对：

1. **长驻程序把每个请求 / 会话 / 阶段跑在协程里，主协程只做编排。** 这样 L2 把每个环都框进构造它的那个工作单元——泄漏随协程消亡。直接建在长命主协程上的环，除进程退出外没有任何上界；消除这种情况的是这条惯用法，不是收集器。
2. **`xray test` 默认开启开发期检测器**：每个协程堆在拆除时扫描，根执行与共享域在退出前扫描；报告出的环使测试非零退出，并点名闭环边。
3. **生产环境盯 `runtime.sharedBytes()`** ——唯一会产生进程级环泄漏的域，恰好就是这个计数计量的域；**`runtime.staticBytes()`** 回答隔壁那个问题（「模块缓存是不是在无界增长」，那是可达性问题，不是环）。
4. **断哪条边是所有权决定，惯用法有三**：`weak` 字段（EXEC_LOCAL 字段环）；`defer holder.field = null`（闭包捕获边，`weak` 够不到）；把回指改成 id、由外部表解析（共享域，`weak` 被禁）。
...
- 对象回收 / RC 归零：回收点虽然精确（§16.8），但不建立跨执行体的顺序关系，因此不能用来承载同步语义。
