...
`Array<u8>` and `Slice<u8>`. Generated fragments receive only `(data,
...
TargetPlan schema 58 additionally freezes the exact zero-argument
...
immutable enum layout ID, discriminant, declaration/member spellings, ordered
nonempty payload field names, namespace receiver identity, and declaration-
ordered payload semantic identities. The analyzer plan is the only owner of
the source-order-to-slot permutation. Target call rows authorize the
constructor and tagged result; CGen only consumes the verified recipe and
cannot infer its type, selector, arity, field order, or representation from Xi.
Direct-local enum arguments may bind distinct owned/borrowed physical
...
TargetPlan function, call, argument, value-representation, actual-slot, and
`CALL_DIRECT_I64` instruction rows. Prepare and verification each perform one
complete TargetPlan verification per module; CGen then uses only an ephemeral
borrowed row view tied to that verified plan and fingerprint. A callee drops its
legacy `XaotFuncAbi` owner only when every inbound edge belongs to this covered
family, and the verifier requires the legacy row to remain zero. An uncovered
module-initializer edge therefore keeps the affected function on the legacy
owner while a covered child call may already use TargetPlan; no name, closure
shape, function body, resolution-only slot, legacy value plan, or fallback may
repair a missing or mutated row. This cutover changes no public calling
convention and grants no aggregate, coroutine, cross-module, dynamic-call, or
whole-scalar ABI authority.

TargetPlan schema 58 freezes the closed `i64.addOverflows`,
...
row mechanically. AOT C emission schema 40 projects every predicate row into
one independently verified `uint8_t` recipe naming exactly one shared
`xr_arith_core_{add,sub,mul}_overflows` helper. Representation refinement keeps
both inputs in `I64`, the predicate result in `I1`, and records no adapter.
CGen consumes the immutable recipe before generic method lowering and consumes
the native predicate directly as a branch condition; it cannot read method
text, dispatch through the runtime method table, box the predicate, or fall
back to a legacy value plan. Missing, duplicate, stale, swapped, or
wrong-helper rows fail closed before C emission.

TargetPlan schema 58 freezes one exact borrowed direct-local
...
schema-58 TargetPlan boundary. Refinement schema 6 records no adapter: it
...
An ordinary multi-module TargetPlan is also one authority, but every semantic
value, function, call argument, cleanup, and recipe remains local to its
canonical module partition. AOT refinement and C-emission construction
therefore require both the verified TargetPlan and the exact owning
SemanticPlan. Their public entry points have no target-only overload or
compatibility facade. Refinement resolves module-local rows through the
partition before binding live Xi values; C-emission schema 40 projects only
that partition and freezes the owning SemanticPlan fingerprint in addition to
the target and profile fingerprints. Passing a foreign module plan, reusing a
local value ID from another partition, or verifying a projection against a
different semantic fingerprint fails closed. CGen selects the projection by
...
dynamic String. Its `Slice<u8>` input may cross a direct-local `READ` boundary
whose source type is top-level `const` and whose parameter type is not, but
only through the const-read ABI judgement: machine kind, size, alignment,
...
module-local and can collide. The
same binding transiently owns the complete covered value and function-ABI rows
for caller, callee, and every module initializer. All four functions use
`TARGET_PLAN` ABI authority and retain fieldwise-zero legacy `XaotFuncAbi`
state; initializer tagged `XrValue` return ABI is projected and independently
verified rather than borrowed from a per-module emission plan.
...
direct-call refinement and program C-emission binding from its one program
TargetPlan before reporting bounded source-AOT `execution=cgen-ready`. CGen mechanically reads
the binding's stable-indexed value, ABI, function, call, and elided-callee
views; it does not rebuild C representation or ABI from machine rows and does
not fall back to per-module C-emission plans. The exact `GET_SHARED` callee
carrier is independently bound to the caller, shared slot, PSC resolver/export
join, and unique call operand-zero use, then omitted from C materialization and
legacy reachability. Required module initializers remain lifecycle roots, but
only the caller is seeded as an ordinary product-body root; the callee survives
solely through the verified direct edge. The kept caller C has
one stable-symbol direct call and no module/name/shared runtime lookup, and the
cold/warm/edit/revert native oracle produces 42/42/43/42. This grants a native
ABI and binary claim only for the exact bounded two-module scalar graph. The
installed public `XrProgram` facade admits only that same exact direct-`i64`
graph through the verified Program TargetPlan and live manifest. It does not
authorize another graph family, dynamic reload, concurrent unload, or general
...
`XaotValuePlan` rows. Its covered direct-local call has no legacy argument or
return boundary steps and cannot return to the old call resolver. CGen consumes
an ephemeral borrowed view of the verified rows to project the exact 16-byte,
8-byte-aligned two-`i64` C value, field offsets 0 and 8, and the direct typed
call. Generated C for the covered functions therefore contains no `XrValue`
boxing, place/aggregate-reference carrier, dynamic selector/index lookup,
entry-cell dispatch, or runtime call. Module initialization, another struct
shape, a nontrivial field, container, coroutine, cross-module call, and every W4
family remain on an explicitly uncovered fail-closed boundary.
...
- T4g: a Windows multi-module freestanding relocatable artifact is one COFF
  ...
  Hosted-fragment context storage belongs to the core AOT runtime independently
  of optional stdlib VM fastpaths. The initializer declaration and definition
  use one linkage decision in both single-unit and multi-unit emission.
...
- T19: `NumberParseError` is the sole scalar-text parse failure ABI. Its
  ...
  namespace rows, generated C, hosted AOT, freestanding AOT, and the VM must
  agree on all three numeric identities before publishing the error. Strings
...
anchor-sha256: src/aot/xaot_prepare.c 2a62a0e378dbf99e262342e90cc3135f45f4b2389823422db7762cb908094560
...
anchor-sha256: src/aot/refine/xr_aot_representation_refinement.c 5e58085cc8dc7eda6accdb8255b11133836180c7a2627db80e11f59a29466729
...
anchor-sha256: src/aot/refine/xr_aot_scalar_value.c fed01c608fca6d84d578aa3bb1165044150fb956c8ff43818212bba09830b678
...
anchor-sha256: tests/unit/aot/test_xr_aot_refinement.c 1404021eb1e2b49805e5d515679bc243998281c987d7a5ec8bd253f2e2268ce5
...
anchor-sha256: src/aot/emit_c/xr_c_emission_plan.c 99e509da6bc433d2901f51da716d35bf1638c167f30acc1e08a88b99a0315575
...
anchor-sha256: src/aot/xr_target_aggregate_c_projection.c 6d97c6678cc4f2b8f9b13ce4922d8259fe6d0fe0df79fa07320bbfcba5963141
...
anchor-sha256: src/plan/semantic/xr_semantic_value_aggregate_shape.h f7cac391767fa760f51f2845affda868fac238601f3c99872ba5c4ae5600c5b1
...
anchor-sha256: src/ir/xi_opt.c 9e331fdf9146f91241f50934b977a8ccb72d93a819c6cd532338599a544ab913
...
anchor-sha256: src/app/cli/xcmd_build.c f872ddcab690111e76d318c8c263ced89956d5323ea1c82c09740788d5304991
...
anchor-sha256: src/stdlib/xstdlib_vm_fastpath_amalgam.c 1e76506a0f62d91bdc6c0237d7537c19dd16a4889b937d2754b0df031cd027c2
anchor-sha256: src/aot/xaot_coro.h 8af75d5a55537dd4302ba1fcc03de3b7739135df7e243b2816c5bb6e4398ee01
