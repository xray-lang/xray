...
and C emission schema 29 binds the same operation and slot to `xrt_release`.
...
The C emission projection schema 29 preserves the exact materialization recipe
...
cleanup authority.
For an exact scalar `XI_CHAN_TRY_RECV`, schema 29 preserves the receiver semantic
...
authority. For the exact String byte-slice intrinsic, schema 29 preserves the
...
For an exact payload-bearing source enum constructor, schema 29 owns the
...
For exact `Array.withCapacity` and `Array.filled` constructors, schema 29 owns
...
For exact scalar `Array.fill(value)`, schema 29 owns the receiver and fill
...
For exact `String.runes()`, schema 29 owns the borrowed String receiver, owned
...
For exact `Iterator<rune>.hasNext()`, schema 29 owns the scalar boolean result,
...
For exact `Iterator<rune>.next()`, schema 29 separately owns the trivial native
...
For exact `rune.toUInt32()`, schema 29 owns the trivial native-u32 result, its
...
For exact `rune.isWhitespace()`, schema 29 owns the trivial native-bool result,
...
For exact `String.slice(start, end)`, schema 29 owns the unique frozen String
...
anchor-sha256: src/aot/emit_c/xr_c_emission_schema.h c4f313632dbe8bca1bf69c48fa27ecd45f11a71b83c881d978287dd0898d82fd
anchor-sha256: src/aot/emit_c/xr_c_emission_plan.c 7770095f1548382221714ff8ba288c4267f4bf28e3fdbd6e7f8549540fda4e69
