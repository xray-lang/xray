...
5. `xr_runtime_generation_activation_available()` means only that the bounded
   ...
   TargetPlan schema 23 preserves SOURCE-namespace, exact ADT-enum, exact
   aggregate field-name, Array-intrinsic, and String-runes result authority in
   the required family mask. Those dynamic rows remain
