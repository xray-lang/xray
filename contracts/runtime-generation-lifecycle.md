...
5. `xr_runtime_generation_activation_available()` means only that the bounded
   ...
   TargetPlan schema 22 preserves SOURCE-namespace, exact ADT-enum, exact
   aggregate field-name, and Array-intrinsic storage authority in the required
   family mask. Those dynamic rows remain
