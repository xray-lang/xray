...
5. `xr_runtime_generation_activation_available()` means only that the bounded
   ...
   TargetPlan schema 27 preserves exact SOURCE-import storage and dense
   ...
   `hasNext`/`next` call authority, plus exact direct-local borrowed `Array`
   parameter storage. Those dynamic rows remain
