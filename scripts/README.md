...
| `scripts/check_object_json_domain.py` | 260：对象/Json 域、公开 `Record` 表面、旧 TypeId、fixed-field Xi op 与 evidence 名称分类 inventory | `--root <repo>`；可选 `--json`、`--fail-on-legacy` | 默认输出可重现 inventory；收敛后 CTest 以 `--fail-on-legacy` 阻止旧对象身份回流 | < 2s |
