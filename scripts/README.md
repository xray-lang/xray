...
| `scripts/t.py`、`scripts/canonical_program_test_profile.py`、`scripts/run_canonical_program_gate.py` | t0–t3完整分层之外提供Task 293的exact `canonical`与`generic-identity`编辑期preflight。Release与ASan读取同一份机器清单；前者覆盖完整canonical边界，后者只覆盖mono、Xglobal、cache payload与source build。两者都先刷新Ninja/CTest manifest，再精确构建所需target，并用JUnit证明实际执行集合；缺项、重复项、额外项、跳过项或命令行过滤均fail closed。`XR_FAST=1`使用source stdlib并关闭VM fastpaths；构建并发由`XR_JOBS`控制，CTest由`XR_CTEST_JOBS`控制且Windows默认上限8；同一build tree由文件锁串行化 | `python scripts/t.py generic-identity`用于泛型身份编辑循环；`python scripts/t.py canonical`用于切片提交门；日常可加`XR_FAST=1`；更窄诊断使用`python scripts/t.py t0 -R '^test_name$'`；exact profile不接受shard | 任一清单、构建或执行证明失败=非0；`generic-identity`和`canonical` PASS都不替代t1–t3、完整backend differential或完整sanitizer | 当前canonical 60项/36个构建目标；generic-identity 4项/4个构建目标。实测generic热树整段约4秒，其中CTest约0.62秒；canonical热树CTest约4.4秒，公共头变更后的构建仍是主要耗时 |
| `scripts/run_asan_focused.py` | 218 防线 2：默认`full`运行全部 C 单测、快速 backend-diff 子集、xxhash 与 bili-analysis-server AOT C发射；`canonical-program`和`generic-identity`是共享清单驱动的exact `PARTIAL` preflight，只增量构建相应目标，不运行diff/workload，也不冒充完整lane。构建固定`XRAY_STDLIB_VM_FASTPATHS=OFF`，`detect_leaks=0`（泄漏归lsan_strict） | env: `XR_ASAN_PROFILE`、`XR_ASAN_JOBS`、`XR_ASAN_BUILD_DIR`、`XR_BUILD_LOCK_TIMEOUT`；full另支持CTest、diff与workload覆盖参数；exact profile拒绝这些inventory覆盖 | full任一必需fixture/workload/测试失败=非0；exact profile的注册或JUnit执行集合漂移=非0 | 当前canonical-program为60项；generic-identity为4项，实测增量构建加执行约32秒。最近一次full为360/390，已知语义RED仍为30项 |
