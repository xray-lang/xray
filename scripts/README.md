...
| `scripts/check_structural_object_json_map_boundary.py` | 静态结构对象、`JSON.Value` 与 Map-backed `JSON.Object` 边界 residue inventory | `--root <repo>`；可选 `--json`、`--fail-on-legacy` | CTest 以 `--fail-on-legacy` 阻止旧 `Json` 表面、row metadata、动态结构对象路径与第二套 JSON object storage 回流 | < 2s |
...
| `scripts/check_contract_freeze.py` | 220：十二份语义契约的 anchor digest 门禁；commit message 无专用 trailer 要求 | `--root <repo>`；注入验证用 `--self-test` | digest 漂移、契约锚点缺失或格式错误=1；dirty/clean tree 使用同一内容门禁 | < 2s |
| `scripts/check_canonical_program_contracts.py` | 294/W0：校验 `XrProgram` 单一程序架构的七份冻结资产、任务组合、旧 owner 迁移、原子 operation walking skeleton 与 forbidden-residue ratchet | `--root <repo>`；mutation 验证用 `--self-test` | schema/交叉引用/规范 JSON/TSV/残留 ceiling 任一不满足=1；通过=0 | < 5s |
| `scripts/check_canonical_cutover_manifests.py` | 293/304 G0：校验现行声明语言能力分母、必需 profile/provider、真实产品入口与旧路径 deletion nodes；阻止合法能力共同拒绝、提前终切或非零删除目标冒充完成 | `--root <repo>`；mutation 验证用 `--self-test` | 缺项、无源码锚点、未知 owner、未闭合能力提前切换或 terminal-zero 矛盾=1；通过=0 | < 5s |
| `tools/corespecgen/corespecgen.py` | 295/W1：校验 `xisa/core` 唯一原子语义 registry，运行 host-independent arithmetic/control KAT oracle，并生成 C metadata、规范表与 consumer coverage inventory | `--root <repo>` 加且仅加一个 `--generate` / `--check` / `--self-test` | schema/ID tombstone/feature/type/effect/ownership/KAT/生成漂移任一错误=1；通过=0 | < 5s |
| `tools/programgen/programgen.py` | 296/W2：校验 `xisa/program` 唯一 XrProgram wire schema，并生成 C 常量与可读格式规范 | `--root <repo>` 加且仅加一个 `--generate` / `--check` / `--self-test` | schema/section ID/encoding/budget/生成漂移任一错误=1；通过=0 | < 5s |
| `scripts/check_xr_program_semantics.py` | 297/W3：校验 bounded semantic admission、opaque `XrValidatedProgram` 与 independent reference evaluator 对 active CoreSpec operation 的完整覆盖 | `--root <repo>`；mutation 验证用 `--self-test` | verifier/evaluator/测试/契约 coverage 漂移或非法依赖回流=1；通过=0 | < 5s |
| `scripts/check_xr_program_aot_native.py` | 300/301：执行 pure native-AOT fixture，并用已验证的跨平台工具读取规范化 defined-symbol inventory；Windows 依次探测 dumpbin、llvm-nm、nm，Linux/macOS 探测 nm、llvm-nm | `--executable <path> --expected-exit <logical-result>` | 结果不符、工具能力缺失、符号输出失败/空/畸形或发现 compiler/VM/backend residue=1；通过=0 | < 5s |
| `scripts/check_xr_program_wave4_exit.py` | 293/301 W7 Wave 4：对 8 个 callable/existential/witness operation 的 source、CoreSpec、verifier、Reference、VM、AOT 证据和新 canonical 路径旧 owner 零残留做可升级退出清单 | `--root <repo>`；`--self-test` 注入变异；`--require-ready` 执行真正退出门 | inventory 在 OPEN 状态如实列 blocker 并返回 0；状态/证据矛盾或残留回流=1；`--require-ready` 有任一 blocker=1 | < 5s |
| `scripts/run_xr_program_wave4_differential.py` | 293/301 W7 Wave 4：退出门变绿后统一运行 source/Reference/VM/native-AOT 聚焦 differential 入口 | `--root <repo> --build-dir <ninja-build>`；可选 `--ctest`；discovery 变异自测用 `--self-test` | 先执行 Wave 4 `--require-ready`；随后通过 CTest JSON discovery 要求预期 8 项集合精确匹配，空集、遗漏、额外项、重复项或任一聚焦 CTest 失败=非0 | 取决于聚焦构建与 native AOT 测试 |
| `scripts/t.py`、`scripts/canonical_program_test_profile.py`、`scripts/run_canonical_program_gate.py` | t0–t3完整分层之外提供Task 293的exact `canonical`编辑期preflight；Release与ASan导入同一份由静态核心门和源码fixture manifest派生的完整清单，只构建清单实际需要的目标（含真实`xray run`产品入口、Xglobal与Xi→Program聚焦证明、runtime-only embedder、泛型实例化、Pipe取消、多挂起点、affine结果和READ existential协程的generated-C原生证据）。选择测试前先刷新Ninja的CMake/CTest manifest，构建后若清单变化则重新计算并构建精确选择集，避免新增测试注册造成假红；清单缺项、改名、必需Ninja目标缺失或被命令行过滤会fail closed。`XR_FAST=1`使用source stdlib并关闭VM fastpaths，从编辑循环中移除两条自举生成边；Windows默认使用独立的`build-fast-clang`和`clang-cl`，避免MSVC大型C11编译器内部错误污染快速树。构建并发由`XR_JOBS`控制；CTest另由`XR_CTEST_JOBS`控制，Windows默认上限8，避免原生符号检查和Python进程过度并发。`t.py`、canonical小型lane和ASan lane通过崩溃安全的OS文件锁独占同一build tree；`XR_BUILD_LOCK_TIMEOUT=0`可立即拒绝忙树，默认等待600秒并报告持有者。会在CTest期再次启动宿主C编译器的allocation、coroutine-graph和provider资格测试共享`xray_native_toolchain`资源锁，避免工具链争用制造假超时。t0只执行12项机器登记的compile-error smoke，完整889项语料仍由t1及以上运行；显式CTest选择器（如`-R`/`-L`/`-I`）只构建和执行被选测试，跳过该tier的辅助语料库并明确报告未覆盖范围；已知RED的terminal readiness/residue门仍由完整资格层执行。小型lane runner也必须使用exact name regex，禁止直接`ctest -L canonical-program`：标签集大于构建目标集时，后者会把未构建的可执行文件报为假红 | `python scripts/t.py canonical`；日常编辑推荐`XR_FAST=1 python scripts/t.py canonical`，聚焦诊断用`XR_FAST=1 python scripts/t.py t0 -R '^test_name$'`；小型lane用`python scripts/run_canonical_program_gate.py --lane semantic|native|asan-source`，Release自动读`XR_BUILD_DIR`/`XR_JOBS`，ASan自动读`XR_ASAN_BUILD_DIR`/`XR_ASAN_JOBS`，显式参数优先；canonical不接受shard | 任一清单项失败、注册集合漂移或构建失败=非0；聚焦PASS仅证明选择集通过；canonical PASS不代表embedded stdlib、fastpaths、t1–t3或完整sanitizer通过 | 当前57项canonical、33个精确构建目标；同一热树26并发CTest为34.69s，默认上限8后为4.39s（无构建整段约6s）；source owner 33项约0.55s；compile-error smoke 12/12约0.18s；5项semantic lane约0.8s；8项native lane约4.7s；ASan source lane约7.7s；CMake变化后的Windows ASan冷构建仍可能重编大量对象 |
| `scripts/run_asan_focused.py` | 218 防线 2：默认`full`运行全部 C 单测、快速 backend-diff 子集、xxhash 与 bili-analysis-server 的全量 AOT C 发射。**一份实现覆盖所有平台**（原 `.sh`/`.ps1`/`_windows.ps1` 三轨已删除）。构建固定`XRAY_STDLIB_VM_FASTPATHS=OFF`，`detect_leaks=0`（泄漏归lsan_strict）。`XR_ASAN_PROFILE=canonical-program`只运行与Release共享的manifest派生exact编辑期preflight、增量构建必需目标并显式报告`PARTIAL`；它不运行diff/workload，也绝不冒充默认完整lane | env: `XR_ASAN_PROFILE`、`XR_ASAN_JOBS`、`XR_ASAN_BUILD_DIR`、`XR_BUILD_LOCK_TIMEOUT`；full另支持`XR_ASAN_CTEST_REGEX`、`XR_ASAN_CTEST_EXCLUDE`、`XR_ASAN_CTEST_SERIAL_REGEX`、`XR_ASAN_DIFF_REGEX`、`XR_ASAN_XXHASH_MAIN`、`XR_ASAN_BILI_MAIN` | full任一必需fixture/workload/测试失败=非0；canonical-program拒绝自定义测试/target regex，注册集合漂移或任一preflight失败=非0 | 当前57项canonical sanitizer preflight通过；最近一次full原始结果611.15s、356/387，额外1项为工具链争用导致的coroutine-graph 60s超时，串行复跑5.53s通过；资源锁已使allocation+coroutine-graph在ASan树9.24s串行通过，完整full重新资格仍待执行，已知语义RED仍为30项 |
| `scripts/run_lsan_strict.py` | 218 防线 4：严格 LeakSanitizer lane——ASan+LSan（`detect_leaks=1`）跑单测面，配 `scripts/lsan.supp`；构建固定 `XRAY_STDLIB_VM_FASTPATHS=OFF`。LSan 仅 Linux 支持，macOS 上明确跳过 | env: `XR_LSAN_JOBS`、`XR_LSAN_BUILD_DIR`、`XR_LSAN_CTEST_REGEX` | 非 Linux=0（跳过）；Linux 有泄漏=非0；普通非 sanitizer 构建的 CTest 常驻 `lsan_strict` | Linux CI 数分钟 |
