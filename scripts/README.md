...
| `scripts/run_asan_focused.py` | 218 防线 2：ASan+UBSan 聚焦面——全部 C 单测、快速 backend-diff 子集、xxhash 与 bili-analysis-server 的全量 AOT C 发射。**一份实现覆盖所有平台**（原 `.sh`/`.ps1`/`_windows.ps1` 三轨已删除：`.ps1` 无任何引用者，且只跑 bili、漏掉 xxhash 与 diff 子集）。构建明确固定 `XRAY_STDLIB_VM_FASTPATHS=OFF`，与统一目标机的唯一受支持自举形态一致，避免 sanitizer 尚未启动就递归进入未资格化的 native fastpath 生成。`detect_leaks=0`（泄漏归 lsan_strict）；构建与否由源码时间戳自行判定——源码新于二进制即重建，否则复用，绝不为陈旧二进制背书 | env: `XR_ASAN_JOBS`、`XR_ASAN_BUILD_DIR`、`XR_ASAN_CTEST_REGEX`、`XR_ASAN_CTEST_EXCLUDE`、`XR_ASAN_CTEST_SERIAL_REGEX`、`XR_ASAN_DIFF_REGEX`、`XR_ASAN_XXHASH_MAIN`、`XR_ASAN_BILI_MAIN` | 必需 bili fixture 或任一已发现 workload/测试失败=非0；xxhash sibling 缺失时明确跳过 | 增量测试面 <10min（全量 ASan 自举另计） |
