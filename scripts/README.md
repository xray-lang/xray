...
非空时返回 1，等 resolver/type-name/docs/spec 清零后可接入 CI。历史 `XI_BYTES_*`、`OP_BYTES_*`
名称应为 0；`xr_array_bytes_*`、CGen bytes helper 等剩余 internal legacy 命名仅进入 inventory，
后续由 204 backend/runtime 收敛切片清理。
