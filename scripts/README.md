...
| `scripts/t.py`、`scripts/canonical_program_test_profile.py` | t0–t3完整分层之外提供Task 293的exact `canonical`编辑期preflight；Release与ASan导入同一34项清单，且只构建11个必需目标（含runtime-only embedder、Pipe取消和多挂起点generated-C原生证据）。清单缺项、改名、必需Ninja目标缺失或被命令行过滤会fail closed；`XR_FAST=1`使用source stdlib并关闭VM fastpaths，从编辑循环中移除两条自举生成边；已知RED的terminal readiness/residue门仍由完整资格层执行 | `python scripts/t.py canonical`；日常编辑推荐`XR_FAST=1 python scripts/t.py canonical`，可用`XR_BUILD_DIR`/`XR_JOBS`，不接受shard | 任一34项失败、注册集合漂移或构建失败=非0；PASS不代表embedded stdlib、fastpaths、t1–t3或完整sanitizer通过 | 热测试实测约4s；CMake变化后的Windows ASan冷构建仍可能重编约457个对象 |
| `scripts/run_asan_focused.py` | 218 防线 2：默认`full`运行全部 C 单测、快速 backend-diff 子集、xxhash 与 bili-analysis-server 的全量 AOT C 发射。**一份实现覆盖所有平台**（原 `.sh`/`.ps1`/`_windows.ps1` 三轨已删除）。构建固定`XRAY_STDLIB_VM_FASTPATHS=OFF`，`detect_leaks=0`（泄漏归lsan_strict）。`XR_ASAN_PROFILE=canonical-program`只运行与Release共享的exact 34项编辑期preflight、增量构建11个目标并显式报告`PARTIAL`；它不运行diff/workload，也绝不冒充默认完整lane | env: `XR_ASAN_PROFILE`、`XR_ASAN_JOBS`、`XR_ASAN_BUILD_DIR`；full另支持`XR_ASAN_CTEST_REGEX`、`XR_ASAN_CTEST_EXCLUDE`、`XR_ASAN_CTEST_SERIAL_REGEX`、`XR_ASAN_DIFF_REGEX`、`XR_ASAN_XXHASH_MAIN`、`XR_ASAN_BILI_MAIN` | full任一必需fixture/workload/测试失败=非0；canonical-program拒绝自定义测试/target regex，注册集合漂移或任一preflight失败=非0 | canonical热运行实测约8s；full增量面<10min |
