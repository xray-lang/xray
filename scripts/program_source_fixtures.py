...
class RegistryTests(unittest.TestCase):
    ...
    def test_live_registry_and_exact_projection_are_complete(self):
        ...
        self.assertEqual(len(registry["cases"]), 22)
        self.assertEqual(len(fixture_records(registry)), 14)
        header, cmake = project_registry(registry)
        self.assertEqual(header.count(b"    X(source_owner_"), 22)
        self.assertEqual(cmake.count(b"add_xr_program_source_native_fixture("), 14)
