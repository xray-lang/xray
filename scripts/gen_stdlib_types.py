...
KEY CONCEPT:
  Uses stdlib/defs/*.def plus explicitly marked stdlib/types/*.xr compiler
  schemas as the analyzer source for in-tree native and intrinsic modules.
  Exported declarations in pure-Xray stdlib modules are merged only into the
  LSP stdlib table so editor metadata follows migrated modules without
...
def return_ownership_contract_init(member):
...
def main():
    ...
    # --embed mode (default): generate in-tree stdlib metadata from declarations.
    print("Loading stdlib definitions and compiler schemas...", file=sys.stderr)
    ...
    print(f"Declarative object shapes: {object_shape_added} object shapes loaded", file=sys.stderr)
    print(f"Declarative enums: {enum_added} enums loaded", file=sys.stderr)
    print(f"Def classes: {class_added} native classes loaded from stdlib/defs/*.def", file=sys.stderr)
    print(f"Declarative type methods: {type_method_added} methods loaded "
          f"({type_method_replaced} replaced)", file=sys.stderr)
