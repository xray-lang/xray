...
def scan_file(filepath):
    ...
    result = {
        ...
        'methods': [],      # XR_DEFINE_BUILTIN entries
    }
...
def generate_header(type_results, module_results):
    ...
    if module_results:
        ...
        for mod_name, mod_data in sorted(module_results.items()):
            ...
            if mod_data.get('methods'):
                lines.append(f"// {mod_name} module functions")
                lines.append(f"static const XaBuiltinMember g_gen_{mod_name}_functions[] = {{")
                for m in mod_data['methods']:
                    is_method = "true" if '(' in m['signature'] else "false"
                    lines.append(f'    {{"{m["name"]}", "{m["signature"]}", "{m["doc"]}", {is_method}, false}},')
                lines.append("};")
                lines.append(f"#define GEN_{mod_name.upper()}_FUNCTION_COUNT {len(mod_data['methods'])}")
                lines.append("")
        ...
        for mod_name, mod_data in sorted(module_results.items()):
            func_ref = f"g_gen_{mod_name}_functions" if mod_data.get('methods') else "NULL"
            func_count = f"GEN_{mod_name.upper()}_FUNCTION_COUNT" if mod_data.get('methods') else "0"
...
def main():
    ...
    if len(args) >= 2 and args[0] == '--xrd':
        ...
        if not result:
            print(f"Error: No XR_DEFINE_BUILTIN found in {filepath}", file=sys.stderr)
            return 1
    ...
    print(f"\nTotal: {total_methods} type methods across {total_types} types, "
          f"{total_functions} module functions across {total_modules} modules", file=sys.stderr)
