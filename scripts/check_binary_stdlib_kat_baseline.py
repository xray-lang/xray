...
BASELINES = (
    ...
    Baseline(
        ...
        contains=(
            'assertEqual(hexEncodeText("Hello"), "48656c6c6f")',
            'assertEqual(hexDecodeText("48656c6c6f"), "Hello")',
            'assertEqual(hexDecodeFails("gg"), true)',
            'assertEqual(hexDecodeFails("abc"), true)',
    ...
    Baseline(
        ...
        path="tests/regression/10_stdlib/1300_encoding.xr",
        contains=(
            'assertEqual(utf8ValidText("Hello"), true)',
            'assertEqual(utf8ValidText("你好"), true)',
            "overlong[0] = 0xC0",
            "surrogate[0] = 0xED",
    ...
    Baseline(
        ...
        contains=(
            'var kat = "123456789"',
            "compress.crc32(kat.bytes())",
            "assertEqual(crc, 3421780262)",
            'var kat = "Wikipedia"',
            "compress.adler32(kat.bytes())",
            "assertEqual(adler, 300286872)",
            "compress.gzip(data.bytes())",
            "compress.deflate(data.bytes())",
            "compress.zlibCompress(data.bytes())",
        ),
    ),
    Baseline(
