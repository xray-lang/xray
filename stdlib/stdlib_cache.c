...
static const XrStdlibObjectShapeDefEntry *stdlib_object_shape_decl_find(const char *module, const char *name) {
    if (!module || !name)
        return NULL;
...
XR_FUNC XrClass *xr_stdlib_object_shape_class_get(XrVMRuntime *isolate, const char *module,
    ...
    if (cache->native_object_shape_count == cache->native_object_shape_capacity) {
        ...
        XrStdlibNativeObjectShapeCacheEntry *next =
            (XrStdlibNativeObjectShapeCacheEntry *) xr_realloc(entries, next_capacity * sizeof(*next));
        if (!next)
            return NULL;
        entries = next;
