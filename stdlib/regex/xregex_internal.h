...
static inline int xr_re_utf8_charlen(const char *p, const char *end) {
    ...
    uint8_t b = (uint8_t)*p;
    if (b < 0x80) return 1;
    if ((b & 0xE0) == 0xC0 && p + 1 < end) return 2;
    if ((b & 0xF0) == 0xE0 && p + 2 < end) return 3;
    if ((b & 0xF8) == 0xF0 && p + 3 < end) return 4;
    return 1;
}
...
static inline bool xr_sparse_set_contains(XrSparseSet *s, int i) {
    return (unsigned)i < (unsigned)s->capacity &&
           (unsigned)s->sparse[i] < (unsigned)s->size &&
           s->dense[s->sparse[i]] == i;
}

static inline bool xr_sparse_set_insert(XrSparseSet *s, int i) {
    if (xr_sparse_set_contains(s, i)) return false;
    s->sparse[i] = s->size;
    s->dense[s->size++] = i;
    return true;
}
...
bool xr_nfa_search(XrProg *prog, const char *text, int len,
                   const char **captures, int ncaptures);
bool xr_dfa_match(XrDFA *dfa, const char *text, int len,
                  const char **match_end, bool anchored);
