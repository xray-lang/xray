...
static XrRegexFlags parse_flags(const char *flags_str) {
    ...
    for (const char *p = flags_str; *p; p++) {
        switch (*p) {
            ...
            case 'm': flags |= XR_RE_MULTILINE; break;
            case 's': flags |= XR_RE_DOTALL; break;
        }
    }
    return flags;
}
...
static XrRegexObject* regex_object_new(XrayIsolate *isolate, XrRegex *re) {
    XrRegexObject *obj = (XrRegexObject*)xr_gc_alloc(&isolate->gc, sizeof(XrRegexObject), XR_TREGEX);
    obj->regex = re;
    return obj;
}
...
static XrRegex* unwrap_regex(XrayIsolate *isolate, XrValue v) {
    (void)isolate;
    if (!XR_IS_REGEX_OBJ(v)) return NULL;
...
static XrValue regex_compile(XrayIsolate *isolate, XrValue *args, int argc) {
    ...
    if (!re) {
        xr_runtime_error(isolate, "regex.compile: %s in pattern '%s'",
...
XR_DEFINE_BUILTIN(regex_compile, "compile", "(pattern: string, flags?: string): any", "Compile a regex pattern")
XR_DEFINE_BUILTIN(regex_test, "test", "(pattern: any, s: string): bool", "Test if pattern matches string")
XR_DEFINE_BUILTIN(regex_find, "find", "(pattern: any, s: string, offset?: int): Json?", "Find first match")
XR_DEFINE_BUILTIN(regex_full_match, "fullFind", "(pattern: any, s: string): Json?", "Full match with captures")
XR_DEFINE_BUILTIN(regex_count, "count", "(pattern: any, s: string): int", "Count matches")
XR_DEFINE_BUILTIN(regex_find_all, "findAll", "(pattern: any, s: string): Array<string>", "Find all matches")
XR_DEFINE_BUILTIN(regex_replace, "replace", "(pattern: any, s: string, replacement: string): string", "Replace first match")
XR_DEFINE_BUILTIN(regex_replace_all, "replaceAll", "(pattern: any, s: string, replacement: string): string", "Replace all matches")
XR_DEFINE_BUILTIN(regex_split, "split", "(pattern: any, s: string): Array<string>", "Split by pattern")
