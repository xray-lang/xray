...
static void ast_cc_add_range(XrArena *arena, XrAstNode *node, uint8_t lo, uint8_t hi) {
    ...
    if (node->cc.count >= node->cc.cap) {
        ...
        XrCharRange *new_ranges =
            (XrCharRange *) xr_arena_alloc(arena, new_cap * sizeof(XrCharRange));
        if (node->cc.ranges) {
            memcpy(new_ranges, node->cc.ranges, node->cc.count * sizeof(XrCharRange));
        }
...
static void ast_list_add(XrArena *arena, XrAstNode *node, XrAstNode *child) {
    ...
    if (node->list.count >= node->list.cap) {
        ...
        XrAstNode **new_children =
            (XrAstNode **) xr_arena_alloc(arena, new_cap * sizeof(XrAstNode *));
        if (node->list.children) {
            memcpy(new_children, node->list.children, node->list.count * sizeof(XrAstNode *));
        }
...
static XrAstNode *parse_escape(XrParser *p) {
    ...
    switch (c) {
        // Simple escapes
        case 'n':
            return ast_literal_char(arena, '\n');
        case 't':
            return ast_literal_char(arena, '\t');
        case 'r':
            return ast_literal_char(arena, '\r');
        case 'f':
            return ast_literal_char(arena, '\f');
        case 'v':
            return ast_literal_char(arena, '\v');
        case 'a':
            return ast_literal_char(arena, '\a');
        case '0':
            return ast_literal_char(arena, '\0');
        ...
        // Assertions
        case 'A':
            return ast_empty_match(arena, XR_EMPTY_BEGIN_TEXT);
        case 'z':
            return ast_empty_match(arena, XR_EMPTY_END_TEXT);
        case 'b':
            return ast_empty_match(arena, XR_EMPTY_WORD_BOUNDARY);
        case 'B':
            return ast_empty_match(arena, XR_EMPTY_NOT_WORD_BOUND);
