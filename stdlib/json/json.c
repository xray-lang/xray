...
static void writer_init(JsonWriter *w, XrayIsolate *isolate, int indent) {
    w->isolate = isolate;
    w->cap = 1024;
...
static void writer_newline(JsonWriter *w) {
    if (w->indent > 0) {
        int pad = w->depth * w->indent;
        // +1 for '\n'
        size_t needed = 1 + pad;
...
static XrValue json_stringify(XrayIsolate *X, XrValue *args, int argc) {
    ...
    XrString *str = xr_string_new(X, writer.buf, writer.len);
    free(writer.buf);
    
    return xr_string_value(str);
}

// Public API: serialize XrValue to JSON C-string (caller must free)
char* xr_json_stringify_to_cstr(XrayIsolate *X, XrValue val, size_t *out_len) {
    ...
    if (out_len) *out_len = writer.len - 1;  // exclude null terminator
    return writer.buf;
}
