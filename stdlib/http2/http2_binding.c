...
static XrHttp2Context *http2_get_context(XrVMRuntime *X) {
    ...
    XrModule *module = registry && registry->loaded_modules
                           ...
                           : NULL;
    return module ? (XrHttp2Context *) module->native_handle : NULL;
}
...
XR_FUNC XrModule *xr_native_module_create_http2(XrVMRuntime *isolate) {
    ...
    if (!module)
        return NULL;
    /* Allocated up front rather than on first use: two coroutines reaching a
     * lazy initialiser would each build a table, and one would be lost along
     * with every connection registered in it. */
    XrHttp2Context *ctx = (XrHttp2Context *) xr_calloc(1, sizeof(XrHttp2Context));
    if (!ctx)
        return NULL;
    ctx->free_head = -1;
