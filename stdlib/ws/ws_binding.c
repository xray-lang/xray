...
static void ws_prof_dump(void) {
    ...
    fprintf(stderr, "recv: fast=%llu slow=%llu (fast%%=%.1f%%)\n",
            (unsigned long long)rf, (unsigned long long)rs,
            total ? 100.0 * rf / total : 0.0);
    fprintf(stderr, "send: fast=%llu slow=%llu (fast%%=%.1f%%)\n",
            (unsigned long long)sf, (unsigned long long)ss,
            (sf+ss) ? 100.0 * sf / (sf+ss) : 0.0);
...
static const char* get_data_arg(XrValue v, size_t *out_len) {
    ...
    if (XR_IS_ARRAY(v)) {
        ...
        if (arr->elem_type == XR_ELEM_U8) {
            if (out_len) *out_len = (size_t)arr->length;
...
static inline XrWebSocket* get_ws_from_ctx(XrWsContext *ctx, int id) {
    if (!ctx || id < 1 || id >= ctx->array_capacity) return NULL;
    return ctx->conn_array[id];
}

static void remove_ws(XrWsContext *ctx, int id) {
    if (!ctx || id < 1 || id >= ctx->array_capacity) return;
    pthread_mutex_lock(&ctx->conn_mutex);
...
static XrValue ws_connect(XrayIsolate *X, XrValue *args, int argc) {
    ...
    if (argc >= 2 && XR_IS_PTR(args[1])) {
        ...
        if (opts) {
            ...
            if (XR_IS_INT(v)) config.connect_timeout_ms = (int)XR_TO_INT(v);
            v = xr_json_get_by_key(X, opts, "pingInterval");
            if (XR_IS_INT(v)) config.ping_interval_ms = (int)XR_TO_INT(v);
            v = xr_json_get_by_key(X, opts, "pongTimeout");
            if (XR_IS_INT(v)) config.pong_timeout_ms = (int)XR_TO_INT(v);
 ...
 */
static XrCFuncResult ws_recvdata_continue(XrayIsolate *X, int status, void *cont_ctx, XrValue *result) {
    WsRecvState *state = (WsRecvState *)cont_ctx;
    ...
    return xr_yield_for_io(X, ws->fd, XR_WAIT_READ, state->timeout_ms,
                           ws_recvdata_continue, state, result);
}
...
static XrCFuncResult ws_recvdata(XrayIsolate *X, XrValue *args, int argc, XrValue *result) {
    ...
    if (!need_more) { *result = xr_null(); return XR_CFUNC_DONE; }
    ...
    return xr_yield_for_io(X, ws->fd, XR_WAIT_READ, timeout_ms,
                           ws_recvdata_continue, state, result);
}
...
static XrCFuncResult ws_echo_conn_upgrade_cont(XrayIsolate *X, int status,
                                                void *ctx, XrValue *result);
static XrCFuncResult ws_echo_conn_loop(XrayIsolate *X, int status,
                                        void *ctx, XrValue *result);
static XrCFuncResult ws_echo_listen_cont(XrayIsolate *X, int status,
                                          void *ctx, XrValue *result);
 ...
 */
static XrCFuncResult ws_echo_conn_init(XrayIsolate *X, XrValue *args,
...
static XrCFuncResult ws_echo_conn_upgrade_cont(XrayIsolate *X, int status,
    ...
    if (status != XR_RESUME_IO_READY) goto fail;

    for (;;) {
        if (ctx->upgrade_buf_used >= WS_UPGRADE_BUF_SIZE - 1) goto fail;
        ...
        if (n == 0) goto fail;
        if (errno != EAGAIN && errno != EWOULDBLOCK) goto fail;
        return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, 5000,
                               ws_echo_conn_upgrade_cont, ctx, result);
    }
...
cleanup:
    {
        ...
        if (rt) {
            ...
            if (pd) xr_netpoll_close(&rt->netpoll, pd);
        }
    }
    close(ctx->fd);
    xr_free(ctx);
...
static XrCFuncResult ws_echo_conn_loop(XrayIsolate *X, int status,
                                        void *user_ctx, XrValue *result) {
    WsEchoConnCtx *ctx = (WsEchoConnCtx *)user_ctx;
    if (status != XR_RESUME_IO_READY) goto cleanup;
    ...
    while (ctx->ws->state == WS_STATE_OPEN) {
        ...
        if (msg) {
            ...
            if (ret == -2) {
                ...
                return xr_yield_for_io(X, ctx->ws->fd, XR_WAIT_WRITE, 5000,
                                       ws_echo_conn_loop, ctx, result);
            }
            if (err || ret == -1) break;
            continue;
        }
        ...
        return xr_yield_for_io(X, ctx->ws->fd, XR_WAIT_READ, -1,
                               ws_echo_conn_loop, ctx, result);
    }
...
cleanup:
    if (ctx->ws) {
        if (ctx->ws->cached_pd && ctx->runtime) {
            xr_netpoll_close(&ctx->runtime->netpoll,
                             (XrPollDesc *)ctx->ws->cached_pd);
...
static XrCFuncResult ws_conn_upgrade_cont(XrayIsolate *X, int status,
                                           void *ctx, XrValue *result);
static XrCFuncResult ws_conn_handler_done(XrayIsolate *X, int status,
                                           void *ctx, XrValue *result);
static XrCFuncResult ws_serve_listen_cont(XrayIsolate *X, int status,
                                           void *ctx, XrValue *result);
 ...
 */
static XrCFuncResult ws_conn_init(XrayIsolate *X, XrValue *args,
                                    int argc, XrValue *result) {
    (void)result;
...
static XrCFuncResult ws_conn_upgrade_cont(XrayIsolate *X, int status,
    ...
    for (;;) {
        ...
        if (errno != EAGAIN && errno != EWOULDBLOCK) goto fail;
        return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, 5000,
                               ws_conn_upgrade_cont, ctx, result);
    }
    ...
    // Upgrade to WebSocket
    {
        ...
        return xr_yield_call_closure(X, ctx->handler, &ctx->conn, 1,
                                     ws_conn_handler_done, ctx, result);
    }
        ...
        if (XR_IS_INT(id_val)) {
            ...
            if (w) {
                if (w->fd >= 0 && ctx->runtime) {
                    ...
                    if (pd) xr_netpoll_close(&ctx->runtime->netpoll, pd);
                }
 ...
 */
static XrCFuncResult ws_serve_listen_init(XrayIsolate *X, XrValue *args,
                                           int argc, XrValue *result) {
    (void)result;
    ...
    return xr_yield_for_io(X, ctx->listen_fd, XR_WAIT_READ, -1,
                           ws_serve_listen_cont, ctx, result);
}
...
static XrCFuncResult ws_serve_listen_cont(XrayIsolate *X, int status,
    ...
    for (;;) {
        ...
        XrCoroutine *coro = xr_coro_create_cfunc(
            X, ws_conn_init, conn_args, 2, "ws.conn");
        if (coro) {
...
static XrCFuncResult ws_serve_wait_cont(XrayIsolate *X, int status,
 ...
 */
static XrCFuncResult ws_echo_listen_init(XrayIsolate *X, XrValue *args,
                                          int argc, XrValue *result) {
    (void)result;
    if (argc < 1 || !XR_IS_INT(args[0])) return XR_CFUNC_DONE;
    ...
    // Yield for first accept readiness
    return xr_yield_for_io(X, ctx->listen_fd, XR_WAIT_READ, -1,
                           ws_echo_listen_cont, ctx, result);
}
...
static XrCFuncResult ws_echo_listen_cont(XrayIsolate *X, int status,
    ...
    for (;;) {
        ...
        XrCoroutine *coro = xr_coro_create_cfunc(
            X, ws_echo_conn_init, conn_args, 1, "ws.echo");
        if (coro) {
            xr_coro_spawn(X, coro);
        } else {
            close(client_fd);
...
static XrCFuncResult ws_echo_serve_yieldable(XrayIsolate *X, XrValue *args, int argc,
    ...
    XrWsContext *ctx = get_ws_context(X);
    if (!ctx) { *result = xr_bool(false); return XR_CFUNC_DONE; }
    ...
    XrCoroutine *listen_coro = xr_coro_create_cfunc(
        X, ws_echo_listen_init, listen_args, 1, "ws.echo.listen");
    if (!listen_coro) {
        close(listen_fd);
        ctx->listen_fd = -1;
        ctx->server_running = false;
 ...
 */
static XrCFuncResult ws_serve_yieldable(XrayIsolate *X, XrValue *args, int argc,
                                         XrValue *result) {
    if (argc < 2) {
        *result = xr_bool(false);
        return XR_CFUNC_DONE;
    ...
    XrCoroutine *listen_coro = xr_coro_create_cfunc(
        X, ws_serve_listen_init, listen_args, 2, "ws.listen");
    if (!listen_coro) {
        close(listen_fd);
        ctx->listen_fd = -1;
        ctx->server_running = false;
...
static XrValue ws_stop_server(XrayIsolate *X, XrValue *args, int argc) {
    (void)args; (void)argc;
    XrWsContext *ctx = get_ws_context(X);
    if (!ctx || !ctx->server_running) return xr_null();
 ...
 */
XrValue xr_ws_upgrade_and_wrap(XrayIsolate *X, int fd,
...
XR_DEFINE_BUILTIN(ws_connect, "connect",
    "(url: string, options?: Json): WsConn?",
    "Connect to a WebSocket server")
XR_DEFINE_BUILTIN(ws_send_yieldable, "send",
    "(conn: WsConn, data: string, binary?: bool): bool",
    "Send data over WebSocket connection")
XR_DEFINE_BUILTIN(ws_recv_yieldable, "recv",
    "(conn: WsConn, timeout?: int): WsMessage?",
    "Receive data from WebSocket connection")
XR_DEFINE_BUILTIN(ws_close, "close",
    "(conn: WsConn, code?: int, reason?: string?): bool",
    "Close a WebSocket connection")
XR_DEFINE_BUILTIN(ws_ping, "ping",
    "(conn: WsConn): bool",
    "Send a ping frame")
XR_DEFINE_BUILTIN(ws_state, "state",
    "(conn: WsConn): string",
    "Get connection state")
XR_DEFINE_BUILTIN(ws_is_open, "isOpen",
    "(conn: WsConn): bool",
    "Check if connection is open")
XR_DEFINE_BUILTIN(ws_recvdata, "recvData",
    "(conn: WsConn, timeout?: int): string?",
