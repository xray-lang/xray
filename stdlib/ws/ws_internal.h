...
/*
 * Internal ownership flags, packed into a single byte so callers inside the WS
 * native layer do not accidentally expose lifetime controls as public API.
 */
...
typedef struct XrWsMessage {
    ...
    uint8_t _flags;     // Internal ownership bits
} XrWsMessage;
 ...
 * MUST be called before ws_connect_start / ws_conn_send_frame_try / ws_conn_recv_try.
