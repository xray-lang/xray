...
static int uint_to_buf(char *buf, size_t n) {
    ...
    while (n > 0) { tmp[len++] = '0' + (char)(n % 10); n /= 10; }
 ...
 */
static int build_response_header(char *buf, int status, const char *content_type,
                                  size_t body_len) {
...
static HttpRawResponse format_response_raw(int status, const char *content_type,
    ...
    if (body_len > 0 && body) memcpy(resp.data + header_len, body, body_len);
    return resp;
}

// Arena-based format_response: allocates from per-request arena (no free needed)
static HttpRawResponse format_response_arena(XrArena *arena, int status,
...
static HttpRawResponse process_handler_result_raw(XrayIsolate *X, XrValue result) {
    ...
    if (XR_IS_PTR(result) && XR_GC_GET_TYPE((XrGCHeader *)XR_TO_PTR(result)) == XR_TARRAY) {
        ...
        if (arr_len >= 1 && data) {
            XrValue v0 = data[0];
            if (XR_IS_INT(v0)) status = (int)XR_TO_INT(v0);
        }
        if (arr_len >= 2 && data) {
            XrValue v1 = data[1];
            if (XR_IS_STRING(v1)) {
...
static XrCFuncResult http_conn_loop(XrayIsolate *X, int status,
                                     void *ctx, XrValue *result);
static XrCFuncResult http_conn_write_cont(XrayIsolate *X, int status,
                                           void *ctx, XrValue *result);
static XrCFuncResult http_conn_handler_done(XrayIsolate *X, int status,
                                             void *ctx, XrValue *result);
static XrCFuncResult http_conn_ws_handler_done(XrayIsolate *X, int status,
                                                void *ctx, XrValue *result);
static XrCFuncResult http_conn_read_body(XrayIsolate *X, int status,
                                          void *ctx, XrValue *result);
static XrCFuncResult http_conn_read_chunked(XrayIsolate *X, int status,
                                             void *ctx, XrValue *result);
static XrCFuncResult http_conn_read_large_body(XrayIsolate *X, int status,
                                                void *ctx, XrValue *result);
static XrCFuncResult http_listen_cont(XrayIsolate *X, int status,
                                       void *ctx, XrValue *result);
...
static XrCFuncResult http_conn_write_cont(XrayIsolate *X, int status,
    ...
    if (status != XR_RESUME_IO_READY) return http_conn_cleanup(ctx);

    while (ctx->write_remaining > 0) {
        ssize_t n = write(ctx->fd, ctx->write_ptr, ctx->write_remaining);
        if (n > 0) { ctx->write_ptr += n; ctx->write_remaining -= n; continue; }
        if (n == 0) return http_conn_cleanup(ctx);
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            return xr_yield_for_io(X, ctx->fd, XR_WAIT_WRITE, -1,
                                   http_conn_write_cont, ctx, result);
        }
...
static XrCFuncResult http_conn_start_write(XrayIsolate *X, HttpConnCtx *ctx,
                                            const char *data, size_t len,
                                            XrContinuation done_cont,
                                            XrValue *result) {
    ctx->write_ptr = data;
 ...
 */
static XrCFuncResult handle_dynamic_route(XrayIsolate *X, HttpConnCtx *ctx,
                                           XrValue *result) {
    ...
    int parsed = xr_http_parse_request_ex(
        ctx->read_buf, ctx->buf_used, &method_str, &method_len,
    ...
    for (size_t i = 0; i < path_len; i++) {
        if (path_str[i] == '?') { pure_path_len = i; break; }
    }
    ...
    XrRouteHandler found_handler = xr_router_find(
        ctx->router, method, path_str, pure_path_len,
        &params, &user_data, &static_resp, &static_len,
        &prebuilt_resp, &prebuilt_len);

    if (prebuilt_resp && prebuilt_len > 0) {
        return http_conn_start_write(X, ctx, prebuilt_resp, prebuilt_len,
                                     http_conn_loop, result);
    }

    if (static_resp && static_len > 0) {
        HttpRawResponse r = format_response_arena(&ctx->req_arena, 200, NULL,
                                                   static_resp, static_len);
        return http_conn_start_write(X, ctx, r.data, r.len,
                                     http_conn_loop, result);
    }
    ...
    if (found_handler == XR_ROUTE_HANDLER_WEBSOCKET && user_data) {
        ...
        return xr_yield_call_closure(X, ws_handler, &ws_conn, 1,
                                     http_conn_ws_handler_done, ctx, result);
    }
    ...
    if (user_data) {
        ...
        char mbuf[16] = {0};
        if (method_len < sizeof(mbuf)) memcpy(mbuf, method_str, method_len);
        ...
        if (is_chunked && parsed > 0) {
            ...
            if (total_needed <= CONN_READ_BUF_SIZE) {
                ...
                if (ctx->body_buf) {
                    int body_in_buf = ctx->buf_used - parsed;
                    if (body_in_buf > ctx->content_length) body_in_buf = ctx->content_length;
                    if (body_in_buf > 0) memcpy(ctx->body_buf, ctx->read_buf + parsed, body_in_buf);
            ...
            xr_json_set_by_key(X, req, "contentLength",
                                XR_FROM_INT((int64_t)ctx->content_length));
        ...
        if (params.count > 0) {
            ...
            for (int i = 0; i < params.count && i < XR_ROUTER_MAX_PARAMS; i++) {
                if (params.params[i].key && params.params[i].value) {
                    ...
                    if (kbl >= sizeof(kb)) kbl = sizeof(kb) - 1;
                    memcpy(kb, params.params[i].key, kbl);
                    kb[kbl] = '\0';
                    xr_json_set_by_key(X, params_obj, kb,
        ...
        return xr_yield_call_closure(X, ctx->handler, &req_val, 1,
                                     http_conn_handler_done, ctx, result);
    }

    // No route matched
    return http_conn_start_write(X, ctx, RESP_404, sizeof(RESP_404) - 1,
                                 http_conn_loop, result);
}
...
static XrCFuncResult http_conn_read_body(XrayIsolate *X, int status,
    ...
    while (ctx->buf_used < ctx->body_total) {
        if (ctx->buf_used >= CONN_READ_BUF_SIZE) break;
        ssize_t n = read(ctx->fd, ctx->read_buf + ctx->buf_used,
                         CONN_READ_BUF_SIZE - ctx->buf_used);
        if (n > 0) { ctx->buf_used += (int)n; continue; }
        if (n == 0) return http_conn_cleanup(ctx);
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, -1,
                                   http_conn_read_body, ctx, result);
        }
        return http_conn_cleanup(ctx);
    }
...
static XrCFuncResult http_conn_read_chunked(XrayIsolate *X, int status,
                                             void *user_ctx, XrValue *result) {
    HttpConnCtx *ctx = (HttpConnCtx *)user_ctx;
    if (status != XR_RESUME_IO_READY) goto chunk_error;

    for (;;) {
        size_t decode_sz = ctx->chunk_buf->size;
        if (decode_sz > 0) {
            ssize_t rc = xr_http_decode_chunked(&ctx->decoder,
                                                ctx->chunk_buf->bytes, &decode_sz);
            ctx->chunk_buf->size = decode_sz;
            if (rc >= 0) goto chunk_done;
            if (rc == -1) goto chunk_error;
        }
        if (ctx->chunk_buf->size > (size_t)MAX_BODY_SIZE) goto chunk_error;

        char *wp = xr_netbuf_reserve(ctx->chunk_buf, 4096);
        if (!wp) goto chunk_error;
        ssize_t n = read(ctx->fd, wp, 4096);
        if (n > 0) { xr_netbuf_advance(ctx->chunk_buf, n); continue; }
        if (n == 0) goto chunk_error;
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, -1,
                                   http_conn_read_chunked, ctx, result);
        }
        goto chunk_error;
    }
...
chunk_error:
    if (ctx->chunk_buf) { xr_netbuf_release(ctx->chunk_buf); ctx->chunk_buf = NULL; }
    return http_conn_start_write(X, ctx, RESP_400, sizeof(RESP_400) - 1,
                                 (XrContinuation)http_conn_cleanup, result);
}
...
static XrCFuncResult http_conn_read_large_body(XrayIsolate *X, int status,
    ...
    while (ctx->body_read < ctx->body_total) {
        ssize_t n = read(ctx->fd, ctx->body_buf + ctx->body_read,
                         ctx->body_total - ctx->body_read);
        if (n > 0) { ctx->body_read += (int)n; continue; }
        if (n == 0) break;
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, -1,
                                   http_conn_read_large_body, ctx, result);
        }
        break;
    }
    if (ctx->body_read >= ctx->body_total) {
...
static XrCFuncResult http_conn_ws_handler_done(XrayIsolate *X, int status,
                                                void *user_ctx, XrValue *result) {
    (void)X; (void)status; (void)result;
    HttpConnCtx *ctx = (HttpConnCtx *)user_ctx;
    return http_conn_cleanup(ctx);
}
 ...
 */
static XrCFuncResult http_conn_init(XrayIsolate *X, XrValue *args,
...
static XrCFuncResult http_conn_loop(XrayIsolate *X, int status,
    ...
    for (;;) {
        ...
        if (n == 0) return http_conn_cleanup(ctx);
        if (errno != EAGAIN && errno != EWOULDBLOCK) return http_conn_cleanup(ctx);

        ctx->batch_count = 0;
        int64_t timeout = (ctx->request_count == 0) ? ctx->read_timeout_ms : -1;
        return xr_yield_for_io(X, ctx->fd, XR_WAIT_READ, timeout,
                               http_conn_loop, ctx, result);
    }
...
dispatch:
    ...
    // Try prebuilt route (zero-alloc fast path)
    {
        ...
        size_t resp_len = 0;
        if (xr_http_try_prebuilt(ctx->router, ctx->read_buf, ctx->buf_used,
                                  &resp, &resp_len)) {
            ctx->request_count++;
            ctx->batch_count++;
            if (ctx->http_ctx) atomic_fetch_add(&ctx->http_ctx->total_requests, 1);
            return http_conn_start_write(X, ctx, resp, resp_len,
                                         http_conn_loop, result);
        }
    }
 ...
 */
static XrCFuncResult http_listen_init(XrayIsolate *X, XrValue *args,
                                       int argc, XrValue *result) {
    (void)result;
    if (argc < 2 || !XR_IS_INT(args[0])) return XR_CFUNC_DONE;
    ...
    return xr_yield_for_io(X, ctx->listen_fd, XR_WAIT_READ, -1,
                           http_listen_cont, ctx, result);
}
...
static XrCFuncResult http_listen_cont(XrayIsolate *X, int status,
    ...
    for (;;) {
        ...
        XrCoroutine *coro = xr_coro_create_cfunc(
            X, http_conn_init, conn_args, 2, "http.conn");
        if (coro) {
    ...
    return xr_yield_for_io(X, lctx->listen_fd, XR_WAIT_READ, -1,
                           http_listen_cont, lctx, result);
}
...
static XrCFuncResult http_listen_wait_cont(XrayIsolate *X, int status,
                                            void *ctx, XrValue *result) {
    (void)status;
...
XrCFuncResult xr_http_listen_impl(XrayIsolate *X, XrValue *args, int nargs, XrValue *result) {
    ...
    XrCoroutine *listen_coro = xr_coro_create_cfunc(
        X, http_listen_init, listen_args, 2, "http.listen");
    if (!listen_coro) {
        close(listen_fd);
        *result = xr_bool(false);
        return XR_CFUNC_DONE;
...
XrValue xr_http_config_impl(XrayIsolate *X, XrValue *args, int argc) {
    ...
    if (!xr_value_is_json(args[0])) return xr_null();
    XrJson *opts = xr_value_to_json(args[0]);

    XrValue v;
    v = xr_json_get_by_key(X, opts, "maxConns");
    if (XR_IS_INT(v)) atomic_store(&ctx->max_conns, (int)XR_TO_INT(v));

    v = xr_json_get_by_key(X, opts, "maxRequestsPerConn");
    if (XR_IS_INT(v)) atomic_store(&ctx->max_requests_per_conn, (int)XR_TO_INT(v));

    v = xr_json_get_by_key(X, opts, "idleTimeout");
    if (XR_IS_INT(v)) atomic_store(&ctx->idle_timeout_ms, (int)XR_TO_INT(v));

    v = xr_json_get_by_key(X, opts, "readTimeout");
    if (XR_IS_INT(v)) atomic_store(&ctx->read_timeout_ms, (int)XR_TO_INT(v));
...
XrValue xr_http_server_stats(XrayIsolate *X, XrValue *args, int argc) {
    ...
    xr_json_set_by_key(X, j, "currentConns",
                        XR_FROM_INT(atomic_load(&ctx->current_conns)));
