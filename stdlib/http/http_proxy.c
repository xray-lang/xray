...
char *xr_proxy_auth_header(const char *username, const char *password) {
    ...
    // Base64 encode
    char *encoded =
        xr_base64_encode((const unsigned char *) credentials, strlen(credentials), NULL);
    xr_free(credentials);
...
char *xr_proxy_connect_request(const char *target_host, int target_port, const char *proxy_auth) {
    if (!target_host)
        return NULL;

    size_t buf_size = 512;
    if (proxy_auth)
        buf_size += strlen(proxy_auth);
