...
// Coroutine API
extern XrCoroutine *xr_coro_create(XrayIsolate *X, XrClosure *closure, 
...
static int parse_request_line(char *line, XrHttpReq *req) {
    ...
    *method_end = '\0';
    ...
    // Parse method
    if (strcmp(line, "GET") == 0) req->method = XR_HTTP_METHOD_GET;
    else if (strcmp(line, "POST") == 0) req->method = XR_HTTP_METHOD_POST;
    else if (strcmp(line, "PUT") == 0) req->method = XR_HTTP_METHOD_PUT;
    else if (strcmp(line, "DELETE") == 0) req->method = XR_HTTP_METHOD_DELETE;
    else if (strcmp(line, "HEAD") == 0) req->method = XR_HTTP_METHOD_HEAD;
    else if (strcmp(line, "OPTIONS") == 0) req->method = XR_HTTP_METHOD_OPTIONS;
    else if (strcmp(line, "PATCH") == 0) req->method = XR_HTTP_METHOD_PATCH;
    else return -1;
...
static bool check_keep_alive(const char *headers) {
    ...
    if (!conn) conn = strstr(headers, "connection:");
    if (!conn) return true;  // HTTP/1.1 defaults to Keep-Alive
...
XrHttpServer *xr_http_server_new(XrayIsolate *isolate) {
    ...
    if (coro && coro->coro_gc) {
        ...
        xr_coro_gc_register_root(coro->coro_gc,
                                  http_server_mark_roots, server);
    }
...
void xr_http_server_free(XrHttpServer *server) {
    ...
    if (server->owner_gc) {
        xr_coro_gc_unregister_root(server->owner_gc,
                                    http_server_mark_roots, server);
    }
...
void xr_http_server_route(XrHttpServer *server,
                              ...
                              const char *path,
                              XrClosure *handler) {
    if (!server || !server->router || !path || !handler) return;
...
void xr_http_server_static(XrHttpServer *server,
                           ...
                           const char *response,
                           size_t response_len) {
    if (!server || !server->router || !path || !response) return;
...
int xr_http_read_request(XrayIsolate *X, int fd, XrHttpReq *req, char *buf, size_t buf_size) {
    ...
    while (total < buf_size - 1) {
        ...
        if (to_read > 4096) to_read = 4096;  // Limit single read size
        
        int n = xr_socket_read(X, fd, buf + total, to_read);
        if (n <= 0) {
            if (total == 0) return -1;  // Connection closed
            break;
        }
        total += n;
        
        // Search for \r\n\r\n in read data
        for (size_t i = ((size_t)total > (size_t)n + 3) ? (size_t)total - (size_t)n - 3 : 0; i + 3 < (size_t)total; i++) {
            if (buf[i] == '\r' && buf[i+1] == '\n' &&
                buf[i+2] == '\r' && buf[i+3] == '\n') {
                found_end = true;
 ...
 */
static void detect_content_type(const char *body, size_t body_len,
 ...
 */
static int xr_http_writev_response(XrayIsolate *X, int fd, 
...
int xr_http_send_error(XrayIsolate *X, int fd, int status, const char *message) {
    ...
    char body[256];
    snprintf(body, sizeof(body), "{\"error\": \"%s\"}", message ? message : http_status_text(status));
...
int xr_http_send_redirect(XrayIsolate *X, int fd, int status, const char *location) {
    ...
    int len = snprintf(response, sizeof(response), HTTP_REDIRECT_TEMPLATE, 
...
bool xr_http_try_prebuilt(XrRouter *router, const char *raw_data, size_t data_len,
    ...
    if (!router || !raw_data || data_len < 14) return false;
    ...
    xr_router_find(router, method, path, path_len,
                   &params, &user_data, &static_resp, &static_len,
                   &prebuilt_resp, &prebuilt_len);
    
    if (!prebuilt_resp || prebuilt_len == 0 || params.count > 0) return false;
