...
static void buf_ensure(XrSerialBuf *buf, size_t need) {
    if (buf->error || buf->len + need <= buf->cap)
        return;
            ...
            switch (heap_type) {
                ...
                case XR_TARRAY: {
                    ...
                    for (int32_t i = 0; i < count; i++) {
                        ...
                        if (encode_value(X, elem, buf, depth + 1) != 0)
                            return -1;
                    }
                    return 0;
                }
                ...
                case XR_TMAP: {
                    ...
                    for (uint32_t i = 0; i < total_nodes && written < count; i++) {
                        ...
                        if (encode_value(X, node->key, buf, depth + 1) != 0)
                            return -1;
                        if (encode_value(X, node->value, buf, depth + 1) != 0)
                            return -1;
                        written++;
                    }
                    return 0;
...
static int decode_value(XrSerialReader *r, XrValue *out) {
    if (r->depth > XR_SERIAL_MAX_DEPTH)
        return -1;

    uint8_t tag;
    if (reader_u8(r, &tag) != 0)
        return -1;
    ...
    switch (tag) {
        ...
        case XR_STAG_STRING: {
            uint32_t slen;
            if (reader_varint32(r, &slen) != 0)
                return -1;
            const uint8_t *sdata;
            if (reader_bytes(r, slen, &sdata) != 0)
                return -1;
        ...
        case XR_STAG_BYTES: {
            uint32_t blen;
            if (reader_varint32(r, &blen) != 0)
                return -1;
            const uint8_t *bdata;
            if (reader_bytes(r, blen, &bdata) != 0)
                return -1;
        ...
        case XR_STAG_ARRAY_I32: {
            ...
            const uint8_t *raw;
            if (reader_bytes(r, byte_len, &raw) != 0)
                return -1;
...
int xr_cluster_decode(XrSerialReader *r, XrValue *out) {
    ...
    uint32_t expected_crc =
        (uint32_t) r->data[payload_len] | ((uint32_t) r->data[payload_len + 1] << 8) |
        ((uint32_t) r->data[payload_len + 2] << 16) | ((uint32_t) r->data[payload_len + 3] << 24);
    ...
    uint8_t version;
    if (reader_u8(r, &version) != 0)
        return -1;
    if (version != XR_SERIAL_VERSION)
        return -1;
