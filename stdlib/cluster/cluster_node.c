...
void xr_cluster_node_free(XrClusterNode *node) {
    ...
    for (int i = 0; i < XR_PENDING_BUCKETS; i++) {
        ...
        while (pr) {
            ...
            if (pr->response_ch)
                xr_channel_close(pr->response_ch);
            xr_free(pr);
            pr = next;
        }
...
void xr_cluster_node_writer_loop(void *arg) {
    ...
    while (atomic_load(&node->writer_running) && node->state == XR_NODE_CONNECTED && node->conn) {
        ...
        if (!batch) {
            ...
            int n;
            if (node->isolate) {
                n = xr_socket_read(node->isolate, pipe_fd, (char *) drain, sizeof(drain));
            } else {
...
void xr_cluster_node_start_reader(struct XrCluster *cluster, XrClusterNode *node) {
    ...
    XrCoroutine *coro =
        xr_coro_create_native(cluster->isolate, cluster_reader_loop, ctx, "cluster_reader");
    if (coro) {
 ...
 * per stdlib/net/io.c).
 */
static void cluster_handshake_set_deadline(XrCluster *cluster, XrIOConn *conn, int timeout_ms) {
    if (!cluster || !cluster->isolate || !conn || conn->fd < 0)
        return;
...
int xr_cluster_node_connect(XrCluster *cluster, XrClusterNode *node) {
     ...
     */
    cluster_handshake_set_deadline(cluster, node->conn, XR_CLUSTER_HANDSHAKE_TIMEOUT_MS);
...
XrClusterNode *xr_cluster_node_accept(XrCluster *cluster, XrIOConn *conn) {
     ...
     */
    cluster_handshake_set_deadline(cluster, conn, XR_CLUSTER_HANDSHAKE_TIMEOUT_MS);
...
XrChannel *xr_cluster_node_add_pending(XrClusterNode *node, uint64_t request_id, XrayIsolate *X,
    ...
    if (!node || !X)
        return NULL;
    if (max_pending <= 0)
        max_pending = XR_MAX_PENDING_REQUESTS;
