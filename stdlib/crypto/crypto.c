...
void xr_md5_update(XrMD5Context *ctx, const uint8_t *data, size_t len) {
    ...
    if (len >= part_len) {
        ...
        for (i = part_len; i + 63 < len; i += 64) md5_transform(ctx, &data[i]);
        index = 0;
    }
...
void xr_sha256_final(XrSHA256Context *ctx, uint8_t digest[32]) {
    ...
    uint8_t len_bytes[8];
    for (int i = 0; i < 8; i++) len_bytes[7 - i] = (uint8_t)(bits >> (i * 8));
...
void xr_sha1_update(XrSHA1Context *ctx, const uint8_t *data, size_t len) {
    ...
    if (len >= part_len) {
        ...
        for (i = part_len; i + 63 < len; i += 64) sha1_transform(ctx, &data[i]);
        index = 0;
    }
    memcpy(&ctx->buffer[index], &data[i], len - i);
}
...
void xr_sha1_final(XrSHA1Context *ctx, uint8_t digest[20]) {
    ...
    for (int i = 0; i < 8; i++) bits[7 - i] = (uint8_t)(total_bits >> (i * 8));
    uint32_t index = (ctx->count[0] >> 3) & 0x3F;
    size_t pad_len = (index < 56) ? (56 - index) : (120 - index);
...
void xr_sha512_update(XrSHA512Context *ctx, const uint8_t *data, size_t len) {
    ...
    if (len >= part_len) {
        ...
        for (i = part_len; i + 127 < len; i += 128) sha512_transform(ctx, &data[i]);
        index = 0;
    }
    memcpy(&ctx->buffer[index], &data[i], len - i);
}
...
void xr_sha512_final(XrSHA512Context *ctx, uint8_t digest[64]) {
    ...
    for (int i = 0; i < 8; i++) len_bytes[7 - i] = (uint8_t)(bits_hi >> (i * 8));
    for (int i = 0; i < 8; i++) len_bytes[15 - i] = (uint8_t)(bits_lo >> (i * 8));
 ...
 */
static void hmac_compute(HashFn hash, int block_size, int digest_size,
...
void xr_aes_init(XrAESContext *ctx, const uint8_t *key, int key_bits) {
    ...
    XR_DCHECK(key_bits == 128 || key_bits == 192 || key_bits == 256, "xr_aes_init: invalid key_bits");
    int nk, nr;
    switch (key_bits) {
        case 128: nk = 4; nr = 10; break;
        case 192: nk = 6; nr = 12; break;
        case 256: nk = 8; nr = 14; break;
        default: memset(ctx, 0, sizeof(*ctx)); return;
    }
    ctx->rounds = nr;
...
void xr_aes_cbc_encrypt(XrAESContext *ctx, const uint8_t *iv,
    ...
    for (size_t i = 0; i < len; i += 16) {
        uint8_t block[16];
        for (int j = 0; j < 16; j++) block[j] = input[i + j] ^ prev[j];
...
void xr_aes_cbc_decrypt(XrAESContext *ctx, const uint8_t *iv,
    ...
    for (size_t i = 0; i < len; i += 16) {
        ...
        for (int j = 0; j < 16; j++) output[i + j] = decrypted[j] ^ prev[j];
...
#ifdef _WIN32
    return BCryptGenRandom(NULL, buffer, (ULONG)len, BCRYPT_USE_SYSTEM_PREFERRED_RNG) == 0 ? 0 : -1;
...
static XrValue crypto_md5(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 1 || !XR_IS_STRING(args[0])) return xr_null();
...
static XrValue crypto_random_bytes(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 1 || !XR_IS_INT(args[0])) return xr_null();
    int len = (int)XR_TO_INT(args[0]);
    if (len <= 0 || len > 1024) return xr_null();
    uint8_t buf[1024];
    char hex[2049];
    if (xr_random_bytes(buf, len) != 0) return xr_null();
...
static XrValue crypto_uuid(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    uint8_t bytes[16];
    if (xr_random_bytes(bytes, 16) != 0) return xr_null();
...
static XrValue crypto_encrypt(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 2 || !XR_IS_STRING(args[0]) || !XR_IS_STRING(args[1])) return xr_null();
    ...
    // Generate random IV
    uint8_t iv[16];
    if (xr_random_bytes(iv, 16) != 0) return xr_null();
...
static XrValue crypto_decrypt(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    if (hex_len < 64 || hex_len % 2 != 0) return xr_null();

    size_t total_bytes = hex_len / 2;
    size_t cipher_len = total_bytes - 16;
    if (cipher_len == 0 || cipher_len % 16 != 0) return xr_null();
...
static XrValue crypto_timing_safe_equal(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 2 || !XR_IS_STRING(args[0]) || !XR_IS_STRING(args[1])) return xr_bool(false);
...
XR_DEFINE_BUILTIN(crypto_md5, "md5",
    "(data: string): string",
    "Compute MD5 hash")
XR_DEFINE_BUILTIN(crypto_sha1, "sha1",
    "(data: string): string",
    "Compute SHA-1 hash")
XR_DEFINE_BUILTIN(crypto_sha256, "sha256",
    "(data: string): string",
    "Compute SHA-256 hash")
XR_DEFINE_BUILTIN(crypto_sha512, "sha512",
    "(data: string): string",
    "Compute SHA-512 hash")
XR_DEFINE_BUILTIN(crypto_hmac, "hmac",
    "(algo: string, key: string, data: string): string",
    "Compute HMAC")
XR_DEFINE_BUILTIN(crypto_random_bytes, "randomBytes",
    "(n: int): string",
    "Generate random bytes")
XR_DEFINE_BUILTIN(crypto_uuid, "uuid",
    "(): string",
    "Generate UUID v4")
XR_DEFINE_BUILTIN(crypto_encrypt, "encrypt",
    "(key: string, plaintext: string): string",
    "AES-256-CBC encrypt")
XR_DEFINE_BUILTIN(crypto_decrypt, "decrypt",
    "(key: string, ciphertext: string): string?",
    "AES-256-CBC decrypt")
XR_DEFINE_BUILTIN(crypto_timing_safe_equal, "timingSafeEqual",
