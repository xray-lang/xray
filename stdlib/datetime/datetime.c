...
XrDateTime* xr_datetime_create(XrayIsolate *isolate, int year, int month, int day,
...
int xr_datetime_to_iso_string(XrDateTime *dt, char *buf, size_t buf_size) {
    ...
    int n;
    if (dt->is_utc) {
        n = snprintf(buf, buf_size, "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ",
                     tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                     tm.tm_hour, tm.tm_min, tm.tm_sec, dt->milliseconds);
    } else {
...
int xr_datetime_is_before(XrDateTime *dt1, XrDateTime *dt2) {
    if (dt1->timestamp != dt2->timestamp) return dt1->timestamp < dt2->timestamp;
...
XrDateTime* xr_datetime_add(XrayIsolate *isolate, XrDateTime *dt, int64_t amount, const char *unit) {
    int64_t seconds = 0;
    ...
    if (strcmp(unit, "millisecond") == 0 || strcmp(unit, "milliseconds") == 0) {
        ...
        if (result->milliseconds < 0) { result->timestamp--; result->milliseconds += 1000; }
        ...
        if (tm.tm_mday > max_day) tm.tm_mday = max_day;
        tm.tm_isdst = -1;
...
int64_t xr_datetime_diff(XrDateTime *dt1, XrDateTime *dt2, const char *unit) {
    int64_t diff_ms = (dt1->timestamp - dt2->timestamp) * 1000
...
static XrValue dt_create(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    return xr_datetime_value(xr_datetime_create(isolate, year, month, day, hour, minute, second, 0));
}
...
static XrValue dt_create_utc(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    int minute = nargs > 4 && XR_IS_INT(args[4]) ? (int)XR_TO_INT(args[4]) : 0;
    int second = nargs > 5 && XR_IS_INT(args[5]) ? (int)XR_TO_INT(args[5]) : 0;
    return xr_datetime_value(xr_datetime_create(isolate, year, month, day, hour, minute, second, 1));
}

static XrValue dt_from_timestamp(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 1) return XR_NULL_VAL;
    int64_t ts = 0;
    if (XR_IS_INT(args[0])) ts = XR_TO_INT(args[0]);
    else if (XR_IS_FLOAT(args[0])) ts = (int64_t)XR_TO_FLOAT(args[0]);
    else return XR_NULL_VAL;
...
static XrValue dt_format(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 1 || !XR_IS_DATETIME(args[0])) return XR_NULL_VAL;
...
static XrValue dt_year(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 1 || !XR_IS_DATETIME(args[0])) return XR_INT(0);
...
static XrValue dt_add(XrayIsolate *isolate, XrValue *args, int nargs) {
    if (nargs < 3 || !XR_IS_DATETIME(args[0]) || !XR_IS_STRING(args[2])) return XR_NULL_VAL;
    XrDateTime *dt = XR_TO_DATETIME(args[0]);
...
static XrValue dt_diff(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 2 || !XR_IS_DATETIME(args[0]) || !XR_IS_DATETIME(args[1])) return XR_INT(0);
...
static XrValue dt_is_before(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 2 || !XR_IS_DATETIME(args[0]) || !XR_IS_DATETIME(args[1])) return XR_FALSE_VAL;
    return xr_datetime_is_before(XR_TO_DATETIME(args[0]), XR_TO_DATETIME(args[1])) ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue dt_is_after(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 2 || !XR_IS_DATETIME(args[0]) || !XR_IS_DATETIME(args[1])) return XR_FALSE_VAL;
    return xr_datetime_is_after(XR_TO_DATETIME(args[0]), XR_TO_DATETIME(args[1])) ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue dt_equals(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 2 || !XR_IS_DATETIME(args[0]) || !XR_IS_DATETIME(args[1])) return XR_FALSE_VAL;
    return xr_datetime_equals(XR_TO_DATETIME(args[0]), XR_TO_DATETIME(args[1])) ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue dt_is_leap_year(XrayIsolate *isolate, XrValue *args, int nargs) {
    (void)isolate;
    if (nargs < 1 || !XR_IS_DATETIME(args[0])) return XR_FALSE_VAL;
...
static XrNativeMethod datetime_methods[] = {
    {"format",       (XrCFunctionPtr)dt_format,        2},
...
static XrNativeMethod datetime_getters[] = {
    {"year",        (XrCFunctionPtr)dt_year,        1},
...
XR_DEFINE_BUILTIN(dt_create, "create", "(year: int, month?: int, day?: int, hour?: int, minute?: int, second?: int): DateTime", "Create local datetime")
XR_DEFINE_BUILTIN(dt_create_utc, "createUTC", "(year: int, month?: int, day?: int, hour?: int, minute?: int, second?: int): DateTime", "Create UTC datetime")
XR_DEFINE_BUILTIN(dt_from_timestamp, "fromTimestamp", "(ts: int): DateTime", "Create datetime from Unix timestamp (seconds)")
XR_DEFINE_BUILTIN(dt_from_timestamp_ms, "fromTimestampMs", "(ts: int): DateTime", "Create datetime from Unix timestamp (milliseconds)")
XR_DEFINE_BUILTIN(dt_parse, "parse", "(s: string, format?: string): DateTime?", "Parse datetime string")
...
XR_DEFINE_BUILTIN(dt_add, "add", "(amount: int, unit: string): DateTime", "Add duration to datetime")
XR_DEFINE_BUILTIN(dt_diff, "diff", "(other: DateTime, unit?: string): int", "Difference between datetimes")
...
XR_DEFINE_BUILTIN(dt_is_before, "isBefore", "(other: DateTime): bool", "Check if before other datetime")
XR_DEFINE_BUILTIN(dt_is_after, "isAfter", "(other: DateTime): bool", "Check if after other datetime")
XR_DEFINE_BUILTIN(dt_equals, "equals", "(other: DateTime): bool", "Check if equal to other datetime")
