...
static XrValue m_to_iso_string(XrayIsolate *iso, XrValue self,
...
static XrValue m_to_string(XrayIsolate *iso, XrValue self,
                           XrValue *args, int argc) {
    (void)args; (void)argc;
    XrDateTime *dt = dt_self(self);
    char buf[256];
...
#define DT_INT_GETTER(name, body)                                        \
    static XrValue name(XrayIsolate *iso, XrValue self,                  \
                        XrValue *args, int argc) {                       \
...
/* Conversions */
static XrValue m_to_utc(XrayIsolate *iso, XrValue self,
                        XrValue *args, int argc) {
    (void)args; (void)argc;
...
static XrValue m_to_local(XrayIsolate *iso, XrValue self,
                          XrValue *args, int argc) {
    (void)args; (void)argc;
...
/* Comparisons */
static XrValue m_is_before(XrayIsolate *iso, XrValue self,
                           XrValue *args, int argc) {
    (void)iso;
    if (argc < 1 || !XR_IS_DATETIME(args[0])) return XR_FALSE_VAL;
    return xr_datetime_is_before(dt_self(self), XR_TO_DATETIME(args[0]))
        ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue m_is_after(XrayIsolate *iso, XrValue self,
                          XrValue *args, int argc) {
    (void)iso;
    if (argc < 1 || !XR_IS_DATETIME(args[0])) return XR_FALSE_VAL;
    return xr_datetime_is_after(dt_self(self), XR_TO_DATETIME(args[0]))
        ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue m_equals(XrayIsolate *iso, XrValue self,
                        XrValue *args, int argc) {
    (void)iso;
    if (argc < 1 || !XR_IS_DATETIME(args[0])) return XR_FALSE_VAL;
    return xr_datetime_equals(dt_self(self), XR_TO_DATETIME(args[0]))
        ? XR_TRUE_VAL : XR_FALSE_VAL;
}

static XrValue m_is_leap_year(XrayIsolate *iso, XrValue self,
                              XrValue *args, int argc) {
    (void)iso; (void)args; (void)argc;
