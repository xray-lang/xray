...
此调用不接收值参数或类型实参，结果为 unit；普通函数不需要 async 标注。词法绑定若遮蔽 `Coro`，仍按该绑定解析，不能偷偷调用调度原语。新 XIR 源码入口按同一 SUSPEND/恢复协议执行，当前单宿主驱动的挂起结果须以匹配的实例 epoch/wake 恢复；完整多线程调度与生成器资格单独验收。
