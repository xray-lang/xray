...
def validate_registry(registry: dict[str, Any]) -> dict[str, dict[Any, dict[str, Any]]]:
    ...
    for operation in operations:
        ...
        for consumer, task in CONSUMER_TASKS.items():
            ...
            expected = "COMPLETE" if consumer == "spec_oracle" else "NOT_YET_ACTIVE"
            require(entry["status"] == expected,
                    f"operation {spelling} {consumer} must be {expected} during W1")
...
def generate_markdown(registry: dict[str, Any], digest: str, kat_count: int) -> str:
    lines = [
        ...
        f"> Epoch: `{registry['epoch']}`; registry SHA-256: `{digest}`; normative KATs: `{kat_count}`.",
        "",
...
def generate_outputs(registry: dict[str, Any], kats: dict[str, Any]) -> dict[Path, str]:
    ...
    digest = hashlib.sha256(canonical_json(registry).encode("utf-8")).hexdigest()
    return {
    ...
    try:
        ...
        if args.generate:
            ...
            print(f"CoreSpec registry: PASS ({len(registry['operations'])} operations, "
                  f"{len(kats['cases'])} KATs, sha256={digest})")
        else:
