...
VALUE_SYSTEM_KEYS = {
    ...
    "module_slot_immediate_contract",
...
PROJECTION_KINDS = {
    ...
    "integer-divmod": "XR_PROGRAM_XI_PROJECTION_INTEGER_DIVMOD",
    ...
    "string-from-scalar": "XR_PROGRAM_XI_PROJECTION_STRING_FROM_SCALAR",
}
...
XI_EXISTENTIAL_KINDS = {
    ...
    "witness-invoke": "XI_EXISTENTIAL_WITNESS_INVOKE",
}
SEMANTIC_PROJECTION_KINDS = {
    ...
    "witness-invoke": "XR_PROGRAM_XI_SEMANTIC_WITNESS_INVOKE",
}
FIRST_OPERAND_POLICIES = {
    "borrow": "XR_PROGRAM_XI_FIRST_OPERAND_BORROW",
    "concrete-by-interface-use": "XR_PROGRAM_XI_FIRST_OPERAND_CONCRETE_BY_INTERFACE_USE",
    "existential-by-interface-use":
        "XR_PROGRAM_XI_FIRST_OPERAND_EXISTENTIAL_BY_INTERFACE_USE",
    "receiver-by-interface-use": "XR_PROGRAM_XI_FIRST_OPERAND_RECEIVER_BY_INTERFACE_USE",
}
EXPECTED_SEMANTIC_MAPPINGS = [
    ...
    ("xi.as", "project", "existential-project", "core.existential.project",
     "existential-by-interface-use"),
    ("xi.call.method", "witness-direct", "witness-direct", "core.call.witness_direct",
     "receiver-by-interface-use"),
    ("xi.call.method.direct", "witness-direct", "witness-direct",
     "core.call.witness_direct", "receiver-by-interface-use"),
    ("xi.call.method", "witness-invoke", "witness-invoke", "core.call.witness_invoke",
     "receiver-by-interface-use"),
    ("xi.call.method.direct", "witness-invoke", "witness-invoke",
     "core.call.witness_invoke", "receiver-by-interface-use"),
]
...
CORE_TYPE_NAMES = {
    ...
    "integer": "XR_PROGRAM_XI_INTEGER_RESULT_TYPE",
...
CONSTANT_SHAPE = (
    ...
    "; kind 5:f64 carries the exact unsigned 64-bit IEEE-754 bit pattern in minimal ULEB128, "
    "ordered by unsigned bits without NaN or signed-zero normalization"
)
...
def validate(schema: dict[str, Any]) -> None:
    ...
    require(type_system == {
        ...
        "display_name_shape": 'after each dynamic shape: bounded UTF-8 type display name followed by declaration-ordered variant display names; each name is minimal-ULEB128 byte length plus bytes; zero means absent; no NUL or ASCII control bytes; names never act as lookup identities',
        ...
        "recursive_value_shape": "reject by-value aggregate/variant cycles; class-reference and record-reference fields and array elements terminate by-value cycle traversal",
    ...
    require(value_system == {
        ...
        "module_slot_immediate_contract": 'immediate kind 13 is dense (ModuleId,ModuleSlotId), resolving the immutable initialization-order module and its declaration-key-ordered slot; producer references use module/declaration semantic keys; no physical address, layout or mutable value is encoded',
...
def validate_source_projection(projection: dict[str, Any], core: dict[str, Any],
    ...
    for row in rows:
        ...
        require(row["result_type"] in {"any", "integer"} or row["result_type"] in core_types,
    ...
    semantic_rows = projection["semantic_mappings"]
    require(isinstance(semantic_rows, list) and semantic_rows,
            "Xi source projection has no semantic mappings")
    semantic_identities: set[tuple[str, str]] = set()
    semantic_contract: list[tuple[str, str, str, str, str]] = []
    for row in semantic_rows:
        require(isinstance(row, dict) and set(row) == SEMANTIC_MAPPING_KEYS,
                "Xi semantic projection fields drifted")
        identity = (row["xi_operation"], row["xi_existential_kind"])
        require(identity not in semantic_identities,
                f"duplicate Xi semantic projection {identity}")
        semantic_identities.add(identity)
        require(row["xi_operation"] in xi_operations,
                f"unknown semantic Xi operation {row['xi_operation']}")
        require(row["xi_existential_kind"] in XI_EXISTENTIAL_KINDS,
                f"unknown Xi existential kind {row['xi_existential_kind']}")
        require(row["projection_kind"] in SEMANTIC_PROJECTION_KINDS,
                f"unknown semantic projection kind {row['projection_kind']}")
        require(row["core_operation"] in core_operations,
                f"unknown semantic CoreSpec operation {row['core_operation']}")
        require(row["first_operand_policy"] in FIRST_OPERAND_POLICIES,
                f"unknown first-operand policy {row['first_operand_policy']}")
        semantic_contract.append((
            row["xi_operation"],
            row["xi_existential_kind"],
            row["projection_kind"],
            row["core_operation"],
            row["first_operand_policy"],
        ))
    require(semantic_contract == EXPECTED_SEMANTIC_MAPPINGS,
            "Xi existential/witness semantic projection contract drifted")
    for core_name, expected_operands in EXPECTED_SEMANTIC_CORE_OWNERSHIP.items():
        operation = core_operations[core_name]
        ownership = operation.get("ownership")
        require(isinstance(ownership, dict) and ownership.get("operands") == expected_operands,
                f"CoreSpec ownership contract drifted for {core_name}")
    ...
    require(sources == {
        ...
        "unconditional-edge",
        "conditional-edge",
        "resolved-nominal-ref-field-access",
        "affine-xi.place.load-writeback",
        "exact-value-struct-xi.agg.new-plus-xi.agg.set-sequence",
        "exact-array-new-plus-complete-index-initialization-prefix",
        "exact-array-index-read-or-replacement",
...
def generate_source_projection_source(projection: dict[str, Any],
    ...
    lines.extend(["};", "", "static const XrProgramXiSemanticProjectionRow",
                  "    xr_program_xi_semantic_projection_rows[] = {"])
    for row in projection["semantic_mappings"]:
        core_name = row["core_operation"]
        require(core_ids[core_name] <= 65535, f"CoreSpec id too wide for {core_name}")
        lines.extend([
            "    {",
            f"        {xi_c_identifier(row['xi_operation'])},",
            f"        {XI_EXISTENTIAL_KINDS[row['xi_existential_kind']]},",
            f"        {core_c_identifier(core_name)},",
            f"        {SEMANTIC_PROJECTION_KINDS[row['projection_kind']]},",
            f"        {FIRST_OPERAND_POLICIES[row['first_operand_policy']]},",
            "    },",
        ])
    lines.extend([
        "};",
...
def generate_spec(schema: dict[str, Any], digest: str) -> str:
    ...
    lines.extend([
        ...
        f"- Array shape: `{type_system['array_shape']}`",
        f"- Variant shape: `{type_system['variant_shape']}`",
        f"- Display names: `{type_system['display_name_shape']}`",
        f"- View shape: `{type_system['view_shape']}`",
        f"- Callable shape: `{type_system['callable_shape']}`",
        f"- Callable pack: `{type_system['callable_pack']}`",
        f"- Callable dispatch: `{type_system['callable_dispatch']}`",
        ...
        f"- Module slot immediate: `{value_system['module_slot_immediate_contract']}`",
