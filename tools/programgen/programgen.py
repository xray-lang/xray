...
def validate(schema: dict[str, Any]) -> None:
    ...
    require(type_system == {
        ...
        "callable_shape": "one canonical SignatureId; every callable TypeId is affine with explicit copy and its signature has no receiver",
        "callable_pack": "one closed FunctionId plus zero operands for an exact captureless signature, or one consumed affine explicit-copy aggregate owner used as a READ hidden receiver; target result borrows may not name that receiver",
