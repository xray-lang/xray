...
def generate_xi_lowering_test(entries: list[XiLoweringDef], ops: list[XiOpDef]) -> str:
    ...
    lines.append('')
    lines.append('#define XI_LOWERING_TEST_REQUIRE(condition) \\')
    lines.append('    do { \\')
    lines.append('        if (!(condition)) { \\')
    lines.append('            fprintf(stderr, \\')
    lines.append('                    "Xi lowering coverage check failed: %s (%s:%d)\\n", \\')
    lines.append('                    #condition, __FILE__, __LINE__); \\')
    ...
    lines.append('}')
    lines.append('')
    return '\n'.join(lines).replace('    assert(', '    XI_LOWERING_TEST_REQUIRE(')
