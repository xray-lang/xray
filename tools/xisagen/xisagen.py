...
def generate_dispatch_emit_header(entries: list[IselEntry]) -> str:
    ...
    wrappers = [
        ...
        },
        {
            ...
            'cases': [
                ...
                ('DIV', 'arm64.sdiv.rrr', 'a64_buf_emit(buf, a64_sdiv(rd, rn, rm));'),
        ...
        },
        {
            ...
            'cases': [
                ...
                ('DIV', 'riscv64.div', 'rv64_buf_emit(buf, rv64_div(rd, rs1, rs2));'),
                ('MOD', 'riscv64.rem', 'rv64_buf_emit(buf, rv64_rem(rd, rs1, rs2));'),
    ...
    generated_cases_by_target: dict[str, int] = {target: 0 for target in VALID_ISEL_TARGETS}
    generated_wrapper_case_count = 0
    for w in wrappers:
        count = len(w['cases'])
        generated_wrapper_case_count += count
        generated_cases_by_target[w['target']] += count

    declared_by_target: dict[str, int] = {target: 0 for target in VALID_ISEL_TARGETS}
    custom_by_target: dict[str, int] = {target: 0 for target in VALID_ISEL_TARGETS}
    for e in entries:
        if e.target not in declared_by_target:
            continue
        declared_by_target[e.target] += 1
        if e.pattern == 'CUSTOM':
            custom_by_target[e.target] += 1
    custom_entry_count = sum(custom_by_target.values())
    patterned_entry_count = len(entries) - custom_entry_count
    ...
    lines.append('/* ========== Generated Emit Coverage ========== */')
    lines.append('')
    lines.append(f'#define XM_DISPATCH_EMIT_WRAPPER_COUNT {len(wrappers)}')
    lines.append(f'#define XM_DISPATCH_EMIT_DECLARED_ENTRY_COUNT {len(entries)}')
    lines.append(f'#define XM_DISPATCH_EMIT_CUSTOM_ENTRY_COUNT {custom_entry_count}')
    lines.append(f'#define XM_DISPATCH_EMIT_PATTERNED_ENTRY_COUNT {patterned_entry_count}')
    lines.append(f'#define XM_DISPATCH_EMIT_GENERATED_CASE_COUNT {generated_wrapper_case_count}')
    for target in ('x64', 'arm64', 'riscv64'):
        macro_target = target.upper()
        lines.append(
            f'#define XM_DISPATCH_EMIT_DECLARED_{macro_target}_COUNT '
            f'{declared_by_target[target]}')
        lines.append(
            f'#define XM_DISPATCH_EMIT_CUSTOM_{macro_target}_COUNT '
            f'{custom_by_target[target]}')
        lines.append(
            f'#define XM_DISPATCH_EMIT_PATTERNED_{macro_target}_COUNT '
            f'{declared_by_target[target] - custom_by_target[target]}')
        lines.append(
            f'#define XM_DISPATCH_EMIT_GENERATED_{macro_target}_COUNT '
            f'{generated_cases_by_target[target]}')
    lines.append('')
...
XI_JIT_TEMPLATE_OPS = {
    'xi.add': ('XM_ADD', 'XM_FADD'),
    'xi.sub': ('XM_SUB', 'XM_FSUB'),
    'xi.mul': ('XM_MUL', 'XM_FMUL'),
    'xi.div': ('XM_DIV', 'XM_FDIV'),
    'xi.mod': ('XM_MOD', 'XM_OP_COUNT'),
    'xi.neg': ('XM_NEG', 'XM_FNEG'),
    'xi.band': ('XM_AND', 'XM_OP_COUNT'),
    'xi.bor': ('XM_OR', 'XM_OP_COUNT'),
    'xi.bxor': ('XM_XOR', 'XM_OP_COUNT'),
    'xi.bnot': ('XM_NOT', 'XM_OP_COUNT'),
    'xi.not': ('XM_EQ', 'XM_OP_COUNT'),
    'xi.shl': ('XM_SHL', 'XM_OP_COUNT'),
    'xi.shr': ('XM_SHR', 'XM_OP_COUNT'),
    'xi.eq': ('XM_EQ', 'XM_FEQ'),
    'xi.ne': ('XM_NE', 'XM_FNE'),
    'xi.eq.strict': ('XM_EQ', 'XM_EQ'),
    'xi.ne.strict': ('XM_NE', 'XM_NE'),
    'xi.lt': ('XM_LT', 'XM_FLT'),
    'xi.le': ('XM_LE', 'XM_FLE'),
    'xi.gt': ('XM_LT', 'XM_FLT'),
    'xi.ge': ('XM_LE', 'XM_FLE'),
...
XI_JIT_TEMPLATE_SWAP_ARGS = {
    'xi.gt',
    'xi.ge',
}

XI_JIT_TEMPLATE_EQ_LIKE = {
    'xi.eq',
    'xi.eq.strict',
}

XI_JIT_TEMPLATE_NE_LIKE = {
    'xi.ne',
    'xi.ne.strict',
}

XI_JIT_TEMPLATE_LOGICAL_NOT = {
    'xi.not',
}
...
def generate_xi_lowering_coverage_header(entries: list[XiLoweringDef]) -> str:
    main_targets = {'vm-bytecode', 'jit-xm', 'aot-c'}
...
def generate_xi_to_c_dispatch_header(entries: list[XiLoweringDef]) -> str:
    ...
    lines.append('')
    lines.append('')
    lines.append('#define XI_TO_C_TEMPLATE_WIDTH_DRIVERS(X) \\')
...
def generate_xi_to_xm_dispatch_header(entries: list[XiLoweringDef]) -> str:
    target_entries = [entry for entry in entries if 'jit-xm' in entry.target_drivers]
    template_entries = [
        entry for entry in target_entries
    ...
    def emit_template_driver_macro(lines: list[str], macro: str, template: str) -> None:
        ...
        lines.append(f'#define {macro}(X) \\')
        for i, entry in enumerate(template_driver_entries):
            driver = entry.target_drivers['jit-xm']
            suffix = ' \\' if i + 1 < len(template_driver_entries) else ''
            lines.append(f'    X({entry.ident}, {driver}){suffix}')
        lines.append('')
        lines.append('')
    ...
    lines.append('/* Source: xisa/xi/lowering.def */')
    lines.append('')
    lines.append('#ifndef XI_TO_XM_DISPATCH_GEN_H')
    lines.append('#define XI_TO_XM_DISPATCH_GEN_H')
    lines.append('')
    lines.append('#include <stdbool.h>')
    lines.append('#include <stdint.h>')
    lines.append('#include "../ir/xi.h"')
    lines.append('#include "xm_ops.h"')
    lines.append('')
    lines.append('#define XI_TO_XM_LOWERING_DRIVERS(X) \\')
    for i, entry in enumerate(target_entries):
        driver = entry.target_drivers['jit-xm']
        suffix = ' \\' if i + 1 < len(target_entries) else ''
        lines.append(f'    X({entry.ident}, "{entry.op_name}", {driver}){suffix}')
    ...
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        int_op, _ = XI_JIT_TEMPLATE_OPS[entry.op_name]
        lines.append(f'        case XI_{entry.ident}: return {int_op};')
    lines.append('        case XI_OP_COUNT: return XM_OP_COUNT;')
    lines.append('        default: return XM_OP_COUNT;')
    lines.append('    }')
    lines.append('    return XM_OP_COUNT;')
    lines.append('}')
    lines.append('')
    lines.append('static inline XmOp xi_to_xm_template_float_op(uint16_t op) {')
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        _, float_op = XI_JIT_TEMPLATE_OPS[entry.op_name]
        lines.append(f'        case XI_{entry.ident}: return {float_op};')
    lines.append('        case XI_OP_COUNT: return XM_OP_COUNT;')
    lines.append('        default: return XM_OP_COUNT;')
    lines.append('    }')
    lines.append('    return XM_OP_COUNT;')
    lines.append('}')
    lines.append('')
    lines.append('static inline bool xi_to_xm_template_swaps_args(uint16_t op) {')
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        if entry.op_name in XI_JIT_TEMPLATE_SWAP_ARGS:
            lines.append(f'        case XI_{entry.ident}: return true;')
    lines.append('        case XI_OP_COUNT: return false;')
    lines.append('        default: return false;')
    lines.append('    }')
    ...
    lines.append('}')
    lines.append('')
    lines.append('static inline bool xi_to_xm_template_eq_like(uint16_t op) {')
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        if entry.op_name in XI_JIT_TEMPLATE_EQ_LIKE:
            lines.append(f'        case XI_{entry.ident}: return true;')
    lines.append('        case XI_OP_COUNT: return false;')
    lines.append('        default: return false;')
    lines.append('    }')
    ...
    lines.append('}')
    lines.append('')
    lines.append('static inline bool xi_to_xm_template_ne_like(uint16_t op) {')
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        if entry.op_name in XI_JIT_TEMPLATE_NE_LIKE:
            lines.append(f'        case XI_{entry.ident}: return true;')
    lines.append('        case XI_OP_COUNT: return false;')
    lines.append('        default: return false;')
    lines.append('    }')
    ...
    lines.append('}')
    lines.append('')
    lines.append('static inline bool xi_to_xm_template_is_logical_not(uint16_t op) {')
    lines.append('    switch ((XiOp) op) {')
    for entry in template_entries:
        if entry.op_name in XI_JIT_TEMPLATE_LOGICAL_NOT:
            lines.append(f'        case XI_{entry.ident}: return true;')
    lines.append('        case XI_OP_COUNT: return false;')
    lines.append('        default: return false;')
    lines.append('    }')
...
def generate_xi_lowering_test(entries: list[XiLoweringDef]) -> str:
    ...
    lines.append('#include "../../../src/jit/xi_to_xm_dispatch_gen.h"')
    ...
    for entry in entries:
        ...
        if 'jit-xm' in entry.target_drivers and entry.template in {'value-binary',
                                                                    'value-unary',
                                                                    'compare'}:
            int_op, float_op = XI_JIT_TEMPLATE_OPS[entry.op_name]
            swaps = 'true' if entry.op_name in XI_JIT_TEMPLATE_SWAP_ARGS else 'false'
            eq_like = 'true' if entry.op_name in XI_JIT_TEMPLATE_EQ_LIKE else 'false'
            ne_like = 'true' if entry.op_name in XI_JIT_TEMPLATE_NE_LIKE else 'false'
            logical_not = 'true' if entry.op_name in XI_JIT_TEMPLATE_LOGICAL_NOT else 'false'
            lines.append(f'    assert(xi_to_xm_template_int_op(XI_{entry.ident}) == {int_op});')
            lines.append(f'    assert(xi_to_xm_template_float_op(XI_{entry.ident}) == {float_op});')
            lines.append(f'    assert(xi_to_xm_template_swaps_args(XI_{entry.ident}) == {swaps});')
            lines.append(f'    assert(xi_to_xm_template_eq_like(XI_{entry.ident}) == {eq_like});')
...
def write_xi_lowering_outputs(output_root: str, entries: list[XiLoweringDef]) -> list[str]:
    outputs = [
        ...
        ('src/jit/xi_to_xm_dispatch_gen.h', generate_xi_to_xm_dispatch_header(entries)),
...
def _test_xi_lowering_parser():
    ...
    jit_header = generate_xi_to_xm_dispatch_header(entries)
    assert 'X(ADD, "xi.add", xi2xm_add)' in jit_header
    assert 'X(COPY, "xi.copy", xi2xm_copy_reject)' in jit_header
    assert 'XI_TO_XM_TEMPLATE_BINARY_DRIVERS' in jit_header
    assert 'X(ADD, xi2xm_add)' in jit_header
    assert 'case XI_ADD: return XM_ADD;' in jit_header
