...
def _golden_arg_to_c(arg: str, op: OperandInfo) -> str:
    """Convert a golden-bytes argument to a C expression."""
    if arg in _X64_REG_MAP:
        return _X64_REG_MAP[arg]
    # Numeric immediate
    try:
        v = int(arg, 0)
        if op.subtype == 'u64':
...
def generate_golden_test(regclasses: list[RegClass], mcinsns: list[McInsn]) -> str:
    ...
    lines = []
    lines.append('/* AUTO-GENERATED by xisagen — DO NOT EDIT */')
    lines.append('/* Golden-bytes verification test for x64 instruction encoding */')
    lines.append('')
    lines.append('#if defined(__x86_64__) || defined(_M_X64)')
    lines.append('')
    ...
    for insn in mcinsns:
        ...
        c_name = None
        if insn.name in _X64_NAME_MAP:
            c_name = _X64_NAME_MAP.get(insn.name)
            if c_name is None:
                continue  # skipped instruction
        else:
            c_name = _x64_c_name(insn.name)
        ...
        for regs, hex_str in insn.golden_bytes:
            ...
            # Build C arguments from golden register/immediate values
            c_args = ['&buf']
            for i, op in enumerate(operands):
                if i < len(regs):
                    c_args.append(_golden_arg_to_c(regs[i], op))
...
def cmd_golden(args: list[str]):
    """Generate golden-bytes verification test from x64.isa.
