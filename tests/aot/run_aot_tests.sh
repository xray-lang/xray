...
run_negative_test() {
    ...
    if grep -Eq "unsupported .*coroutine Xi op|unsupported AOT sync call to suspendable function|exceptions inside AOT coroutine are unsupported|unsupported Xi op ERR_" "$log_out"; then
        echo "PASS (rejected)"
        PASS=$((PASS + 1))
        rm -f "$c_out" "$log_out"
    else
