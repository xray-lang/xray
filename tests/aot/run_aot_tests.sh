...
run_negative_test() {
    ...
    if grep -Eq "unsupported .*coroutine Xi op|unsupported AOT sync call to suspendable function|unsupported AOT indirect call|exceptions inside AOT coroutine are unsupported|unsupported Xi op ERR_|semantic analysis failed|derived Clone .* no consumable verified plan|: error: " "$log_out"; then
        echo "PASS (rejected)"
        PASS=$((PASS + 1))
        rm -rf "$case_work"
        return 0
