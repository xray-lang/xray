...
FREESTANDING_TOP_VAR_AGG_REASSIGN_SRC="$PROJECT_DIR/tests/aot/filetests/link/freestanding_top_var_aggregate_reassign.xr"
FREESTANDING_TOP_VAR_AGG_REASSIGN_OBJ="$WORK/freestanding_top_var_aggregate_reassign.o"
FREESTANDING_TOP_VAR_AGG_REASSIGN_LOG="$WORK/freestanding_top_var_aggregate_reassign.log"
if "$XRAY" build --native --profile freestanding --shared --keep-c --rebuild \
        --dump-link-command \
        --cache-dir "$BUILD_CACHE" -o "$FREESTANDING_TOP_VAR_AGG_REASSIGN_OBJ" \
        "$FREESTANDING_TOP_VAR_AGG_REASSIGN_SRC" >"$FREESTANDING_TOP_VAR_AGG_REASSIGN_LOG" 2>&1; then
    FREESTANDING_TOP_VAR_AGG_REASSIGN_C="$(sed -n 's/^Kept C source: //p' \
    ...
    if [ -f "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" ]; then
        ...
        expect_log_contains "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" \
            ...
            "freestanding-profile/top-var-aggregate-reassign: copies whole value into static data"
        expect_log_not_contains "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" "xrt_shared[0] =" \
            "freestanding-profile/top-var-aggregate-reassign: avoids shared-slot storage"
        expect_log_not_contains "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" "xrt_arc_alloc" \
            "freestanding-profile/top-var-aggregate-reassign: avoids hosted aggregate allocation"
        expect_log_not_contains "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" "xr_aggregate_ref" \
            "freestanding-profile/top-var-aggregate-reassign: avoids runtime aggregate refs"
        expect_log_not_contains "$FREESTANDING_TOP_VAR_AGG_REASSIGN_C" "#include \"xrt.h\"" \
            "freestanding-profile/top-var-aggregate-reassign: avoids hosted umbrella"
    else
        record_fail "freestanding-profile/top-var-aggregate-reassign: kept C source missing"
        sed 's/^/      /' "$FREESTANDING_TOP_VAR_AGG_REASSIGN_LOG" | sed -n '1,120p'
    fi
    FREESTANDING_TOP_VAR_AGG_REASSIGN_UNDEFINED="$(
        nm_undefined_normalized "$FREESTANDING_TOP_VAR_AGG_REASSIGN_OBJ")"
    FREESTANDING_TOP_VAR_AGG_REASSIGN_UNEXPECTED="$(
        printf '%s\n' "$FREESTANDING_TOP_VAR_AGG_REASSIGN_UNDEFINED" |
            sed '/^[[:space:]]*$/d' |
            grep -Ev '^(memcpy|memmove|memset|memcmp)$' || true)"
    if [ -z "$FREESTANDING_TOP_VAR_AGG_REASSIGN_UNEXPECTED" ]; then
        record_pass "freestanding-profile/top-var-aggregate-reassign: undefined symbols stay in memcpy family"
    else
        record_fail "freestanding-profile/top-var-aggregate-reassign: unexpected undefined symbols"
        printf '%s\n' "$FREESTANDING_TOP_VAR_AGG_REASSIGN_UNEXPECTED" | sed 's/^/      /'
    fi
else
    record_fail "freestanding-profile/top-var-aggregate-reassign: object build failed"
    sed 's/^/      /' "$FREESTANDING_TOP_VAR_AGG_REASSIGN_LOG" | sed -n '1,120p'
fi
...
if "$XRAY" build --native --profile freestanding --shared --keep-c --rebuild \
    ...
    if [ -f "$FREESTANDING_RAWMUT_TOP_VAR_AGG_C" ]; then
        expect_log_contains "$FREESTANDING_RAWMUT_TOP_VAR_AGG_C" \
            "static struct { int64_t f0; int64_t f1; int64_t f2; int64_t f3; } _xctstruct_freestanding_rawmut_of_top_var_aggregate_addr_" \
            "freestanding-profile/rawmut-top-var-aggregate-addr: materializes mutable struct as static data"
        expect_log_contains "$FREESTANDING_RAWMUT_TOP_VAR_AGG_C" ".f0 =" \
...
if "$XRAY" build --native --profile freestanding --shared --keep-c --rebuild \
    ...
    if [ -f "$FREESTANDING_RAWMUT_TOP_VAR_FIELD_C" ]; then
        expect_log_contains "$FREESTANDING_RAWMUT_TOP_VAR_FIELD_C" \
            "static struct { int64_t f0; int64_t f1; int64_t f2; int64_t f3; } _xctstruct_freestanding_rawmut_of_top_var_field_addr_" \
            "freestanding-profile/rawmut-top-var-field-addr: materializes mutable struct as static data"
        expect_log_contains "$FREESTANDING_RAWMUT_TOP_VAR_FIELD_C" ".f1 =" \
        ...
        expect_log_not_contains "$FREESTANDING_RAWMUT_TOP_VAR_FIELD_C" \
            "const struct { int64_t f0; int64_t f1; int64_t f2; int64_t f3; } _xctstruct_freestanding_rawmut_of_top_var_field_addr_" \
...
if "$XRAY" build --native --profile freestanding --shared --keep-c --rebuild \
    ...
    if [ -n "$FREESTANDING_STATIC_IMPORT_EXPORT_C" ] &&
       [ -f "$FREESTANDING_STATIC_IMPORT_EXPORT_C" ]; then
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_EXPORT_C" \
            ...
            "freestanding-profile/static-import: exporter materializes plain tuple const"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_EXPORT_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_EXPORT_C" \
            ...
            "freestanding-profile/static-import: exporter materializes plain string const"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_EXPORT_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_EXPORT_C" \
            'const XrValue xray_const__freestanding_static_data_lib_PLAIN_LABEL_CUBE[2][2][2]' \
            "freestanding-profile/static-import: exporter materializes plain string fixed-array cube const"
    else
    ...
    if [ -n "$FREESTANDING_STATIC_IMPORT_ENTRY_C" ] &&
       [ -f "$FREESTANDING_STATIC_IMPORT_ENTRY_C" ]; then
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            ...
            "freestanding-profile/static-import: importer declares plain tuple const extern"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            ...
            "freestanding-profile/static-import: importer declares plain string const extern"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            'extern const XrValue xray_const__freestanding_static_data_lib_PLAIN_LABEL_CUBE[2][2][2]' \
            "freestanding-profile/static-import: importer declares plain string fixed-array cube const extern"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            ...
            "freestanding-profile/static-import: importer reads plain tuple const field directly"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
        ...
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            ...
            "freestanding-profile/static-import: importer reads plain string const directly"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            'xray_const__freestanding_static_data_lib_PLAIN_LABELED.f0' \
            "freestanding-profile/static-import: importer reads plain string tuple lane directly"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
            'xray_const__freestanding_static_data_lib_PLAIN_LABELED.f1' \
            "freestanding-profile/static-import: importer reads plain string tuple scalar lane directly"
        expect_log_contains "$FREESTANDING_STATIC_IMPORT_ENTRY_C" \
    ...
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_PAIR" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain tuple data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain tuple data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_BYTES" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain fixed-array data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain fixed-array data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_MATRIX" \
    ...
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_CUBE" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain fixed-array cube data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain fixed-array cube data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
        ...
        record_fail "freestanding-profile/static-import: weak plain union data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_WORD" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain union fixed-array data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain union fixed-array data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_LABEL" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain string data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain string data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_LABELED" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain string tuple data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain string tuple data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
    ...
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
            "xray_const__freestanding_static_data_lib_PLAIN_LABEL_CUBE" \
            "$FREESTANDING_STATIC_IMPORT_NM"; then
        record_pass "freestanding-profile/static-import: weak plain string fixed-array cube data symbol is external"
    else
        record_fail "freestanding-profile/static-import: weak plain string fixed-array cube data symbol missing"
        sed 's/^/      /' "$FREESTANDING_STATIC_IMPORT_NM" | sed -n '1,80p'
    fi
    if object_has_weak_symbol "$FREESTANDING_STATIC_IMPORT_OBJ" \
