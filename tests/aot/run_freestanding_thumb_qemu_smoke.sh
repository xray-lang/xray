...
fn kernel_entry() -> int32 {
    mem.volatileStore(mem.fromAddress(MPS2_LEDS), 0x5a, 4)
    mem.volatileStore(mem.fromAddress(UART0_DATA), 'M' as int, 4)
    mem.volatileStore(mem.fromAddress(UART0_DATA), '4' as int, 4)
    mem.volatileStore(mem.fromAddress(UART0_DATA), 10, 4)
    while (true) {
    }
    return 0
}
