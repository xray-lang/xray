...
static void fill_bag_init(FillBag *bag) {
    bag->values = int_array_new_uninit(0);
}

static int64_t fill_bag_fill(FillBag *bag, int64_t n, int64_t rounds) {
...
static int64_t class_array_field_fill_loop_run(int64_t rounds) {
    FillBag bag;
    fill_bag_init(&bag);
    return fill_bag_fill(&bag, 256, rounds);
}

int main(void) {
    printf("%lld\n", (long long) class_array_field_fill_loop_run(20000));
    return 0;
}
