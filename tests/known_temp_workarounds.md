...
| throw-heap-type | `src/jit/xm_jit_runtime.c` `xr_jit_throw` | `9da287d` | JIT-internal XrValue uses a sparse descriptor where `heap_type` may be 0 and is rebuilt on demand; VM expects a complete `heap_type`. Without backfill `xr_is_exception` sees `heap_type=0` for a real Exception and the value is silently re-wrapped | Unify the XrValue descriptor contract so JIT-internal values carry a complete `heap_type` by construction, enforced by `XR_DCHECK` at every cross-tier CALL_C boundary; the explicit rebuild then becomes a no-op the compiler can elide | xingleixu | 2026-05-13 | After the cross-tier XrValue descriptor contract is enforced |
