...
static void test_determinism_roundtrip_and_identity(void) {
    ...
    CHECK(first.bytes[10] == 11u && first.bytes[11] == 0u);
    ...
    memcpy(previous_f64_vector, message_vector, sizeof(previous_f64_vector));
    ...
    xr_program_compute_id(message_vector, sizeof(message_vector), &previous_id);
    xr_program_id_hex(previous_id, previous_hex);
    CHECK(strcmp(previous_hex, "456a1708865174e260191ed7f1654a4efa8f98cba831b31461f858f2f995b354") == 0);
    /* This scalar-only fixed wire gains only the new minor version. Class
     * parent rows have separate exact-byte and hostile-wire assertions. */
    message_vector[10] = 10u;
    /* The registry adds Channel construction and acquire-state observation. */
    static const uint8_t channel_core_spec[32] = {0xe8,0xae,0x1d,0x46,0x1b,0x9b,0xa5,0xe0,0xec,0x96,0x20,0x86,0x06,0x08,0x1a,0x4a,0x77,0xba,0x0f,0x7e,0xea,0xad,0x93,0xcf,0x29,0x30,0x83,0x9c,0x08,0x10,0x1b,0x7b};
    memcpy(message_vector + 13u, channel_core_spec, sizeof(channel_core_spec));
    /* The unchanged scalar wire binds the new integer bit-operation semantics. */
    static const uint8_t bitwise_core_spec[32] = {0xf5,0x57,0x92,0x4b,0xee,0x5d,0xc1,0x1c,0xde,0x3e,0x3e,0x07,0x75,0xfe,0xc7,0x42,0xf3,0x57,0x1d,0x7e,0x42,0xd5,0xce,0x56,0x64,0x82,0xd3,0x36,0x48,0x60,0xff,0xbc};
    memcpy(message_vector + 13u, bitwise_core_spec, sizeof(bitwise_core_spec));
    /* The scalar wire remains unchanged; the registry adds owned scalar slicing. */
    static const uint8_t slice_core_spec[32] = {0x39,0xd2,0x16,0x59,0xb8,0x38,0xf7,0xda,0xa2,0x88,0x1f,0xc8,0x60,0x9b,0xb2,0x69,0x0b,0x21,0xbc,0x25,0xdc,0x9a,0xf2,0xd4,0x38,0xdf,0xe3,0x31,0xe1,0xc4,0xb3,0x34};
    memcpy(message_vector + 13u, slice_core_spec, sizeof(slice_core_spec));
    /* The fixed scalar wire binds runtime-length default array allocation. */
    static const uint8_t array_default_core_spec[32] = {0x8d,0xd2,0x97,0x61,0x6b,0x54,0xe8,0x5f,0x50,0x1b,0x4f,0x68,0x50,0xa2,0xaf,0x35,0x0b,0xa1,0xd8,0xa1,0x80,0x23,0x30,0x94,0x81,0x58,0xcb,0x6e,0xc8,0x5a,0xa5,0xd9};
    memcpy(message_vector + 13u, array_default_core_spec, sizeof(array_default_core_spec));
    /* Independent normative wire: builtin 20 is affine and cannot be copied. */
    static const uint8_t builder_vector[505] = {
        ,
    };
    /* The scalar wire is unchanged; only the added pure byte operation changes
     * its semantic registry binding. The previous literal remains an oracle. */
    uint8_t byte_compare_vector[sizeof(builder_vector)];
    memcpy(byte_compare_vector, builder_vector, sizeof(builder_vector));
    static const uint8_t byte_compare_core[32] = {0x6b,0xd4,0x31,0xf9,0x00,0x83,0xd5,0x5f,0x13,0xac,0x84,0x9c,0x0c,0x71,0x5a,0x2b,0x42,0xb8,0x0d,0x5a,0x48,0x2c,0xcf,0xa2,0x29,0x73,0x10,0x8d,0xd2,0x5c,0x32,0xf8};
    memcpy(byte_compare_vector + 13u, byte_compare_core, sizeof(byte_compare_core));
...
static void test_hostile_structure_and_budget(void) {
    ...
    CHECK(xr_program_decode_structure(artifact.bytes, artifact.size, NULL, &valid_view, diagnostic,
                                      sizeof(diagnostic)) == XR_PROGRAM_DECODE_OK);
    /* Every builtin row has exact identity, kind and ownership.
     * Changing any field must not reinterpret a scalar as a dynamic type. */
    for (size_t row = 0u; row < 20u; ++row) {
        size_t row_offset =
    ...
    memcpy(mutated, artifact.bytes, artifact.size);
    mutated[XR_PROGRAM_MAGIC_SIZE + 2u] = 10u;
...
int main(void) {
    test_parent_types();
    test_parent_type_cycle_wire_and_allocations();
    test_parent_type_requires_nominal_target_wire();
    ...
    test_channel_types();
    test_channel_type_allocation_failures();
