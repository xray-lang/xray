 ...
 *
 * xr_program_array_type_checks.inc.c - Independent array type wire and ownership checks
 */

static void array_types_init(XrCoreIrTypeInput types[3]) {
    memset(types, 0, 3u * sizeof(*types));
    for (uint32_t index = 0u; index < 3u; ++index) {
        types[index].local_id = (uint16_t) (90u + index);
        types[index].kind = XR_CORE_IR_TYPE_ARRAY;
        types[index].ownership = XR_CORE_IR_TYPE_OWNERSHIP_AFFINE;
        types[index].copy_contract = XR_CORE_IR_COPY_EXPLICIT;
    }
    types[0].key.bytes[0] = 3u;
    types[0].array_element_type = XR_CORE_TYPE_STRING;
    types[1].key.bytes[0] = 1u;
    types[1].array_element_type = 90u;
    types[2].key.bytes[0] = 2u;
    types[2].array_element_type = XR_CORE_TYPE_PANIC_INFO;
    types[2].copy_contract = XR_CORE_IR_COPY_FORBIDDEN;
}

static XrProgramBuildStatus write_array_types(const XrCoreIrTypeInput *types,
...
static void test_array_type_roundtrip(void) {
    XrCoreIrTypeInput types[3];
    array_types_init(types);
    XrProgramArtifact artifact = {0}, reordered = {0};
    CHECK(write_array_types(types, &artifact) == XR_PROGRAM_BUILD_OK);
    XrCoreIrTypeInput temp = types[0];
    types[0] = types[2];
    types[2] = temp;
    CHECK(write_array_types(types, &reordered) == XR_PROGRAM_BUILD_OK);
    ...
    if (validated) {
        ...
        CHECK(validated->types[0].kind == XR_CORE_IR_TYPE_ARRAY);
        CHECK(validated->types[0].array_element_type == 34u);
        CHECK(validated->types[1].array_element_type == XR_CORE_TYPE_PANIC_INFO);
        CHECK(validated->types[1].copy_contract == XR_CORE_IR_COPY_FORBIDDEN);
        CHECK(validated->types[2].array_element_type == XR_CORE_TYPE_STRING);
...
static void test_array_type_invalid_construction(void) {
    for (uint32_t mutation = 0u; mutation < 12u; ++mutation) {
        XrCoreIrTypeInput types[3];
        array_types_init(types);
        uint16_t field = XR_CORE_TYPE_I64;
        switch (mutation) {
            case 0u: types[0].array_element_type = XR_CORE_TYPE_VOID; break;
            case 1u: types[0].array_element_type = XR_CORE_TYPE_TYPE_VARIABLE; break;
            case 2u: types[0].array_element_type = UINT16_MAX; break;
            ...
            case 6u: types[2].copy_contract = XR_CORE_IR_COPY_EXPLICIT; break;
            case 7u: types[0].copy_contract = XR_CORE_IR_COPY_FORBIDDEN; break;
            case 8u: types[0].field_types = &field; types[0].field_count = 1u; break;
            case 9u: types[0].view_element_type = XR_CORE_TYPE_I64; break;
            case 10u: types[0].array_element_type = 92u; break;
            case 11u: types[0].kind = XR_CORE_IR_TYPE_AGGREGATE; break;
        }
        XrProgramArtifact artifact = {0};
        XrProgramBuildStatus expected = mutation == 1u || mutation == 2u
            ? XR_PROGRAM_BUILD_UNRESOLVED_REFERENCE : XR_PROGRAM_BUILD_INVALID_INPUT;
        CHECK(write_array_types(types, &artifact) == expected);
        CHECK(artifact.bytes == NULL);
        xr_program_artifact_free(&artifact);
    }
}

static void test_array_type_hostile_wire(void) {
    XrCoreIrTypeInput types[3];
    array_types_init(types);
    XrProgramArtifact artifact = {0};
    CHECK(write_array_types(types, &artifact) == XR_PROGRAM_BUILD_OK);
    ...
    const uint8_t first[38] = {32u, 38u, 1u, 1u, 1u, [36] = 34u};
    ...
    for (uint32_t mutation = 0u; mutation < 7u; ++mutation) {
        ...
        switch (mutation) {
            ...
            case 5u: mutated[row + 38u + 3u] = 1u; break;
        ...
        CHECK(structural == (mutation < 4u ? XR_PROGRAM_DECODE_NONCANONICAL
                                           : XR_PROGRAM_DECODE_OK));
        XrValidatedProgram *validated = NULL;
...
static void test_array_type_allocation_failures(void) {
    XrCoreIrTypeInput types[3];
    array_types_init(types);
    ...
    for (size_t failure = 1u; failure < 256u; ++failure) {
        ...
        XrProgramBuildStatus status = write_array_types(types, &artifact);
        if (status != XR_PROGRAM_BUILD_OK)
    ...
    printf("Array type constructor/writer allocation points: %zu\n", allocations);
    XrProgramArtifact artifact = {0};
    CHECK(write_array_types(types, &artifact) == XR_PROGRAM_BUILD_OK);
    if (!artifact.bytes)
        return;
    ...
    CHECK(allocations != 0u && allocations < 255u);
    printf("Array type verifier allocation points: %zu\n", allocations);
    xr_program_artifact_free(&artifact);
}
