...
static int add_breakpoint(XrDebugState *state, const char *path, int line,
                          const char *condition, const char *log_msg, const char *hit_cond) {
    ...
    bp->line = line;
    bp->enabled = true;
    if (condition) bp->condition = strdup(condition);
    if (log_msg) bp->log_message = strdup(log_msg);
    if (hit_cond) bp->hit_condition = strdup(hit_cond);
