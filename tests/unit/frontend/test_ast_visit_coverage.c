...
static bool is_expression_node(AstNodeType t) {
    ...
    case AST_LITERAL_STRING: case AST_LITERAL_REGEX:
    case AST_LITERAL_NULL: case AST_LITERAL_TRUE: case AST_LITERAL_FALSE:
    /* Binary */
    ...
    /* Ternary / null */
    case AST_TERNARY: case AST_NULLISH_COALESCE:
    case AST_OPTIONAL_CHAIN: case AST_FORCE_UNWRAP:
    case AST_AS_EXPR: case AST_IS_EXPR: case AST_RANGE:
    /* Unary */
    ...
    /* Call / member / index */
    case AST_CALL_EXPR: case AST_MEMBER_ACCESS: case AST_INDEX_GET:
    case AST_INDEX_SET: case AST_SLICE_EXPR: case AST_MEMBER_SET:
    /* Literals (aggregate) */
    case AST_ARRAY_LITERAL: case AST_MAP_LITERAL: case AST_SET_LITERAL:
    case AST_OBJECT_LITERAL: case AST_STRUCT_LITERAL:
    case AST_TEMPLATE_STRING:
    /* OOP */
    case AST_NEW_EXPR: case AST_THIS_EXPR: case AST_SUPER_CALL:
    /* Enum */
    case AST_ENUM_ACCESS: case AST_ENUM_CONVERT: case AST_ENUM_INDEX:
    /* Coroutine */
    case AST_GO_EXPR: case AST_AWAIT_EXPR:
    case AST_CHANNEL_NEW: case AST_CHAN_SEND: case AST_CHAN_RECV:
    case AST_CANCELLED_EXPR: case AST_MOVE_EXPR:
    /* Match */
    case AST_MATCH_EXPR:
    /* Function expression */
...
static void check_node(CoverageCtx *ctx, AstNode *node) {
        ...
        if (!t) {
            ...
            if (ctx->logged < ctx->max_missing_log) {
                fprintf(stderr, "    MISSING TYPE: node_type=%d node_id=%u line=%d\n",
                        node->type, node->node_id, (int)node->line);
                ctx->logged++;
            }
        }
    }
    ...
    case AST_MULTI_VAR_DECL:
        for (int i = 0; i < node->as.multi_var_decl.value_count; i++)
            check_node(ctx, node->as.multi_var_decl.values[i]);
        break;

    case AST_MULTI_ASSIGN:
        for (int i = 0; i < node->as.multi_assign.target_count; i++)
            check_node(ctx, node->as.multi_assign.targets[i]);
        for (int i = 0; i < node->as.multi_assign.value_count; i++)
            check_node(ctx, node->as.multi_assign.values[i]);
        break;
    ...
    case AST_TYPE_ALIAS: case AST_INTERFACE_DECL: case AST_INTERFACE_METHOD:
...
static bool assert_all_typed(const char *source, const char *label) {
    ...
    if (ctx.missing_type > 0) {
        fprintf(stderr, "  [%s] FAIL: %d/%d expression nodes missing type\n",
                label, ctx.missing_type, ctx.total_expr);
        ok = false;
    } else {
