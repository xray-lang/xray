...
static bool verify_parent_chain(XaScope *scope, XaScope *expected_parent) {
    if (!scope)
        return true;
    if (scope->parent != expected_parent)
        return false;
    for (int i = 0; i < scope->child_count; i++) {
...
TEST(function_creates_child_scope) {
    AnalysisResult r = analyze_source("fn foo(a: i64) -> i64 {\n"
                                      "    var b = a + 1\n"
                                      "    return b\n"
                                      "}\n");
...
TEST(while_in_function_creates_scope) {
    ...
    AnalysisResult r = analyze_source("fn loop_fn() -> i64 {\n"
                                      "    var i = 0\n"
                                      "    while (i < 10) {\n"
                                      "        var temp = i\n"
                                      "        i += 1\n"
...
TEST(nested_functions_nested_scopes) {
    AnalysisResult r = analyze_source("fn outer() -> i64 {\n"
                                      "    var a = 1\n"
                                      "    fn inner() -> i64 {\n"
                                      "        var b = 2\n"
                                      "        return a + b\n"
...
TEST(function_isolation) {
    ...
    AnalysisResult r = analyze_source("fn foo() -> i64 {\n"
                                      "    var secret = 42\n"
                                      "    return secret\n"
                                      "}\n"
                                      "var result = foo()\n");
...
TEST(parent_chain_valid) {
    AnalysisResult r = analyze_source("fn foo(x: i64) -> i64 {\n"
                                      "    if (x > 0) {\n"
                                      "        var y = x\n"
                                      "        return y\n"
                                      "    }\n"
...
TEST(class_creates_class_scope) {
    AnalysisResult r = analyze_source("class Foo {\n"
                                      "    x: i64\n"
                                      "    init(val: i64) {\n"
                                      "        this.x = val\n"
                                      "    }\n"
...
static bool scopes_well_formed(XaScope *scope) {
    ...
    for (int i = 0; i < scope->child_count; i++) {
        if (!scope->children[i])
            return false;
        if (!scopes_well_formed(scope->children[i]))
            return false;
    }
    return true;
}
...
TEST(scope_consistency_after_analysis) {
    ...
    /* 2. No scope aliasing */
    assert(scopes_unique(global) && "all scopes must be distinct objects");

    /* 3. Well-formed */
    assert(scopes_well_formed(global) && "all scopes must have valid child arrays");
...
TEST(reanalysis_scope_stability) {
    ...
    const char *source = "fn foo(x: i64) -> i64 {\n"
                         "    var y = x + 1\n"
                         "    return y\n"
                         "}\n"
                         "var z = foo(1)\n";
    ...
    assert(count1 == count2 && "repeated analysis must produce identical scope count");
    free_result(&r2);
}
