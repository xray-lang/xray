...
static bool build_native_profile(XaotTarget *aot_target, XrTargetProfile **out) {
    ...
    return xaot_target_profile_codegen_facts(aot_target, &codegen) &&
           xtc_target_profile_build_current_native_hosted(&codegen, out, error, sizeof(error));
}
...
static void test_canonical_native_projection_is_deterministic(void) {
    ...
    char error[256] = {0};
    CHECK(xr_runtime_target_profile_build_native_hosted(&first, error, sizeof(error)));
    CHECK(xr_runtime_target_profile_build_native_hosted(&second, error, sizeof(error)));
    CHECK(first != NULL);
    CHECK(second != NULL);
    ...
    CHECK(!xr_runtime_target_profile_build_native_hosted(NULL, error, sizeof(error)));
...
static void test_native_authority_is_deterministic(void) {
    ...
    const XrTargetMachineFacts *machine = xr_target_profile_machine_facts(first);
    CHECK(machine != NULL);
    CHECK(machine && machine->runtime_profile == XR_TARGET_RUNTIME_PROFILE_HOSTED);
...
static void test_freestanding_authority_is_not_hosted_projection(void) {
    ...
    CHECK(xr_runtime_target_authority_native_hosted(&hosted) == XR_RUNTIME_ABI_OK);
    CHECK(xr_runtime_target_authority_native_freestanding(requested, &freestanding) ==
          XR_RUNTIME_ABI_OK);
    CHECK(xr_runtime_target_authority_native_freestanding(foundation, &minimal) ==
          XR_RUNTIME_ABI_OK);
    CHECK(freestanding.machine.runtime_profile == XR_TARGET_RUNTIME_PROFILE_FREESTANDING);
    CHECK(freestanding.provider_count == 3);
    CHECK(minimal.provider_count == 2);
    CHECK(xr_target_provider_set_fingerprint(hosted.providers, hosted.provider_count, &hosted_mask,
    ...
    CHECK(xtc_target_profile_build_native_hosted(&native_target, &codegen, &hosted_profile, error,
                                                 sizeof(error)));
    CHECK(xtc_target_profile_build_native_freestanding(
        &native_target, &codegen, requested, &freestanding_profile, error, sizeof(error)));
    CHECK(hosted_profile != NULL);
    CHECK(freestanding_profile != NULL);
    if (hosted_profile && freestanding_profile) {
        CHECK(!xr_fingerprint_equal(xr_target_profile_fingerprint(hosted_profile),
                                    xr_target_profile_fingerprint(freestanding_profile)));
        CHECK(!xr_target_profile_require_exact(hosted_profile, freestanding_profile, NULL, 0));
    }
...
static void test_cross_target_and_reserved_facts_fail_closed(void) {
    ...
    CHECK(!xtc_target_profile_build_native_hosted(&native_target, &codegen, &profile, error,
                                                  sizeof(error)));
    CHECK(profile == NULL);
    ...
    for (size_t i = 0; i < count; i++) {
        if (xtc_target_parse(names[i], &cross_target, NULL, 0) && !cross_target.is_native) {
            found_cross = true;
            break;
        }
    }
    CHECK(found_cross);
    CHECK(found_cross && !xtc_target_profile_build_native_hosted(&cross_target, &codegen, &profile,
                                                                 error, sizeof(error)));
    CHECK(profile == NULL);
    xaot_target_free(&aot_target);
}
...
static void test_compiler_session_retains_exact_profile(void) {
    ...
    profile = NULL;
    const XrTargetProfile *installed = xr_compiler_session_target_profile(session);
    CHECK(installed != NULL);
    CHECK(installed && xr_target_profile_verify(installed, NULL, 0));
    CHECK(installed && memcmp(xr_compiler_session_target_data_layout(session),
...
static void test_compiler_session_rejects_conflicting_layout_authority(void) {
    ...
    CHECK(xaot_target_init(&aot_target, NULL));
    CHECK(build_native_profile(&aot_target, &profile));
    CHECK(sizeof(void *) == 8 ? xr_target_data_layout_init_ilp32(&conflicting_layout)
...
static void test_compiler_session_rejects_conflicting_explicit_profile(void) {
    ...
    CHECK(xr_runtime_target_profile_build_native_hosted(&hosted, error, sizeof(error)));
    CHECK(xtc_target_profile_build_native_freestanding(&native_target, &codegen, providers,
                                                       &freestanding, error, sizeof(error)));
    XrCompilerSessionConfig config = {.target_profile = hosted};
    XrCompilerSession *session = xr_compiler_session_new(&config);
    CHECK(session != NULL);
    CHECK(session && !xr_compiler_session_set_target_profile(session, freestanding));
    CHECK(session &&
          xr_target_profile_require_exact(hosted, xr_compiler_session_target_profile(session),
                                          error, sizeof(error)));
    xr_compiler_session_delete(session);
