 ...
 *
 * test_xi_to_xir_e2e.c - End-to-end test: Xi IR → XIR → codegen → execute
 *
 * Validates that xi_to_xir_lower produces XIR that survives the full
...
TEST(add_i64) {
    ...
    xir_run_pipeline(xir, XIR_OPT_BASIC);

    /* Generate machine code */
    XirCodeAlloc alloc;
    xir_code_alloc_init(&alloc);
    XirCodegenResult res = xir_codegen_native(xir, &alloc);
...
int main(void) {
    env_init();
    fprintf(stderr, "=== Xi-to-XIR E2E Tests ===\n");
