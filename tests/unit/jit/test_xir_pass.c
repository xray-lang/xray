...
/*
 * Test: BR with constant true condition → JMP to true branch.
 *
 *   entry:
 *     v0 = CONST_I64(1)   ; true
 *     BR v0, then, else
...
/*
 * Test: BR with constant false condition → JMP to false branch.
 */
...
/*
 * Test: BR with both targets the same → JMP.
 */
 ...
 *   → entry: JMP then   (else becomes unreachable)
 *   → remove_unreachable: nblk decreases
 */
...
/*
 * Test: Trivial phi where all args are the same → MOV.
 *
 *   header:
...
/*
 * Test: Two blocks A→B where B has single pred → merged into one block.
 *
 *   A: v0 = CONST_I64(1)
 *      JMP B
 *   B: v1 = CONST_I64(2)
 ...
 *     RET v2
 *
 * After pipeline: entry → JMP then, else block removed.
 */
 ...
 *
 *   Block 0 (entry → JMP b1):
 *     v0 = CONST_I64(10)
 *     v1 = CONST_I64(20)
 *     v2 = ADD v0, v1      // canonical order
 *
 *   Block 1:
 *     v3 = ADD v1, v0      // reversed order — should be CSE'd by GVN
 *     v4 = ADD v2, v3
 *     RET v4
 *
...
static void test_gvn_normalize_commutative(void) {
    ...
    /* Reversed arg order — should normalize to same hash */
...
/*
 * Test: Two consecutive STORE_FIELD to same (obj, offset) — first is dead.
 *
...
/*
 * Test: STORE, LOAD, STORE — first store is NOT dead (it was read).
 *
...
static void test_dse_store_load_store(void) {
    ...
    /* First store should still be STORE_FIELD (not dead — was read by LOAD) */
 ...
 *
 * CFG: entry(0) → header(1) → body(2) → header(1)  [back-edge]
 *                  header(1) → exit(3)
 *
 ...
 *
 * CFG: entry(0) → outer_hdr(1) → inner_hdr(2) → inner_body(3) → inner_hdr(2)
 *                                  inner_hdr(2) → outer_body(4) → outer_hdr(1)
 *                  outer_hdr(1) → exit(5)
 *
 * Expected loop_depth:
...
static void test_licm_loop_depth(void) {
    ...
    /* outer_hdr: BR → inner_hdr or exit */
    ...
    /* inner_hdr: BR → inner_body or outer_body */
    ...
    /* inner_body → inner_hdr (inner back-edge) */
    ...
    /* outer_body → outer_hdr (outer back-edge) */
 ...
 *
 * CFG: entry(0): BR(cond) → then(1), else(2)
