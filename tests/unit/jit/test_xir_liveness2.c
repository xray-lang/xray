 ...
 *
 * test_xir_liveness2.c - Unit tests for dataflow-based liveness analysis
 */
...
static void test_single_block(void) {
    ...
    XirFunc *func = xir_func_new("single");
    ...
    assert(live.nblk == 1);
    assert(xir_bset_empty(&live.blocks[0].live_in));
    assert(xir_bset_empty(&live.blocks[0].live_out));
...
static void test_diamond(void) {
    ...
    XirFunc *func = xir_func_new("diamond");
    XirBlock *entry = xir_func_add_block(func, "entry");
    ...
    XirRef c10 = xir_const_i64(func, 10);
    XirRef v1 = xir_emit_unary(func, entry, XIR_CONST_I64, XR_REP_I64, c10);
    ...
    /* then: v2 = v1 + v1 */
    XirRef v2 = xir_emit(func, then_blk, XIR_ADD, XR_REP_I64, v1, v1);
    xir_block_set_jmp(then_blk, merge);
    xir_block_add_pred(merge, then_blk, func->arena);

    /* else: v3 = 99 */
    XirRef c99 = xir_const_i64(func, 99);
    XirRef v3 = xir_emit_unary(func, else_blk, XIR_CONST_I64, XR_REP_I64, c99);
    xir_block_set_jmp(else_blk, merge);
    xir_block_add_pred(merge, else_blk, func->arena);

    /* merge: phi(v2, v3), ret */
    XirPhi *phi = xir_add_phi(func, merge, XR_REP_I64);
    xir_phi_set_arg(phi, 0, v2);  /* from then */
    xir_phi_set_arg(phi, 1, v3);  /* from else */
    ...
    uint32_t v1_idx = XIR_REF_INDEX(v1);
    assert(xir_live_at_exit(&live, 0, v1_idx));

    /* v1 should be live-in of then block */
    assert(xir_live_at_entry(&live, 1, v1_idx));

    /* v1 should NOT be live-in of else block (not used there) */
    assert(!xir_live_at_entry(&live, 2, v1_idx));

    /* v2 should be live-out of then (phi arg in merge) */
    uint32_t v2_idx = XIR_REF_INDEX(v2);
    assert(xir_live_at_exit(&live, 1, v2_idx));

    /* v3 should be live-out of else (phi arg in merge) */
    uint32_t v3_idx = XIR_REF_INDEX(v3);
    assert(xir_live_at_exit(&live, 2, v3_idx));

    /* merge live_in should have phi->dst (from phi def) but
     * NOT v2 or v3 (those come via phi, not direct use) */
    assert(!xir_live_at_entry(&live, 3, v2_idx));
    assert(!xir_live_at_entry(&live, 3, v3_idx));
...
static void test_loop(void) {
    ...
    XirFunc *func = xir_func_new("loop");
    XirBlock *entry = xir_func_add_block(func, "entry");
    XirBlock *header = xir_func_add_block(func, "header");
    ...
    xir_block_add_pred(header, entry, func->arena);  /* pred 0 */
    xir_block_add_pred(header, body, func->arena);   /* pred 1 (back-edge) */
    xir_block_add_pred(body, header, func->arena);
    xir_block_add_pred(exit_blk, header, func->arena);
    ...
    XirPhi *phi_i = xir_add_phi(func, header, XR_REP_I64);
    xir_phi_set_arg(phi_i, 0, v0);

    XirRef v2 = xir_emit(func, header, XIR_LT, XR_REP_I64, phi_i->dst, v1);
    xir_block_set_br(header, v2, body, exit_blk);

    /* body: i + 1, jump back */
    XirRef c1 = xir_const_i64(func, 1);
    XirRef vc1 = xir_emit_unary(func, body, XIR_CONST_I64, XR_REP_I64, c1);
    XirRef v3 = xir_emit(func, body, XIR_ADD, XR_REP_I64, phi_i->dst, vc1);
    xir_block_set_jmp(body, header);
    xir_phi_set_arg(phi_i, 1, v3);  /* back-edge */

    /* exit: return phi_i */
    xir_block_set_ret(exit_blk, phi_i->dst);

    XirLive live;
    xir_live_compute(&live, func);

    uint32_t v1_idx = XIR_REF_INDEX(v1);
    uint32_t phi_idx = XIR_REF_INDEX(phi_i->dst);
...
static void test_dead_var(void) {
    ...
    XirFunc *func = xir_func_new("dead");
    XirBlock *entry = xir_func_add_block(func, "entry");
    ...
    uint32_t v0_idx = XIR_REF_INDEX(v0);
    assert(!xir_live_at_entry(&live, 0, v0_idx));
    assert(!xir_live_at_exit(&live, 0, v0_idx));
...
/*
 * Test 5: Chain of blocks — value passes through middle block.
 *
 *   entry:
 *     v0 = CONST_I64(42)
 *     JMP mid
...
static void test_passthrough(void) {
    ...
    XirFunc *func = xir_func_new("passthru");
    XirBlock *entry = xir_func_add_block(func, "entry");
    XirBlock *mid = xir_func_add_block(func, "mid");
    XirBlock *tail = xir_func_add_block(func, "tail");

    XirRef c42 = xir_const_i64(func, 42);
    XirRef v0 = xir_emit_unary(func, entry, XIR_CONST_I64, XR_REP_I64, c42);
    xir_block_set_jmp(entry, mid);
    xir_block_add_pred(mid, entry, func->arena);

    XirRef c0 = xir_const_i64(func, 0);
    XirRef v1 = xir_emit_unary(func, mid, XIR_CONST_I64, XR_REP_I64, c0);
    (void)v1;
    xir_block_set_jmp(mid, tail);
    xir_block_add_pred(tail, mid, func->arena);

    xir_block_set_ret(tail, v0);
    ...
    assert(xir_live_at_entry(&live, 1, v0_idx));
    assert(xir_live_at_exit(&live, 1, v0_idx));

    /* v0 should be live-in of tail (used in RET) */
    assert(xir_live_at_entry(&live, 2, v0_idx));
