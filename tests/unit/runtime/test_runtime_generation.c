...
static bool ensure_fixture_module_identity(XiFunc *root) {
    ...
    if (!root->module) {
        root->module = xi_module_new("fixture/runtime_generation.xr",
                                     root->name ? root->name : "runtime_generation_fixture", root);
        if (!root->module)
            return false;
    }
...
static XrTargetPlan *finish_plan(XiFunc *root, XrSemanticPlan **semantic_out,
                                 XrTargetProfile **profile_out) {
    XrSemanticPlan *semantic = NULL;
    char diagnostic[512] = {0};
    REQUIRE(ensure_fixture_module_identity(root));
    REQUIRE(xr_semantic_plan_build(root, &semantic, diagnostic, sizeof(diagnostic)));
    xi_func_free(root);

    XrRuntimeTargetAuthority native;
    REQUIRE(xr_runtime_target_authority_native_hosted(&native) == XR_RUNTIME_ABI_OK);
    XrTargetProfileBuildInput input = {
        .machine = native.machine,
        .runtime_abi = &native.runtime_abi,
        .object_header_materialization = &native.object_header_materialization,
    ...
    bool targeted = xr_target_plan_build(semantic, profile, &plan, diagnostic, sizeof(diagnostic));
    if (!targeted)
...
static XrTargetPlan *build_plan(XrSemanticPlan **semantic_out, XrTargetProfile **profile_out) {
...
static XrTargetPlan *build_assertion_plan(XrSemanticPlan **semantic_out,
                                          XrTargetProfile **profile_out) {
    XiFunc *function = xi_func_new("generation_assertion_probe", &stub_unit);
...
static XrTargetPlan *build_unsupported_plan(XrSemanticPlan **semantic_out,
                                            XrTargetProfile **profile_out) {
    ...
    XiValue *result = xi_binary(function, entry, XI_BIT_ROTL, &stub_int, left, right);
...
static XrTargetPlan *build_divide_by_zero_plan(XrSemanticPlan **semantic_out,
                                               XrTargetProfile **profile_out) {
    ...
    XiValue *result = xi_binary(function, entry, XI_DIV, &stub_int, left, right);
    REQUIRE(left != NULL && right != NULL && result != NULL);
    xi_block_set_return(entry, result);
    function->stage = XI_STAGE_OPTIMIZED;
    return finish_plan(function, semantic_out, profile_out);
...
static XrTargetPlan *build_multi_function_plan(XrSemanticPlan **semantic_out,
                                               XrTargetProfile **profile_out) {
...
static void test_scalar_generation_lifecycle(XrTargetPlan *plan) {
    ...
    REQUIRE(result == 42);
    REQUIRE(xr_module_generation_snapshot(generation, &after));
    REQUIRE(after.state == XR_MODULE_GENERATION_ACTIVE && after.total_pins == 0 &&
    ...
    REQUIRE(xr_module_generation_snapshot(generation, &after));
    REQUIRE(after.total_pins == 1 && after.pins_by_kind[XR_MODULE_GENERATION_INFLIGHT_CALL] == 1);
...
static void test_unsupported_generation_remains_verified(XrTargetPlan *plan) {
    char diagnostic[512] = {0};
    XrRuntimeGenerationBudget budget = make_budget(2);
    XrRuntimeGenerationAuthority *authority = NULL;
    REQUIRE(xr_runtime_generation_authority_create(&budget, &authority, diagnostic,
...
static void test_multi_function_generation_is_typed_but_not_sole(XrTargetPlan *plan) {
    char diagnostic[512] = {0};
    XrRuntimeGenerationBudget budget = make_budget(2);
    XrRuntimeGenerationAuthority *authority = NULL;
    REQUIRE(xr_runtime_generation_authority_create(&budget, &authority, diagnostic,
...
static void test_divide_by_zero_generation_fails_at_execution(XrTargetPlan *plan) {
    char diagnostic[512] = {0};
...
static void seed_snapshot(XrModuleGenerationSnapshot *snapshot, XrModuleGenerationState state,
                          uint64_t revision) {
    ...
    for (size_t i = 0; i < sizeof(snapshot->identity.generation_fingerprint); i++)
        snapshot->identity.generation_fingerprint[i] = (uint8_t) (i + 1u);
    snapshot->state = state;
    snapshot->revision = revision;
}

static void expect_transition_rejected(const XrModuleGenerationSnapshot *before,
                                       const XrModuleGenerationSnapshot *after,
                                       XrModuleGenerationMutation mutation,
                                       XrModuleGenerationPinKind pin_kind) {
    char diagnostic[256] = {0};
    REQUIRE(!xr_module_generation_verify_transition(before, after, mutation, pin_kind, diagnostic,
                                                    sizeof(diagnostic)));
    REQUIRE(strstr(diagnostic, "XR_EXEC_5005") != NULL);
}
...
static void test_independent_state_machine_verifier(void) {
    ...
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_VERIFY, XR_MODULE_GENERATION_PIN, diagnostic,
        sizeof(diagnostic)));
    ...
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_PREPARE, XR_MODULE_GENERATION_PIN,
        diagnostic, sizeof(diagnostic)));
    before = after;
    after.state = XR_MODULE_GENERATION_ACTIVE;
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_ACTIVATE, XR_MODULE_GENERATION_PIN,
        diagnostic, sizeof(diagnostic)));
    ...
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_PIN_ACQUIRE, XR_MODULE_GENERATION_CALLBACK,
        diagnostic, sizeof(diagnostic)));
    before = after;
    after.state = XR_MODULE_GENERATION_DRAINING;
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_BEGIN_DRAIN, XR_MODULE_GENERATION_PIN,
        diagnostic, sizeof(diagnostic)));
    before = after;
    after.total_pins--;
    after.pins_by_kind[XR_MODULE_GENERATION_CALLBACK]--;
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_PIN_RELEASE, XR_MODULE_GENERATION_CALLBACK,
        diagnostic, sizeof(diagnostic)));
    before = after;
    after.state = XR_MODULE_GENERATION_RETIRED;
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_RETIRE, XR_MODULE_GENERATION_PIN, diagnostic,
        sizeof(diagnostic)));
    before = after;
    after.state = XR_MODULE_GENERATION_UNLOADED;
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_UNLOAD, XR_MODULE_GENERATION_PIN, diagnostic,
        sizeof(diagnostic)));

    XrModuleGenerationSnapshot invalid = after;
    invalid.identity.generation_number++;
    expect_transition_rejected(&before, &invalid, XR_MODULE_GENERATION_MUTATION_UNLOAD,
    ...
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_POISON, XR_MODULE_GENERATION_PIN, diagnostic,
        sizeof(diagnostic)));
    before = after;
    after.state = XR_MODULE_GENERATION_RETIRED;
    ...
    after.revision++;
    REQUIRE(xr_module_generation_verify_transition(
        &before, &after, XR_MODULE_GENERATION_MUTATION_ROLLBACK, XR_MODULE_GENERATION_PIN,
        diagnostic, sizeof(diagnostic)));
}
...
static void test_assertion_generation_capability_identity(XrTargetPlan *plan) {
    char diagnostic[512] = {0};
    XrRuntimeGenerationBudget budget = make_budget(1);
    XrRuntimeGenerationAuthority *authority = NULL;
    XrLoadedModuleGeneration *generation = NULL;
...
static void *load_generation_thread(void *opaque) {
    ...
    context->loaded = xr_module_generation_load_verified_target_plan(
        context->authority, context->plan, &context->generation, diagnostic, sizeof(diagnostic));
    return NULL;
}
...
static void test_concurrent_generation_budget(XrTargetPlan *plan) {
    ...
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
        ...
        REQUIRE(xr_thread_create(&threads[i], load_generation_thread, &contexts[i]));
    }
    uint32_t loaded = 0;
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
    ...
    for (uint32_t i = 0; i < THREAD_COUNT; i++) {
        if (!contexts[i].generation)
            continue;
        REQUIRE(
            xr_module_generation_rollback(contexts[i].generation, diagnostic, sizeof(diagnostic)));
        REQUIRE(
            xr_module_generation_unload(&contexts[i].generation, diagnostic, sizeof(diagnostic)));
    }
    REQUIRE(xr_runtime_generation_authority_destroy(&authority, diagnostic, sizeof(diagnostic)));
}
...
int main(void) {
    ...
    XrTargetPlan *unsupported = build_unsupported_plan(&unsupported_semantic, &unsupported_profile);
    XrSemanticPlan *zero_semantic = NULL;
    XrTargetProfile *zero_profile = NULL;
    XrTargetPlan *zero = build_divide_by_zero_plan(&zero_semantic, &zero_profile);
    XrSemanticPlan *multi_semantic = NULL;
    XrTargetProfile *multi_profile = NULL;
    XrTargetPlan *multi = build_multi_function_plan(&multi_semantic, &multi_profile);
    XrSemanticPlan *assertion_semantic = NULL;
    XrTargetProfile *assertion_profile = NULL;
    XrTargetPlan *assertion = build_assertion_plan(&assertion_semantic, &assertion_profile);
    ...
    REQUIRE(xr_target_plan_functions(multi, &function_count) != NULL && function_count == 2);
