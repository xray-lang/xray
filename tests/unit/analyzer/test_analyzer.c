...
TEST(analyzer_finalizes_local_fresh_return_ownership_after_body_inference) {
    ...
    TEST(analyzer_slice_mutator_effect_is_independent_of_discarded_result) {
        ...
        const XaMemoryEffectSummary *keep =
            analyzer_function_memory_effect_summary(a, "keepResult");
        ...
        const XaMemoryRootEffect *discard_root =
            memory_effect_root(discard, XA_MEMORY_ROOT_PARAM, 0);
    ...
    TEST(analyzer_allocation_effect_propagates_and_validates_contracts) {
        ...
        const XaAllocationSummary *cycle_entry =
            analyzer_function_allocation_summary(a, "cycleEntry");
        ...
        const XaAllocationSummary *callback_ok =
            analyzer_function_allocation_summary(a, "callbackOk");
        ...
        const XaAllocationSummary *value_error =
            analyzer_function_allocation_summary(a, "valueError");
    ...
    static bool effect_summary_has_enum_named(XaAnalyzer * analyzer, const XaEffectSummary *summary,
        ...
        for (uint32_t i = 0; i < summary->escaping.count; i++) {
            XrType *type = xa_effect_db_error_type_handle(analyzer->effect_db,
    ...
    static const XaErrorTypeSet *effect_summary_enum_set_named(
        XaAnalyzer * analyzer, const XaEffectSummary *summary, const char *name) {
        if (!analyzer || !summary || !name)
            return NULL;
        for (uint32_t i = 0; i < summary->escaping.count; i++) {
            XrType *type = xa_effect_db_error_type_handle(analyzer->effect_db,
    ...
    static void check_throw_effect_consistency(XaAnalyzer * a, const char *name, int expected) {
        ...
        if (expected >= 0 && sym->links.throw_effect != (XrFnThrowEffect) expected) {
            printf("FAILED: '%s' throw effect %d != expected %d\n", name,
                   (int) sym->links.throw_effect, expected);
            tests_failed++;
            return;
        }
    }
    ...
    TEST(analyzer_codegen_controls_are_semantic_neutral_and_type_closed) {
        ...
        ASSERT(a != NULL);
        AstNode *program =
            xr_parse(g_session, "import codegen\n"
    ...
    TEST(analyzer_error_effect_records_direct_throw_variant) {
        ...
        const XaEffectSummary *payload_arg =
            analyzer_function_effect_summary(a, "throwsPayloadArg");
        ...
        const XaErrorTypeSet *specific_set =
            effect_summary_enum_set_named(a, specific, "DirectErr");
    ...
    TEST(analyzer_error_effect_propagates_stable_var_function_values) {
        ...
        const XaEffectSummary *stable_var =
            analyzer_function_effect_summary(a, "viaStableVarAlias");
        const XaEffectSummary *stable_chain =
            analyzer_function_effect_summary(a, "viaStableVarAliasChain");
        const XaEffectSummary *rebound_var =
            analyzer_function_effect_summary(a, "viaReboundVarAlias");
        ...
        const XaEffectSummary *while_union =
            analyzer_function_effect_summary(a, "viaWhileTargetUnion");
        ...
        ASSERT((try_mutated_catch_read->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) !=
               0);
        ASSERT(returned_function_value->error_set_completeness == XA_EFFECT_COMPLETE);
        ASSERT((returned_function_value->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) ==
               0);
        ...
        ASSERT((returned_function_value_base->error_unknown_reasons &
                XA_UNKNOWN_DYNAMIC_CALL_TARGET) == 0);
        ...
        ASSERT((captured_function_value->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) ==
               0);
        ...
        ASSERT((higher_order_callback->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) ==
               0);
        ...
        ASSERT((higher_order_function_expr->error_unknown_reasons &
                XA_UNKNOWN_DYNAMIC_CALL_TARGET) == 0);
    ...
    TEST(analyzer_error_effect_propagates_generic_specialization_target_sets) {
        ...
        const XaEffectSummary *generic_string =
            analyzer_function_effect_summary(a, "viaGenericString");
        const XaEffectSummary *specialized_int =
            analyzer_function_effect_summary(a, "runGeneric$i64");
    ...
    TEST(analyzer_error_effect_propagates_immediate_function_expr_calls) {
        ...
        const XaEffectSummary *rebound =
            analyzer_function_effect_summary(a, "viaReboundStoredLambda");
        const XaEffectSummary *rebound_to_throwing =
    ...
    TEST(analyzer_error_effect_propagates_direct_method_calls) {
        ...
        const XaEffectSummary *temporary =
            analyzer_function_effect_summary(a, "viaTemporaryMethod");
        const XaEffectSummary *static_method =
            analyzer_function_effect_summary(a, "viaStaticMethod");
        const XaEffectSummary *method_hof =
            analyzer_function_effect_summary(a, "viaMethodHigherOrder");
    ...
    TEST(analyzer_error_effect_propagates_module_export_calls) {
        ...
        const char *callback_source =
            "export enum CallbackErr { Foreign, Local }\n"
        ...
        ASSERT(
            xa_analyzer_collect_export_symbols_checked(a, (XrAstNode *) lib_program, &lib_exports));
        ...
        ASSERT(xa_analyzer_collect_export_symbols_checked(a, (XrAstNode *) star_program,
                                                          &star_exports));
        ...
        const XaEffectSummary *star_selective =
            analyzer_function_effect_summary(a, "viaStarSelective");
        ...
        ASSERT((reexported_namespace_hof->error_unknown_reasons &
                XA_UNKNOWN_MISSING_IMPORTED_EFFECT) == 0);
        ASSERT((star_hof->error_unknown_reasons & XA_UNKNOWN_MISSING_IMPORTED_EFFECT) == 0);
        ASSERT((local_callback_hof->error_unknown_reasons & XA_UNKNOWN_MISSING_IMPORTED_EFFECT) ==
               0);
        ASSERT((foreign_callback_hof->error_unknown_reasons & XA_UNKNOWN_MISSING_IMPORTED_EFFECT) ==
               0);
        ASSERT((namespace_callback_hof->error_unknown_reasons &
                XA_UNKNOWN_MISSING_IMPORTED_EFFECT) == 0);
        ...
        ASSERT((reexported_namespace_hof->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) ==
               0);
        ...
        ASSERT((namespace_callback_hof->error_unknown_reasons & XA_UNKNOWN_DYNAMIC_CALL_TARGET) ==
               0);
        ...
        const XaErrorTypeSet *star_hof_set =
            effect_summary_enum_set_named(a, star_hof, "ImportedErr");
    ...
    TEST(analyzer_xrd_signatures_fail_closed_without_typed_contracts) {
        ...
        const XaAllocationSummary *alloc_ok =
            analyzer_function_allocation_summary(a, "allocNativeOk");
    ...
    TEST(analyzer_xrd_native_typed_byte_contracts_reject_legacy_aliases) {
        ...
        snprintf(span_only_path, sizeof(span_only_path), "%s/native_legacy_bytespan_only.xrd",
                 tmpdir);
        snprintf(view_only_path, sizeof(view_only_path), "%s/native_legacy_byteview_only.xrd",
                 tmpdir);
    ...
    TEST(analyzer_error_effect_consumes_builtin_type_member_contracts) {
        ...
        XaBuiltinMember *slice_bytes =
            mutable_builtin_type_member(string_type, "sliceBytes", false);
        ...
        const XaErrorTypeSet *static_set =
            effect_summary_enum_set_named(a, static_call, "Utf8Error");
    ...
    TEST(analyzer_error_effect_subtracts_typed_catches) {
        ...
        const XaEffectSummary *typed_rethrows =
            analyzer_function_effect_summary(a, "typedRethrows");
        ...
        const XaErrorTypeSet *try_inner_alias_set = effect_summary_enum_set_named(
            a, catch_try_inner_alias_does_not_clobber_outer, "CatchErr");
        ...
        const XaErrorTypeSet *reassigned_dynamic_index_set = effect_summary_enum_set_named(
            a, catch_reassigned_dynamic_index_invalidates, "CatchErr");
        ...
        const XaErrorTypeSet *other_field_preserved_set = effect_summary_enum_set_named(
            a, catch_struct_other_field_mutation_preserves, "CatchErr");
    ...
    TEST(analyzer_error_effect_tracks_map_catch_aliases) {
        ...
        const XaEffectSummary *dynamic =
            analyzer_function_effect_summary(a, "mapDynamicKeyRethrows");
        ...
        const XaEffectSummary *mismatch =
            analyzer_function_effect_summary(a, "mapDynamicKeyMismatch");
        ...
        const XaEffectSummary *len_preserve =
            analyzer_function_effect_summary(a, "mapLenPreserves");
        ...
        const XaEffectSummary *delete_singleton_then_set_value = analyzer_function_effect_summary(
            a, "mapDeleteSingletonThenSetCaughtValueIteratorRethrows");
        ...
        const XaErrorTypeSet *invalidated_set =
            effect_summary_enum_set_named(a, invalidated, "MapErr");
        ...
        const XaErrorTypeSet *set_caught_set =
            effect_summary_enum_set_named(a, set_caught, "MapErr");
        ...
        const XaErrorTypeSet *single_key_set =
            effect_summary_enum_set_named(a, single_key, "MapErr");
        ...
        const XaErrorTypeSet *entries_key_set =
            effect_summary_enum_set_named(a, entries_key, "MapErr");
        ...
        const XaErrorTypeSet *clear_keys_set =
            effect_summary_enum_set_named(a, clear_keys, "MapErr");
        ...
        const XaErrorTypeSet *values_iter_set =
            effect_summary_enum_set_named(a, values_iter, "MapErr");
    ...
    TEST(analyzer_error_effect_len_shadow_does_not_preserve_map_provenance) {
        ...
        const XaEffectSummary *summary =
            analyzer_function_effect_summary(a, "mapUserLenInvalidates");
        ASSERT(summary != NULL);
    ...
    TEST(analyzer_error_effect_tracks_set_iterator_catch_aliases) {
        ...
        const XaEffectSummary *dedup =
            analyzer_function_effect_summary(a, "setDedupIteratorRethrows");
        const XaEffectSummary *mixed =
            analyzer_function_effect_summary(a, "setMixedIteratorFallsBack");
        ...
        const XaErrorTypeSet *add_caught_set =
            effect_summary_enum_set_named(a, add_caught, "SetErr");
        ...
        const XaErrorTypeSet *values_view_set =
            effect_summary_enum_set_named(a, values_view, "SetErr");
    ...
    TEST(analyzer_error_effect_converges_recursive_components) {
        ...
        const XaErrorTypeSet *cycle_a_set =
            effect_summary_enum_set_named(a, cycle_a, "RecursiveErr");
        const XaErrorTypeSet *cycle_b_set =
            effect_summary_enum_set_named(a, cycle_b, "RecursiveErr");
        const XaErrorTypeSet *cycle_c_set =
            effect_summary_enum_set_named(a, cycle_c, "RecursiveErr");
        ...
        const XaErrorTypeSet *caught_b_set =
            effect_summary_enum_set_named(a, caught_b, "RecursiveErr");
    ...
    TEST(analyzer_rejects_builtin_generic_arity) {
        ...
        XrCompilerSessionScope scope;
        ASSERT(
            xr_compiler_session_push_arena(g_session, &arena, "builtin_generic_arity.xr", &scope));
        ...
        XrType *bare_iterable =
            xr_tref_resolve_in_analyzer(a, xr_tref_named(g_session, "Iterable"));
    ...
    TEST(analyzer_rejects_generator_yield_value_mismatch) {
        ...
        xr_arena_init(&arena, XR_ARENA_SEGMENT_SIZE);
        XrCompilerSessionScope scope;
        ASSERT(xr_compiler_session_push_arena(g_session, &arena,
                                              "generator_yield_value_mismatch.xr", &scope));
    ...
    TEST(analyzer_type_ref_failures_use_error_recovery) {
        ...
        xr_arena_init(&arena, XR_ARENA_SEGMENT_SIZE);
        XrCompilerSessionScope scope;
        ASSERT(xr_compiler_session_push_arena(g_session, &arena, "type_ref_error_recovery.xr",
                                              &scope));
        ...
        XrTypeRef *invalid_fixed =
            xr_tref_fixed_array_expr(g_session, xr_tref_int(g_session), NULL, 0);
    ...
    TEST(analyzer_container_recovery_rejects_poisoned_success_types) {
        ...
        ASSERT(analyzer_diag_contains(
            a, "cannot infer type arguments for generic constructor 'Array'"));
    ...
    TEST(type_substitute_preserves_function_param_modes) {
        ...
        XrType *actuals[] = {xr_type_new_int(NULL), xr_type_new_string(NULL),
                             xr_type_new_bool(NULL)};
