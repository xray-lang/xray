...
static void test_map_user_hash_eq_wrong_type_falls_back_without_user_calls(void) {
    ...
    xrt_map_set_user_hash_eq(map, token, XR_FROM_INT(99), token_type, token_hash, token_eq);

    reset_call_counts();
    ASSERT_TRUE(xrt_map_has_user_hash_eq(map, token, token_type, token_hash, token_eq),
    ...
    reset_call_counts();
    ASSERT_TRUE(!xrt_map_has_user_hash_eq(map, other, token_type, token_hash, token_eq),
    ...
    ASSERT_NULL(xrt_map_get_user_hash_eq(map, other, token_type, token_hash, token_eq),
                "wrong-type map get returns missing");

    reset_call_counts();
    xrt_map_set_user_hash_eq(map, other, XR_FROM_INT(55), token_type, token_hash, token_eq);
    ...
    ASSERT_EQ_INT(xrt_map_get_user_hash_eq(map, other, token_type, token_hash, token_eq).i, 55,
                  "wrong-type fallback entry round-trips through generic equality");
...
static void test_set_user_hash_eq_wrong_type_falls_back_without_user_calls(void) {
    ...
    ASSERT_TRUE(xrt_set_add_user_hash_eq(set, token, token_type, token_hash, token_eq),
                "exact token inserted");

    reset_call_counts();
    ASSERT_TRUE(xrt_set_has_user_hash_eq(set, token, token_type, token_hash, token_eq),
    ...
    reset_call_counts();
    ASSERT_TRUE(!xrt_set_has_user_hash_eq(set, other, token_type, token_hash, token_eq),
    ...
    reset_call_counts();
    ASSERT_TRUE(xrt_set_add_user_hash_eq(set, other, token_type, token_hash, token_eq),
