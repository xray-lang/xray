...
evidence_object_shape_by_domain(const XgGlobalEvidence *ev, uint8_t domain, uint32_t ordinal) {
    if (!ev)
        return NULL;
...
evidence_object_field_by_domain(const XgGlobalEvidence *ev, uint8_t domain, uint32_t ordinal) {
    if (!ev)
        return NULL;
...
static uint32_t evidence_object_field_count_by_domain(const XgGlobalEvidence *ev, uint8_t domain) {
    uint32_t count = 0;
    if (!ev)
        return 0;
    for (uint32_t i = 0; i < ev->nobject_fields; i++) {
...
TEST(global_evidence_producer_records_explicit_json_shape_access) {
    ...
    ASSERT_EQ_UINT((*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).field_count,
                   2);
    ASSERT_TRUE(((*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).flags &
                 XG_OBJECT_SHAPE_STATIC_KEYS) != 0);
    ASSERT_EQ_UINT(
        (*evidence_object_field_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).shape_id,
        (*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).object_shape_id);
    ASSERT_EQ_UINT((*evidence_object_field_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).field_ordinal,
                   0);
    ASSERT_EQ_UINT((*evidence_object_field_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).name_id,
                   xg_name_id("name"));
...
TEST(global_evidence_producer_records_json_computed_key_access) {
    ...
    ASSERT_EQ_UINT(
        ev.json_dynamic_accesses[0].receiver_shape_id,
        (*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).object_shape_id);
    ...
    ASSERT_TRUE(
        (ev.json_dynamic_accesses[0].flags & XG_JSON_DYNAMIC_ACCESS_RECEIVER_SHAPE_PROVEN) != 0);
...
TEST(global_evidence_producer_propagates_json_shape_through_direct_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser() -> Json {\n"
                         "    return { name: \"ada\", age: 1 }\n"
                         "}\n"
                         "fn readReturned() -> Json {\n"
    ...
    ASSERT_EQ_UINT(
        ev.object_accesses[0].receiver_shape_id,
        (*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 0)).object_shape_id);
    ASSERT_EQ_UINT(ev.object_accesses[0].access_kind, XG_OBJECT_ACCESS_FIELD_GET);
    ASSERT_EQ_UINT(ev.object_accesses[0].field_ordinal, 0);
...
TEST(global_evidence_producer_propagates_json_shape_through_indirect_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser() -> Json {\n"
                         "    var user: Json = { name: \"ada\", age: 1 }\n"
                         "    return user\n"
                         "}\n"
                         "fn readReturnedIndirect() -> int {\n"
                         "    return makeUser().age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    ...
    ASSERT_EQ_UINT(
        ev.object_accesses[0].receiver_shape_id,
        (*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 1)).object_shape_id);
    ASSERT_EQ_UINT(ev.object_accesses[0].access_kind, XG_OBJECT_ACCESS_FIELD_GET);
    ASSERT_EQ_UINT(ev.object_accesses[0].field_ordinal, 1);
    ASSERT_TRUE((ev.object_accesses[0].flags & XG_OBJECT_ACCESS_RECEIVER_SHAPE_PROVEN) != 0);
...
TEST(global_evidence_producer_does_not_propagate_json_shape_after_indirect_return_escape) {
    setup_parser_session();
    const char *source = "fn consume(j: Json) -> int {\n"
                         "    return 1\n"
                         "}\n"
                         "fn makeUser() -> Json {\n"
                         "    var user: Json = { name: \"ada\", age: 1 }\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_branch_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         ...
                         "    }\n"
                         "}\n"
                         "fn readReturnedBranch(flag: bool) -> int {\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_branch_local_return_receiver) {
    ...
    ASSERT_EQ_UINT(
        ev.object_accesses[0].receiver_shape_id,
        (*evidence_object_shape_by_domain(&ev, XG_OBJECT_DOMAIN_JSON, 2)).object_shape_id);
    ASSERT_EQ_UINT(ev.object_accesses[0].access_kind, XG_OBJECT_ACCESS_FIELD_GET);
    ASSERT_EQ_UINT(ev.object_accesses[0].field_ordinal, 1);
    ASSERT_TRUE((ev.object_accesses[0].flags & XG_OBJECT_ACCESS_RECEIVER_SHAPE_PROVEN) != 0);
...
TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_branch_return) {
    ...
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         ...
                         "        return { name: \"ada\", age: 1 }\n"
                         "    } else {\n"
                         "        return { name: \"bob\", score: 2 }\n"
                         "    }\n"
                         "}\n"
                         "fn readReturnedBranchUnknown(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_fallthrough_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         ...
                         "    return { name: \"bob\", age: 2 }\n"
                         "}\n"
                         "fn readReturnedFallthrough(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_fallthrough_return) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         ...
                         "    return { name: \"bob\", score: 2 }\n"
                         "}\n"
                         "fn readReturnedFallthroughUnknown(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_else_if_fallthrough_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool, other: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         "    } else if (other) {\n"
                         "        return { name: \"bob\", age: 2 }\n"
                         "    }\n"
                         "    return { name: \"eve\", age: 3 }\n"
                         "}\n"
                         "fn readReturnedElseIfFallthrough(flag: bool, other: bool) -> int {\n"
                         "    return makeUser(flag, other).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_else_if_fallthrough_return) {
    setup_parser_session();
    const char *source =
        "fn makeUser(flag: bool, other: bool) -> Json {\n"
        "    if (flag) {\n"
        "        return { name: \"ada\", age: 1 }\n"
        "    } else if (other) {\n"
        "        return { name: \"bob\", age: 2 }\n"
        "    }\n"
        "    return { name: \"eve\", score: 3 }\n"
        "}\n"
        "fn readReturnedElseIfFallthroughUnknown(flag: bool, other: bool) -> int {\n"
        "    return makeUser(flag, other).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_match_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(kind: int) -> Json {\n"
                         "    return match (kind) {\n"
                         "        0 -> ({ name: \"ada\", age: 1 }),\n"
                         "        1 -> ({ name: \"bob\", age: 2 }),\n"
                         "        _ -> ({ name: \"eve\", age: 3 })\n"
                         "    }\n"
                         "}\n"
                         "fn readReturnedMatch(kind: int) -> int {\n"
                         "    return makeUser(kind).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_match_return) {
    setup_parser_session();
    const char *source = "fn makeUser(kind: int) -> Json {\n"
                         "    return match (kind) {\n"
                         "        0 -> ({ name: \"ada\", age: 1 }),\n"
                         ...
                         "    }\n"
                         "}\n"
                         "fn readReturnedMatchUnknown(kind: int) -> int {\n"
                         "    return makeUser(kind).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_match_assignment_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(kind: int) -> Json {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    match (kind) {\n"
                         "        0 -> (user = { name: \"ada\", age: 1 }),\n"
                         "        1 -> (user = { name: \"bob\", age: 2 }),\n"
                         "        _ -> (user = { name: \"eve\", age: 3 })\n"
                         "    }\n"
                         "    return user\n"
                         "}\n"
                         "fn readReturnedMatchAssigned(kind: int) -> int {\n"
                         "    return makeUser(kind).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_match_assignment_return) {
    setup_parser_session();
    const char *source = "fn makeUser(kind: int) -> Json {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    match (kind) {\n"
                         "        0 -> (user = { name: \"ada\", age: 1 }),\n"
                         "        1 -> (user = { name: \"bob\", score: 2 }),\n"
                         "        _ -> (user = { name: \"eve\", age: 3 })\n"
                         "    }\n"
                         "    return user\n"
                         "}\n"
                         "fn readReturnedMatchAssignedUnknown(kind: int) -> int {\n"
                         "    return makeUser(kind).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_ignores_unreachable_json_return_after_closed_branch_return) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         ...
                         "    return { name: \"eve\", score: 3 }\n"
                         "}\n"
                         "fn readReturnedClosedBranch(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_use_unreachable_json_return_to_mask_mismatched_closed_branch) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         ...
                         "    return { name: \"eve\", age: 3 }\n"
                         "}\n"
                         "fn readReturnedClosedBranchUnknown(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_return_initialized_local) {
    setup_parser_session();
    const char *source = "fn makeUser() -> Json {\n"
                         "    return { name: \"ada\", age: 1 }\n"
                         "}\n"
                         "fn readReturnedLocal() -> Json {\n"
                         "    var j: Json = makeUser()\n"
                         "    return j.age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
...
TEST(global_evidence_producer_propagates_json_shape_through_branch_return_initialized_local) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    if (flag) {\n"
                         "        return { name: \"ada\", age: 1 }\n"
                         ...
                         "    }\n"
                         "}\n"
                         "fn readReturnedBranchLocal(flag: bool) -> Json {\n"
                         "    var j: Json = makeUser(flag)\n"
                         "    return j.age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_branch_assignment_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    var scratch: Json = { spare: 0 }\n"
                         "    if (flag) {\n"
                         "        user = { name: \"ada\", age: 1 }\n"
                         "        scratch = { left: 1 }\n"
                         "    } else {\n"
                         ...
                         "    return user\n"
                         "}\n"
                         "fn readReturnedBranchAssigned(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_branch_assignment_return) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    if (flag) {\n"
                         "        user = { name: \"ada\", age: 1 }\n"
                         "    } else {\n"
                         "        user = { name: \"bob\", score: 2 }\n"
                         "    }\n"
                         "    return user\n"
                         "}\n"
                         "fn readReturnedBranchAssignedUnknown(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_propagates_json_shape_through_conditional_assignment_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var user: Json = { name: \"ada\", age: 1 }\n"
                         "    if (flag) {\n"
                         ...
                         "    return user\n"
                         "}\n"
                         "fn readReturnedConditionalAssigned(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_conditional_assignment_return) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var user: Json = { name: \"ada\", age: 1 }\n"
                         "    if (flag) {\n"
                         ...
                         "    return user\n"
                         "}\n"
                         "fn readReturnedConditionalAssignedUnknown(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    return flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }\n"
                         "}\n"
                         "fn readReturnedTernary(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_local_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", age: 2 }\n"
                         "    return flag ? left : right\n"
                         "}\n"
                         "fn readReturnedTernaryLocal(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_return) {
    setup_parser_session();
    const char *source =
        "fn makeUser(flag: bool) -> Json {\n"
        "    return flag ? { name: \"ada\", age: 1 } : { name: \"bob\", score: 2 }\n"
        "}\n"
        "fn readReturnedTernaryUnknown(flag: bool) -> int {\n"
        "    return makeUser(flag).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_propagates_json_shape_through_ternary_initialized_local_return_receiver) {
    setup_parser_session();
    const char *source =
        "fn makeUser(flag: bool) -> Json {\n"
        "    var user: Json = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }\n"
        "    return user\n"
        "}\n"
        "fn readReturnedTernaryInitializedLocal(flag: bool) -> int {\n"
        "    return makeUser(flag).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_assignment_return_receiver) {
    setup_parser_session();
    const char *source = "fn makeUser(flag: bool) -> Json {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    user = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }\n"
                         "    return user\n"
                         "}\n"
                         "fn readReturnedTernaryAssigned(flag: bool) -> int {\n"
                         "    return makeUser(flag).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_assignment_return) {
    setup_parser_session();
    const char *source =
        "fn makeUser(flag: bool) -> Json {\n"
        "    var user: Json = { name: \"seed\", age: 0 }\n"
        "    user = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", score: 2 }\n"
        "    return user\n"
        "}\n"
        "fn readReturnedTernaryAssignedUnknown(flag: bool) -> int {\n"
        "    return makeUser(flag).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_initialized_local_access) {
    setup_parser_session();
    const char *source =
        "fn readTernaryInitializedLocal(flag: bool) -> int {\n"
        "    var user: Json = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }\n"
        "    return user.age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_initialized_local_access) {
    setup_parser_session();
    const char *source =
        "fn readTernaryInitializedLocalUnknown(flag: bool) -> Json {\n"
        "    var user: Json = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", score: 2 }\n"
        "    return user.age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_assigned_local_access) {
    setup_parser_session();
    const char *source = "fn readTernaryAssignedLocal(flag: bool) -> int {\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    user = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }\n"
                         "    return user.age\n"
                         "}\n";
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_assigned_local_access) {
    setup_parser_session();
    const char *source =
        "fn readTernaryAssignedLocalUnknown(flag: bool) -> Json {\n"
        "    var user: Json = { name: \"seed\", age: 0 }\n"
        "    user = flag ? { name: \"ada\", age: 1 } : { name: \"bob\", score: 2 }\n"
        "    return user.age\n"
        "}\n";
...
TEST(
    global_evidence_producer_propagates_json_shape_through_ternary_local_arm_initialized_local_access) {
    setup_parser_session();
    const char *source = "fn readTernaryLocalArmInitializedLocal(flag: bool) -> int {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", age: 2 }\n"
                         "    var user: Json = flag ? left : right\n"
                         "    return user.age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_propagates_json_shape_through_ternary_local_arm_assigned_local_access) {
    setup_parser_session();
    const char *source = "fn readTernaryLocalArmAssignedLocal(flag: bool) -> int {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", age: 2 }\n"
                         "    var user: Json = { name: \"seed\", age: 0 }\n"
                         "    user = flag ? left : right\n"
                         "    return user.age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_local_arm_local_access) {
    setup_parser_session();
    const char *source = "fn readTernaryLocalArmLocalUnknown(flag: bool) -> Json {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", score: 2 }\n"
                         "    var user: Json = flag ? left : right\n"
                         "    return user.age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_expression_receiver) {
    setup_parser_session();
    const char *source =
        "fn readTernaryExpressionReceiver(flag: bool) -> int {\n"
        "    return (flag ? { name: \"ada\", age: 1 } : { name: \"bob\", age: 2 }).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_expression_receiver) {
    setup_parser_session();
    const char *source =
        "fn readTernaryExpressionReceiverUnknown(flag: bool) -> Json {\n"
        "    return (flag ? { name: \"ada\", age: 1 } : { name: \"bob\", score: 2 }).age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(global_evidence_producer_propagates_json_shape_through_ternary_local_expression_receiver) {
    setup_parser_session();
    const char *source = "fn readTernaryLocalExpressionReceiver(flag: bool) -> int {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", age: 2 }\n"
                         "    return (flag ? left : right).age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_local_expression_receiver) {
    setup_parser_session();
    const char *source = "fn readTernaryLocalExpressionReceiverUnknown(flag: bool) -> Json {\n"
                         "    var left: Json = { name: \"ada\", age: 1 }\n"
                         "    var right: Json = { name: \"bob\", score: 2 }\n"
                         "    return (flag ? left : right).age\n"
                         "}\n";
...
TEST(global_evidence_producer_propagates_json_shape_through_closure_capture) {
    setup_parser_session();
    const char *source = "fn readCaptured() -> string {\n"
                         "    var j: Json = { name: \"ada\", age: 1 }\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_array_closure_container) {
    setup_parser_session();
    const char *source = "fn readCapturedArray() -> int {\n"
                         "    var users: Array<Json> = []\n"
                         "    users.push({ name: \"ada\", age: 7 })\n"
                         "    const read = fn() -> int {\n"
                         "        return users[0].age\n"
                         "    }\n"
                         "    return read()\n"
    ...
    ASSERT_TRUE(
        xg_global_evidence_build_from_module_graph(&ev, &graph, XG_BUILD_NATIVE_RELEASE, 0));
    const XgBodySummary *outer = evidence_find_body_by_name(&ev, "readCapturedArray");
    const XgBodySummary *closure = evidence_find_body_by_name(&ev, "<anonymous>");
    ASSERT_NOT_NULL(outer);
    ASSERT_NOT_NULL(closure);
    ASSERT_TRUE(outer->func_id != closure->func_id);
    ...
    ASSERT_EQ_UINT(bundle.json_dynamic_access_plans[0].action,
...
TEST(global_evidence_producer_keeps_json_field_initializer_shape_through_constructor_branch) {
    setup_parser_session();
    const char *source = "class Holder {\n"
                         "    payload: Json = { name: \"ada\", age: 7 }\n"
                         "    constructor() {\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_constructor_branch_field_assignment) {
    setup_parser_session();
    const char *source = "class Holder {\n"
                         "    payload: Json\n"
                         "    constructor() {\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_constructor_ternary_field_assignment) {
    setup_parser_session();
    const char *source =
        "class Holder {\n"
        "    payload: Json\n"
        "    constructor(flag: bool) {\n"
        "        this.payload = flag ? { name: \"ada\", age: 7 } : { name: \"bob\", age: 8 }\n"
        "    }\n"
        "    readPayloadAge() -> int {\n"
        "        return this.payload.age\n"
        "    }\n"
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_constructor_branch_field_assignment) {
    setup_parser_session();
    const char *source = "class Holder {\n"
                         "    payload: Json\n"
                         "    constructor() {\n"
...
TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_constructor_ternary_field_assignment) {
    setup_parser_session();
    const char *source =
        "class Holder {\n"
        "    payload: Json\n"
        "    constructor(flag: bool) {\n"
        "        this.payload = flag ? { name: \"ada\", age: 7 } : { name: \"bob\" }\n"
        "    }\n"
        "    readPayloadAge() -> Json {\n"
        "        return this.payload.age\n"
        "    }\n"
...
TEST(global_evidence_producer_propagates_json_shape_through_array_literal_container) {
    setup_parser_session();
    const char *source =
        "fn readArrayLiteralAge() -> int {\n"
        "    var users: Array<Json> = [{ name: \"ada\", age: 7 }, { name: \"bob\", age: 8 }]\n"
        "    return users[0].age\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_array_push_container) {
    setup_parser_session();
    const char *source = "fn readArrayPushAge() -> int {\n"
                         "    var users: Array<Json> = []\n"
                         "    users.push({ name: \"ada\", age: 7 })\n"
                         "    users.push({ name: \"bob\", age: 8 })\n"
                         "    return users[0].age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_shape_through_array_alias_container) {
    setup_parser_session();
    const char *source = "fn readArrayAliasAge() -> int {\n"
                         "    var users: Array<Json> = []\n"
                         "    users.push({ name: \"ada\", age: 7 })\n"
                         "    var alias = (users)\n"
                         "    var assigned: Array<Json> = []\n"
                         "    assigned = (alias)\n"
                         "    return assigned[0].age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
...
TEST(global_evidence_producer_clears_json_container_shape_after_mismatched_element_set) {
    ...
    const char *source = "fn readAfterMismatchedSet() -> int {\n"
                         "    var users: Array<Json> = [{ name: \"ada\", age: 1 }]\n"
                         "    users[0] = { name: \"bob\", score: 2 }\n"
                         "    return users[0].age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
...
TEST(global_evidence_producer_propagates_json_bridge_shape_through_field_and_container_receivers) {
    ...
    const char *source = "type User = { name: string, age: int }\n"
                         ...
                         "    }\n"
                         "}\n"
                         "fn readContainerJson(users: Array<User>) -> int {\n"
                         "    return users[0].age\n"
                         "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    ...
    ASSERT_TRUE(
        xg_global_evidence_build_from_module_graph(&ev, &graph, XG_BUILD_NATIVE_RELEASE, 0));
    const XgBodySummary *field_reader = evidence_find_body_by_name(&ev, "readFieldJson");
    ...
    ASSERT_TRUE(dispatch_access_index != UINT32_MAX);
    {
        ...
        bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);
        {
            ...
            for (uint32_t i = 0; i < ev.nobject_shape_flows; i++) {
                ...
                if (flow->target_func_id != different->func_id || flow->target_param_ordinal != 0 ||
...
RUN_TEST(global_evidence_producer_propagates_json_shape_through_fallthrough_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_fallthrough_return);
RUN_TEST(
    global_evidence_producer_propagates_json_shape_through_else_if_fallthrough_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_else_if_fallthrough_return);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_match_return_receiver);
RUN_TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_match_return);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_match_assignment_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_match_assignment_return);
RUN_TEST(global_evidence_producer_ignores_unreachable_json_return_after_closed_branch_return);
RUN_TEST(
    global_evidence_producer_does_not_use_unreachable_json_return_to_mask_mismatched_closed_branch);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_return_initialized_local);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_branch_return_initialized_local);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_branch_assignment_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_branch_assignment_return);
RUN_TEST(
    global_evidence_producer_propagates_json_shape_through_conditional_assignment_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_conditional_assignment_return);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_return_receiver);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_local_return_receiver);
RUN_TEST(global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_return);
RUN_TEST(
    global_evidence_producer_propagates_json_shape_through_ternary_initialized_local_return_receiver);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_assignment_return_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_assignment_return);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_initialized_local_access);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_initialized_local_access);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_assigned_local_access);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_assigned_local_access);
RUN_TEST(
    global_evidence_producer_propagates_json_shape_through_ternary_local_arm_initialized_local_access);
...
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_local_arm_local_access);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_expression_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_expression_receiver);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_ternary_local_expression_receiver);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_ternary_local_expression_receiver);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_closure_capture);
RUN_TEST(global_evidence_producer_propagates_json_shape_through_array_closure_container);
...
RUN_TEST(
    global_evidence_producer_propagates_json_shape_through_constructor_ternary_field_assignment);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_constructor_branch_field_assignment);
RUN_TEST(
    global_evidence_producer_does_not_propagate_json_shape_through_mismatched_constructor_ternary_field_assignment);
