...
static void finalize_object_shape_fields_fixture(XgObjectShapeSummary *shape,
    ...
    if (!xg_object_shape_stable_key(shape->domain, fields, field_count, &shape->stable_shape_key))
        abort();
    shape->shape_hash = shape->stable_shape_key;
}

static uint32_t evidence_object_shape_count_by_domain(const XgGlobalEvidence *ev, uint8_t domain) {
    uint32_t count = 0;
    if (!ev)
        return 0;
...
static void assert_single_body_rejected(const XgBodySummary *body, const char *expected_error) {
    XgGlobalEvidence ev;
    XgBuildKey key = {.source_hash = 0x9110,
                      .compiler_semver_hash = 0x9111,
                      .profile_hash = 0x9112,
                      .imported_summary_hash = 0x9113,
                      .module_id = 1,
                      .profile = XG_BUILD_NATIVE_RELEASE};
    XgDeclSummary decl = {.module_id = 1,
                          .source_node_id = body ? body->source_node_id : 0,
                          .decl_id = 1,
                          .kind = XG_DECL_FUNC,
                          .name_id = body ? body->name_id : 0,
                          .signature_key = body ? body->signature_key : 0,
                          .source_span_id = body ? body->source_span_id : 0};
    XiFunc init_func;
    XiModule module;
    XiModule *modules[1];
    XaotBundle bundle;
    ...
    memset(&init_func, 0, sizeof(init_func));
    init_func.name = "init";
    init_func.xg_body_func_id = body ? body->func_id : XG_NO_ID;
    memset(&module, 0, sizeof(module));
    module.path = "test.xr";
    module.name = "test";
    module.init = &init_func;
...
TEST(global_evidence_verifier_rejects_param_storage_key_without_vector) {
    XgBodySummary body = {.func_id = 1,
                          ...
                          .source_node_id = 9014,
                          .owner_decl_id = 1,
                          .name_id = xg_name_id("init"),
                          .signature_key = 20,
                          .source_span_id = 3,
                          .kind = XG_BODY_FUNCTION,
                          .body_hash = 0x9014,
                          .param_storage_key = UINT32_C(3812009484)};
    assert_single_body_rejected(&body, "param storage key lacks vector");
}
...
TEST(global_evidence_hash_is_content_stable) {
    ...
    b.generic_insts[0].declaration_type_arg_count = 2;
    ASSERT_NE(xg_global_evidence_hash(&a), xg_global_evidence_hash(&b));
    b.generic_insts[0].declaration_type_arg_count = 1;
    b.generic_insts[0].receiver_class_id = 10;
    ASSERT_NE(xg_global_evidence_hash(&a), xg_global_evidence_hash(&b));
    b.generic_insts[0].receiver_class_id = XG_NO_ID;
    b.generic_insts[0].specialization_effect =
...
TEST(global_evidence_package_import_remaps_generic_constraint_interface) {
    XgGlobalEvidence package;
    XgGlobalEvidence target;
    XgEvidencePackageImportReport report;
    XgBuildKey package_key = {.module_id = 7, .profile = XG_BUILD_NATIVE_RELEASE};
    XgBuildKey target_key = {.module_id = 1, .profile = XG_BUILD_NATIVE_RELEASE};
    XgModuleSummary package_module = {.module_id = 7,
                                       .name_id = 7,
                                       .canonical_hash = UINT64_C(0x7007),
                                       .source_hash = UINT64_C(0x7008),
                                       .kind = XR_MOD_MEMORY};
    XgModuleSummary target_module = {.module_id = 1,
                                      .name_id = 1,
                                      .canonical_hash = UINT64_C(0x1001),
                                      .source_hash = UINT64_C(0x1002),
                                      .kind = XR_MOD_MEMORY};
    XgGenericInstSummary package_inst = {
        .generic_inst_id = 1,
        .module_id = 7,
        .constraint_interface_id = 5,
        .name_id = 17,
        .declaration_type_key = UINT64_C(0x700000001),
        .declaration_type_arg_key_start = UINT64_C(0x700000002),
        .declaration_type_arg_count = 1,
        .kind = XG_GENERIC_INST_FUNCTION,
        .flags = XG_GENERIC_INST_CONCRETE_TYPES | XG_GENERIC_INST_INTERFACE_CONSTRAINT,
    };
    XgGenericInstSummary target_inst = {
        .generic_inst_id = 10,
        .module_id = 1,
        .constraint_interface_id = 100,
        .name_id = 11,
        .declaration_type_key = UINT64_C(0x100000001),
        .declaration_type_arg_key_start = UINT64_C(0x100000002),
        .declaration_type_arg_count = 1,
        .kind = XG_GENERIC_INST_FUNCTION,
        .flags = XG_GENERIC_INST_CONCRETE_TYPES | XG_GENERIC_INST_INTERFACE_CONSTRAINT,
    };
    ...
    xg_global_evidence_init(&package, package_key);
    xg_global_evidence_init(&target, target_key);
    ASSERT_NOT_NULL(xg_global_evidence_add_module(&package, &package_module));
    ASSERT_NOT_NULL(xg_global_evidence_add_generic_inst(&package, &package_inst));
    ASSERT_NOT_NULL(xg_global_evidence_add_module(&target, &target_module));
    ASSERT_NOT_NULL(xg_global_evidence_add_generic_inst(&target, &target_inst));

    payload = xg_global_evidence_cache_payload_dump(&package,
                                                    XG_EVIDENCE_CACHE_GLOBAL_EVIDENCE);
    ASSERT_NOT_NULL(payload);
    ASSERT_TRUE(xg_global_evidence_import_package_payload(&target, payload, &report));
    ASSERT_EQ_UINT(report.rows_imported, 1);
    ASSERT_EQ_UINT(target.ngeneric_insts, 2);
    ASSERT_EQ_UINT(target.generic_insts[1].generic_inst_id, 11);
    ASSERT_EQ_UINT(target.generic_insts[1].constraint_interface_id, 105);
...
static void init_cache_key_fixture(XgGlobalEvidence *ev, XgBuildKey key) {
    ...
    XgGenericInstSummary inst = {.generic_inst_id = 1,
                                 ...
                                 .declaration_type_arg_key_start = 45,
                                 .declaration_type_arg_count = 1,
...
TEST(global_evidence_dump_lists_core_rows) {
    ...
    XgGenericInstSummary generic_inst = {
        ...
        .declaration_type_arg_key_start = 902,
        .declaration_type_arg_count = 2,
...
TEST(global_evidence_verifier_rejects_missing_generic_constraint_interface) {
    XgGlobalEvidence ev;
    XgBuildKey key = {.module_id = 1, .profile = XG_BUILD_NATIVE_RELEASE};
    XgGenericInstSummary inst = {
        .generic_inst_id = 1,
        .module_id = 1,
        .constraint_interface_id = 99,
        .declaration_type_key = 11,
        .declaration_type_arg_key_start = 12,
        .declaration_type_arg_count = 1,
        .kind = XG_GENERIC_INST_FUNCTION,
    ...
    ASSERT_MSG(strstr(err, "constraint interface is missing") != NULL, err);
...
TEST(global_evidence_lowers_to_aot_class_plans) {
    ...
    dump = xaot_bundle_dump_global_evidence_plan(&bundle);
    ASSERT_NOT_NULL(dump);
...
TEST(global_evidence_verifier_rederives_dispatch_plans) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&good, err, sizeof(err)), err);
    good.method_dispatch_plans[0].kind = XAOT_DISPATCH_VTABLE;
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&good, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_owner, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_source, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_source_node, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_ordinal, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_method_name, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_signature, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_arg_range, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_arg_count, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_target, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_target_count, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT dispatch direct target does not re-derive"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing, err, sizeof(err)));
...
TEST(global_evidence_lowers_large_interface_set_to_itable_abi_plan) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)), err);
...
TEST(global_evidence_verifier_rederives_profile_actions) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&metadata_bad, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&capability_bad, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&static_bad, err, sizeof(err)));
...
TEST(global_evidence_verifier_rederives_static_data_materialization_contract) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&bad_section, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&bad_align, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&bad_type_hash, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&bad_data_hash, err, sizeof(err)));
...
TEST(global_evidence_verifier_rederives_func_attr_body_summary) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_effect, err, sizeof(err)));
    ...
    ASSERT_NOT_NULL(xaot_bundle_add_func_attr_plan(&read_mem_pure, &helper_func, XAOT_FN_ATTR_PURE,
                                                   &read_mem_body));
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(xaot_verify_global_evidence_plan(&read_mem_pure, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&read_mem_const, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&call_contradicts_summary, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&contradicts_summary, err, sizeof(err)));
...
TEST(global_evidence_composes_direct_callee_effects_for_func_attr) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(xaot_verify_global_evidence_plan(&const_caller, err, sizeof(err)));
    ...
    ASSERT_NOT_NULL(xaot_bundle_add_func_attr_plan(&pure_caller, &caller_func, XAOT_FN_ATTR_PURE,
                                                   &ev.bodies[0]));
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(xaot_verify_global_evidence_plan(&pure_caller, err, sizeof(err)));
    ...
    ASSERT_NOT_NULL(xaot_bundle_add_func_attr_plan(&stale_const, &caller_func, XAOT_FN_ATTR_CONST,
                                                   &ev.bodies[0]));
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_const, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT function attribute plan claims const but func reads memory"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&effectful_callee, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT function attribute plan contradicts body summary"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&extra_direct_calls, err, sizeof(err)));
...
TEST(global_evidence_composes_direct_method_effects_for_func_attr) {
    ...
    ASSERT_EQ_UINT(method_caller.func_attr_plans[0].evidence, XAOT_FN_ATTR_EV_BODY_SUMMARY |
                                                                  ...
                                                                  XAOT_FN_ATTR_EV_CALLEE_SUMMARY);
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(xaot_verify_global_evidence_plan(&method_caller, err, sizeof(err)));
...
TEST(global_evidence_composes_single_implementor_interface_effects_for_func_attr) {
    ...
    ASSERT_EQ_UINT(interface_caller.func_attr_plans[0].evidence,
                   XAOT_FN_ATTR_EV_BODY_SUMMARY | XAOT_FN_ATTR_EV_XI_EFFECT_SCAN |
                       XAOT_FN_ATTR_EV_CALLEE_SUMMARY);
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&interface_caller, err, sizeof(err)), err);
...
TEST(global_evidence_composes_vtable_target_set_effects_for_func_attr) {
    ...
    leaf_func.xg_body_func_id = leaf_body.func_id;
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&vtable_caller, err, sizeof(err)), err);
    vtable_caller.dispatch_target_cases[1].method_owner_class_id = 99;
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&vtable_caller, err, sizeof(err)));
...
TEST(global_evidence_verifier_rederives_body_callsite_ranges) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_kind, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_decl, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_range, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&orphan_callsite, err, sizeof(err)));
...
TEST(global_evidence_verifier_rejects_stale_callsite_identity_rows) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_id_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_source_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_id_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_source_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&range_hole_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&invalid_kind_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&closure_stale_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&direct_no_target_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&direct_missing_body_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&method_stale_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&method_no_receiver_bundle, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT global evidence method callsite identity is stale"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&method_no_signature_bundle, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT global evidence method callsite identity is stale"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&method_wrong_target_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&interface_stale_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(
        !xaot_verify_global_evidence_plan(&interface_missing_decl_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(
        !xaot_verify_global_evidence_plan(&interface_missing_method_bundle, err, sizeof(err)));
    ASSERT_NOT_NULL(
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&native_stale_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&extern_stale_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&extern_missing_decl_bundle, err, sizeof(err)));
...
TEST(global_evidence_verifier_rejects_stale_interface_extends_rows) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_parent_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&cycle_bundle, err, sizeof(err)));
...
TEST(global_evidence_verifier_rederives_method_body_signature) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&swapped_source_identity, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT global evidence method body owner method does not re-derive"));
...
TEST(global_evidence_verifier_rejects_stale_body_identity_rows) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_decl_source_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_decl_source_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_body_source_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_body, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&valid_module_bundle, err, sizeof(err)), err);
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_module_body, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_body, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&native_missing_body, err, sizeof(err)), err);
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_decl_body, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&duplicate_method_body, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT global evidence method body owner method does not re-derive"));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&missing_method_body, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_global_evidence_plan(&native_missing_method_body, err, sizeof(err)),
               err);
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&native_method_with_body, err, sizeof(err)));
...
TEST(global_evidence_producer_records_generic_instantiation_roots) {
    ...
    ASSERT_EQ_UINT(inst->receiver_class_id, XG_NO_ID);
    ASSERT_EQ_UINT(inst->receiver_type_arg_count, 0);
    ASSERT_EQ_UINT(inst->declaration_type_arg_count, 1);
    ASSERT_EQ_UINT(inst->specialization_effect, XG_GENERIC_SPECIALIZATION_EFFECT_NONE);
    ASSERT_TRUE((inst->flags & XG_GENERIC_INST_CONCRETE_TYPES) != 0);
    ASSERT_TRUE(inst->declaration_type_key != 0);
    ASSERT_TRUE(inst->declaration_type_arg_key_start != 0);
...
TEST(global_evidence_generic_effect_identity_is_orthogonal_to_type_tuple) {
    setup_parser_session();
    const char *source =
        "enum GenericEffectError { Boom }\n"
        "fn passGenericEffect(x: i64) -> i64 { return x }\n"
        "fn failGenericEffect(x: i64) -> i64 { throw GenericEffectError.Boom }\n"
        "fn applyGenericEffect<T>(x: T, cb: fn(T) -> T) -> T { return cb(x) }\n"
        "fn useNoThrowGenericEffect() -> i64 {\n"
        "  return applyGenericEffect<i64>(1, passGenericEffect)\n"
        "}\n"
        "fn useMayThrowGenericEffect() -> i64 {\n"
        "  return applyGenericEffect<i64>(2, failGenericEffect)\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
    init_memory_module_spec(&spec);
    spec.ast = ast;
    spec.source_path = "generic-effect-identity.xr";
    int topo_order[1] = {0};
    XrModuleGraph graph = {
        .specs = &spec,
        .spec_count = 1,
    ...
    ASSERT_TRUE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0u, NULL, 0u, analyzer));
    const XgGenericInstSummary *may_throw = NULL;
    const XgGenericInstSummary *no_throw = NULL;
    for (uint32_t i = 0; i < evidence.ngeneric_insts; ++i) {
        const XgGenericInstSummary *inst = &evidence.generic_insts[i];
        if (inst->name_id != xg_name_id("applyGenericEffect"))
            continue;
        if (inst->specialization_effect == XG_GENERIC_SPECIALIZATION_EFFECT_MAY_THROW)
            may_throw = inst;
        if (inst->specialization_effect == XG_GENERIC_SPECIALIZATION_EFFECT_NO_THROW)
            no_throw = inst;
    }
    ASSERT_NOT_NULL(may_throw);
    ASSERT_NOT_NULL(no_throw);
    ASSERT_EQ_UINT(may_throw->declaration_type_arg_count, 1u);
    ASSERT_EQ_UINT(no_throw->declaration_type_arg_count, 1u);
    ASSERT_EQ_UINT(may_throw->declaration_type_key, no_throw->declaration_type_key);
    ASSERT_EQ_UINT(may_throw->declaration_type_arg_key_start,
                   no_throw->declaration_type_arg_key_start);
    ASSERT_NE(may_throw->specialization_effect, no_throw->specialization_effect);
...
TEST(global_evidence_generic_cache_preserves_identity_high_bits) {
    XgGlobalEvidence dst = {0};
    XgGlobalEvidence decoded = {0};
    XgBuildKey key = {.module_id = 1, .profile = XG_BUILD_NATIVE_RELEASE};
    XgGenericInstSummary existing = {.generic_inst_id = 1,
                                     .module_id = 1,
                                     .name_id = 7,
                                     .declaration_type_key = UINT64_C(0x100000001),
                                     .declaration_type_arg_key_start = UINT64_C(0x300000002),
                                     .declaration_type_arg_count = 1,
                                     .kind = XG_GENERIC_INST_FUNCTION,
                                     .flags = XG_GENERIC_INST_CONCRETE_TYPES};
    XgGenericInstSummary distinct = existing;
    distinct.generic_inst_id = 2;
    distinct.declaration_type_key = UINT64_C(0x200000001);
    distinct.declaration_type_arg_key_start = UINT64_C(0x400000002);
    distinct.specialization_effect = XG_GENERIC_SPECIALIZATION_EFFECT_MAY_THROW;

    xg_global_evidence_init(&dst, key);
    ASSERT_NOT_NULL(xg_global_evidence_add_generic_inst(&dst, &existing));
    ASSERT_NOT_NULL(xg_global_evidence_add_generic_inst(&dst, &distinct));

    char *payload = xg_global_evidence_cache_payload_dump(&dst, XG_EVIDENCE_CACHE_GLOBAL_EVIDENCE);
    ASSERT_NOT_NULL(payload);
    ASSERT_TRUE(xg_evidence_cache_payload_materialize(payload, &decoded));
    ASSERT_EQ_UINT(decoded.ngeneric_insts, 2);
    ASSERT_EQ_UINT(decoded.generic_insts[0].declaration_type_key, UINT64_C(0x100000001));
    ASSERT_EQ_UINT(decoded.generic_insts[1].declaration_type_key, UINT64_C(0x200000001));
    ASSERT_EQ_UINT(decoded.generic_insts[1].declaration_type_arg_key_start,
                   UINT64_C(0x400000002));
    ASSERT_EQ_UINT(decoded.generic_insts[1].specialization_effect,
                   XG_GENERIC_SPECIALIZATION_EFFECT_MAY_THROW);

    xr_free(payload);
    xg_global_evidence_free(&decoded);
...
TEST(global_evidence_post_mono_records_concrete_nested_generic_root) {
    ...
    for (uint32_t i = 0u; i < evidence.ngeneric_insts; ++i) {
        ...
        ASSERT_TRUE(inst->declaration_type_key != 0u);
        ASSERT_TRUE(inst->declaration_type_arg_key_start != 0u);
...
TEST(global_evidence_post_mono_records_concrete_generic_method_root) {
    ...
    ASSERT_EQ_UINT(inst->receiver_type_arg_count, 0u);
    ASSERT_TRUE(inst->receiver_type_key != 0u);
    ...
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].receiver_class_id, inst->receiver_class_id);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].receiver_type_key, inst->receiver_type_key);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].receiver_type_arg_key_start,
                   inst->receiver_type_arg_key_start);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].receiver_type_arg_count,
                   inst->receiver_type_arg_count);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].declaration_type_key,
                   inst->declaration_type_key);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].declaration_type_arg_key_start,
                   inst->declaration_type_arg_key_start);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].declaration_type_arg_count,
                   inst->declaration_type_arg_count);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].specialization_effect,
...
TEST(global_evidence_post_mono_preserves_inherited_generic_method_receiver) {
    setup_parser_session();
    const char *source =
        "class GenericBase {\n"
        "  apply<T>(value: T) -> T { return value }\n"
        "}\n"
        "class GenericChild extends GenericBase { }\n"
        "fn answer() -> i64 {\n"
        "  var child = GenericChild()\n"
        "  return child.apply(42)\n"
        "}\n";
    AstNode *ast = xr_parse(g_session, source);
    ASSERT_NOT_NULL(ast);
    XrModuleSpec spec;
    init_memory_module_spec(&spec);
    spec.ast = ast;
    spec.source_path = "inherited-generic-method-root.xr";
    int topo_order[1] = {0};
    XrModuleGraph graph = {
        .specs = &spec,
        .spec_count = 1,
    ...
    ASSERT_EQ_UINT(evidence.ngeneric_insts, 1u);
    const XgGenericInstSummary *inst = &evidence.generic_insts[0];
    ASSERT_NE(inst->receiver_class_id, inst->origin_class_id);
    const XgClassSummary *receiver =
        evidence_find_class_by_id(&evidence, inst->receiver_class_id);
    ASSERT_NOT_NULL(receiver);
    ASSERT_EQ_UINT(receiver->parent_class_id, inst->origin_class_id);
    ASSERT_EQ_UINT(evidence.ngeneric_body_uses, 1u);
    ASSERT_EQ_UINT(evidence.generic_body_uses[0].receiver_class_id, inst->receiver_class_id);
    const XgBodySummary *specialized =
        evidence_find_body_by_func(&evidence, inst->specialized_func_id);
    ASSERT_NOT_NULL(specialized);
    ASSERT_EQ_UINT(specialized->owner_class_id, inst->origin_class_id);
...
TEST(global_evidence_records_generic_body_storage_code_size_plans) {
    ...
    XgGenericInstSummary inst = {.generic_inst_id = 1,
                                 ...
                                 .root_callsite_id = 1,
                                 .name_id = xg_name_id("origin"),
                                 .declaration_type_key = 200,
                                 .declaration_type_arg_key_start = 201,
                                 .declaration_type_arg_count = 1,
                                 .source_span_id = 12,
                                 .kind = XG_GENERIC_INST_FUNCTION,
                                 .flags = XG_GENERIC_INST_CONCRETE_TYPES |
    ...
    memset(err, 0, sizeof(err));
    ASSERT_FALSE(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_MSG(strstr(err, "generic inst specialization effect is invalid") != NULL, err);
    ev.generic_insts[0].specialization_effect = XG_GENERIC_SPECIALIZATION_EFFECT_NONE;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);

    ev.generic_insts[0].declaration_type_arg_key_start = 0;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);
    memset(err, 0, sizeof(err));
    ASSERT_FALSE(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_MSG(strstr(err, "generic inst declaration type identity is invalid") != NULL, err);
    ev.generic_insts[0].declaration_type_arg_key_start = 201;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);

    ev.generic_insts[0].receiver_type_key = 1;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);
    memset(err, 0, sizeof(err));
    ASSERT_FALSE(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_MSG(strstr(err, "generic inst non-method receiver identity is invalid") != NULL, err);
    ev.generic_insts[0].receiver_type_key = 0;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);

    ev.generic_body_uses[0].declaration_type_key ^= UINT64_C(1);
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);
    memset(err, 0, sizeof(err));
    ASSERT_FALSE(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_MSG(strstr(err, "generic body-use identity does not match inst") != NULL, err);
    ev.generic_body_uses[0].declaration_type_key ^= UINT64_C(1);
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);

    ev.generic_body_uses[0].specialization_effect =
        XG_GENERIC_SPECIALIZATION_EFFECT_MAY_THROW;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);
    memset(err, 0, sizeof(err));
    ASSERT_FALSE(xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_MSG(strstr(err, "generic body-use identity does not match inst") != NULL, err);
    ev.generic_body_uses[0].specialization_effect = XG_GENERIC_SPECIALIZATION_EFFECT_NONE;
    bundle.global_evidence_plan.evidence_hash = xg_global_evidence_hash(&ev);

    bundle.generic_body_plans[0].declaration_type_key ^= UINT64_C(1);
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&bundle, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT generic body plan identity does not re-derive"));
    bundle.generic_body_plans[0].declaration_type_key ^= UINT64_C(1);
...
TEST(xaot_verifier_rejects_stale_enum_scalar_plan) {
    ...
    XrType payload_type = {.kind = XR_KIND_INT, .scalar_rep = XR_NATIVE_I64};
    XrType *payload_types[] = {&payload_type};
    const char *ok_payload_names[] = {"value"};
    const char *err_payload_names[] = {"error"};
    ...
    members[0].payload_names = ok_payload_names;
    members[0].payload_types = payload_types;
    members[1].name = "Err";
    members[1].ordinal = 1;
    members[1].payload_count = 1;
    members[1].payload_names = err_payload_names;
    members[1].payload_types = payload_types;
    ...
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_enum_plan(&bundle, &bundle.enum_plans[0], err, sizeof(err)), err);

    bundle.enum_plans[0].scalar_action = XAOT_ENUM_SCALAR_RUNTIME_AGGREGATE;
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_enum_plan(&bundle, &bundle.enum_plans[0], err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT enum scalar action does not re-derive"));
    bundle.enum_plans[0].scalar_action = XAOT_ENUM_SCALAR_COMPACT_AGGREGATE;

    const char *saved_payload_name = ok_payload_names[0];
    ok_payload_names[0] = "";
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_enum_plan(&bundle, &bundle.enum_plans[0], err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "incomplete payload field metadata"));
    ok_payload_names[0] = saved_payload_name;

    const char *saved_variant_name = members[1].name;
    members[1].name = members[0].name;
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_enum_plan(&bundle, &bundle.enum_plans[0], err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "duplicate variant names"));
    members[1].name = saved_variant_name;
    memset(err, 0, sizeof(err));
    ASSERT_MSG(xaot_verify_enum_plan(&bundle, &bundle.enum_plans[0], err, sizeof(err)), err);
...
TEST(global_evidence_rebinds_evidence_local_nominal_ids_by_stable_key) {
    ...
    init_memory_module_spec(&spec);
    spec.ast = ast;
    spec.source_path = "nominal-evidence-rebind.xr";
    int topo_order[1] = {0};
    XrModuleGraph graph;
    memset(&graph, 0, sizeof(graph));
    graph.specs = &spec;
    ...
    xa_analyzer_analyze(analyzer, spec.source_path, ast);
    ASSERT_EQ_UINT(analyzer->diagnostic_count, 0);
    XaSymbol *box_symbol = xa_analyzer_lookup(analyzer, "Box");
    XaSymbol *marker_symbol = xa_analyzer_lookup(analyzer, "Marker");
    XaSymbol *tone_symbol = xa_analyzer_lookup(analyzer, "Tone");
    ASSERT_NOT_NULL(box_symbol);
    ASSERT_NOT_NULL(marker_symbol);
    ASSERT_NOT_NULL(tone_symbol);
    ASSERT_NOT_NULL(box_symbol->links.class_info);
    ASSERT_NOT_NULL(marker_symbol->links.class_info);
    ASSERT_NOT_NULL(tone_symbol->links.class_info);
    XrClassInfo *box_info = box_symbol->links.class_info;
    XrClassInfo *marker_info = marker_symbol->links.class_info;
    XrClassInfo *tone_info = tone_symbol->links.class_info;
    uint64_t box_declaration_key = box_info->declaration_key;
    uint64_t marker_declaration_key = marker_info->declaration_key;
    uint64_t tone_declaration_key = tone_info->declaration_key;
    ASSERT_NE(box_declaration_key, 0u);
    ASSERT_NE(marker_declaration_key, 0u);
    ASSERT_NE(tone_declaration_key, 0u);
    ASSERT_EQ_UINT(box_info->xg_nominal_key, 0u);
    ASSERT_EQ_UINT(marker_info->xg_nominal_key, 0u);
    ASSERT_EQ_UINT(tone_info->xg_nominal_key, 0u);

    XgGlobalEvidence evidence = {0};
    ASSERT_TRUE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    ASSERT_EQ_UINT(evidence.nclasses, 1);
    uint64_t box_nominal_key = box_info->xg_nominal_key;
    uint64_t marker_nominal_key = marker_info->xg_nominal_key;
    uint64_t tone_nominal_key = tone_info->xg_nominal_key;
    ASSERT_EQ_UINT(box_info->declaration_key, box_declaration_key);
    ASSERT_EQ_UINT(marker_info->declaration_key, marker_declaration_key);
    ASSERT_EQ_UINT(tone_info->declaration_key, tone_declaration_key);
    XgInterfaceId marker_interface_id = marker_info->xg_interface_id;
    ASSERT_TRUE(box_nominal_key != 0);
    ASSERT_TRUE(marker_nominal_key != 0);
    ASSERT_TRUE(tone_nominal_key != 0);
    ASSERT_TRUE(marker_interface_id != XG_NO_ID);
    xg_global_evidence_free(&evidence);

    box_info->xg_decl_id = UINT32_MAX - 1;
    box_info->xg_class_id = UINT32_MAX - 2;
    box_info->xg_interface_id = XG_NO_ID;
    marker_info->xg_decl_id = UINT32_MAX - 3;
    tone_info->xg_decl_id = UINT32_MAX - 4;
    ASSERT_TRUE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    ASSERT_EQ_UINT(evidence.nclasses, 1);
    XgDeclId class_decl_id = XG_NO_ID;
    XgDeclId interface_decl_id = XG_NO_ID;
    XgDeclId enum_decl_id = XG_NO_ID;
    for (uint32_t i = 0; i < evidence.ndecls; i++) {
        if (evidence.decls[i].kind == XG_DECL_CLASS)
    ...
    ASSERT_TRUE(interface_decl_id != XG_NO_ID);
    ASSERT_TRUE(enum_decl_id != XG_NO_ID);
    ASSERT_EQ_UINT(box_info->xg_decl_id, evidence.classes[0].decl_id);
    ASSERT_EQ_UINT(box_info->xg_decl_id, class_decl_id);
    ASSERT_EQ_UINT(box_info->xg_class_id, evidence.classes[0].class_id);
    ASSERT_EQ_UINT(box_info->xg_interface_id, XG_NO_ID);
    ASSERT_EQ_UINT(box_info->xg_nominal_key, box_nominal_key);
    ASSERT_EQ_UINT(marker_info->xg_decl_id, interface_decl_id);
    ASSERT_EQ_UINT(marker_info->xg_interface_id, marker_interface_id);
    ASSERT_EQ_UINT(marker_info->xg_nominal_key, marker_nominal_key);
    ASSERT_EQ_UINT(tone_info->xg_decl_id, enum_decl_id);
    ASSERT_EQ_UINT(tone_info->xg_class_id, XG_NO_ID);
    ASSERT_EQ_UINT(tone_info->xg_interface_id, XG_NO_ID);
    ASSERT_EQ_UINT(tone_info->xg_nominal_key, tone_nominal_key);
    xg_global_evidence_free(&evidence);

    box_info->xg_nominal_key =
        box_nominal_key == UINT64_MAX ? box_nominal_key - 1 : box_nominal_key + 1;
    ASSERT_FALSE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    xg_global_evidence_free(&evidence);

    box_info->xg_nominal_key = box_nominal_key;
    box_info->xg_interface_id = UINT32_MAX;
    ASSERT_FALSE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    xg_global_evidence_free(&evidence);
    ...
    marker_info->xg_interface_id = marker_interface_id;
    tone_info->xg_class_id = UINT32_MAX;
    ASSERT_FALSE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    xg_global_evidence_free(&evidence);

    tone_info->xg_class_id = XG_NO_ID;
    box_info->xg_nominal_key = 0;
    box_info->xg_decl_id = UINT32_MAX;
    box_info->xg_class_id = XG_NO_ID;
    box_info->xg_interface_id = XG_NO_ID;
    ASSERT_FALSE(xg_global_evidence_build_from_module_graph_with_imported_modules_and_analyzer(
        &evidence, &graph, XG_BUILD_NATIVE_RELEASE, 0, NULL, 0, analyzer));
    xg_global_evidence_free(&evidence);
...
TEST(global_evidence_producer_resolves_namespace_class_constructor_and_local_methods) {
    setup_parser_session();
    const char *http_source = "export class Server {\n"
                              "    constructor() {}\n"
                              "    route(path: string) -> bool { return true }\n"
                              "}\n";
    const char *entry_source = "import http\n"
                               "fn serve() -> bool {\n"
...
TEST(global_evidence_producer_resolves_transitive_interface_implementors) {
    ...
    char *dump = xaot_bundle_dump_global_evidence_plan(&bundle);
    ASSERT_NOT_NULL(dump);
...
TEST(global_evidence_producer_records_derived_eq_hash_plan) {
    ...
    ASSERT_TRUE(ev.nhash_eqs >= 3);
    const XgHashEqSummary *key_hash_eq =
        xg_global_evidence_find_hash_eq(&ev, ev.derives[0].type_key);
    ASSERT_NOT_NULL(key_hash_eq);
    ASSERT_EQ_UINT(key_hash_eq->kind, XG_HASH_EQ_DERIVE);
    ASSERT_EQ_UINT(key_hash_eq->type_key, ev.derives[0].type_key);
    ASSERT_EQ_UINT(key_hash_eq->eq_derive_id, ev.derives[0].derive_id);
    ASSERT_EQ_UINT(key_hash_eq->hash_derive_id, ev.derives[1].derive_id);
...
TEST(global_evidence_rejects_unhashable_derived_eq_hash_field) {
    XgBuildKey key = {.source_hash = 61,
                      .compiler_semver_hash = 62,
                      .profile_hash = 63,
                      .imported_summary_hash = 64,
                      .module_id = 1,
                      .profile = XG_BUILD_NATIVE_RELEASE};
    XgGlobalEvidence ev;
    xg_global_evidence_init(&ev, key);
    XgDeriveSummary eq = {.derive_id = 1,
                          ...
                          .derive_hash = UINT64_C(0x6162)};
    XgDeriveSummary hash = {.derive_id = 2,
                            .module_id = 1,
                            .owner_decl_id = 2,
                            .source_span_id = 3,
                            .type_key = 4,
                            .derive_kind = XG_DERIVE_HASH,
                            .field_start = 2,
                            .field_count = 1,
                            .flags = XG_DERIVE_OPT_IN,
                            .derive_hash = UINT64_C(0x6163)};
    XgDerivedFieldSummary eq_field = {.field_id = 1,
                                      .derive_id = 1,
                                      .field_ordinal = 0,
                                      .name_id = 10,
                                      .type_key = 99,
    ...
    XgDerivedFieldSummary hash_field = {.field_id = 2,
                                        .derive_id = 2,
                                        .field_ordinal = 0,
                                        .name_id = 10,
                                        .type_key = 99,
    ...
    ASSERT_NOT_NULL(xg_global_evidence_add_derived_field(&ev, &eq_field));
    ASSERT_NOT_NULL(xg_global_evidence_add_derive(&ev, &hash));
    ASSERT_NOT_NULL(xg_global_evidence_add_derived_field(&ev, &hash_field));
    ...
    ASSERT_EQ_UINT(plan->unproven_reason, XAOT_EQ_HASH_UNPROVEN_UNHASHABLE_FIELD);
    ASSERT_EQ_UINT(plan->evidence, XAOT_EQ_HASH_EV_EQ_ROW | XAOT_EQ_HASH_EV_HASH_ROW |
                                       XAOT_EQ_HASH_EV_SAME_TYPE | XAOT_EQ_HASH_EV_SAME_FIELDS);
    char *plan_dump = xaot_bundle_dump_global_evidence_plan(&bundle);
    ASSERT_NOT_NULL(plan_dump);
    ASSERT_NOT_NULL(strstr(plan_dump, "reason=unhashable_field"));
    xr_free(plan_dump);
    xaot_bundle_free(&bundle);
    xg_global_evidence_free(&ev);
}
...
TEST(global_evidence_records_object_merge_plans) {
    ...
    ASSERT_NOT_NULL(strstr(dump, "kind=patch domain=0"));
...
TEST(global_evidence_records_options_bag_plans) {
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&stale_action_bundle, err, sizeof(err)));
    ...
    memset(err, 0, sizeof(err));
    ASSERT_TRUE(!xaot_verify_global_evidence_plan(&owner_mismatch_bundle, err, sizeof(err)));
    ASSERT_NOT_NULL(strstr(err, "AOT options evidence action does not re-derive"));
...
TEST(global_evidence_records_sequence_capacity_bulk_encoding_rows) {
    ...
    XgGlobalEvidence ev;
    xg_global_evidence_init(&ev, key);
    XgBodySummary owner_body = {.func_id = 7,
                                .module_id = 1,
                                .name_id = xg_name_id("sequence_owner"),
                                .kind = XG_BODY_MODULE_INIT,
                                .body_hash = UINT64_C(0x10701)};
    ...
    XgBulkOpSummary zero_fill_bulk = {.op_id = 3,
                                      .owner_func_id = 7,
                                      .source_span_id = 105,
                                      .body_ordinal = 2,
                                      .op_kind = XG_BULK_FILL,
                                      ...
                                      .dst_type_key = 702,
                                      .length_expr_id = 5,
                                      .flags = XG_BULK_POD | XG_BULK_ZERO_FILL};
    ...
    ASSERT_NOT_NULL(xg_global_evidence_add_bulk_op(&ev, &zero_fill_bulk));
    ...
    ASSERT_EQ_UINT(zero_fill_bulk_plan->action, XAOT_BULK_INLINE_MEMSET);
    ASSERT_EQ_UINT(zero_fill_bulk_plan->evidence, XAOT_BULK_EV_GLOBAL_ROW | XAOT_BULK_EV_POD |
    ...
    memset(&init_func, 0, sizeof(init_func));
    init_func.name = "init";
    init_func.xg_body_func_id = owner_body.func_id;
    memset(&module, 0, sizeof(module));
    module.path = "test.xr";
    module.name = "test";
    module.init = &init_func;
    ...
    XgGlobalEvidence *mutable_evidence = (XgGlobalEvidence *) bundle.global_evidence_plan.evidence;
...
TEST(global_evidence_producer_records_sequence_capacity_bulk_encoding_rows) {
    ...
    for (uint32_t i = 0; i < ev.nbulk_ops; i++) {
        ...
        if (ev.bulk_ops[i].op_kind == XG_BULK_FILL &&
            (ev.bulk_ops[i].flags & XG_BULK_ZERO_FILL) != 0 &&
            (ev.bulk_ops[i].flags & XG_BULK_POD) != 0) {
            ASSERT_TRUE((ev.bulk_ops[i].flags & XG_BULK_WRITE_BARRIER) == 0);
            saw_pod_zero_fill = true;
        }
    }
    ASSERT_TRUE(saw_bulk_copy);
    ASSERT_TRUE(saw_managed_fill);
    ASSERT_TRUE(saw_pod_zero_fill);
...
TEST(global_evidence_producer_records_json_codec_calls) {
    ...
    const char *source =
        ...
        "    var encoded: JSON.Value = JSON.value(user)\n"
    ...
    for (uint32_t i = 0; i < ev.nobject_shapes; i++) {
        ...
        ASSERT_TRUE(ev.object_shapes[i].shape_kind == XG_OBJECT_SHAPE_LITERAL ||
    ...
    for (uint32_t i = 0; i < ev.njson_codecs; i++) {
        ...
        for (uint32_t j = i + 1; j < ev.njson_codecs; j++) {
            ASSERT_TRUE(ev.json_codecs[i].owner_func_id != ev.json_codecs[j].owner_func_id ||
                        ev.json_codecs[i].source_node_id != ev.json_codecs[j].source_node_id);
        }
    }
    ...
    struct_shape = xg_global_evidence_find_object_shape(&ev, ev.json_codecs[1].output_shape_id);
...
TEST(global_evidence_producer_records_non_string_readonly_static_map_set_tables) {
    ...
    ASSERT_EQ_UINT(ev.nkey_accesses, 3);

    uint32_t static_maps = 0;
    uint32_t static_sets = 0;
    for (uint32_t i = 0; i < ev.nmap_shapes; i++) {
        const XgMapShapeSummary *shape = &ev.map_shapes[i];
        ASSERT_EQ_UINT(shape->source, XG_MAP_SHAPE_SRC_STATIC);
        ASSERT_EQ_UINT(shape->literal_count, 2);
        ASSERT_EQ_UINT(shape->entry_count, 2);
        ASSERT_TRUE((shape->flags & XG_MAP_SHAPE_LITERAL) != 0);
        ASSERT_TRUE((shape->flags & XG_MAP_SHAPE_STATIC) != 0);
        ASSERT_TRUE((shape->flags & XG_MAP_SHAPE_READONLY) != 0);
        if (shape->container_kind == XG_MAP_CONTAINER_MAP)
            static_maps++;
        else if (shape->container_kind == XG_MAP_CONTAINER_SET)
            static_sets++;
        else
            ASSERT_TRUE(false);

        for (uint32_t ordinal = 0; ordinal < shape->entry_count; ordinal++) {
            const XgMapEntrySummary *entry = &ev.map_entries[shape->entry_start - 1u + ordinal];
            ASSERT_EQ_UINT(entry->shape_id, shape->shape_id);
            ASSERT_EQ_UINT(entry->entry_ordinal, ordinal);
            ASSERT_TRUE((entry->flags & XG_MAP_ENTRY_CONST_KEY) != 0);
            ASSERT_TRUE((entry->flags & XG_MAP_ENTRY_INT_KEY) != 0);
            ASSERT_TRUE(entry->key_i64 == 1 || entry->key_i64 == 2);
            ASSERT_TRUE(entry->prehash != 0);
            if (shape->container_kind == XG_MAP_CONTAINER_MAP)
                ASSERT_TRUE((entry->flags & XG_MAP_ENTRY_CONST_VALUE) != 0);
        }
    }
    ASSERT_EQ_UINT(static_maps, 1);
    ASSERT_EQ_UINT(static_sets, 1);
    ...
    for (uint32_t i = 0; i < ev.nmap_shapes; i++) {
        const XaotMapShapePlan *shape_plan =
            xaot_bundle_find_map_shape_plan(&bundle, ev.map_shapes[i].shape_id);
        ASSERT_NOT_NULL(shape_plan);
        ASSERT_EQ_UINT(shape_plan->action, XAOT_MAP_SHAPE_READONLY_STATIC_TABLE);
    }
    uint32_t index_gets = 0;
    uint32_t map_gets = 0;
    uint32_t set_has = 0;
    for (uint32_t i = 0; i < ev.nkey_accesses; i++) {
        const XgKeyAccessSummary *access = &ev.key_accesses[i];
        const XaotKeyAccessPlan *access_plan =
            xaot_bundle_find_key_access_plan(&bundle, access->access_id);
        ASSERT_NOT_NULL(access_plan);
        ASSERT_EQ_UINT(access_plan->action, XAOT_KEY_ACCESS_PREHASHED_LOOKUP);
        ASSERT_TRUE((access_plan->evidence & XAOT_MAP_EV_CONST_KEY) != 0);
        ASSERT_TRUE((access_plan->evidence & XAOT_MAP_EV_PREHASH) != 0);
        if (access->container_kind == XG_MAP_CONTAINER_MAP && access->op == XG_KEY_ACCESS_INDEX_GET)
            index_gets++;
        else if (access->container_kind == XG_MAP_CONTAINER_MAP && access->op == XG_KEY_ACCESS_GET)
            map_gets++;
        else if (access->container_kind == XG_MAP_CONTAINER_SET && access->op == XG_KEY_ACCESS_HAS)
            set_has++;
    }
    ASSERT_EQ_UINT(index_gets, 1);
...
TEST(global_evidence_composes_source_class_receiver_effects) {
    ...
    const XgCallsiteSummary *wait_call = NULL;
    for (uint32_t i = 0; i < ev.ncallsites; i++) {
        if (ev.callsites[i].kind == XG_CALL_METHOD &&
            ev.callsites[i].method_name_id == xg_name_id("wait")) {
            wait_call = &ev.callsites[i];
            break;
        }
    }
    ASSERT_NOT_NULL(wait_call);
    uint32_t init_effects = 0;
    ASSERT_TRUE(xg_callsite_effects_compose_closed_world_calls(&ev, wait_call, &init_effects));
...
TEST(entry_plan_defers_open_function_value_reachability_until_callable_convergence) {
    ...
    XgBodySummary handler = {.func_id = 2,
                             ...
                             .capability_bits = XG_CAP_TIMER,
                             .body_hash = 0x197b};
    XgCallsiteSummary call = {.callsite_id = 1, .owner_func_id = 1, .kind = XG_CALL_CLOSURE};
    XiFunc init_func;
    XiFunc handler_func;
    XiFunc *children[1];
    XiModule module;
...
TEST(storage_and_capture_plans_close_owner_actions) {
    ...
    init_func.xg_body_func_id = entry.func_id;
    init_func.nshared = 2;
    XiModuleSlot slot_declarations[2] = {
        {.name = slot_names[0], .is_const = slot_consts[0] != 0},
        {.name = slot_names[1], .is_const = slot_consts[1] != 0},
    };
    init_func.module_slots = slot_declarations;
    init_func.children = children;
    init_func.nchildren = 1;
    static_addr.id = 7;
...
RUN_TEST(global_evidence_package_import_remaps_generic_constraint_interface);
...
RUN_TEST(global_evidence_verifier_rejects_missing_generic_constraint_interface);
...
RUN_TEST(global_evidence_generic_effect_identity_is_orthogonal_to_type_tuple);
RUN_TEST(global_evidence_final_generic_identity_fails_closed_without_semantic_binding);
RUN_TEST(global_evidence_generic_cache_preserves_identity_high_bits);
...
RUN_TEST(global_evidence_rejects_unhashable_derived_eq_hash_field);
