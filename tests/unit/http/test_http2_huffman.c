...
static void collect_header(const char *name, size_t name_len,
                           const char *value, size_t value_len,
                           void *user_data) {
    CollectedHeaders *c = (CollectedHeaders *)user_data;
    if (c->count >= 8) return;
    if (name_len >= sizeof(c->name[0])) name_len = sizeof(c->name[0]) - 1;
    if (value_len >= sizeof(c->value[0])) value_len = sizeof(c->value[0]) - 1;
...
TEST(huffman_rfc7541_c_4_1) {
    ...
    ASSERT_STR_EQ(c.name[0], ":method");   ASSERT_STR_EQ(c.value[0], "GET");
    ASSERT_STR_EQ(c.name[1], ":scheme");   ASSERT_STR_EQ(c.value[1], "http");
    ASSERT_STR_EQ(c.name[2], ":path");     ASSERT_STR_EQ(c.value[2], "/");
...
TEST(huffman_rfc7541_c_6_1_response) {
    static const uint8_t block[] = {
        ...
        0x61, 0x96, 0xd0, 0x7a, 0xbe, 0x94, 0x10, 0x54, 0xd4, 0x44,
              0xa8, 0x20, 0x05, 0x95, 0x04, 0x0b, 0x81, 0x66, 0xe0,
              0x82, 0xa6, 0x2d, 0x1b, 0xff,
        0x6e, 0x91, 0x9d, 0x29, 0xad, 0x17, 0x18, 0x63, 0xc7, 0x8f,
    ...
    ASSERT_EQ_INT(rc, 0);
    ASSERT_EQ_INT(c.count, 4);
    ASSERT_STR_EQ(c.name[0], ":status");         ASSERT_STR_EQ(c.value[0], "302");
    ASSERT_STR_EQ(c.name[1], "cache-control");   ASSERT_STR_EQ(c.value[1], "private");
