...
static void test_plan_snapshot_and_determinism(void) {
    ...
    REQUIRE(strcmp(target_hex,
                   "b450fd414f9b9b7429faf3937064663e680991d39ea58d39e9f37a29b8499920") == 0);
...
static XrSemanticPlan *build_direct_local_tagged_ref_semantic(XrType *value_type,
    ...
    XiValue *root_value = append_stringbuilder ? emit_exact_string_builder(root, root_entry)
                                               : root->params[0];
    XiValue *closure = xi_value_new(root, root_entry, XI_STACK_ALLOC, &stub_function, 0);
...
static XrSemanticPlan *build_source_class_array_push_semantic(void) {
    ...
    XiModule *module =
        xi_module_new("pkg/target_source_class_array_push.xr", "target_source_class_array_push",
                      function);
    REQUIRE(module != NULL);
    REQUIRE(xi_module_set_identity(
        module, "memory-module-v1:id=33:target-source-class-array-push-v1"));
    function->module = module;
    XiClassData source_class = {
        .class_info = &stub_target_source_class_info,
        .class_name = "FinalTargetWorker",
...
static XrSemanticPlan *build_source_export_semantic(XrSemanticPlan **dependency_out,
    ...
    XiFunc *write_bytes = xi_func_new("writeBytes", exact_i64 ? &stub_int : &stub_unit);
    ...
    if (with_argument) {
        ...
        write_bytes->params[0] =
            xi_param(write_bytes, write_entry, 0,
                     reference ? &stub_raw_pointer : (exact_i64 ? &stub_int : &stub_bool));
    ...
    xi_block_set_return(write_entry, exact_i64 ? write_bytes->params[0] : NULL);
    ...
    XiFunc *caller = xi_func_new("_serverWriteAll", exact_i64 ? &stub_int : &stub_unit);
    ...
    XiValue *method = xi_value_new(caller, caller_entry, XI_CALL_METHOD,
                                   exact_i64 ? &stub_int : &stub_unit, with_argument ? 2 : 1);
...
static void test_imported_source_class_constructor_authority(void) {
    ...
    REQUIRE(operation && operation->opcode == XI_CALL &&
            ...
            strstr(semantic_target->canonical_key, "call-target-v10:schema=40:") != NULL &&
...
static void test_channel_close_call_authority(void) {
    ...
    REQUIRE(strcmp(call_hex, "5c64cffa00dc068422364cbfa0dd22001050d5c0882704138b849799fbced411") ==
            0);
...
static void test_direct_local_call_adapter_family(void) {
    ...
    REQUIRE(strcmp(call_hex, "e741fe1df7e93c6d63a37f27f0af86f1cdcb4229835dfc0f5519f0b03804f332") ==
            0);
...
static void test_tail_coroutine_chain_fingerprint(void) {
    ...
    REQUIRE(strcmp(tail_hex, "04b20272aa64f26614244779e59bbbe8741330111ce5e7006e2f35d4d37efa7d") ==
            0);
...
static void test_source_class_array_push_authority(void) {
    ...
    XrTargetCallArgumentRecord *receiver_argument =
...
static XrTargetCallRecord *expect_dynamic_stringbuilder_call(XrTargetPlan *plan, uint8_t convention,
    ...
    uint8_t saved_ownership = call->result_ownership;
    call->result_ownership = saved_ownership == XR_TARGET_CALL_BORROW
                                 ? XR_TARGET_CALL_RETURN_OWNED
    ...
    plan->slots[result->slot].ownership =
        saved_slot_ownership == XR_TARGET_OWNERSHIP_BORROWED ? XR_TARGET_OWNERSHIP_OWNED
                                                             : XR_TARGET_OWNERSHIP_BORROWED;
    expect_verify_failure(plan, "XR_TARGET_1001");
...
static void test_direct_local_stringbuilder_ref_append_authority(void) {
    ...
    REQUIRE(append != NULL &&
            append->result_ownership == XI_GEN_RESULT_OWNERSHIP_BORROWED &&
            append->result_alias_operand == 0);
    bool built = xr_target_plan_build(semantic, profile, &plan, error, sizeof(error));
    if (!built)
