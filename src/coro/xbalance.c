...
struct XrWorker *xr_choose_target_worker(struct XrRuntime *runtime, int exclude) {
    if (!runtime || runtime->worker_count <= 1) return NULL;
    ...
    int a = (int)(r % n);
    int b = (int)((r >> 16) % n);
    if (a == exclude) a = (a + 1) % n;
    if (b == exclude || b == a) b = (b + 1) % n;
    if (b == exclude) b = (b + 1) % n;
    if (b >= n) b = 0;
...
void xr_check_balance(struct XrRuntime *runtime, struct XrWorker *worker) {
    ...
    int expected = 0;
    if (!atomic_compare_exchange_strong(&runtime->balance_info.checking_balance,
                                        &expected, 1)) {
    ...
    for (int p = 0; p < XR_RUNQ_COUNT; p++) {
        ...
        for (int i = 0; i < wc; i++) {
            if (len[i][p] > max_l) { max_l = len[i][p]; max_w = i; }
            if (len[i][p] < min_l) { min_l = len[i][p]; min_w = i; }
        }
...
int xr_try_emigrate(struct XrWorker *worker) {
    if (!worker || !worker->p.runtime) return 0;
    ...
    for (int p = 0; p < XR_RUNQ_COUNT; p++) {
        ...
        if (target_id < 0 || target_id >= runtime->worker_count) continue;
        
        int len = xr_runq_len(&worker->p.runq[p]);
        if (len <= mp->prio[p].limit_here) continue;
        
        struct XrWorker *target = &runtime->workers[target_id];
        int stolen = xr_runq_steal(&worker->p.runq[p], &target->p.runq[p],
                                    XR_MIGRATION_MAX_STEAL);
        if (stolen > 0) {
...
void xr_runq_get_info(struct XrWorker *worker, int prio, int *len, int *reds) {
    ...
    int idx = prio_to_runq(prio);
    if (len) *len = xr_runq_len(&worker->p.runq[idx]);
    if (reds) *reds = (idx >= 0 && idx < XR_RUNQ_COUNT) ? worker->p.runq_reds[idx] : 0;
}
