...
XR_FUNC bool xr_task_register_await_node(XrTask *task, XrCoroutine *waiter, XrTaskAwaitNode *node,
                                         int waiter_index) {
    if (!task || !waiter || !node)
        return false;
...
XrValue xr_task_consume_result_copy(XrRuntimeCore *core, XrTask *task,
