...
bool xr_steal_queue_push(XrStealQueue *q, struct XrCoroutine *coro) {
    if (!q || !q->buffer || !coro) return false;
...
struct XrCoroutine* xr_steal_queue_steal(XrStealQueue *q) {
    ...
    // Read data
    struct XrCoroutine *coro = atomic_load_explicit(
        &q->buffer[t & q->mask], memory_order_relaxed);
...
int xr_steal_queue_snapshot(XrStealQueue *q, struct XrCoroutine **out_buf, int max_out) {
    if (!q || !q->buffer || !out_buf || max_out <= 0) return 0;
    ...
    for (int64_t i = t; i < b && count < max_out; i++) {
        struct XrCoroutine *c = atomic_load_explicit(
            &q->buffer[i & q->mask], memory_order_relaxed);
        if (c) {
            out_buf[count++] = c;
        }
    }
