    ...
    if (offset < 0 && vm_ctx && vm_ctx->current_coro) {
        ...
        ** to "no". */
        ...
        if (isolate->vm.jit) {
            ...
            bool _osr_static_blocked =
                atomic_load_explicit(&_osr_proto->jit_static_blocked, memory_order_relaxed) != 0;
            uint32_t _osr_threshold = (uint32_t) isolate->vm.jit_threshold;
            if (_osr_threshold == 0)
                _osr_threshold = 1;
            void *_osr_pending =
                atomic_load_explicit(&_osr_proto->jit_entry_pending, memory_order_acquire);
            bool _osr_probe_active = !_osr_static_blocked &&
                                     (_osr_proto->jit_entry || _osr_pending ||
                                      atomic_load_explicit(&_osr_proto->exec_count,
                                                           memory_order_relaxed) < _osr_threshold);
            if (_osr_probe_active && !xm_proto_has_exception_control(_osr_proto)) {
                bool _do_osr = false;
                ...
                if (_do_osr) {
                    ...
                    xr_coro_jit_state(coro)->scratch->call_closure = cl;
                    xr_coro_jit_state(coro)->scratch->osr_deopt_pc = -1;
                    XrValue _osr_result;
                    int _osr_rc =
                        xm_jit_osr_trigger(isolate->vm.jit, _osr_proto, coro, _target_pc, base,
                                           _osr_proto->maxstacksize,
                    ...
                    if (_osr_rc == XM_JIT_OK) {
                        ...
                        if (!ci->closure || !ci->closure->proto)
                            return XR_VM_OK;
                        VM_SET_STACK_TOP(VM_STACK + ci->base_offset +
                    ...
                    if (xr_coro_jit_state(coro)->scratch->osr_deopt_pc >= 0) {
                        pc = PROTO_CODE_BASE(_osr_proto) +
                             xr_coro_jit_state(coro)->scratch->osr_deopt_pc;
                        xr_coro_jit_state(coro)->scratch->osr_deopt_pc = -1;
                        offset = 0;
                    }
                }
