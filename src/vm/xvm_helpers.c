...
/*
 * Create closure object with flat upvalue array.
 * upvals[] is sized from proto->upvalues count; populated by OP_CLOSURE.
 */
XrClosure *xr_vm_closure_new(XrayIsolate *isolate, XrProto *proto,
    ...
    // Initialize all upvals to null (populated by OP_CLOSURE)
    for (int i = 0; i < nuv; i++) {
...
// Closures are GC-managed, no manual free needed
