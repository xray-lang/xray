 ...
 *
 * xvm_invoke.c - Dispatch helpers for invoke / superinvoke
 *
 * Implements per-receiver-type dispatch for OP_INVOKE and
 * OP_SUPERINVOKE. Each function takes the interpreter state as
 * parameters, so this is a normal translation unit (unlike the
 * xvm_dispatch_*.inc.c files included from inside the run()
 * switch). Declarations live in xvm_dispatch_helpers.h.
 *
 * Owns:
...
/* ========== Dispatch: OP_INVOKE Task Handle ========== */
...
XR_FUNC XrDispatchAction vm_invoke_task_handle(XrVMRuntime *isolate, XrVMContext *vm_ctx,
    ...
    if (nargs == 0 && method_symbol == SYMBOL_CANCEL) {
        ...
        if (!xr_task_is_done(task) && coro) {
            ...
            /* A coroutine that owes a `defer` chain unwinds on its own worker
             * and marks its task cancelled when that finishes. Completing the
             * task here instead would let an awaiter observe the cancellation
             * before the cleanup it is waiting on has run. */
    ...
    if (nargs == 1 && method_symbol == SYMBOL_LINK) {
        ...
        if (!xr_value_is_task(arg)) {
            VM_THROW(frame, pc, XR_ERR_TYPE_MISMATCH, "task.link: argument must be a Task");
        }
    ...
    if (nargs == 1 && method_symbol == SYMBOL_UNLINK) {
        ...
        XrValue arg = base[a + 2];
        if (!xr_value_is_task(arg)) {
            VM_THROW(frame, pc, XR_ERR_TYPE_MISMATCH, "task.unlink: argument must be a Task");
        }
        XrTask *other = xr_value_to_task(arg);
    ...
    VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "task handle does not support this method");
}
...
XR_FUNC XrDispatchAction vm_invoke_coro_handle(XrVMRuntime *isolate, XrValue receiver,
    ...
    VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "coroutine handle does not support this method");
}
...
/* ========== Dispatch: OP_INVOKE Class Constructor/Static Method ========== */
...
XR_FUNC XrDispatchAction vm_invoke_class(XrVMRuntime *isolate, XrVMContext *vm_ctx,
    ...
    if (strcmp(method_name_chars, XR_KEYWORD_CONSTRUCTOR) == 0) {
        ...
        if (method == NULL || !(method->flags & XMETHOD_FLAG_STATIC)) {
            VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "static method '%s' not found on class '%s'",
        ...
        if (method->type == XMETHOD_PRIMITIVE && method->as.primitive != NULL) {
            ...
            VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "invalid static method '%s'",
                     method_name_chars);
        }
    }
}

/* ========== Dispatch: OP_SUPERINVOKE ========== */
...
XR_FUNC XrDispatchAction vm_superinvoke(XrVMRuntime *isolate, XrVMContext *vm_ctx,
    ...
    if (current_method_class == NULL) {
        VM_THROW(frame, pc, XR_ERR_TYPE_MISMATCH, "cannot determine current method's class");
    }

    XrClass *super_class = current_method_class->super;
    if (super_class == NULL) {
        VM_THROW(frame, pc, XR_ERR_TYPE_MISMATCH, "class '%s' has no superclass",
                 current_method_class->name);
    }

    XrValue method_name_val = PROTO_CONSTANT(frame->closure->proto, b);
    if (!XR_IS_STRING(method_name_val)) {
        VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "invalid method name in super call");
    }
    ...
    if (method == NULL || method->type == XMETHOD_NONE) {
        VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "superclass method '%s' not found in class '%s'",
    ...
    if (method->type != XMETHOD_CLOSURE || method->as.closure == NULL) {
        VM_THROW(frame, pc, XR_ERR_TYPE_NO_METHOD, "superclass method '%s' has invalid type",
                 method_name);
    }
