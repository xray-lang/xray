...
static bool boundary_value(XrXirScalar value, XrXirType type) {
    if (type == XR_XIR_UNIT)
        return value.type == XR_XIR_UNIT && !value.reserved && !value.payload;
    return xr_xir_scalar_argument(&value, type);
}
...
static bool entry_arguments(const XrXirCallEntry *entry, const XrXirScalar *arguments,
                             uint32_t count) {
    if (count != entry->parameter_count || (count && !arguments))
        return false;
    for (uint32_t i = 0; i < count; ++i)
        if (!xr_xir_scalar_argument(&arguments[i], entry->parameters[i]))
            return false;
    return true;
}
...
static XrXirCallStatus push_frame(XrXirCall *call, uint32_t id,
                                  const XrXirScalar *arguments, uint32_t count) {
    ...
    uint64_t bytes = arguments_offset + (uint64_t) count * sizeof(XrXirScalar);
    ...
    frame->arguments = (XrXirScalar *) ((unsigned char *) frame + (size_t) arguments_offset);
    if (count)
        memcpy(frame->arguments, arguments, (size_t) count * sizeof(*arguments));
    call->top = frame;
    ++accounting->depth;
    ...
    *bytes = sizeof(XrXirCall) + (uint64_t) config->entry_count * sizeof(XrXirCallEntry);
    ...
    for (uint32_t i = 0; i < config->entry_count; ++i) {
        ...
        if (!entry->resume || entry->parameter_count > 65536 ||
            ...
            (entry->result != XR_XIR_UNIT && entry->result != XR_XIR_BOOL && entry->result != XR_XIR_I64))
            return XR_XIR_CALL_BAD_ARGUMENT;
...
static void accept_action(XrXirCall *call, XrXirAction action) {
    if (action.kind == XR_XIR_ACTION_CALL) {
        ...
        call->top->inbox = call_result(XR_XIR_CALL_READY);
    ...
    XrXirCallResult result = {returning ? XR_XIR_CALL_RETURNED : XR_XIR_CALL_THROWN, action.value, 0};
