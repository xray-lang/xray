...
#define XR_ARRAY_MEMBERS(M) \
    ...
    M("splice",     "(start: int, deleteCount?: int, ...items: T): Array<T>",   "Change array contents",            true)  \
...
#define XR_STRING_MEMBERS(M) \
    ...
    M("replace",     "(search: string, replacement: string): string",           "Replace first occurrence",         true)  \
    M("replaceAll",  "(search: string, replacement: string): string",           "Replace all occurrences",          true)  \
