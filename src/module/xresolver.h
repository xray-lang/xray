...
XR_FUNC bool xr_depgraph_add_root(XrDepGraph *graph, const char *name, const char *constraint);
XR_FUNC XrDepNode *xr_depgraph_find(XrDepGraph *graph, const char *name);

/* ========== Resolution API ========== */

// lockfile is optional (for pinning versions)
XR_FUNC XrResolveResult *xr_resolve_dependencies(XrDepGraph *graph, XrPackageInfoFn get_info,
