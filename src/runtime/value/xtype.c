...
XrType *xr_type_new_json_value(XrayIsolate *X) {
    ...
    XrType *raw_members[6] = {
        xr_type_new_bool(X),   xr_type_new_int(X),  xr_type_new_float(X),
        xr_type_new_string(X), xr_type_new_json(X), xr_type_new_array(X, xr_type_new_unknown(X)),
    };
    XrType *type = type_alloc(X, XR_KIND_UNION);
    if (!type)
        return xr_type_new_unknown(X);
...
bool xr_type_is_json_value(XrType *type) {
    ...
    if (type->kind != XR_KIND_UNION)
        return false;
    if (type->union_type.member_count != 6)
        return false;
