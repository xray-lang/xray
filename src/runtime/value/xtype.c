...
static void init_singleton(XrType *t, XrTypeKind kind, uint32_t id,
                           bool nullable, uint8_t native_width) {
...
static XrType *type_alloc(XrTypeKind kind) {
    XrTypePool *pool = get_current_pool();
    XR_CHECK(pool != NULL, "Type pool not set - call xr_type_set_current_pool first");
    return xr_pool_alloc_type(pool, kind);
}

XrType *xr_type_new(XrTypeKind kind) {
    return type_alloc(kind);
}

// Primitive type constructors (return process-level singletons)
XrType *xr_type_new_int(void)     { return &g_type_int; }
XrType *xr_type_new_float(void)   { return &g_type_float; }
XrType *xr_type_new_string(void)  { return &g_type_string; }
XrType *xr_type_new_bool(void)    { return &g_type_bool; }
XrType *xr_type_new_null(void)    { return &g_type_null; }
XrType *xr_type_new_unknown(void) { return &g_type_unknown; }
XrType *xr_type_new_never(void)   { return &g_type_never; }
XrType *xr_type_new_void(void)    { return &g_type_void; }

// Native-width integer type (int8/16/32/64, uint8/16/32/64)
XrType *xr_type_new_int_width(int width) {
    XrType *type = type_alloc(XR_KIND_INT);
    if (!type) return NULL;
...
XrType *xr_type_new_float_width(int width) {
    XrType *type = type_alloc(XR_KIND_FLOAT);
    if (!type) return NULL;
    type->native_width = (uint8_t)width;
    return type;
}
...
XrType *xr_type_new_set(XrType *element_type) {
    XrType *type = type_alloc(XR_KIND_SET);
    if (!type) return NULL;
    type->container.element_type = element_type;
    return type;
}

XrType *xr_type_new_channel(XrType *element_type) {
    XrType *type = type_alloc(XR_KIND_CHANNEL);
    if (!type) return NULL;
    type->container.element_type = element_type;
    return type;
}

XrType *xr_type_new_task(XrType *result_type) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (!type) return NULL;
...
// Object types
XrType *xr_type_new_json(void) {
    return &g_type_json;  // Process-level singleton (plain Json without fields)
}

XrType *xr_type_new_json_with_fields(const char **names, XrType **types, int count) {
    XrType *type = type_alloc(XR_KIND_JSON);
    if (!type) return NULL;
    XrTypePool *pool = get_current_pool();
...
XrType *xr_type_new_object(const char **field_names, XrType **field_types,
    ...
    if (field_count > 0 && field_names) {
        ...
        if (type->object.field_names) {
            for (int i = 0; i < field_count; i++) {
                type->object.field_names[i] = field_names[i] ? xr_pool_strdup(pool, field_names[i]) : NULL;
            }
        }
    }
...
XrType *xr_type_new_object_anonymous(const char **field_names, XrType **field_types, int field_count) {
    return xr_type_new_object(field_names, field_types, field_count, true, NULL);
}

// Optional type (T?) - unified: uses is_nullable on the base type itself
XrType *xr_type_new_optional(XrType *base_type) {
    return xr_type_make_nullable(base_type);
}

XrType *xr_type_get_base(XrType *optional_type) {
    return xr_type_non_nullable(optional_type);
}

// Type parameter (for generics)
XrType *xr_type_new_type_param(const char *name, int id) {
    XrType *type = type_alloc(XR_KIND_TYPE_PARAM);
    if (!type) return NULL;
    XrTypePool *pool = get_current_pool();
...
XrType *xr_type_new_type_param_constrained(const char *name, int id, XrType *constraint) {
    XrType *type = xr_type_new_type_param(name, id);
    if (type) {
...
XrType *xr_type_new_class(const char *class_name) {
    XrType *type = type_alloc(XR_KIND_CLASS);
    if (type && class_name) {
        XrTypePool *pool = get_current_pool();
...
XrType *xr_type_new_interface(const char *interface_name) {
    XrType *type = type_alloc(XR_KIND_INTERFACE);
...
XrType *xr_type_new_bigint(void) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (type) type->instance.class_name = "BigInt";
    return type;
}

XrType *xr_type_new_datetime(void) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (type) type->instance.class_name = "DateTime";
    return type;
}

XrType *xr_type_new_bytes(void) {
    XrType *type = type_alloc(XR_KIND_BYTES);
    return type;
}

XrType *xr_type_new_regex(void) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (type) type->instance.class_name = "Regex";
    return type;
}

XrType *xr_type_new_stringbuilder(void) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (type) type->instance.class_name = "StringBuilder";
    return type;
}

XrType *xr_type_new_named_instance(const char *name) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (type) type->instance.class_name = name;
    return type;
}

XrType *xr_type_new_enum(const char *enum_name) {
    XrType *type = type_alloc(XR_KIND_ENUM);
    if (type && enum_name) {
...
XrType *xr_type_new_instance(XrClassInfo *class_info) {
    XrType *type = type_alloc(XR_KIND_INSTANCE);
    if (!type) return NULL;
...
XrType *xr_type_new_generic_instance(const char *class_name, XrClassInfo *class_info,
...
// Function type
XrType *xr_type_new_function(XrType **param_types, int param_count,
    ...
    XrType *type = type_alloc(XR_KIND_FUNCTION);
    if (!type) return NULL;
    XrTypePool *pool = get_current_pool();
...
XrType *xr_type_new_tuple(XrType **element_types, int count) {
    if (count <= 0) return xr_type_new_void();
    if (count == 1 && element_types && element_types[0]) {
        return element_types[0];  // Single element is not a tuple
    }

    XrType *type = type_alloc(XR_KIND_TUPLE);
    if (!type) return NULL;
    XrTypePool *pool = get_current_pool();
...
int xr_type_tuple_count(XrType *type) {
    if (!type || !XR_TYPE_IS_TUPLE(type)) return 0;
    return type->tuple.element_count;
}

XrType *xr_type_tuple_get(XrType *type, int index) {
    if (!type || !XR_TYPE_IS_TUPLE(type)) return NULL;
    if (index < 0 || index >= type->tuple.element_count) return NULL;
...
static int union_normalize(XrType **in, int in_count,
                           XrType **out, bool *out_nullable) {
    bool has_null = false;
    bool has_int = false;
    ...
    for (int i = 0; i < in_count; i++) {
        XrType *t = in[i];
        if (!t || XR_TYPE_IS_NEVER(t)) continue;
        if (XR_TYPE_IS_UNKNOWN(t)) return -1;
        if (XR_TYPE_IS_NULL(t)) { has_null = true; continue; }
        if (t->is_nullable) { has_null = true; }
        if (XR_TYPE_IS_INT(t)) has_int = true;
        if (XR_TYPE_IS_FLOAT(t)) has_float = true;

        // Dedup by xr_type_equals
        bool dup = false;
        for (int j = 0; j < out_count; j++) {
            if (xr_type_equals(out[j], t)) { dup = true; break; }
        }
...
XrType *xr_type_new_union(XrType **members, int count) {
    ...
    // unknown fallback
    if (result_count < 0) return xr_type_new_unknown();

    // Degenerate: empty -> never
    if (result_count == 0) {
        return has_null ? xr_type_new_null() : xr_type_new_never();
    }

    // Single member
    if (result_count == 1) {
        if (has_null) return xr_type_make_nullable(result[0]);
        return result[0];
    }

    // Exceeds max -> degrade to unknown
    if (result_count > XR_UNION_MAX_MEMBERS) return xr_type_new_unknown();

    // 3. Sort by kind for canonical order
    union_sort_by_kind(result, result_count);

    // 4. Allocate union type
    XrType *type = type_alloc(XR_KIND_UNION);
    if (!type) return xr_type_new_unknown();
    XrTypePool *pool = get_current_pool();
    type->union_type.members = (XrType **)xr_pool_alloc(
        pool, sizeof(XrType *) * result_count);
    if (!type->union_type.members) return xr_type_new_unknown();
    memcpy(type->union_type.members, result, sizeof(XrType *) * result_count);
    type->union_type.member_count = (uint8_t)result_count;
    if (has_null) type->is_nullable = true;
    return type;
}

// Merge two types into a union (or apply special rules).
// Type union rules: int|float->float, T|null->T?
XrType *xr_type_union(XrType *a, XrType *b) {
    if (!a) return b;
    if (!b) return a;
    if (xr_type_equals(a, b)) return a;

    // unknown | T = unknown
    if (XR_TYPE_IS_UNKNOWN(a) || XR_TYPE_IS_UNKNOWN(b)) return xr_type_new_unknown();

    // never | T = T
    if (XR_TYPE_IS_NEVER(a)) return b;
    if (XR_TYPE_IS_NEVER(b)) return a;

    // Numeric promotion: int | float -> float
    if (XR_TYPE_IS_INT(a) && XR_TYPE_IS_FLOAT(b)) return b;
    if (XR_TYPE_IS_FLOAT(a) && XR_TYPE_IS_INT(b)) return a;

    // T | null = T? (nullable shortcut)
    if (XR_TYPE_IS_NULL(a) && !XR_TYPE_IS_NULL(b))
        return xr_type_make_nullable(b);
    if (XR_TYPE_IS_NULL(b) && !XR_TYPE_IS_NULL(a))
        return xr_type_make_nullable(a);

    // General case: create real union
    XrType *pair[2] = {a, b};
    return xr_type_new_union(pair, 2);
}

// Union accessor: number of members (0 if not a union)
int xr_type_union_count(XrType *type) {
    if (!type || !XR_TYPE_IS_UNION(type)) return 0;
...
XrType *xr_type_union_member(XrType *type, int index) {
    if (!type || !XR_TYPE_IS_UNION(type)) return NULL;
    if (index < 0 || index >= type->union_type.member_count) return NULL;
...
bool xr_type_union_contains(XrType *type, XrTypeKind kind) {
    if (!type || !XR_TYPE_IS_UNION(type)) return type && type->kind == kind;
    for (int i = 0; i < type->union_type.member_count; i++) {
        if (type->union_type.members[i]->kind == kind) return true;
    }
    return false;
}

// Remove all members with given kind from union; return simplified type
XrType *xr_type_union_remove(XrType *type, XrTypeKind kind) {
    if (!type) return NULL;
    if (!XR_TYPE_IS_UNION(type)) {
        return type->kind == kind ? xr_type_new_never() : type;
    }
    ...
    if (kept_count == 0) return xr_type_new_never();
    if (kept_count == 1) {
        XrType *r = kept[0];
        if (type->is_nullable) r = xr_type_make_nullable(r);
        return r;
    }
    XrType *r = xr_type_new_union(kept, kept_count);
    if (type->is_nullable && r) r->is_nullable = true;
    return r;
}
...
XrType *xr_type_new_json_value(void) {
    ...
    XrType *raw_members[6] = {
        ...
        xr_type_new_string(),
        xr_type_new_json(),
        xr_type_new_array(xr_type_new_unknown()),
    };
    XrType *type = type_alloc(XR_KIND_UNION);
    if (!type) return xr_type_new_unknown();
    XrTypePool *pool = get_current_pool();
...
bool xr_type_is_json_value(XrType *type) {
    ...
    if (type->union_type.member_count != 6) return false;
    bool has[6] = {false};  // bool, int, float, string, json, array
    for (int i = 0; i < 6; i++) {
        XrType *m = type->union_type.members[i];
        if (!m) return false;
        switch (m->kind) {
        case XR_KIND_BOOL:   has[0] = true; break;
        case XR_KIND_INT:    has[1] = true; break;
        case XR_KIND_FLOAT:  has[2] = true; break;
        case XR_KIND_STRING: has[3] = true; break;
        case XR_KIND_JSON:   has[4] = true; break;
        case XR_KIND_ARRAY:  has[5] = true; break;
...
static XrType *xr_type_union_filter(XrType *type, XrTypeKind kind) {
    if (!type) return NULL;
    if (!XR_TYPE_IS_UNION(type)) {
        return type->kind == kind ? type : xr_type_new_never();
    }
    for (int i = 0; i < type->union_type.member_count; i++) {
        if (type->union_type.members[i]->kind == kind)
...
XrType *xr_type_new_fixed_array(XrType *element_type, int length) {
    XrType *type = type_alloc(XR_KIND_FIXED_ARRAY);
    if (!type) return NULL;
...
// Copy a type
XrType *xr_type_copy(XrType *type) {
    if (!type) return NULL;
    XR_DCHECK(type->kind < XR_KIND_COUNT, "type_copy: invalid kind");

    XrType *copy = type_alloc(type->kind);
    if (!copy) return NULL;
    ...
    switch (type->kind) {
    case XR_KIND_ARRAY: case XR_KIND_SET:
    ...
    case XR_KIND_MAP:
        ...
        break;
    case XR_KIND_ENUM:
        copy->enum_type.enum_name = type->enum_type.enum_name ?
            xr_pool_strdup(pool, type->enum_type.enum_name) : NULL;
        break;
    case XR_KIND_INSTANCE: case XR_KIND_CLASS:
        copy->instance.class_name = type->instance.class_name ? xr_pool_strdup(pool, type->instance.class_name) : NULL;
        ...
        if (type->instance.type_arg_count > 0 && type->instance.type_args) {
            copy->instance.type_args = xr_pool_alloc(pool, sizeof(XrType*) * type->instance.type_arg_count);
    ...
    case XR_KIND_FUNCTION:
        if (type->function.param_count > 0) {
            copy->function.param_types = xr_pool_alloc(pool, sizeof(XrType*) * type->function.param_count);
    ...
    case XR_KIND_JSON:
        if (type->object.field_count > 0) {
            ...
            copy->object.type_name = type->object.type_name ? xr_pool_strdup(pool, type->object.type_name) : NULL;
            if (type->object.field_names) {
                copy->object.field_names = xr_pool_alloc(pool, sizeof(const char*) * type->object.field_count);
                if (copy->object.field_names) {
                    for (int i = 0; i < type->object.field_count; i++) {
                        copy->object.field_names[i] = type->object.field_names[i] ?
                            xr_pool_strdup(pool, type->object.field_names[i]) : NULL;
                    }
                }
            }
            if (type->object.field_types) {
                copy->object.field_types = xr_pool_alloc(pool, sizeof(XrType*) * type->object.field_count);
    ...
    case XR_KIND_TYPE_PARAM:
        copy->type_param.name = type->type_param.name ? xr_pool_strdup(pool, type->type_param.name) : NULL;
    ...
    case XR_KIND_UNION:
        ...
        if (type->union_type.member_count > 0 && type->union_type.members) {
            copy->union_type.members = xr_pool_alloc(pool,
...
XrType *xr_type_make_nullable(XrType *type) {
    if (!type) return NULL;
    ...
    if (type == &g_type_float)  return &g_type_float_nullable;
    if (type == &g_type_string) return &g_type_string_nullable;
    if (type == &g_type_bool)   return &g_type_bool_nullable;
    ...
    if (type->frozen && g_current_pool) {
        ...
        case XR_KIND_STRING: return &g_type_string_nullable;
        case XR_KIND_BOOL:   return &g_type_bool_nullable;
        case XR_KIND_JSON:
            if (type->object.field_count == 0)
    ...
    if (type->frozen) {
        XrType *copy = xr_type_copy(type);
        if (copy) copy->is_nullable = true;
        return copy;
    }
...
bool xr_type_assignable(XrType *target, XrType *source) {
    ...
    if (XR_TYPE_IS_UNKNOWN(target) || XR_TYPE_IS_UNKNOWN(source)) return true;

    // never is assignable to anything (bottom type)
    if (XR_TYPE_IS_NEVER(source)) return true;
    ...
    if (XR_TYPE_IS_UNION(target)) {
        ...
        if (source->kind == XR_KIND_NULL && target->is_nullable) return true;
    ...
    if (xr_type_equals(target, source)) return true;

    // null is compatible with nullable type (T?)
    if (XR_TYPE_IS_NULL(source) && target->is_nullable) return true;

    // Nullable target: compare base types (T assignable to T?, T? assignable to T?)
    if (target->is_nullable) {
        XrType *target_base = xr_type_non_nullable(target);
        XrType *source_base = source->is_nullable ? xr_type_non_nullable(source) : source;
        if (target_base && source_base) return xr_type_assignable(target_base, source_base);
    }
    ...
    if (XR_TYPE_IS_FLOAT(target) && XR_TYPE_IS_INT(source)) return true;

    // Json compatibility - Map, string, int, float, bool → Json (Array is NOT Json)
    if (XR_TYPE_IS_JSON(target) && !XR_TYPE_IS_JSON(source)) {
        if (XR_TYPE_IS_MAP(source) || XR_TYPE_IS_STRING(source) ||
            XR_TYPE_IS_INT(source) || XR_TYPE_IS_FLOAT(source) || XR_TYPE_IS_BOOL(source)) {
            return true;
        }
    }
    ...
    if (XR_TYPE_IS_JSON(target) && XR_TYPE_IS_JSON(source)) {
        ...
        if (target->object.field_count == 0) return true;
        ...
        if (source->object.field_count == 0) return true;

        // Check structural compatibility (source has all target fields)
        for (int i = 0; i < target->object.field_count; i++) {
            XrType *target_field_type = target->object.field_types ?
                target->object.field_types[i] : NULL;
                ...
                if (target->object.field_names && source->object.field_names &&
                    ...
                    if (target_field_type && source->object.field_types) {
                        ...
                        XrType *check_type = is_optional ?
                            xr_type_non_nullable(target_field_type) : target_field_type;
    ...
    if (target->kind == XR_KIND_BYTES && XR_TYPE_IS_ARRAY(source)) return true;
    if (XR_TYPE_IS_ARRAY(target) && source->kind == XR_KIND_BYTES) return true;
    if (target->kind == XR_KIND_BYTES && source->kind == XR_KIND_BYTES) return true;

    // Array type compatibility (invariant, but any is special)
    if (XR_TYPE_IS_ARRAY(target) && XR_TYPE_IS_ARRAY(source)) {
        if (!target->container.element_type || !source->container.element_type) return true;
    ...
    if (XR_TYPE_IS_MAP(target) && XR_TYPE_IS_MAP(source)) {
        if (!target->map.key_type || !source->map.key_type) return true;
    ...
    if (target->kind == XR_KIND_FIXED_ARRAY && XR_TYPE_IS_ARRAY(source)) {
        if (!target->fixed_array.element_type || !source->container.element_type) return true;
    ...
    if (XR_TYPE_IS_ARRAY(target) && source->kind == XR_KIND_FIXED_ARRAY) {
        if (!target->container.element_type || !source->fixed_array.element_type) return true;
    ...
    if (target->kind == XR_KIND_FIXED_ARRAY && source->kind == XR_KIND_FIXED_ARRAY) {
        if (target->fixed_array.length != source->fixed_array.length) return false;
        if (!target->fixed_array.element_type || !source->fixed_array.element_type) return true;
    ...
    if (XR_TYPE_IS_FUNCTION(target) && XR_TYPE_IS_FUNCTION(source)) {
        ...
        for (int i = 0; i < target->function.param_count; i++) {
            ...
            if (!t_param || !s_param) continue;
            if (XR_TYPE_IS_UNKNOWN(t_param) || XR_TYPE_IS_UNKNOWN(s_param)) continue;
            if (t_param->kind == XR_KIND_TYPE_PARAM || s_param->kind == XR_KIND_TYPE_PARAM) continue;
    ...
    if ((target->kind == XR_KIND_CLASS || target->kind == XR_KIND_INSTANCE ||
        ...
        if (xr_type_is_subclass_of(source, target)) return true;
        ...
        if (target_name) {
            for (XrClassInfo *ci = source->instance.class_ref; ci; ci = ci->base) {
                for (int i = 0; i < ci->interface_count; i++) {
                    if (ci->interface_names[i] &&
                        strcmp(ci->interface_names[i], target_name) == 0)
                        return true;
                }
            }
        }
...
bool xr_type_equals(XrType *a, XrType *b) {
    ...
    if (a->kind == XR_KIND_INSTANCE || a->kind == XR_KIND_CLASS) {
        ...
        if (a->instance.class_ref && b->instance.class_ref &&
            ...
            if (a->instance.type_arg_count != b->instance.type_arg_count) return false;
        ...
        if (!a->instance.class_name || !b->instance.class_name) return false;

        // Compare base class names
        if (strcmp(a->instance.class_name, b->instance.class_name) != 0) return false;
    ...
    if (a->kind == XR_KIND_FUNCTION) {
        ...
        if (!xr_type_equals(a->function.return_type, b->function.return_type)) return false;
    ...
    if (a->kind == XR_KIND_JSON) {
        if (a->object.field_count != b->object.field_count) return false;
        for (int i = 0; i < a->object.field_count; i++) {
            if (!a->object.field_names || !b->object.field_names) continue;
            if (!a->object.field_names[i] || !b->object.field_names[i]) continue;
            if (strcmp(a->object.field_names[i], b->object.field_names[i]) != 0) {
                return false;
            }
            // Compare field types (not just names)
            if (a->object.field_types && b->object.field_types &&
                a->object.field_types[i] && b->object.field_types[i]) {
    ...
    if (a->kind == XR_KIND_UNION) {
        if (a->union_type.member_count != b->union_type.member_count) return false;
...
XrType *xr_type_filter(XrType *type, XrTypeKind kind) {
    if (!type) return NULL;
    if (XR_TYPE_IS_UNION(type)) return xr_type_union_filter(type, kind);
    if (type->kind == kind) return type;
    return xr_type_new_never();
}

// Type narrowing: exclude type if it matches given kind
XrType *xr_type_exclude(XrType *type, XrTypeKind kind) {
    if (!type) return NULL;
    if (XR_TYPE_IS_UNION(type)) return xr_type_union_remove(type, kind);
    if (type->kind == kind) return xr_type_new_never();
    return type;
}

// Remove null from type
XrType *xr_type_non_nullable(XrType *type) {
    if (!type) return NULL;

    // If type is nullable, return non-nullable version
    if (type->is_nullable) {
        switch (type->kind) {
        case XR_KIND_INT:    return xr_type_new_int();
        case XR_KIND_FLOAT:  return xr_type_new_float();
        case XR_KIND_STRING: return xr_type_new_string();
        case XR_KIND_BOOL:   return xr_type_new_bool();
        case XR_KIND_JSON:
        ...
        XrType *result = xr_type_copy(type);
        if (result) result->is_nullable = false;
        return result;
    }
...
bool xr_type_is_subclass_of(XrType *type, XrType *target) {
    ...
    if (type->kind != XR_KIND_CLASS && type->kind != XR_KIND_INSTANCE) return false;
    if (target->kind != XR_KIND_CLASS && target->kind != XR_KIND_INSTANCE) return false;
