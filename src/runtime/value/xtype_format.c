...
const char *xr_type_to_string(XrType *type) {
    ...
    if (!type) return TYPE_NAME_UNKNOWN;
    
    // Simple types: return static constants (no allocation)
    if (XR_TYPE_IS_UNKNOWN(type)) return TYPE_NAME_UNKNOWN;
    if (XR_TYPE_IS_NEVER(type)) return TYPE_NAME_NEVER;
    if (XR_TYPE_IS_VOID(type)) return TYPE_NAME_VOID;
    if (XR_TYPE_IS_INT(type) && !type->is_nullable) return TYPE_NAME_INT;
    if (XR_TYPE_IS_FLOAT(type) && !type->is_nullable) return TYPE_NAME_FLOAT;
    if (XR_TYPE_IS_STRING(type) && !type->is_nullable) return TYPE_NAME_STRING;
    if (XR_TYPE_IS_BOOL(type) && !type->is_nullable) return TYPE_NAME_BOOL;
    if (XR_TYPE_IS_NULL(type)) return TYPE_NAME_NULL;
    ...
    if (XR_TYPE_IS_UNION(type)) {
        ...
        for (int i = 0; i < type->union_type.member_count && pos < TYPE_STR_BUF_SIZE - 4; i++) {
            if (i > 0) { pos += snprintf(buf + pos, TYPE_STR_BUF_SIZE - pos, " | "); }
    ...
    if (XR_TYPE_IS_ARRAY(type)) {
        const char *elem = type->container.element_type ? 
            xr_type_to_string(type->container.element_type) : "unknown";
        snprintf(buf, TYPE_STR_BUF_SIZE, "Array<%s>", elem);
        return xr_pool_strdup(pool, buf);
    }
    
    if (XR_TYPE_IS_MAP(type)) {
        const char *key = type->map.key_type ?
            xr_type_to_string(type->map.key_type) : "unknown";
        const char *val = type->map.value_type ?
            xr_type_to_string(type->map.value_type) : "unknown";
    ...
    if (type->kind == XR_KIND_FIXED_ARRAY) {
        const char *elem = type->fixed_array.element_type ?
            xr_type_to_string(type->fixed_array.element_type) : "unknown";
    ...
    if (type->kind == XR_KIND_BYTES) return "Array<uint8>";
    if (xr_type_is_named_class(type, "Regex")) return "Regex";
    if (xr_type_is_named_class(type, "BigInt")) return "BigInt";
    if (xr_type_is_named_class(type, "StringBuilder")) return "StringBuilder";
    if (xr_type_is_named_class(type, "Exception")) return "Exception";
    ...
    if (XR_TYPE_IS_INSTANCE(type) || XR_TYPE_IS_CLASS(type)) {
        if (!type->instance.class_name) return "Object";
        ...
        if (type->instance.type_arg_count > 0 && type->instance.type_args) {
            ...
            for (int i = 0; i < type->instance.type_arg_count && remaining > 2; i++) {
                ...
                const char *arg_str = type->instance.type_args[i] ?
                    xr_type_to_string(type->instance.type_args[i]) : "unknown";
    ...
    if (XR_TYPE_IS_FUNCTION(type)) {
        ...
        for (int i = 0; i < type->function.param_count && remaining > 2; i++) {
            ...
            const char *param_str = type->function.param_types[i] ?
                xr_type_to_string(type->function.param_types[i]) : "unknown";
            n = snprintf(ptr, remaining, "%s", param_str);
            ptr += n; remaining -= n;
        }
        
        const char *ret_str = type->function.return_type ?
            xr_type_to_string(type->function.return_type) : "void";
    ...
    if (XR_TYPE_IS_TUPLE(type)) {
        ...
        for (int i = 0; i < type->tuple.element_count && remaining > 2; i++) {
            ...
            const char *elem_str = type->tuple.element_types[i] ?
                xr_type_to_string(type->tuple.element_types[i]) : "unknown";
...
bool xr_type_is_inherently_immutable(XrType *type) {
    ...
    switch (type->kind) {
    case XR_KIND_INT: case XR_KIND_FLOAT: case XR_KIND_BOOL:
    case XR_KIND_NULL: case XR_KIND_STRING: case XR_KIND_VOID:
        return true;
    default:
        return false;
    }
...
bool xr_type_is_const(XrType *type) {
    if (!type) return false;
    if (xr_type_is_inherently_immutable(type)) return true;
...
XrType *xr_type_object_get_field(XrType *type, const char *field_name) {
    if (!type || !field_name) return NULL;
    if (type->kind != XR_KIND_JSON) return NULL;
