...
#define XR_IS_ARRAY_OR_SLICE(v) (XR_IS_PTR(v) && (XR_HEAP_TYPE(v) == XR_TARRAY || XR_HEAP_TYPE(v) == XR_TARRAY_SLICE))
...
#define XR_SET_INT(v, x)    do { (v).descriptor = 0; (v).i = (int64_t)(x); (v).tag = XR_TAG_I64; } while(0)
#define XR_SET_FLOAT(v, x)  do { (v).descriptor = 0; (v).f = (double)(x);  (v).tag = XR_TAG_F64; } while(0)
...
#define XR_TONUMBER(v, result) \
    (XR_IS_FLOAT(v) ? ((result) = (v).f, 1) : \
     XR_IS_INT(v)   ? ((result) = (double)(v).i, 1) : 0)
...
static inline bool xr_value_same(XrValue a, XrValue b) {
    ...
    switch (a.tag) {
        ...
        case XR_TAG_BOOL:  return a.i == b.i;
        case XR_TAG_F64:   return a.f == b.f;
        case XR_TAG_PTR:   return a.ptr == b.ptr;
...
static inline XrValue xr_int(xr_Integer i) { return XR_FROM_INT(i); }
static inline XrValue xr_float(xr_Number n) { return XR_FROM_FLOAT(n); }
...
#define xr_value(x) _Generic((x), \
    ...
    struct XrModule*: xr_value_from_module \
)(x)
