...
static inline bool xr_kind_is_builtin_iterable(XrTypeKind k) {
    return k == XR_KIND_ARRAY || k == XR_KIND_MAP || k == XR_KIND_SET ||
           k == XR_KIND_STRING || k == XR_KIND_BYTES;
}
...
// Object type: supports structured JSON fields (migrated from sema)
typedef struct XrObjectType {
    ...
    bool allow_extension;       // true = anonymous object, false = strict type alias
...
static inline bool xr_type_is_named_class(const XrType *t, const char *name) {
    if (!t || t->kind != XR_KIND_INSTANCE) return false;
...
static inline XrRep xr_type_base_rep(const XrType *t) {
    ...
    case XR_KIND_VOID:   return XR_REP_VOID;
    case XR_KIND_STRING: case XR_KIND_ARRAY: case XR_KIND_MAP:
    case XR_KIND_SET:    case XR_KIND_JSON:  case XR_KIND_INSTANCE:
    case XR_KIND_CHANNEL: case XR_KIND_BYTES:
    case XR_KIND_INTERFACE: case XR_KIND_CLASS:
    case XR_KIND_FUNCTION: case XR_KIND_ENUM:
        return XR_REP_PTR;
    default:
...
static inline XrRep xr_type_rep(const XrType *t) {
    ...
    if (t->kind == XR_KIND_UNION) {
        ...
        if (t->union_type.member_count == 0) return XR_REP_TAGGED;
...
static inline uint8_t xr_type_to_slot_type(XrType *type) {
    if (!type) return XR_SLOT_ANY;
    if (type->native_width != 0 && !type->is_nullable) {
        uint8_t nw = type->native_width;
        if (nw == XR_NATIVE_F32 || nw == XR_NATIVE_F64) return XR_SLOT_F64;
        if (nw == XR_NATIVE_BOOL) return XR_SLOT_BOOL;
        return XR_SLOT_I64; // I8/U8/I16/U16/I32/U32/I64/U64 all widen to I64
    }
    switch (type->kind) {
    case XR_KIND_INT:            return XR_SLOT_I64;
    case XR_KIND_FLOAT:          return XR_SLOT_F64;
    case XR_KIND_BOOL:           return XR_SLOT_BOOL;
    case XR_KIND_STRING: case XR_KIND_ARRAY: case XR_KIND_MAP:
    case XR_KIND_SET: case XR_KIND_JSON: case XR_KIND_INSTANCE:
    case XR_KIND_CHANNEL: case XR_KIND_BYTES:
    case XR_KIND_INTERFACE: case XR_KIND_CLASS:
    ...
    case XR_KIND_UNION: {
        ...
        if (type->union_type.member_count == 0) return XR_SLOT_ANY;
...
static inline uint8_t xr_type_to_xr_tag(const XrType *t) {
    ...
    switch (t->kind) {
    case XR_KIND_INT:    return XR_TAG_I64;
    case XR_KIND_FLOAT:  return XR_TAG_F64;
    case XR_KIND_BOOL:   return 1;    // XR_TAG_BOOL: payload 0=false, 1=true
    case XR_KIND_NULL:   return XR_TAG_NULL;
    case XR_KIND_STRING: case XR_KIND_ARRAY: case XR_KIND_MAP:
    case XR_KIND_SET:    case XR_KIND_JSON:  case XR_KIND_INSTANCE:
    case XR_KIND_CHANNEL: case XR_KIND_BYTES:
    case XR_KIND_INTERFACE: case XR_KIND_CLASS:
    ...
    case XR_KIND_UNION: {
        ...
        if (t->union_type.member_count == 0) return 0xFF;
        uint8_t common = xr_type_to_xr_tag(t->union_type.members[0]);
        if (common == 0xFF) return 0xFF;
        bool all_same = true;
        bool all_numeric = (common == XR_TAG_I64 || common == XR_TAG_F64);
        for (int i = 1; i < t->union_type.member_count; i++) {
            uint8_t mt = xr_type_to_xr_tag(t->union_type.members[i]);
            if (mt != common) all_same = false;
            if (mt != XR_TAG_I64 && mt != XR_TAG_F64) all_numeric = false;
        }
        if (all_same) return common;
        if (all_numeric) return 0xFC; // XR_RTAG_NUMERIC: int|float union
        return 0xFF;
    }
    default:
        return 0xFF;
...
static inline uint8_t xr_type_element_gc_tag(XrType *type) {
    if (!type) return XR_SLOT_ANY;
    switch (type->kind) {
    case XR_KIND_ARRAY: case XR_KIND_SET: case XR_KIND_CHANNEL:
...
static inline bool xr_is_json_coercion(XrType *target, XrType *source) {
    ...
    if (!src_dynamic) return false;
    // Single type: primitive, Json, or Array
    if (xr_kind_is_primitive(target->kind)) return true;
    if (target->kind == XR_KIND_JSON) return true;
    if (target->kind == XR_KIND_ARRAY) return true;
    ...
    if (target->kind == XR_KIND_UNION) {
        for (int i = 0; i < target->union_type.member_count; i++) {
            ...
            if (!m || (!xr_kind_is_primitive(m->kind) &&
                       m->kind != XR_KIND_JSON && m->kind != XR_KIND_ARRAY))
                return false;
        }
