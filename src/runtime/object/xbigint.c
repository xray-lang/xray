...
static XrBigInt* bigint_alloc(struct XrCoroutine *coro, uint32_t cap) {
    ...
    if (b) {
        XrayCoreClasses *core = xr_isolate_get_core_classes(
            xr_coro_get_isolate(coro));
        (void)core;
...
XrBigInt* xr_bigint_from_string(struct XrCoroutine *coro, const char *str) {
    ...
    if (slen == 0) return xr_bigint_new(coro, 0);
...
XrBigInt* xr_bigint_new_on_gc(struct XrGC *gc, int64_t value) {
    ...
    if (value == 0) {
        ...
        b->limbs[0] = (uint32_t)(value & 0xFFFFFFFFULL);
        uint32_t high = (uint32_t)(value >> 32);
        if (high == 0) { b->len = 1; } else { b->limbs[1] = high; b->len = 2; }
    }
    return b;
}
...
XrBigInt* xr_bigint_from_string_on_gc(struct XrGC *gc, const char *str) {
    ...
    if (str[0] == '0' && str[1] != '\0') {
        ...
        else if (str[1] == 'o' || str[1] == 'O') { base = 8; str += 2; }
    }
    while (*str == '0' && *(str + 1) != '\0') str++;
    size_t slen = strlen(str);
    if (slen == 0) return xr_bigint_new_on_gc(gc, 0);
    uint32_t cap = (base == 16) ? (uint32_t)(slen/8)+2 : (base == 2) ? (uint32_t)(slen/32)+2 : (uint32_t)(slen/9)+2;
    XrBigInt *result = bigint_alloc_on_gc(gc, cap);
    if (!result) return NULL;
    result->limbs[0] = 0; result->len = 1; result->sign = 1;
    for (size_t i = 0; i < slen; i++) {
        char c = str[i];
        int digit;
        if (base == 16) { digit = hex_digit_value(c); }
        else if (base == 2) { digit = (c == '0' || c == '1') ? c - '0' : -1; }
        else if (base == 8) { digit = (c >= '0' && c <= '7') ? c - '0' : -1; }
        else { digit = isdigit(c) ? c - '0' : -1; }
        if (digit < 0) return NULL;
        uint64_t carry = digit;
        for (uint32_t j = 0; j < result->len; j++) {
            uint64_t prod = (uint64_t)result->limbs[j] * base + carry;
...
int xr_bigint_cmp_int(XrBigInt *a, int64_t n) {
    ...
    if (a->len > n_len) {
        ...
        cmp = 0;
        if (a->len == 2) {
            if (a->limbs[1] > n_hi) { cmp = 1; }
            else if (a->limbs[1] < n_hi) { cmp = -1; }
            else if (a->limbs[0] > n_lo) { cmp = 1; }
            else if (a->limbs[0] < n_lo) { cmp = -1; }
        } else {
            // a->len == 1
            if (a->limbs[0] > n_lo) { cmp = 1; }
            else if (a->limbs[0] < n_lo) { cmp = -1; }
        }
    }
...
XrBigInt* xr_bigint_sub(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b) {
    ...
    b->sign = -b->sign;
    if (b->len == 1 && b->limbs[0] == 0) b->sign = 1;  // -0 = 0
...
static XrBigInt* bigint_lshift_limbs(struct XrCoroutine *coro, XrBigInt *a, uint32_t B) {
    if (xr_bigint_is_zero(a)) return xr_bigint_new(coro, 0);
    if (B == 0) return xr_bigint_copy(coro, a);
...
static XrBigInt* bigint_mul_karatsuba(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b) {
    ...
    if (!z2_shifted || !z1_shifted) return NULL;
...
XrBigInt* xr_bigint_mul(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b) {
    ...
    if (result) {
        ...
        if (result->len == 1 && result->limbs[0] == 0) result->sign = 1;
    }
    return result;
}
...
static uint32_t limbs_lshift(uint32_t *rp, const uint32_t *ap, uint32_t n, unsigned shift) {
    if (shift == 0) {
        if (rp != ap) memcpy(rp, ap, n * sizeof(uint32_t));
        return 0;
    }
    uint32_t carry = 0;
...
bool xr_bigint_divmod(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b,
                      XrBigInt **q_out, XrBigInt **r_out) {
    // Division by zero
    ...
    if (xr_bigint_is_zero(a)) {
        if (q_out) *q_out = xr_bigint_new(coro, 0);
        if (r_out) *r_out = xr_bigint_new(coro, 0);
        return true;
    }
    ...
    if (cmp < 0) {
        if (q_out) *q_out = xr_bigint_new(coro, 0);
        if (r_out) *r_out = xr_bigint_copy(coro, a);
        return true;
    }

    // |a| == |b| => q=±1, r=0
    if (cmp == 0) {
        if (q_out) *q_out = xr_bigint_new(coro, a->sign * b->sign);
        if (r_out) *r_out = xr_bigint_new(coro, 0);
        return true;
    }
    ...
    if (quotient->len == 1 && quotient->limbs[0] == 0) quotient->sign = 1;

    if (q_out) *q_out = quotient;
    ...
    if (r_out) {
        ...
        if (remainder) {
            ...
            if (remainder->len == 1 && remainder->limbs[0] == 0) remainder->sign = 1;
        }
        *r_out = remainder;
    }
...
char* xr_bigint_to_string(XrBigInt *a) {
    ...
    if (!tmp) { xr_free(result); return NULL; }
...
uint32_t xr_bigint_bit_length(XrBigInt *a) {
    ...
    if (xr_bigint_is_zero(a)) return 0;
...
static XrBigInt* bigint_from_twos_comp(struct XrCoroutine *coro,
                                        uint32_t *tc, uint32_t len, bool negative) {
    if (!negative) {
        XrBigInt *r = bigint_alloc(coro, len);
        if (!r) return NULL;
        for (uint32_t i = 0; i < len; i++) r->limbs[i] = tc[i];
        r->len = len;
        r->sign = 1;
        bigint_normalize(r);
        return r;
...
static XrBigInt* bigint_bitwise(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b,
                                 BitwiseOp op, bool result_negative) {
    ...
    if (!ta || !tb) { xr_free(ta); xr_free(tb); return NULL; }
