...
void xr_gc_traverse_shape(XrGC *gc, void *obj, void *X) {
    (void)X;
    XrShape *shape = (XrShape*)obj;
    if (!shape) return;
    
    if (shape->parent) {
        xr_gc_mark_object(gc, (XrGCHeader*)shape->parent);
    }
    
    if (shape->transitions && shape->transitions->entries) {
        for (uint16_t i = 0; i < shape->transitions->count; i++) {
            XrShape *child = shape->transitions->entries[i].target;
            if (child) {
                xr_gc_mark_object(gc, (XrGCHeader*)child);
            }
        }
    }
}
