...
static inline XrArrayElemType xr_tid_to_elem_type(uint8_t tid) {
    switch (tid) {
        ...
        default:
            return XR_ELEM_ANY;  // string, object, etc. → generic storage
    }
}
...
static inline XrValue xr_array_get_element(XrArray *arr, int32_t index) {
    switch (arr->elem_type) {
        case XR_ELEM_ANY:
            return ((XrValue *) arr->data)[index];
        case XR_ELEM_I8:
            return xr_int(((int8_t *) arr->data)[index]);
        case XR_ELEM_U8:
            return xr_int(((uint8_t *) arr->data)[index]);
        case XR_ELEM_I16:
            return xr_int(((int16_t *) arr->data)[index]);
        case XR_ELEM_U16:
            return xr_int(((uint16_t *) arr->data)[index]);
        case XR_ELEM_I32:
            return xr_int(((int32_t *) arr->data)[index]);
        case XR_ELEM_U32:
            return xr_int(((uint32_t *) arr->data)[index]);
        case XR_ELEM_I64:
            return xr_int(((int64_t *) arr->data)[index]);
        case XR_ELEM_U64:
            return xr_int((int64_t) ((uint64_t *) arr->data)[index]);
        case XR_ELEM_F32:
            return XR_FROM_FLOAT((double) ((float *) arr->data)[index]);
        case XR_ELEM_F64:
            return XR_FROM_FLOAT(((double *) arr->data)[index]);
        case XR_ELEM_BOOL:
            return ((uint8_t *) arr->data)[index] ? XR_TRUE_VAL : XR_FALSE_VAL;
        default:
            return xr_null();
    }
}
