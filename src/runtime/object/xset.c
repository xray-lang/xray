...
XrSet* xr_set_new(struct XrCoroutine *coro) {
    ...
    set->tombstones = 0;
    set->entries = NULL;
    set->flags = XR_SET_FLAG_ENTRIES_ON_GC;
    set->elem_tid = 0;
...
void xr_set_resize(XrSet *set, uint32_t new_capacity) {
    ...
    /* Force malloc during resize to avoid Immix blob overlap.
     * GC blob allocation may return memory overlapping with the old
     * entries array (Immix line recycling), and the memset below
     * would zero out old entries before we can re-insert them. */
    if (set->flags & XR_SET_FLAG_ENTRIES_ON_GC) {
        set->flags &= ~XR_SET_FLAG_ENTRIES_ON_GC;
    }
    new_entries = (XrSetEntry*)xr_malloc(alloc_bytes);
    ...
    if (old_entries != NULL) {
        ...
        // Free old entries (skip for GC blob - reclaimed by Immix sweep)
        if (!(set->flags & XR_SET_FLAG_ENTRIES_ON_GC)) {
            xr_free(old_entries);
        }
    }
