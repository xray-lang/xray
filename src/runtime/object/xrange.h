...
XR_FUNC XrRange* xr_range_new_with_step(struct XrCoroutine *coro, int64_t start, int64_t end, int64_t step);
...
static inline bool xr_range_contains(XrRange *r, int64_t value) {
    if (!r || r->step == 0) return false;
    if (r->step > 0) {
        if (value < r->start || value > r->end) return false;
