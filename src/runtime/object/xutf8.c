...
int xr_utf8_decode(const char *str, size_t len, uint32_t *out_cp) {
    ...
    // 2 bytes (110xxxxx 10xxxxxx)
        ...
        if (!xr_utf8_is_continuation(b1)) goto invalid;
    ...
    // 3 bytes (1110xxxx 10xxxxxx 10xxxxxx)
        ...
        if (!xr_utf8_is_continuation(b1) || !xr_utf8_is_continuation(b2)) goto invalid;
    ...
    // 4 bytes (11110xxx 10xxxxxx 10xxxxxx 10xxxxxx)
        ...
        if (!xr_utf8_is_continuation(b1) || 
            !xr_utf8_is_continuation(b2) || 
            !xr_utf8_is_continuation(b3)) goto invalid;
        
        cp = ((b0 & 0x07) << 18) | ((b1 & 0x3F) << 12) | 
             ((b2 & 0x3F) << 6) | (b3 & 0x3F);
        
        // Check overlong encoding and range
        if (cp < 0x10000 || cp > XR_UNICODE_MAX) goto invalid;
        size = 4;
    }
    else {
        goto invalid;
...
invalid:
    if (out_cp) *out_cp = XR_UNICODE_INVALID;
    return 1;  // Skip one invalid byte
}

bool xr_utf8_char_at(const char *str, size_t len, size_t index, 
...
bool xr_utf8_char_range(const char *str, size_t len,
                        size_t start_idx, size_t end_idx,
                        size_t *out_start, size_t *out_end) {
    if (!str || start_idx > end_idx) return false;
...
size_t xr_utf8_byte_to_utf16_offset(const char *str, size_t len, size_t byte_offset) {
    ...
    while (byte_pos < byte_offset) {
        ...
        if (byte_pos + utf8_size > byte_offset) break;
