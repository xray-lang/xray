...
XrArray* xr_array_with_capacity_typed(struct XrCoroutine *coro, int capacity, XrArrayElemType elem_type) {
...
void xr_array_unshift(XrArray *arr, XrValue value) {
    ...
    memmove((uint8_t*)arr->data + arr->elem_size,
            arr->data,
...
XrValue xr_array_shift(XrArray *arr) {
    ...
    if (arr->length > 1) {
        memmove(arr->data,
                (uint8_t*)arr->data + arr->elem_size,
...
XrArray* xr_array_map(XrayIsolate *iso, XrArray *arr, struct XrClosure *callback) {
    if (!arr || !callback) return xr_array_new(xr_current_coro(iso));
...
XrArray* xr_array_filter(XrayIsolate *iso, XrArray *arr, struct XrClosure *callback) {
    ...
    XrArray *result = xr_array_with_capacity_typed(xr_current_coro(iso), arr->length, (XrArrayElemType)arr->elem_type);
...
XrValue xr_array_reduce(XrayIsolate *iso, XrArray *arr, struct XrClosure *callback, XrValue initial) {
...
XrValue xr_array_find(XrayIsolate *iso, XrArray *arr, struct XrClosure *callback) {
    if (!arr || !callback) return xr_null();
    for (int i = 0; i < arr->length; i++) {
        XrValue elem = xr_array_get_element(arr, i);
        XrValue args[1];
        args[0] = elem;
...
#define TYPED_FILL(type, arr, val, start, end) do { \
    type *d = (type*)(arr)->data; \
    type v = (val); \
    for (int _i = (start); _i < (end); _i++) d[_i] = v; \
} while(0)
...
void xr_array_fill(XrArray *arr, XrValue value, int start, int end) {
    ...
    switch (arr->elem_type) {
        case XR_ELEM_I8:  TYPED_FILL(int8_t, arr, (int8_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_U8: {
            // Special case: memset for byte arrays
            uint8_t v = (uint8_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value));
            memset((uint8_t*)arr->data + start, v, (size_t)count);
            break;
        }
        case XR_ELEM_I16: TYPED_FILL(int16_t, arr, (int16_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_U16: TYPED_FILL(uint16_t, arr, (uint16_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_I32: TYPED_FILL(int32_t, arr, (int32_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_U32: TYPED_FILL(uint32_t, arr, (uint32_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_I64: TYPED_FILL(int64_t, arr, (int64_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_U64: TYPED_FILL(uint64_t, arr, (uint64_t)(XR_IS_INT(value) ? XR_TO_INT(value) : (int64_t)XR_TO_FLOAT(value)), start, end); break;
        case XR_ELEM_F32: TYPED_FILL(float, arr, (float)(XR_IS_FLOAT(value) ? XR_TO_FLOAT(value) : (double)XR_TO_INT(value)), start, end); break;
        case XR_ELEM_F64: TYPED_FILL(double, arr, (double)(XR_IS_FLOAT(value) ? XR_TO_FLOAT(value) : (double)XR_TO_INT(value)), start, end); break;
        case XR_ELEM_BOOL: {
...
static int xr_sort_with_comparator(const void *a, const void *b, void *ctx_ptr) {
    ...
    if (ctx->comparator) {
        ...
        if (XR_IS_INT(result)) return (int)XR_TO_INT(result);
...
static void xr_array_hybrid_sort(XrValue *data, int n, XrSortCtx *ctx) {
    ...
    for (int width = XR_SORT_INSERTION_THRESHOLD; width < n; width *= 2) {
        for (int lo = 0; lo < n; lo += width * 2) {
            int mid = lo + width - 1;
            int hi = lo + width * 2 - 1;
            if (mid >= n) break;      // only one run left, already sorted
            if (hi >= n) hi = n - 1;
...
#define TYPED_INSERTION(type, d, lo, hi) do { \
    for (int _i = (lo) + 1; _i <= (hi); _i++) { \
        type _key = (d)[_i]; \
        int _j = _i - 1; \
        while (_j >= (lo) && (d)[_j] > _key) { (d)[_j+1] = (d)[_j]; _j--; } \
        (d)[_j+1] = _key; \
    } \
} while(0)

// Typed merge for numeric arrays
#define TYPED_MERGE(type, d, tmp, lo, mid, hi) do { \
    memcpy((tmp) + (lo), (d) + (lo), (size_t)((hi) - (lo) + 1) * sizeof(type)); \
    int _i = (lo), _j = (mid) + 1, _k = (lo); \
    while (_i <= (mid) && _j <= (hi)) { \
        if ((tmp)[_i] <= (tmp)[_j]) (d)[_k++] = (tmp)[_i++]; \
        else (d)[_k++] = (tmp)[_j++]; \
    } \
    while (_i <= (mid)) (d)[_k++] = (tmp)[_i++]; \
    while (_j <= (hi)) (d)[_k++] = (tmp)[_j++]; \
} while(0)

// Typed hybrid sort (insertion + merge)
#define TYPED_SORT(type, arr, n) do { \
    ...
    if (_n <= XR_SORT_INSERTION_THRESHOLD) { \
        ...
        if (!_tmp) { TYPED_INSERTION(type, _d, 0, _n - 1); break; } \
...
static void xr_array_typed_sort_with_comparator(XrayIsolate *iso, XrArray *arr, struct XrClosure *cmp) {
    int n = arr->length;
...
void xr_array_sort(XrayIsolate *iso, XrArray *arr, struct XrClosure *comparator) {
    ...
    int n = arr->length;
    switch (arr->elem_type) {
        case XR_ELEM_I8:  TYPED_SORT(int8_t, arr, n); break;
        case XR_ELEM_U8:  TYPED_SORT(uint8_t, arr, n); break;
        case XR_ELEM_I16: TYPED_SORT(int16_t, arr, n); break;
        case XR_ELEM_U16: TYPED_SORT(uint16_t, arr, n); break;
        case XR_ELEM_I32: TYPED_SORT(int32_t, arr, n); break;
        case XR_ELEM_U32: TYPED_SORT(uint32_t, arr, n); break;
        case XR_ELEM_I64: TYPED_SORT(int64_t, arr, n); break;
        case XR_ELEM_U64: TYPED_SORT(uint64_t, arr, n); break;
        case XR_ELEM_F32: TYPED_SORT(float, arr, n); break;
        case XR_ELEM_F64: TYPED_SORT(double, arr, n); break;
        default: break;
    }
}
...
XrArray* xr_array_copy(struct XrCoroutine *coro, XrArray *arr) {
    if (!arr) return xr_array_new(coro);
    if (arr->elem_type == XR_ELEM_ANY) {
        return xr_array_from_values(coro, (XrValue*)arr->data, arr->length);
    }
    // Typed array: allocate same type and memcpy
    XrArray *new_arr = xr_array_with_capacity_typed(coro, arr->length, (XrArrayElemType)arr->elem_type);
...
void xr_array_grow(XrArray *arr) {
    ...
    int32_t new_capacity = old_capacity == 0 
...
XrArray* xr_array_slice_to_array(struct XrCoroutine *coro, XrArray *slice) {
    if (!coro || !slice) return xr_array_new(coro);
    ...
    XrArray *arr = xr_array_with_capacity_typed(coro, slice->length, (XrArrayElemType)slice->elem_type);
    if (!arr) return NULL;
...
void xr_array_append_data(XrArray *arr, const uint8_t *src_data, int32_t len) {
    if (!arr || !src_data || len <= 0) return;
    if (arr->elem_type != XR_ELEM_U8) return;
