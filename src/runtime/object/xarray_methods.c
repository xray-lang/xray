...
static inline XrArray *array_self(XrValue self) {
    ...
    XR_DCHECK(XR_IS_ARRAY_OR_SLICE(self),
              "array method: receiver is not an array or slice");
...
static XrValue m_slice(XrayIsolate *iso, XrValue self, XrValue *args, int argc) {
    ...
    if (arr->elem_type != XR_ELEM_ANY) {
        ...
        if (result) {
            memcpy(result->data, (XrValue *)arr->data + start,
                   (size_t)count * sizeof(XrValue));
            result->length = count;
...
static XrValue m_concat(XrayIsolate *iso, XrValue self, XrValue *args, int argc) {
    ...
    if (!result) return xr_value_from_array(xr_array_new(xr_current_coro(iso)));
    if (arr->length > 0) {
...
const XrMethodSlot xr_array_method_table[SYMBOL_BUILTIN_COUNT] = {
    ...
    /* Query (pure / no-GC) */
    [SYMBOL_IS_EMPTY]   = { m_is_empty,   0, 0,
                             XR_METHOD_FLAG_PURE | XR_METHOD_FLAG_NO_GC },
    [SYMBOL_HAS]        = { m_has,        1, 1,
                             XR_METHOD_FLAG_PURE | XR_METHOD_FLAG_NO_GC },
    [SYMBOL_INCLUDES]   = { m_includes,   1, 1,
                             XR_METHOD_FLAG_PURE | XR_METHOD_FLAG_NO_GC },
    [SYMBOL_INDEXOF]    = { m_index_of,   1, 1,
                             XR_METHOD_FLAG_PURE | XR_METHOD_FLAG_NO_GC },
