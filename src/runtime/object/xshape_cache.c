...
static int shape_fields_match(XrShape *shape, XrString **interned_names, uint32_t field_count, XrayIsolate *isolate) {
    XR_DCHECK(shape != NULL, "shape_fields_match: NULL shape");
    if (shape->field_count != field_count) return 0;
...
XrShape* xr_shape_cache_get_or_create(
    XrayIsolate *isolate,
    XrShapeCache *cache,
    ...
    while (entry) {
        if (entry->hash == hash && shape_fields_match(entry->shape, interned_names, field_count, isolate)) {
            cache->hit_count++;
            return entry->shape;
        }
        entry = entry->next;
