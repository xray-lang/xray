 ...
 *   by xr_register_panic_info_class (xclass_system.c). It has no dedicated
 ...
 *   chain — xr_value_is_panic_info walks XrClass->primary_supers.
 *
 * MEMORY:
 *   Allocation goes through xr_instance_new(X, core->panicInfoClass).
 *   Each field lives at the index baked into PANIC_INFO_FIELD_* (declared
...
#ifndef XPANIC_INFO_H
#define XPANIC_INFO_H
 ...
 */
XR_FUNC bool xr_value_is_panic_info(XrayIsolate *X, XrValue v);
...
// closure dispatch.
XR_FUNC XrValue xr_panic_info_new(XrayIsolate *X, XrErrorCode code, const char *message);

// Same but accepts a printf-style format.
XR_FUNC XrValue xr_panic_info_newf(XrayIsolate *X, XrErrorCode code, const char *fmt, ...);

// Build an Exception from an XrError. message is moved; the original
// XrError is not freed (caller's lifetime).
XR_FUNC XrValue xr_panic_info_from_error(XrayIsolate *X, XrError *error);
...
XR_FUNC XrValue xr_panic_info_from_value(XrayIsolate *X, XrValue value);
...
XR_FUNC XrErrorCode xr_panic_info_get_code(XrayIsolate *X, XrValue exception);
XR_FUNC const char *xr_panic_info_get_message(XrayIsolate *X, XrValue exception);
XR_FUNC XrValue xr_panic_info_get_stacktrace(XrayIsolate *X, XrValue exception);
XR_FUNC XrValue xr_panic_info_get_data(XrayIsolate *X, XrValue exception);

/* ========== Stack Trace ========== */

// Append "at <funcName> (line <line>)" to the exception's stack array.
// Lazily creates the stack array if it's null.
XR_FUNC void xr_panic_info_add_frame(XrayIsolate *X, XrValue exception, const char *funcName,
                                     int line);

/* ========== Output ========== */

XR_FUNC void xr_panic_info_print(XrayIsolate *X, XrValue exception);

/* ========== Class Registration ========== */

// Build the Exception XrClass and store it in core->panicInfoClass.
// Must be called from xr_core_init after Object is registered.
XR_FUNC void xr_register_panic_info_class(XrayIsolate *X);
