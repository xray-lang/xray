 ...
 * MEMORY LAYOUT:
 *   ┌─────────────────────┐
 *   │ XrObjHeader (16B)    │ object header (type = XR_TINSTANCE)
 *   ├─────────────────────┤
...
typedef struct XrBigInt {
    XrObjHeader hdr;        // object header (must be first)
...
XR_FUNC XrBigInt *xr_bigint_mul(struct XrCoroutine *coro, XrBigInt *a,
                                XrBigInt *b);  // a * b (Karatsuba for large)
XR_FUNC XrBigInt *xr_bigint_div(struct XrCoroutine *coro, XrBigInt *a,
                                XrBigInt *b);  // a / b (truncate toward zero)
...
XR_FUNC bool xr_bigint_divmod(struct XrCoroutine *coro, XrBigInt *a, XrBigInt *b, XrBigInt **q,
                              XrBigInt **r);
