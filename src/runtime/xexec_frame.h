...
#define XR_CALL_KEEP_FUNC                                                                          \
    (1 << 5)  // write return value to result_offset (operator-overload / property-invoke frames)
// XR_CALL_REPLAY_PRESET: dispatch pre-published replay state (pc-1 + YIELDED)
// before calling a yieldable native; a continuation setup restores
// continue-from-next and clears it. Cleared wherever YIELDED is cleared.
...
#define VMCTX_FIXED_HEAP(ctx) (xr_isolate_get_fixed_heap(VMCTX_ISOLATE(ctx)))
