...
typedef struct XrVMState {
    ...
    /* Runtime nominal class bound to each named layout id. Anonymous layouts
     * keep a NULL entry and therefore cannot participate in method dispatch. */
    struct XrClass **struct_layout_classes;
...
XR_FUNC uint16_t xr_struct_layout_register(XrVMState *vm, struct XrAggregateLayout *layout);
XR_FUNC struct XrAggregateLayout *xr_struct_layout_lookup_by_id(XrVMState *vm, uint16_t layout_id);
XR_FUNC bool xr_struct_layout_bind_class(XrVMState *vm, struct XrAggregateLayout *layout,
                                         struct XrClass *cls);
XR_FUNC struct XrClass *xr_struct_layout_class_by_id(XrVMState *vm, uint16_t layout_id);
