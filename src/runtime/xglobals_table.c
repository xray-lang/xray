...
XrGlobalsTable *xr_globals_create(size_t initial_capacity) {
    ...
    if (!globals)
        return NULL;
    if (initial_capacity < GLOBALS_MIN_CAPACITY)
        initial_capacity = GLOBALS_MIN_CAPACITY;
...
bool xr_globals_resize(XrGlobalsTable *globals, size_t new_capacity) {
    if (!globals)
        return false;
    if (new_capacity < globals->count)
        new_capacity = globals->count;
    if (new_capacity < GLOBALS_MIN_CAPACITY)
        new_capacity = GLOBALS_MIN_CAPACITY;
    XrValue *new_values = (XrValue *) xr_realloc(globals->values, sizeof(XrValue) * new_capacity);
    if (!new_values)
        return false;
    for (size_t i = globals->capacity; i < new_capacity; i++)
        new_values[i] = xr_null();
...
int xr_globals_add(XrGlobalsTable *globals, XrValue value) {
    ...
    if (globals->count >= globals->capacity) {
        if (!xr_globals_resize(globals, globals->capacity * GLOBALS_GROWTH_FACTOR))
            return -1;
    }
...
XrValue xr_globals_get(XrGlobalsTable *globals, int index) {
    if (!globals || index < 0 || (size_t) index >= globals->count)
        return xr_null();
...
bool xr_globals_set(XrGlobalsTable *globals, int index, XrValue value) {
    ...
    if ((size_t) index >= globals->capacity) {
        if (!xr_globals_resize(globals, ((size_t) index + 1) * GLOBALS_GROWTH_FACTOR))
            return false;
    }
    globals->values[index] = value;
    if ((size_t) index >= globals->count)
        globals->count = (size_t) index + 1;
    return true;
}
