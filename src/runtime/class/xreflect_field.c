 ...
 *
 * xreflect_field.c - Field accessor (zero-copy)
 *
 * KEY CONCEPT:
 *   All data from XrClass->fields[index]. O(1) direct access.
 */
...
static inline XrFieldMetadata* get_field_metadata_internal(XrValue val) {
...
XrValue xr_field_getName(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    XrFieldMetadata *meta = get_field_metadata_internal(args[0]);
...
XrValue xr_field_getType(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    XrFieldMetadata *field = get_field_metadata_internal(args[0]);
