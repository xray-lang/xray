...
// Get or create reflection cache for a class
static XrReflectCache* get_or_create_cache(XrayIsolate *X, XrClass *klass) {
    if (!klass) return NULL;
    
    // Check if cache already exists
    if (klass->reflect_cache) {
        return klass->reflect_cache;
    }
    
    // Create new cache
    XrReflectCache *cache = xr_reflect_cache_create(X, klass);
    if (cache) {
        klass->reflect_cache = cache;
    }
    return cache;
}
...
XrValue xr_type_getDeclaredFields(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    XrReflectCache *cache = get_or_create_cache(X, meta->klass);
...
XrValue xr_type_getField(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    XrClass *current_class = meta->klass;
    while (current_class != NULL) {
        // Try to use cache for this class
        XrReflectCache *cache = get_or_create_cache(X, current_class);
...
XrValue xr_type_getDeclaredMethods(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    // Use reflection cache for performance
    XrReflectCache *cache = get_or_create_cache(X, klass);
