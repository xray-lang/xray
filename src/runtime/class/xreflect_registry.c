...
// Cache-miss fallback for xr_registry_find_type. Originally this also
// chased builtin classes via a strcasecmp chain and scanned the
// globals table linearly for user-defined classes. Both paths are
// dead after P7's eager registration: every builtin registers itself
// inside xr_core_init -> xr_class_new -> builder finalize, and every
// user-defined class registers inside its own builder finalize, so
// the hashmap in xr_registry_find_type always has an entry by the
