...
XrValue xr_reflect_elementType(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    if (nargs < 1) return tid_to_string_value(isolate, 0);
    XrValue obj = args[0];
    if (!XR_IS_PTR(obj)) return tid_to_string_value(isolate, 0);
...
XrValue xr_reflect_typeOf(XrayIsolate *isolate, XrValue *args, int nargs) {
    ...
    if (!XR_IS_PTR(obj)) {
        if (XR_IS_INT(obj)) return tid_to_string_value(isolate, XR_TID_INT);
        if (XR_IS_FLOAT(obj)) return tid_to_string_value(isolate, XR_TID_FLOAT);
        if (XR_IS_BOOL(obj)) return tid_to_string_value(isolate, XR_TID_BOOL);
        if (XR_IS_NULL(obj)) return tid_to_string_value(isolate, XR_TID_NULL);
        return tid_to_string_value(isolate, 0);
    }
    ...
    switch (heap_type) {
        ...
        case XR_TMAP: {
            ...
            if (map->key_tid != 0 || map->value_tid != 0) {
                ...
                const char *vn = map->value_tid ? xr_typeid_name((XrTypeId)map->value_tid) : "unknown";
        ...
        case XR_TSET: {
            ...
            if (set->elem_tid != 0) {
                n = snprintf(buf, sizeof(buf), "Set<%s>",
                    xr_typeid_name((XrTypeId)set->elem_tid));
            } else {
        ...
        case XR_TINSTANCE: {
            ...
            if (argc >= 2) {
                ...
                int tid0 = XR_INST_TYPE_ARG0(&inst->gc);
                n = snprintf(buf, sizeof(buf), "%s<%s>", cls_name,
                    xr_typeid_name((XrTypeId)tid0));
            } else {
    ...
    if (n <= 0) return tid_to_string_value(isolate, 0);
    size_t len = (size_t)n;
