...
#define XR_CLASS_INTERFACE (1 << 2)
#define XR_CLASS_INITIALIZED (1 << 3)       // Static constructor executed
#define XR_CLASS_VALUE_TYPE (1 << 4)        // Struct: value type with copy semantics
#define XR_CLASS_FLAT_COPYABLE (1 << 5)     // All non-pointer fields are primitive (memcpy safe)
#define XR_CLASS_HAS_SUBCLASSES (1 << 6)    // At least one subclass exists (CHA devirtualization)
#define XR_CLASS_GENERIC_SKELETON (1 << 7)  // Uninstantiable generic template (e.g. Box)
#define XR_CLASS_MONOMORPHIZED (1 << 8)     // Generated by monomorphization (e.g. Box$i64)
#define XR_CLASS_DYNAMIC_LAYOUT (1 << 9)    // Dynamic field layout (Json object / object literal)
#define XR_CLASS_HAS_NATIVE_BODY (1 << 10)  // Has XrNativeBodyDesc (Array, Map, etc.)
#define XR_CLASS_DYNAMIC_SEALED (1 << 11)   // Dynamic-layout class rejects new field transitions
