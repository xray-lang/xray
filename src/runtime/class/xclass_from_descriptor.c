...
XrClass *xr_class_from_descriptor(XrVMRuntime *isolate, const XrClassDescriptor *desc,
    ...
    if (cls->fields) {
        for (uint32_t i = 0; i < desc->instance_field_count; i++) {
            int field_index = xr_class_lookup_field_by_name(
                isolate, cls, desc->instance_fields[i].name);
            ...
            if (!xr_json_decode_schema_clone_for_class(
                    isolate, &desc->instance_fields[i].json_decode_schema,
            ...
            if (xr_json_value_kind_base(field->json_value_kind) ==
                XR_JSON_VALUE_STRUCT_OBJECT)
