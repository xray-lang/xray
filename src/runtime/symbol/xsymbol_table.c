...
static const char *xr_builtin_symbol_names[] = {
    // Common methods
    "length", "isEmpty", "has", "get", "set",
    "delete", "clear", "keys", "values", "entries",
    // Functional methods
    "hasValue", "forEach", "map", "filter", "reduce",
    // String methods
    "slice", "charAt", "substring", "indexOf", "contains",
    "startsWith", "endsWith", "toLowerCase", "toUpperCase",
    "trim", "split", "replace", "replaceAll", "repeat", "concat",
    "byteAt", "reverseBytes", "translate", "translateBytes",
    "trimStart", "trimEnd", "padStart", "padEnd", "lastIndexOf",
    "toInt", "toFloat", "isLetter", "isNumber", "isAlphanumeric",
    "isWhitespace", "ord", "match",
    // Array methods
    "push", "pop", "shift", "unshift", "join", "reverse",
    // Iterator methods
    "iterator", "entriesIterator", "hasNext", "next",
    // Number methods
    "floor", "ceil", "round", "abs", "sqrt", "pow", "toFixed", "toBigInt",
    "max", "min", "toHex",
    // UTF-8 methods
    "charLength", "byteLength", "codePointAt", "fromCodePoint", "chars",
    // Set methods
    "add", "union", "intersection", "difference", "symmetricDifference",
    "toArray", "isSubset", "isSuperset",
    // Enum/type methods
    "name", "value", "ordinal", "toString", "memberCount", "getMember",
    // DateTime methods
    "format", "year", "month", "day", "hour", "minute", "second",
    "weekday", "timestamp", "toUTC", "toLocal",
    "millisecond", "isBefore", "isAfter", "equals",
    "isLeapYear", "daysInMonth", "toISOString", "yearday",
    ...
    "done", "cancelled", "cancel", "result", "error", "monitor", "link", "unlink",
    // Channel methods
    "send", "recv", "trySend", "tryRecv", "sendTimeout", "recvTimeout",
    "close", "isClosed",
    // BigInt methods
    "sign", "isZero", "isNegative", "isPositive",
    // Logger methods
    "debug", "info", "warn", "fatal", "child",
...
XrSymbolTable* xr_symbol_table_create(void) {
    ...
    table->id_to_name = (const char**)xr_malloc(
        sizeof(const char*) * (size_t)table->capacity
    );
    if (!table->id_to_name) {
...
bool xr_symbol_table_init_builtins(XrSymbolTable *table) {
    ...
    XR_CHECK(BUILTIN_NAME_COUNT == SYMBOL_BUILTIN_COUNT - 1, "symbol table: builtin name count mismatch");
...
SymbolId xr_symbol_register_in_table(XrSymbolTable *table, const char *name) {
    if (!table || !name) return SYMBOL_INVALID;

    SymbolId existing = xr_symbol_lookup_in_table(table, name);
    if (existing != SYMBOL_INVALID) return existing;

    if (table->count >= table->capacity) {
        if (!expand_capacity(table)) return SYMBOL_INVALID;
    }
    ...
    if (!name_copy) return SYMBOL_INVALID;
...
SymbolId xr_symbol_lookup_in_table(XrSymbolTable *table, const char *name) {
    ...
    if (!value) return SYMBOL_INVALID;
