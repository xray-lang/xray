...
#define DYNARRAY_ADD(arr, elem, T) \
    ({ T _tmp = (elem); xr_dynarray_add_raw(arr, &_tmp); })

#define DYNARRAY_GET(arr, index, T) \
    (*(T*)xr_dynarray_get_raw(arr, index))

#define DYNARRAY_GET_PTR(arr, index, T) \
    ((T*)xr_dynarray_get_raw(arr, index))

#define DYNARRAY_SET(arr, index, elem, T) \
    do { T _tmp = (elem); xr_dynarray_set_raw(arr, index, &_tmp); } while(0)
