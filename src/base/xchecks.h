...
#if XR_DEBUG
    #define XR_DCHECK_EQ(a, b, msg) \
        XR_CHECK((a) == (b), msg)
    #define XR_DCHECK_NE(a, b, msg) \
        XR_CHECK((a) != (b), msg)
    #define XR_DCHECK_LT(a, b, msg) \
        XR_CHECK((a) < (b), msg)
    #define XR_DCHECK_LE(a, b, msg) \
        XR_CHECK((a) <= (b), msg)
    #define XR_DCHECK_GT(a, b, msg) \
        XR_CHECK((a) > (b), msg)
    #define XR_DCHECK_GE(a, b, msg) \
        XR_CHECK((a) >= (b), msg)
#else
...
#if XR_DEBUG
    #define XR_DCHECK_BOUNDS(index, limit, msg) \
        XR_CHECK_BOUNDS(index, limit, msg)
#else
...
#define XR_CHECK_REDS(X, cost) do {                              \
...
#define XR_VM_IC_METHOD_BIND(cache, expected_offset) do { \
    if ((cache)->debug_instruction_offset == -1) { \
        ...
        fprintf(stderr, "[IC BUG] Method cache slot collision: " \
...
#define XR_VM_IC_FIELD_BIND(cache, expected_offset) do { \
    if ((cache)->debug_instruction_offset == -1) { \
        (cache)->debug_instruction_offset = (expected_offset); \
    } else if ((cache)->debug_instruction_offset != (expected_offset)) { \
        fprintf(stderr, "[IC BUG] Field cache slot collision: " \
                "expected offset %d but got %d\n", \
                (cache)->debug_instruction_offset, (expected_offset)); \
        abort(); \
    } \
...
#define xray_api_check(cond, msg) \
    do { XR_DCHECK(cond, msg); if (XR_UNLIKELY(!(cond))) return; } while(0)

// Value-returning API check
#define xray_api_checkr(cond, msg, retval) \
    do { XR_DCHECK(cond, msg); if (XR_UNLIKELY(!(cond))) return (retval); } while(0)
