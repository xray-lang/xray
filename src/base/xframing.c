 ...
 */
static const char *find_header_field(const char *haystack, const char *hay_end,
...
XrFrameStatus xr_frame_parse(const char *buf, size_t buf_len,
                              size_t *header_end, int *content_length) {
    if (!buf || !header_end || !content_length) return XR_FRAME_ERROR;
    ...
    if (!sep) return XR_FRAME_PARTIAL;
    ...
    if (!cl) return XR_FRAME_ERROR;

    /* Skip "Content-Length:" and whitespace */
    const char *val = cl + (sizeof(CL) - 1);
    while (val < sep && isspace((unsigned char)*val)) val++;
    ...
    if (!has_digit || n < 0) return XR_FRAME_ERROR;
