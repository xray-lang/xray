...
static const char* find_any_scalar(const char *s, size_t len, 
...
static const char* find_any_sse2(const char *s, size_t len, 
                                  const char *chars, int nchars) {
    ...
    for (; i + 16 <= len; i += 16) {
        ...
        __m128i result = _mm_or_si128(_mm_or_si128(r0, r1), 
                                       _mm_or_si128(r2, r3));
...
static const char* find_any_avx512(const char *s, size_t len, 
                                    const char *chars, int nchars) {
    if (nchars <= 0 || nchars > 4) {
        return find_any_scalar(s, len, chars, nchars);
    }
...
static const char* find_any_neon(const char *s, size_t len, 
                                  const char *chars, int nchars) {
    if (nchars <= 0 || nchars > 4) {
        return find_any_scalar(s, len, chars, nchars);
    }
...
const char* xr_simd_find_range_neon(const char *s, size_t len,
    ...
    if (XR_LIKELY(len >= 16 && range_len >= 2 && range_len <= 8)) {
        ...
        for (; i + 16 <= len; i += 16) {
            ...
            // Merge results
            uint8x16_t in_any = vorrq_u8(vorrq_u8(in_range1, in_range2),
                                          vorrq_u8(in_range3, in_range4));
...
const char* xr_simd_skip_range_neon(const char *s, size_t len,
                                     const char *ranges, int range_len) {
    size_t i = 0;
...
const char* xr_simd_find_any(const char *s, size_t len, 
                              const char *chars, int nchars) {
    if (len < 16 || nchars <= 0) {
        return find_any_scalar(s, len, chars, nchars);
    }
...
const char* xr_simd_find_csv_delim(const char *s, size_t len, 
...
const uint8_t xr_char_class[256] = {
    ...
    0x00, 0x08, 0x10, 0x00, 0x00, 0x10, 0x00, 0x00,  // \t=0x09, \n=0x0A, \r=0x0D
 ...
 */
const char* xr_simd_find_range_sse42(const char *s, size_t len,
                                      const char *ranges, int range_len) {
 ...
 */
const char* xr_simd_skip_range_sse42(const char *s, size_t len,
                                      const char *ranges, int range_len) {
    if (XR_UNLIKELY(len < 16)) {
        goto scalar;
    }
