...
static void resize(XrHashMap *map, uint32_t new_capacity) {
    if (map->is_arena_allocated) {
        XR_DCHECK(false, "hashmap resize: arena-allocated map hit load factor, consider larger initial capacity");
        return;
    }
    ...
    for (uint32_t i = 0; i < old_capacity; i++) {
        ...
        if (entry->key != NULL) {
            // Direct insert without load factor check
            uint32_t h = hash_string(entry->key) & (new_capacity - 1);
