...
#define XR_MAP_PROFILE_ALLOC_HEADER(bytes) xr_profile_stats_alloc(&g_map_header_stats, bytes)

#define XR_MAP_PROFILE_ALLOC_NODES(bytes) xr_profile_stats_alloc(&g_map_node_stats, bytes)

#define XR_MAP_PROFILE_FREE_HEADER(bytes) xr_profile_stats_free(&g_map_header_stats, bytes)

#define XR_MAP_PROFILE_FREE_NODES(bytes) xr_profile_stats_free(&g_map_node_stats, bytes)
...
#define XR_ARRAY_PROFILE_ALLOC_HEADER(bytes) xr_profile_stats_alloc(&g_array_header_stats, bytes)

#define XR_ARRAY_PROFILE_ALLOC_ELEMENTS(bytes) xr_profile_stats_alloc(&g_array_elem_stats, bytes)
...
#define XR_STRING_PROFILE_ALLOC(bytes) xr_profile_stats_alloc(&g_string_stats, bytes)
    ...
    do {                                                                                           \
        ...
        if (g_string_intern_hits + g_string_intern_misses > 0) {                                   \
            printf("Intern rate:    %.1f%%\n",                                                     \
                   100.0 * g_string_intern_hits /                                                  \
                       (g_string_intern_hits + g_string_intern_misses));                           \
        }                                                                                          \
    } while (0)
...
#define XR_CLASS_PROFILE_ALLOC(bytes) xr_profile_stats_alloc(&g_class_stats, bytes)

#define XR_INSTANCE_PROFILE_ALLOC(bytes) xr_profile_stats_alloc(&g_instance_stats, bytes)
