...
XrUnicodeProperty xr_unicode_property_lookup(const char *name, int len) {
    ...
    if (!name || len <= 0) return XR_UP_INVALID;
...
bool xr_unicode_property_ranges(XrUnicodeProperty prop,
                                const XrUnicodeRange **out_ranges,
                                int *out_count) {
    if (!out_ranges || !out_count) return false;
