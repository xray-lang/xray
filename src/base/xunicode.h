...
XR_FUNC bool xr_unicode_property_ranges(XrUnicodeProperty prop, const XrUnicodeRange **out_ranges,
                                        int *out_count);
...
static inline bool xr_unicode_is_alnum(uint32_t cp) {
    ...
    return xr_unicode_is_property(cp, XR_UP_L) || xr_unicode_is_property(cp, XR_UP_Nd);
}
