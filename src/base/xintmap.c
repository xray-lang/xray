...
static void resize(XrIntMap *map, uint32_t new_capacity) {
    ...
    map->entries = xr_calloc(new_capacity, sizeof(XrIntMapEntry));
    if (!map->entries) {
...
XrIntMap* xr_intmap_new(void) {
    XrIntMap *map = xr_calloc(1, sizeof(XrIntMap));
    if (!map) return NULL;
    
    map->capacity = XR_INTMAP_MIN_CAPACITY;
    map->entries = xr_calloc(map->capacity, sizeof(XrIntMapEntry));
    if (!map->entries) {
        xr_free(map);
        return NULL;
    }
