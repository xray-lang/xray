...
static void verifier_bytes(XrSHA256Context *context, const uint8_t *bytes,
                           size_t size) {
...
static void verifier_slot(XrSHA256Context *context,
                          XrScalarCallDecisionSlot slot) {
...
static XrFingerprint verifier_fingerprint(const XrScalarCallDecision *decision) {
    ...
    verifier_bytes(&context, decision->generation_id.bytes,
    ...
    verifier_bytes(&context, decision->call_identity.bytes,
...
static const XrProgramSemanticFunctionRecord *verifier_find_function(
...
static bool verifier_function_exact(
    ...
    return function && xr_scalar_i64_function_contract(shape, &semantic) &&
           same_fingerprint(function->signature_fingerprint,
                            semantic.signature_fingerprint) &&
           same_fingerprint(function->effect_fingerprint,
                            semantic.effect_fingerprint) &&
...
bool xr_scalar_call_decision_verify(const XrScalarCallDecision *decision,
    ...
    if (!decision || !closure || !target_profile ||
        ...
        return reject(error, error_size,
                      "scalar call verification requires complete authorities");
    ...
    if (!machine || decision->schema != XR_SCALAR_CALL_DECISION_SCHEMA_VERSION ||
        decision->native_abi != machine->native_abi ||
        decision->native_abi <= XR_TARGET_ABI_NONE ||
        decision->native_abi >= XR_TARGET_ABI_COUNT ||
        decision->sealed != 1 ||
        ...
        decision->capability_mask != 0 ||
        decision->argument.machine_rep != XR_MACHINE_REP_I64 ||
    ...
    if (xr_program_semantic_closure_module_count(closure) != 1 ||
        ...
        return reject(error, error_size,
                      "program closure is outside the bounded scalar family");

    XrGenerationClosureId actual_generation =
    ...
    if (!same_bytes(decision->generation_id.bytes, actual_generation.bytes,
        ...
        !same_fingerprint(decision->closure_fingerprint,
                          closure_fingerprint) ||
        !same_fingerprint(decision->target_profile_fingerprint,
                          target_fingerprint))
        return reject(error, error_size,
                      "decision authority fingerprints are stale");
    ...
    if (!call || !caller || !callee || caller == callee ||
        ...
        return reject(error, error_size,
                      "decision does not bind the exact scalar source call");
    ...
    if (!same_fingerprint(decision->fingerprint, expected_fingerprint))
        return reject(error, error_size,
                      "scalar call decision fingerprint is invalid");
    return true;
}
