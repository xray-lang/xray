...
static inline bool xr_target_freestanding_assertion_equality_type_supported(
...
static inline bool xr_target_semantic_capability_mask(
    const XrSemanticPlan *semantic, uint8_t runtime_profile, uint64_t *out) {
    if (!semantic || !out ||
...
static inline bool xr_target_plan_capability_mask(const XrTargetPlan *plan,
                                                  uint64_t *out) {
    if (!plan || !out)
        return false;
    uint32_t count = 0;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (record->id != i || bit == 0 ||
            ...
            (i != 0 && records[i - 1].capability >= record->capability) ||
            (mask & bit) != 0)
            return false;
        mask |= bit;
    }
...
static inline bool xr_target_capability_mask_is_backed(
    uint64_t capability_mask, uint64_t provider_mask) {
    const uint64_t language_capabilities = XR_TARGET_CAPABILITY_MASK(
        XR_TARGET_CAPABILITY_TYPED_ERROR_BOUNDARY);
    return (capability_mask &
            ~(provider_mask | language_capabilities)) == 0;
}
