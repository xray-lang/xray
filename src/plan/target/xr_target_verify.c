...
static bool semantic_is_product_program_family(const XrSemanticPlan *semantic) {
    const XrSemanticProgramProvenance *row = xr_semantic_plan_program_provenance(semantic);
    return row && row->program_family == XR_PROGRAM_SEMANTIC_FAMILY_LEAF_VALUE_PRODUCT_DIRECT_CALL;
}
...
static bool verify_product_program_shape(const XrSemanticPlan *semantic,
    ...
    if (!semantic || !shape || !semantic_is_product_program_family(semantic) || !provenance ||
    ...
    for (uint32_t row = 0; row < 3; row++) {
        ...
        if (binding->kind == XR_PROGRAM_SEMANTIC_TYPE_LEAF_VALUE_PRODUCT) {
            if (shape->product || binding->field_count != 6 || type->kind != XR_KIND_TUPLE ||
            ...
            if (shape->i64 || type->kind != XR_KIND_INT || type->scalar_rep != XR_NATIVE_I64 ||
                type->child_count != 0)
                return false;
            shape->i64 = binding;
        } else if (binding->kind == XR_PROGRAM_SEMANTIC_TYPE_EXACT_SCALAR &&
                   binding->exact_scalar == XR_EXACT_SCALAR_U8) {
            if (shape->u8 || type->kind != XR_KIND_INT || type->scalar_rep != XR_NATIVE_U8 ||
                type->child_count != 0)
                return false;
            shape->u8 = binding;
        } else {
    ...
    for (uint32_t ordinal = 0; ordinal < 6; ordinal++) {
        ...
        const XrSemanticProgramTypeFieldBinding *field =
            xr_semantic_plan_program_type_field_binding(semantic,
                                                        shape->product->field_begin + ordinal);
    ...
    for (uint32_t row = 0; row < 2; row++) {
        ...
        if (!binding || !operation || binding->program_row >= 2 ||
            ...
            return false;
        uint32_t caller = operation->function == shape->caller_bindings[0]->semantic_function ? 0u
...
static bool verify_leaf_program_provenance(const XrSemanticPlan *semantic) {
    ...
    return row && semantic_is_leaf_program_family(semantic) &&
           ...
           row->reserved == 0 && !fingerprint_is_zero(row->program_fingerprint) &&
...
#define XR_VERIFY_WHOLE_TABLE(name)                                                                \
    ...
    XR_VERIFY_WHOLE_TABLE(i64_overflow_predicates);
...
/* Dependency ordinals are local to the SemanticPlan that issued them. A
 * partitioned program retains the complete module set, so reconstruct the
 * exact dependency owner from the local dependency row instead of accidentally
 * indexing the entry module's dependency vector. */
static const XrSemanticPlan *verifier_semantic_dependency(const XrTargetPlan *plan,
                                                          const XrSemanticPlan *semantic,
                                                          uint32_t dependency) {
    if (!plan || !semantic || dependency >= xr_semantic_plan_dependency_count(semantic))
        return NULL;
    if (!plan->module_partitions_count)
        return semantic == plan->semantic_plan && dependency < plan->semantic_dependency_count
                   ? plan->semantic_dependencies[dependency]
                   : NULL;
    const XrSemanticDependencyRecord *required = xr_semantic_plan_dependency(semantic, dependency);
    const XrSemanticPlan *match = NULL;
    for (uint32_t row = 0; required && row < plan->semantic_module_count; row++) {
        const XrSemanticPlan *candidate = plan->semantic_modules[row];
...
static bool verify_resource_budgets(const XrTargetPlan *plan, char *error, size_t error_size) {
    ...
    if (plan->semantic_dependency_count > (SIZE_MAX - total) / sizeof(*plan->semantic_dependencies))
    ...
    if (plan->semantic_module_count > (SIZE_MAX - total) / sizeof(*plan->semantic_modules))
        return report(error, error_size, "XR_EXEC_5003", "TargetPlan byte budget overflow");
...
/* A semantic-dependent machine representation names a row in the program-wide
 * layout table. Resolve that row back to the one module partition that owns it
 * before interpreting the layout's module-local semantic_type. */
static bool verifier_layout_owner(const XrTargetPlan *plan, uint32_t layout_index,
                                  const XrSemanticPlan **semantic,
    ...
    if (layout)
        *layout = NULL;
    if (!plan || layout_index >= plan->layouts_count)
        return false;
    ...
    for (uint32_t partition = 0; partition < verifier_partition_count(plan); partition++) {
        ...
        if (!verifier_partition_view(plan, partition, &view) ||
            !range_valid(view.range.layouts_begin, view.range.layouts_count, plan->layouts_count))
            return false;
        if (layout_index < view.range.layouts_begin ||
            layout_index - view.range.layouts_begin >= view.range.layouts_count)
            continue;
        if (owner)
            return false;
        owner = view.semantic;
    ...
    if (semantic)
        *semantic = owner;
    if (layout)
        *layout = &plan->layouts[layout_index];
    return true;
}
...
static bool borrowed_raw_pointer_rep_is_exact(const XrTargetPlan *plan,
    ...
    bool found = false;
    for (uint32_t partition = 0; plan && partition < verifier_partition_count(plan); partition++) {
        XrTargetPartitionView view;
        if (!verifier_partition_view(plan, partition, &view) ||
            !range_valid(view.range.value_reps_begin, view.range.value_reps_count,
                         plan->value_reps_count))
            return false;
        uint32_t value_rep_end = view.range.value_reps_begin + view.range.value_reps_count;
        for (uint32_t i = view.range.value_reps_begin; i < value_rep_end; i++) {
            ...
            for (uint32_t p = 0; p < (uint32_t) xr_semantic_plan_parameter_count(view.semantic);
                 p++) {
                const XrSemanticParameterRecord *candidate =
                    xr_semantic_plan_parameter(view.semantic, p);
...
static bool rep_kind_contract_is_exact(const XrTargetPlan *plan,
    ...
    switch (rep->kind) {
        case XR_MACHINE_REP_ENUM_ORDINAL: {
            ...
            if (!verifier_layout_owner(plan, rep->detail, &semantic, &layout) ||
...
static bool semantic_array_allocation_is_exact(const XrSemanticPlan *semantic,
    ...
    return element && capacity_type && function &&
           xr_target_array_allocation_element_storage(semantic, element,
...
static bool semantic_direct_local_reference_type_is_exact_verify(const XrSemanticPlan *semantic,
                                                                 uint32_t type_index,
                                                                 bool indexes_elements,
                                                                 uint8_t *storage) {
...
static bool semantic_direct_local_reference_value_parameter_is_exact_verify(
    ...
    if (storage)
        *storage = XR_TARGET_ARRAY_STORAGE_NONE;
    return xr_semantic_dynamic_value_carrier_type_is_exact(
...
static bool semantic_direct_local_reference_result_is_exact_verify(const XrSemanticPlan *semantic,
                                                                   uint32_t operation_index) {
    const XrSemanticOperationRecord *operation =
    ...
    if (!semantic || !operation || !xr_semantic_local_call_result_opcode_is_exact(operation) ||
        ...
        !semantic_direct_local_reference_type_is_exact_verify(semantic, operation->result_type,
                                                              false, NULL))
        return false;
    const XrSemanticFunctionRecord *callee =
...
static bool semantic_direct_local_ref_place_is_exact_verify(
    const XrSemanticPlan *semantic, const XrSemanticOperandRecord *call_operand,
    uint32_t semantic_function, uint32_t *storage_value) {
...
static bool semantic_direct_local_ref_address_is_exact_verify(
    ...
    if (!xr_semantic_ref_argument_local_addr_is_exact(
        ...
        return false;
    if (tagged && xr_semantic_native_direct_ref_address_is_exact(semantic, address))
        return true;
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands = xr_semantic_plan_operands(semantic, &operand_count);
    ...
    for (uint32_t target_index = 0; operands && target_index < target_count; target_index++) {
        ...
        for (uint16_t ordinal = shift == 0u ? 1u : 0u;
            ...
            if (!parameter || parameter->function != target->function ||
                ...
                !(tagged ? semantic_direct_local_tagged_ref_parameter_is_exact_verify(
...
static bool semantic_source_class_field_result_is_exact_verify(
    const XrTargetPlan *plan, const XrSemanticPlan *semantic,
    const XrSemanticOperationRecord *operation, uint32_t semantic_value,
...
/* Independently re-derive an exact source structural field selection and bind
 * it to the value row under verification. */
static bool semantic_source_structural_field_result_is_exact_verify(
    ...
    return xr_semantic_source_structural_field_result_carrier_is_exact(semantic, operation,
                                                                       &carrier) &&
...
/* A borrowed writeback reads either an exact ref parameter or a LOCAL_ADDR
 * proved by a semantic ref call. Executable calls may be absent in cold bodies;
 * their absence does not change the retained storage's type or ownership. */
static bool semantic_direct_local_tagged_ref_place_load_is_exact_verify(
    const XrTargetPlan *plan, const XrTargetPartitionView *view,
    const XrSemanticOperationRecord *load, uint32_t semantic_value, uint32_t semantic_type,
    uint32_t semantic_function) {
    ...
    if (semantic_direct_local_tagged_ref_parameter_is_exact_verify(semantic, place_parameter,
        ...
        const XrTargetValueRepRecord *binding = verifier_value_rep(plan, view, place->value);
        ...
        return binding && register_rep && memory_rep && slot &&
               ...
               slot->function == view->range.functions_begin + semantic_function &&
...
static int semantic_aggregate_eligibility(const XrSemanticPlan *plan, uint32_t semantic_type,
    ...
    int aggregate = xr_semantic_aggregate_type_kind(type) == 1 &&
                            verify_target_managed_aggregate_graph(
                                plan, semantic_type, managed_stack, 0, &managed_fields) &&
                            managed_fields != 0
                        ? 1
...
/* Two function indices, and they are not the same number once a plan holds
 * more than one module: the slot names a row in the plan's one function table,
 * while the value and the parameter belong to their module's own numbering. */
static bool reconstruct_value_slot_identity(const XrSemanticPlan *semantic,
                                            const XrTargetSlotRecord *slot,
                                            uint32_t target_function, uint32_t semantic_value,
                                            uint32_t semantic_function, XrStableId *out) {
    return reconstruct_value_slot_identity_for_semantic(semantic, slot, target_function,
...
static bool semantic_direct_local_nullable_string_result_is_exact(
    ...
    if (!semantic || !operation || !xr_semantic_local_call_result_opcode_is_exact(operation) ||
        ...
        operation->result_alias_operand != -1 || operation->return_parameter != -1 ||
        operation->return_complete != 1 || operation->return_provenance != XR_SEM_RETURN_OWNED ||
        !xr_semantic_nullable_tagged_string_type_is_exact(
            xr_semantic_plan_type(semantic, operation->result_type)))
        return false;
    const XrSemanticFunctionRecord *callee =
        semantic_direct_local_callee_for_operation(semantic, operation_index);
...
static bool semantic_source_dependency_owned_reference_result_is_exact_verify(
    ...
    return caller && dependency && operation && callee &&
           ...
           (((xr_semantic_tagged_string_type_is_exact(caller_type) ||
              xr_semantic_nullable_tagged_string_type_is_exact(caller_type)) &&
             (xr_semantic_tagged_string_type_is_exact(callee_type) ||
              xr_semantic_nullable_tagged_string_type_is_exact(callee_type))) ||
            (semantic_direct_local_reference_type_is_exact_verify(caller, operation->result_type,
                                                                  false, NULL) &&
             semantic_direct_local_reference_type_is_exact_verify(dependency, callee->return_type,
                                                                  false, NULL))) &&
...
static bool semantic_source_dependency_owned_reference_result_for_operation_is_exact_verify(
    ...
    if (!operation || (!(xr_semantic_tagged_string_type_is_exact(type) ||
                         xr_semantic_nullable_tagged_string_type_is_exact(type)) &&
                       !semantic_direct_local_reference_type_is_exact_verify(
                           semantic, operation->result_type, false, NULL)))
        return false;
...
static bool operation_is_exact_array_member_tagged_store(const XrSemanticPlan *semantic,
    ...
    if (!type || operand_index >= operand_count ||
        !xr_semantic_array_member_owned_reference_type_is_exact(semantic, type) ||
...
static bool collect_exact_channel_values(const XrTargetPlan *plan, const XrSemanticPlan *semantic,
                                         uint8_t **out, char *error, size_t error_size) {
    if (out)
        *out = NULL;
    size_t function_count = xr_semantic_plan_function_count(semantic);
    ...
    if (function_count) {
        const XrSemanticFunctionRecord *last =
            xr_semantic_plan_function(semantic, (uint32_t) function_count - 1u);
    ...
    for (uint32_t i = 0; i < (uint32_t) operation_count; i++) {
        ...
        bool allocation = verifier_channel_allocation_is_exact(semantic, operation);
        bool alias =
            verifier_channel_identity_copy_is_exact(semantic, operation, exact, value_count);
...
static bool verifier_channel_receive_is_exact(const XrTargetPlan *plan,
                                              ...
                                              const uint8_t *exact_channel_values,
                                              uint32_t value_count) {
    if (!plan || !semantic || !operation || !exact_channel_values ||
    ...
    return receiver->value < value_count && exact_channel_values[receiver->value] != 0 &&
           ...
           verifier_channel_type_is_exact(semantic, receiver->type, &element_type) &&
           element_type == operation->result_type &&
           semantic_type_expected_rep(xr_semantic_plan_type(semantic, operation->result_type),
                                      &result_kind) == 1 &&
...
static bool collect_exact_channel_receive_values(const XrTargetPlan *plan,
    ...
    if (value_count && !exact_channel_values)
        return report(error, error_size, "XR_TARGET_1001",
                      "channel receive storage set is missing");
    uint8_t *exact = value_count ? (uint8_t *) xr_calloc(value_count, sizeof(*exact)) : NULL;
    if (value_count && !exact)
        return report(error, error_size, "XR_EXEC_5003",
    ...
    for (uint32_t i = 0; i < (uint32_t) operation_count; i++) {
        ...
        if (semantic_type_expected_rep(xr_semantic_plan_type(semantic, operation->result_type),
                                       &receive_kind) != 1 ||
            receive_kind == XR_MACHINE_REP_VOID)
            continue;
        if (!verifier_channel_receive_is_exact(plan, semantic, operation, exact_channel_values,
                                               value_count)) {
            xr_free(exact);
            return report(error, error_size, "XR_TARGET_1001",
...
static bool collect_exact_direct_local_callee_values(const XrTargetPlan *plan,
                                                     const XrSemanticPlan *semantic,
        ...
        for (uint16_t a = 0; a < use->operand_count; a++) {
            ...
            const XrSemanticOperationRecord *source =
                source_index == XR_SEMANTIC_INDEX_NONE
                    ? NULL
                    : xr_semantic_plan_operation(semantic, source_index);
    ...
    for (uint32_t i = 0; i < operation_count; i++) {
        ...
        if (invalid[value] || use_count[value] == 0 ||
            !xr_semantic_direct_local_callee_type_is_exact(semantic, operation,
...
static bool collect_exact_source_namespace_values(const XrTargetPlan *plan,
                                                  const XrSemanticPlan *semantic,
                                                  uint8_t **out_exact, char *error,
                                                  size_t error_size) {
    if (out_exact)
    ...
    for (uint32_t i = 0; i < (uint32_t) target_size; i++) {
        ...
        bool mixed_native = target && target->kind == XR_SEM_CALL_TARGET_NATIVE_NAMESPACE_YIELDABLE;
        if (!source_export && !mixed_native)
            continue;
        if (target->operation >= operation_count ||
            (source_export && target->dependency >= xr_semantic_plan_dependency_count(semantic)) ||
            (mixed_native && target->dependency != XR_SEMANTIC_INDEX_NONE) ||
            target_by_operation[target->operation] != XR_SEMANTIC_INDEX_NONE)
            goto invalid;
        target_by_operation[target->operation] = i;
    }
    ...
    for (uint32_t i = 0; i < operation_count; i++) {
        ...
        for (uint32_t depth = 0;; depth++) {
            ...
            if (!source || source->result_value != current_value ||
                ...
                (candidate[current_value] &&
                 ((!mixed_native && dependency[current_value] != target->dependency) ||
        ...
        if (!import || !load || receiver->type != load->result_type || import->function != 0 ||
            ...
            (mixed_native && import->import_resolution != XR_SEM_IMPORT_RESOLUTION_SOURCE_MODULE &&
             import->import_resolution != XR_SEM_IMPORT_RESOLUTION_NATIVE_STDLIB))
            goto invalid;
        if (mixed_native && import->import_resolution == XR_SEM_IMPORT_RESOLUTION_NATIVE_STDLIB) {
            for (uint32_t value = 0; value < value_count; value++) {
                if (visit_epoch[value] != receiver_epoch && visit_epoch[value] != import_epoch)
                    continue;
                candidate[value] = 0;
                dependency[value] = XR_SEMANTIC_INDEX_NONE;
                consumer[value] = XR_SEMANTIC_INDEX_NONE;
            }
            continue;
        }
        if (!verifier_source_import_dependency_is_exact(semantic, import, metadata, metadata_count,
                                                        named_export, &import_dependency) ||
            (!mixed_native && import_dependency != target->dependency))
            goto invalid;
        if (mixed_native)
            for (uint32_t value = 0; value < value_count; value++)
                if (visit_epoch[value] == receiver_epoch || visit_epoch[value] == import_epoch)
                    dependency[value] = import_dependency;
    }
    for (uint32_t i = 0; i < operation_count; i++) {
            ...
            if (expected && (use->opcode == XI_CALL || use->opcode == XI_CALL_METHOD)) {
                ...
                const XrSemanticCallTargetRecord *target =
                    target_index != XR_SEMANTIC_INDEX_NONE
                        ? xr_semantic_plan_call_target(semantic, target_index)
                        : NULL;
                bool target_owns_source_dependency =
                    target && target->kind == XR_SEM_CALL_TARGET_SOURCE_EXPORT &&
                    target->dependency == dependency[operand->value];
                bool mixed_native =
                    target && target->kind == XR_SEM_CALL_TARGET_NATIVE_NAMESPACE_YIELDABLE;
                expected = target && target->operation == i &&
                           (target_owns_source_dependency || mixed_native) &&
...
static bool collect_exact_dynamic_types(const XrTargetPlan *plan, const XrTargetPartitionView *view,
                                        const uint8_t *exact_direct_callees,
                                        const uint8_t *exact_go_callees,
                                        const uint8_t *exact_channel_values,
                                        const uint8_t *exact_source_namespaces,
                                        const uint8_t *exact_native_module_namespaces,
                                        uint8_t **out, char *error, size_t error_size) {
    if (out)
        *out = NULL;
    const XrSemanticPlan *semantic = view ? view->semantic : NULL;
    size_t type_count = xr_semantic_plan_type_count(semantic);
    size_t operation_count = xr_semantic_plan_operation_count(semantic);
    ...
    if (!plan || !semantic || !out || type_count > UINT32_MAX || operation_count > UINT32_MAX)
        return report(error, error_size, "XR_EXEC_5003",
    ...
    for (uint32_t i = 0; i < (uint32_t) type_count; i++)
        if (semantic_nullable_scalar_type_is_exact(xr_semantic_plan_type(semantic, i)))
            exact_types[i] = 1;
    for (uint32_t i = 0; i < (uint32_t) operation_count; i++) {
        const XrSemanticOperationRecord *operation = xr_semantic_plan_operation(semantic, i);
        ...
        bool exact_array_hof_result =
            semantic_array_hof_is_exact_verify(semantic, operation, &array_hof_kind, NULL, NULL,
                                               NULL, NULL, NULL) &&
        ...
        if (xr_semantic_owned_closure_is_exact(semantic, operation) ||
            ...
            xr_semantic_container_copy_is_exact(semantic, operation, NULL, NULL) ||
            semantic_array_fill_scalar_is_exact_verify(semantic, operation, NULL, NULL, NULL) ||
            ...
            semantic_source_class_field_result_is_exact_verify(
                plan, semantic, operation, operation->result_value, operation->result_type,
                operation->function) ||
            semantic_source_structural_field_result_is_exact_verify(
                semantic, operation, operation->result_value, operation->result_type,
                operation->function) ||
            ...
            xr_semantic_runtime_constructor_is_exact(semantic, operation) ||
            ...
            xr_semantic_iterator_rune_nth_is_exact(semantic, operation, NULL, NULL) ||
            semantic_map_entries_iterator_is_exact_verify(semantic, operation, NULL, NULL) ||
            ...
            operation_is_exact_rune_to_uint32_verify(semantic, operation, NULL) ||
            ...
            (operation->opcode == XI_GO && operation->operand_count != 0 && semantic_operands &&
             ...
             xr_semantic_direct_local_go_task_result_is_exact(semantic, operation, true, NULL)) ||
     ...
     * or not at all. */
    size_t parameter_count = xr_semantic_plan_parameter_count(semantic);
    for (uint32_t i = 0; i < (uint32_t) parameter_count; i++) {
        ...
        if (!parameter || parameter->type >= type_count ||
            (xr_semantic_class_instance_parameter_source_class(semantic, i) ==
...
static bool verify_value_binding(
    ...
    uint8_t *bound_slots, uint32_t *failure_reason) {
...
#define XR_VALUE_BINDING_FAIL(reason)                                                              \
    ...
    bool operation_result_void =
        xr_semantic_operation_result_void_governs_storage(semantic, operation, semantic_value,
                                                          semantic_type, semantic_function) &&
    ...
    bool exact_native_direct_fresh =
        (xr_semantic_native_direct_fresh_result_is_exact(semantic, operation, NULL) ||
         xr_semantic_native_yieldable_fresh_result_is_exact(semantic, operation, NULL)) && operation &&
        operation->result_value == semantic_value && operation->result_type == semantic_type &&
        operation->function == semantic_function;
    ...
    uint8_t array_hof_kind = XR_TARGET_ARRAY_HOF_NONE;
    bool exact_array_hof_result =
        semantic_array_hof_is_exact_verify(semantic, operation, &array_hof_kind, NULL, NULL, NULL,
                                           NULL, NULL) &&
        array_hof_kind != XR_TARGET_ARRAY_HOF_REDUCE;
    ...
    bool exact_source_class_field_result = semantic_source_class_field_result_is_exact_verify(
        plan, semantic, operation, semantic_value, semantic_type, semantic_function);
    bool exact_source_structural_field_result =
        semantic_source_structural_field_result_is_exact_verify(semantic, operation, semantic_value,
                                                                semantic_type, semantic_function);
    ...
    bool exact_bigint_value = xr_semantic_bigint_value_is_exact(semantic, operation);
    ...
    bool exact_string_concat = xr_semantic_string_concat_is_exact(semantic, operation) &&
                               operation && operation->result_value == semantic_value &&
                               operation->result_type == semantic_type;
    /* Rebuilt the same way and for the same reason: both `string(x)` and the
     * exact UTF-8 decoders allocate a fresh String owner whose only storage
     * fact is the outer tagged value. */
     ...
     * stays unproven. */
    bool exact_class_object = xr_semantic_class_object_is_exact(semantic, operation) && operation &&
                              operation->result_value == semantic_value &&
                              operation->result_type == semantic_type;
    ...
    bool exact_direct_string_result =
        (semantic_direct_local_string_result_is_exact(semantic, operation_index) ||
    ...
    bool exact_source_dependency_owned_reference_result =
        ...
        operation && operation->result_value == semantic_value &&
        operation->result_type == semantic_type;
    bool exact_runtime_constructor = xr_semantic_runtime_constructor_is_exact(semantic, operation);
    ...
    bool exact_string_runes = xr_semantic_string_runes_is_exact(semantic, operation, NULL);
    ...
    bool exact_string_slice_range =
        xr_semantic_string_slice_range_is_exact(semantic, operation, NULL, NULL, NULL);
    bool exact_rune_to_string = xr_semantic_rune_to_string_is_exact(semantic, operation, NULL) &&
                                operation && operation->result_value == semantic_value &&
                                operation->result_type == semantic_type &&
                                operation->function == semantic_function;
    ...
    bool exact_string_byte_view = semantic_string_byte_slice_view_is_exact(semantic, operation);
    /* The view a range slice produces takes the same VIEW pair, re-proved from
     * the one judgement the builder published it from. */
    bool exact_range_slice_view = operation && operation->result_value == semantic_value &&
                                  operation->result_type == semantic_type &&
                                  operation->function == semantic_function &&
                                  xr_semantic_slice_view_result_is_exact(semantic, operation, NULL);
    const XrSemanticParameterRecord *parameter =
        operation ? NULL
                  : semantic_parameter_for_value(semantic, semantic_function, semantic_value);
    uint32_t parameter_index =
        operation ? XR_SEMANTIC_INDEX_NONE
                  : semantic_parameter_index_for_value(semantic, semantic_function, semantic_value);
    ...
    bool exact_string_value_parameter = xr_semantic_direct_local_string_value_parameter_is_exact(
                                            semantic, parameter, &string_parameter_callee_owns) &&
                                        parameter->type == semantic_type;
    ...
    bool exact_direct_array_result =
        operation && operation->result_value == semantic_value &&
        operation->result_type == semantic_type &&
        semantic_direct_local_reference_result_is_exact_verify(semantic, operation_index);
    ...
    uint32_t receiver_parameter =
        operation ? XR_SEMANTIC_INDEX_NONE
                  : xr_semantic_class_parameter_for_value(semantic, semantic_value);
    const XrSemanticParameterRecord *receiver_record =
        xr_semantic_plan_parameter(semantic, receiver_parameter);
    bool exact_class_receiver = xr_semantic_class_instance_parameter_source_class(
                                    semantic, receiver_parameter) != XR_SEMANTIC_INDEX_NONE &&
    ...
    bool leaf_family = semantic_is_leaf_program_family(semantic);
    bool leaf_shape_exact = leaf_family && verify_leaf_program_shape(semantic, &leaf_program);
    ...
    bool product_family = semantic_is_product_program_family(semantic);
    bool product_shape_exact =
        product_family && verify_product_program_shape(semantic, &product_program);
    ...
    if (eligibility == 0 && !deferred_operation) {
        if (exact_leaf_aggregate_type || exact_product_type)
            ...
            aggregate = -1;
        /* A reference can be a field of an aggregate without itself having
         * aggregate storage. Only inline aggregate roots enter that walk. */
        else if (xr_semantic_aggregate_type_kind(type) != 0)
    ...
    if (exact_heap_closure || exact_panic_catch || exact_dynamic_value ||
        ...
        exact_direct_array_result || exact_native_storage_parameter) {
    ...
    const XrTargetValueRepRecord *forwarded =
        exact_owner_transfer  ? verifier_value_rep(plan, view, owner_transfer_source)
        : exact_identity_copy ? verifier_value_rep(plan, view, identity_copy_source)
                              : NULL;
    /* Geometry comes from the source; ownership comes from the verified
     * operation. A rename borrows, while a balanced owner transfer owns. */
    if (forwarded) {
        XrTargetMachineRepRecord expected_register = plan->machine_reps[forwarded->register_rep];
        XrTargetMachineRepRecord expected_memory = plan->machine_reps[forwarded->memory_rep];
        if (expected_kind == XR_MACHINE_REP_DYN_VALUE) {
            uint8_t ownership = operation->result_ownership == XI_GEN_RESULT_OWNERSHIP_BORROWED
                                    ? XR_TARGET_OWNERSHIP_BORROWED
                                    : XR_TARGET_OWNERSHIP_OWNED;
            expected_register.ownership = expected_memory.ownership = ownership;
        }
        if (!machine_reps_are_storage_compatible(&expected_register,
                                                 &plan->machine_reps[record->register_rep]) ||
            !machine_reps_are_storage_compatible(&expected_memory,
                                                 &plan->machine_reps[record->memory_rep]))
            XR_VALUE_BINDING_FAIL(9);
    }
    if (expected_kind == XR_MACHINE_REP_DYN_VALUE && !forwarded) {
    ...
    /* The function index came from the module's own SemanticPlan; the function
     * table is one table for the whole plan. Its module's range is what turns
     * one into the other, and with a single partition that range starts at 0. */
    uint32_t target_function_index = view->range.functions_begin + semantic_function;
    if (semantic_function >= view->range.functions_count || record->slot >= plan->slots_count)
        XR_VALUE_BINDING_FAIL(5);
    const XrTargetFunctionRecord *target_function = &plan->functions[target_function_index];
    ...
    /* Twelve separate facts about one slot. Reporting them under one code makes
     * a frame-packing disagreement indistinguishable from a stale identity. */
    if (target_function->id != target_function_index ||
        target_function->semantic_function != semantic_function)
        XR_VALUE_BINDING_FAIL(61);
    if (!range_valid(target_function->slot_begin, target_function->slot_count, plan->slots_count) ||
        record->slot < target_function->slot_begin ||
        record->slot >= target_function->slot_begin + target_function->slot_count)
        XR_VALUE_BINDING_FAIL(62);
    if (bound_slots[record->slot])
        XR_VALUE_BINDING_FAIL(63);
    if (slot->function != target_function_index)
        XR_VALUE_BINDING_FAIL(64);
    if (!reconstruct_value_slot_identity(semantic, slot, target_function_index, semantic_value,
                                         semantic_function, &expected_slot_identity) ||
        !xr_stable_id_equal(slot->identity, expected_slot_identity))
        XR_VALUE_BINDING_FAIL(65);
    if (slot->register_rep != record->register_rep || slot->memory_rep != record->memory_rep)
        XR_VALUE_BINDING_FAIL(66);
    if (slot->size != memory->memory_size || slot->align != memory->memory_align ||
        ...
        XR_VALUE_BINDING_FAIL(67);
    bound_slots[record->slot] = 1;
#undef XR_VALUE_BINDING_FAIL
    return true;
}

/* The six value sets a binding judgement consults. Every one of them is a
 * bitmap indexed by a value number, and a value number means something only
 * inside the SemanticPlan that issued it -- so a set belongs to one module and
 * is collected once per module rather than once per plan. */
typedef struct XrTargetExactValueSets {
    uint8_t *direct_callees;
    uint32_t *direct_callee_targets;
    uint8_t *go_callees;
    uint32_t *go_callee_targets;
    uint8_t *channel_values;
    uint8_t *channel_receives;
    uint8_t *source_namespaces;
    uint8_t *native_module_namespaces;
} XrTargetExactValueSets;

static void exact_value_sets_dispose(XrTargetExactValueSets *sets) {
    xr_free(sets->direct_callees);
    xr_free(sets->direct_callee_targets);
    xr_free(sets->go_callees);
    xr_free(sets->go_callee_targets);
    xr_free(sets->channel_values);
    xr_free(sets->channel_receives);
    xr_free(sets->source_namespaces);
    xr_free(sets->native_module_namespaces);
    memset(sets, 0, sizeof(*sets));
}

static bool exact_value_sets_collect(const XrTargetPlan *plan, const XrSemanticPlan *semantic,
                                     XrTargetExactValueSets *sets, char *error, size_t error_size) {
    memset(sets, 0, sizeof(*sets));
    if (collect_exact_direct_local_callee_values(plan, semantic, &sets->direct_callees,
                                                 &sets->direct_callee_targets, error, error_size) &&
        collect_exact_direct_local_go_callee_values(plan, semantic, &sets->go_callees,
                                                    &sets->go_callee_targets, error, error_size) &&
        collect_exact_channel_values(plan, semantic, &sets->channel_values, error, error_size) &&
        collect_exact_channel_receive_values(plan, semantic, sets->channel_values,
                                             &sets->channel_receives, error, error_size) &&
        collect_exact_source_namespace_values(plan, semantic, &sets->source_namespaces, error,
                                              error_size) &&
        collect_exact_native_module_namespace_values(
            plan, semantic, &sets->native_module_namespaces, error, error_size))
        return true;
    exact_value_sets_dispose(sets);
    return false;
}

/* One module's values. Its SemanticPlan owns a value space that is dense in
 * itself, so the coverage bitmap is allocated per module; slot indices are
 * global, so every module marks into the one bitmap the caller owns and the
 * completeness check waits until all of them have been read. */
static bool verify_value_reps_partition(const XrTargetPlan *plan, const XrTargetPartitionView *view,
                                        uint8_t *bound_slots, uint32_t *failed_value,
                                        uint32_t *failed_type, uint32_t *failed_opcode,
                                        uint32_t *failure_reason,
                                        const XrSemanticPlan **failed_semantic,
                                        uint32_t *failed_module, char *error, size_t error_size) {
    const XrSemanticPlan *semantic = view->semantic;
    ...
    if (view->range.functions_count != semantic_functions)
        return report(error, error_size, "XR_TARGET_1001",
    ...
    for (uint32_t function_index = 0; function_index < (uint32_t) semantic_functions;
        ...
        if (!function || function->value_begin != expected_values ||
            ...
            return report(error, error_size, "XR_TARGET_1001",
                          "semantic value ranges are not dense within their module");
    ...
    for (uint32_t local = 0; local < view->range.value_reps_count; local++) {
        const XrTargetValueRepRecord *record =
            &plan->value_reps[view->range.value_reps_begin + local];
        if (record->semantic_value >= expected_values ||
            (local && record->semantic_value <= previous_value) ||
    ...
    XrTargetExactValueSets sets;
    if (!exact_value_sets_collect(plan, semantic, &sets, error, error_size))
        return false;
    uint8_t *defined =
        expected_values ? (uint8_t *) xr_calloc(expected_values, sizeof(*defined)) : NULL;
    ...
    bool valid = true;
    uint32_t parameters = (uint32_t) xr_semantic_plan_parameter_count(semantic);
    for (uint32_t index = 0; valid && index < parameters; index++) {
        const XrSemanticParameterRecord *parameter = xr_semantic_plan_parameter(semantic, index);
        if (!parameter || parameter->value >= expected_values)
            ...
            valid = verify_value_binding(
                ...
                XR_SEMANTIC_INDEX_NONE, sets.direct_callees, sets.go_callees, sets.channel_values,
                sets.channel_receives, sets.source_namespaces, sets.native_module_namespaces,
                bound_slots, failure_reason);
            if (!valid) {
    ...
    uint32_t operations = (uint32_t) xr_semantic_plan_operation_count(semantic);
        ...
        if (!operation)
            ...
            if (operation->result_value >= expected_values)
                ...
                valid = verify_value_binding(
                    ...
                    operation->function, operation, index, sets.direct_callees, sets.go_callees,
                    sets.channel_values, sets.channel_receives, sets.source_namespaces,
                    sets.native_module_namespaces, bound_slots, failure_reason);
                if (!valid) {
    ...
    for (uint32_t local = 0; valid && local < view->range.value_reps_count; local++) {
        const XrTargetValueRepRecord *record =
            &plan->value_reps[view->range.value_reps_begin + local];
...
static bool verify_value_reps(const XrTargetPlan *plan, char *error, size_t error_size) {
    ...
    for (uint32_t partition = 0; valid && partition < verifier_partition_count(plan); partition++) {
        ...
        valid = verify_value_reps_partition(plan, &view, bound_slots, &failed_value, &failed_type,
                                            &failed_opcode, &failure_reason, &failed_semantic,
                                            &failed_module, error, error_size);
    }
    ...
    xr_free(bound_slots);
    if (!valid) {
        /* A shape refusal already wrote its own diagnosis; only a binding
         * failure, which names a module, is reported here. */
        const XrSemanticTypeRecord *failed_semantic_type =
            failed_type == XR_SEMANTIC_INDEX_NONE || !failed_semantic
                ? NULL
                : xr_semantic_plan_type(failed_semantic, failed_type);
        if (failed_semantic && getenv("XRAY_TARGET_TRACE")) {
            const XrSemanticOperationRecord *source =
                xr_semantic_unique_value_definition(failed_semantic, failed_value);
            uint32_t operand_count = 0;
            const XrSemanticOperandRecord *operands =
                xr_semantic_plan_operands(failed_semantic, &operand_count);
            for (unsigned depth = 0; source && depth < 8; depth++) {
                const XrSemanticTypeRecord *source_type =
                    xr_semantic_plan_type(failed_semantic, source->result_type);
                fprintf(
                    stderr,
                    "[target] binding source value=%u type=%u opcode=%s immediate=%lld operands=%u "
                    "ownership=%u alias=%d return=%u complete=%u dynamic=%u identity=%u key=%s\n",
                    source->result_value, source->result_type,
                    xi_generated_op_name((XiOp) source->opcode),
                    (long long) source->semantic_immediate, source->operand_count,
                    source->result_ownership, source->result_alias_operand,
                    source->return_provenance, source->return_complete,
                    xr_semantic_dynamic_value_is_exact(failed_semantic, source),
                    xr_semantic_identity_copy_is_exact(failed_semantic, source, NULL),
        ...
        if (failed_semantic && error && error_size)
            ...
            snprintf(error, error_size,
                     "XR_TARGET_1001: target slots are not all bound to a value");
        return false;
    }
    return true;
}
...
static bool verify_layouts_partition(const XrTargetPlan *plan, const XrTargetPartitionView *view,
                                     const uint8_t *exact_dynamic_types, char *error,
                                     size_t error_size) {
    const XrSemanticPlan *semantic = view ? view->semantic : NULL;
    if (!plan || !semantic)
        return report(error, error_size, "XR_TARGET_1002",
                      "layout partition has no semantic owner");
    size_t semantic_types = xr_semantic_plan_type_count(semantic);
    ...
    uint32_t next_field = view->range.fields_begin;
    uint32_t layout_end = view->range.layouts_begin + view->range.layouts_count;
    uint32_t field_end = view->range.fields_begin + view->range.fields_count;
    for (uint32_t i = view->range.layouts_begin; i < layout_end; i++) {
        ...
        if (layout->id != i || layout->semantic_type >= semantic_types ||
            ...
            layout->extent < view->range.extents_begin ||
            layout->extent - view->range.extents_begin >= view->range.extents_count ||
            layout->field_begin != next_field || layout->field_begin < view->range.fields_begin ||
            !range_valid(layout->field_begin, layout->field_count, field_end))
        ...
        bool product_shape_exact =
            !product_family || verify_product_program_shape(semantic, &product_program);
        bool program_leaf_product = product_shape_exact && product_family &&
        ...
        bool exact_array_layout = semantic_direct_local_array_type_is_exact_verify(
            semantic, layout->semantic_type, false, &expected_array_storage);
        ...
        if (layout->kind == XR_TARGET_LAYOUT_SCALAR) {
            ...
            bool exact_dynamic_type =
                ...
                (xr_semantic_tagged_string_type_is_exact(semantic_type) ||
                 xr_semantic_nullable_tagged_string_type_is_exact(semantic_type)) ||
                xr_semantic_adt_enum_type_is_exact(semantic_type) ||
                xr_semantic_array_type_row_is_exact(semantic_type);
            ...
            for (uint32_t r = 0; r < plan->machine_reps_count; r++) {
                const XrTargetMachineRepRecord *rep = &plan->machine_reps[r];
                representation_count += rep->kind == XR_MACHINE_REP_VIEW && rep->detail == i &&
            ...
            if (scalar != 0 ||
                ...
                (semantic_type->kind == XR_KIND_FIXED_ARRAY &&
                 (semantic_type->child_count != 1 ||
                  !semantic_fixed_array_count(semantic, layout->semantic_type,
                                              &expected_fields))) ||
                (semantic_type->kind != XR_KIND_FIXED_ARRAY &&
        ...
        bool named_value_aggregate = !program_leaf_aggregate && !program_leaf_product &&
                                     xr_semantic_value_aggregate_shape_for_type(
                                         semantic, layout->semantic_type, &aggregate_shape);
        if (semantic_type->kind == XR_KIND_INSTANCE &&
            (semantic_type->flags & XR_SEM_TYPE_AGGREGATE_EXACT) != 0 && !program_leaf_aggregate &&
            !program_leaf_product && !named_value_aggregate)
            return report(error, error_size, "XR_TARGET_1002",
        ...
        for (uint32_t f = 0; f < layout->field_count; f++) {
            ...
            int child_layout_index = verifier_layout_for_type(plan, view, child_type);
            ...
            if (child_layout->kind == XR_TARGET_LAYOUT_SCALAR) {
                const XrSemanticTypeRecord *child_semantic =
                    xr_semantic_plan_type(semantic, child_type);
    ...
    if (next_field != field_end)
        return report(error, error_size, "XR_TARGET_1002",
                      "layout field ranges do not exactly partition their module table");
    return true;
}

static bool verify_aggregate_layouts_acyclic(const XrTargetPlan *plan, char *error,
                                             size_t error_size) {
    uint8_t *states =
...
static bool verifier_array_member_reference_contract_is_exact(
    ...
    const XrSemanticTypeRecord *element_type) {
    return xr_semantic_array_member_reference_contract_is_exact(semantic, shape, operation,
...
static bool operation_is_exact_native_module_scalar_call(const XrSemanticPlan *semantic,
    ...
    uint32_t arity = 0;
    bool exact_namespace =
        operation && operation->intrinsic_kind == XR_SEM_INTRINSIC_NATIVE_MODULE_SCALAR_CALL &&
        xr_semantic_native_module_scalar_call_shape_is_exact(semantic, operation, &selector,
                                                             &receiver_value, &arity);
    if (exact_namespace) {
        const char *module_path =
            xr_semantic_native_module_namespace_path(semantic, receiver_value);
        exact_namespace = module_path && xr_stdlib_metadata_exact_native_direct_member(
                                             module_path, selector, (uint16_t) arity) != NULL;
    }
    const XrStdlibDefEntry *direct = NULL;
    bool exact_direct =
        operation && operation->intrinsic_kind == XR_SEM_INTRINSIC_NATIVE_MODULE_SCALAR_CALL &&
        xr_semantic_native_direct_scalar_call_shape_is_exact(semantic, operation, &direct);
    if (!exact_namespace && !exact_direct)
        return false;
    if (exact_direct)
        arity = direct->argc;
    if (argument_count)
...
verifier_native_module_namespace_value_is_exact(const XrSemanticPlan *semantic,
    ...
    if (operation->opcode == XI_IMPORT_REF) {
        if (!xr_semantic_native_module_import_is_exact(semantic, operation, NULL))
            return false;
    } else if (operation->opcode == XI_GET_SHARED) {
        if (!xr_semantic_native_module_load_is_exact(semantic, operation) ||
            !xr_semantic_native_module_namespace_path(semantic, operation->result_value))
            return false;
    } else {
        return false;
    }
...
static bool overflow_predicate_covers_call_operation(const XrTargetPlan *plan,
    ...
    if (!program || program->program_family != XR_PROGRAM_SEMANTIC_FAMILY_I64_OVERFLOW_PREDICATE ||
        !binding || binding->program_row >= view->range.i64_overflow_predicates_count)
        return false;
    uint32_t global_row = view->range.i64_overflow_predicates_begin + binding->program_row;
    const XrTargetI64OverflowPredicateRecord *row = &plan->i64_overflow_predicates[global_row];
    return row->id == global_row && row->program_row == binding->program_row &&
...
static bool verify_calls_partition(const XrTargetPlan *plan, const XrTargetPartitionView *view,
    ...
    const XrSemanticPlan *semantic = view ? view->semantic : NULL;
    if (!plan || !semantic)
        return report(error, error_size, "XR_TARGET_1003", "call partition has no semantic owner");
    const XrSemanticPlan *const *dependencies =
        (const XrSemanticPlan *const *) plan->semantic_dependencies;
    const XrSemanticPlan **owned_dependencies = NULL;
    uint32_t dependency_count = plan->semantic_dependency_count;
    if (plan->module_partitions_count &&
        !xr_target_semantic_program_module_direct_dependencies(
            ...
            view->range.semantic_module, &owned_dependencies, &dependency_count, error, error_size))
        return false;
    if (owned_dependencies)
        dependencies = (const XrSemanticPlan *const *) owned_dependencies;
    ...
    for (uint32_t operation_index = 0; valid && operation_index < semantic_operations;
        ...
        if (xr_semantic_runtime_constructor_is_exact(semantic, operation)) {
            if (expected_calls == UINT32_MAX) {
                valid = false;
                break;
            }
        ...
        if (xr_semantic_builtin_runtime_method_is_exact(semantic, operation, NULL, NULL)) {
            if (expected_calls == UINT32_MAX) {
                valid = false;
                break;
            }
        ...
        if (xr_semantic_string_utf8_static_call_is_exact(semantic, operation, NULL, NULL)) {
            if (expected_calls == UINT32_MAX) {
                valid = false;
                break;
            }
    ...
    valid = valid && view->range.calls_count == expected_calls;
    ...
    uint32_t next_argument = view->range.call_arguments_begin;
    uint32_t next_adapter = view->range.adapters_begin;
    ...
    uint32_t call_end = view->range.calls_begin + view->range.calls_count;
    uint32_t argument_end = view->range.call_arguments_begin + view->range.call_arguments_count;
    uint32_t adapter_end = view->range.adapters_begin + view->range.adapters_count;
    const XrTargetCallRecord *checked_call = NULL;
    for (uint32_t i = view->range.calls_begin; valid && i < call_end; i++) {
        ...
        const XrSemanticPlan *dependency =
            (source || imported_class_construction) && target->dependency < dependency_count
        ...
        bool runtime_constructor =
            !semantic_target && xr_semantic_runtime_constructor_is_exact(semantic, operation);
        ...
        const XaBuiltinReceiverMethodSpec *builtin_runtime_spec = NULL;
        uint32_t builtin_runtime_receiver = XR_SEMANTIC_INDEX_NONE;
        ...
        bool string_utf8_static = !semantic_target && xr_semantic_string_utf8_static_call_is_exact(
                                                          semantic, operation, &string_utf8_method,
                                                          &string_utf8_argument);
        ...
        const XrTargetValueRepRecord *result =
            operation ? verifier_value_rep(plan, view, operation->result_value) : NULL;
        int result_scalar =
            result_type ? semantic_type_expected_rep(result_type, &result_kind) : -1;
        bool direct_nullable_scalar_result =
            direct && operation && callee &&
            xr_semantic_local_call_result_opcode_is_exact(operation) &&
            operation->result_value != XR_SEMANTIC_INDEX_NONE &&
            operation->result_type == callee->return_type &&
            operation->result_alias_operand == -1 && operation->return_parameter == -1 &&
            callee->return_parameter == -1 && semantic_nullable_scalar_type_is_exact(result_type);
        bool direct_string_result = direct && semantic_direct_local_string_result_is_exact(
                                                  semantic, call->semantic_operation);
        bool direct_nullable_string_result =
            direct && semantic_direct_local_nullable_string_result_is_exact(
                          semantic, call->semantic_operation);
        bool direct_array_result = direct && semantic_direct_local_reference_result_is_exact_verify(
                                                 semantic, call->semantic_operation);
        ...
        if (runtime_constructor || stringbuilder_to_string || stringbuilder_append_string ||
        ...
        if (string_byte_slice_view ||
            (direct && xr_semantic_direct_local_slice_result_is_exact(semantic, operation, NULL))) {
            result_scalar = 1;
        ...
        valid =
            ...
            slot_binds_value_in_function(plan, result,
                                         view->range.functions_begin + operation->function) &&
            operation->operand_begin <= operand_count &&
            operation->operand_count <= operand_count - operation->operand_begin && call->id == i &&
            call->calling_convention > XR_TARGET_CALL_CONVENTION_INVALID &&
            call->calling_convention < XR_TARGET_CALL_CONVENTION_COUNT &&
            call->target_kind > XR_TARGET_CALL_TARGET_INVALID &&
            call->target_kind < XR_TARGET_CALL_TARGET_COUNT &&
            call->caller_function == view->range.functions_begin + operation->function &&
            ...
            call->argument_begin >= view->range.call_arguments_begin &&
            range_valid(call->argument_begin, call->argument_count, argument_end) &&
        ...
        if (direct) {
            valid =
                ...
                call->callee_function == view->range.functions_begin + target->function &&
                ...
                (!(direct_string_result || direct_nullable_string_result || direct_array_result ||
            ...
            for (uint32_t ordinal = 0; valid && ordinal < call->argument_count; ordinal++) {
                ...
                bool argument_ref_place = semantic_direct_local_ref_place_is_exact_verify(
                    semantic, operand, operation->function, &caller_storage_value);
                const XrTargetValueRepRecord *caller_value =
                    verifier_value_rep(plan, view, caller_storage_value);
                const XrTargetValueRepRecord *callee_value =
                    parameter ? verifier_value_rep(plan, view, parameter->value) : NULL;
                ...
                bool argument_scalar_ref =
                    ...
                    semantic_direct_local_scalar_ref_parameter_is_exact_verify(semantic, parameter,
                                                                               NULL) &&
                    operand->parameter_mode == XR_PARAM_REF && operand->access == XR_CALL_ARG_REF &&
                    operand->origin != XI_PLACE_ORIGIN_NONE &&
                    operand->lifetime == XI_PLACE_LIFETIME_CALL_BOUND &&
                ...
                bool const_read_boundary =
                    ...
                    xr_target_const_read_call_boundary(
                        plan->machine_reps, plan->machine_reps_count, caller_value, callee_value,
                        xr_semantic_type_is_const_read_admission(
                            xr_semantic_plan_type(semantic, operand->type),
                            xr_semantic_plan_type(semantic, parameter->type), parameter->mode));
                ...
                bool argument_pointer_weakens =
                    ...
                    xr_semantic_raw_pointer_argument_satisfies_parameter(
                        xr_semantic_plan_type(semantic, operand->type),
                        xr_semantic_plan_type(semantic, parameter->type));
                bool argument_type_admitted =
                    parameter &&
                    xr_semantic_parameter_type_admits_argument(
                        semantic, xr_semantic_plan_type(semantic, parameter->type),
                        xr_semantic_plan_type(semantic, operand->type), parameter->mode);
                ...
                valid =
                    ...
                    (argument_type_admitted || argument_pointer_weakens) &&
                    ...
                    slot_binds_value_in_function(plan, callee_value,
                                                 view->range.functions_begin + target->function) &&
            ...
            for (uint32_t ordinal = 0; valid && ordinal < call->argument_count; ordinal++) {
                ...
                const XrSemanticTypeRecord *operand_type =
                    xr_semantic_plan_type(semantic, operand->type);
                const XrTargetValueRepRecord *caller_value =
                    verifier_value_rep(plan, view, operand->value);
            ...
            for (uint16_t ordinal = 0; valid && ordinal < call->argument_count; ordinal++) {
                ...
                bool reference = operand && operand->parameter_mode == XR_PARAM_REF;
                uint16_t callee_register =
                    reference && caller_value ? caller_value->register_rep : (uint16_t) tagged_rep;
                uint16_t callee_memory =
                    reference && caller_value ? caller_value->memory_rep : (uint16_t) tagged_rep;
                ...
                valid =
                    ...
                    argument->callee_register_rep == callee_register &&
                    argument->callee_memory_rep == callee_memory && argument->ordinal == ordinal &&
                    argument->mode ==
                        (reference ? XR_TARGET_CALL_REFERENCE : XR_TARGET_CALL_VALUE) &&
                    argument->ownership ==
                        (reference ? XR_TARGET_CALL_BORROW : XR_TARGET_CALL_READ) &&
                    (!reference ||
                     (callee_register < plan->machine_reps_count &&
                      callee_memory < plan->machine_reps_count &&
                      plan->machine_reps[callee_register].kind == XR_MACHINE_REP_RAW_PTR &&
                      plan->machine_reps[callee_memory].kind == XR_MACHINE_REP_RAW_PTR)) &&
                    argument->transfer_mode == XR_TRANSFER_SHARE &&
                    argument->flags == (reference ? XR_TARGET_CALL_ARGUMENT_ADDRESSABLE : 0) &&
            ...
            for (uint32_t ordinal = 0; valid && ordinal < call->argument_count; ordinal++) {
                ...
                const XrTargetValueRepRecord *callee_value =
                    !imported_class_construction && parameter
                        ? verifier_value_rep(plan, view, parameter->value)
                        : NULL;
                const XrSemanticTypeRecord *operand_type =
                    xr_semantic_plan_type(semantic, operand->type);
                const XrSemanticTypeRecord *parameter_type =
        ...
        } else if (runtime_constructor) {
            valid = runtime_constructor && !suspends &&
                    reconstruct_call_identity("xray-target-runtime-constructor-v1", operation->id,
                    ...
                    call->target_kind == XR_TARGET_CALL_TARGET_RUNTIME_CONSTRUCTOR && result &&
                    result->slot < plan->slots_count &&
                    plan->slots[result->slot].root_kind == XR_TARGET_ROOT_DYNAMIC &&
                    plan->slots[result->slot].ownership == XR_TARGET_OWNERSHIP_OWNED;
            if (!valid) {
            ...
            valid = result_type && !suspends &&
                    reconstruct_call_identity("xray-target-builtin-runtime-method-v1",
                                              ...
                                              builtin_runtime_receiver, &expected_identity) &&
                    xr_stable_id_equal(call->identity, expected_identity) &&
                    xr_stable_id_equal(call->native_callee_identity, builtin_runtime_identity) &&
                    call->semantic_call_target == XR_SEMANTIC_INDEX_NONE &&
                    call->callee_function == XR_SEMANTIC_INDEX_NONE &&
                    call->source_dependency == XR_SEMANTIC_INDEX_NONE &&
                    call->source_export == XR_SEMANTIC_INDEX_NONE &&
            ...
            valid =
                ...
                slot_binds_value_in_function(plan, caller_value,
                                             view->range.functions_begin + operation->function) &&
        ...
        } else if (string_utf8_static) {
            valid = result_type && !suspends &&
                    reconstruct_call_identity("xray-target-string-utf8-static-v1", operation->id,
                                              result_type->id, string_utf8_method,
                                              &expected_identity) &&
                    xr_stable_id_equal(call->identity, expected_identity) &&
                    call->semantic_call_target == XR_SEMANTIC_INDEX_NONE &&
                    call->callee_function == XR_SEMANTIC_INDEX_NONE &&
                    ...
                    plan->slots[result->slot].root_kind == XR_TARGET_ROOT_DYNAMIC &&
                    plan->slots[result->slot].ownership == XR_TARGET_OWNERSHIP_OWNED &&
                    string_utf8_argument != XR_SEMANTIC_INDEX_NONE;
            if (!valid) {
                break;
            }
    ...
    valid = valid && next_argument == argument_end && next_adapter == adapter_end;
    xr_free(covered);
    xr_free(state_counts);
    xr_free(suspendable);
    xr_free(reverse_head);
...
static bool verify_product_aggregate_machine_rep(const XrTargetPlan *plan, uint16_t rep_index,
                                                 uint32_t layout_index) {
    if (rep_index >= plan->machine_reps_count)
        return false;
    const XrTargetMachineRepRecord *rep = &plan->machine_reps[rep_index];
...
static bool verify_product_target_layout(const XrTargetPlan *plan,
    ...
    if (product->kind != XR_TARGET_LAYOUT_AGGREGATE || product->fixed_prefix_size != 48 ||
        product->align != 8 || product->field_count != 6 || product->root_field_count != 0 ||
        product->array_element_storage != XR_TARGET_ARRAY_STORAGE_NONE ||
        i64->kind != XR_TARGET_LAYOUT_SCALAR || i64->fixed_prefix_size != 8 || i64->align != 8 ||
        i64->field_count != 0 || u8->kind != XR_TARGET_LAYOUT_SCALAR ||
        u8->fixed_prefix_size != 1 || u8->align != 1 || u8->field_count != 0 ||
        !range_valid(product->field_begin, 6, plan->fields_count))
        return false;
    for (uint32_t ordinal = 0; ordinal < 6; ordinal++) {
        const XrTargetFieldRecord *field = &plan->fields[product->field_begin + ordinal];
        uint32_t size = ordinal == 2 ? 1u : 8u;
        uint32_t align = ordinal == 2 ? 1u : 8u;
        if (field->layout != (uint32_t) product_index || field->semantic_field != ordinal ||
            field->semantic_name != XR_SEMANTIC_INDEX_NONE || field->offset != ordinal * 8u ||
            field->size != size || field->align != align ||
            field->root_kind != XR_TARGET_ROOT_NONE || field->flags != 0 || field->reserved != 0 ||
            (ordinal == 2 ? !verify_product_u8_machine_rep(plan, field->memory_rep)
                          : !verify_leaf_scalar_machine_rep(plan, field->memory_rep)))
            return false;
    }
    uint16_t aggregate_rep = XR_MACHINE_REP_COUNT;
    uint32_t matches = 0;
    for (uint32_t i = 0; i < plan->machine_reps_count; i++)
        if (verify_product_aggregate_machine_rep(plan, (uint16_t) i, (uint32_t) product_index)) {
            aggregate_rep = (uint16_t) i;
            matches++;
        }
    if (matches != 1)
...
static bool verify_product_target_call(const XrTargetPlan *plan,
                                       const XrVerifyProductProgramShape *shape, uint32_t caller,
                                       uint16_t aggregate_rep, uint8_t *covered) {
    const XrSemanticOperationRecord *operation = shape->calls[caller];
    const XrTargetValueRepRecord *value = xr_target_plan_value_rep(plan, operation->result_value);
    ...
    return slot->semantic_value == operation->result_value &&
           ...
           slot->size == 48 && slot->align == 8 && slot->register_rep == aggregate_rep &&
           slot->memory_rep == aggregate_rep && slot->role == XR_TARGET_SLOT_TEMPORARY &&
...
static bool verify_product_program_target(const XrTargetPlan *plan, char *error,
    ...
    for (uint32_t i = 0; valid && i < plan->slots_count; i++) {
        ...
        valid = slot->register_rep == aggregate_rep && slot->memory_rep == aggregate_rep &&
                slot->size == 48 && slot->align == 8 && slot->root_kind == XR_TARGET_ROOT_NONE &&
                slot->ownership == XR_TARGET_OWNERSHIP_TRIVIAL;
...
static bool verify_exact_owned_string_slot(const XrTargetPlan *plan,
                                           const XrTargetPartitionView *view, uint32_t function,
                                           uint32_t semantic_value, uint32_t semantic_operation,
                                           uint32_t *slot_out) {
    const XrTargetValueRepRecord *binding = verifier_value_rep(plan, view, semantic_value);
    const XrTargetSlotRecord *slot =
        binding && binding->slot < plan->slots_count ? &plan->slots[binding->slot] : NULL;
    const XrTargetMachineRepRecord *register_rep =
    ...
    if (!slot || !register_rep || !memory_rep || register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        ...
        slot->function != view->range.functions_begin + function ||
...
static bool verify_roots_and_cleanups_partition(const XrTargetPlan *plan,
                                                const XrTargetPartitionView *view, char *error,
                                                size_t error_size) {
    const XrSemanticPlan *semantic = view ? view->semantic : NULL;
    if (!plan || !semantic)
        return report(error, error_size, "XR_TARGET_1002",
                      "root lifecycle partition has no semantic owner");
    ...
    uint32_t next_root = view->range.root_maps_begin;
    uint32_t next_root_slot = view->range.root_slots_begin;
    uint32_t next_cleanup = view->range.cleanups_begin;
    uint32_t next_projection = 0;
    uint32_t next_release = 0;
    bool valid = true;
    for (uint32_t function = 0; valid && function < view->range.functions_count; function++) {
        const XrSemanticFunctionRecord *semantic_function =
            xr_semantic_plan_function(semantic, function);
        uint32_t target_function_index = view->range.functions_begin + function;
        const XrTargetFunctionRecord *target_function = &plan->functions[target_function_index];
        ...
        for (uint32_t block_offset = 0; valid && block_offset < semantic_function->block_count;
            ...
            for (uint32_t operation = block ? block->operation_begin : 0;
                ...
                while (next_projection < projection_count &&
                    ...
                    valid =
                        verify_exact_owned_string_slot(plan, view, function, record->producer_value,
                ...
                if (root_count) {
                    ...
                    if (next_root >= view->range.root_maps_begin + view->range.root_maps_count)
                        valid = false;
                    const XrTargetRootMapRecord *root = valid ? &plan->root_maps[next_root] : NULL;
                    valid =
                        valid && root->id == next_root && root->function == target_function_index &&
                        ...
                        root->flags == (XR_TARGET_ROOT_SUSPEND | XR_TARGET_ROOT_CANCEL |
                                        XR_TARGET_ROOT_EXIT) &&
                        root->slot_begin >= view->range.root_slots_begin &&
                        range_valid(root->slot_begin, root->slot_count,
                                    view->range.root_slots_begin + view->range.root_slots_count);
                ...
                if (drop_count) {
                    ...
                    for (uint32_t i = 0; valid && i < drop_count; i++) {
                        if (next_cleanup >= view->range.cleanups_begin + view->range.cleanups_count)
                            valid = false;
                        ...
                        valid =
                            ...
                            cleanup->function == target_function_index &&
                ...
                uint32_t slot = XR_SEMANTIC_INDEX_NONE;
                valid = verify_exact_owned_string_slot(plan, view, function, release.released_value,
                                                       release.producer_operation, &slot);
                if (!valid ||
                    next_cleanup >= view->range.cleanups_begin + view->range.cleanups_count) {
                    valid = false;
                    break;
                }
                const XrTargetCleanupRecord *cleanup = &plan->cleanups[next_cleanup];
                valid = cleanup->id == next_cleanup && cleanup->function == target_function_index &&
    ...
    valid = valid && next_root == view->range.root_maps_begin + view->range.root_maps_count &&
            next_root_slot == view->range.root_slots_begin + view->range.root_slots_count &&
            next_cleanup == view->range.cleanups_begin + view->range.cleanups_count &&
...
static bool verify_roots_and_cleanups(const XrTargetPlan *plan, char *error, size_t error_size) {
    for (uint32_t partition = 0; partition < verifier_partition_count(plan); partition++) {
        XrTargetPartitionView view;
        if (!verifier_partition_view(plan, partition, &view) ||
            !verify_roots_and_cleanups_partition(plan, &view, error, error_size))
            return false;
    }
    return true;
}

static bool accumulate_semantic_capability_requirements(const XrSemanticPlan *semantic,
                                                        const XrTargetProfileDraft *facts,
                                                        uint64_t *expected_mask, char *error,
                                                        size_t error_size) {
    ...
    for (uint32_t i = 0; i < xr_semantic_plan_operation_count(semantic); i++) {
        ...
        if (facts && facts->machine.runtime_profile == XR_TARGET_RUNTIME_PROFILE_FREESTANDING &&
            ...
            const XrSemanticTypeRecord *right =
                xr_semantic_plan_type(semantic, operands[operation->operand_begin + 1u].type);
        ...
        if ((assertion.required_capabilities & XR_ASSERTION_CAPABILITY_TYPED_ERROR_BOUNDARY) != 0)
            *expected_mask |= xr_target_capability_mask(XR_TARGET_CAPABILITY_TYPED_ERROR_BOUNDARY);
...
static bool verify_program_graph_machine_rep_set(const XrTargetPlan *plan, char *error,
                                                 size_t error_size) {
...
bool xr_target_semantic_capability_requirements(const XrSemanticPlan *const *modules,
    ...
    if (!modules || !module_count || module_count > XR_TARGET_MAX_PROGRAM_MODULES || !facts ||
        !expected_mask)
        return report(error, error_size, "XR_TARGET_1004",
    ...
    for (uint32_t module = 0; module < module_count; module++)
        if (!accumulate_semantic_capability_requirements(modules[module], facts, &mask, error,
                                                         error_size))
            return false;
...
static bool verify_adapters_and_capabilities(const XrTargetPlan *plan, char *error,
    ...
    if (!xr_target_semantic_capability_requirements(modules, module_count, plan->profile,
                                                    &expected_mask, error, error_size))
        return false;
...
static bool verify_coroutines_partition(const XrTargetPlan *plan, const XrTargetPartitionView *view,
                                        char *error, size_t error_size) {
    const XrSemanticPlan *semantic = view ? view->semantic : NULL;
    if (!plan || !semantic)
        return report(error, error_size, "XR_CORO_4000",
                      "coroutine partition has no semantic owner");
    uint32_t function_count = (uint32_t) xr_semantic_plan_function_count(semantic);
    uint32_t operation_count = (uint32_t) xr_semantic_plan_operation_count(semantic);
    uint32_t entity_count = (uint32_t) xr_semantic_plan_entity_count(semantic);
    uint32_t block_count = (uint32_t) xr_semantic_plan_block_count(semantic);
    ...
    uint32_t state_end = view->range.coroutines_begin + view->range.coroutines_count;
    uint32_t call_end = view->range.calls_begin + view->range.calls_count;
    uint32_t instruction_end = view->range.instructions_begin + view->range.instructions_count;
    for (uint32_t state = view->range.coroutines_begin; state < state_end; state++)
        resume_instruction_by_state[state] = XR_SEMANTIC_INDEX_NONE;
    bool valid = view->range.functions_count == function_count;
    for (uint32_t instruction = view->range.instructions_begin;
         valid && instruction < instruction_end; instruction++) {
        ...
        if (state < view->range.coroutines_begin || state >= state_end ||
            resume_instruction_by_state[state] != XR_SEMANTIC_INDEX_NONE) {
            valid = false;
            break;
        }
    ...
    for (uint32_t call = view->range.calls_begin; valid && call < call_end; call++) {
    ...
    valid = valid && expected_states == view->range.coroutines_count;
    uint32_t next_state = view->range.coroutines_begin;
    for (uint32_t function = 0; valid && function < function_count; function++) {
        const XrTargetFunctionRecord *record =
            &plan->functions[view->range.functions_begin + function];
        valid = record->coroutine_begin == next_state &&
                record->coroutine_count == function_states[function] &&
                record->coroutine_begin >= view->range.coroutines_begin &&
                range_valid(record->coroutine_begin, record->coroutine_count, state_end);
        next_state += record->coroutine_count;
    }
    valid = valid && next_state == state_end;
    for (uint32_t state_index = view->range.coroutines_begin; valid && state_index < state_end;
         state_index++) {
        ...
        if (!entity || !operation || entity->kind != XR_SEM_ENTITY_COROUTINE_STATE ||
            ...
            state->function != view->range.functions_begin + operation->function ||
            ...
            state->function < view->range.functions_begin ||
            state->function - view->range.functions_begin >= function_count ||
            entity->ordinal == 0 ||
            entity->ordinal > function_states[state->function - view->range.functions_begin] ||
            state_index !=
        ...
        const XrTargetValueRepRecord *result =
            verifier_value_rep(plan, view, operation->result_value);
        if (result) {
            ...
            if (plan->machine_reps[result->memory_rep].kind != XR_MACHINE_REP_VOID) {
                if (!slot_binds_value_in_function(
                        plan, result, view->range.functions_begin + operation->function)) {
            valid = false;
                    break;
                }
...
static bool verify_coroutines(const XrTargetPlan *plan, char *error, size_t error_size) {
    for (uint32_t partition = 0; partition < verifier_partition_count(plan); partition++) {
        XrTargetPartitionView view;
        if (!verifier_partition_view(plan, partition, &view) ||
            !verify_coroutines_partition(plan, &view, error, error_size))
            return false;
    }
    return true;
}
...
static bool verify_entry_expectations_partition(const XrTargetPlan *plan,
                                                const XrTargetPartitionView *view, char *error,
                                                size_t error_size) {
    if (!plan || !view || !view->semantic)
        return report(error, error_size, "XR_TARGET_1005",
                      "entry expectation partition has no semantic owner");
    const XrTargetMachineFacts *machine = xr_target_profile_machine_facts(plan->profile);
    uint8_t *seen = view->range.entry_expectations_count
                        ? (uint8_t *) xr_calloc(view->range.entry_expectations_count, 1)
                        : NULL;
    if (view->range.entry_expectations_count && !seen)
        return report(error, error_size, "XR_EXEC_5003",
                      "entry expectation verifier budget exhausted");
    bool valid = machine != NULL;
    uint32_t expectation_end =
        view->range.entry_expectations_begin + view->range.entry_expectations_count;
    uint32_t call_end = view->range.calls_begin + view->range.calls_count;
    uint32_t instruction_end = view->range.instructions_begin + view->range.instructions_count;
    for (uint32_t i = view->range.entry_expectations_begin; valid && i < expectation_end; i++) {
        const XrTargetEntryExpectationRecord *record = &plan->entry_expectations[i];
        const XrTargetCallRecord *call =
            record->call >= view->range.calls_begin && record->call < call_end
                ? &plan->calls[record->call]
                : NULL;
        const XrSemanticPlan *dependency =
            call ? verifier_semantic_dependency(plan, view->semantic, call->source_dependency)
                 : NULL;
        ...
        valid = call && dependency && export_record &&
                ...
                reconstruct_call_identity("xray-target-entry-expectation-v1", call->identity,
                                          ...
                                          i - view->range.entry_expectations_begin,
                                          &expected_identity) &&
    ...
    for (uint32_t i = view->range.instructions_begin; valid && i < instruction_end; i++) {
        ...
        if (instruction->immediate_bits > UINT32_MAX ||
            expectation < view->range.entry_expectations_begin || expectation >= expectation_end ||
            seen[expectation - view->range.entry_expectations_begin]) {
            valid = false;
            break;
        }
        const XrTargetEntryExpectationRecord *record = &plan->entry_expectations[expectation];
        valid = record->call >= view->range.calls_begin && record->call < call_end &&
                plan->calls[record->call].caller_function == instruction->function &&
                plan->calls[record->call].result_slot == instruction->result_slot;
        seen[expectation - view->range.entry_expectations_begin] = 1;
    }
    for (uint32_t i = 0; valid && i < view->range.entry_expectations_count; i++) {
        if (seen[i])
            continue;
        const XrTargetEntryExpectationRecord *record =
            &plan->entry_expectations[view->range.entry_expectations_begin + i];
        const XrTargetCallRecord *call =
            record->call >= view->range.calls_begin && record->call < call_end
                ? &plan->calls[record->call]
                : NULL;
        bool caller_executable = false;
        for (uint32_t instruction = view->range.instructions_begin;
             call && instruction < instruction_end; instruction++)
            caller_executable |= plan->instructions[instruction].function == call->caller_function;
        /* A persistent source-entry expectation also serves AOT emission for a
         * function outside the bounded typed instruction subset. Once that
         * function has any executable rows, however, the expectation must be
         * consumed by exactly one CALL_ENTRY row. */
        valid = call && !caller_executable;
    }
    xr_free(seen);
...
static bool verify_entry_expectations(const XrTargetPlan *plan, char *error, size_t error_size) {
    for (uint32_t partition = 0; partition < verifier_partition_count(plan); partition++) {
        XrTargetPartitionView view;
        if (!verifier_partition_view(plan, partition, &view) ||
            !verify_entry_expectations_partition(plan, &view, error, error_size))
            return false;
    }
    return true;
}
...
static bool verify_debug_facts(const XrTargetPlan *plan, char *error, size_t error_size) {
    ...
    for (uint32_t i = 0; i < plan->debug_facts_count; i++) {
        ...
        if (!semantic_operation || operation != fact->semantic_operation ||
            instruction->function != view.range.functions_begin + semantic_operation->function ||
        ...
        if (ownership && semantic_operation->result_value != XR_SEMANTIC_INDEX_NONE) {
            for (uint32_t owner = 0; owner < xr_ownership_certificate_owner_count(ownership);
                ...
                if (record->function != semantic_operation->function ||
...
#define XR_GRAPH_VERIFY_RANGES(name)                                                               \
    ...
    XR_GRAPH_VERIFY_RANGES(i64_overflow_predicates);
...
static bool verify_program_graph_debug_partition(const XrTargetPlan *plan,
                                                 const XrTargetModulePartitionRecord *partition,
                                                 const XrSemanticPlan *semantic, char *error,
                                                 size_t error_size) {
    ...
    for (uint32_t local = 0; local < partition->debug_facts_count; local++) {
        ...
        const XrTargetFunctionRecord *target_function =
            instruction->function < plan->functions_count ? &plan->functions[instruction->function]
                                                          : NULL;
        uint32_t operation = debug_operation_for_instruction(plan, instruction);
...
static bool graph_semantic_value_binding(const XrSemanticPlan *semantic, uint32_t value,
    ...
    for (uint32_t row = 0; row < xr_semantic_plan_function_count(semantic); row++) {
        const XrSemanticFunctionRecord *candidate = xr_semantic_plan_function(semantic, row);
    ...
    for (uint32_t row = 0; row < xr_semantic_plan_parameter_count(semantic); row++) {
        const XrSemanticParameterRecord *parameter = xr_semantic_plan_parameter(semantic, row);
    ...
    for (uint32_t row = 0; row < xr_semantic_plan_operation_count(semantic); row++) {
        const XrSemanticOperationRecord *operation = xr_semantic_plan_operation(semantic, row);
...
static bool graph_semantic_result_ownership_is_exact(const XrSemanticPlan *semantic,
                                                     const XrSemanticOperationRecord *operation) {
...
static bool graph_semantic_value_storage_kind(const XrSemanticPlan *semantic, uint32_t value,
    ...
    if (!graph_semantic_value_binding(semantic, value, &semantic_function, &semantic_type_index,
                                      &operation_index, &is_parameter))
        return false;
    const XrSemanticTypeRecord *semantic_type =
        xr_semantic_plan_type(semantic, semantic_type_index);
    uint16_t scalar_kind = XR_MACHINE_REP_COUNT;
    int scalar = semantic_type ? semantic_type_expected_rep(semantic_type, &scalar_kind) : -1;
    const XrSemanticOperationRecord *definition =
    ...
    bool result_void =
        definition && xr_semantic_operation_result_void_governs_storage(
                          semantic, definition, value, semantic_type_index, semantic_function);
    bool generated_result_void =
        definition && definition->opcode < XI_OP_COUNT &&
...
graph_value_rep_for_semantic_value(const XrTargetPlan *plan,
                                   const XrTargetModulePartitionRecord *partition,
                                   uint32_t semantic_value) {
...
static bool verify_program_graph_instruction_semantics(
    ...
    for (uint32_t local = 0; local < partition->instructions_count; local++) {
        ...
        const XrTargetFunctionRecord *target_function =
            instruction->function < plan->functions_count ? &plan->functions[instruction->function]
                                                          : NULL;
        const XrTargetSlotRecord *result = instruction->result_slot < plan->slots_count
                                               ? &plan->slots[instruction->result_slot]
                                               : NULL;
        const XrTargetSlotRecord *left = instruction->operand_slots[0] < plan->slots_count
                                             ? &plan->slots[instruction->operand_slots[0]]
                                             : NULL;
        const XrTargetSlotRecord *right = instruction->operand_slots[1] < plan->slots_count
                                              ? &plan->slots[instruction->operand_slots[1]]
                                              : NULL;
        if (!target_function || !graph_function_in_partition(partition, instruction->function))
            return report(error, error_size, "XR_TARGET_1003",
        ...
        if (instruction->opcode == XR_TARGET_INSTRUCTION_CONST_I64) {
            const XrSemanticOperationRecord *operation =
                result ? xr_semantic_plan_operation(semantic, result->semantic_operation) : NULL;
        ...
        if (instruction->opcode == XR_TARGET_INSTRUCTION_RETURN_I64) {
            ...
            for (uint32_t block = 0; block < xr_semantic_plan_block_count(semantic); block++) {
                const XrSemanticBlockRecord *candidate = xr_semantic_plan_block(semantic, block);
...
static bool verify_program_graph_instruction_coverage(
    ...
    for (uint32_t parameter_index = 0; parameter_index < xr_semantic_plan_parameter_count(semantic);
         parameter_index++) {
        const XrSemanticParameterRecord *parameter =
            xr_semantic_plan_parameter(semantic, parameter_index);
        uint16_t kind = XR_MACHINE_REP_COUNT;
        if (!parameter || !graph_semantic_value_storage_kind(semantic, parameter->value, NULL, NULL,
                                                             NULL, NULL, &kind))
            return report(error, error_size, "XR_TARGET_1003",
        ...
        for (uint32_t local = 0; local < partition->instructions_count; local++) {
            const XrTargetInstructionRecord *instruction =
                &plan->instructions[partition->instructions_begin + local];
            const XrTargetFunctionRecord *function = instruction->function < plan->functions_count
                                                         ? &plan->functions[instruction->function]
                                                         : NULL;
            const XrTargetSlotRecord *result = instruction->result_slot < plan->slots_count
                                                   ? &plan->slots[instruction->result_slot]
                                                   : NULL;
            matches += instruction->opcode == XR_TARGET_INSTRUCTION_PARAM_I64 && function &&
                       function->semantic_function == parameter->function && result &&
                       result->semantic_value == parameter->value &&
    ...
    for (uint32_t operation_index = 0; operation_index < xr_semantic_plan_operation_count(semantic);
         operation_index++) {
        const XrSemanticOperationRecord *operation =
            xr_semantic_plan_operation(semantic, operation_index);
        ...
        if (operation && operation->opcode == XI_CONST)
            ...
            continue;
        uint16_t kind = XR_MACHINE_REP_COUNT;
        if (!graph_semantic_value_storage_kind(semantic, operation->result_value, NULL, NULL, NULL,
                                               NULL, &kind) ||
    ...
    for (uint32_t block_index = 0; block_index < xr_semantic_plan_block_count(semantic);
         block_index++) {
        const XrSemanticBlockRecord *block = xr_semantic_plan_block(semantic, block_index);
        uint16_t kind = XR_MACHINE_REP_COUNT;
        if (!block || block->control_value == XR_SEMANTIC_INDEX_NONE)
            continue;
        if (!graph_semantic_value_storage_kind(semantic, block->control_value, NULL, NULL, NULL,
                                               NULL, &kind) ||
            kind != XR_MACHINE_REP_I64)
            return report(error, error_size, "XR_TARGET_1003",
        ...
        for (uint32_t local = 0; local < partition->instructions_count; local++) {
            ...
            const XrTargetFunctionRecord *function = instruction->function < plan->functions_count
                                                         ? &plan->functions[instruction->function]
                                                         : NULL;
            const XrTargetSlotRecord *operand = instruction->operand_slots[0] < plan->slots_count
                                                    ? &plan->slots[instruction->operand_slots[0]]
                                                    : NULL;
            matches += instruction->opcode == XR_TARGET_INSTRUCTION_RETURN_I64 && function &&
                       function->semantic_function == block->function && operand &&
                       operand->semantic_value == block->control_value;
        }
        if (matches != 1u)
            return report(error, error_size, "XR_TARGET_1003",
...
static bool verify_program_graph_partitions(const XrTargetPlan *plan, uint32_t *entry_partition,
    ...
    *producer_partition = UINT32_MAX;
    ...
    for (uint32_t i = 0; i < 2u; i++) {
        ...
        const XrSemanticProgramProvenance *program =
            semantic ? xr_semantic_plan_program_provenance(semantic) : NULL;
        if (!program || partition->program_module_row != i || program->program_module_row != i ||
...
static bool verify_program_graph_rows(const XrTargetPlan *plan, char *error, size_t error_size) {
    for (uint32_t partition_index = 0; partition_index < 2u; partition_index++) {
        const XrTargetModulePartitionRecord *partition = &plan->module_partitions[partition_index];
        const XrSemanticPlan *semantic = graph_partition_semantic(plan, partition_index);
        uint32_t semantic_value_count = 0;
        if (!semantic || partition->functions_count != xr_semantic_plan_function_count(semantic) ||
            !graph_semantic_value_count(semantic, &semantic_value_count))
            return false;
        for (uint32_t local_extent = 0; local_extent < partition->extents_count; local_extent++) {
            ...
            for (uint32_t local_layout = 0; local_layout < partition->layouts_count; local_layout++)
                references +=
        ...
        for (uint32_t i = 0; i < partition->value_reps_count; i++) {
            const XrTargetValueRepRecord *row = &plan->value_reps[partition->value_reps_begin + i];
            uint32_t semantic_function = XR_SEMANTIC_INDEX_NONE;
            uint32_t semantic_type_index = XR_SEMANTIC_INDEX_NONE;
            ...
            bool value_exact = graph_semantic_value_storage_kind(
                semantic, row->semantic_value, &semantic_function, &semantic_type_index,
                &defining_operation, &parameter_value, &expected_kind);
            const XrTargetSlotRecord *bound_slot = NULL;
            uint32_t bound_slot_count = 0u;
            for (uint32_t slot = 0; slot < partition->slots_count; slot++) {
                const XrTargetSlotRecord *candidate = &plan->slots[partition->slots_begin + slot];
            ...
            uint32_t local_function = bound_slot && bound_slot->function < plan->functions_count
            ...
            bool slot_exact =
                expected_kind == XR_MACHINE_REP_VOID
                    ...
                    : bound_slot_count == 1u && row->slot == bound_slot->id &&
                          ...
                          bound_slot->role == (parameter_value ? XR_TARGET_SLOT_PARAMETER
                                                               : XR_TARGET_SLOT_TEMPORARY) &&
                          bound_slot->semantic_operation ==
                              (parameter_value ? XR_SEMANTIC_INDEX_NONE : defining_operation) &&
                          reconstruct_value_slot_identity_for_semantic(
                              semantic, bound_slot, bound_slot->function, row->semantic_value,
                              semantic_function, &expected_slot_identity) &&
                          xr_stable_id_equal(bound_slot->identity, expected_slot_identity) &&
        ...
        for (uint32_t value = 0; value < semantic_value_count; value++) {
            ...
            uint16_t machine_kind = XR_MACHINE_REP_COUNT;
            if (!graph_semantic_value_storage_kind(semantic, value, &semantic_function,
                                                   &semantic_type, NULL, NULL, &machine_kind))
                continue;
            ...
            for (uint32_t prior = 0; prior < value; prior++) {
                ...
                if (graph_semantic_value_storage_kind(semantic, prior, &prior_function, &prior_type,
                                                      NULL, NULL, &prior_kind) &&
            ...
            for (uint32_t layout = 0; layout < partition->layouts_count; layout++)
                matching_layouts +=
                    plan->layouts[partition->layouts_begin + layout].semantic_type == semantic_type;
            if (matching_layouts != 1u)
                return report(error, error_size, "XR_TARGET_1002",
                              "program graph required layout row is missing or ambiguous");
        }
        bool derived_row_counts_exact = partition->value_reps_count == required_value_count &&
        ...
        for (uint32_t i = 0; i < partition->layouts_count; i++) {
            ...
            uint16_t expected_rep = XR_MACHINE_REP_COUNT;
            int scalar =
                semantic_type ? semantic_type_expected_rep(semantic_type, &expected_rep) : -1;
            bool physical_match = false;
            for (uint32_t rep = 0; rep < plan->machine_reps_count; rep++) {
                uint16_t required_rep =
                    row->kind == XR_TARGET_LAYOUT_DYNAMIC ? XR_MACHINE_REP_DYN_VALUE : expected_rep;
            ...
            if (row->id != index || row->semantic_type >= xr_semantic_plan_type_count(semantic) ||
                ...
                (row->kind != XR_TARGET_LAYOUT_SCALAR && row->kind != XR_TARGET_LAYOUT_DYNAMIC) ||
                row->array_element_storage != XR_TARGET_ARRAY_STORAGE_NONE ||
                !is_power_of_two(row->align) || row->fixed_prefix_size % row->align != 0u ||
                ...
                !stable_id_is_zero(row->equality_hash) || row->extent < partition->extents_begin ||
                ...
                row->field_count >
                    partition->fields_count - (row->field_begin - partition->fields_begin) ||
        ...
        for (uint32_t i = 0; i < partition->functions_count; i++) {
            ...
            if (row->id != index || row->semantic_function != i || row->slot_begin != next_slot ||
                row->slot_begin < partition->slots_begin ||
                row->slot_begin - partition->slots_begin > partition->slots_count ||
                row->slot_count >
                    partition->slots_count - (row->slot_begin - partition->slots_begin) ||
                row->root_begin != 0u || row->root_count != 0u || row->cleanup_begin != 0u ||
                row->cleanup_count != 0u || row->coroutine_begin != 0u ||
                row->coroutine_count != 0u || row->reserved != 0u)
                return report(error, error_size, "XR_TARGET_1002",
            ...
            for (uint32_t local_slot = 0; local_slot < row->slot_count; local_slot++) {
                ...
                if (slot->id != slot_index || slot->function != index || value_bindings != 1u ||
                    ...
                    (local_slot && xr_stable_id_compare(plan->slots[slot_index - 1u].identity,
                                                        slot->identity) >= 0))
                    return report(error, error_size, "XR_TARGET_1002",
            ...
            if (!xr_checked_align_u32(previous_end, expected_frame_align, &expected_frame_size) ||
                row->frame_align != expected_frame_align || row->frame_size != expected_frame_size)
                return report(error, error_size, "XR_TARGET_1002",
        ...
        if (!verify_program_graph_debug_partition(plan, partition, semantic, error, error_size))
            return false;
        if (!verify_program_graph_instruction_semantics(plan, partition, semantic, error,
                                                        error_size))
            return false;
        if (!verify_program_graph_instruction_coverage(plan, partition, semantic, error,
                                                       error_size))
            return false;
    }
    for (uint32_t i = 0; i < plan->instructions_count;) {
        ...
        if (function >= plan->functions_count ||
            !xr_target_instruction_rows_control_flow_is_exact(
                ...
                plan->call_arguments, plan->call_arguments_count, NULL, 0u, plan->coroutines,
...
static bool verify_program_graph_plan(const XrTargetPlan *plan, char *error, size_t error_size) {
    ...
    if (plan->program_graphs_count != 1u || !plan->program_graphs ||
        ...
        plan->root_slots_count != 0u || plan->cleanups_count != 0u || plan->adapters_count != 0u ||
        plan->coroutines_count != 0u || plan->calls_count != 1u || plan->call_arguments_count != 1u)
        return report(error, error_size, "XR_TARGET_1001",
    ...
    const XrSemanticFunctionRecord *entry_semantic_function =
        entry_function ? xr_semantic_plan_function(entry, entry_function->semantic_function) : NULL;
    ...
    const XrSemanticParameterRecord *parameter =
        producer_semantic_function && producer_semantic_function->parameter_count == 1u
            ? xr_semantic_plan_parameter(producer, producer_semantic_function->parameter_begin)
            : NULL;
    const XrTargetCallArgumentRecord *argument = graph->target_argument < plan->call_arguments_count
    ...
    uint32_t semantic_argument = operation ? operation->operand_begin + 1u : XR_SEMANTIC_INDEX_NONE;
    ...
    const XrTargetValueRepRecord *caller_value =
        semantic_argument_row
            ? graph_value_rep_for_semantic_value(plan, &plan->module_partitions[entry_partition],
                                                 semantic_argument_row->value)
            : NULL;
    const XrTargetValueRepRecord *callee_value =
        parameter ? graph_value_rep_for_semantic_value(
                        plan, &plan->module_partitions[producer_partition], parameter->value)
                  : NULL;
    const XrTargetValueRepRecord *result_value =
        operation ? graph_value_rep_for_semantic_value(
                        plan, &plan->module_partitions[entry_partition], operation->result_value)
                  : NULL;
    ...
    bool call_identity_exact =
        target && operation &&
        reconstruct_call_identity("xray-target-call-v5", target->id, operation->id, 0u,
                                  &expected_call_identity);
    bool argument_identity_exact =
        target && parameter &&
        reconstruct_call_identity("xray-target-source-call-argument-v1", target->id, parameter->id,
                                  0u, &expected_argument_identity);
    ...
    uint8_t expected_argument_ownership =
        reference_parameter ? XR_TARGET_CALL_WRITEBACK
        : semantic_argument_row && semantic_argument_row->ownership_action == XR_SEM_OPERAND_CONSUME
            ? XR_TARGET_CALL_CONSUME
            : XR_TARGET_CALL_READ;
    uint8_t expected_argument_flags =
        reference_parameter ? XR_TARGET_CALL_ARGUMENT_ADDRESSABLE : 0u;
    if (!program || !entry_function || !producer_function || !call_binding || !operation || !call ||
        !target || !source_export || !entry_semantic_function || !producer_semantic_function ||
        !parameter || !argument || graph->schema != XR_TARGET_PROGRAM_GRAPH_SCHEMA_VERSION ||
        ...
        graph->caller_slot != argument->caller_slot ||
        graph->callee_slot != argument->callee_slot ||
        ...
        !xr_fingerprint_equal(graph->target_profile_fingerprint,
                              xr_target_profile_fingerprint(plan->profile)) ||
        !xr_stable_id_equal(graph->entry_function_identity, entry_function->program_function) ||
        ...
        !xr_stable_id_equal(graph->exported_function_identity, source_export->exported_entity) ||
    ...
    if (!entry_target_function || !producer_target_function ||
        ...
        call->target_kind != XR_TARGET_CALL_TARGET_PROGRAM_DIRECT || !call_identity_exact ||
        !argument_identity_exact || !xr_stable_id_equal(call->identity, expected_call_identity) ||
        ...
        plan->machine_reps[call->error_register_rep].kind != XR_MACHINE_REP_VOID ||
        plan->machine_reps[call->error_memory_rep].kind != XR_MACHINE_REP_VOID ||
        call->argument_count != 1u || call->adapter_count != 0u || !profile_facts ||
        call->native_abi != profile_facts->machine.native_abi || call->flags != 0u ||
        call->result_mode != XR_TARGET_CALL_VALUE ||
        call->result_ownership != XR_TARGET_CALL_NONE ||
        call->error_mode != XR_TARGET_CALL_NO_CALL_OWNED_CHANNEL ||
        call->array_intrinsic_kind != XR_TARGET_ARRAY_INTRINSIC_NONE ||
        ...
        call->array_hof_kind != XR_TARGET_ARRAY_HOF_NONE ||
        call->array_result_element_storage != XR_TARGET_ARRAY_STORAGE_NONE ||
        call->reserved8[0] != 0u || call->reserved8[1] != 0u || call->reserved8[2] != 0u ||
        operation->opcode != XI_CALL || operation->operand_count != 2u || !semantic_argument_row ||
        semantic_argument_row->role != XR_SEM_OPERAND_ARGUMENT ||
        semantic_argument_row->parameter != 0 ||
        (semantic_argument_row->flags & XR_SEM_OPERAND_CALL_CONTRACT) == 0u || !parameter ||
        parameter->function != producer_function->semantic_function || parameter->ordinal != 0u ||
        ...
        !xr_semantic_parameter_type_admits_argument(
            ...
            xr_semantic_plan_type(entry, semantic_argument_row->type), parameter->mode) ||
        ...
        argument->semantic_value != semantic_argument_row->value || argument->ordinal != 0u ||
    ...
    if (direct_calls != 1u || entry_calls != 0u || !verify_extents(plan, error, error_size) ||
...
bool xr_target_plan_verify(const XrTargetPlan *plan, char *error, size_t error_size) {
    ...
    if (!xr_target_profile_verify(plan->profile, error, error_size) ||
        ...
        !verify_machine_reps(plan, error, error_size))
        return false;
    if (!verify_value_reps(plan, error, error_size) || !verify_extents(plan, error, error_size))
        return false;

    /* Type/value ordinals restart in every SemanticPlan. Rebuild the dynamic
     * type proof and validate the layout slice while that module's index space
     * is explicit; no aggregate bitmap is meaningful across partitions. */
    for (uint32_t partition = 0; partition < verifier_partition_count(plan); partition++) {
        XrTargetPartitionView view;
        XrTargetExactValueSets sets;
        uint8_t *exact_dynamic_types = NULL;
        if (!verifier_partition_view(plan, partition, &view) ||
            !exact_value_sets_collect(plan, view.semantic, &sets, error, error_size))
            return false;
        bool valid = collect_exact_dynamic_types(plan, &view, sets.direct_callees, sets.go_callees,
                                                 sets.channel_values, sets.source_namespaces,
                                                 sets.native_module_namespaces,
                                                 &exact_dynamic_types, error, error_size) &&
                     verify_layouts_partition(plan, &view, exact_dynamic_types, error, error_size);
        xr_free(exact_dynamic_types);
        exact_value_sets_dispose(&sets);
        if (!valid)
            return false;
    }
    bool verified = verify_aggregate_layouts_acyclic(plan, error, error_size) &&
                    verify_extent_references(plan, error, error_size) &&
