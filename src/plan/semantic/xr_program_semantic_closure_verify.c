...
static XrStableId verifier_private_leaf_function_declaration(
    const XrProgramSemanticModuleRecord *module,
    const XrProgramSemanticFunctionRecord *row) {
...
static XrFingerprint verifier_private_leaf_call_contract(uint32_t role, XrStableId callee,
    ...
    static const uint8_t domain[] =
        "xray-source-module-scalar-private-leaf-call-contract-v1";
    XrSHA256Context context;
    xr_sha256_init(&context);
    verifier_hash_framed_bytes(&context, domain, sizeof(domain) - 1u);
    verifier_hash_u32(&context, XR_PROGRAM_SEMANTIC_CLOSURE_SCHEMA_VERSION);
...
static bool verifier_private_leaf_identity(XrStableId identity,
    ...
    for (uint32_t i = 0; i < XR_STDLIB_DEF_ENTRY_COUNT; i++) {
        ...
        if (!entry->module || !entry->name || !entry->signature || !entry->aot ||
            ...
            entry->define[0] != '\0' || entry->runtime_capabilities != 0u)
            continue;
        char key[512];
...
static bool verify_function_rows(const XrProgramSemanticClosure *closure, char *error,
    ...
    for (uint32_t i = 0; i < closure->function_count; i++) {
        ...
        if (row->parameter_begin > closure->function_parameter_count ||
            ...
            (typed ? (row->parameter_count > (overflow ? 2u : private_leaf ? 0u : 1u) ||
        ...
        XrFingerprint expected_effect = (leaf || product)   ? verifier_leaf_effect()
                                        : (scalar || graph || private_leaf) ? verifier_scalar_effect()
        ...
        if ((leaf || product) && function_module >= 0) {
            ...
            expected_declaration = verifier_private_leaf_function_declaration(
                &closure->modules[(uint32_t) function_module], row);
            expected_instance = verifier_private_leaf_function_instance(expected_declaration,
                                                                        expected_signature);
        ...
        if (private_leaf) {
            private_leaf_entries +=
                row->flags == XR_PROGRAM_SEMANTIC_FUNCTION_ENTRY ? 1u : 0u;
            private_leaf_exports +=
                row->flags == XR_PROGRAM_SEMANTIC_FUNCTION_EXPORTED ? 1u : 0u;
        }
        if (overflow)
...
static bool verify_call_rows(const XrProgramSemanticClosure *closure, char *error,
    ...
    for (uint32_t i = 0; i < closure->call_count; i++) {
        ...
        bool private_family =
            closure->family ==
            XR_PROGRAM_SEMANTIC_FAMILY_SOURCE_MODULE_SCALAR_PRIVATE_LEAF_CALL;
        bool private_external =
            private_family && callee < 0 && verifier_private_leaf_identity(row->callee_function,
                                                                           NULL);
        ...
        if (verifier_stable_id_zero(row->callsite_identity) ||
            ...
            (closure->family == XR_PROGRAM_SEMANTIC_FAMILY_SCALAR_MODULE_GRAPH_DIRECT_CALL
                 ? verifier_stable_id_zero(row->resolver_binding)
             : private_family
                 ? (callee >= 0 ? verifier_stable_id_zero(row->resolver_binding)
                                : !verifier_stable_id_zero(row->resolver_binding))
                 : !verifier_stable_id_zero(row->resolver_binding)) ||
...
static bool verify_source_private_leaf_rows(const XrProgramSemanticClosure *closure, char *error,
    ...
    const XrProgramSemanticModuleRecord *export_module =
        &closure->modules[(uint32_t) export_module_index];
    XrStableId expected_binding = verifier_source_graph_import_binding(
        entry_module, export_module, edge->import_locator);
...
static bool verify_call_graph(const XrProgramSemanticClosure *closure, char *error,
    ...
    bool private_family =
        ...
        XR_PROGRAM_SEMANTIC_FAMILY_SOURCE_MODULE_SCALAR_PRIVATE_LEAF_CALL;
    for (uint32_t i = 0; i < closure->call_count; i++) {
        bool external = private_family &&
                        find_function(closure, closure->calls[i].callee_function) < 0;
