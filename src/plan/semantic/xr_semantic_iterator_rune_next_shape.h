...
static inline bool
xr_semantic_iterator_rune_next_receiver_is_exact(const XrSemanticPlan *plan,
...
static inline bool
xr_semantic_iterator_rune_next_is_exact(const XrSemanticPlan *plan,
                                        const XrSemanticOperationRecord *operation,
                                        uint32_t *receiver_value) {
    ...
    if (!xr_semantic_string_runes_result_type_is_exact(plan, receiver_type) || !result_type ||
        ...
        !xr_semantic_iterator_rune_next_receiver_is_exact(plan, operation, receiver->value))
        return false;
    if (receiver_value)
