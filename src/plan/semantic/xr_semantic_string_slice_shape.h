...
static inline bool xr_semantic_string_slice_string_type_is_exact(
    ...
    return type && type->kind == XR_KIND_STRING &&
           ...
           !type->source_enum_key && type->child_count == 0 &&
...
static inline bool xr_semantic_string_slice_i64_type_is_exact(
    const XrSemanticTypeRecord *type) {
    XrStableId zero = {{0}};
...
static inline bool xr_semantic_string_slice_operand_is_exact(
    const XrSemanticOperandRecord *operand, uint8_t role, int32_t parameter,
    ...
    return operand && operand->role == role && operand->parameter == parameter &&
           ...
           operand->access == XR_CALL_ARG_PLAIN && operand->origin == 0 &&
           operand->lifetime == 0 && operand->escape == 0 &&
           operand->flags == XR_SEM_OPERAND_CALL_CONTRACT;
}

static inline bool xr_semantic_string_slice_receiver_has_prior_identity(
    const XrSemanticPlan *plan, const XrSemanticOperationRecord *slice,
    ...
    for (uint32_t i = 0; i < (uint32_t) parameter_count; i++) {
        ...
        if (!parameter || parameter->value != receiver->value)
            continue;
        if (parameter->function != slice->function ||
    ...
    for (uint32_t i = 0; i < (uint32_t) operation_count; i++) {
        ...
        if (!source || source->result_value != receiver->value)
            continue;
        if (source->function != slice->function ||
            source->result_type != receiver->type ||
...
static inline bool xr_semantic_string_slice_range_is_exact(
    ...
    if (!plan || !operation || !operands || !metadata ||
        ...
        operation->flags !=
            (xi_generated_op_default_flags(XI_CALL_METHOD) | XI_FLAG_TAIL) ||
    ...
    const XrSemanticTypeRecord *start_type =
        xr_semantic_plan_type(plan, start->type);
    const XrSemanticTypeRecord *end_type =
        xr_semantic_plan_type(plan, end->type);
    if (operation->result_type != receiver->type || start->type != end->type ||
        ...
        !xr_semantic_string_slice_receiver_has_prior_identity(
            plan, operation, receiver) ||
        !xr_semantic_string_slice_operand_is_exact(
            receiver, XR_SEM_OPERAND_RECEIVER, -1, XR_SEM_OPERAND_BORROW) ||
        !xr_semantic_string_slice_operand_is_exact(
            start, XR_SEM_OPERAND_ARGUMENT, 0, XR_SEM_OPERAND_CONSUME) ||
        !xr_semantic_string_slice_operand_is_exact(
            end, XR_SEM_OPERAND_ARGUMENT, 1, XR_SEM_OPERAND_CONSUME))
        return false;
    if (receiver_value)
