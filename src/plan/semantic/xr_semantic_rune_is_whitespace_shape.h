 ...
 *
 * xr_semantic_rune_is_whitespace_shape.h - Exact iterator-rune predicate authority
 */
...
static inline bool xr_semantic_rune_is_whitespace_receiver_is_exact(
...
static inline bool xr_semantic_rune_is_whitespace_is_exact(
    const XrSemanticPlan *plan, const XrSemanticOperationRecord *operation,
    uint32_t *receiver_value) {
    ...
    if (!plan || !operation || !operands || !metadata ||
        ...
        strcmp(metadata[operation->metadata_begin], "isWhitespace") != 0 ||
    ...
    if (!receiver_type || receiver_type->kind != XR_KIND_RUNE ||
        ...
        receiver_type->enum_layout_id != 0 || receiver_type->enum_member_count != 0 ||
        receiver_type->enum_flags != 0 || receiver_type->reserved_enum != 0 ||
        receiver_type->flags != 0 || !receiver_type->canonical_key ||
        strcmp(receiver_type->canonical_key, expected_rune) != 0 ||
        !result_type || result_type->kind != XR_KIND_BOOL ||
        ...
        result_type->enum_layout_id != 0 || result_type->enum_member_count != 0 ||
        result_type->enum_flags != 0 || result_type->reserved_enum != 0 ||
        result_type->flags != 0 || !result_type->canonical_key ||
        ...
        !xr_semantic_rune_is_whitespace_receiver_is_exact(
            plan, operation, receiver->value))
        return false;
    if (receiver_value)
...
#endif  // XR_SEMANTIC_RUNE_IS_WHITESPACE_SHAPE_H
