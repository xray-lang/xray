...
xr_semantic_native_direct_signature_type_matches(const XrSemanticPlan *plan, uint32_t type_index,
    ...
    if (spelling_length > 7 && memcmp(spelling, "Array<", 6) == 0 &&
        ...
        return xr_semantic_array_type_row_is_exact(type) &&
               (type->flags & XR_SEM_TYPE_NULLABLE) == 0 && children &&
               type->child_begin < child_count &&
               xr_semantic_native_direct_signature_type_matches(plan, children[type->child_begin],
                                                                module, spelling + 6,
                                                                spelling_length - 7u, false);
    }
...
static inline bool xr_semantic_native_direct_signature_is_exact(
    ...
    for (uint32_t ordinal = 0; ordinal < entry->argc; ordinal++) {
        ...
        while (type < end && *type == ' ')
            type++;
        bool reference = (size_t) (end - type) >= 4u && memcmp(type, "ref ", 4) == 0;
        if (reference)
            type += 4;
        if (arguments[ordinal].parameter_mode != (reference ? XR_PARAM_REF : XR_PARAM_READ))
            return false;
...
static inline bool xr_semantic_native_direct_call_shape_is_exact(
    ...
    if (!plan || !operation || !operands || !metadata || operation->opcode != XI_CALL ||
        ...
        (operation->flags & XI_FLAG_MAY_SUSPEND) != 0 ||
        operation->effects != xi_generated_op_effects(XI_CALL) ||
        operation->result_alias_operand != -1 || !result_type)
        return false;
        ...
        if (argument->role != XR_SEM_OPERAND_ARGUMENT ||
            ...
            argument->flags !=
                (XR_SEM_OPERAND_CALL_CONTRACT |
                 (argument->parameter_mode == XR_PARAM_REF ? XR_SEM_OPERAND_ADDRESSABLE : 0)) ||
            argument->ownership_action != XR_SEM_OPERAND_BORROW ||
            (argument->parameter_mode != XR_PARAM_READ &&
             argument->parameter_mode != XR_PARAM_REF) ||
            argument->access !=
                (argument->parameter_mode == XR_PARAM_REF ? XR_CALL_ARG_REF : XR_CALL_ARG_PLAIN) ||
            (argument->parameter_mode == XR_PARAM_REF
                 ? argument->origin != XI_PLACE_ORIGIN_STACK_LOCAL ||
            ...
            return false;
        if (argument->parameter_mode == XR_PARAM_REF) {
            const XrSemanticOperationRecord *address =
                xr_semantic_unique_operation_for_value(plan, operation->function, argument->value);
            if (!xr_semantic_ref_argument_local_addr_is_exact(plan, address, argument->type,
                                                              NULL) ||
                !xr_semantic_array_type_row_is_exact(xr_semantic_plan_type(plan, argument->type)))
                return false;
        }
    }
...
/* A native ref parameter borrows a caller-owned tagged slot through a real
 * address. Signature mode, local provenance and lifetime are proved by the
 * call judgement; no member name or provider-specific opcode selects it. */
static inline bool
xr_semantic_native_direct_ref_address_is_exact(const XrSemanticPlan *plan,
                                               const XrSemanticOperationRecord *address) {
    ...
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands = xr_semantic_plan_operands(plan, &operand_count);
    for (uint32_t i = 0; operands && i < xr_semantic_plan_call_target_count(plan); i++) {
        ...
        if (!target || target->kind != XR_SEM_CALL_TARGET_NATIVE_DIRECT || !call ||
            call->function != address->function ||
            !xr_semantic_native_direct_call_shape_is_exact(plan, call, NULL, NULL))
            continue;
        for (uint16_t ordinal = 1; ordinal < call->operand_count; ordinal++) {
            const XrSemanticOperandRecord *argument = &operands[call->operand_begin + ordinal];
            if (argument->parameter_mode == XR_PARAM_REF &&
                argument->value == address->result_value && argument->type == address->result_type)
                return true;
        }
    }
    return false;
