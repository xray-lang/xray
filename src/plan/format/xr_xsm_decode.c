...
#define XR_COUNT_STORAGE(field, type)                                                              \
    ...
    XR_COUNT_STORAGE(source_methods, XrSemanticSourceMethodRecord);
...
#define XR_ALLOC_TABLE(field, count_value, type)                                                   \
    ...
    XR_ALLOC_TABLE(source_methods, count.source_methods, XrSemanticSourceMethodRecord);
    ...
    XR_ALLOC_TABLE(program_type_bindings, count.program_types, XrSemanticProgramTypeBinding);
    XR_ALLOC_TABLE(program_type_field_bindings, count.program_type_fields,
...
#undef XR_ALLOC_CERT
    ...
    plan->source_method_count = plan->source_method_capacity = count.source_methods;
...
static bool take_counts(XrXsmReader *reader, XrXsmCounts *count) {
    ...
    count->source_methods = xr_xsm_take_u32(reader);
    ...
    return !reader->failed && count->types <= 1000000u && count->source_classes <= 100000u &&
           ...
           count->entities <= 80000000u && count->type_children <= 8000000u &&
           count->parameters <= 25600000u && count->captures <= 6400000u &&
           ...
           count->loop_invariants <= 40000000u &&
           count->program_types <= XR_PROGRAM_SEMANTIC_CLOSURE_MAX_TYPES &&
           count->program_types <= count->types &&
           count->program_type_fields <= XR_PROGRAM_SEMANTIC_CLOSURE_MAX_TYPE_FIELDS &&
...
static void decode_program_provenance(XrXsmReader *reader, XrSemanticPlan *plan) {
    XrSemanticProgramProvenance *provenance = &plan->program_provenance;
    provenance->schema = xr_xsm_take_u32(reader);
    provenance->program_schema = xr_xsm_take_u32(reader);
    provenance->program_family = xr_xsm_take_u32(reader);
    provenance->type_count = xr_xsm_take_u32(reader);
    ...
    for (uint32_t i = 0; i < plan->program_type_binding_count; i++) {
        ...
        xr_xsm_take_bytes(reader, binding->source_class_identity.bytes,
                          sizeof(binding->source_class_identity.bytes));
        binding->semantic_type = xr_xsm_take_u32(reader);
        binding->program_row = xr_xsm_take_u32(reader);
        binding->field_begin = xr_xsm_take_u32(reader);
        binding->field_count = xr_xsm_take_u32(reader);
        binding->kind = xr_xsm_take_u8(reader);
        binding->exact_scalar = xr_xsm_take_u8(reader);
        binding->flags = xr_xsm_take_u8(reader);
        binding->reserved = xr_xsm_take_u8(reader);
    }
    for (uint32_t i = 0; i < plan->program_type_field_binding_count; i++) {
        XrSemanticProgramTypeFieldBinding *binding = &plan->program_type_field_bindings[i];
        xr_xsm_take_bytes(reader, binding->program_owner_type.bytes,
                          sizeof(binding->program_owner_type.bytes));
        xr_xsm_take_bytes(reader, binding->program_field_type.bytes,
                          sizeof(binding->program_field_type.bytes));
        binding->owner_program_row = xr_xsm_take_u32(reader);
        binding->field_program_row = xr_xsm_take_u32(reader);
        binding->semantic_field_type = xr_xsm_take_u32(reader);
        binding->declaration_ordinal = xr_xsm_take_u32(reader);
    }
    for (uint32_t i = 0; i < plan->program_function_binding_count; i++) {
        XrSemanticProgramFunctionBinding *binding = &plan->program_function_bindings[i];
    ...
    for (uint32_t i = 0; i < plan->program_call_binding_count; i++) {
        ...
        xr_xsm_take_bytes(reader, binding->callsite.bytes, sizeof(binding->callsite.bytes));
...
static void decode_types(XrXsmReader *reader, XrSemanticPlan *plan) {
    for (uint32_t i = 0; i < plan->type_count; i++) {
        ...
        xr_xsm_take_bytes(reader, record->source_class_identity.bytes,
...
static void decode_source_classes(XrXsmReader *reader, XrSemanticPlan *plan) {
...
static void decode_source_methods(XrXsmReader *reader, XrSemanticPlan *plan) {
    for (uint32_t i = 0; i < plan->source_method_count; i++) {
        XrSemanticSourceMethodRecord *record = &plan->source_methods[i];
        xr_xsm_take_bytes(reader, record->id.bytes, sizeof(record->id.bytes));
        record->canonical_key = take_plan_string(reader, plan, false);
        record->name = take_plan_string(reader, plan, false);
        record->source_class = xr_xsm_take_u32(reader);
        record->function = xr_xsm_take_u32(reader);
        record->member_ordinal = xr_xsm_take_u16(reader);
...
static void decode_operations(XrXsmReader *reader, XrSemanticPlan *plan) {
    for (uint32_t i = 0; i < plan->operation_count; i++) {
        ...
        record->view_source_value = xr_xsm_take_u32(reader);
        record->view_element_type = xr_xsm_take_u32(reader);
        record->view_source_operand = decode_twos_complement_i16(xr_xsm_take_u16(reader));
        record->view_source_parameter = decode_twos_complement_i16(xr_xsm_take_u16(reader));
        record->intrinsic_kind = xr_xsm_take_u8(reader);
        record->view_origin = xr_xsm_take_u8(reader);
        ...
        record->array_hof_kind = xr_xsm_take_u8(reader);
        record->array_result_element_storage = xr_xsm_take_u8(reader);
    }
...
static void decode_dependencies_and_exports(XrXsmReader *reader, XrSemanticPlan *plan) {
...
static bool decode_xsm(const uint8_t *bytes, size_t size, const XrSemanticPlan *const *dependencies,
    ...
    XrFingerprint expected_registry_fingerprint, expected_stdlib_registry_fingerprint,
    ...
    xr_xsm_take_bytes(&header, expected_stdlib_registry_fingerprint.bytes,
                      sizeof(expected_stdlib_registry_fingerprint.bytes));
    xr_xsm_take_bytes(&header, expected_fingerprint.bytes, sizeof(expected_fingerprint.bytes));
    if (header.failed || memcmp(magic, xr_xsm_artifact_magic, XR_XSM_ARTIFACT_MAGIC_SIZE) != 0 ||
    ...
    XrFingerprint current_registry_fingerprint, current_stdlib_registry_fingerprint;
    xr_semantic_op_registry_fingerprint(&current_registry_fingerprint);
    xr_stdlib_metadata_registry_fingerprint(&current_stdlib_registry_fingerprint);
    ...
    if (!xr_fingerprint_equal(expected_stdlib_registry_fingerprint,
                              current_stdlib_registry_fingerprint))
        return report(error, error_size, "XR_ARTIFACT_2003",
                      "XSM stdlib registry fingerprint is incompatible");
