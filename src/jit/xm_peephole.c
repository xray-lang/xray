 ...
 *
 * xm_peephole.c - Post-codegen ARM64 peephole optimiser.
 *
 * DESIGN:
...
static inline uint32_t enc_rt(uint32_t w) {
    return w & 0x1Fu;
}
static inline uint32_t enc_rn(uint32_t w) {
    return (w >> 5) & 0x1Fu;
}
static inline uint32_t enc_imm12(uint32_t w) {
    return (w >> 10) & 0xFFFu;
}
...
static inline uint32_t mov_rd(uint32_t w) {
    return w & 0x1Fu;
}
static inline uint32_t mov_rm(uint32_t w) {
    return (w >> 16) & 0x1Fu;
}
...
static inline uint32_t cmp_rn(uint32_t w) {
    return (w >> 5) & 0x1Fu;
}
static inline uint32_t cmp_imm(uint32_t w) {
    return (w >> 10) & 0xFFFu;
}
...
static inline uint32_t bcond_cond(uint32_t w) {
    return w & 0xFu;
}
...
static inline int32_t bcond_offset(uint32_t w) {
    ...
    if (raw & 0x40000)
        raw |= (int32_t) 0xFFF80000; /* sign-extend */
    return raw;
}
 ...
 *   base = 0xA9000000 */
static inline uint32_t make_stp_x(uint32_t rt1, uint32_t rt2, uint32_t rn, int32_t imm7) {
    return 0xA9000000u | (((uint32_t) imm7 & 0x7Fu) << 15) | (rt2 << 10) | (rn << 5) | rt1;
}

/* LDP Xt1, Xt2, [Xn, #imm7] offset 64-bit:
 *   base = 0xA9400000 */
static inline uint32_t make_ldp_x(uint32_t rt1, uint32_t rt2, uint32_t rn, int32_t imm7) {
    return 0xA9400000u | (((uint32_t) imm7 & 0x7Fu) << 15) | (rt2 << 10) | (rn << 5) | rt1;
}
...
static inline uint32_t subs_rd(uint32_t w) {
    return w & 0x1Fu;
}
static inline uint32_t subs_rn(uint32_t w) {
    return (w >> 5) & 0x1Fu;
}
static inline uint32_t subs_rm(uint32_t w) {
    return (w >> 16) & 0x1Fu;
}
...
static inline int32_t b_offset(uint32_t w) {
    ...
    if (raw & 0x2000000)
        raw |= (int32_t) 0xFC000000; /* sign-extend */
    return raw;
}

/* Invert B.cond: flip bit 0 (EQ↔NE, LT↔GE, etc.) */
static inline uint32_t invert_cond(uint32_t cond) {
    return cond ^ 1u;
}
...
static inline uint32_t movz_rd(uint32_t w) {
    return w & 0x1Fu;
}
static inline uint32_t movz_hw(uint32_t w) {
    return (w >> 21) & 0x3u;
}
static inline uint32_t movz_imm16(uint32_t w) {
    return (w >> 5) & 0xFFFFu;
}
...
static inline uint32_t movk_rd(uint32_t w) {
    return w & 0x1Fu;
}
static inline uint32_t movk_hw(uint32_t w) {
    return (w >> 21) & 0x3u;
}
static inline uint32_t movk_imm16(uint32_t w) {
    return (w >> 5) & 0xFFFFu;
}
...
static uint32_t try_str_ldr(uint32_t *code, uint32_t i, uint32_t n) {
    ...
    /* 64-bit STR+LDR */
    if (is_str_x(a) && is_ldr_x(b) && enc_rt(a) == enc_rt(b) && enc_rn(a) == enc_rn(b) &&
        enc_imm12(a) == enc_imm12(b)) {
        code[i + 1] = A64_NOP;
        return 1;
    }
    /* 32-bit STR+LDR */
    if (is_str_w(a) && is_ldr_w(b) && enc_rt(a) == enc_rt(b) && enc_rn(a) == enc_rn(b) &&
        enc_imm12(a) == enc_imm12(b)) {
        code[i + 1] = A64_NOP;
        return 1;
    }
...
static uint32_t try_cmp_cbz(uint32_t *code, uint32_t i, uint32_t n) {
    ...
    code[i] = (cond == COND_EQ) ? make_cbz(rn, new_off) : make_cbnz(rn, new_off);
    code[i + 1] = A64_NOP;
    return 1;
}
...
static uint32_t try_ldp_stp(uint32_t *code, uint32_t i, uint32_t n) {
    ...
    /* STR+STR fusion → STP */
    if (is_str_x(a) && is_str_x(b) && enc_rn(a) == enc_rn(b) &&
    ...
    /* LDR+LDR fusion → LDP */
    if (is_ldr_x(a) && is_ldr_x(b) && enc_rn(a) == enc_rn(b) && enc_rt(a) != enc_rt(b) &&
...
static uint32_t try_movz_movk(uint32_t *code, uint32_t i, uint32_t n) {
    if (i + 1 >= n)
        return 0;
    uint32_t a = code[i], b = code[i + 1];
    if (!is_movz_x(a) || !is_movk_x(b))
        return 0;
    if (movz_rd(a) != movk_rd(b))
        return 0;
...
uint32_t xm_peephole(A64Buf *buf) {
    XR_DCHECK(buf != NULL, "xm_peephole: NULL buf");
    if (!buf->code || buf->count < 2)
        return 0;
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        r = try_ldp_stp(code, i, n);
        if (r) {
            nops += r;
            i++;
            continue;
        }
        r = try_str_ldr(code, i, n);
        if (r) {
            nops += r;
            continue;
        }
        r = try_dup_str(code, i, n);
        if (r) {
            nops += r;
            continue;
        }
        r = try_redundant_cmp(code, i, n);
        if (r) {
            nops += r;
            i++;
            continue;
        }
        r = try_cmp_cbz(code, i, n);
        if (r) {
            nops += r;
            i++;
            continue;
        }
        r = try_bcond_over_b(code, i, n);
        if (r) {
            nops += r;
            i++;
            continue;
        }
        r = try_movz_movk(code, i, n);
        if (r) {
            nops += r;
            i++;
            continue;
        }
        r = try_mov_self(code, i, n);
        if (r) {
            nops += r;
            continue;
        }
