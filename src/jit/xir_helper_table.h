 ...
 *
 * xir_helper_table.h - Central declaration table for all JIT C helpers
 *
 * DESIGN:
 *   Every JIT C helper is declared exactly once in XIR_HELPER_DEF.
 *   The macro table generates:
 *     - XirHelperId enum (compile-time IDs)
 *     - xir_helper_info[] metadata array (function pointer, return rep, flags)
 ...
 *
 *   Adding a new helper requires ONE edit: add a line to XIR_HELPER_DEF.
 *   Forgetting to add it causes a linker error (function not in table).
 *
 * XIR_HELPER_DEF(name, nargs, ret_rep, flags)
 *
 ...
 *              when the helper's XirType.kind is XIR_TK_BOOL)
 *   flags:    bitwise OR of XIR_HF_* flags:
 *             XIR_HF_GC      — may trigger GC (needs safepoint)
 *             XIR_HF_DEOPT   — may request deoptimization
 *             XIR_HF_THROW   — may throw exception
 *             XIR_HF_SUSPEND — may suspend coroutine (CPS)
 */

#ifndef XIR_HELPER_TABLE_H
#define XIR_HELPER_TABLE_H
...
typedef enum {
#define XIR_HELPER_ENUM_(name, nargs, ret_rep, flags) XIR_HELPER_##name,
    XIR_HELPER_DEF(XIR_HELPER_ENUM_)
#undef XIR_HELPER_ENUM_
    XIR_HELPER__COUNT
} XirHelperId;
...
extern const XirHelperInfo xir_helper_info[XIR_HELPER__COUNT];
...
// Lookup helper ID by function pointer. Returns XIR_HELPER__COUNT if not found.
...
static inline bool xir_helper_may_gc(XirHelperId id) {
    return id < XIR_HELPER__COUNT && (xir_helper_info[id].flags & XIR_HF_GC);
}

// Check if a helper may suspend
static inline bool xir_helper_may_suspend(XirHelperId id) {
    return id < XIR_HELPER__COUNT && (xir_helper_info[id].flags & XIR_HF_SUSPEND);
}

// Check if a helper returns a dynamic-typed value (needs runtime tag)
static inline bool xir_helper_is_tagged(XirHelperId id) {
