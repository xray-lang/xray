 ...
 *
 * xir_printer.c - XIR text dump for debugging
 *
 * KEY CONCEPT:
 *   Human-readable text representation of XIR for debugging and
...
void xir_print_ref(FILE *out, XirFunc *func, XirRef ref) {
    XR_DCHECK(out != NULL, "xir_print_ref: NULL out");
    switch (XIR_REF_KIND(ref)) {
        ...
        case XIR_REF_SLOT:
            fprintf(out, "s%u", XIR_REF_INDEX(ref));
            break;
        case XIR_REF_BLOCK:
            fprintf(out, "@b%u", XIR_REF_INDEX(ref));
            break;
        case XIR_REF_NONE:
            fprintf(out, "_");
            break;
...
void xir_print_ins(FILE *out, XirFunc *func, XirIns *ins) {
    XR_DCHECK(out != NULL, "xir_print_ins: NULL out");
    XR_DCHECK(func != NULL, "xir_print_ins: NULL func");
    XR_DCHECK(ins != NULL, "xir_print_ins: NULL ins");
    fprintf(out, "    ");

    // Print destination
    if (!xir_ref_is_none(ins->dst)) {
        xir_print_ref(out, func, ins->dst);
        fprintf(out, " =%s ", type_name(ins->rep));
    }

    // Print opcode
    fprintf(out, "%s", xir_op_name(ins->op));

    // Print arguments
    if (!xir_ref_is_none(ins->args[0])) {
        fprintf(out, " ");
        xir_print_ref(out, func, ins->args[0]);
    }
    if (!xir_ref_is_none(ins->args[1])) {
        fprintf(out, ", ");
        xir_print_ref(out, func, ins->args[1]);
    }
...
static void print_block_label(FILE *out, XirBlock *blk) {
    if (blk->label) {
...
void xir_print_block(FILE *out, XirFunc *func, XirBlock *blk) {
    XR_DCHECK(out != NULL, "xir_print_block: NULL out");
    XR_DCHECK(func != NULL, "xir_print_block: NULL func");
    XR_DCHECK(blk != NULL, "xir_print_block: NULL blk");
    // Block header
    fprintf(out, "  ");
    ...
    for (XirPhi *phi = blk->phis; phi; phi = phi->next) {
        ...
        for (uint16_t i = 0; i < phi->narg; i++) {
            ...
            xir_print_ref(out, func, phi->args[i]);
        }
        fprintf(out, "\n");
    }
    ...
    for (uint32_t i = 0; i < blk->nins; i++) {
        xir_print_ins(out, func, &blk->ins[i]);
    }
...
void xir_print_func(FILE *out, XirFunc *func) {
    XR_DCHECK(out != NULL, "xir_print_func: NULL out");
    XR_DCHECK(func != NULL, "xir_print_func: NULL func");
