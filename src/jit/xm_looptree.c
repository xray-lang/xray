 ...
 *
 * xm_looptree.c - Natural-loop forest derived from the dominator tree.
 *
 * ALGORITHM:
 *   1. Build the cached dominator tree (xm_func_get_domtree).
 ...
 * xm_ensure_preheader().
 */
static XmBlock *find_unique_preheader(XmBlock *hdr, const uint8_t *in_loop, XmFunc *func) {
    XmBlock *found = NULL;
    for (uint32_t p = 0; p < hdr->npred; p++) {
        XmBlock *cand = hdr->preds[p];
...
/*
 * Allocate a XmLoop populated from the given header + body bitmap.
 * The body array is heap-allocated and owned by the returned XmLoop
...
static XmLoop *alloc_loop(XmFunc *func, uint32_t hdr_bi, uint32_t latch_bi,
    ...
    XmLoop *loop = (XmLoop *) xr_calloc(1, sizeof(XmLoop));
    if (!loop)
        return NULL;
    loop->header = hdr;
    ...
    if (body_count > 0) {
        loop->body = (XmBlock **) xr_malloc(body_count * sizeof(XmBlock *));
        if (!loop->body) {
            xr_free(loop);
            return NULL;
        }
...
static void free_loop(XmLoop *loop) {
    if (!loop)
        return;
    xr_free(loop->body);
    xr_free(loop);
...
static void xm_loopinfo_free(XmLoopInfo *info) {
    if (!info)
        return;
    if (info->all_loops) {
 ...
 */
static XmLoopInfo *build_loop_info(XmFunc *func) {
    ...
    for (uint32_t s = 0; s < n; s++) {
        XmBlock *src = func->blocks[s];
        XmBlock *succ[2] = {src->s1, src->s2};
        for (int k = 0; k < 2; k++) {
            if (!succ[k])
                continue;
    ...
    XmLoopInfo *info = (XmLoopInfo *) xr_calloc(1, sizeof(XmLoopInfo));
    if (!info) {
        xr_free(headers);
        xr_free(first_latch_seen);
        xr_free(first_latch);
    ...
    info->nblk = n;
    info->block_to_loop = (XmLoop **) xr_calloc(n, sizeof(XmLoop *));
    info->all_loops = nloop ? (XmLoop **) xr_calloc(nloop, sizeof(XmLoop *)) : NULL;
    if ((!info->block_to_loop && n) || (nloop && !info->all_loops)) {
        xm_loopinfo_free(info);
        xr_free(headers);
        xr_free(first_latch_seen);
        xr_free(first_latch);
        return NULL;
    ...
    for (uint32_t hbi = 0; hbi < n; hbi++) {
        ...
        for (uint32_t s = 0; s < n; s++) {
            ...
            for (int k = 0; k < 2; k++) {
                ...
                if (t != hbi)
                    continue;
                if (!xm_dom_covers(dt, hbi, s))
                    continue;
                collect_loop_body(func, dt, hbi, s, bitmap);
            }
        }

        XmLoop *L = alloc_loop(func, hbi, first_latch[hbi], bitmap);
        if (!L) {
            xr_free(body_bitmap);
            xr_free(headers);
            xr_free(first_latch_seen);
    ...
    for (uint32_t li = 0; li < info->nloop; li++) {
        XmLoop *L = info->all_loops[li];
        uint32_t d = 1;
        for (XmLoop *p = L->parent; p; p = p->parent)
            d++;
        L->depth = d;
    }
...
const XmLoopInfo *xm_func_get_loops(XmFunc *func) {
    if (!func || func->nblk == 0)
        return NULL;
    if (func->loopinfo)
...
void xm_func_invalidate_loops(XmFunc *func) {
    if (!func)
        return;
    if (func->loopinfo) {
        xm_loopinfo_free(func->loopinfo);
        func->loopinfo = NULL;
    }
    /* Loops depend on the dominator tree, so keep invalidation
     * transitive. */
    xm_func_invalidate_domtree(func);
}

uint32_t xm_block_loop_depth(XmFunc *func, uint32_t blk_id) {
    const XmLoopInfo *info = xm_func_get_loops(func);
    if (!info || blk_id >= info->nblk)
        return 0;
    XmLoop *L = info->block_to_loop[blk_id];
    return L ? L->depth : 0;
}
