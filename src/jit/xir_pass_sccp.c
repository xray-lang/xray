 ...
 *
 * xir_pass_sccp.c - Sparse Conditional Constant Propagation.
 *
 * KEY CONCEPT:
 *   A single pass that folds constants, kills unreachable blocks and
 *   simplifies branches in one fixed-point computation.  This
 *   replaces the old trio xir_pass_const_prop / xir_pass_branch_simp /
 *   xir_pass_remove_unreachable that used to require multiple pipeline
...
static SccpVal eval_ins(SccpCtx *ctx, XirIns *ins) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            if ((xir_ref_is_vreg(va) && XIR_REF_INDEX(va) == v) ||
                ...
                ssa_push(&ctx->ssa, XIR_REF_INDEX(ins->dst));
            }
        }
        for (XirPhi *phi = blk->phis; phi; phi = phi->next) {
            ...
            for (uint32_t k = 0; k < phi->narg; k++) {
                ...
                if (xir_ref_is_vreg(a) && XIR_REF_INDEX(a) == v) {
                    ssa_push(&ctx->ssa, XIR_REF_INDEX(phi->dst));
                    break;
                }
            }
        }
...
static XirRef emit_const_i64(XirFunc *func, int64_t v) {
    return xir_const_i64(func, v);
}
static XirRef emit_const_f64(XirFunc *func, double v) {
    return xir_const_f64(func, v);
}

/* Rewrite an instruction when its dst became a constant. */
static void rewrite_to_const(SccpCtx *ctx, XirIns *ins, SccpVal v) {
    switch (v.kind) {
        case SCCP_CONST_I64:
        case SCCP_CONST_BOOL: {
            XirRef k = emit_const_i64(ctx->func, v.i64);
        ...
        case SCCP_CONST_F64: {
            XirRef k = emit_const_f64(ctx->func, v.f64);
        ...
        case SCCP_CONST_PTR:
            ...
            if (v.ptr == NULL) {
                ...
                ins->args[0] = xir_const_ptr(ctx->func, NULL);
                ins->args[1] = XIR_NONE;
            }
            break;
        default:
...
static void rewrite_function(SccpCtx *ctx) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        if (!ctx->reachable[bi]) {
            ...
            for (uint32_t i = 0; i < blk->nins; i++) {
                blk->ins[i].op = XIR_NOP;
                blk->ins[i].dst = XIR_NONE;
                blk->ins[i].args[0] = XIR_NONE;
                blk->ins[i].args[1] = XIR_NONE;
                blk->ins[i].flags = 0;
            }
            blk->phis = NULL;
        ...
        if (blk->jmp.type == XIR_JMP_BR) {
            SccpVal c = ref_value(ctx, blk->jmp.arg);
            if (c.kind == SCCP_CONST_BOOL || c.kind == SCCP_CONST_I64) {
                XirBlock *taken = (c.i64 != 0) ? blk->s1 : blk->s2;
