 ...
 *
 * xm_coalesce.c - Aggressive register coalescing for Xm
 *
 * KEY CONCEPT:
...
static void build_conflicts(ConflictMatrix *cm, XmFunc *func, XmLive *live) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        XmBSet live_set;
        xm_bset_init(&live_set, func->nvreg);

        // Start with live-out
        if (bi < live->nblk)
            xm_bset_copy(&live_set, &live->blocks[bi].live_out);
        ...
        for (int32_t ii = (int32_t) blk->nins - 1; ii >= 0; ii--) {
            ...
            if (xm_op_is_copy(ins->op) && xm_ref_is_vreg(ins->args[0]))
                ignore_vreg = XM_REF_INDEX(ins->args[0]);
            ...
            if (xm_ref_is_vreg(ins->dst)) {
                ...
                if (dv < func->nvreg) {
                    int iter = 0, bit;
                    while ((bit = xm_bset_iter(&live_set, &iter)) >= 0) {
                        uint32_t lv = (uint32_t) bit;
                        if (lv != dv && lv != ignore_vreg)
                            cm_set(cm, dv, lv);
                    }
                    xm_bset_clr(&live_set, dv);
                }
            }
        ...
        xm_bset_free(&live_set);
    }
}

/* ========== Rewrite Instructions ========== */

static inline void rewrite_ref(UnionFind *uf, XmRef *ref) {
    if (xm_ref_is_vreg(*ref)) {
        uint32_t v = XM_REF_INDEX(*ref);
        uint32_t root = uf_find(uf, v);
        if (root != v)
            *ref = XM_REF(XM_REF_VREG, root);
    }
}

static void rewrite_instructions(XmFunc *func, UnionFind *uf) {
...
static uint32_t delete_coalesced_movs(XmFunc *func) {
    uint32_t deleted = 0;
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        XmBlock *blk = func->blocks[bi];
        if (!blk)
        ...
        for (uint32_t ii = 0; ii < blk->nins; ii++) {
            ...
            if (!xm_op_is_copy(ins->op))
                continue;
            if (!xm_ref_is_vreg(ins->dst) || !xm_ref_is_vreg(ins->args[0]))
                continue;

            uint32_t dv = XM_REF_INDEX(ins->dst);
            uint32_t sv = XM_REF_INDEX(ins->args[0]);
...
uint32_t xm_coalesce(XmFunc *func) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        uint32_t depth = xm_block_loop_depth(func, blk->id);
        if (depth > 10)
            depth = 10;
        uint32_t w = 1u << depth;
        for (uint32_t ii = 0; ii < blk->nins; ii++) {
