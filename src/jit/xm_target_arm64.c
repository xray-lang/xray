 ...
 *
 * xm_target_arm64.c - ARM64 target description for Xm backend
 *
 * KEY CONCEPT:
 *   Defines the ARM64 register inventory and frame layout as a
 *   static XmTarget instance. This is the single source of truth
...
const XmTarget xm_target_arm64 = {
    .name = "arm64",
    ...
    .max_spill_slots = 32,  // must match XM_MAX_SPILL_SLOTS (uint32_t spill_bitmap)

    .max_vregs = 4096,
};

// Global current target pointer — set to this platform's target
const XmTarget *xm_current_target = &xm_target_arm64;
