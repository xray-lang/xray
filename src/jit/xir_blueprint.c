...
static bool opcode_reads_slot(XrInstruction inst, int reg) {
    ...
    case OP_TOSTRING: case OP_TYPEOF: case OP_COPY:
    case OP_BOX_I64: case OP_BOX_F64:
    case OP_UNBOX_I64: case OP_UNBOX_F64:
    case OP_ARRAY_LEN: case OP_TYPENAME: case OP_CHR:
        return reg == b;
    ...
    case OP_INVOKE: case OP_INVOKE_BUILTIN: case OP_INVOKE_DIRECT:
    case OP_INVOKE_TAIL: case OP_SUPERINVOKE:
        return reg >= a && reg <= a + b;
    ...
    case OP_CHAN_SEND:
        return reg == a || reg == b;
    case OP_CHAN_RECV: case OP_CHAN_TRY_RECV:
        return reg == b;

    // Go spawn: reads func + args
    case OP_GO: case OP_GO_INVOKE:
        return reg >= a && reg <= a + b;
...
static void generate_loop_live_maps(XrBlueprint *bp, XrProto *proto) {
    ...
    if (!proto->loop_headers || proto->loop_header_count == 0) return;
    ...
    for (uint32_t pc = 0; pc < code_count; pc++) {
        ...
        for (int li = 0; li < nloops; li++) {
            if ((int)pc == (int)proto->loop_headers[li] &&
                bp->nloops < nloops) {
                    ...
                    if (back_edge < 0) back_edge = (int)code_count - 1;
                    ...
                    if (maxstack > 256) maxstack = 256;

                    for (int s = 0; s < maxstack && info->nlive < XR_BP_MAX_LOOP_LIVE; s++) {
                        if (!slot_written[s]) continue;  // pure DEF-USE: only slots with a real write are live
