 ...
 *
 * xm_arm64_disasm.h - ARM64 instruction disassembler for JIT debugging
 *
 * KEY CONCEPT:
...
#ifndef XM_ARM64_DISASM_H
#define XM_ARM64_DISASM_H
...
static const char *a64d_reg(uint32_t r, int is_sp) {
    static const char *names[] = {"x0",  "x1",  "x2",  "x3",  "x4",  "x5",  "x6",  "x7",
                                  "x8",  "x9",  "x10", "x11", "x12", "x13", "x14", "x15",
                                  "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23",
                                  "x24", "x25", "x26", "x27", "x28", "fp",  "lr",  "sp"};
    static const char *names_zr[] = {"x0",  "x1",  "x2",  "x3",  "x4",  "x5",  "x6",  "x7",
                                     "x8",  "x9",  "x10", "x11", "x12", "x13", "x14", "x15",
                                     "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23",
...
static const char *a64d_wreg(uint32_t r) {
    static const char *names[] = {"w0",  "w1",  "w2",  "w3",  "w4",  "w5",  "w6",  "w7",
...
static int a64_disasm_one(char *buf, int bufsz, uint32_t inst, uint32_t pc_offset) {
    ...
    if ((inst >> 24) == 0xb4 || (inst >> 24) == 0xb5) {
        int is_nz = (inst >> 24) & 1;
        int32_t off = (int32_t) a64d_sext((inst >> 5) & 0x7ffff, 19) * 4;
        return snprintf(buf, bufsz, "%s %s, 0x%x", is_nz ? "cbnz" : "cbz", a64d_reg(rd, 0),
                        pc_offset + off);
    }
    ...
    if ((opc & 0xfe) == 0xa8 ||  // GP 64-bit (X-register pairs)
        ...
        if (is_post)
            return snprintf(buf, bufsz, "%s %s, %s, [%s], #%d", is_load ? "ldp" : "stp", r1, r2t,
                            a64d_reg(rn, 1), imm7);
        else if (is_pre)
            return snprintf(buf, bufsz, "%s %s, %s, [%s, #%d]!", is_load ? "ldp" : "stp", r1, r2t,
                            a64d_reg(rn, 1), imm7);
    ...
    if ((opc & 0x7f) == 0x11 || (opc & 0x7f) == 0x51) {
        ...
        if (sf)
            return snprintf(buf, bufsz, "%s %s, %s, #%u%s", is_sub ? "sub" : "add", a64d_reg(rd, 1),
                            a64d_reg(rn, 1), imm12, sh ? ", lsl #12" : "");
        else
            return snprintf(buf, bufsz, "%s %s, %s, #%u%s", is_sub ? "sub" : "add", a64d_wreg(rd),
                            a64d_wreg(rn), imm12, sh ? ", lsl #12" : "");
    }
    ...
    if ((opc & 0x7f) == 0x31 || (opc & 0x7f) == 0x71) {
        ...
        uint32_t imm12 = (inst >> 10) & 0xfff;
        if (rd == 31) {
            return snprintf(buf, bufsz, "cmp %s, #%u", sf ? a64d_reg(rn, 1) : a64d_wreg(rn), imm12);
        }
        return snprintf(buf, bufsz, "%s %s, %s, #%u", is_sub ? "subs" : "adds",
                        sf ? a64d_reg(rd, 0) : a64d_wreg(rd), sf ? a64d_reg(rn, 1) : a64d_wreg(rn),
                        imm12);
    }
    ...
    if ((inst & 0x7f200000) == 0x0b000000) {
        ...
        if (S && rd == 31 && is_sub) {
            if (shift)
                return snprintf(buf, bufsz, "cmp %s, %s, %s #%u",
                                ...
                                sf ? a64d_reg(rm, 0) : a64d_wreg(rm), shn[shtype], shift);
            return snprintf(buf, bufsz, "cmp %s, %s", sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
        ...
        if (shift)
            return snprintf(buf, bufsz, "%s %s, %s, %s, %s #%u", op,
                            ...
                            sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm), shn[shtype], shift);
        return snprintf(buf, bufsz, "%s %s, %s, %s", op, sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
    }
    ...
    if ((inst & 0x1f800000) == 0x12800000) {
        ...
        return snprintf(buf, bufsz, "%s %s, #0x%x", mn, sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        imm16);
    }
    ...
    if ((inst & 0x1f000000) == 0x0a000000) {
        ...
        if (opc2 == 1 && !N && rn == 31 && shift == 0)
            return snprintf(buf, bufsz, "mov %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
        // MVN alias: ORN Xd, XZR, Xm
        if (opc2 == 1 && N && rn == 31 && shift == 0)
            return snprintf(buf, bufsz, "mvn %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
        // TST alias: ANDS XZR, Xn, Xm
        if (opc2 == 3 && rd == 31 && shift == 0)
            return snprintf(buf, bufsz, "tst %s, %s", sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
        if (shift)
            return snprintf(buf, bufsz, "%s%s %s, %s, %s, lsl #%u", op, N ? "n" : "",
                            sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm), shift);
        return snprintf(buf, bufsz, "%s%s %s, %s, %s", op, N ? "n" : "",
                        sf ? a64d_reg(rd, 0) : a64d_wreg(rd), sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                        sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
    }
    ...
    if ((inst & 0x7fe0fc00) == 0x1b000000) {
        ...
        int sf = (inst >> 31) & 1;
        if (ra == 31)
            return snprintf(buf, bufsz, "mul %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
        return snprintf(buf, bufsz, "madd %s, %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm),
    ...
    if ((inst & 0x7fe0fc00) == 0x1b008000) {
        ...
        int sf = (inst >> 31) & 1;
        if (ra == 31)
            return snprintf(buf, bufsz, "mneg %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            sf ? a64d_reg(rn, 0) : a64d_wreg(rn),
                            sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
        return snprintf(buf, bufsz, "msub %s, %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm),
                        sf ? a64d_reg(ra, 0) : a64d_wreg(ra));
    }
    if ((inst & 0x7fe0fc00) == 0x1ac00c00) {
        int sf = (inst >> 31) & 1;
        return snprintf(buf, bufsz, "sdiv %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
    }
    // LSLV/ASRV/LSRV
    if ((inst & 0x7fe0f000) == 0x1ac02000) {
        ...
        return snprintf(buf, bufsz, "%s %s, %s, %s", ops[op2], sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm));
    }
    ...
    if ((inst & 0x7fe00c00) == 0x1a800000) {
        ...
        if (rm == 31 && rn == 31 && cond != 14)
            return snprintf(buf, bufsz, "cset %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                            a64d_cond(cond ^ 1));
        return snprintf(buf, bufsz, "csel %s, %s, %s, %s", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
                        sf ? a64d_reg(rn, 0) : a64d_wreg(rn), sf ? a64d_reg(rm, 0) : a64d_wreg(rm),
                        a64d_cond(cond));
    }
    ...
    if ((inst & 0xffc00000) == 0xf9400000) {
        ...
        return snprintf(buf, bufsz, "ldr %s, [%s, #%u]", a64d_reg(rd, 0), a64d_reg(rn, 1), imm12);
    }
    if ((inst & 0xffc00000) == 0xf9000000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 8;
        return snprintf(buf, bufsz, "str %s, [%s, #%u]", a64d_reg(rd, 0), a64d_reg(rn, 1), imm12);
    }

    // LDR/STR unsigned offset (32-bit W)
    if ((inst & 0xffc00000) == 0xb9400000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 4;
        return snprintf(buf, bufsz, "ldr %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }
    if ((inst & 0xffc00000) == 0xb9000000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 4;
        return snprintf(buf, bufsz, "str %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }

    // LDRB / STRB unsigned offset
    if ((inst & 0xffc00000) == 0x39400000) {
        uint32_t imm12 = (inst >> 10) & 0xfff;
        return snprintf(buf, bufsz, "ldrb %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }
    if ((inst & 0xffc00000) == 0x39000000) {
        uint32_t imm12 = (inst >> 10) & 0xfff;
        return snprintf(buf, bufsz, "strb %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }

    // LDRH / STRH unsigned offset
    if ((inst & 0xffc00000) == 0x79400000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 2;
        return snprintf(buf, bufsz, "ldrh %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }
    if ((inst & 0xffc00000) == 0x79000000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 2;
        return snprintf(buf, bufsz, "strh %s, [%s, #%u]", a64d_wreg(rd), a64d_reg(rn, 1), imm12);
    }
    ...
    if ((inst & 0x7f800000) == 0x53000000) {
        ...
        if (!sf && imms == 0x1f)
            return snprintf(buf, bufsz, "lsr %s, %s, #%u", a64d_wreg(rd), a64d_wreg(rn), immr);
        if (sf && imms == 0x3f)
            return snprintf(buf, bufsz, "lsr %s, %s, #%u", a64d_reg(rd, 0), a64d_reg(rn, 0), immr);
        return snprintf(buf, bufsz, "ubfm %s, %s, #%u, #%u", sf ? a64d_reg(rd, 0) : a64d_wreg(rd),
    ...
    if ((inst & 0x9f000000) == 0x90000000) {
        ...
        return snprintf(buf, bufsz, "adrp %s, #0x%llx", a64d_reg(rd, 0),
                        (long long) (pc_offset + imm));
    }

    // FP: LDR/STR Dt (64-bit FP, unsigned offset)
    if ((inst & 0xffc00000) == 0xfd400000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 8;
        return snprintf(buf, bufsz, "ldr d%u, [%s, #%u]", rd, a64d_reg(rn, 1), imm12);
    }
    if ((inst & 0xffc00000) == 0xfd000000) {
        uint32_t imm12 = ((inst >> 10) & 0xfff) * 8;
        return snprintf(buf, bufsz, "str d%u, [%s, #%u]", rd, a64d_reg(rn, 1), imm12);
    }
