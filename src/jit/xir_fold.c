...
static bool fold_get_const_i64(XirFunc *func, XirRef ref, int64_t *out) {
    ...
    if (cidx >= func->nconst) return false;
...
static bool fold_get_const_f64(XirFunc *func, XirRef ref, double *out) {
    ...
    if (idx >= func->nvreg) return false;
    XirIns *def = func->vregs[idx].def;
    if (!def || def->op != XIR_CONST_F64) return false;
    if (!xir_ref_is_const(def->args[0])) return false;
    uint32_t cidx = XIR_REF_INDEX(def->args[0]);
    if (cidx >= func->nconst) return false;
...
static uint16_t fold_def_op(XirFunc *func, XirRef ref) {
    if (!xir_ref_is_vreg(ref)) return XIR_NOP;
    uint32_t idx = XIR_REF_INDEX(ref);
    if (idx >= func->nvreg) return XIR_NOP;
    XirIns *def = func->vregs[idx].def;
...
static XirRef fold_def_arg0(XirFunc *func, XirRef ref) {
    if (!xir_ref_is_vreg(ref)) return XIR_NONE;
    uint32_t idx = XIR_REF_INDEX(ref);
    if (idx >= func->nvreg) return XIR_NONE;
    XirIns *def = func->vregs[idx].def;
 ...
 */
static bool fold_int_binary(XirFunc *func, XirBlock *blk,
                            uint16_t op, XirRef a, XirRef b,
                            XirRef *result) {
    ...
    if (b_const) {
        ...
        case XIR_SHL:
        case XIR_SHR:
            if (vb == 0) { *result = a; return true; }
            break;
        case XIR_MUL:
            if (vb == 1) { *result = a; return true; }
            if (vb == 0) {
        ...
        case XIR_DIV:
            ...
            break;
        case XIR_AND:
            if (vb == -1) { *result = a; return true; }  // x & 0xFFFFFFFFFFFFFFFF
            if (vb == 0) {
                XirRef cref = xir_const_i64(func, 0);
                *result = xir_emit_unary(func, blk, XIR_CONST_I64, XR_REP_I64, cref);
                return true;
    ...
    if (a_const) {
        ...
        case XIR_OR:
        case XIR_XOR:
            if (va == 0) { *result = b; return true; }
            break;
        case XIR_MUL:
            if (va == 1) { *result = b; return true; }
            if (va == 0) {
                XirRef cref = xir_const_i64(func, 0);
                *result = xir_emit_unary(func, blk, XIR_CONST_I64, XR_REP_I64, cref);
                return true;
    ...
    if (a_const && b_const) {
        ...
        case XIR_SUB: r = va - vb; break;
        case XIR_MUL: r = va * vb; break;
        case XIR_DIV: if (vb == 0) { ok = false; break; } r = va / vb; break;
        case XIR_MOD: if (vb == 0) { ok = false; break; } r = va % vb; break;
        case XIR_AND: r = va & vb; break;
        case XIR_OR:  r = va | vb; break;
        case XIR_XOR: r = va ^ vb; break;
        case XIR_SHL: r = va << (vb & 63); break;
        case XIR_SHR: r = (int64_t)((uint64_t)va >> (vb & 63)); break;
        case XIR_EQ:  r = (va == vb) ? 1 : 0; break;
        case XIR_NE:  r = (va != vb) ? 1 : 0; break;
        case XIR_LT:  r = (va < vb)  ? 1 : 0; break;
        case XIR_LE:  r = (va <= vb) ? 1 : 0; break;
        case XIR_GT:  r = (va > vb)  ? 1 : 0; break;
        case XIR_GE:  r = (va >= vb) ? 1 : 0; break;
...
static bool fold_float_binary(XirFunc *func, XirBlock *blk,
                              uint16_t op, XirRef a, XirRef b,
    ...
    if (b_const) {
        ...
        case XIR_FADD:
        case XIR_FSUB:
            if (vb == 0.0) { *result = a; return true; }
            break;
        case XIR_FMUL:
            if (vb == 1.0) { *result = a; return true; }
            break;
        case XIR_FDIV:
            if (vb == 1.0) { *result = a; return true; }
            break;
    ...
    if (a_const) {
        switch (op) {
        case XIR_FADD:
            if (va == 0.0) { *result = b; return true; }
            break;
        case XIR_FMUL:
            if (va == 1.0) { *result = b; return true; }
            break;
        default:
            break;
        }
    ...
    if (a_const && b_const) {
        ...
        case XIR_FSUB: r = va - vb; break;
        case XIR_FMUL: r = va * vb; break;
        case XIR_FDIV:
            if (vb == 0.0) { ok = false; break; }
            r = va / vb; break;
        default: ok = false; break;
        }
        if (ok) {
        ...
        case XIR_FNE: cr = (va != vb) ? 1 : 0; break;
        case XIR_FLT: cr = (va < vb)  ? 1 : 0; break;
        case XIR_FLE: cr = (va <= vb) ? 1 : 0; break;
        default: cmp_ok = false; break;
        }
        if (cmp_ok) {
...
XirRef xir_fold_emit(XirFunc *func, XirBlock *blk,
    ...
    if ((op >= XIR_FADD && op <= XIR_FDIV) ||
        (op >= XIR_FEQ && op <= XIR_FLE)) {
    ...
    if (op == XIR_NEG || op == XIR_NOT || op == XIR_FNEG ||
        op == XIR_I2F || op == XIR_F2I) {
...
XirRef xir_fold_emit_unary(XirFunc *func, XirBlock *blk,
