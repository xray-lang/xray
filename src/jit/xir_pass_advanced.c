 ...
 */
XirRef xir_inline_function(XirFunc *caller, XirBlock *call_block, uint32_t call_ins_idx,
...
#define REMAP_REF(r)                                                                               \
    ...
    for (uint32_t i = post_start; i < call_block->nins; i++) {
        ...
        xir_emit_raw(caller, cont_blk, src->op, src->rep, src->dst, src->args[0], src->args[1]);
    ...
    for (uint32_t bi = 0; bi < callee->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < src_blk->nins; i++) {
            ...
            xir_emit_raw(caller, dst_blk, ins.op, ins.rep, ins.dst, ins.args[0], ins.args[1]);
        ...
        switch (src_blk->jmp.type) {
            ...
            case XIR_JMP_BR: {
                ...
                if (s1_id >= callee->nblk || s2_id >= callee->nblk)
                    break;
                xir_block_set_br(dst_blk, cond, cloned_blocks[s1_id], cloned_blocks[s2_id]);
...
static bool sf_needs_barrier(XirFunc *func, XirIns *ins) {
    ...
    if (ins->flags & XIR_FLAG_NO_BARRIER)
        return false;

    uint8_t tag = ins->rep;
    if (tag == XR_TAG_PTR)
        return true;
    if (tag != 0xFF /* XIR_SF_TAG_RUNTIME */)
        return false;
...
XirPassChange xir_insert_write_barriers(XirFunc *func) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            if (blk->ins[i].op == XIR_STORE_FIELD && sf_needs_barrier(func, &blk->ins[i]))
                barrier_count++;
        }
        if (barrier_count == 0)
            continue;
        ...
        XirIns *new_ins = (XirIns *) xir_arena_alloc(func->arena, new_nins * sizeof(XirIns));
        if (!new_ins)
            continue;
    ...
    return total_barriers ? (XirPassChange){false, true, false, 0, 0, 0} : xir_pass_no_change();
}
...
static bool xir_ref_uses_vreg(XirRef a0, XirRef a1, XirRef r) {
    return (xir_ref_is_vreg(a0) && a0 == r) || (xir_ref_is_vreg(a1) && a1 == r);
}
...
#define ARC_MAX_LOCAL 64
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            if (ins->op == XIR_STORE_FIELD)
                stored = ins->args[1];
            else if (ins->op == XIR_RT_INDEX_SET)
                stored = ins->args[1];
            else if (ins->op == XIR_RT_ARRAY_PUSH)
                stored = ins->args[1];
            if (xir_ref_is_none(stored) || !xir_ref_is_vreg(stored))
                continue;
            for (uint32_t j = 0; j < nloc; j++) {
                if (loc_ref[j] == stored) {
                    loc_skip[j] = true;
        ...
        for (uint32_t j = 0; j < nloc; j++) {
            ...
            if (last_use[j] < 0) {
                ...
                for (uint32_t ob = 0; ob < func->nblk && !used_elsewhere; ob++) {
                    ...
                    if (!xir_ref_is_none(other->jmp.arg) && other->jmp.arg == loc_ref[j])
                        used_elsewhere = true;
                }
                if (used_elsewhere) {
                    loc_skip[j] = true;
    ...
    for (uint32_t g = 0; g < ng; g++) {
        ...
        for (uint32_t bi2 = 0; bi2 < func->nblk; bi2++) {
            ...
            for (uint32_t i = 0; i < blk2->nins; i++) {
                ...
                if (!xir_ref_uses_vreg(ins->args[0], ins->args[1], ref))
                    continue;
                lu = (int32_t) i;
            }
            // Check JMP arg
        ...
        if (tblk->jmp.type == XIR_JMP_RET && !xir_ref_is_none(tblk->jmp.arg) &&
            tblk->jmp.arg == ref)
            continue;
        ...
        for (uint32_t bi2 = 0; bi2 < func->nblk && !transferred; bi2++) {
            ...
            for (uint32_t i = 0; i < blk2->nins && !transferred; i++) {
                ...
                if (ins->op == XIR_STORE_FIELD)
                    ...
                    stored = ins->args[1];
                if (!xir_ref_is_none(stored) && stored == ref)
                    transferred = true;
            }
        }
        if (transferred)
...
#undef ARC_MAX_GLOBAL
    return any_inserted ? (XirPassChange){false, true, false, 0, 0, 0} : xir_pass_no_change();
}
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            for (int a = 0; a < 2; a++) {
                ...
                if (a == 0 && (ins->op == XIR_LOAD_FIELD || ins->op == XIR_STORE_FIELD ||
                               ins->op == XIR_GUARD_SHAPE || ins->op == XIR_BARRIER_FWD))
                    continue;
...
static bool ea_scalar_replace(XirFunc *func, const EaAlloc *a) {
    ...
    for (uint32_t i = 0; i < blk->nins; i++) {
        ...
        if (ins->op == XIR_STORE_FIELD && xir_ref_is_vreg(ins->args[0]) && ins->args[0] == aref) {
            int32_t off = 0;
        ...
        if (ins->op == XIR_LOAD_FIELD && xir_ref_is_vreg(ins->args[0]) && ins->args[0] == aref) {
            int32_t off = 0;
            if (xir_ref_is_const(ins->args[1])) {
                uint32_t ci = XIR_REF_INDEX(ins->args[1]);
                if (ci < func->nconst)
        ...
        if (ins->op == XIR_GUARD_SHAPE && xir_ref_is_vreg(ins->args[0]) && ins->args[0] == aref) {
            ins->op = XIR_NOP;
            ins->dst = XIR_NONE;
            ins->args[0] = XIR_NONE;
            ins->args[1] = XIR_NONE;
        ...
        if (ins->op == XIR_BARRIER_FWD && xir_ref_is_vreg(ins->args[0]) && ins->args[0] == aref) {
            ins->op = XIR_NOP;
            ins->dst = XIR_NONE;
            ins->args[0] = XIR_NONE;
            ins->args[1] = XIR_NONE;
...
XirPassChange xir_pass_escape_analysis(XirFunc *func) {
    ...
    xr_free(allocs);
    return n_replaced ? (XirPassChange){false, true, true, n_replaced, 0, 0} : xir_pass_no_change();
}
...
XirPassChange xir_pass_alloc_sink(XirFunc *func) {
    ...
    for (uint32_t bi = 0; bi < nblk; bi++) {
        ...
        for (uint32_t ii = 0; ii < blk->nins; ii++) {
            ...
            if (blk->jmp.type != XIR_JMP_BR || !blk->s1 || !blk->s2)
                continue;
            ...
            for (uint32_t u = 0; u < nuse; u++) {
                ...
                if (!xir_dom_covers(dt, s1_id, ub))
                    all_in_s1 = false;
                if (!xir_dom_covers(dt, s2_id, ub))
                    all_in_s2 = false;
            }
    ...
    return n_sunk ? (XirPassChange){false, true, false, n_sunk, 0, 0} : xir_pass_no_change();
}
    ...
    while (did_inline && inlined_count < max_inline && inlined_size < max_inlined_size) {
        ...
        for (uint32_t bi = 0; bi < func->nblk && !did_inline; bi++) {
            ...
            for (uint32_t ii = 0; ii < blk->nins && !did_inline; ii++) {
                ...
                // --- Inline ---
                XirRef result =
                    xir_inline_function(func, blk, ii, callee_func, call_args, (uint32_t) nargs);
...
#undef MAX_INLINE_CALLEES
    return inlined_count > 0 ? (XirPassChange){true, true, true, 0, 0, 0} : xir_pass_no_change();
}
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            if (ins->op == XIR_GUARD_NONNULL && xir_ref_is_vreg(ins->args[0])) {
                ...
                XirType gct = xir_ref_ctype(func, ins->args[0]);
                if (vi < func->nvreg &&
                    (xir_type_is_ptr(gct.kind) || func->vregs[vi].is_fresh_alloc)) {
                    n_elim++;
                    ins->op = XIR_NOP;
                    ins->dst = XIR_NONE;
                    ins->args[0] = XIR_NONE;
...
XirPassChange xir_pass_propjnz(XirFunc *func) {
    ...
    return any_prop ? (XirPassChange){false, false, true, 0, 0, 0} : xir_pass_no_change();
}
...
static uint32_t gcm_early(XirFunc *func, uint32_t *idom, uint32_t *depth, uint32_t *early,
...
XirPassChange xir_pass_gcm(XirFunc *func) {
    if (!func || func->nblk < 2 || func->nvreg == 0)
        return xir_pass_no_change();
    if (func->nblk > XIR_MAX_FUNC_BLOCKS || func->nvreg > XIR_MAX_FUNC_VREGS)
        return xir_pass_no_change();
                    ...
                    for (int a = 0; a < 2; a++) {
                        ...
                        for (uint32_t j = i + 1; j < blk->nins; j++) {
                            if (xir_ref_is_vreg(blk->ins[j].dst) &&
                                XIR_REF_INDEX(blk->ins[j].dst) == av && blk->ins[j].op != XIR_NOP) {
                                if (out < cap)
