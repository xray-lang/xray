 ...
 *
 * xm_tfa.c - Type Flow Analysis implementation
 *
 * KEY CONCEPT:
...
TfaSummary *tfa_register_func(TfaState *tfa, XrProto *proto) {
    ...
    uint8_t np = (proto->numparams < TFA_MAX_PARAMS) ? (uint8_t) proto->numparams : TFA_MAX_PARAMS;
    s->nparam = np;
...
static TfaSummary *tfa_dequeue(TfaState *tfa) {
    if (tfa->worklist_head >= tfa->worklist_tail)
        return NULL;
...
void tfa_add_call(TfaState *tfa, TfaSummary *caller, TfaSummary *callee, XrType *const *arg_types,
                  uint8_t nargs) {
    if (!tfa || !caller || !callee)
        return;
    if (tfa->ncall >= tfa->call_cap) {
        if (!tfa_grow_calls(tfa))
            return;
    }
...
void tfa_apply_results(TfaState *tfa) {
    ...
    for (uint32_t i = 0; i < tfa->nsummary; i++) {
        ...
        if (proto->numparams > 0) {
            ...
            uint8_t np =
                (proto->numparams < TFA_MAX_PARAMS) ? (uint8_t) proto->numparams : TFA_MAX_PARAMS;
        ...
        switch (op) {
            ...
            case OP_CALL_KEEP: {
                ...
                if (callee_proto) {
                    ...
                    if (!callee)
                        callee = tfa_register_func(tfa, callee_proto);
                    if (callee && caller) {
...
void tfa_dump_stats(TfaState *tfa) {
    ...
    fprintf(stderr,
            "[TFA] functions: %u, call sites: %u, "
            "iterations: %u, types refined: %u\n",
            tfa->nsummary, tfa->ncall, tfa->iterations, tfa->types_refined);
