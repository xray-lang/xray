...
static void emit_xm_ins(CodegenCtx *ctx, XmIns *ins) {
    XR_DCHECK(ctx != NULL, "emit_xm_ins: NULL ctx");
    XR_DCHECK(ins != NULL, "emit_xm_ins: NULL ins");
    ...
    switch (ins->op) {
        ...
        case XM_MOD: {
            ...
            A64Reg rn = xra_operand(ctx, ins->args[0], SCRATCH_REG);
            A64Reg rm = xra_operand(ctx, ins->args[1], SCRATCH_REG2);
            // Division by zero guard (same as XM_DIV)
            add_patch(ctx, PATCH_DEOPT_CBZ, 0, rm);
            a64_buf_emit(&ctx->buf, a64_nop());  // patched to CBZ rm, deopt
            ctx->has_deopt = true;
        ...
        default:
            ...
            if (xm_emit_mem_ops(ctx, ins, rd))
                break;
            xr_log_warning("jit", "unhandled Xm opcode %d in emit_xm_ins", ins->op);
...
static void emit_osr_stubs(CodegenCtx *ctx, XmCodegenResult *result) {
...
static void build_runtime_deopt_table(CodegenCtx *ctx, XmCodegenResult *result) {
 ...
 */
static void emit_resume_entry(CodegenCtx *ctx, XmCodegenResult *result) {
    if (ctx->nsuspend == 0)
        return;
