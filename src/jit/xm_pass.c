 ...
 *
 * xm_pass.c - Xm optimization passes implementation
 *
 * KEY CONCEPT:
 *   Each pass operates on XmFunc in-place. Passes are composable
 ...
 */
static inline bool dce_is_removable(const XmIns *ins) {
    if (!xm_ref_is_vreg(ins->dst))
        return false;
    if (ins->flags & (XM_FLAG_SIDE_EFFECT | XM_FLAG_MAY_THROW))
        return false;
    if (xm_op_has_side_effect(ins->op))
        return false;
    return true;
}
 ...
 */
static inline void dce_dec_use(uint32_t *use_count, XmRef ref, uint32_t nvreg, uint32_t *wl,
                               uint32_t *wl_top, const uint8_t *removable) {
    if (!xm_ref_is_vreg(ref))
        return;
    uint32_t v = XM_REF_INDEX(ref);
    if (v >= nvreg)
        return;
    if (use_count[v] > 0)
        use_count[v]--;
...
XmPassChange xm_pass_dce(XmFunc *func) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (XmPhi *phi = blk->phis; phi; phi = phi->next) {
            if (xm_ref_is_vreg(phi->dst)) {
                removable[XM_REF_INDEX(phi->dst)] = 1;
            }
        }
        for (uint32_t i = 0; i < blk->nins; i++) {
            XmIns *ins = &blk->ins[i];
            if (dce_is_removable(ins)) {
                removable[XM_REF_INDEX(ins->dst)] = 1;
            }
        }
    }
 ...
 * args[0] <= args[1] (by XmRef numerical value). This makes
 * ADD(v5,v2) hash the same as ADD(v2,v5). */
static void gvn_normalize(XmIns *ins) {
    if (xm_op_is_commutative(ins->op) && ins->args[0] > ins->args[1]) {
        XmRef tmp = ins->args[0];
...
/* Table sizing bounds come from xm_pass_limits.h; aliased here for
 * readability of the local CSE code. */
#define CSE_MIN_TABLE XM_CSE_MIN_TABLE
#define CSE_MAX_TABLE XM_CSE_MAX_TABLE
...
typedef struct {
    ...
    XmRef arg0;
    XmRef arg1;
    XmRef result;  // dst vreg of the first occurrence
} CseEntry;

static uint32_t cse_hash(uint16_t op, XmRef a0, XmRef a1) {
...
XmPassChange xm_pass_cse(XmFunc *func) {
    if (!func)
        return xm_pass_no_change();
...
/* Helper: if vreg is defined by XM_CONST_I64, return its i64 value.
 * Returns false if not a constant. */
static bool gvn_get_const_i64(XmFunc *func, XmRef ref, int64_t *out) {
    if (!xm_ref_is_vreg(ref))
        return false;
...
static bool gvn_assoc_const(XmFunc *func, XmIns *ins, const XmDefUse *du) {
    if (!xm_ref_is_vreg(ins->dst))
        return false;
    if (!du)
        return false;
    ...
    uint16_t op = ins->op;
    if (op != XM_ADD && op != XM_MUL && op != XM_AND && op != XM_OR && op != XM_XOR)
        return false;
    ...
    if (!xm_ref_is_vreg(ins->args[var_slot]))
        return false;

    // Check if varg is defined by the same op with one constant arg
    uint32_t vidx = XM_REF_INDEX(ins->args[var_slot]);
    if (vidx >= func->nvreg)
        return false;
    XmIns *vdef = func->vregs[vidx].def;
    if (!vdef || vdef->op != op)
        return false;
    ...
    if (inner_const_slot < 0)
        return false;
    XmRef inner_var = vdef->args[1 - inner_const_slot];
    ...
    uint32_t const_vreg = XM_REF_INDEX(ins->args[const_slot]);
    if (const_vreg >= func->nvreg)
        return false;
    if (xm_defuse_nuses(du, const_vreg) != 1)
        return false;
    ...
    XmRef new_cref = xm_const_i64(func, combined);
    XmIns *cdef = func->vregs[const_vreg].def;
    if (cdef && cdef->op == XM_CONST_I64) {
...
#define GVN_MIN_TABLE XM_GVN_MIN_TABLE
...
XmPassChange xm_pass_gvn(XmFunc *func) {
    if (!func || func->nblk < 2)
        return xm_pass_no_change();
     ...
     * functions still have probe room; XM_MAX_FUNC_TOTAL_INS bounds
    ...
    const XmDefUse *du = xm_func_get_defuse(func);
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            for (uint32_t probe = 0; probe < gvn_tsize; probe++) {
                ...
                if (e->key == h && e->op == ins->op && e->arg0 == ins->args[0] &&
                    ...
                    if (xm_dom_covers(dt, e->def_blk, bi)) {
                        ins->op = XM_MOV;
                        ins->args[0] = e->result;
                        ins->args[1] = XM_NONE;
                        n_replaced++;
...
static bool s2l_get_offset(XmFunc *func, XmRef ref, int64_t *out) {
...
// Look up (obj, offset) in the S2L table. Returns value vreg or XM_NONE.
static XmRef s2l_lookup(const S2lTable *t, XmRef obj, int64_t offset) {
...
static S2lEntry *s2l_find(S2lTable *t, XmRef obj, int64_t offset) {
    uint32_t h = ((uint32_t) obj * 31 + (uint32_t) offset) & t->mask;
    for (uint32_t probe = 0; probe < t->size; probe++) {
        uint32_t idx = (h + probe) & t->mask;
        S2lEntry *e = &t->entries[idx];
...
static void s2l_insert(S2lTable *t, XmRef obj, int64_t offset, XmRef value, bool is_store,
 ...
 * for probing.  Max capped at the central XM_MAX_FUNC_TOTAL_INS to
 * bound worst-case allocation. */
static uint32_t s2l_table_size(XmFunc *func) {
    uint32_t fields = 0;
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        XmBlock *blk = func->blocks[bi];
        if (!blk)
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            if (op == XM_LOAD_FIELD || op == XM_STORE_FIELD)
                fields++;
        }
    }
    ...
    while (size < target)
        size <<= 1;
    if (size > XM_MAX_FUNC_TOTAL_INS)
        size = XM_MAX_FUNC_TOTAL_INS;
    return size;
}

XmPassChange xm_pass_store_to_load(XmFunc *func) {
    if (!func || func->nblk == 0)
        return xm_pass_no_change();
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
                ...
                if (s2l_get_offset(func, ins->dst, &offset)) {
                    ...
                    if (prev && prev->is_store) {
                        ...
                        if (dead_blk && prev->store_idx < dead_blk->nins) {
                            XmIns *dead = &dead_blk->ins[prev->store_idx];
                            n_fwd++;
                            dead->op = XM_MOV;
                            dead->dst = XM_NONE;
                            dead->args[0] = XM_NONE;
            ...
            if (ins->op == XM_LOAD_FIELD) {
                ...
                XmRef fwd = s2l_lookup(&t, obj, offset);
                if (!xm_ref_is_none(fwd)) {
                ...
                s2l_insert(&t, obj, offset, xm_ref_is_none(fwd) ? ins->dst : fwd, false, bi, i);
                continue;
            }
...
#define IFCONV_MAX_INS XM_IFCONV_MAX_INS
#define IFCONV_MAX_PHIS XM_IFCONV_MAX_PHIS

static bool ifconv_ok_branch(XmBlock *blk) {
    int n = 0;
    for (uint32_t i = 0; i < blk->nins; i++) {
        XmIns *ins = &blk->ins[i];
        if (ins->op == XM_NOP || xm_op_is_copy(ins->op))
            continue;
        if (ins->flags & (XM_FLAG_SIDE_EFFECT | XM_FLAG_MAY_THROW))
            return false;
        if (!xm_op_is_pure(ins->op) && ins->op != XM_CONST_I64 && ins->op != XM_CONST_F64 &&
            ins->op != XM_CONST_PTR)
            return false;
        n++;
    }
...
static bool ifconv_ok_join(XmBlock *blk) {
    int n = 0;
...
static XmRef ifconv_phi_arg(XmPhi *phi, XmBlock *pred, XmBlock *joinblk) {
...
XmPassChange xm_pass_ifconvert(XmFunc *func) {
    if (!func || func->nblk < 3)
        return xm_pass_no_change();
    ...
    for (int round = 0; round < IFCONV_MAX_ROUNDS; round++) {
        ...
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            XmBlock *ifblk = func->blocks[bi];

            // Must be a conditional branch
            if (ifblk->jmp.type != XM_JMP_BR)
                continue;
            ...
            XmBlock *thenblk = ifblk->s1;
            XmBlock *elseblk = ifblk->s2;

            // Both branches must JMP to the same join block
            if (thenblk->jmp.type != XM_JMP_JMP)
                continue;
            if (elseblk->jmp.type != XM_JMP_JMP)
                continue;
            if (thenblk->s1 != elseblk->s1)
                continue;

            XmBlock *joinblk = thenblk->s1;
            ...
            for (XmPhi *p = joinblk->phis; p; p = p->next) {
                XmRef true_val = ifconv_phi_arg(p, thenblk, joinblk);
                XmRef false_val = ifconv_phi_arg(p, elseblk, joinblk);
                if (xm_ref_is_none(true_val) || xm_ref_is_none(false_val))
                    continue;

                // Determine type from phi dst
                uint8_t sel_type = XR_REP_I64;
                if (xm_ref_is_vreg(p->dst)) {
                    uint32_t idx = XM_REF_INDEX(p->dst);
                    if (idx < func->nvreg)
                        sel_type = func->vregs[idx].rep;
                }

                xm_emit_raw(func, ifblk, XM_SELECT, sel_type, p->dst, true_val, false_val);
            }

            // Rewire: ifblk JMP → joinblk
            ifblk->jmp.type = XM_JMP_JMP;
            ifblk->jmp.arg = XM_NONE;
            ifblk->s1 = joinblk;
            ifblk->s2 = NULL;

            // Clear then/else blocks (will be cleaned by remove_unreachable)
            thenblk->nins = 0;
            thenblk->jmp.type = XM_JMP_NONE;
            thenblk->s1 = NULL;
            elseblk->nins = 0;
            elseblk->jmp.type = XM_JMP_NONE;
            elseblk->s1 = NULL;
 ...
 *
 * Drives off the cached XmLoopInfo (xm_looptree.h), so it no longer
 ...
 * deleted) XmBlock::loop_depth field.  The central pass budget
 * XM_LICM_MAX_ITERATIONS bounds chain-invariant propagation; every
...
static void licm_build_def_block(XmFunc *func, uint32_t *db, uint32_t nv) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++)
            if (xm_ref_is_vreg(blk->ins[i].dst)) {
                uint32_t idx = XM_REF_INDEX(blk->ins[i].dst);
                if (idx < nv)
                    db[idx] = bi;
            }
    }
 ...
 * |db|.
 */
static bool licm_ref_outside(XmRef ref, const uint32_t *db, uint32_t nv, const uint8_t *in_loop,
                             uint32_t nblk) {
    if (!xm_ref_is_vreg(ref))
        return true;
    uint32_t idx = XM_REF_INDEX(ref);
...
static uint32_t licm_block_index(XmFunc *func, const XmBlock *blk) {
 ...
 */
static bool licm_definitely_no_alias(XmFunc *func, XmRef a, XmRef b) {
    if (a == b)
        return false;
    const XmAliasInfo *ia = xm_func_get_alias(func, a);
    const XmAliasInfo *ib = xm_func_get_alias(func, b);
    if (!ia || !ib)
        return false;

    if (ia->source == XM_ALIAS_FRESH_ALLOC && ib->source == XM_ALIAS_FRESH_ALLOC)
        return ia->origin != ib->origin;

    if ((ia->source == XM_ALIAS_FRESH_ALLOC && ib->source == XM_ALIAS_PARAM) ||
        (ib->source == XM_ALIAS_FRESH_ALLOC && ia->source == XM_ALIAS_PARAM))
        return true;

    return false;
}

XmPassChange xm_pass_licm(XmFunc *func) {
    if (!func || func->nblk < 2 || func->nvreg == 0)
        return xm_pass_no_change();

    const XmLoopInfo *loops = xm_func_get_loops(func);
    ...
    /* all_loops is already sorted innermost-first by xm_looptree, so
    ...
    for (uint32_t li = 0; li < loops->nloop; li++) {
        XmLoop *L = loops->all_loops[li];
        if (!L->preheader)
            continue;  // no unique preheader → skip

        /* Rebuild the body bitmap from XmLoop::body[]. */
        ...
        for (uint32_t i = 0; i < L->nbody; i++) {
            ...
            for (uint32_t k = 0; k < lb->nins; k++) {
                XmIns *ins = &lb->ins[k];
                if (ins->op != XM_STORE_FIELD || !xm_ref_is_vreg(ins->args[0]))
                    continue;
                XmRef obj = ins->args[0];
                int64_t off = 0;
                bool has_off = false;
        ...
        for (int iter = 0; iter < XM_LICM_MAX_ITERATIONS; iter++) {
            bool hoisted_any = false;

            for (uint32_t i = 0; i < L->nbody; i++) {
                XmBlock *loop_blk = L->body[i];
                if (loop_blk == preheader)
                    continue;
                uint32_t write = 0;

                for (uint32_t k = 0; k < loop_blk->nins; k++) {
                    XmIns *ins = &loop_blk->ins[k];
                    bool can_hoist = false;

                    if (xm_op_is_pure(ins->op) && xm_ref_is_vreg(ins->dst)) {
                        can_hoist = true;
                        for (int a = 0; a < 2; a++) {
                    ...
                    } else if (ins->op == XM_LOAD_FIELD && xm_ref_is_vreg(ins->dst)) {
                        XmRef obj = ins->args[0];
 ...
 */
XmPassChange xm_pass_canonicalize(XmFunc *func) {
    if (!func || func->nvreg == 0)
        return xm_pass_no_change();
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (uint32_t i = 0; i < blk->nins; i++) {
            ...
            XmRef prev_a0 = ins->args[0], prev_a1 = ins->args[1];
            ...
            switch (ins->op) {
                ...
                case XM_FADD:
                case XM_FMUL:
                    if (xm_ref_is_vreg(ins->args[0]) && xm_ref_is_vreg(ins->args[1])) {
                ...
                case XM_NEG: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_NEG && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                ...
                case XM_NOT: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_NOT && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->args[0] = adef->args[0];
                    }
                    break;
                ...
                case XM_FNEG: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_FNEG && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->rep = XR_REP_F64;
                        ins->args[0] = adef->args[0];
                    }
                ...
                case XM_UNBOX_I64: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_BOX_I64 && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->args[0] = adef->args[0];
                    }
                    break;
                ...
                case XM_UNBOX_F64: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_BOX_F64 && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->rep = XR_REP_F64;
                        ins->args[0] = adef->args[0];
                    }
                ...
                case XM_BOX_I64: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_UNBOX_I64 && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                ...
                case XM_BOX_F64: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_UNBOX_F64 && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->rep = XR_REP_TAGGED;
                        ins->args[0] = adef->args[0];
                    }
                ...
                case XM_FLE: {
                    ...
                    if (xm_op_is_commutative(ins->op) && xm_ref_is_vreg(ins->args[0]) &&
                        xm_ref_is_vreg(ins->args[1]) && ins->args[0] > ins->args[1]) {
                        XmRef tmp = ins->args[0];
                        ins->args[0] = ins->args[1];
                        ins->args[1] = tmp;
                    }
                ...
                case XM_REDEFINE: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && xm_op_is_copy(adef->op) && xm_ref_is_vreg(adef->args[0])) {
                        ins->args[0] = adef->args[0];
                    }
                    break;
                }
                ...
                case XM_I2F: {
                    ...
                    if (ai >= func->nvreg)
                        break;
                    XmIns *adef = func->vregs[ai].def;
                    if (adef && adef->op == XM_F2I && xm_ref_is_vreg(adef->args[0])) {
                        ins->op = XM_MOV;
                        ins->rep = XR_REP_F64;
                        ins->args[0] = adef->args[0];
                    }
                ...
                case XM_FSUB: {
                    if (ins->args[0] == ins->args[1]) {
                        XmRef zero = xm_const_f64(func, 0.0);
                        ins->op = XM_CONST_F64;
                        ins->rep = XR_REP_F64;
                        ins->args[0] = zero;
                        ins->args[1] = XM_NONE;
...
XmPassChange xm_pass_dse(XmFunc *func) {
    if (!func)
        return xm_pass_no_change();
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (int32_t i = (int32_t) blk->nins - 1; i >= 0; i--) {
            ...
            if (ins->op == XM_LOAD_FIELD) {
                XmRef lobj = ins->args[0];
                XmRef loff = ins->dst;  // LOAD_FIELD dst encodes offset
            ...
            if (ins->op == XM_STORE_FIELD) {
                XmRef sobj = ins->args[0];
                XmRef soff = ins->dst;  // packed (tag, offset)
...
#ifndef NDEBUG
/* Count instructions with XM_FLAG_SIDE_EFFECT.  Used by the
 * XM_PASS_VERIFY_SE check: passes like DCE/CSE/GVN must never
 * reduce this count (doing so means a side-effectful instruction
 * was incorrectly removed or merged). */
static uint32_t xm_count_se_(XmFunc *fn) {
    uint32_t n = 0;
    for (uint32_t bi = 0; bi < fn->nblk; bi++)
        for (uint32_t ii = 0; ii < fn->blocks[bi]->nins; ii++)
            if (fn->blocks[bi]->ins[ii].flags & XM_FLAG_SIDE_EFFECT)
                n++;
    return n;
}
#endif
...
static XmPassChange fixedpoint_invoke(XmFunc *func, XrProto *proto, const XmPassDesc *d) {
#ifndef NDEBUG
    uint32_t se_pre = (d->flags & XM_PASS_VERIFY_SE) ? xm_count_se_(func) : 0;
#endif
    XmPassChange ch;
    if (d->flags & XM_PASS_NEEDS_PROTO) {
        ch = d->fn.p ? d->fn.p(func, proto) : xm_pass_no_change();
    } else {
        ch = d->fn.v ? d->fn.v(func) : xm_pass_no_change();
    }
    if (!(d->flags & XM_PASS_NO_RESET) &&
        (ch.cfg_changed || ch.vreg_defs_changed || ch.ins_changed))
        XM_RESET_ANALYSIS(func);
#ifndef NDEBUG
    if (d->flags & XM_PASS_VERIFY_SE) {
        uint32_t se_post = xm_count_se_(func);
        if (se_post < se_pre) {
    ...
    if (!(d->flags & XM_PASS_SKIP_CFG_VERIFY)) {
        if (ch.cfg_changed)
...
XmPipelineStats xm_run_fixedpoint(XmFunc *func, XrProto *proto, const XmPassDesc *passes,
    ...
    for (uint32_t round = 0; round < max_rounds; round++) {
        ...
        for (uint32_t i = 0; i < npass; i++) {
            XmPassChange ch = fixedpoint_invoke(func, proto, &passes[i]);
 ...
 * pipeline.  Groups are driven by xm_run_fixedpoint with a modest
...
static const XmPassDesc PG_CANON[] = {
    {"select_rep", {.v = xm_pass_select_rep}, XM_PASS_SKIP_CFG_VERIFY},
    {"sccp", {.v = xm_pass_sccp}, 0},
    {"type_prop", {.v = xm_pass_type_prop}, XM_PASS_SKIP_CFG_VERIFY},
    {"specialize", {.v = xm_pass_specialize}, 0},
    {"canonicalize", {.v = xm_pass_canonicalize}, 0},
...
static const XmPassDesc PG_BASIC[] = {
    {"cse", {.v = xm_pass_cse}, XM_PASS_VERIFY_SE},
    {"store_to_load", {.v = xm_pass_store_to_load}, 0},
    {"escape_analysis", {.v = xm_pass_escape_analysis}, 0},
...
static const XmPassDesc PG_CLEANUP[] = {
    {"canonicalize", {.v = xm_pass_canonicalize}, 0},
    {"alloc_sink", {.v = xm_pass_alloc_sink}, 0},
    {"escape_analysis", {.v = xm_pass_escape_analysis}, 0},
    {"dce", {.v = xm_pass_dce}, XM_PASS_VERIFY_SE},
};

static const XmPassDesc PG_RANGE[] = {
    {"insert_redefines", {.v = xm_pass_insert_redefines}, 0},
    {"range_analysis", {.v = xm_pass_range_analysis}, 0},
    {"dce", {.v = xm_pass_dce}, XM_PASS_VERIFY_SE},
...
/* Thin wrapper around xm_run_fixedpoint that logs stats when the
 * pipeline-verbose toggle is on.  Keeps xm_run_pipeline_ex free of
 * instrumentation clutter. */
static void run_group(const char *group_name, XmFunc *func, XrProto *proto,
                      const XmPassDesc *passes, uint32_t npass, uint32_t max_rounds,
                      XmCompileBudget *budget) {
    // Skip group entirely if budget already exhausted
    if (budget && budget->timed_out)
        return;
    XmPipelineStats s = xm_run_fixedpoint(func, proto, passes, npass, max_rounds, budget);
    if (pipeline_verbose()) {
...
bool xm_run_pipeline_ex(XmFunc *func, XmOptLevel opt, XrProto *proto,
     ...
     * be resolved. */
    run_group("canon", func, proto, PG_CANON, XM_PIPELINE_SIZE(PG_CANON), 3, &budget);

    if (opt >= XM_OPT_BASIC && !budget.timed_out) {
        xm_rebuild_preds(func);
        xm_verify_cfg(func);
        xm_verify_types(func);
        run_group("basic", func, proto, PG_BASIC, XM_PIPELINE_SIZE(PG_BASIC), 3, &budget);
    }

    if (opt >= XM_OPT_FULL && !budget.timed_out) {
        if (proto) {
            (void) xm_pass_auto_inline(func, proto);
            XM_RESET_ANALYSIS(func);
    ...
    (void) xm_pass_reorder_blocks(func);
    XM_RESET_ANALYSIS(func);
    (void) xm_insert_write_barriers(func);
    XM_RESET_ANALYSIS(func);
