...
bool xir_translate_object_ops(XirBuilder *b, XirBlock **cur_blk, uint32_t pc, XrInstruction inst,
                              OpCode op) {
    ...
    switch (op) {
        ...
        case OP_NEWJSON: {
            ...
            XirRef v = xir_emit(b->func, blk, XIR_ALLOC, XR_REP_PTR, pack_ref, size_ref);
            ...
            // Zero-initialize overflow pointer and fields
            {
                ...
                xir_emit_raw(b->func, blk, XIR_STORE_FIELD, 0 /* null/ptr */, ov_off, v, zero_val);
                ...
                if (!safe_noinit) {
                    for (int fi = 0; fi < field_count && fi < 32; fi++) {
                        ...
                        xir_emit_raw(b->func, blk, XIR_STORE_FIELD, 0 /* XR_TAG_NULL */, off_ref, v,
                                     zero_val);
                    }
                }
            }
        ...
        case OP_GETPROP: {
            ...
            if (b->aot_mode && rb < 256 && b->import_modules[rb] && b->aot_import_map &&
                b->aot_import_count > 0 && b->isolate) {
                const char *mod_path = b->import_modules[rb];
                const char *export_name = xr_symbol_get_name_by_id(b->isolate, (int) sym);
                if (export_name) {
...

            {
                ...
                if (known_shape && known_shape->symbol_to_index &&
                    ...
                    if (field_idx >= 0) {
                        ...
                        builder_set_slot(b, a, v);
                        {
                            XirVReg *_sa = builder_vreg_for_slot(b, a);
                            if (_sa)
                                _sa->shape_hint = NULL;
                        }
                        b->ops_translated++;
                ...
                if (b->ic_fields_snapshot) {
                    ...
                    if (ic && ic->json_shape_id != 0 && ic->cached_symbol == (int) sym) {
                        struct XrShape *ic_shape =
                            xr_shape_get_by_id(b->isolate, ic->json_shape_id);
                        if (ic_shape && ic_shape->symbol_to_index &&
                            ...
                            if (field_idx >= 0) {
                                ...
                                XirRef shape_id_ref =
                                    xir_const_i64(b->func, (int64_t) (uint32_t) ic_shape->id);
                                xir_emit(b->func, blk, XIR_GUARD_SHAPE, XR_REP_VOID, obj,
                                         shape_id_ref);
                                blk->ins[blk->nins - 1].flags |= XIR_FLAG_SIDE_EFFECT;
                                ...
                                int32_t byte_offset =
                                    XR_JSON_FIELDS_OFFSET + field_idx * XR_XRVALUE_SIZE;
                                XirRef off_ref = xir_const_i64(b->func, (int64_t) byte_offset);
                                XirRef v = xir_emit(b->func, blk, XIR_LOAD_FIELD, XR_REP_I64, obj,
                                                    off_ref);
                                builder_tag_from_slot(b, v, a);
                                builder_set_slot(b, a, v);
                                {
                                    XirVReg *_sa2 = builder_vreg_for_slot(b, a);
                                    if (_sa2)
                                        _sa2->shape_hint = NULL;
                                }
                                b->ops_translated++;
                                return true;
                            }
...
#define IS_CLASS_OR_INSTANCE(t) ((t)->kind == XR_KIND_INSTANCE || (t)->kind == XR_KIND_CLASS)
                if (!recv_type || !IS_CLASS_OR_INSTANCE(recv_type)) {
                    XirVReg *recv_v = builder_vreg_for_slot(b, rb);
                    if (recv_v && recv_v->xrtype && IS_CLASS_OR_INSTANCE(recv_v->xrtype))
...
#undef IS_CLASS_OR_INSTANCE
                    ...
                    if (klass) {
                        ...
                        if (fi >= 0 && fi < klass->field_count) {
                            ...
                            int32_t byte_off = XR_INSTANCE_FIELDS_OFFSET + fi * XR_XRVALUE_SIZE;
                            ...
                            if (ftype &&
                                (strcmp(ftype, "float") == 0 || strcmp(ftype, "double") == 0 ||
                                 strcmp(ftype, "f64") == 0))
                                frep = XR_REP_F64;
                            XirRef fv = xir_emit(b->func, blk, XIR_LOAD_FIELD, frep, obj2, off2);
                            blk->ins[blk->nins - 1].flags |= XIR_FLAG_SIDE_EFFECT;
                            if (frep == XR_REP_F64)
                        ...
                        if (method && method->type == XMETHOD_CLOSURE && method->as.closure &&
                            ...
                            XirRef ph =
                                xir_emit_unary(b->func, blk, XIR_CONST_PTR, XR_REP_PTR, null_c);
                            builder_tag_vreg(b, ph, VTAG_TAGGED, 0);
                            builder_set_slot(b, a, ph);
                            XirVReg *mv = builder_vreg_for_slot(b, a);
                            if (mv)
                                mv->callee_proto = method->as.closure->proto;
                            b->ops_translated++;
                            return true;
                        }
                    }
            ...
            uint8_t gp_rep = builder_tagged_rep(b);
            ...
            builder_set_slot(b, a, v);
            {
                XirVReg *_sa3 = builder_vreg_for_slot(b, a);
                if (_sa3)
                    _sa3->shape_hint = NULL;
            }
            b->ops_translated++;
            return true;
        }
        ...
        case OP_SETPROP: {
            ...
            // Fast path: known shape → direct STORE_FIELD
            {
                    ...
                    if (ic && ic->json_shape_id != 0 && ic->cached_symbol == (int) sym) {
                        ...
                        if (ic_shape && ic_shape->symbol_to_index &&
                            ...
                            if (field_idx >= 0) {
                                ...
                                XirRef off_ref = xir_const_i64(b->func, (int64_t) byte_offset);
                                uint8_t sf_tag = builder_ref_sf_tag(b, val, rc);
                                xir_emit_raw(b->func, blk, XIR_STORE_FIELD, sf_tag, off_ref, obj,
                                             val);
                                b->ops_translated++;
                                return true;
                            }
                ...
                int64_t encoded =
                    ((int64_t) val_bc_slot_sp << 40) | ((int64_t) (uint32_t) sym << 8) | val_tag;
        ...
        case OP_JSON_GETK: {
...

            {
                ...
                if (known_shape && known_shape->symbol_to_index &&
                    ...
                    if (field_idx >= 0) {
                        ...
                        builder_set_slot(b, a, v);
                        {
                            XirVReg *_sa4 = builder_vreg_for_slot(b, a);
                            if (_sa4)
                                _sa4->shape_hint = NULL;
                        }
                        b->ops_translated++;
                        return true;
                    }
                    ...
                    if (ic && ic->json_shape_id != 0 && ic->cached_symbol == (int) sym) {
                        ...
                        if (ic_shape && ic_shape->symbol_to_index &&
                            ...
                            if (field_idx >= 0) {
                                ...
                                builder_set_slot(b, a, v);
                                {
                                    XirVReg *_sa5 = builder_vreg_for_slot(b, a);
                                    if (_sa5)
                                        _sa5->shape_hint = NULL;
                                }
                                b->ops_translated++;
                                return true;
                            }
            ...
            builder_set_slot(b, a, v);
            {
                XirVReg *_sa6 = builder_vreg_for_slot(b, a);
                if (_sa6)
                    _sa6->shape_hint = NULL;
            }
            b->ops_translated++;
            return true;
        }
        ...
        case OP_JSON_GET: {
            ...
            uint8_t field_rep = XR_REP_I64;
            {
                ...
                if (r == XR_REP_PTR)
                    field_rep = XR_REP_PTR;
                else if (r == XR_REP_F64)
                    field_rep = XR_REP_F64;
        ...
        case OP_TFIELD_GET: {
            ...
            uint8_t ftype =
                (c < 64 && (b->proto->tfield_float_bitmap >> c) & 1) ? XR_REP_F64 : XR_REP_I64;
        ...
        case OP_INDEX_GET: {
            ...
            // emit inline element load: base + index * elem_size → typed load
            {
                ...
                if (_av_rb && _av_rb->array_etype != 0xFF) {
                    ...
                    if (load_op >= 0) {
                        ...
                        XirRef es_v = xir_emit_unary(b->func, blk, XIR_CONST_I64, XR_REP_I64, es_c);
                        XirRef byte_off = xir_emit(b->func, blk, XIR_MUL, XR_REP_I64, key, es_v);
                        XirRef addr = xir_emit(b->func, blk, XIR_ADD, XR_REP_PTR, obj, byte_off);
                            ...
                            b->slot_rep[a] = rep;
                        {
                            ...
                            if (_av_a) {
                                _av_a->array_etype = 0xFF;
                                _av_a->array_ecount = 0;
                            }
                        }
                        b->ops_translated++;
                        return true;
            ...
            if (obj_vtag == VTAG_TAGGED) {
                ...
                if (obj_rep == XR_REP_PTR)
                    obj_vtag = VTAG_PTR;
                else if (obj_rep == XR_REP_I64)
                    obj_vtag = VTAG_I64;
                else if (obj_rep == XR_REP_F64)
                    obj_vtag = VTAG_F64;
            }
            uint8_t key_vtag = ref_vtag(b->func, key);
            if (key_vtag == VTAG_TAGGED) {
                uint8_t key_rep = ref_xir_type(b->func, key);
                if (key_rep == XR_REP_I64)
                    key_vtag = VTAG_I64;
                else if (key_rep == XR_REP_F64)
                    key_vtag = VTAG_F64;
                else if (key_rep == XR_REP_PTR)
                    key_vtag = VTAG_PTR;
            }
            ...
            XirRef enc_ref = xir_const_i64(b->func, encoded);
            XirRef enc_val = xir_emit_unary(b->func, blk, XIR_CONST_I64, XR_REP_I64, enc_ref);
            XirRef result = xir_emit(b->func, blk, XIR_CALL_C, res_type, fn_ref, enc_val);
            blk->ins[blk->nins - 1].flags |= XIR_FLAG_SIDE_EFFECT;
        ...
        case OP_INDEX_SET: {
            ...
            // Encode slot types: bits[31:24]=val_bc_slot, [23:16]=obj_st, [15:8]=key_st,
            // [7:0]=val_st val_bc_slot allows runtime to read slot_runtime_tags[bc_slot] when
            // val_st is imprecise.
        ...
        case OP_TOSTRING: {
            ...
            if (b->aot_mode) {
                ...
                arg_val = xir_emit_unary(b->func, blk, XIR_CONST_I64, XR_REP_I64, enc2_const);
            } else {
                ...
                arg_val = xir_emit_unary(b->func, blk, XIR_CONST_I64, XR_REP_I64, hint_const);
            }
            XirRef result = xir_emit(b->func, blk, XIR_CALL_C, XR_REP_PTR, fn_ref, arg_val);
            blk->ins[blk->nins - 1].flags |= XIR_FLAG_SIDE_EFFECT;
        ...
        case OP_STRBUF_FINISH: {
            ...
            XirRef fn_ref = xir_const_ptr(b->func, b->aot_mode ? (void *) xrt_strbuf_finish_sentinel
                                                               : (void *) xr_jit_strbuf_finish);
            XirRef zero = xir_const_i64(b->func, 0);
            XirRef zval = xir_emit_unary(b->func, blk, XIR_CONST_I64, XR_REP_I64, zero);
            XirRef result = xir_emit(b->func, blk, XIR_CALL_C, XR_REP_PTR, fn_ref, zval);
            blk->ins[blk->nins - 1].flags |= XIR_FLAG_SIDE_EFFECT;
        ...
        case OP_GETSHARED: {
            ...
            if (b->shared_protos && shared_index >= 0 && shared_index < b->nshared_protos &&
                b->shared_protos[shared_index] != NULL) {
                {
                    XirVReg *_pv = builder_vreg_for_slot(b, a);
                    if (_pv)
                        _pv->callee_proto = b->shared_protos[shared_index];
                }
            }
            b->ops_translated++;
            return true;
        ...
        case OP_SETSHARED: {
            ...
            int64_t encoded_idx = ((int64_t) val_bc_slot_ss << 24) | ((int64_t) val_tag << 16) |
                                  (shared_index & 0xFFFF);
