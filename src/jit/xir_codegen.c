...
A64Reg xra_get(CodegenCtx *ctx, XirRef ref) {
    if (xir_ref_is_none(ref)) return A64_XZR;
    if (!xir_ref_is_vreg(ref)) return A64_XZR;
    ...
    if (idx < ctx->func->nvreg && ctx->func->vregs[idx].rep == XR_REP_F64) {
        if (ri < 0) { fprintf(stderr, "[regalloc-fp] func=%s vreg=%u pos=%d blk=%u\n", ctx->func->name?ctx->func->name:"?", idx, ctx->cur_ra_pos, ctx->cur_blk_id); fflush(stderr); ctx->had_error = true; return alloc_fp_regs[0]; }
        return alloc_fp_regs[ri];
    }
    if (ri >= MAX_PHYS_REGS) {
        // Regalloc returned an index beyond physical register table — treat as error
        fprintf(stderr, "[regalloc-max] func=%s vreg=%u ri=%d pos=%d blk=%u\n", ctx->func->name?ctx->func->name:"?", idx, ri, ctx->cur_ra_pos, ctx->cur_blk_id); fflush(stderr);
        ctx->had_error = true;
        return SCRATCH_REG;
    }
    if (ri < 0) {
        ...
        fprintf(stderr,
            ...
            ctx->func->name ? ctx->func->name : "?",
            idx, (idx < ctx->func->nvreg) ? ctx->func->vregs[idx].rep : -1,
            ctx->cur_ra_pos, ctx->xra->nvreg, ctx->cur_blk_id);
        fflush(stderr);
        ctx->had_error = true;
        return SCRATCH_REG;
    }
...
static void emit_gap_moves_before(CodegenCtx *ctx, uint32_t ins_idx) {
    ...
    while (ctx->gap_move_cursor < n &&
           gm[ctx->gap_move_cursor].gap_blk == bid &&
...
void emit_ptr_spill_writeback(CodegenCtx *ctx) {
    ...
    for (uint32_t v = 0; v < ctx->func->nvreg; v++) {
        ...
        int8_t ri = xra_reg_at_pos(ctx->xra, v, pos);
        if (ri < 0) ri = xra_reg_at_pos(ctx->xra, v, pos + 1);
        if (ri < 0) continue;  // not in a register
...
static void emit_edge_copies(CodegenCtx *ctx, XirBlock *target, XirBlock *from) {
    ...
    if (ctx->xra) {
        XraEdgeCopy ec[64];
        uint32_t nc = xra_edge_copies(ctx->xra, ctx->func, target, from,
                                       ec, 64);
        for (uint32_t i = 0; i < nc && n < 64; i++) {
            A64Reg d = ec[i].is_fp ? alloc_fp_regs[ec[i].dst_idx]
                                   : alloc_regs[ec[i].dst_idx];
            if (ec[i].is_reload) {
            ...
            A64Reg s = ec[i].is_fp ? alloc_fp_regs[ec[i].src_idx]
                                   : alloc_regs[ec[i].src_idx];
            if (d != s)
    ...
    #define EMIT_COPY(c) do { \
        if ((c).is_reload) { \
            int32_t _off = SPILL_BASE + (c).spill_slot * 8; \
            if ((c).is_fp) a64_buf_emit(&ctx->buf, a64_ldr_fp((c).dst, A64_SP, _off)); \
            else           a64_buf_emit(&ctx->buf, a64_ldr((c).dst, A64_SP, _off)); \
        } else { \
            if ((c).is_fp) a64_buf_emit(&ctx->buf, a64_fmov((c).dst, (c).src)); \
            else           a64_buf_emit(&ctx->buf, a64_mov((c).dst, (c).src));   \
        } \
    } while(0)
    ...
    while (progress) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            for (uint32_t j = 0; j < n; j++) {
                ...
                if (!copies[j].is_reload &&
                    copies[j].is_fp == copies[i].is_fp &&
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        while (found) {
            ...
            for (uint32_t j = 0; j < n; j++) {
                ...
                if (copies[j].is_fp != cycle_fp) continue;
...
static A64Cond xir_cmp_to_false_cc(uint16_t op, bool *is_float) {
    *is_float = false;
    switch (op) {
        case XIR_LT:  return A64_CC_GE;
        case XIR_LE:  return A64_CC_GT;
        case XIR_GT:  return A64_CC_LE;
        case XIR_GE:  return A64_CC_LT;
        case XIR_EQ:  return A64_CC_NE;
        case XIR_NE:  return A64_CC_EQ;
        case XIR_FEQ: *is_float = true; return A64_CC_NE;
        case XIR_FNE: *is_float = true; return A64_CC_EQ;
        case XIR_FLT: *is_float = true; return A64_CC_PL;
        case XIR_FLE: *is_float = true; return A64_CC_HI;
        default:      return A64_CC_AL;
    }
}
...
static bool xir_op_clobbers_flags(uint16_t op) {
    ...
    if (op >= XIR_EQ && op <= XIR_FLE) return true;
    // SELECT_COND emits CMP_IMM
    if (op == XIR_SELECT_COND) return true;
    // Guards emit CMP internally
    if (op == XIR_GUARD_TAG || op == XIR_GUARD_CLASS ||
        op == XIR_GUARD_KLASS || op == XIR_GUARD_NONNULL ||
        op == XIR_GUARD_SHAPE || op == XIR_GUARD_BOUNDS) return true;
    // Safepoint emits CMP for reductions check
    if (op == XIR_SAFEPOINT) return true;
    // Calls: callee may clobber NZCV
    if (op == XIR_CALL || op == XIR_CALL_C || op == XIR_CALL_C_LEAF ||
        op == XIR_CALL_KNOWN || op == XIR_CALL_KNOWN_REG ||
        op == XIR_CALL_SELF_DIRECT ||
        op == XIR_CALL_DIRECT) return true;
    // ALLOC: inline bump-pointer has CMP for limit check
    if (op == XIR_ALLOC) return true;
    // RT_* helpers call into C runtime
    if (op >= XIR_RT_ADD && op <= XIR_RT_ISNULL) return true;
    // BOX/UNBOX may involve runtime calls
    if (op == XIR_BOX_I64 || op == XIR_BOX_F64 ||
        op == XIR_UNBOX_I64 || op == XIR_UNBOX_F64) return true;
    return false;
}
...
static void prescan_fuse_cmp_br(CodegenCtx *ctx, XirBlock *blk) {
    ...
    if (blk->jmp.type != XIR_JMP_BR) return;
    XirRef cond = blk->jmp.arg;
    if (!xir_ref_is_vreg(cond)) return;
...
A64Reg xra_arg(CodegenCtx *ctx, XirRef ref, A64Reg scratch_reg) {
    ...
    if (!xir_ref_is_vreg(ref)) return A64_XZR;
    uint32_t idx = XIR_REF_INDEX(ref);
    if (idx >= ctx->xra->nvreg) return A64_XZR;
    ...
    if (blk_reg < 0) blk_reg = xra_reg_at_pos(ctx->xra, idx, ctx->cur_ra_pos + 1);
    if (blk_reg >= 0) return xra_get(ctx, ref);
...
void xra_maybe_spill(CodegenCtx *ctx, XirRef dst_ref) {
    ...
    if (v >= ctx->xra->nvreg) return;
    int16_t slot = xra_vreg_spill(ctx->xra, v);
    if (slot < 0) return;  // not spilled or remat — nothing to store
...
A64Reg xra_operand(CodegenCtx *ctx, XirRef ref, A64Reg scratch_reg) {
    if (xir_ref_is_const(ref)) {
        ...
        if (idx < ctx->func->nconst) {
            a64_load_imm64(&ctx->buf, scratch_reg,
                           (uint64_t)ctx->func->consts[idx].val.raw);
            return scratch_reg;
        }
        return A64_XZR;
    }
...
static bool const_i64_fully_folded(CodegenCtx *ctx, XirBlock *blk, XirRef dst) {
    ...
    for (uint32_t i = 0; i < blk->nins; i++) {
        ...
        for (int a = 0; a < 2; a++) {
            ...
            if (XIR_REF_INDEX(ins->args[a]) != vi) continue;
            total++;
    ...
    switch (ins->op) {
        ...
        case XIR_FEQ: case XIR_FNE: case XIR_FLT: case XIR_FLE: {
            ...
            if (!xir_ref_is_none(ctx->fused_cmp_ref) &&
                xir_ref_is_vreg(ins->dst) && ins->dst == ctx->fused_cmp_ref)
                break;
            A64Cond cc;
            switch (ins->op) {
                case XIR_FEQ: cc = A64_CC_EQ; break;
                case XIR_FNE: cc = A64_CC_NE; break;
                case XIR_FLT: cc = A64_CC_MI; break;  // LT for unordered-aware
                case XIR_FLE: cc = A64_CC_LS; break;   // LE for unordered-aware
        ...
        case XIR_EQ: case XIR_NE: {
            ...
            A64Cond cc;
            switch (ins->op) {
                case XIR_LT: cc = A64_CC_LT; break;
                case XIR_LE: cc = A64_CC_LE; break;
                case XIR_GT: cc = A64_CC_GT; break;
                case XIR_GE: cc = A64_CC_GE; break;
                case XIR_EQ: cc = A64_CC_EQ; break;
                case XIR_NE: cc = A64_CC_NE; break;
                default: cc = A64_CC_AL; break;
            }
            a64_buf_emit(&ctx->buf, a64_cset(rd, cc));
            break;
        ...
        case XIR_UNBOX_I64: {
            ...
            if (src_type == XR_REP_PTR) {
                ...
                A64Reg rn = xra_arg(ctx, ins->args[0], SCRATCH_REG);
                if (rd != rn) a64_buf_emit(&ctx->buf, a64_mov(rd, rn));
            }
            break;
        }
        ...
        default:
            ...
            if (xir_emit_call_ops(ctx, ins, rd)) break;
            if (xir_emit_mem_ops(ctx, ins, rd)) break;
...
static void emit_prologue(CodegenCtx *ctx) {
    ...
    add_cs_patch(ctx, 0); a64_buf_emit(&ctx->buf, a64_stp(A64_X21, A64_X22, A64_SP, 32));
    add_cs_patch(ctx, 1); a64_buf_emit(&ctx->buf, a64_stp(A64_X23, A64_X24, A64_SP, 48));
    add_cs_patch(ctx, 2); a64_buf_emit(&ctx->buf, a64_stp(A64_X25, A64_X26, A64_SP, 64));
    add_cs_patch(ctx, 3); a64_buf_emit(&ctx->buf, a64_stp(A64_X27, A64_X28, A64_SP, 80));
    // Save FP callee-saved registers d8-d15
    add_cs_patch(ctx, 4); a64_buf_emit(&ctx->buf, a64_stp_fp(8, 9, A64_SP, 96));
    add_cs_patch(ctx, 5); a64_buf_emit(&ctx->buf, a64_stp_fp(10, 11, A64_SP, 112));
    add_cs_patch(ctx, 6); a64_buf_emit(&ctx->buf, a64_stp_fp(12, 13, A64_SP, 128));
    add_cs_patch(ctx, 7); a64_buf_emit(&ctx->buf, a64_stp_fp(14, 15, A64_SP, 144));
    // MOV x29, SP
...
void emit_epilogue(CodegenCtx *ctx) {
    ...
    add_cs_patch(ctx, 0); a64_buf_emit(&ctx->buf, a64_ldp(A64_X21, A64_X22, A64_SP, 32));
    add_cs_patch(ctx, 1); a64_buf_emit(&ctx->buf, a64_ldp(A64_X23, A64_X24, A64_SP, 48));
    add_cs_patch(ctx, 2); a64_buf_emit(&ctx->buf, a64_ldp(A64_X25, A64_X26, A64_SP, 64));
    add_cs_patch(ctx, 3); a64_buf_emit(&ctx->buf, a64_ldp(A64_X27, A64_X28, A64_SP, 80));
    // Restore FP callee-saved registers d8-d15
    add_cs_patch(ctx, 4); a64_buf_emit(&ctx->buf, a64_ldp_fp(8, 9, A64_SP, 96));
    add_cs_patch(ctx, 5); a64_buf_emit(&ctx->buf, a64_ldp_fp(10, 11, A64_SP, 112));
    add_cs_patch(ctx, 6); a64_buf_emit(&ctx->buf, a64_ldp_fp(12, 13, A64_SP, 128));
    add_cs_patch(ctx, 7); a64_buf_emit(&ctx->buf, a64_ldp_fp(14, 15, A64_SP, 144));
...
static void emit_block(CodegenCtx *ctx, uint32_t block_idx) {
    ...
    for (uint32_t i = 0; i < blk->nins; i++) {
        ...
        if (blk->ins[i].op == XIR_CONST_I64 &&
            const_i64_fully_folded(ctx, blk, blk->ins[i].dst)) {
            continue;
        }
        ...
        // Exception check after call-sites
        {
            ...
            if (op == XIR_CALL_C || op == XIR_CALL_DIRECT ||
                op == XIR_CALL_SELF_DIRECT || op == XIR_CALL_KNOWN_REG) {
                if (blk->exception_handler) {
                    // In try block: branch to catch handler
                    a64_buf_emit(&ctx->buf, a64_ldr(SCRATCH_REG, JIT_CTX_REG,
                                                     XIR_JIT_EXCEPTION_OFFSET));
                    add_patch(ctx, PATCH_CBNZ, blk->exception_handler->id,
                              SCRATCH_REG);
                    a64_buf_emit(&ctx->buf, a64_nop());
    ...
    switch (blk->jmp.type) {
        case XIR_JMP_JMP: {
            ...
            bool jmp_is_next = (block_idx + 1 < ctx->func->nblk) &&
        ...
        case XIR_JMP_BR: {
            ...
            if (!xir_ref_is_none(ctx->fused_cmp_ref)) {
                ...
                if (blk->is_loop_header) {
                    ...
                    add_patch(ctx, PATCH_B_COND, blk->s2->id,
                              (A64Reg)ctx->fused_false_cc);
                    a64_buf_emit(&ctx->buf, a64_nop());  // placeholder
                    ...
                    ctx->buf.code[bcond_pos] = a64_b_cond(ctx->fused_false_cc,
                                                           bcond_offset);
            ...
            if (!xir_ref_is_none(blk->jmp.arg)) {
                ...
                if (ret_idx < ctx->func->nvreg) {
                    ...
                    if (vt != 0xFF) {
                        ...
                        if (mt == XR_REP_F64) ret_xr_tag = XR_TAG_F64;
                        else if (mt == XR_REP_PTR) ret_xr_tag = XR_TAG_PTR;
                        else ret_xr_tag = 0xFF;                       // unknown: use heuristic
                    }
                }
...
static void emit_deopt_stub(CodegenCtx *ctx) {
    ...
    a64_buf_emit(&ctx->buf, a64_str_w(SCRATCH_REG2, JIT_CTX_REG,
                                       XIR_JIT_DEOPT_ID_OFFSET));

    // Save frame pointer for spill slot recovery
    a64_buf_emit(&ctx->buf, a64_str(A64_FP, JIT_CTX_REG,
                                     XIR_JIT_DEOPT_SPILL_BASE_OFFSET));
    ...
    for (int i = 0; i < MAX_PHYS_REGS; i++) {
        ...
        a64_buf_emit(&ctx->buf, a64_str(r, JIT_CTX_REG,
                                         gp_base + (int32_t)r * 8));
    }
    ...
    for (int i = 0; i < MAX_FP_REGS; i++) {
        a64_buf_emit(&ctx->buf, a64_str_fp(i, JIT_CTX_REG,
                                            fp_base + i * 8));
    }
...
static bool osr_should_skip_vreg(CodegenCtx *ctx, XirBlock *osr_blk,
...
static void osr_materialize_const(CodegenCtx *ctx, XirIns *def,
                                   int8_t phys, int8_t fp) {
    if (!def) return;
    if (def->op == XIR_CONST_I64 && xir_ref_is_const(def->args[0])) {
        uint32_t ci = XIR_REF_INDEX(def->args[0]);
        uint64_t val = ctx->func->consts[ci].val.raw;
        if (phys >= 0) a64_load_imm64(&ctx->buf, alloc_regs[phys], val);
        else if (fp >= 0) {
        ...
        if (fp >= 0) {
            a64_load_imm64(&ctx->buf, SCRATCH_REG2, val);
            a64_buf_emit(&ctx->buf, a64_fmov_from_gpr(alloc_fp_regs[fp], SCRATCH_REG2));
        } else if (phys >= 0) a64_load_imm64(&ctx->buf, alloc_regs[phys], val);
...
static void emit_osr_stubs(CodegenCtx *ctx, XirCodegenResult *result) {
    for (uint32_t i = 0; i < ctx->nosr_snap; i++) {
        ...
        a64_buf_emit(&ctx->buf, a64_str_w(SCRATCH_REG2, JIT_CTX_REG, XIR_JIT_ACTIVE_SMAP_ID_OFFSET));
        // Save JIT frame SP for GC access
        a64_buf_emit(&ctx->buf, a64_str(A64_FP, JIT_CTX_REG, XIR_JIT_FRAME_SP_OFFSET));
        ...
        XirBlock *osr_blk = (snap->block_id < ctx->func->nblk)
        ...
        for (uint8_t li = 0; li < bp_loop->nlive && nslots < XIR_MAX_OSR_SLOTS; li++) {
            ...
            for (uint32_t v = 0; v < ctx->func->nvreg; v++) {
                ...
                if (ri < 0) continue;
                if (osr_should_skip_vreg(ctx, osr_blk, v, ri)) continue;

                bool is_fp = (ctx->func->vregs[v].rep == XR_REP_F64);
                if (!is_fp) {
                    A64Reg dst = alloc_regs[ri];
                    a64_buf_emit(&ctx->buf, a64_ldr(dst, SCRATCH_REG,
                                 (int32_t)(slot * 8)));
                    ...
                    A64Reg dst = alloc_fp_regs[ri];
                    a64_buf_emit(&ctx->buf, a64_ldr_fp(dst, SCRATCH_REG,
                                 (int32_t)(slot * 8)));
                    entry->slots[nslots].bc_slot = (int16_t)slot;
                    entry->slots[nslots].phys_reg = (uint8_t)dst;
        ...
        for (uint32_t v = 0; v < ctx->func->nvreg && nslots < XIR_MAX_OSR_SLOTS; v++) {
            int8_t ri = xra_vreg_reg_at(ctx->xra, snap->block_id, v);
            if (ri < 0) continue;
            if (ctx->func->vregs[v].bc_slot >= 0) continue; // handled above
            if (!ctx->func->vregs[v].def) continue;
            bool is_fp = (ctx->func->vregs[v].rep == XR_REP_F64);
...
static void patch_branches(CodegenCtx *ctx) {
    for (uint32_t i = 0; i < ctx->npatch; i++) {
        ...
        if (p->target_blk >= ctx->nblock_offsets) continue;
...
static void build_runtime_deopt_table(CodegenCtx *ctx, XirCodegenResult *result) {
    ...
    result->ndeopt = 0;
    if (!func->deopt_infos || func->ndeopt == 0) return;
 ...
 * 2. Reloads ALL live registers from coro->jit_suspend_state
...
static void emit_resume_entry(CodegenCtx *ctx, XirCodegenResult *result) {
    ...
    // === Reload ALL registers from coro->jit_suspend_state ===
    // ADD x17, x19, #suspend_regs_offset
    a64_buf_emit(&ctx->buf, a64_add_imm(SCRATCH_REG2, CORO_REG, XIR_CORO_SUSPEND_REGS_OFFSET));
    ...
    // suspend_state.spill[0..nspill-1].  Copy them to the new frame.
    {
        ...
        for (uint32_t s = 0; s < ns; s++) {
            ...
            // LDR x17, [x19, #regs_off]  (x19 = CORO_REG)
            a64_buf_emit(&ctx->buf, a64_ldr(SCRATCH_REG2, CORO_REG, regs_off));
    ...
    for (uint32_t i = 0; i < ctx->nsuspend; i++) {
        ...
        // Recompute x17 = &coro->jit_suspend_state (needed for result + result_tag)
        a64_buf_emit(&ctx->buf, a64_add_imm(SCRATCH_REG2, CORO_REG,
        ...
        if (bc_slot >= 0 && bc_slot < 256) {
            a64_buf_emit(&ctx->buf, a64_ldrb(SCRATCH_REG, SCRATCH_REG2,
...
XirCodegenResult xir_codegen_arm64(XirFunc *func, XirCodeAlloc *alloc) {
    ...
    XirCodegenResult result = { .code = NULL, .code_size = 0, .success = false, .error = NULL, .nosr = 0 };
    ...
    for (uint32_t i = 0; i < func->nblk; i++) {
        if (func->blocks[i]->id > max_blk_id) max_blk_id = func->blocks[i]->id;
    }
