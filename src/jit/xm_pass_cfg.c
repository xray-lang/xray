 ...
 *
 * xm_pass_cfg.c - Xm CFG and control flow optimization passes
 *
 * KEY CONCEPT:
 ...
 */
XmPassChange xm_pass_copy_prop(XmFunc *func) {
    if (!func || func->nvreg == 0)
        return xm_pass_no_change();

    uint32_t nv = func->nvreg;
    XmRef *copy_of = (XmRef *) xr_malloc(nv * sizeof(XmRef));
    if (!copy_of)
        return xm_pass_no_change();
    ...
    while (changed) {
        ...
        for (uint32_t i = 0; i < nv; i++)
            copy_of[i] = XM_REF(XM_REF_VREG, i);
        ...
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            ...
            for (uint32_t i = 0; i < blk->nins; i++) {
                XmIns *ins = &blk->ins[i];
                if (!xm_op_is_copy(ins->op))
                    continue;
                if (!xm_ref_is_vreg(ins->dst))
                    continue;
                if (!xm_ref_is_vreg(ins->args[0]))
                    continue;
                uint32_t dst = XM_REF_INDEX(ins->dst);
                uint32_t src = XM_REF_INDEX(ins->args[0]);
                // Only propagate if reps match — crossing rep
                // boundaries would break typed instructions.
        ...
        for (uint32_t i = 0; i < nv; i++) {
            XmRef r = copy_of[i];
            uint32_t limit = nv;
            while (xm_ref_is_vreg(r) && limit-- > 0) {
                uint32_t idx = XM_REF_INDEX(r);
                if (idx >= nv)
                    break;
                if (copy_of[idx] == r)
                    break;
        ...
        for (uint32_t i = 0; i < nv; i++) {
            XmRef root = copy_of[i];
            if (!xm_ref_is_vreg(root))
                continue;
 ...
 */
XmPassChange xm_pass_phi_simp(XmFunc *func) {
    if (!func)
        return xm_pass_no_change();
    ...
    while (changed) {
        ...
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            ...
            while (*pp) {
                ...
                if (trivial && !xm_ref_is_none(unique)) {
                    ...
                    if (xm_ref_is_vreg(unique)) {
                        uint32_t ui = XM_REF_INDEX(unique);
                    ...
                    if (blk->nins < blk->ins_cap) {
                        memmove(&blk->ins[1], &blk->ins[0], blk->nins * sizeof(XmIns));
                        memset(&blk->ins[0], 0, sizeof(XmIns));
                        blk->ins[0].op = XM_MOV;
                        blk->ins[0].rep = phi->rep;
                        blk->ins[0].dst = phi->dst;
                        blk->ins[0].args[0] = unique;
                        blk->ins[0].args[1] = XM_NONE;
                        blk->nins++;
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        for (XmPhi *phi = blk->phis; phi; phi = phi->next) {
            ...
            for (uint32_t p = 0; p < phi->narg; p++) {
                ...
                uint16_t aht = 0;
                if (xm_ref_is_vreg(arg)) {
                    XmType act = xm_ref_ctype(func, arg);
 ...
 */
void xm_rebuild_preds(XmFunc *func) {
    if (!func || func->nblk == 0)
        return;
    ...
    for (uint32_t i = 0; i < nblk; i++) {
        ...
        if (blk->npred > 0 && blk->phis) {
            old[i].preds = (XmBlock **) xr_malloc(blk->npred * sizeof(XmBlock *));
            if (old[i].preds)
                memcpy(old[i].preds, blk->preds, blk->npred * sizeof(XmBlock *));
        }
        blk->npred = 0;
    }
    ...
    for (uint32_t i = 0; i < nblk; i++) {
        XmBlock *blk = func->blocks[i];
        if (blk->s1)
            xm_block_add_pred(blk->s1, blk, func->arena);
        if (blk->s2 && blk->s2 != blk->s1)
            xm_block_add_pred(blk->s2, blk, func->arena);
    }
    ...
    for (uint32_t i = 0; i < nblk; i++) {
        ...
        for (XmPhi *phi = blk->phis; phi; phi = phi->next) {
            XmRef *new_args = (XmRef *) xm_arena_calloc(func->arena, blk->npred, sizeof(XmRef));
            if (!new_args)
                continue;
 ...
 */
void xm_verify_cfg(XmFunc *func) {
#ifndef NDEBUG
    if (!func || func->nblk == 0)
        return;
    ...
    for (uint32_t i = 0; i < func->nblk; i++) {
        ...
        for (XmPhi *phi = blk->phis; phi; phi = phi->next) {
            ...
            for (uint16_t a = 0; a < phi->narg; a++) {
                ...
                if (xm_ref_is_none(r))
                    continue;
                if (xm_ref_is_vreg(r))
                    XR_DCHECK(XM_REF_INDEX(r) < func->nvreg, "phi arg vreg out of range");
                if (xm_ref_is_const(r))
                    XR_DCHECK(XM_REF_INDEX(r) < func->nconst, "phi arg const out of range");
            }
            // Check phi dst vreg is valid
            if (xm_ref_is_vreg(phi->dst))
                XR_DCHECK(XM_REF_INDEX(phi->dst) < func->nvreg, "phi dst vreg out of range");
        }
        ...
        for (uint32_t j = 0; j < blk->nins; j++) {
            XmIns *ins = &blk->ins[j];
            if (xm_ref_is_vreg(ins->dst))
                XR_DCHECK(XM_REF_INDEX(ins->dst) < func->nvreg, "ins dst vreg out of range");
            for (int a = 0; a < 2; a++) {
                XmRef r = ins->args[a];
                if (xm_ref_is_vreg(r))
                    XR_DCHECK(XM_REF_INDEX(r) < func->nvreg, "ins arg vreg out of range");
                if (xm_ref_is_const(r))
                    XR_DCHECK(XM_REF_INDEX(r) < func->nconst, "ins arg const out of range");
            }
        }
    ...
    for (uint32_t v = 0; v < func->nvreg; v++) {
        ...
        XR_DCHECK(xm_ref_is_vreg(def->dst) && XM_REF_INDEX(def->dst) == v,
        ...
        for (uint32_t v = 0; v < func->nvreg; v++) {
            ...
            for (uint16_t a = 0; a < func->vregs[v].call_nargs; a++) {
                ...
                if (xm_ref_is_none(ref))
                    continue;
                if (xm_ref_is_vreg(ref))
                    XR_DCHECK(XM_REF_INDEX(ref) < func->nvreg,
                              "call_arg_pool: vreg ref out of range");
                if (xm_ref_is_const(ref))
                    XR_DCHECK(XM_REF_INDEX(ref) < func->nconst,
...
/*
 * Check if an Xm opcode may trigger garbage collection.
 * Used by verify_types to check ALLOC safety zones.
 */
static bool xm_op_may_gc(uint16_t op) {
    switch (op) {
        case XM_ALLOC:
        case XM_CALL:
        case XM_CALL_C:
 ...
 *   8. ALLOC dst is PTR
 */
void xm_verify_types(XmFunc *func) {
#ifndef NDEBUG
    if (!func || func->nblk == 0)
        return;
...
#define REF_TYPE(r)                                                                                \
    ...
    for (uint32_t i = 0; i < func->nblk; i++) {
        ...
        for (XmPhi *phi = blk->phis; phi; phi = phi->next) {
            if (xm_ref_is_vreg(phi->dst)) {
                uint32_t vi = XM_REF_INDEX(phi->dst);
        ...
        for (uint32_t j = 0; j < blk->nins; j++) {
            ...
            if (ins->op >= XM_FADD && ins->op <= XM_FNEG) {
                for (int a = 0; a < 2; a++) {
            ...
            if (ins->op >= XM_FEQ && ins->op <= XM_FLE) {
                for (int a = 0; a < 2; a++) {
                    if (xm_ref_is_none(ins->args[a]))
                        continue;
                    uint8_t at = REF_TYPE(ins->args[a]);
 ...
 */
XmPassChange xm_pass_split_critical_edges(XmFunc *func) {
    if (!func || func->nblk == 0)
        return xm_pass_no_change();
    ...
    for (uint32_t ei = 0; ei < nedges; ei++) {
        XmBlock *from = func->blocks[edges[ei].from_bi];
        int slot = edges[ei].succ_slot;
        XmBlock *target = (slot == 0) ? from->s1 : from->s2;
        if (!target)
            continue;

        // Insert empty block: from → mid → target
        XmBlock *mid = xm_func_add_block(func, NULL);
        if (!mid)
            continue;
        mid->jmp.type = XM_JMP_JMP;
        mid->jmp.arg = XM_NONE;
        mid->s1 = target;
        ...
        xm_block_add_pred(mid, from, func->arena);
 ...
 */
XmPassChange xm_pass_merge_blocks(XmFunc *func) {
    if (!func || func->nblk <= 1)
        return xm_pass_no_change();
    ...
    while (changed) {
        ...
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            XmBlock *a = func->blocks[bi];
            if (a->jmp.type != XM_JMP_JMP)
                continue;
            ...
            memcpy(&a->ins[a->nins], b->ins, b->nins * sizeof(XmIns));
            a->nins = total;
 ...
 * src == dst.
 */
XmPassChange xm_pass_select_rep(XmFunc *func) {
    if (!func || func->nvreg == 0)
        return xm_pass_no_change();
    ...
    XmIns **def_ins = (XmIns **) xr_calloc(nv, sizeof(XmIns *));
    if (!def_ins)
        return xm_pass_no_change();
    ...
    while (changed) {
        ...
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            ...
            for (uint32_t i = 0; i < blk->nins; i++) {
                ...
                if (op == XM_UNBOX_I64)
                    inverse_op = XM_BOX_I64;
                else if (op == XM_UNBOX_F64)
                    inverse_op = XM_BOX_F64;
                else if (op == XM_BOX_I64)
                    inverse_op = XM_UNBOX_I64;
                else if (op == XM_BOX_F64)
                    inverse_op = XM_UNBOX_F64;
                else
                    continue;

                if (!xm_ref_is_vreg(ins->args[0]))
                    continue;
                uint32_t src_idx = XM_REF_INDEX(ins->args[0]);
                if (src_idx >= nv || !def_ins[src_idx])
                    continue;

                XmIns *src_ins = def_ins[src_idx];
 ...
 */
static int32_t blk_weight(XmFunc *func, XmBlock *blk) {
    if (!blk)
        return -1;
    int32_t w = (int32_t) xm_block_loop_depth(func, blk->id) * 2;
    if (blk->is_deferred)
 ...
 */
XmPassChange xm_pass_reorder_blocks(XmFunc *func) {
    if (!func || func->nblk <= 2)
        return xm_pass_no_change();

    uint32_t n = func->nblk;
    XmBlock **order = (XmBlock **) xr_malloc(n * sizeof(XmBlock *));
    if (!order)
        return xm_pass_no_change();
...
#define BLK_IDX(blk) ((blk) && (blk)->id <= max_id ? id_to_idx[(blk)->id] : -1)
    ...
    XmBlock *cur = func->blocks[0];
    while (norder < n) {
        XmBlock *next = NULL;

        if (cur) {
            XmBlock *cand1 = NULL, *cand2 = NULL;

            if (cur->jmp.type == XM_JMP_BR) {
            ...
            } else if (cur->jmp.type == XM_JMP_JMP) {
                cand1 = cur->s1;
            }
