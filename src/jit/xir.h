...
#define XIR_REF(kind, index) \
    ((XirRef)(((uint32_t)(kind) << XIR_REF_INDEX_BITS) | ((uint32_t)(index) & XIR_REF_INDEX_MASK)))
...
static inline bool xir_ref_is_vreg(XirRef r)  { return XIR_REF_KIND(r) == XIR_REF_VREG; }
static inline bool xir_ref_is_const(XirRef r) { return XIR_REF_KIND(r) == XIR_REF_CONST; }
static inline bool xir_ref_is_none(XirRef r)  { return XIR_REF_KIND(r) == XIR_REF_NONE; }
...
static inline uint8_t xir_type_to_rep(uint8_t kind) {
    switch (kind) {
    case XIR_TK_INT:  case XIR_TK_BOOL:    return XR_REP_I64;
    case XIR_TK_FLOAT:                      return XR_REP_F64;
    case XIR_TK_PTR:  case XIR_TK_STRING:
    case XIR_TK_ARRAY: case XIR_TK_MAP:
    case XIR_TK_FUNC:                       return XR_REP_PTR;
...
static inline XirType xir_type_from_rep(uint8_t rep) {
    switch (rep) {
    case XR_REP_I64: return (XirType){ XIR_TK_INT,   0, 0 };
    case XR_REP_F64: return (XirType){ XIR_TK_FLOAT, 0, 0 };
    case XR_REP_PTR: return (XirType){ XIR_TK_PTR,   0, 0 };
    default:         return (XirType){ XIR_TK_UNKNOWN, 0, 0 };
    }
}
...
static inline uint8_t vtag_to_value_tag(uint8_t vtag) {
    ...
    case VTAG_BOOL:        return 1;    // XR_TAG_BOOL (payload 0=false, 1=true)
    case VTAG_NUMERIC:     return 0xFC; // XR_RTAG_NUMERIC
    default:               return 0xFF; // XR_RTAG_UNKNOWN (VTAG_TAGGED)
    }
}
...
static inline uint8_t value_tag_to_vtag(uint8_t tag) {
    switch (tag) {
    case 0:    return VTAG_NULL;         // XR_TAG_NULL
    case 3:    return VTAG_I64;         // XR_TAG_I64
    case 4:    return VTAG_F64;         // XR_TAG_F64
    case 5:    return VTAG_PTR;         // XR_TAG_PTR
    case 1:    return VTAG_BOOL;         // XR_TAG_BOOL
    case 0xFC: return VTAG_NUMERIC;     // XR_RTAG_NUMERIC
...
static inline uint8_t vtag_to_type_kind(uint8_t vtag) {
    switch (vtag) {
    case VTAG_I64:     return XIR_TK_INT;
    case VTAG_F64:     return XIR_TK_FLOAT;
    case VTAG_PTR:     return XIR_TK_PTR;
    case VTAG_BOOL:    return XIR_TK_BOOL;
    case VTAG_NUMERIC: return XIR_TK_NUMERIC;
    case VTAG_NULL:    return XIR_TK_NULL;
    case VTAG_TAGGED:  return XIR_TK_TAGGED;
    default:           return XIR_TK_UNKNOWN;
    }
}
...
static inline uint8_t type_kind_to_vtag(uint8_t kind) {
    switch (kind) {
    case XIR_TK_INT:     return VTAG_I64;
    case XIR_TK_FLOAT:   return VTAG_F64;
    case XIR_TK_BOOL:    return VTAG_BOOL;
    case XIR_TK_NULL:    return VTAG_NULL;
    case XIR_TK_PTR: case XIR_TK_STRING: case XIR_TK_ARRAY:
    case XIR_TK_MAP: case XIR_TK_FUNC:
                         return VTAG_PTR;
    case XIR_TK_NUMERIC: return VTAG_NUMERIC;
    case XIR_TK_TAGGED:  return VTAG_TAGGED;
    default:             return VTAG_TAGGED;
    }
}
...
typedef union {
    ...
    struct { const char *chars; uint32_t len; } str; // AOT string literal
} XirConstVal;
...
// Instructions
XR_FUNC XirRef xir_emit(XirFunc *func, XirBlock *blk, uint16_t op, uint8_t type, XirRef a, XirRef b);
...
XR_FUNC void   xir_emit_raw(XirFunc *func, XirBlock *blk, uint16_t op, uint8_t type,
...
XR_FUNC void     xir_func_bind_call_args(XirFunc *func, XirRef dst,
...
static inline XirType xir_ref_ctype(XirFunc *func, XirRef ref) {
    if (!xir_ref_is_vreg(ref)) return XIR_TYPE_UNKNOWN;
    uint32_t vi = XIR_REF_INDEX(ref);
    if (vi >= func->nvreg) return XIR_TYPE_UNKNOWN;
    XirIns *def = func->vregs[vi].def;
    if (def) return def->ctype;
