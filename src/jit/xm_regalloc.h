 ...
 *
 * xm_regalloc.h - Linear scan register allocation with per-segment mapping
 *
 * KEY CONCEPT:
...
#ifndef XM_REGALLOC_H
#define XM_REGALLOC_H
...
typedef struct {
    ...
    // exceeds XM_MAX_SPILL_SLOTS).  Codegen must check this before
    // emitting any code and abort compilation with a diagnostic error.
    bool had_error;
} XraResult;

/*
 * Run linear scan register allocation on a Xm function.
 *
 * Algorithm:
 ...
 */
XR_FUNC XraResult *xra_run(XmFunc *func);
