 ...
 *
 * xm_jit_runtime.h - JIT runtime bridge function declarations
 *
 * KEY CONCEPT:
...
#ifndef XM_JIT_RUNTIME_H
#define XM_JIT_RUNTIME_H
...
XR_FUNC XrJitResult xr_jit_chan_method_recv_is_value(struct XrCoroutine *coro, int64_t extra_arg);
 ...
 * after live registers are saved to xr_coro_jit_state(coro)->suspend.
 ...
 * Reloads saved registers from xr_coro_jit_state(coro)->suspend and jumps to
 * the continuation point via jit_suspend_id jump table. */
XR_FUNC int xm_jit_resume(struct XrCoroutine *coro, struct XrValue *result);

#endif  // XM_JIT_RUNTIME_H
