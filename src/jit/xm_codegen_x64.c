...
static bool x64_is_fp_vreg(X64CodegenCtx *ctx, XmRef ref) {
    if (!xm_ref_is_vreg(ref))
        return false;
    uint32_t idx = XM_REF_INDEX(ref);
...
/* Forward declaration: defined after x64_emit_xm_ins */
static void x64_emit_alloc_ins(X64CodegenCtx *ctx, XmIns *ins, X64Reg rd, uint8_t gc_type,
                               uint16_t gc_extra, uint32_t alloc_size);

static void x64_emit_xm_ins(X64CodegenCtx *ctx, XmIns *ins) {
...
static void x64_emit_edge_copies(X64CodegenCtx *ctx, XmBlock *target, XmBlock *from) {
    if (!ctx->xra)
        return;
    typedef struct {
        X64Reg dst, src;
...
static void x64_emit_osr_stubs(X64CodegenCtx *ctx, XmCodegenResult *result) {
...
static void x64_emit_resume_entry(X64CodegenCtx *ctx, XmCodegenResult *result) {
    if (ctx->nsuspend == 0)
        return;
