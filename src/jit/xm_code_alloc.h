 ...
 *
 * xm_code_alloc.h - Executable memory allocator for JIT code
 *
 * KEY CONCEPT:
...
#ifndef XM_CODE_ALLOC_H
#define XM_CODE_ALLOC_H
...
#define XM_CODE_PAGE_SIZE (64 * 1024)             // 64KB per page
#define XM_CODE_CACHE_DEFAULT (64 * 1024 * 1024)  // 64MB default budget
...
XR_FUNC void xm_code_alloc_init(XmCodeAlloc *alloc);

// Destroy: unmap all pages
XR_FUNC void xm_code_alloc_destroy(XmCodeAlloc *alloc);

// Allocate executable memory for JIT code
// Returns writable pointer. Call xm_code_alloc_finalize() after writing.
// alignment: 0 or power of 2 (0 defaults to 16-byte alignment)
XR_FUNC void *xm_code_alloc(XmCodeAlloc *alloc, size_t size, size_t alignment);
...
XR_FUNC void xm_code_make_executable(void *ptr, size_t size);
...
XR_FUNC void xm_code_make_writable(void *ptr, size_t size);

// Flush instruction cache after writing code
// Required on ARM64 (unified cache not guaranteed)
XR_FUNC void xm_code_flush_icache(void *ptr, size_t size);

// Set the code cache budget in bytes (0 = unlimited, clamped to XM_CODE_CACHE_MAX).
XR_FUNC void xm_code_alloc_set_budget(XmCodeAlloc *alloc, size_t budget_bytes);

// Returns true if total_allocated exceeds the budget.
static inline bool xm_code_alloc_over_budget(XmCodeAlloc *alloc) {
    return alloc->budget > 0 && alloc->total_allocated > alloc->budget;
}

// Bump the global epoch (call at GC safepoints).
static inline void xm_code_alloc_bump_epoch(XmCodeAlloc *alloc) {
    alloc->epoch++;
}
...
XR_FUNC void xm_code_alloc_retire(XmCodeAlloc *alloc, void *code, size_t size);

// Reclaim garbage entries whose retire_epoch < safe_epoch.
// safe_epoch = min(saved_epoch) across all active coroutines.
XR_FUNC void xm_code_alloc_reclaim(XmCodeAlloc *alloc, uint64_t safe_epoch);

/* ========== Statistics ========== */

static inline size_t xm_code_alloc_total(XmCodeAlloc *alloc) {
    return alloc->total_allocated;
}

static inline size_t xm_code_alloc_used(XmCodeAlloc *alloc) {
    return alloc->total_used;
}

#endif  // XM_CODE_ALLOC_H
