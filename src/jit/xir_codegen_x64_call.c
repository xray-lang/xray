 ...
 *
 * xir_codegen_x64_call.c - XIR codegen for call instructions (x86-64)
 *
 * Split from xir_codegen_x64.c to stay within the 3000-line limit.
...
bool x64_emit_call_ins(X64CodegenCtx *ctx, XirIns *ins, X64Reg rd) {
    ...
    switch (ins->op) {
        case XIR_CALL_C: {
            ...
            x64_mov_rm32(&ctx->buf, X64_RCX, X64_JIT_CTX_REG, (int32_t) XIR_JIT_DEOPT_ID_OFFSET);
        ...
        case XIR_CALL_SELF_DIRECT: {
            ...
            if (reg_passing) {
                ...
                x64_mov_rr(&ctx->buf, X64_RDI, X64_CORO_REG);
                x64_lea(&ctx->buf, X64_RSI, X64_JIT_CTX_REG, (int32_t) XIR_JIT_CALL_ARGS_OFFSET);
        ...
        case XIR_CALL_KNOWN: {
            ...
            x64_mov_mr(&ctx->buf, X64_JIT_CTX_REG, (int32_t) XIR_JIT_CALL_PROTO_OFFSET,
                       X64_SCRATCH_REG);
            ...
            x64_mov_rm(&ctx->buf, X64_RCX, X64_JIT_CTX_REG, (int32_t) XIR_JIT_CALL_ARGS_OFFSET);
            x64_mov_mr(&ctx->buf, X64_JIT_CTX_REG, (int32_t) XIR_JIT_CALL_CLOSURE_OFFSET, X64_RCX);

            /* Fast path direct CALL: RDI=coro, RSI=&call_args[1], CALL r11 */
            x64_mov_rr(&ctx->buf, X64_RDI, X64_CORO_REG);
            x64_lea(&ctx->buf, X64_RSI, X64_JIT_CTX_REG, (int32_t) (XIR_JIT_CALL_ARGS_OFFSET + 8));
            x64_call_r(&ctx->buf, X64_SCRATCH_REG);
        ...
        case XIR_CALL_DIRECT: {
            ...
            x64_movzx_rm8(&ctx->buf, X64_RCX, X64_SCRATCH_REG, (int32_t) XIR_GC_TYPE_OFFSET);
            ...
            x64_mov_mr(&ctx->buf, X64_JIT_CTX_REG, (int32_t) XIR_JIT_CALL_CLOSURE_OFFSET,
                       X64_SCRATCH_REG);
