 ...
 *
 * xm_regalloc.c - Linear scan register allocation with Active/Inactive state machine
 *
 * KEY CONCEPT:
...
typedef struct {
    ...
    int16_t next_spill;
    int32_t slot_end[XM_MAX_SPILL_SLOTS];  // last-end position per slot
    bool slot_is_ptr[XM_MAX_SPILL_SLOTS];  // true if slot last used by PTR vreg
...
static bool vfp(XmFunc *f, uint32_t v) {
    return v < f->nvreg && f->vregs[v].rep == XR_REP_F64;
}

static bool is_remat(XmFunc *f, uint32_t v) {
    if (v >= f->nvreg)
        return false;
    XmIns *d = f->vregs[v].def;
    if (!d)
        return false;
    /* Constants: single-instruction immediate load. */
    if (d->op == XM_CONST_I64 || d->op == XM_CONST_F64 || d->op == XM_CONST_PTR)
        return true;
    /* LOAD_CORO_BYTE with a const offset: single LDRB from the
     * callee-saved jit_ctx register (x28), always available. */
    if (d->op == XM_LOAD_CORO_BYTE && xm_ref_is_const(d->args[0]))
        return true;
    ...
    if ((d->op == XM_I2F || d->op == XM_F2I) && xm_ref_is_const(d->args[0]))
        return true;
    return false;
}
...
static void compute_deferred(LsCtx *ctx) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        XmBlock *b = f->blocks[i];
        // Blocks ending in unreachable (deopt/throw)
        if (b->jmp.type == XM_JMP_UNREACHABLE) {
...
static void build_ranges(LsCtx *ctx) {
    ...
    for (uint32_t bi = 0; bi < func->nblk; bi++) {
        ...
        if (bi < live->nblk) {
            const XmBSet *lo = &live->blocks[bi].live_out;
            int iter = 0, bit;
            while ((bit = xm_bset_iter(lo, &iter)) >= 0) {
        ...
        for (XmPhi *p = blk->phis; p; p = p->next) {
            for (uint32_t a = 0; a < p->narg; a++) {
                ...
                if (v >= ctx->nvreg)
                    continue;
                XmBlock *pred = (a < blk->npred) ? blk->preds[a] : NULL;
                if (!pred)
                    continue;
        ...
        if (bi < live->nblk) {
            const XmBSet *li = &live->blocks[bi].live_in;
            int iter = 0, bit;
            while ((bit = xm_bset_iter(li, &iter)) >= 0) {
                if ((uint32_t) bit < ctx->nvreg)
                    range_add_iv(ctx, ctx->vreg_ranges[bit], bs, be);
            }
        }
...
static void compute_bundles(LsCtx *ctx) {
    ...
    for (uint32_t bi = 0; bi < f->nblk; bi++) {
        for (XmPhi *p = f->blocks[bi]->phis; p; p = p->next) {
            if (!xm_ref_is_vreg(p->dst))
                continue;
            uint32_t dv = XM_REF_INDEX(p->dst);
            if (dv >= nv)
                continue;
            ...
            for (uint32_t a = 0; a < p->narg; a++) {
                if (!xm_ref_is_vreg(p->args[a]))
                    continue;
                uint32_t av = XM_REF_INDEX(p->args[a]);
                if (av >= nv || av == dv)
                    continue;
...
static void compute_hints(LsCtx *ctx) {
    ...
    for (uint32_t v = 0; v < f->num_params && v < nv; v++) {
        LsRange *r = ctx->vreg_ranges[v];
        if (r->is_fp) {
            if (fpi < xm_current_target->nfpr)
                r->hint = (int8_t) fpi++;
        } else {
            if (gpi < xm_current_target->ngpr)
                r->hint = (int8_t) gpi++;
        }
    }
    ...
    for (int round = 0; round < 3; round++) {
        ...
        for (uint32_t bi = 0; bi < f->nblk; bi++) {
            ...
            for (uint32_t ii = 0; ii < blk->nins; ii++) {
                ...
                if (!xm_ref_is_vreg(ins->dst) || !xm_ref_is_vreg(ins->args[0]))
                    continue;
                uint32_t dv = XM_REF_INDEX(ins->dst), sv = XM_REF_INDEX(ins->args[0]);
                if (dv >= nv || sv >= nv)
                    continue;
                LsRange *dr = ctx->vreg_ranges[dv], *sr = ctx->vreg_ranges[sv];
...
static LsRange *split_between(LsCtx *ctx, LsRange *r, int32_t from, int32_t to) {
    ...
    for (uint32_t i = 0; i < ctx->func->nblk; i++) {
        ...
        int depth = (int) xm_block_loop_depth(ctx->func, ctx->func->blocks[i]->id);
...
static void assign_spill_slot(LsCtx *ctx, LsRange *r) {
    ...
    if (ctx->bundle_parent && r->vreg < ctx->nvreg) {
        ...
        if (ctx->bundle_spill[bid] != XRA_SPILL_NONE) {
            ...
            XR_DCHECK(r->spill >= XM_MAX_SPILL_SLOTS ||
            ...
            if (r->spill >= 0 && r->spill < XM_MAX_SPILL_SLOTS && rend > ctx->slot_end[r->spill])
    ...
    for (int16_t s = 0; s < ctx->next_spill && s < XM_MAX_SPILL_SLOTS; s++) {
...
static bool alloc_free_reg(LsCtx *ctx, LsRange *range, LsRange ***wl, uint32_t *wl_len,
    ...
    int mx = isfp ? xm_current_target->nfpr : xm_current_target->ngpr;
...
XraResult *xra_run(XmFunc *func) {
     ...
     *   1. res->nspill <= XM_MAX_SPILL_SLOTS:
     ...
     *   2. If the function contains any XM_SUSPEND, also:
     *        res->nspill <= XM_SUSPEND_SPILL_MAX
    ...
    if (res->nspill > XM_MAX_SPILL_SLOTS)
        res->had_error = true;
    if (func->nsuspend > 0 && res->nspill > XM_SUSPEND_SPILL_MAX)
        res->had_error = true;
...
uint32_t xra_edge_copies(const XraResult *r, XmFunc *func, XmBlock *target, XmBlock *from,
    ...
    if (pi >= 0) {
        for (XmPhi *p = target->phis; p; p = p->next) {
            if (!xm_ref_is_vreg(p->dst))
                continue;
            if ((uint32_t) pi >= p->narg)
                continue;
            XmRef sr = p->args[pi];
            if (xm_ref_is_none(sr) || !xm_ref_is_vreg(sr))
                continue;
            uint32_t dv = XM_REF_INDEX(p->dst);
            uint32_t sv = XM_REF_INDEX(sr);
    ...
    for (uint32_t v = 0; v < r->nvreg && n < mx; v++) {
        ...
        bool is_phi = false;
        if (pi >= 0) {
            for (XmPhi *p = target->phis; p && !is_phi; p = p->next) {
                if (xm_ref_is_vreg(p->dst) && XM_REF_INDEX(p->dst) == v)
                    is_phi = true;
            }
        }
        if (is_phi)
