 ...
 *
 * xm_jit.c - JIT compiler state and VM integration
 *
 * KEY CONCEPT:
 ...
 *
 * Both sync compile (xm_jit_try_compile) and background install
 * (xm_jit_install_bg_result) MUST use this single function.
...
XmJitState *xm_jit_init(XrayIsolate *isolate, int threshold) {
    ...
    xm_code_alloc_init(&jit->code_alloc);
    jit->isolate = isolate;
    ...
    if (threshold > 1) {
        XmCompileQueue *q = (XmCompileQueue *) xr_calloc(1, sizeof(XmCompileQueue));
        if (q) {
...
void xm_jit_destroy(XmJitState *jit) {
    if (!jit)
        return;
    ...
    xm_code_alloc_destroy(&jit->code_alloc);
    xr_free(jit->compiled_protos);
    if (jit->tfa) {
        tfa_free(jit->tfa);
        xr_free(jit->tfa);
    }
    // Deregister JIT hooks (mirrors the registration in xm_jit_init).
    xr_jit_hooks = NULL;
...
static void xm_jit_evict_coldest(XmJitState *jit) {
...
bool xm_jit_try_compile(XmJitState *jit, XrProto *proto) {
    ...
    if (xm_code_alloc_over_budget(&jit->code_alloc)) {
        xm_jit_evict_coldest(jit);
    }
    ...
    if (proto->jit_entry) {
        ...
        if (proto->jit_opt_level >= XM_OPT_FULL)
            return true;
    ...
    // that all bg tasks share the same XmBgTask snapshot construction.
    if (jit->bg_queue) {
        ...
        // bg thread replaces this with the actual XmBgResult* when done.
    ...
    XmICSnapshot ic_snap = {NULL, NULL};
    if (!conservative) {
        XrVMContext *cur_ctx = xr_vm_current_ctx(jit->isolate);
        if (cur_ctx) {
    ...
    // Fallback: infer return type by scanning Xm RET blocks.
    // Covers --jit-force (threshold=1) where no type feedback exists yet,
    // but the Xm builder assigned typed vregs to return values.
    if (!proto->return_type_info) {
        uint8_t inferred_slot = XR_SLOT_ANY;
        bool first = true;
        for (uint32_t bi = 0; bi < func->nblk; bi++) {
            XmBlock *blk2 = func->blocks[bi];
            if (blk2->jmp.type != XM_JMP_RET || !xm_ref_is_vreg(blk2->jmp.arg))
                continue;
            uint32_t vi = XM_REF_INDEX(blk2->jmp.arg);
            if (vi >= func->nvreg)
                continue;
            uint8_t vtype = func->vregs[vi].rep;
            XmType rct = xm_ref_ctype(func, blk2->jmp.arg);
 ...
 *   - Deopt return: XM_DEOPT_MARKER signals deopt; xr_coro_jit_state(coro)->scratch->deopt_regs[]
...
int xm_jit_call(void *jit_entry, XrCoroutine *coro, XrValue *args, int nargs,
    ...
    if (!jit_state->suspend) {
        jit_state->suspend = xr_calloc(1, sizeof(XrJitSuspendState));
        if (!jit_state->suspend)
            return XM_JIT_DEOPT;
    }
    ...
    {
        XrProto *p = (XrProto *) xr_coro_jit_state(coro)->scratch->call_proto;
        xr_coro_jit_state(coro)->scratch->active_stack_map = p ? p->stack_map : NULL;
    }
    ...
    for (int i = 0; i < nargs && i < 8; i++)
        xr_coro_jit_state(coro)->scratch->param_tags[i] = (int64_t) args[i].tag;
    ...
    // EQ(param, null) null-checks work correctly in JIT code.
    {
        ...
        for (int i = nargs; i < nparams && i < 8; i++)
            xr_coro_jit_state(coro)->scratch->param_tags[i] = 0;  // XR_TAG_NULL
    }
...
#ifdef _WIN32
    int64_t ret = ((XmJitFn) jit_entry)((intptr_t) coro, raw_args);
#else
    XrJitResult jr = ((XmJitFn) jit_entry)((intptr_t) coro, raw_args);
    ...
    if (ret == XM_DEOPT_MARKER) {
        // Deopt triggered: registers saved in xr_coro_jit_state(coro)->scratch->deopt_regs[] by
        // stub. If JIT helper set an exception (e.g. division by zero), propagate to VM exception
        // system so try-catch can handle it properly.
        if (xr_coro_jit_state(coro)->scratch->exception) {
            XrValue exc;
            exc.descriptor = 0;
            exc.tag = XR_TAG_PTR;
            exc.ptr = xr_coro_jit_state(coro)->scratch->exception;
        ...
        {
            XrProto *dp = (XrProto *) xr_coro_jit_state(coro)->scratch->call_proto;
...
int xm_jit_resume(XrCoroutine *coro, XrValue *result) {
    ...
    XR_DCHECK(jit_state->suspend != NULL, "xm_jit_resume: NULL jit_suspend");

    // Restore JIT scratch context from the proto that was suspended
    XrProto *proto = (XrProto *) jit_state->resume_proto;
    if (!proto)
        return XM_JIT_DEOPT;
...
#ifdef _WIN32
    int64_t ret = ((XmJitFn) resume_entry)((intptr_t) coro, NULL);
#else
    XrJitResult jr = ((XmJitFn) resume_entry)((intptr_t) coro, NULL);
    int64_t ret = jr.payload;
#endif
    ...
    if (ret == (int64_t) XM_SUSPEND_MARKER) {
        ...
        // Return XM_JIT_SUSPEND on the stack so caller does not need to
...
void xm_jit_read_multi_ret(XrCoroutine *coro, XrValue *results, int nresults) {
    XrJitScratch *ctx = xr_coro_jit_state(coro)->scratch;
...
int32_t xm_jit_deopt_recover(XrCoroutine *coro, XrValue *frame, int maxstack) {
    ...
    for (uint16_t i = 0; i < entry->nslots; i++) {
        ...
        switch (s->loc_kind) {
            case DEOPT_LOC_REG:
                if (s->loc.phys_reg < 29)
                    raw = xr_coro_jit_state(coro)->scratch->deopt_regs[s->loc.phys_reg];
                break;
            case DEOPT_LOC_FP_REG:
                if (s->loc.phys_reg < 16)
                    raw = xr_coro_jit_state(coro)->scratch->deopt_fp_regs[s->loc.phys_reg];
                break;
            case DEOPT_LOC_SPILL: {
                ...
                if (slot >= 0 && slot < 32)
                    raw = xr_coro_jit_state(coro)->scratch->deopt_spill_save[slot];
                break;
            }
        ...
        if (tag == XR_RTAG_UNKNOWN && s->vreg_idx < XR_JIT_MAX_VREG_TAGS) {
            uint8_t rt = xr_coro_jit_state(coro)->scratch->vreg_runtime_tags[s->vreg_idx];
...
// Args are pre-stored in xr_coro_jit_state(coro)->scratch->call_args by JIT code.
// Proto pointer is in xr_coro_jit_state(coro)->scratch->call_proto (set by JIT
// prologue-equivalent). extra_arg is unused (0).
