...
#include "../../include/xray_hosted_fragment_runtime.h"
...
const XrHostedFragmentHostOps xr_hosted_fragment_host_ops = {
...
const void *xr_hosted_fragment_runtime_ops(void) {
    return &g_stdlib_aot_vm_host_ops;
}

void *xr_hosted_fragment_current_coroutine(XrVMRuntime *isolate) {
    return xr_current_coro(isolate);
}

bool xr_hosted_fragment_context_init(XrVMRuntime *isolate, const char *module_name,
                                     XrHostedFragmentContext *context,
                                     XrHostedFragmentSignal *signal) {
    if (!isolate || !context || !signal)
        return false;
...
XrValue xr_hosted_fragment_int(int64_t value) {
    return xr_int(value);
}

bool xr_hosted_fragment_as_int(XrValue value, int64_t *out) {
    if (!out || !XR_IS_INT(value))
        return false;
    *out = value.i;
    return true;
}
    ...
    switch ((XrHostedFragmentStatus) signal->status) {
        ...
        case XR_HOSTED_FRAGMENT_SUSPEND:
            if (!signal->continuation || !continuation ||
                ...
                (void) xr_hosted_fragment_handle_signal(isolate, symbol, signal);
                return XR_CFUNC_ERROR;
            }
