...
XrType *xa_visit_call(XaInferContext *ctx, AstNode *node) {
    ...
    if (call->callee && call->callee->type == AST_MEMBER_ACCESS && call->type_arg_count == 1) {
        ...
        if (ma->name && strcmp(ma->name, "decode") == 0 && ma->object &&
            ...
            for (int i = 0; structural_target && i < target_type->object.field_count; i++) {
                XrType *field_type =
            ...
            if (structural_target && (!target_capability.supported ||
                                      !xr_type_is_json_decode_field_supported(target_type))) {
                XrLocation loc = {
                    .file = ctx->file_path, .line = node->line, .column = node->column};
                char msg[256];
    ...
    if (callee_type->kind == XR_KIND_CLASS) {
        ...
        if (call->callee && call->callee->type == AST_VARIABLE) {
            ...
            XaSymbol *class_sym =
                fn_sym && fn_sym->kind == XA_SYM_CLASS ? fn_sym : NULL;
            if (!class_sym)
