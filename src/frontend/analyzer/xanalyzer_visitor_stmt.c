...
void xa_visit_var_decl_stmt(XaInferContext *ctx, AstNode *node) {
    ...
    if (!var->initializer && !links->declared_type) {
        ...
        snprintf(msg, sizeof(msg),
            "Variable '%s' must have a type annotation or initializer", var->name);
        xa_analyzer_add_diagnostic(ctx->analyzer, XR_DIAG_SEV_ERROR, XR_ERR_ANALYZE_MISSING_TYPE, msg, &loc);
    }
    ...
    if (var->initializer) {
        ...
        var->initializer->compile_type = init_type;
        ...
        if (links->declared_type && !XR_TYPE_IS_UNKNOWN(links->declared_type)) {
            ...
            if (!null_err && !xr_type_assignable(links->declared_type, init_type)) {
                // Json/JsonValue→concrete type: allowed at compile time, runtime check inserted by codegen.
                // e.g. let x: int = json["key"] is legal but requires runtime validation.
                if (!xr_is_json_coercion(links->declared_type, init_type)) {
                    ...
                    xa_analyzer_add_diagnostic(ctx->analyzer, XR_DIAG_SEV_ERROR, XR_ERR_ANALYZE_TYPE_MISMATCH, msg, &loc);
                }
            }
            ...
            if (var->initializer && var->initializer->type == AST_ARRAY_LITERAL &&
                var->initializer->as.array_literal.count == 0 &&
                XR_TYPE_IS_ARRAY(init_type) &&
                ...
                xa_analyzer_add_diagnostic(ctx->analyzer, XR_DIAG_SEV_ERROR, XR_ERR_ANALYZE_TYPE_MISMATCH,
                    "Empty array '[]' requires a type annotation, e.g. let x: Array<int> = []", &loc);
            }
        }
...
void xa_visit_assignment_stmt(XaInferContext *ctx, AstNode *node) {
    ...
    if (sym->is_const) {
        ...
        xa_analyzer_add_diagnostic(ctx->analyzer, XR_DIAG_SEV_ERROR, XR_ERR_ANALYZE_CONST_ASSIGN, msg, &loc);
        return;
    }
    ...
    if (sym->kind == XA_SYM_PARAMETER && sym->passing_mode == XR_PARAM_IN) {
        ...
        char msg[128];
        snprintf(msg, sizeof(msg),
            "Cannot assign to 'in' parameter '%s' (readonly reference)", assign->name);
    ...
    if (!XR_TYPE_IS_UNKNOWN(var_type)) {
        XrLocation loc = { .file = ctx->file_path, .line = node->line, .column = node->column };
        // Check null safety first (null→T, T?→T)
        bool null_err = xa_check_null_safety(ctx->analyzer, var_type, value_type,
            "Assignment", &loc);
        if (!null_err && !xr_type_assignable(var_type, value_type)) {
            ...
            if (!xr_is_json_coercion(var_type, value_type)) {
                char msg[256];
                snprintf(msg, sizeof(msg),
                    "Type '%s' is not assignable to '%s' (type '%s')",
...
void xa_visit_if_stmt(XaInferContext *ctx, AstNode *node) {
    ...
    if (ctx->flow) {
        ctx->flow->current_flow = xa_flow_create_condition(ctx->flow,
    ...
    if (ctx->flow) ctx->flow->current_flow = saved;
    ...
    if (ret->value_count == 0) {
        ...
        if (ret->values[0]) {
            ...
            if (ctx->expected_return_type && !XR_TYPE_IS_UNKNOWN(ctx->expected_return_type)) {
                ...
                XaScope *s = ctx->analyzer->current_scope;
                while (s && s->kind != XA_SCOPE_FUNCTION) s = s->parent;
                if (s && s->ast_node) {
