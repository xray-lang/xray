...
static const char xr_native_def_array[] =
    ...
    "Bytes (Array<uint8>) decodes as UTF-8 text — round-trips with\n    // string.toBytes(). Other "
    "element types render the \"[a, b]\" form.\n    toString() -> string\n    // Iteration "
    "protocol — used by for-in lowering. Users can also\n    // call them directly; "
...
static const char xr_native_def_string[] =
    ...
    "Every method declared here must exist in the VM method table\n// (xstring_methods.c) with "
    "matching semantics; methods also supported by\n// the AOT dispatch (src/aot/xrt_method.h + "
    "xi_method_sym.def) compile to\n// native code, while VM-only methods fail AOT codegen with an "
    "explicit\n// \"unsupported AOT method\" error (never a silent null).\n\n@native\nclass string "
    ...
    "-> bool\n    padStart(length: int, pad?: string) -> string\n    padEnd(length: int, pad?: "
    "string) -> string\n    match(pattern: Regex) -> Array<string>?\n    // UTF-8 byte bridge — "
    "round-trips with Bytes.toString().\n    toBytes() -> Bytes\n    translate(table: Map<string, "
    "string>) -> string\n    // Iteration protocol — iterator() yields each character, while\n    "
    "// entriesIterator() yields (charIndex, character) tuples. Both\n    // are used by for-in "
    "lowering and may be called directly.\n    iterator() -> Iterator<char>\n    entriesIterator() "
