...
static const XaBuiltinMember g_gen_encoding_functions[] = {
    ...
    {"utf16Encode", "(data: string, endian?: int): Array<uint8>", "UTF-16 encode to bytes", true,
     false},
    {"utf16Decode", "(data: string | Array<uint8>, endian?: int, stripBom?: bool): string?",
     "UTF-16 decode to string (auto-detects BOM)", true, false},
...
static const XaBuiltinHandleField g_gen_http_httpresponse_fields[] = {
    ...
    {"body", "string", false}, {"error", "string", true},      {"ok", "bool", true},
};
...
static const XaBuiltinHandleField g_gen_http_downloadresult_fields[] = {
    ...
    {"completed", "bool", false}, {"error", "string", true},
};
...
static const XaBuiltinMember g_gen_http_functions[] = {
    ...
    {"route", "(method: string, path: string, handler: fn): ()", "Register a route handler", true,
     false},
    {"static", "(prefix: string, dir: string): ()", "Serve static files from directory", true,
     false},
    {"stopServer", "(): ()", "Stop the HTTP server", true, false},
    {"parseRequest", "(fd: int): Array<unknown>?", "Parse raw HTTP request data", true, false},
    {"sendResponse", "(fd: int, body: string, status?: int): bool", "Send HTTP response on fd",
     true, false},
    ...
    {"formDataAppend", "(form: Json, name: string, value: string): ()", "Append field to form data",
     true, false},
    {"formDataAppendFile", "(form: Json, name: string, path: string, filename?: string): ()",
...
static const XaBuiltinHandleField g_gen_io_filestat_fields[] = {
    ...
    {"atime", "int", true},       {"ctime", "int", false},  {"uid", "int", true},
...
static const XaBuiltinMember g_gen_net_functions[] = {
    {"dial", "(host: string, port: int, timeout?: int): NetConn?", "Dial a TCP connection", true,
     false},
    {"listen", "(port: int, backlog?: int): NetListener?", "Start listening on a port", true,
     false},
    ...
    {"readInto", "(conn: NetConn, buffer: Bytes, maxlen?: int): int",
     "Read data into a reusable Bytes buffer", true, false},
    {"write", "(conn: NetConn, data: string): int", "Write data to connection", true, false},
    {"writeBytes", "(conn: NetConn, data: Bytes): int", "Write Bytes data to connection", true,
     false},
    {"copy", "(src: NetConn, dst: NetConn, bufferSize?: int): int",
     "Copy a TCP/TLS stream using a reusable native buffer", true, false},
    {"copyBidirectional", "(a: NetConn, b: NetConn): Json",
     "Copy two TCP/TLS streams in both directions", true, false},
    {"shutdownRead", "(conn: NetConn): bool", "Shut down the read side of a TCP connection", true,
     false},
    {"shutdownWrite", "(conn: NetConn): bool", "Shut down the write side of a TCP connection", true,
     false},
    ...
    {"setReadDeadline", "(conn: NetConn, deadline: int): bool", "Set read deadline in monotonic ms",
     true, false},
    {"setWriteDeadline", "(conn: NetConn, deadline: int): bool",
     "Set write deadline in monotonic ms", true, false},
    {"setDeadline", "(conn: NetConn, deadline: int): bool",
     "Set read and write deadlines in monotonic ms", true, false},
    {"setAcceptDeadline", "(listener: NetListener, deadline: int): bool",
     "Set accept deadline in monotonic ms", true, false},
    {"lastError", "(handle: NetConn | NetListener): string?", "Return the last network error name",
     true, false},
    {"lastErrno", "(handle: NetConn | NetListener): int", "Return the last system errno", true,
     false},
    {"lookup", "(hostname: string): string?", "DNS lookup", true, false},
    {"hasTLS", "(): bool", "Check if TLS support is available", true, false},
    {"dialTLS", "(host: string, port: int, timeout?: int): NetConn?", "Dial a TLS connection", true,
     false},
    {"upgradeTLS", "(conn: NetConn, hostname: string, timeout?: int): NetConn?",
     "Upgrade connection to TLS", true, false},
...
static const XaBuiltinMember g_gen_regex_functions[] = {
    ...
    {"findAll", "(pattern: Regex, s: string, limit?: int): Array<RegexMatch>", "Find all matches",
     true, false},
...
static const XaBuiltinMember g_gen_ws_functions[] = {
    ...
    {"recvData", "(conn: WsConn, timeout?: int): string?",
     "High-performance recv returning data string directly (no Json wrapper)", true, false},
    {"echoServe", "(port: int): bool", "Pure C echo server with zero VM/GC overhead per message",
     true, false},
};
