...
static uint32_t next_symbol_id(void) {
    if (g_symbol_id_ptr) return (*g_symbol_id_ptr)++;
    return 0;
}
...
static void links_release_dynamic(XaSymbolLinks *links) {
    if (!links) return;
    if (links->param_types) xr_free(links->param_types);
    if (links->param_names) {
        for (int i = 0; i < links->param_count; i++) {
            if (links->param_names[i]) xr_free((char*)links->param_names[i]);
        }
...
void xa_symbol_free(XaSymbol *symbol) {
    ...
    if (symbol->name) xr_free((void*)symbol->name);
    xr_free(symbol);
}
...
XaRefLocation *xa_symbol_get_refs(XaSymbolLinks *links, int *count) {
    ...
    if (count) *count = links->ref_count;
...
void xa_scope_add_symbol(XaScope *scope, XaSymbol *symbol) {
    if (!scope || !symbol || !symbol->name) return;
    symbol->scope = scope;
...
void xa_class_info_free(XrClassInfo *info) {
    if (!info) return;
    if (info->name) xr_free((void*)info->name);
    if (info->fields) xr_free(info->fields);
    if (info->methods) xr_free(info->methods);
    if (info->static_fields) xr_free(info->static_fields);
    if (info->static_methods) xr_free(info->static_methods);
    if (info->constructor_params) xr_free(info->constructor_params);
    if (info->vtable) xr_free(info->vtable);
    if (info->struct_layout) xr_free(info->struct_layout);
    if (info->members_map) xr_hashmap_free(info->members_map);
    xr_free(info);
}
...
XaSymbol *xa_class_info_lookup_member(XrClassInfo *info, const char *name) {
    ...
    if (info->members_map) {
        ...
        for (int i = 0; i < info->method_count; i++) {
            if (strcmp(info->methods[i]->name, name) == 0) return info->methods[i];
        }
        for (int i = 0; i < info->static_field_count; i++) {
            if (strcmp(info->static_fields[i]->name, name) == 0) return info->static_fields[i];
        }
...
XrType **xa_symbol_links_get_param_types(XaSymbolLinks *links, int *count) {
    ...
    if (count) *count = links->param_count;
...
const char *xa_symbol_links_get_type_param_name(XaSymbolLinks *links, int index) {
    if (!links || index < 0 || index >= links->type_param_count) return NULL;
...
XaSymbol *xa_scope_lookup_by_id(XaScope *scope, uint32_t id) {
    ...
    for (uint32_t i = 0; i < map->capacity; i++) {
        ...
        if (entry->key && entry->value != XR_HASHMAP_TOMBSTONE) {
            ...
            if (sym && sym->id == id) return sym;
        }
    }
    if (scope->parent) return xa_scope_lookup_by_id(scope->parent, id);
    return NULL;
}
