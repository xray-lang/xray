...
struct XaTypedProgram {
    ...
    XaNodeProviderCallEntry *provider_calls;
...
XaTypedProgramPublishResult xa_typed_program_publish(struct XaAnalyzer *analyzer,
    ...
    if (!xa_node_table_snapshot_provider_calls((const XaNodeTable *) analyzer->node_table,
                                               &program->provider_calls,
                                               &program->provider_call_count)) {
        ...
        xr_free(program);
        result.reason = XA_TYPED_PROGRAM_REASON_ANALYSIS_RESOURCE_FAILURE;
        result.detail = "provider-call snapshot allocation failed (AnalysisResourceFailure)";
        return result;
    }
    if (!xa_node_table_snapshot_callable_target_sets((const XaNodeTable *) analyzer->node_table,
        ...
        xr_free(program->provider_calls);
        xr_free(program->target_queries);
        xr_free(program->function_expr_effects);
        xr_free(program->call_error_effects);
        xr_free(program->conversions);
...
bool xa_typed_program_provider_call(const XaTypedProgram *program, const struct AstNode *call_node,
                                    XaProviderCallFact *out_fact) {
    if (!xa_typed_program_is_current(program) || !call_node || call_node->type != AST_CALL_EXPR)
        return false;
    uint32_t low = 0;
    uint32_t high = program->provider_call_count;
    while (low < high) {
        uint32_t mid = low + (high - low) / 2;
        const XaNodeProviderCallEntry *entry = &program->provider_calls[mid];
        if (entry->node_id < call_node->node_id)
            low = mid + 1;
        else if (entry->node_id > call_node->node_id)
            high = mid;
