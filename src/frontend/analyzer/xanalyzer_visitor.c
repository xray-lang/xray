 ...
 * symbol links rather than on the XR_KIND_TYPE_PARAM value, so this resolves
 * them through xa_lookup_type_param_constraints, the same scope walk the
 * Hashable check uses.
 *
 * Methods, interface methods, functions and classes are all equivalent here:
 * a method's own `<T: ...>` list reaches its symbol links like any other. */
static bool xa_type_param_iteration_element(XaInferContext *ctx, XrType *type_param,
                                            XrType **out_elem) {
    if (!ctx || !ctx->analyzer || !type_param || type_param->kind != XR_KIND_TYPE_PARAM)
        return false;
    if (xa_analyzer_is_iterable(ctx->analyzer, type_param->type_param.constraint, out_elem))
        return true;
    ...
    if (!name)
        return false;
    int count = 0;
    XrType **constraints = xa_lookup_type_param_constraints(ctx, NULL, name, &count, NULL);
    for (int i = 0; i < count; i++) {
        if (xa_analyzer_is_iterable(ctx->analyzer, constraints ? constraints[i] : NULL, out_elem))
            return true;
    }
    return false;
}
        ...
        case AST_FOR_IN_STMT: {
            ...
            if (is_enum_iter) {
                ...
                if (xr_type_is_enum_metadata_named(coll_type, XR_ENUM_VARIANTS_TYPE_NAME)) {
                    ...
                    if (xa_type_param_iteration_element(ctx, coll_type, &elem)) {
                        if (fi->is_keyvalue) {
            ...
            /* for-in has no modeled loop header; `continue` and `break`
             * edges rejoin the linear flow at the loop tail so their writes
             * stay visible after the loop (spec §2.13 N-8). */
            if (ctx->flow) {
