...
static XaJsonCapabilityResult json_capability_result(bool supported,
...
static bool json_capability_stack_contains(const XaJsonCapabilityWalk *walk,
                                           const XrType *type) {
...
static XaJsonCapabilityResult json_capability_visit(const XrType *type,
                                                    XaJsonCapabilityMode mode,
        ...
        case XR_KIND_INSTANCE:
            if (!type->instance.class_ref) {
                return json_capability_result(
                    false, mode == XA_JSON_CAPABILITY_ENCODE
    ...
    if (json_capability_stack_contains(walk, type) || walk->depth >= 64) {
        return json_capability_result(false,
                                      XA_JSON_CAPABILITY_UNSUPPORTED_RECURSIVE_ALIAS, type);
    }
    ...
    switch (type->kind) {
        ...
        case XR_KIND_MAP:
            if (!type->map.key_type || !XR_TYPE_IS_STRING(type->map.key_type) ||
                ...
                result = json_capability_result(false,
                                                XA_JSON_CAPABILITY_NON_STRING_MAP_KEY, type);
            } else {
