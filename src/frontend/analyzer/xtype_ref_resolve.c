...
/* Arity of every built-in type head, taken from builtin_symbols.def. The
 * prelude rows are listed there with their arity too, so no second lookup
 * through the prelude symbol table is needed. */
