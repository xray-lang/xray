...
static void ea_register_var(EaContext *ctx, const char *name, AstNode *decl) {
    ...
    if (scope->count >= EA_MAX_VARS_PER_SCOPE) return;
...
static bool ea_is_channel_send(AstNode *callee) {
    if (!callee || callee->type != AST_MEMBER_ACCESS) return false;
...
static void ea_walk(EaContext *ctx, AstNode *node) {
    ...
    case AST_FUNCTION_EXPR: {
        ...
        if (fn->body) ea_walk(ctx, fn->body);
    ...
    case AST_LITERAL_STRING: case AST_LITERAL_NULL:
    case AST_LITERAL_TRUE: case AST_LITERAL_FALSE:
    case AST_BREAK_STMT: case AST_CONTINUE_STMT:
    case AST_THIS_EXPR: case AST_YIELD_STMT:
    case AST_CANCELLED_EXPR: case AST_CHANNEL_NEW:
