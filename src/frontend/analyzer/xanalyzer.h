...
struct XaAnalyzer {
     ...
     * this analyzer so the id stamped on the clone resolves correctly. The
     * scope is a child of global_scope purely for teardown ownership — no
     * lexical lookup chain passes through it, so its names never become
     * visible to user code. Created on first use. */
