...
AstNode *xr_parse_if_statement(Parser *parser) {
    ...
    if (xr_parser_check(parser, TK_NAME) &&
        ...
        return xr_ast_if_stmt(parser->X, condition, then_branch, NULL, line);
    }
...
AstNode *xr_parse_for_in_statement(Parser *parser) {
    ...
    AstNode *body = xr_parse_block(parser);
    if (is_keyvalue) {
        return xr_ast_for_in_keyvalue_stmt(parser->X, first_name, second_name,
                                           item_type, collection, body, line);
    } else {
        return xr_ast_for_in_stmt(parser->X, first_name, item_type, collection, body, line);
    }
}
