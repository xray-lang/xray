...
ClassInfo *xr_class_registry_register(ClassRegistry *registry, const char *class_name) {
    ...
    if (registry->class_count >= registry->capacity) {
        ...
        ClassInfo **new_classes =
            (ClassInfo **) xr_realloc(registry->classes, sizeof(ClassInfo *) * new_capacity);
        if (!new_classes)
            return NULL;
...
ClassInfo *xr_class_registry_lookup(ClassRegistry *registry, const char *class_name) {
    if (!registry || !class_name)
        return NULL;
...
bool xr_class_registry_is_class(ClassRegistry *registry, const char *name) {
    return xr_class_registry_lookup(registry, name) != NULL;
}

int xr_class_add_instance_field(ClassInfo *class_info, const char *field_name, int index) {
    ...
    if (class_info->instance_field_count >= class_info->instance_field_capacity) {
        ...
        FieldInfo *new_fields =
            (FieldInfo *) xr_realloc(class_info->instance_fields, sizeof(FieldInfo) * new_capacity);
        if (!new_fields)
            return -1;
...
int xr_class_add_instance_field_typed(ClassInfo *class_info, const char *field_name, int index,
                                      uint8_t slot_type) {
    if (!class_info || !field_name)
        return -1;
...
uint8_t xr_class_get_field_slot_type(ClassInfo *class_info, const char *field_name) {
    if (!class_info || !field_name)
        return 0;
...
int xr_class_find_instance_field_index(ClassInfo *class_info, const char *field_name) {
    if (!class_info || !field_name)
        return -1;
...
int xr_class_add_method(ClassInfo *class_info, const char *method_name, int index) {
    ...
    if (class_info->method_count >= class_info->method_capacity) {
        ...
        MethodInfo *new_methods =
            (MethodInfo *) xr_realloc(class_info->methods, sizeof(MethodInfo) * new_capacity);
        if (!new_methods)
            return -1;
...
int xr_class_find_method_index(ClassInfo *class_info, const char *method_name) {
    if (!class_info || !method_name)
        return -1;
