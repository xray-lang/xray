...
void compile_import(XrCompilerContext *ctx, XrCompiler *c, ImportStmtNode *node) {
    ...
    XrString *module_name = xr_compile_time_intern(ctx->X, node->module_name, strlen(node->module_name));
    ...
    if (node->member_count > 0) {
        for (int i = 0; i < node->member_count; i++) {
            ...
            if (existing && existing->depth == c->scope_depth) {
                xr_compiler_error(ctx, c, "import member '%s' conflicts with existing variable", local_name);
            ...
            int global_sym = xr_symbol_register_in_table(xr_isolate_get_symbol_table(ctx->X), member->name);
        ...
        if (existing_shared >= 0) {
            xr_compiler_error(ctx, c, "module variable '%s' already defined, use 'import ... as alias' to specify different name", var_name);
            reg_free(c, module_reg);
            return;
        }
...
void compile_export(XrCompilerContext *ctx, XrCompiler *c, ExportStmtNode *node) {
    ...
    if (c->scope_depth > 0 || c->type != FUNCTION_SCRIPT) {
        xr_compiler_error(ctx, c, "export can only be used at module top-level, not inside function or block");
        return;
    }
    ...
    if (node->from_path) {
        ...
        XrString *module_name = xr_compile_time_intern(ctx->X, node->from_path, strlen(node->from_path));
        XrValue module_name_val = xr_string_value(module_name);
        int name_idx = xr_vm_proto_add_constant(c->proto, module_name_val);
