...
XR_FUNC int compile_call_internal(XrCompilerContext *ctx, XrCompiler *compiler, CallExprNode *node, bool is_tail, uint8_t *out_slot_type);
...
XR_FUNC int compile_exprs_consecutive(XrCompilerContext *ctx, XrCompiler *compiler,
...
XR_FUNC XrLocalInfo* scope_define_local(XrCompilerContext *ctx, XrCompiler *compiler, XrString *name);

// Define local variable with specific register (for method parameters)
XR_FUNC XrLocalInfo* scope_define_local_reg(XrCompilerContext *ctx, XrCompiler *compiler, XrString *name, int reg);
...
XR_FUNC void shared_reset_state(XrCompilerContext *ctx, int index);  // Reset to OWNED on reassignment
