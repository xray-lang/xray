...
// Return value: register number, or -2 meaning "not handled, fall through"
...
int compile_call_internal(XrCompilerContext *ctx, XrCompiler *compiler, CallExprNode *node, bool is_tail, uint8_t *out_slot_type_unused) {
    ...
    if (node->callee->type == AST_VARIABLE) {
        VariableNode *callee_var = &node->callee->as.variable;
        
        const BuiltinEntry *entry = builtin_lookup(callee_var->name);
        if (entry) {
            int result = entry->compile(ctx, compiler, node);
            if (result != BUILTIN_NOT_HANDLED) {
                // Builtins emit specialized opcodes (e.g. OP_TOINT), not
                // OP_CALL/OP_TAILCALL. If called in tail position, the
                // caller assumes TAILCALL has implicit return semantics,
                // so we must emit RETURN1 explicitly here.
                if (is_tail) {
            ...
            if (!is_static_call) {
                ...
                for (int ui = 0; ui < uv_count; ui++) {
                    if (compiler->upvalues[ui].name &&
                        strcmp(compiler->upvalues[ui].name, var_name) == 0) {
