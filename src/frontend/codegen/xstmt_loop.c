 ...
 */
static int emit_leaf_cond_jump(
    XrCompilerContext *ctx, XrCompiler *compiler,
    AstNode *cond, int k_extra)
{
    ...
    if (cond->type >= AST_BINARY_LT && cond->type <= AST_BINARY_GE) {
        ...
        switch (cond->type) {
            case AST_BINARY_LT: cmp_op = OP_LT; cmp_op_imm = OP_LTI; break;
            case AST_BINARY_LE: cmp_op = OP_LE; cmp_op_imm = OP_LEI; break;
            case AST_BINARY_GT: cmp_op = OP_LT; cmp_op_imm = OP_LEI; swap_reg = true; k_imm ^= 1; break;
            case AST_BINARY_GE: cmp_op = OP_LE; cmp_op_imm = OP_LTI; swap_reg = true; k_imm ^= 1; break;
            default: cmp_op = OP_LT; cmp_op_imm = OP_LTI; break;
        }
 ...
 */
static void compile_cond_chain(
    XrCompilerContext *ctx, XrCompiler *compiler,
    AstNode *cond, int *false_list, int *true_list)
{
...
compile_body:
    ;  // Empty statement (C label requires statement)
