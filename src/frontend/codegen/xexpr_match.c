    ...
    for (int i = 0; i < node->arm_count; i++) {
        ...
        // 4.2 Handle literal pattern (including binding variables)
            ...
            if (value->type == AST_VARIABLE && guard) {
                 ...
                 * Note: binding variable doesn't need explicit removal, it's just a local variable definition
                 * Subsequent branches will override or scope naturally expires after branch ends
                 */
