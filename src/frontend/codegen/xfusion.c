            ...
            if (op2 == OP_ADD || op2 == OP_SUB || op2 == OP_MUL) {
                ...
                if (c == reg_k && a != reg_k && bx < PROTO_CONST_COUNT(proto)) {
                    ...
                    if (xr_fusion_is_small_int(kval, &imm)) {
                         ...
                         * Solution: Backtrack to check R[b]'s source, ensure it's not a string constant
                         */
                        bool is_safe = true;
                        ...
                        for (int i = pc - 1; i >= 0 && i >= pc - 10; i--) {
                            ...
                            if (prev_op == OP_LOADK && prev_a == b) {
                                ...
                                if (prev_bx < PROTO_CONST_COUNT(proto)) {
                                    ...
                                    // If left operand is string, cannot use integer immediate optimization
...
** 3. Correctly handle comparison instruction semantics (R[A] < sB, immediate in second operand position)
*/
int xr_fusion_cmp_const(XrProto *proto) {
    ...
    for (int pc = 0; pc < PROTO_CODE_COUNT(proto) - 1; pc++) {
        ...
        if (op1 == OP_LOADK) {
            ...
            if ((op2 == OP_LT || op2 == OP_LE) &&
                bx < PROTO_CONST_COUNT(proto)) {
