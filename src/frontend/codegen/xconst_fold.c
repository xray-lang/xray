...
static XrConstEvalResult fold_unary_value(AstNodeType op, XrValue operand) {
    switch (op) {
        case AST_UNARY_NEG:
            if (XR_IS_INT(operand))   return success_result(xr_int(-XR_TO_INT(operand)));
            if (XR_IS_FLOAT(operand)) return success_result(xr_float(-XR_TO_FLOAT(operand)));
            return fail_result();
        case AST_UNARY_NOT: {
...
static XrConstEvalResult fold_binary_values(AstNodeType op, XrValue lv, XrValue rv) {
    ...
    if (XR_IS_INT(lv) && XR_IS_INT(rv)) {
        ...
        switch (op) {
            case AST_BINARY_ADD:
                if (__builtin_add_overflow(l, r, &res)) return fail_result();
                return success_result(xr_int(res));
            case AST_BINARY_SUB:
                if (__builtin_sub_overflow(l, r, &res)) return fail_result();
                return success_result(xr_int(res));
            case AST_BINARY_MUL:
                if (__builtin_mul_overflow(l, r, &res)) return fail_result();
                return success_result(xr_int(res));
            case AST_BINARY_DIV:
                if (r == 0 || (l == INT64_MIN && r == -1)) return fail_result();
                return success_result(xr_int(l / r));
            case AST_BINARY_MOD:
                if (r == 0 || (l == INT64_MIN && r == -1)) return fail_result();
                return success_result(xr_int(l % r));
            case AST_BINARY_BAND: return success_result(xr_int(l & r));
            case AST_BINARY_BOR:  return success_result(xr_int(l | r));
            case AST_BINARY_BXOR: return success_result(xr_int(l ^ r));
            case AST_BINARY_LSHIFT:
                if (r < 0 || r >= 64) return fail_result();
                return success_result(xr_int(l << r));
            case AST_BINARY_RSHIFT:
                if (r < 0 || r >= 64) return fail_result();
                return success_result(xr_int(l >> r));
            case AST_BINARY_LT:  return success_result(make_bool(l < r));
            case AST_BINARY_LE:  return success_result(make_bool(l <= r));
            case AST_BINARY_GT:  return success_result(make_bool(l > r));
            case AST_BINARY_GE:  return success_result(make_bool(l >= r));
            case AST_BINARY_EQ:
            ...
            case AST_BINARY_NE_STRICT:
                return success_result(make_bool(l != r));
            case AST_BINARY_AND: return success_result(make_bool(l && r));
            case AST_BINARY_OR:  return success_result(make_bool(l || r));
            default: return fail_result();
        }
    }
    ...
    if ((XR_IS_INT(lv) || XR_IS_FLOAT(lv)) && (XR_IS_INT(rv) || XR_IS_FLOAT(rv))) {
        ...
        switch (op) {
            case AST_BINARY_ADD: return success_result(xr_float(l + r));
            case AST_BINARY_SUB: return success_result(xr_float(l - r));
            case AST_BINARY_MUL: return success_result(xr_float(l * r));
            case AST_BINARY_DIV:
                if (r == 0.0) return fail_result();
                return success_result(xr_float(l / r));
            case AST_BINARY_LT:  return success_result(make_bool(l < r));
            case AST_BINARY_LE:  return success_result(make_bool(l <= r));
            case AST_BINARY_GT:  return success_result(make_bool(l > r));
            case AST_BINARY_GE:  return success_result(make_bool(l >= r));
            case AST_BINARY_EQ:  return success_result(make_bool(l == r));
            case AST_BINARY_NE:  return success_result(make_bool(l != r));
            default: return fail_result();
        }
    }
...
static XrConstEvalResult fold_string_binary(AstNodeType op, XrValue lv, XrValue rv, XrayIsolate *X) {
    ...
    if (XR_IS_STRING(lv) && XR_IS_INT(rv) && op == AST_BINARY_MUL) {
        ...
        if (count > 10000) return fail_result();
...
static XrConstEvalResult eval_literal(XrayIsolate *X, AstNode *expr) {
    switch (expr->type) {
        ...
        case AST_LITERAL_TRUE:  return success_result(make_bool(true));
        case AST_LITERAL_FALSE: return success_result(make_bool(false));
        case AST_LITERAL_NULL:  return success_result(xr_null());
        default: return fail_result();
    }
}
...
XrConstEvalResult xr_const_eval(XrayIsolate *X, AstNode *expr) {
    ...
    // Unary
    if (expr->type == AST_UNARY_NEG || expr->type == AST_UNARY_NOT || expr->type == AST_UNARY_BNOT) {
        XrConstEvalResult operand = xr_const_eval(X, expr->as.unary.operand);
        if (!operand.success) return fail_result();
    ...
    if (is_binary_node(expr->type)) {
        ...
        if (!left.success) return fail_result();
        XrConstEvalResult right = xr_const_eval(X, bin->right);
        if (!right.success) return fail_result();
...
XrConstEvalResult xr_const_eval_with_ctx(XrCompilerContext *ctx, AstNode *expr) {
    if (!ctx || !expr) return fail_result();
    ...
    if (expr->type == AST_VARIABLE) {
        ...
        if (entry) {
            switch (entry->type) {
                case CONST_INT:    return success_result(xr_int(entry->value.int_val));
                case CONST_FLOAT:  return success_result(xr_float(entry->value.float_val));
                case CONST_STRING: return success_result(xr_string_value(entry->value.str_val));
                case CONST_BOOL:   return success_result(make_bool(entry->value.bool_val));
                case CONST_NULL:   return success_result(xr_null());
            }
        }
        return fail_result();
    }
