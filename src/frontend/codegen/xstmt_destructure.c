...
static void compile_array_destructure(XrCompilerContext *ctx, XrCompiler *compiler,
                                      XrDestructurePattern *pattern, int source_reg, bool is_const,
                                      XrType *elem_type);
...
static void compile_array_elem_to_var(XrCompilerContext *ctx, XrCompiler *compiler,
                                      const char *name, int source_reg, int index, bool is_const,
                                      XrType *elem_type) {
    (void) is_const;
    ...
    if (compiler->scope_depth == 0) {
        ...
        if (elem_type)
            local_set_compile_type(local, elem_type);
...
static void compile_object_field_to_var(XrCompilerContext *ctx, XrCompiler *compiler,
    ...
    if (compiler->scope_depth == 0) {
        ...
        XrLocalInfo *local = scope_define_local(ctx, compiler, name_str);
        if (field_type)
            local_set_compile_type(local, field_type);
        target_reg = local->reg;
    }

    int global_sym =
        xr_symbol_register_in_table((XrSymbolTable *) xr_isolate_get_symbol_table(ctx->X), field);
...
static void compile_array_destructure(XrCompilerContext *ctx, XrCompiler *compiler,
    ...
    for (int i = 0; i < pattern->as.array.element_count; i++) {
        ...
        if (elem->type == PATTERN_IDENTIFIER) {
            compile_array_elem_to_var(ctx, compiler, elem->as.identifier.name, source_reg, index,
                                      is_const, elem_type);
        }
...
static void compile_object_destructure(XrCompilerContext *ctx, XrCompiler *compiler,
    ...
    for (int i = 0; i < pattern->as.object.field_count; i++) {
        ...
        if (vp->type == PATTERN_IDENTIFIER) {
            XrType *field_type =
                (init_type && field) ? xr_type_object_get_field(init_type, field) : NULL;
            compile_object_field_to_var(ctx, compiler, field, source_reg, is_const, field_type);
        }
    }
}
 ...
 */
void compile_destructure_decl(XrCompilerContext *ctx, XrCompiler *compiler,
    ...
    XrType *init_type = node->initializer ? get_expr_type(ctx, compiler, node->initializer) : NULL;

    if (node->pattern->type == PATTERN_ARRAY) {
        XrType *elem_type =
            (init_type && XR_TYPE_IS_ARRAY(init_type)) ? init_type->container.element_type : NULL;
        compile_array_destructure(ctx, compiler, node->pattern, source_reg, node->is_const,
                                  elem_type);
    } else if (node->pattern->type == PATTERN_OBJECT) {
        compile_object_destructure(ctx, compiler, node->pattern, source_reg, node->is_const,
                                   init_type);
    }
 ...
 */
void compile_destructure_assign(XrCompilerContext *ctx, XrCompiler *compiler,
...
void compile_multi_var_decl(XrCompilerContext *ctx, XrCompiler *compiler, MultiVarDeclNode *node) {
    ...
    if (value_count == 1 && node->values[0]->type == AST_CALL_EXPR) {
        ...
        if (name_count > 1) {
            ...
            for (int pc = code_count - 1; pc >= pc_before; pc--) {
                ...
                if (op == OP_CALL || op == OP_CALL_STATIC) {
                    ...
                    PROTO_SET_CODE(compiler->proto, pc, CREATE_ABC(op, _a, _b, name_count));
                    break;
                }
            }
        }
        ...
        for (int i = 0; i < name_count; i++) {
            ...
            if (var_type)
                local_set_compile_type(locals[i], var_type);
        }
    ...
    if (value_count != name_count) {
        xr_compiler_error(ctx, compiler,
                          "multi-value declaration count mismatch: left %d, right %d", name_count,
                          value_count);
        return;
    }
    ...
    for (int i = 0; i < name_count; i++) {
        ...
        if (val_type)
            local_set_compile_type(local, val_type);
 ...
 * a, b = b, a
 *
 * Key: must first calculate all right-side values to temporary registers, then assign to left-side
 * variables This way can correctly handle variable swap a, b = b, a
 */
void compile_multi_assign(XrCompilerContext *ctx, XrCompiler *compiler, MultiAssignNode *node) {
    ...
    bool is_single_call = (value_count == 1 && node->values[0]->type == AST_CALL_EXPR);

    if (!is_single_call && value_count != target_count) {
        xr_compiler_error(ctx, compiler, "multi-value assignment count mismatch: left %d, right %d",
    ...
    if (is_single_call && target_count > 1) {
        ...
        for (int pc = code_count - 1; pc >= pc_before; pc--) {
            ...
            if (op == OP_CALL || op == OP_CALL_STATIC) {
                int _a = GETARG_A(inst);
                int _b = GETARG_B(inst);
                PROTO_SET_CODE(compiler->proto, pc, CREATE_ABC(op, _a, _b, target_count));
                break;
            }
        }
    }
