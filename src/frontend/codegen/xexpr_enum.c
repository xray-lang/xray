 ...
 */
static int compile_enum_access_internal(XrCompilerContext *ctx, XrCompiler *compiler, EnumAccessNode *node) {
    ...
    XrString *enum_name_str = xr_compile_time_intern(ctx->X, node->enum_name, strlen(node->enum_name));
    ...
    XrString *member_name_str = xr_compile_time_intern(ctx->X, node->member_name, strlen(node->member_name));
 ...
 */
static int compile_enum_convert_internal(XrCompilerContext *ctx, XrCompiler *compiler, EnumConvertNode *node) {
    // Lookup enum type (check shared first, then globals)
    XrString *enum_name_str = xr_compile_time_intern(ctx->X, node->enum_name, strlen(node->enum_name));
    int enum_shared_idx = shared_get_in_scope(ctx, compiler, enum_name_str);
    int enum_global_idx = -1;
...
XrExprDesc compile_enum_convert(XrCompilerContext *ctx, XrCompiler *compiler, EnumConvertNode *node) {
    XrExprDesc e = {0};
