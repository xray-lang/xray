...
static bool check_coro_closure_upvalues(XrCompilerContext *ctx, XrCompiler *c, 
    ...
    for (int i = 0; i < upvalue_count; i++) {
        ...
        // const (non-shared) - safe: immutable, deep copied to new coro heap
        if (uv->is_const) {
            continue;
        }
        
        // shared let - forbidden (must use Move semantics)
        if (uv->storage_mode == XR_STORAGE_SHARED && !uv->is_const) {
            xr_compiler_error(ctx, c, 
                "Coroutine closure cannot capture mutable shared variable '%s'\n"
                "hint: Pass through parameter (Move semantics): go worker(%s)",
                var_name, var_name);
            has_error = true;
            continue;
        }
        
        // Normal mutable variables - forbidden
        xr_compiler_error(ctx, c, 
            "Coroutine closure cannot capture mutable variable '%s'\n"
            "hint: Use one of the following:\n"
            "  1. Pass through parameter: go worker(%s)  // automatic deep copy\n"
            "  2. Declare as const: const %s = ...",
