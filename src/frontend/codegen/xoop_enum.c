...
static XrValue eval_const_expr(XrCompilerContext *ctx, XrCompiler *compiler, AstNode *expr) {
    ...
    switch (expr->type) {
        ...
        default:
            xr_compiler_error(ctx, compiler, "Enum value must be compile-time constant (int/string/float/bool literal)");
            return xr_null();
    }
}
...
static bool values_equal(XrValue a, XrValue b) {
    ...
    if (XR_TID_IS_FLOAT(tid_a)) return XR_TO_FLOAT(a) == XR_TO_FLOAT(b);
    if (tid_a == XR_TID_STRING) return XR_TO_STRING(a) == XR_TO_STRING(b);
    return false;
}
...
void compile_enum_decl(XrCompilerContext *ctx, XrCompiler *compiler, EnumDeclNode *node) {
    ...
    for (int i = 0; i < node->member_count; i++) {
        ...
        if (member->as.enum_member.value) {
            ...
            if (xr_value_typeid(member_value) != base_type) {
                xr_compiler_error(ctx, compiler, 
                    ...
                    member_name, xr_typeid_name(base_type), xr_typeid_name(xr_value_typeid(member_value)));
                continue;
            }
            ...
            if (base_type != XR_TID_INT) {
                xr_compiler_error(ctx, compiler, 
                    "Enum member '%s' requires explicit value (only int enums support auto-increment)",
                    member_name);
        ...
        for (int j = 0; j < seen_count; j++) {
            if (seen_values[j].used && values_equal(seen_values[j].value, member_value)) {
                xr_compiler_error(ctx, compiler, 
                    "Enum member '%s' has duplicate value",
                    member_name);
                break;
            }
        }
    ...
    XrEnumType *enum_type_obj = xr_enum_type_new(ctx->X, node->name, base_type,
