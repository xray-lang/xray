...
static void compiler_record_inst_type(XrCompiler *compiler, int pc, struct XrType *type) {
    ...
    if (pc >= compiler->inst_type_cap) {
        ...
        struct XrType **buf = (struct XrType **) xr_realloc(compiler->inst_type_buf,
                                                            new_cap * sizeof(struct XrType *));
        if (!buf)
            return;
 ...
 */
static void xexpr_discharge2reg(XrCompilerContext *ctx, XrCompiler *compiler, XrExprDesc *e,
                                int target_reg) {
 ...
 */
void xexpr_to_specific_reg(XrCompilerContext *ctx, XrCompiler *compiler, XrExprDesc *e,
                           int target_reg) {
    ...
    if (e->t >= 0 || e->f >= 0) {
        ...
        if (e->f >= 0)
            patch_jump_list(em, e->f, false_pc);
        if (e->t >= 0)
            patch_jump_list(em, e->t, true_pc);
...
void xexpr_debug_print(const XrExprDesc *e) {
    printf("[XrExprDesc] kind=%s reg=%d has_jumps=%d is_const=%d", xexpr_kind_name(e->kind), e->reg,
           e->has_jumps, e->is_const);
...
int xexpr_ensure_boxed(XrCompilerContext *ctx, XrCompiler *compiler, XrExprDesc *e, int reg) {
    if (xexpr_is_raw_i64(e)) {
