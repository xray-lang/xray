...
bool xr_debug_eval_condition_truthy(XrayIsolate *isolate, const char *condition) {
    ...
    bool is_truthy =
        result && (strcmp(result, "true") == 0 || strcmp(result, "1") == 0 ||
                   (result[0] != '\0' && strcmp(result, "false") != 0 && strcmp(result, "0") != 0 &&
                    strcmp(result, "null") != 0 && strncmp(result, "<undefined:", 11) != 0 &&
...
static XrDebugAction hook_on_exception(XrayIsolate *isolate, const char *message,
    ...
    if ((is_uncaught && dbg->break_on_uncaught) || (!is_uncaught && dbg->break_on_caught)) {
...
static XrDebugHooks g_debug_hooks = {
    .on_line = hook_on_line, .on_exception = hook_on_exception, .is_enabled = hook_is_enabled};
    ...
    if (XR_IS_INT(val)) {
        ...
        switch (hdr->type) {
            case XR_TFUNCTION: {
                ...
                const char *name =
                    cl->proto && cl->proto->name ? XR_STRING_CHARS(cl->proto->name) : "anonymous";
...
XdapResumeResult xr_debug_resume_execution(XrayIsolate *isolate) {
    ...
    switch (result) {
        case 0:
            return XDAP_RESUME_STOPPED;
        case 1:
            return XDAP_RESUME_TERMINATED;
        default:
            return XDAP_RESUME_ERROR;
    }
}
