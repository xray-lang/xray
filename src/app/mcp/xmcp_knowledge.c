...
"    speak(): string { return \"${this.name} says ...\" }\n"
"}\n"
"\n"
...
"    map(f: fn(T): T): Box<T> { return new Box(f(this.value)) }\n"
"}\n"
"\n"
...
static const char TOPIC_COLLECTIONS[] =
"## Collections\n"
"\n"
"### Array\n"
"```xray\n"
...
"arr.filter((x: int): bool => x > 2)\n"
"arr.reduce((acc: int, x: int): int => acc + x, 0)\n"
"arr.find((x: int): bool => x > 3)\n"
...
static const char TOPIC_STRING[] =
"## Strings\n"
"\n"
...
"```\n"
"`let ch = Channel(10)` is a **compile error**.\n"
"\n"
"### Operations\n"
"```xray\n"
...
"```xray\n"
"fn producer(ch: Channel<int>): void {  // use Channel<T> in type position\n"
"    ch.send(42)\n"
"}\n"
"```\n"
"\n"
...
static const char TOPIC_COROUTINE[] =
...
"| Channel | `const ch = Channel(n)` | Deep-copy on send |\n"
...
"\n"
"let ch = Channel(1)        // ERROR: Channel must be const\n"
"\n"
...
static const char TOPIC_MODULES[] =
"## Modules & Imports\n"
"\n"
"```xray\n"
"import http\n"
...
static const char TOPIC_TESTING[] =
"## Testing\n"
"\n"
"```xray\n"
"@test\n"
...
"\n"
"@test(skip)\n"
"fn test_todo(): void { ... }  // skipped\n"
"\n"
"@test(timeout: 5000)\n"
"fn test_slow(): void { ... }  // 5s timeout\n"
"\n"
"// Assert that a function throws\n"
"assert_throws((): void => { throw \"error\" })\n"
"```\n"
"\n"
...
static const char TOPIC_BUILTIN[] =
"## Built-in Functions\n"
"\n"
...
static const TopicDef BUILTIN_TOPICS[] = {
    ...
    {"control_flow",      "if,else,while,for,loop,match,switch,try,catch,throw,error_handling", TOPIC_CONTROL_FLOW},
    ...
    {"coroutine",         "go,goroutine,goroutines,await,task,async,concurrency,scope,select,defer", TOPIC_COROUTINE},
...
"## Testing\n"
"```xray\n"
"@test fn test_add(): void { assert_eq(1+1, 2) }\n"
"```\n"
"\n"
"## Important notes\n"
"- `print()` outputs with newline (no `println`)\n"
"- Arrow function params MUST have type annotations\n"
"- Channel(n) at construction, Channel<T> in type position\n"
...
void xmcp_knowledge_load(XmcpKnowledge *kb) {
    ...
    for (int i = 0; BUILTIN_TOPICS[i].name != NULL; i++) {
        XR_DCHECK(kb->topic_count < XMCP_MAX_TOPICS,
                  "xmcp_knowledge_load: too many topics");
        if (kb->topic_count >= XMCP_MAX_TOPICS) break;
    ...
    for (int i = 0; BUILTIN_MODULES[i].name != NULL; i++) {
        XR_DCHECK(kb->module_count < XMCP_MAX_MODULES,
                  "xmcp_knowledge_load: too many modules");
        if (kb->module_count >= XMCP_MAX_MODULES) break;
...
char *xmcp_knowledge_search_stdlib(XmcpKnowledge *kb, const char *query,
    ...
    for (int i = 0; i < kb->module_count; i++) {
        ...
        if (module_filter && module_filter[0]) {
            if (strcasecmp(m->name, module_filter) != 0) continue;
        }
    ...
    if (found == 0) {
        ...
        for (int i = 0; i < kb->module_count; i++) {
            if (i > 0) len += (size_t)snprintf(result + len, cap - len, ", ");
...
const char *xmcp_knowledge_get_cheatsheet(void) { return CHEATSHEET; }
const char *xmcp_knowledge_get_concurrency(void) { return CONCURRENCY_MODEL; }
const char *xmcp_knowledge_get_stdlib_list(void) { return STDLIB_LIST; }
