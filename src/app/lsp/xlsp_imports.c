...
XlspExportedSymbol *xlsp_extract_exports(XrLspServer *server, const char *file_path) {
    ...
    if (cached) {
        ...
        for (XlspExportedSymbol *s = cached->symbols; s; s = s->next) {
            ...
            copy->kind = s->kind;
            if (s->signature) copy->signature = xr_strdup(s->signature);
            if (s->documentation) copy->documentation = xr_strdup(s->documentation);
            copy->line = s->line;
                    ...
                    if (paren.type == TK_LPAREN) {
                        ...
                        while (depth > 0) {
                            ...
                            if (depth > 0 && t.type == TK_NAME) {
                                ...
                                first_param = false;
                                int param_len = t.length < (int)(sizeof(sig_buf) - sig_len - 5) ? t.length : (int)(sizeof(sig_buf) - sig_len - 5);
    ...
    for (XlspExportedSymbol *s = head; s; s = s->next) {
        ...
        if (s->documentation) copy->documentation = xr_strdup(s->documentation);
        copy->line = s->line;
        if (!cache_head) cache_head = cache_tail = copy;
