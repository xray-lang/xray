...
static const XrCliOptionSpec empty_options[] = {
    XR_CLI_OPT_END
};
...
static XrCliCommandSpec cli_commands[] = {
    ...
    /* Artifact commands */
    {"compile", "Compile to bytecode or C",   NULL, compile_options, 1, 1,  false, false, NULL, NULL, 0},
...
const char *xr_cli_opt_string(const XrCliOptionMap *map, const char *name,
    ...
    int idx = find_option_index(map, name);
    if (idx < 0 || !map->present[idx]) return default_val;
