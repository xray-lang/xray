...
static void body_add_match_object_pattern_accesses_for_shape(XgBodyCollect *bc,
                                                             const AstNode *pattern,
                                                             XgObjectShapeId shape_id) {
    if (!bc || !pattern || shape_id == XG_NO_ID)
        return;
    if (pattern->type == AST_PATTERN_MULTI) {
        for (int i = 0; i < pattern->as.pattern_multi.count; i++)
            body_add_match_object_pattern_accesses_for_shape(
                bc, pattern->as.pattern_multi.patterns[i], shape_id);
        return;
    }
    if (pattern->type != AST_PATTERN_OBJECT)
        return;

    const PatternObjectNode *object_pattern = &pattern->as.pattern_object;
    for (int i = 0; i < object_pattern->count; i++) {
        const char *name = object_pattern->field_names ? object_pattern->field_names[i] : NULL;
        const AstNode *subpattern = object_pattern->patterns ? object_pattern->patterns[i] : NULL;
        int field_index = body_object_shape_static_field_index(bc, shape_id, NULL, name);
        if (!name || field_index < 0 || field_index > UINT16_MAX)
            continue;
        (void) body_add_resolved_object_field_access(bc, pattern, false, shape_id, name,
                                                     (uint16_t) field_index);

        const XgObjectFieldSummary *field =
            body_find_object_shape_field(bc, shape_id, (uint16_t) field_index);
        XgObjectShapeId nested_shape_id =
            field ? body_lookup_static_object_shape_for_type_key(bc, field->type_key) : XG_NO_ID;
        body_add_match_object_pattern_accesses_for_shape(bc, subpattern, nested_shape_id);
    }
}
...
static void body_add_match_object_pattern_accesses(XgBodyCollect *bc, const AstNode *pattern,
    ...
    XgObjectShapeId shape_id;
    if (!bc || !pattern || !subject)
        return;
    shape_id = body_lookup_object_shape(bc, subject, NULL);
    if (shape_id != XG_NO_ID)
        body_add_match_object_pattern_accesses_for_shape(bc, pattern, shape_id);
}
