...
static uint64_t hash_json_dynamic_access_summary(uint64_t hash, const XgJsonDynamicAccessSummary *row) {
    if (!row)
        return hash_u32(hash, 0);
...
static uint64_t hash_object_shape_flow_summary(uint64_t hash,
                                               const XgObjectShapeFlowSummary *row) {
    if (!row)
        return hash_u32(hash, 0);
...
XR_FUNC bool xg_global_evidence_reserve_json_dynamic_accesses(XgGlobalEvidence *evidence,
                                                      uint32_t capacity) {
    return evidence && reserve_array((void **) &evidence->json_dynamic_accesses, &evidence->json_dynamic_access_cap,
                                     capacity, sizeof(XgJsonDynamicAccessSummary));
}
...
XR_FUNC bool xg_global_evidence_reserve_object_access_cases(XgGlobalEvidence *evidence,
                                                            uint32_t capacity) {
    return evidence &&
           reserve_array((void **) &evidence->object_access_cases,
...
XR_FUNC bool xg_global_evidence_reserve_object_shape_flows(XgGlobalEvidence *evidence,
                                                           uint32_t capacity) {
    return evidence &&
           reserve_array((void **) &evidence->object_shape_flows,
                         &evidence->object_shape_flow_cap, capacity,
                         sizeof(XgObjectShapeFlowSummary));
}
...
xg_global_evidence_add_json_dynamic_access(XgGlobalEvidence *evidence, const XgJsonDynamicAccessSummary *summary) {
    XgJsonDynamicAccessSummary *row;
    if (!evidence || !summary ||
        !xg_global_evidence_reserve_json_dynamic_accesses(evidence, evidence->njson_dynamic_accesses + 1))
        return NULL;
...
xg_global_evidence_add_object_access_case(XgGlobalEvidence *evidence,
    ...
    if (!evidence || !summary || !xg_global_evidence_reserve_object_access_cases(
                                   evidence, evidence->nobject_access_cases + 1))
        return NULL;
...
xg_global_evidence_add_object_shape_flow(XgGlobalEvidence *evidence,
    ...
    if (!evidence || !summary || !xg_global_evidence_reserve_object_shape_flows(
                                   evidence, evidence->nobject_shape_flows + 1))
        return NULL;
...
XR_FUNC uint64_t xg_global_evidence_hash(const XgGlobalEvidence *evidence) {
    ...
    hash = hash_mix(hash, &evidence->njson_dynamic_accesses, sizeof(evidence->njson_dynamic_accesses));
    ...
    hash = hash_mix(hash, &evidence->nobject_access_cases,
                    sizeof(evidence->nobject_access_cases));
    hash = hash_mix(hash, &evidence->nobject_shape_flows,
                    sizeof(evidence->nobject_shape_flows));
...
static void dump_cache_payload_global_extra(FILE *out, const XgGlobalEvidence *evidence) {
    fprintf(out,
            ...
            "object_accesses=%u object_access_cases=%u object_shape_flows=%u object_merges=%u options=%u map_shapes=%u map_entries=%u "
            ...
            evidence ? evidence->njson_dynamic_accesses : 0, evidence ? evidence->njson_codecs : 0,
            evidence ? evidence->nobject_shapes : 0, evidence ? evidence->nobject_fields : 0,
            evidence ? evidence->nobject_accesses : 0,
            evidence ? evidence->nobject_access_cases : 0,
            evidence ? evidence->nobject_shape_flows : 0,
            evidence ? evidence->nobject_merges : 0,
    ...
    for (uint32_t i = 0; i < evidence->njson_codecs; i++) {
        ...
        fprintf(
            ...
            c->input_shape_id, c->output_shape_id, (unsigned) c->field_count,
            c->flags);
    }
    for (uint32_t i = 0; i < evidence->nobject_shapes; i++) {
        ...
        fprintf(out,
                ...
                s->object_shape_id, s->module_id, s->owner_func_id, s->type_key,
                s->stable_type_key, (unsigned) s->shape_kind, (unsigned) s->domain,
    ...
    for (uint32_t i = 0; i < evidence->nobject_fields; i++) {
        ...
        fprintf(out,
                ...
                f->stable_name_key, f->type_key, f->stable_type_key, f->default_value_id,
                f->flags);
    }
    for (uint32_t i = 0; i < evidence->nobject_accesses; i++) {
        ...
        fprintf(out,
                ...
                "shape_count=%u receiver_param=%u shape_set=%u constraint_shape=%u epoch=%u span=%u field_name=%u result_type=%u "
                "field=%u flags=0x%x\n",
    ...
    for (uint32_t i = 0; i < evidence->nobject_access_cases; i++) {
        ...
        fprintf(out,
                ...
                c->case_id, c->object_access_id, c->receiver_shape_set_id,
                c->receiver_shape_id, c->stable_shape_key, c->mutation_epoch,
                (unsigned) c->field_ordinal, (unsigned) c->domain);
    }
    for (uint32_t i = 0; i < evidence->nobject_shape_flows; i++) {
...
static bool materialize_payload_global_extra(const char **cursor, XgGlobalEvidence *evidence) {
    ...
    if (sscanf(line,
               ...
               " encoding=%" SCNu32
               " json_dynamic_accesses=%" SCNu32 " json_codecs=%" SCNu32 " object_shapes=%" SCNu32
               " object_fields=%" SCNu32 " object_accesses=%" SCNu32
               " object_access_cases=%" SCNu32 " object_shape_flows=%" SCNu32
               " object_merges=%" SCNu32
               ...
               &json_dynamic_access_count, &json_codec_count,
               &object_shape_count, &object_field_count, &object_access_count,
               &object_access_case_count, &object_shape_flow_count, &object_merge_count,
    ...
    for (uint32_t i = 0; i < json_dynamic_access_count; i++) {
        ...
        memset(&row, 0, sizeof(row));
        if (sscanf(line,
                   "json-dynamic-access id=%" SCNu32 " module=%" SCNu32 " func=%" SCNu32 " shape=%" SCNu32
                   " kind=%" SCNu32 " span=%" SCNu32 " key=%" SCNu32 " result_type=%" SCNu32
                   " field=%" SCNu32 " flags=0x%" SCNx32 " %c",
                   &row.json_dynamic_access_id, &row.module_id, &row.owner_func_id, &row.receiver_shape_id,
    ...
    for (uint32_t i = 0; i < json_codec_count; i++) {
        ...
        if (sscanf(line,
                   ...
                   " input_shape=%" SCNu32 " output_shape=%" SCNu32
                   ...
                   &row.input_shape_id, &row.output_shape_id, &field_count,
                   &row.flags, &trailing) != 12)
            return false;
    ...
    for (uint32_t i = 0; i < object_shape_count; i++) {
        ...
        memset(&row, 0, sizeof(row));
        if (sscanf(line,
                   "object-shape id=%" SCNu32 " module=%" SCNu32
                   " func=%" SCNu32 " type=%" SCNu32 " stable_type=%" SCNx64
                   " kind=%" SCNu32 " domain=%" SCNu32 " provenance=%" SCNu32
                   " exact=%" SCNu32 " fresh=%" SCNu32 " escaped=%" SCNu32
                   " epoch=%" SCNu32 " span=%" SCNu32 " fields=%" SCNu32 "+%" SCNu32
                   " flags=0x%" SCNx32 " stable_shape=%" SCNx64 " hash=%" SCNx64 " %c",
                   &row.object_shape_id, &row.module_id, &row.owner_func_id,
                   &row.type_key, &row.stable_type_key, &shape_kind, &domain, &provenance,
                   &concrete_exact, &fresh, &escaped, &row.mutation_epoch, &row.source_span_id,
                   &row.field_name_start, &field_count, &row.flags, &row.stable_shape_key,
                   &row.shape_hash, &trailing) != 18)
            return false;
    ...
    for (uint32_t i = 0; i < object_field_count; i++) {
        ...
        if (sscanf(line,
                   ...
                   " default=%" SCNu32
                   " flags=0x%" SCNx32 " %c",
                   &row.field_id, &row.shape_id, &field_ordinal, &row.name_id,
                   &row.stable_name_key, &row.type_key, &row.stable_type_key,
                   &row.default_value_id, &row.flags, &trailing) != 9)
            return false;
        row.field_ordinal = (uint16_t) field_ordinal;
    ...
    for (uint32_t i = 0; i < object_access_count; i++) {
        ...
        memset(&row, 0, sizeof(row));
        if (sscanf(line,
                   "object-access id=%" SCNu32 " module=%" SCNu32 " func=%" SCNu32
                   " shape=%" SCNu32 " kind=%" SCNu32 " domain=%" SCNu32
                   " syntax=%" SCNu32 " shape_count=%" SCNu32 " receiver_param=%" SCNu32
                   " shape_set=%" SCNu32 " constraint_shape=%" SCNu32
                   " epoch=%" SCNu32 " span=%" SCNu32 " field_name=%" SCNu32
                   ...
                   &row.mutation_epoch, &row.source_span_id,
                   &row.field_name_id, &row.result_type_key, &field_ordinal, &row.flags,
                   &trailing) != 17)
            return false;
        row.access_kind = (uint8_t) access_kind;
        row.domain = (uint8_t) domain;
    ...
    for (uint32_t i = 0; i < object_access_case_count; i++) {
        ...
        memset(&row, 0, sizeof(row));
        if (sscanf(line,
                   "object-access-case id=%" SCNu32 " access=%" SCNu32
                   " shape_set=%" SCNu32 " shape=%" SCNu32 " stable_shape=%" SCNx64
                   " epoch=%" SCNu32 " field=%" SCNu32 " domain=%" SCNu32 " %c",
    ...
    for (uint32_t i = 0; i < object_shape_flow_count; i++) {
        ...
        memset(&row, 0, sizeof(row));
        if (sscanf(line,
                   "object-shape-flow id=%" SCNu32 " callsite=%" SCNu32
                   " source_func=%" SCNu32 " target_func=%" SCNu32
                   " concrete_shape=%" SCNu32 " source_param=%" SCNu32
                   " target_param=%" SCNu32 " flags=0x%" SCNx32 " %c",
                   &row.flow_id, &row.callsite_id, &row.source_func_id, &row.target_func_id,
                   &row.concrete_shape_id, &source_param_ordinal, &target_param_ordinal,
                   &row.flags, &trailing) != 8)
            return false;
...
static void collect_import_offsets(const XgGlobalEvidence *target,
    ...
    for (uint32_t i = 0; i < target->njson_dynamic_accesses; i++) {
        offsets->json_dynamic_access_id =
            max_u32(offsets->json_dynamic_access_id, target->json_dynamic_accesses[i].json_dynamic_access_id);
        offsets->func_id = max_u32(offsets->func_id, target->json_dynamic_accesses[i].owner_func_id);
    ...
    for (uint32_t i = 0; i < target->nobject_access_cases; i++) {
        ...
        offsets->object_access_case_id =
            max_u32(offsets->object_access_case_id, row->case_id);
...
static uint32_t package_non_module_row_count(const XgGlobalEvidence *package) {
    return package->ndecls + package->nclasses + package->nclass_fields + package->nmethods +
           ...
           package->nderived_methods + package->njson_dynamic_accesses +
           package->njson_codecs + package->nobject_shapes +
           package->nobject_fields + package->nobject_accesses + package->nobject_access_cases +
           package->nobject_shape_flows +
           package->nobject_merges +
...
XR_FUNC char *xg_global_evidence_dump(const XgGlobalEvidence *evidence) {
    ...
    fprintf(out,
            ...
            evidence->nderived_fields, evidence->nderived_methods,
            evidence->njson_dynamic_accesses, evidence->njson_codecs,
