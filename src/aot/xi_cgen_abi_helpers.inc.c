...
/* How one parameter is spelled in C, taken from the same signature row the
 * declaration and every call are emitted from. A slot the plan has no row for
 * is spelled as the carrier, which is what an unstated boundary means. */
static const char *cg_func_param_abi_c_type(XiCgenCtx *ctx, const XiFunc *f, uint16_t param_idx) {
    XrCFunctionAbiEmissionView view;
    return cg_func_param_abi_emission(ctx, f, param_idx, &view) && view.c_type ? view.c_type
                                                                               : "XrValue";
}
...
static void emit_value_as_direct_call_arg(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    XrCCallArgumentEmissionView direct_tagged_ref_argument = {0};
    if (cg_direct_local_tagged_ref_argument_emission(ctx, f, call, arg_index, arg,
                                                     &direct_tagged_ref_argument)) {
        ...
        if (xaot_value_storage_rep(slot_rep) != XR_REP_RAWPTR || !slot_c_type ||
            strcmp(slot_c_type, direct_tagged_ref_argument.c_type) != 0) {
            fprintf(stderr,
        ...
        fprintf(out, "(%s)(", direct_tagged_ref_argument.c_type);
        emit_vref(out, arg);
        fprintf(out, ")");
        return;
    }
