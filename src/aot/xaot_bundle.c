...
static bool xaot_bundle_add_json_dynamic_access_plan(XaotBundle *bundle, const XgGlobalEvidence *evidence,
                                             const XgJsonDynamicAccessSummary *access) {
    XaotJsonDynamicAccessPlan *plan;
    if (!bundle || !evidence || !access)
        return false;
    if (!reserve_plan_array((void **) &bundle->json_dynamic_access_plans, &bundle->json_dynamic_access_plan_cap,
...
static bool xaot_bundle_add_json_dynamic_access_plans(XaotBundle *bundle,
    ...
    for (uint32_t i = 0; i < evidence->njson_dynamic_accesses; i++) {
        if (!xaot_bundle_add_json_dynamic_access_plan(bundle, evidence, &evidence->json_dynamic_accesses[i]))
            return false;
    }
    return true;
}
...
static bool xaot_bundle_add_object_access_plan(XaotBundle *bundle, const XgGlobalEvidence *evidence,
    ...
    for (uint32_t i = 0; i < evidence->nobject_access_cases; i++) {
        ...
        if (source->object_access_id != access->object_access_id)
            continue;
        if (!reserve_plan_array((void **) &bundle->object_access_case_plans,
                                &bundle->object_access_case_plan_cap,
                                bundle->nobject_access_case_plans + 1,
                                sizeof(XaotObjectAccessCasePlan), 8))
            return false;
...
XR_FUNC char *xaot_bundle_dump_plan(const XaotBundle *bundle) {
    ...
    for (uint32_t ji = 0; ji < bundle->njson_dynamic_access_plans; ji++) {
        ...
        fprintf(out,
                ...
                ji, jp->json_dynamic_access_id, jp->module_id, jp->owner_func_id, jp->receiver_shape_id,
                xg_json_dynamic_access_kind_name(jp->access_kind), json_dynamic_access_action_name(jp->action),
