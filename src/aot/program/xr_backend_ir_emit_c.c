...
static const char *representation_c_type(uint8_t representation) {
    switch (representation) {
        case XR_BACKEND_VALUE_BOOL_U8:
            return "uint8_t";
        case XR_BACKEND_VALUE_I64:
            return "int64_t";
        case XR_BACKEND_VALUE_U32:
        case XR_BACKEND_VALUE_ERROR_U32:
            return "uint32_t";
        case XR_BACKEND_VALUE_VOID:
            return "void";
    }
    return NULL;
}
...
static bool emit_function_signature(CBuffer *buffer, const XrBackendFunction *function,
                                    uint32_t function_id, bool prototype) {
    ...
    if (function->parameter_count == 0u) {
        ...
        for (uint32_t parameter = 0; parameter < function->parameter_count; ++parameter) {
            ...
            if (!xr_backend_representation_for_type(function->parameter_types[parameter],
                                                    &representation))
                return false;
            if (!append_format(buffer, "%s%s p%u", parameter ? ", " : "",
                               representation_c_type(representation), parameter))
                return false;
        }
    }
...
static bool emit_parallel_edge(CBuffer *buffer, const XrBackendFunction *function,
    ...
    for (uint32_t argument = 0; argument < target->argument_count; ++argument) {
        ...
        const char *type = representation_c_type(function->value_representations[target_value]);
        if (!append_format(buffer, "        %s edge_%u = v%u;\n", type, argument, source_value))
            return false;
    }
    for (uint32_t argument = 0; argument < target->argument_count; ++argument) {
...
static bool emit_function(CBuffer *buffer, const XrBackendIR *ir, uint32_t function_id) {
    ...
    if (!emit_function_signature(buffer, function, function_id, false))
        return false;
    for (uint32_t parameter = 0; parameter < function->parameter_count; ++parameter) {
    ...
    for (uint32_t value = 0; value < function->value_count; ++value) {
        const char *type = representation_c_type(function->value_representations[value]);
        if (!type || !append_format(buffer, "    %s v%u = 0;\n", type, value) ||
...
XrBackendStatus xr_backend_ir_emit_c(const XrBackendIR *ir, bool standalone_main,
    ...
    for (uint32_t function = 0; emitted && function < ir->function_count; ++function)
        emitted = emit_function_signature(&buffer, &ir->functions[function], function, true);
