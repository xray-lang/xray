...
#include "../../plan/semantic/xr_semantic_closure_storage_shape.h"
#include "../../plan/semantic/xr_semantic_source_dependency_call_shape.h"
#include "../../plan/semantic/xr_semantic_json_codec_shape.h"
...
#include "../../plan/semantic/xr_semantic_owner_transfer_shape.h"
#include "../../plan/semantic/xr_semantic_source_class_field_shape.h"
#include "../../plan/semantic/xr_semantic_cleanup_shape.h"
#include "../../plan/semantic/xr_semantic_const_variant_shape.h"
...
#include "../../plan/semantic/xr_semantic_string_utf8_shape.h"
...
typedef struct CollectContext {
    ...
    uint32_t target_layout_begin;
    uint32_t target_layout_count;
    uint32_t target_call_begin;
    uint32_t target_call_count;
...
/* Collector-side scope resolution.  Do not interpret a module-local semantic
 * ordinal until the verified module fingerprint/provenance has selected one
 * unique TargetPlan partition. */
static bool collect_target_scope_init(CollectContext *ctx) {
    if (!ctx || !ctx->semantic ||
        !xr_target_plan_partition_for_semantic(ctx->target_plan, ctx->semantic,
                                               &ctx->target_partition))
        return false;
    ctx->semantic_fingerprint = xr_semantic_plan_fingerprint(ctx->semantic);
    const XrSemanticProgramProvenance *provenance =
        xr_semantic_plan_program_provenance(ctx->semantic);
    ctx->module_identity = (XrStableId) {{0}};
    ctx->program_module_row = 0u;
    if (provenance) {
        ctx->module_identity = provenance->program_module;
        ctx->program_module_row = provenance->program_module_row;
    } else {
        const XrSemanticEntityRecord *entity = xr_semantic_plan_unique_module_entity(ctx->semantic);
        if (!entity)
            return false;
        ctx->module_identity = entity->id;
        ctx->program_module_row = ctx->target_partition;
    }
    uint32_t partition_count = 0u;
    const XrTargetModulePartitionRecord *partitions =
        xr_target_plan_module_partitions(ctx->target_plan, &partition_count);
    uint32_t layout_count = 0u, call_count = 0u;
    (void) xr_target_plan_layouts(ctx->target_plan, &layout_count);
    (void) xr_target_plan_calls(ctx->target_plan, &call_count);
    if (!partition_count) {
        ctx->target_layout_begin = 0u;
        ctx->target_layout_count = layout_count;
        ctx->target_call_begin = 0u;
        ctx->target_call_count = call_count;
        return true;
    }
    if (!partitions || ctx->target_partition >= partition_count)
        return false;
    ...
    if (partition->layouts_begin > layout_count ||
        ...
        partition->calls_begin > call_count ||
        partition->calls_count > call_count - partition->calls_begin)
        return false;
    ctx->target_layout_begin = partition->layouts_begin;
    ctx->target_layout_count = partition->layouts_count;
    ctx->target_call_begin = partition->calls_begin;
    ctx->target_call_count = partition->calls_count;
    return true;
}

static const XrTargetValueRepRecord *collect_target_value_rep(const CollectContext *ctx,
                                                              uint32_t semantic_value) {
    return ctx ? xr_target_plan_value_rep_for_module(ctx->target_plan, ctx->target_partition,
                                                     semantic_value)
               : NULL;
}

static bool collect_indices_init(CollectContext *ctx) {
    if (!collect_target_scope_init(ctx))
        return false;
    ...
    uint32_t layout_end = ctx->target_layout_begin + ctx->target_layout_count;
    for (uint32_t i = ctx->target_layout_begin; i < layout_end; i++) {
    ...
    uint32_t call_end = ctx->target_call_begin + ctx->target_call_count;
    for (uint32_t i = ctx->target_call_begin; i < call_end; i++) {
...
typedef struct VerifyAuthority {
    ...
    XrTargetProgramReachability program_reachability;
    bool program_reachability_active;
    uint64_t bytes;
    uint64_t work;
} VerifyAuthority;
...
static bool aot_program_reachability_init(VerifyAuthority *ctx) {
    ...
    if (!partitions || !module_count || ctx->program_module_row >= module_count)
        return false;
    const XrSemanticPlan **modules =
        (const XrSemanticPlan **) xr_calloc(module_count, sizeof(*modules));
    if (!modules)
        return false;
    bool exact = true;
    for (uint32_t row = 0u; exact && row < module_count; row++) {
        modules[row] = xr_target_plan_program_module(ctx->target_plan, row);
        exact = modules[row] != NULL;
    }
    exact = exact && modules[ctx->program_module_row] == ctx->semantic &&
            xr_target_program_reachability_build(modules, module_count, &ctx->program_reachability,
                                                 NULL, 0u);
    xr_free(modules);
    if (!exact) {
        xr_target_program_reachability_dispose(&ctx->program_reachability);
        return false;
    }
    ctx->program_reachability_active = true;
    return true;
}

static bool aot_program_function_is_reachable(const VerifyAuthority *ctx, uint32_t function) {
    return ctx && (!ctx->program_reachability_active ||
                   xr_target_program_function_is_reachable(&ctx->program_reachability,
                                                           ctx->program_module_row, function));
}

/* Verifier-side reconstruction deliberately does not call the collector's
...
static bool verify_target_scope_init(VerifyAuthority *ctx) {
    ...
    const XrSemanticProgramProvenance *provenance =
        xr_semantic_plan_program_provenance(ctx->semantic);
    ctx->module_identity = zero;
    ctx->program_module_row = 0u;
    if (provenance) {
        if (!provenance->module_count ||
            provenance->program_module_row >= provenance->module_count ||
            xr_stable_id_equal(provenance->program_module, zero))
            return false;
        ctx->module_identity = provenance->program_module;
        ctx->program_module_row = provenance->program_module_row;
    } else {
    ...
    (void) xr_target_plan_calls(ctx->target_plan, &call_count);
    if (!partition_count) {
        if (partitions || xr_target_plan_program_module_count(ctx->target_plan) != 1u ||
            !xr_fingerprint_equal(xr_target_plan_semantic_fingerprint(ctx->target_plan),
                                  ctx->semantic_fingerprint) ||
            (provenance &&
             (provenance->module_count != 1u || provenance->program_module_row != 0u)))
            return false;
        ctx->target_partition = 0u;
        ctx->target_layout_count = layout_count;
        ctx->target_call_count = call_count;
        return true;
    }
    if (!partitions || (provenance && xr_target_plan_program_module_count(ctx->target_plan) !=
                                          provenance->module_count))
        return false;
    uint32_t matches = 0u;
    for (uint32_t i = 0u; i < partition_count; i++) {
        const XrTargetModulePartitionRecord *partition = &partitions[i];
        uint32_t expected_row = provenance ? ctx->program_module_row : i;
        if (partition->program_module_row != expected_row ||
            !xr_stable_id_equal(partition->module_identity, ctx->module_identity) ||
            !xr_fingerprint_equal(partition->semantic_fingerprint, ctx->semantic_fingerprint) ||
            xr_target_plan_semantic_module(ctx->target_plan, i) != ctx->semantic)
            continue;
        if (partition->layouts_begin > layout_count ||
            partition->layouts_count > layout_count - partition->layouts_begin ||
            partition->calls_begin > call_count ||
            partition->calls_count > call_count - partition->calls_begin)
            return false;
        ctx->target_partition = i;
        ctx->program_module_row = expected_row;
        ctx->target_layout_begin = partition->layouts_begin;
        ctx->target_layout_count = partition->layouts_count;
        ctx->target_call_begin = partition->calls_begin;
        ctx->target_call_count = partition->calls_count;
...
static bool verify_target_function_index(const VerifyAuthority *ctx, uint32_t semantic_function,
                                         uint32_t *out_target_function) {
    if (out_target_function)
        *out_target_function = XR_SEMANTIC_INDEX_NONE;
    uint32_t partition_count = 0u;
    const XrTargetModulePartitionRecord *partitions =
        ctx ? xr_target_plan_module_partitions(ctx->target_plan, &partition_count) : NULL;
    if (!ctx || !out_target_function)
        return false;
    if (!partition_count) {
        *out_target_function = semantic_function;
        return semantic_function < ctx->function_count;
    }
    if (!partitions || ctx->target_partition >= partition_count ||
        semantic_function >= partitions[ctx->target_partition].functions_count)
        return false;
    *out_target_function = partitions[ctx->target_partition].functions_begin + semantic_function;
    return true;
}

static bool verify_target_function_is(const VerifyAuthority *ctx, uint32_t semantic_function,
                                      uint32_t target_function) {
    uint32_t expected = XR_SEMANTIC_INDEX_NONE;
    return verify_target_function_index(ctx, semantic_function, &expected) &&
           target_function == expected;
}

static const XrTargetLayoutRecord *verify_target_layout(const VerifyAuthority *ctx,
                                                        uint32_t semantic_type) {
    uint32_t count = 0;
    const XrTargetLayoutRecord *layouts =
        ctx ? xr_target_plan_layouts(ctx->target_plan, &count) : NULL;
    uint32_t index = ctx && semantic_type < ctx->type_count ? ctx->layout_by_type[semantic_type]
                                                            : XR_SEMANTIC_INDEX_NONE;
    return layouts && index < count ? &layouts[index] : NULL;
}

static const XrTargetCallRecord *verify_target_call(const VerifyAuthority *ctx,
                                                    uint32_t semantic_operation) {
    uint32_t count = 0;
    const XrTargetCallRecord *calls = ctx ? xr_target_plan_calls(ctx->target_plan, &count) : NULL;
    uint32_t index = ctx && semantic_operation < ctx->operation_count
                         ? ctx->call_by_operation[semantic_operation]
                         : XR_SEMANTIC_INDEX_NONE;
    return calls && index < count ? &calls[index] : NULL;
}
 ...
 * owns that answer. */
static AotI64OverflowPredicateMatch
aot_i64_overflow_predicate_binding(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (!call || !predicates || !slots || !operands ||
        !xr_target_plan_find_function(ctx->target_plan, ctx->semantic, operation->function,
                                      &target_function) ||
    ...
    if (!match || match->id >= predicate_count || match->id != call->program_row ||
        ...
        !xr_stable_id_equal(match->callsite, call->callsite) || match->result_slot >= slot_count ||
        match->receiver_slot >= slot_count || match->argument_slot >= slot_count ||
    ...
    if (result->id != match->result_slot || receiver->id != match->receiver_slot ||
        ...
        !argument_register || !argument_memory || result_register->kind != XR_MACHINE_REP_I1 ||
        result_memory->kind != XR_MACHINE_REP_I1 || receiver_register->kind != XR_MACHINE_REP_I64 ||
...
static void rep_survey_row(const CollectContext *ctx, const char *side, uint32_t source_value,
    ...
    const char *selector = use && use->metadata_count == 1 && use->metadata_begin < metadata_count
                               ...
                               : "";
    const XrTargetValueRepRecord *binding = collect_target_value_rep(ctx, source_value);
...
static void rep_trace_origin(const VerifyAuthority *ctx, uint32_t value) {
    ...
    if (parameter_index != XR_SEMANTIC_INDEX_NONE) {
        ...
        if (type)
            fprintf(stderr,
                    "[aot-refine]     parameter type  = kind %u, flags %u, class %u, key %.240s\n",
                    (unsigned) type->kind, (unsigned) type->flags, type->source_class,
                    type->canonical_key ? type->canonical_key : "<missing>");
        const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, value);
        if (parameter)
            fprintf(stderr,
                    "[aot-refine]     parameter ABI   = mode %u, ownership %u, flags %u, "
                    "slot %u\n",
                    (unsigned) parameter->mode, (unsigned) parameter->ownership,
                    (unsigned) parameter->flags, binding ? binding->slot : UINT32_MAX);
        return;
    }
    ...
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, operation->result_type);
    if (type && type->canonical_key)
        fprintf(stderr, "[aot-refine]     type key        = %s\n", type->canonical_key);
    if (operation->opcode == XI_CLOSURE_NEW || operation->opcode == XI_STACK_ALLOC ||
        operation->opcode == XI_LOAD_UPVAL) {
        uint32_t function_index = operation->opcode == XI_LOAD_UPVAL
            ? operation->function : operation->callable_function;
        const XrSemanticFunctionRecord *function =
            xr_semantic_plan_function(ctx->semantic, function_index);
        if (function) {
            fprintf(stderr, "[aot-refine]     callable body   = %u parent=%u captures=%u immediate=%lld\n",
                    function_index, function->parent, function->capture_count,
                    (long long) operation->semantic_immediate);
            for (uint32_t i = 0; i < function->capture_count; ++i) {
                const XrSemanticCaptureRecord *capture =
                    xr_semantic_plan_capture(ctx->semantic, function->capture_begin + i);
                if (capture)
                    fprintf(stderr, "[aot-refine]     capture %u       = source=%u kind=%u flags=%u "
                            "function=%u value=%u type=%u source-type=%u domain=%u capability=%u\n",
                            i, capture->source, capture->kind, capture->flags,
                            capture->source_function, capture->source_value, capture->type,
                            capture->source_type, capture->storage_domain, capture->value_capability);
            }
        }
    }
...
static void rep_materialization_survey_row(const VerifyAuthority *ctx, uint32_t source_value,
    ...
    if (!rep_survey_enabled())
        return;
    uint32_t definer = ctx && ctx->operation_by_value && source_value < ctx->value_count
                           ? ctx->operation_by_value[source_value]
                           : XR_SEMANTIC_INDEX_NONE;
    const XrSemanticOperationRecord *definition =
        definer != XR_SEMANTIC_INDEX_NONE ? xr_semantic_plan_operation(ctx->semantic, definer)
                                          : NULL;
...
static void rep_materialized_adapter_survey_row(const XrSemanticPlan *semantic,
...
static bool oracle_tagged_reference_carrier_storage(const VerifyAuthority *ctx,
                                                    uint32_t semantic_value, XrRep *out_storage,
                                                    uint16_t *out_machine_kind);
static bool oracle_reference_parameter_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                               XrRep *out_storage, uint16_t *out_machine_kind);
static bool oracle_dynamic_direct_local_reference_result_storage(const VerifyAuthority *ctx,
                                                                 uint32_t semantic_value,
                                                                 XrRep *out_storage,
                                                                 uint16_t *out_machine_kind);
...
static void verify_authority_dispose(VerifyAuthority *ctx) {
    ...
    ctx->program_reachability_active = false;
}
 ...
 * Element storage follows the same scalar or owned-reference judgement as
 * allocation and member stores in the target and C-emission plans. */
...
static bool aot_array_allocation_is_exact(const XrSemanticPlan *semantic,
    ...
    bool exact = element && capacity_type && function &&
                 xr_target_array_allocation_element_storage(
                     semantic, element, operation->array_element_storage, &semantic_storage) &&
                 semantic_exact_i64_type(capacity_type) && capacity->role == XR_SEM_OPERAND_VALUE &&
...
/* Structural objects own a heap root. Their exact field shape does not grant
 * inline value storage; every allocation and call transports a tagged root. */
static bool aot_struct_object_type_is_exact(const XrSemanticTypeRecord *type) {
    XrStableId zero = {{0}};
    return type && type->kind == XR_KIND_STRUCT_OBJECT &&
           (type->flags & (uint8_t) ~XR_SEM_TYPE_CONST) ==
               (uint8_t) (XR_SEM_TYPE_REFERENCE_CAPABLE | XR_SEM_TYPE_OWNERSHIP_ROOT) &&
...
/* A structural copy may retain the nullable bit on its tagged result.  Keep
 * the ordinary allocation predicate strict and give this one call family an
 * explicit nullable authority. */
static bool aot_struct_object_copy_type_is_exact(const XrSemanticTypeRecord *type) {
    return type && type->kind == XR_KIND_STRUCT_OBJECT &&
           (type->flags & (uint8_t) ~(XR_SEM_TYPE_CONST | XR_SEM_TYPE_NULLABLE)) ==
               (uint8_t) (XR_SEM_TYPE_REFERENCE_CAPABLE | XR_SEM_TYPE_OWNERSHIP_ROOT) &&
           type->scalar_rep == XR_SCALAR_REP_NONE && type->builtin_type == XR_TID_NULL &&
           type->child_count != 0 && type->aggregate_extent == type->child_count &&
           type->aggregate_align == 0 && type->source_class == XR_SEMANTIC_INDEX_NONE &&
           type->canonical_key != NULL;
}
...
static bool aot_json_namespace_codec_is_exact(const XrSemanticPlan *semantic,
                                              const XrSemanticOperationRecord *operation,
    ...
    if (!semantic || !operation ||
        operation->intrinsic_kind != XR_SEM_INTRINSIC_JSON_NAMESPACE_CODEC ||
    ...
    if (!result_type || !xr_semantic_json_codec_result_is_exact(
...
static bool aot_array_type_is_exact(const XrSemanticPlan *semantic, uint32_t type_index,
    ...
    if (!aot_array_element_storage_is_exact(element_type, &element)) {
        if (indexes_elements)
            return false;
        element = xr_semantic_tagged_string_type_is_exact(element_type) ||
...
/* The subject a direct-local ref boundary may address. Arrays require an exact
 * scalar element storage because the callee can index them; a source-class
 * instance has no element domain and therefore carries NONE. Both still use
 * the same tagged outer cell and RAW_PTR callee place. */
static bool aot_direct_ref_subject_type_is_exact(const XrSemanticPlan *semantic,
...
static bool aot_tagged_ref_parameter_is_exact(const XrSemanticPlan *semantic,
    ...
    if (!semantic || !parameter ||
        ...
        return false;
    return aot_direct_ref_subject_type_is_exact(semantic, parameter->type, storage);
}

static bool aot_reference_boundary_type_is_exact(const XrSemanticPlan *semantic,
                                                 uint32_t type_index, uint8_t *storage) {
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(semantic, type_index);
    if (xr_semantic_task_type_is_exact(semantic, type_index) ||
        xr_semantic_tagged_tuple_type_is_exact(semantic, type)) {
        if (storage)
            *storage = XR_TARGET_ARRAY_STORAGE_NONE;
        return true;
    }
    if (aot_struct_object_type_is_exact(type) &&
        xr_semantic_source_structural_shape_is_exact(semantic, type_index)) {
        if (storage)
            *storage = XR_TARGET_ARRAY_STORAGE_NONE;
        return true;
    }
    return aot_array_type_is_exact(semantic, type_index, false, storage);
}

static bool aot_reference_value_parameter_is_exact(const XrSemanticPlan *semantic,
                                                   const XrSemanticParameterRecord *parameter,
    ...
    return xr_semantic_direct_local_reference_parameter_is_exact(semantic, parameter,
                                                                 callee_owns) &&
           aot_reference_boundary_type_is_exact(semantic, parameter->type, storage);
}
...
static bool aot_direct_local_reference_result_is_exact(const XrSemanticPlan *semantic,
                                                       uint32_t operation_index) {
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(semantic, operation_index);
    if (!semantic || !operation || !xr_semantic_local_call_result_opcode_is_exact(operation) ||
        operation->result_alias_operand != -1 || operation->return_parameter != -1 ||
        operation->return_complete != 1 || operation->return_provenance != XR_SEM_RETURN_OWNED ||
        !aot_reference_boundary_type_is_exact(semantic, operation->result_type, NULL))
        return false;
    size_t target_count = xr_semantic_plan_call_target_count(semantic);
    const XrSemanticFunctionRecord *callee = NULL;
    for (size_t i = 0; i < target_count; i++) {
...
typedef struct AotSourceClassArrayFillAuthority {
    uint32_t operation;
    uint32_t receiver;
    uint32_t element;
    uint32_t start;
...
static bool aot_scalar_array_range_fill_semantic_is_exact(const XrSemanticPlan *semantic,
                                                          uint32_t operation_index,
    ...
    for (uint16_t ordinal = 1; ordinal < operation->operand_count; ordinal++) {
        ...
        if (!xr_semantic_array_member_argument_is_exact(shape, &rows[ordinal], argument_type,
                                                        ordinal, element_type_index) ||
 ...
 * guess participates. */
static bool aot_source_class_array_fill_semantic_is_exact(const XrSemanticPlan *semantic,
                                                          uint32_t operation_index,
                                                          AotSourceClassArrayFillAuthority *out) {
    const XrSemanticOperationRecord *operation =
        semantic ? xr_semantic_plan_operation(semantic, operation_index) : NULL;
    uint32_t operand_count = 0, metadata_count = 0, child_count = 0;
    const XrSemanticOperandRecord *operands =
    ...
    const XrSemanticFunctionRecord *function =
        xr_semantic_plan_function(semantic, operation->function);
    if (!shape || shape->element_operand != 1 ||
        ...
        !function || !xr_semantic_array_type_row_is_exact(array) ||
        array->child_begin >= child_count)
        return false;
    uint32_t element_type_index = children[array->child_begin];
    const XrSemanticTypeRecord *element_type = xr_semantic_plan_type(semantic, element_type_index);
    bool exact_result = operation->result_type == rows[0].type &&
                        operation->result_alias_operand == 0 &&
    ...
    if (operation->result_value < function->value_begin ||
        ...
        return false;
    if (out)
        *out = (AotSourceClassArrayFillAuthority) {
            .operation = operation_index,
            .receiver = rows[0].value,
            .element = rows[1].value,
            .start = rows[2].value,
...
static bool aot_array_hof_semantic_is_exact(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (!result_exact || !producer || producer_index >= operation_index ||
        ...
        !xr_semantic_owned_closure_is_exact(ctx->semantic, producer) ||
...
static bool aot_array_reserve_use_is_exact(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (!receiver_type || receiver_type->kind != XR_KIND_ARRAY ||
        ...
        capacity->ownership_action != XR_SEM_OPERAND_CONSUME)
        return false;
    const XrTargetCallRecord *call = verify_target_call(ctx, operation_index);
    ...
    if (!call ||
        ...
        !verify_target_function_is(ctx, operation->function, call->caller_function) ||
        ...
        !aot_stable_id_is_zero(call->source_declaration_identity) ||
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        if (invalid[value] || use_count[value] == 0 ||
            !xr_semantic_direct_local_callee_type_is_exact(
                ctx->semantic, operation, ctx->direct_callee_target_by_value[value]))
            goto invalid_authority;
...
static bool aot_index_source_namespace_values(VerifyAuthority *ctx) {
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        for (uint32_t depth = 0;; depth++) {
            ...
            if (!source || source->result_value != current_value ||
                ...
                (candidate[current_value] &&
                 ((!mixed_native && ctx->source_namespace_dependency_by_value[current_value] !=
                                        target->dependency) ||
        ...
        if (mixed_native && import->import_resolution == XR_SEM_IMPORT_RESOLUTION_NATIVE_STDLIB) {
            for (uint32_t value = 0; value < ctx->value_count; value++) {
                if (visit_epoch[value] != receiver_epoch && visit_epoch[value] != import_epoch)
                    continue;
                candidate[value] = 0;
                ctx->source_namespace_dependency_by_value[value] = XR_SEMANTIC_INDEX_NONE;
        ...
        if (mixed_native)
            for (uint32_t value = 0; value < ctx->value_count; value++)
                if (visit_epoch[value] == receiver_epoch || visit_epoch[value] == import_epoch)
                    ctx->source_namespace_dependency_by_value[value] = import_dependency;
    }
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        for (uint16_t a = 0; a < use->operand_count; a++) {
            ...
            if (expected && (use->opcode == XI_CALL || use->opcode == XI_CALL_METHOD)) {
                ...
                bool target_owns_source_dependency =
                    ...
                    target->dependency == ctx->source_namespace_dependency_by_value[operand->value];
                bool mixed_native =
...
static bool aot_go_store_dominates_slot_reads(const XrSemanticPlan *semantic,
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        for (uint16_t a = 0; a < use->operand_count; a++) {
            ...
            bool exact =
                ...
                initialized &&
                aot_go_store_dominates_slot_reads(ctx->semantic, &graph, store->semantic_immediate,
                                                  row->operation) &&
                ...
                xr_semantic_owned_closure_is_exact(ctx->semantic, closure) &&
                ...
                xr_semantic_direct_local_callee_type_is_exact(ctx->semantic, load, target);
            for (uint16_t argument = 1; exact && argument < use->operand_count; argument++) {
                ...
                exact =
                    parameter &&
                    xr_semantic_type_is_const_variant(ctx->semantic, arg->type, parameter->type) &&
...
static const XiValue *materialized_unique_return_witness(const XiFunc *function) {
    if (!function)
        return NULL;
    const XiValue *witness = NULL;
    for (uint32_t block = 0; block < function->nblocks; block++) {
        const XiBlock *candidate = function->blocks ? function->blocks[block] : NULL;
        if (!candidate || candidate->kind != XI_BLOCK_RETURN || !candidate->control)
            continue;
        if (witness)
            return NULL;
        witness = candidate->control;
    }
    return witness;
}

/* Program-semantic aggregates deliberately replace the analyzer's instance
 * carrier classification with a frozen value shape.  Re-prove that replacement
 * from the TargetPlan projection and the exact PSC row stamped on the live SSA
 * witness; never infer it from the source XrType flags or spelling. */
static bool materialized_signature_type_matches(const VerifyAuthority *ctx, uint32_t semantic_type,
                                                const XrType *live_type, const XiValue *witness) {
    const XrSemanticTypeRecord *type =
    ...
    bool projection_exact = ctx &&
                            xr_c_leaf_aggregate_projection(ctx->target_plan, ctx->semantic,
                                                           semantic_type, &projection) &&
                            projection.kind == XR_C_AGGREGATE_PROJECTION_NAMED_STRUCT;
    bool exact =
        provenance &&
        provenance->program_family == XR_PROGRAM_SEMANTIC_FAMILY_LEAF_VALUE_AGGREGATE_DIRECT_CALL &&
        binding && type && witness && witness->type && live_type &&
        witness->psc_type_index == binding->program_row &&
        binding->semantic_type == semantic_type &&
...
static bool verify_static_shared_callable_type_authority(VerifyAuthority *ctx,
    ...
    bool frozen_exact =
        xr_semantic_direct_local_callee_type_is_exact(ctx->semantic, operation, target_index);
    bool live_type_exact = semantic_type && source_type_matches(live->type, semantic_type);
    if (!frozen_exact || !semantic_type || !semantic_target ||
        shared_owner_index != semantic_target->parent || !shared_owner ||
        shared_owner->semantic_plan_function_index != shared_owner_index || !live_type_exact ||
    ...
    const XiValue *return_witness = materialized_unique_return_witness(callee);
    bool callee_return_exact =
        callee && callee->return_type &&
        materialized_signature_type_matches(ctx, semantic_target->return_type, callee->return_type,
                                            return_witness);
    bool live_return_exact =
        !typed_function ||
        materialized_signature_type_matches(ctx, semantic_target->return_type,
                                            live->type->function.return_type, return_witness);
    if (!callee || callee->parent_func != shared_owner || pointer_matches != 1 ||
        ...
        callee->nparams != semantic_target->parameter_count || !callee_return_exact ||
        !live_return_exact) {
        return false;
    }
        ...
        if (!parameter || !callee->params || !callee->params[i] ||
            (typed_function &&
             (!live->type->function.params ||
              ...
              !materialized_signature_type_matches(
                  ctx, parameter->type, live->type->function.params[i].type, callee->params[i]))) ||
            !materialized_signature_type_matches(ctx, parameter->type, callee->params[i]->type,
                                                 callee->params[i]))
            return false;
    }
    return true;
...
static bool verify_authority_init(VerifyAuthority *ctx) {
    if (!verify_target_scope_init(ctx)) {
        set_diag(ctx ? ctx->diag : NULL, XR_AOT_REFINEMENT_SOURCE_IDENTITY, 0, 0, 0);
        return false;
    }
    ...
    return aot_program_reachability_init(ctx) && aot_index_direct_local_callee_values(ctx) &&
...
static bool oracle_value_aggregate_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrSemanticParameterRecord *parameter =
        parameter_index != XR_SEMANTIC_INDEX_NONE
            ? xr_semantic_plan_parameter(ctx->semantic, parameter_index)
            : NULL;
    /* A parameter row and its XI_PARAM SSA operation intentionally define the
     * same value. The parameter owns the TargetPlan slot, whose operation is
     * therefore NONE; the duplicate operation is admitted only when it is the
     * exact parameter definition frozen by SemanticPlan. */
    if (parameter && operation &&
        (operation->opcode != XI_PARAM || operation->function != parameter->function ||
         operation->result_value != parameter->value || operation->result_type != parameter->type ||
    ...
    bool leaf_family =
        provenance &&
        provenance->program_family == XR_PROGRAM_SEMANTIC_FAMILY_LEAF_VALUE_AGGREGATE_DIRECT_CALL;
    XrCAggregateProjection leaf_projection = {0};
    bool exact_shape = leaf_family ? xr_c_leaf_aggregate_projection(ctx->target_plan, ctx->semantic,
                                                                    semantic_type, &leaf_projection)
    ...
    if (!exact_shape || !binding || !register_rep || !memory_rep || !layout || !slot ||
        ...
        !verify_target_function_is(ctx, semantic_function, slot->function) ||
...
/* Object fields are accessed through an exact tagged root. Parameters, call
 * results and local producers share their existing physical slot validators;
 * nested field roots retain the bounded ordinal reconstruction. */
static bool struct_object_carrier_storage_depth(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    if (!ctx || depth > XR_AOT_STRUCT_OBJECT_MAX_NESTING || semantic_value >= ctx->value_count ||
        ...
        return false;
    /* Identity renames preserve the proved root. The target verifier checks
     * every renamed binding before this physical representation check. */
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ...
            : NULL;
    const XrSemanticParameterRecord *parameter =
        xr_semantic_plan_parameter(ctx->semantic, ctx->parameter_by_value[semantic_value]);
    const XrSemanticTypeRecord *type =
    ...
    if (!aot_struct_object_type_is_exact(type))
        return false;
    if (oracle_reference_parameter_storage(ctx, semantic_value, out_storage, out_machine_kind) ||
        oracle_dynamic_direct_local_reference_result_storage(ctx, semantic_value, out_storage,
                                                             out_machine_kind) ||
        oracle_dynamic_container_copy_storage(ctx, semantic_value, out_storage, out_machine_kind) ||
        oracle_dynamic_array_index_result_storage(ctx, semantic_value, out_storage,
                                                  out_machine_kind) ||
        oracle_dynamic_merge_carrier_storage(ctx, semantic_value, XR_KIND_STRUCT_OBJECT,
                                             out_storage, out_machine_kind))
        return true;
    ...
    bool carrier = aot_struct_object_allocation_is_exact(ctx->semantic, operation) ||
                   xr_semantic_optional_reference_project_is_exact(ctx->semantic, operation) ||
    ...
    if (operation->opcode != XI_OBJECT_GET_F)
        return oracle_dynamic_value_storage(ctx, semantic_value, out_storage, out_machine_kind);
    *out_storage = XR_REP_TAGGED;
 ...
 *
 * Null uses the established nullable-carrier admission. A non-null payload must
 * match the
 * nullable field's type, field by field and through its canonical key;
 * no unrelated widening is
 * authorized. */
static bool aot_field_store_type_is_admissible(const VerifyAuthority *ctx, uint32_t stored_type,
    ...
    if (xr_semantic_null_inhabits_parameter(stored, field) ||
        xr_semantic_null_inhabits_nullable_scalar_parameter(stored, field))
        return true;
...
/* External class types keep declaration identity across independently numbered
 * module tables. Both field readers and writers require one frozen owner. */
static bool oracle_external_class_type_is_declared(const VerifyAuthority *ctx,
                                                   const XrSemanticTypeRecord *type) {
    if (!ctx || !xr_semantic_external_class_instance_type_is_exact(type))
        return false;
    bool found = false;
    uint32_t module_count = xr_target_plan_program_module_count(ctx->target_plan);
    for (uint32_t row = 0; row < module_count; row++) {
        const XrSemanticPlan *module = xr_target_plan_program_module(ctx->target_plan, row);
        if (!module)
            return false;
        uint32_t class_count = (uint32_t) xr_semantic_plan_source_class_count(module);
        for (uint32_t index = 0; index < class_count; index++) {
            const XrSemanticSourceClassRecord *declaration =
                xr_semantic_plan_source_class(module, index);
            if (!declaration || !xr_stable_id_equal(declaration->id, type->source_class_identity))
                continue;
            if (found || module == ctx->semantic ||
                !xr_semantic_class_declaration_is_frozen(module, index))
                return false;
            found = true;
        }
    }
...
static bool oracle_source_class_field_read_storage(const VerifyAuthority *ctx,
                                                   uint32_t semantic_value, XrRep *out_storage,
                                                   uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, ctx->operation_by_value[semantic_value]);
    uint8_t carrier = 0;
    if (operation && operation->result_value == semantic_value &&
        xr_semantic_source_class_field_result_carrier_is_exact(ctx->semantic, operation, &carrier) &&
        carrier == XR_SEM_SOURCE_CLASS_FIELD_RESULT_BORROWED_TAGGED) {
        *out_storage = XR_REP_TAGGED;
        *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
        return true;
    }
    if (!xr_semantic_source_class_field_read_shape_is_exact(ctx->semantic, operation) ||
        operation->result_value != semantic_value ||
        !xr_semantic_managed_field_result_type_is_exact(ctx->semantic, operation->result_type))
        return false;
    const XrSemanticOperandRecord *receiver =
        xr_semantic_class_field_read_receiver_is_exact(ctx->semantic, operation);
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, receiver->type);
    if (xr_semantic_class_instance_type_source_class(ctx->semantic, type) == XR_SEMANTIC_INDEX_NONE &&
        !oracle_external_class_type_is_declared(ctx, type))
        return false;
    *out_storage = XR_REP_TAGGED;
    *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
    return true;
...
static bool oracle_fixed_array_backing_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    if (!operation || !type || !binding || !register_rep || !memory_rep || !layout || !slot ||
        ...
        !verify_target_function_is(ctx, operation->function, slot->function) ||
        slot->register_rep != binding->register_rep || slot->memory_rep != binding->memory_rep ||
        slot->root_kind != XR_TARGET_ROOT_NONE || slot->ownership != XR_TARGET_OWNERSHIP_TRIVIAL)
        return false;
...
/* Tuple payload access follows the exact tagged root, including local return
 * transfers and Optional projections, rather than inventing an inline layout. */

static bool oracle_tagged_tuple_storage(const VerifyAuthority *ctx, uint32_t value,
                                        XrRep *storage, uint16_t *kind) {
    if (!ctx || value >= ctx->value_count ||
        !oracle_resolve_identity_rename(ctx, value, &value))
        return false;
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, ctx->operation_by_value[value]);
    const XrSemanticParameterRecord *parameter =
        xr_semantic_plan_parameter(ctx->semantic, ctx->parameter_by_value[value]);
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(
        ctx->semantic, parameter ? parameter->type : operation ? operation->result_type
                                                              : XR_SEMANTIC_INDEX_NONE);
    if (!xr_semantic_tagged_tuple_type_is_exact(ctx->semantic, type) ||
        (type->flags & XR_SEM_TYPE_NULLABLE) != 0)
        return false;
    return oracle_dynamic_value_storage(ctx, value, storage, kind) ||
           oracle_reference_parameter_storage(ctx, value, storage, kind) ||
           oracle_dynamic_direct_local_reference_result_storage(ctx, value, storage, kind) ||
           oracle_dynamic_array_index_result_storage(ctx, value, storage, kind);
}
...
static bool oracle_aggregate_field_access_is_exact(const VerifyAuthority *ctx,
    ...
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    if (operation && operation->opcode == XI_TUPLE_GET &&
        xr_semantic_source_structural_field_read_is_exact(ctx->semantic, operation, NULL, NULL)) {
        XrRep storage = XR_REP_TAGGED;
        uint16_t kind = XR_MACHINE_REP_COUNT;
        if (oracle_tagged_tuple_storage(ctx, operands[operation->operand_begin].value,
                                       &storage, &kind))
            return storage == XR_REP_TAGGED && kind == XR_MACHINE_REP_DYN_VALUE;
    }
    XrStableId zero = {{0}};
    ...
    if (receiver->role != XR_SEM_OPERAND_VALUE || receiver->parameter != -1 ||
        ...
        return false;
    const XrTargetValueRepRecord *receiver_binding = verify_target_value_rep(ctx, receiver->value);
...
static bool aot_array_hof_target_is_exact(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    const XrTargetValueRepRecord *result =
        operation ? verify_target_value_rep(ctx, operation->result_value) : NULL;
    const XrTargetMachineRepRecord *result_register =
    ...
    if (!operation || !callee || !call || !result || !result_register || !result_memory ||
        ...
        !verify_target_function_is(ctx, operation->function, call->caller_function) ||
        !verify_target_function_is(ctx, operation->callable_function, call->callee_function) ||
        call->source_dependency != XR_SEMANTIC_INDEX_NONE ||
        call->source_export != XR_SEMANTIC_INDEX_NONE ||
        !aot_stable_id_is_zero(call->source_declaration_identity) ||
        !aot_stable_id_is_zero(call->source_callee_identity) ||
        ...
        !verify_target_function_is(ctx, operation->function, result_slot->function) ||
    ...
    for (uint16_t ordinal = 0; ordinal < call->argument_count; ordinal++) {
        ...
        const XrTargetValueRepRecord *caller = verify_target_value_rep(ctx, operand->value);
...
static bool oracle_dynamic_closure_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ...
            : NULL;
    if (!operation || operation->result_value != semantic_value ||
        !xr_semantic_owned_closure_is_exact(ctx->semantic, operation))
        return false;
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, semantic_value);
    const XrTargetMachineRepRecord *register_rep =
        binding ? xr_target_plan_machine_rep(ctx->target_plan, binding->register_rep) : NULL;
...
static bool oracle_dynamic_array_fill_scalar_storage(const VerifyAuthority *ctx,
    ...
    const XrTargetCallRecord *call = verify_target_call(ctx, operation_index);
    const XrTargetLayoutRecord *layout =
        operation ? verify_target_layout(ctx, operation->result_type) : NULL;
...
static bool oracle_native_yieldable_fresh_result_call(const VerifyAuthority *ctx,
                                                       uint32_t operation_index,
                                                       const XrTargetValueRepRecord *binding) {
    const XrSemanticOperationRecord *operation =
        ctx ? xr_semantic_plan_operation(ctx->semantic, operation_index) : NULL;
    ...
    const XrTargetCallRecord *calls =
        ctx ? xr_target_plan_calls(ctx->target_plan, &call_count) : NULL;
    uint32_t index = ctx && operation_index < ctx->operation_count
                         ? ctx->call_by_operation[operation_index] : XR_SEMANTIC_INDEX_NONE;
    ...
    uint32_t caller = XR_SEMANTIC_INDEX_NONE;
    return verify_target_function_index(ctx, operation->function, &caller) &&
           target->operation == operation_index && target->function == XR_SEMANTIC_INDEX_NONE &&
           target->dependency == XR_SEMANTIC_INDEX_NONE &&
           target->source_export == XR_SEMANTIC_INDEX_NONE &&
           target->callable_type == XR_SEMANTIC_INDEX_NONE &&
           ...
           aot_stable_id_is_zero(call->source_declaration_identity) &&
           aot_stable_id_is_zero(call->source_callee_identity) &&
           aot_stable_id_is_zero(call->native_callee_identity) &&
           call->flags == XR_TARGET_CALL_SUSPEND && call->argument_count == 0 &&
           call->calling_convention == (namespace_call
               ? XR_TARGET_CALL_CONVENTION_NATIVE_NAMESPACE_YIELDABLE
               : XR_TARGET_CALL_CONVENTION_NATIVE_YIELDABLE) &&
           call->target_kind == (namespace_call ? XR_TARGET_CALL_TARGET_NATIVE_NAMESPACE_YIELDABLE
                                                : XR_TARGET_CALL_TARGET_NATIVE_YIELDABLE) &&
           call->result_value == operation->result_value &&
           binding->semantic_value == operation->result_value &&
...
static bool oracle_dynamic_value_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    if (!operation || !verify_target_function_index(ctx, operation->function, &target_function))
        return false;
    bool native_direct_fresh = operation &&
        (oracle_native_direct_fresh_result_call(ctx, operation_index, semantic_value, binding) ||
         oracle_native_yieldable_fresh_result_call(ctx, operation_index, binding));
    if (!operation || operation->result_value != semantic_value ||
    ...
    if (runtime_method) {
        runtime_call = verify_target_call(ctx, operation_index);
        if (!runtime_call ||
            ...
            !aot_stable_id_is_zero(runtime_call->source_declaration_identity) ||
...
static bool oracle_dynamic_heap_literal_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrTargetSlotRecord *slot =
        binding && binding->slot < slot_count ? &slots[binding->slot] : NULL;
    const XrTargetLayoutRecord *layout = verify_target_layout(ctx, operation->result_type);
    if (!binding || !register_rep || !memory_rep || !slot || !layout ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        memory_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        register_rep->root_kind != XR_TARGET_ROOT_DYNAMIC ||
...
static bool oracle_iterator_rune_nth_call(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrTargetValueRepRecord *result = verify_target_value_rep(ctx, semantic_value);
    const XrTargetValueRepRecord *caller = verify_target_value_rep(ctx, index_value);
    uint32_t call_count = 0;
    uint32_t argument_count = 0;
    const XrTargetCallRecord *calls = xr_target_plan_calls(ctx->target_plan, &call_count);
    const XrTargetCallArgumentRecord *arguments =
...
/* The conversion consumes one exact native Rune and materializes the owned
 * tagged String row already frozen by TargetPlan. No generic display or method
 * fallback can satisfy this judgement. */
static bool oracle_rune_to_string_call(const VerifyAuthority *ctx, uint32_t semantic_value,
                                       XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    uint32_t operation_index = ctx->operation_by_value[semantic_value];
    const XrSemanticOperationRecord *operation =
        operation_index < ctx->operation_count
            ...
            : NULL;
    uint32_t receiver = XR_SEMANTIC_INDEX_NONE;
    if (!xr_semantic_rune_to_string_is_exact(ctx->semantic, operation, &receiver) ||
        operation->result_value != semantic_value)
        return false;
    XrRep receiver_storage = XR_REP_TAGGED;
    uint16_t receiver_kind = XR_MACHINE_REP_COUNT;
    ...
    const XrTargetSlotRecord *slot =
        binding && binding->slot < slot_count ? &slots[binding->slot] : NULL;
    const XrTargetCallRecord *call = verify_target_call(ctx, operation_index);
    uint32_t call_index = call ? call->id : XR_SEMANTIC_INDEX_NONE;
    const XrTargetLayoutRecord *layout =
        operation ? verify_target_layout(ctx, operation->result_type) : NULL;
    uint32_t extent_count = 0;
    const XrTargetExtentRecord *extents = xr_target_plan_extents(ctx->target_plan, &extent_count);
    const XrTargetExtentRecord *extent =
        layout && layout->extent < extent_count ? &extents[layout->extent] : NULL;
    XrStableId expected_call;
    if (!result_type || !binding || !register_rep || !memory_rep || !slot || !call || !layout ||
        !extent ||
        !aot_pair_identity("xray-target-rune-to-string-v1", operation->id, result_type->id,
                           receiver, &expected_call) ||
        !xr_stable_id_equal(call->identity, expected_call) || call->id != call_index ||
        binding->semantic_value != semantic_value ||
        call->semantic_call_target != XR_SEMANTIC_INDEX_NONE ||
...
static bool oracle_dynamic_container_copy_storage(const VerifyAuthority *ctx,
    ...
    if (!operation || operation->result_value != semantic_value ||
        !xr_semantic_container_copy_is_exact(ctx->semantic, operation, NULL, &semantic_storage))
        return false;
    uint32_t child_count = 0;
    const uint32_t *children = xr_semantic_plan_type_children(ctx->semantic, &child_count);
    ...
    const XrSemanticTypeRecord *element =
        children && array && array->child_begin < child_count
            ? xr_semantic_plan_type(ctx->semantic, children[array->child_begin])
            : NULL;
    uint8_t expected_storage = XR_TARGET_ARRAY_STORAGE_NONE;
    if (xr_semantic_tagged_string_type_is_exact(array)) {
        expected_storage = XR_TARGET_ARRAY_STORAGE_NONE;
    } else if (array && aot_struct_object_copy_type_is_exact(array)) {
        if (!xr_semantic_source_structural_copy_shape_is_exact(ctx->semantic,
                                                               operation->result_type))
            return false;
    } else if (semantic_storage == XR_ELEM_ANY) {
        if (!element || (!xr_semantic_tagged_string_type_is_exact(element) &&
                         xr_semantic_class_instance_type_source_class(ctx->semantic, element) ==
                             XR_SEMANTIC_INDEX_NONE))
            return false;
        expected_storage = XR_TARGET_ARRAY_STORAGE_TAGGED;
    } else if (!aot_array_element_storage_is_exact(element, &expected_storage)) {
        return false;
    }
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, semantic_value);
    const XrTargetMachineRepRecord *register_rep =
    ...
    const XrTargetLayoutRecord *layout =
        layouts && layout_index < layout_count ? &layouts[layout_index] : NULL;
    if (!binding || !register_rep || !memory_rep || !slot || !call || !layout ||
        binding->semantic_value != semantic_value ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        memory_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        register_rep->root_kind != XR_TARGET_ROOT_DYNAMIC ||
        ...
        layout->kind != XR_TARGET_LAYOUT_DYNAMIC ||
        layout->array_element_storage != expected_storage)
        return false;
    *out_storage = XR_REP_TAGGED;
    *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
    return true;
...
/* All tagged temporary producers share the same physical carrier contract.
 * Layout and function ordinals are resolved inside the verified module scope;
 * unrelated modules can reuse the same semantic type and value ordinals. */
static bool tagged_value_temporary_rows_are_exact(const VerifyAuthority *ctx,
    ...
    const XrTargetLayoutRecord *layout =
        layouts && layout_index < layout_count ? &layouts[layout_index] : NULL;
    uint32_t target_function = XR_SEMANTIC_INDEX_NONE;
    return binding && binding->semantic_value == semantic_value && register_rep && memory_rep &&
           slot && layout && verify_target_function_index(ctx, operation->function, &target_function) &&
           register_rep->kind == XR_MACHINE_REP_DYN_VALUE &&
           memory_rep->kind == XR_MACHINE_REP_DYN_VALUE &&
           register_rep->root_kind == XR_TARGET_ROOT_DYNAMIC &&
           memory_rep->root_kind == XR_TARGET_ROOT_DYNAMIC &&
...
static bool oracle_dynamic_source_class_object_storage(const VerifyAuthority *ctx,
    ...
    if (!operation || operation->result_value != semantic_value ||
        ...
        return false;
    if (!tagged_value_temporary_rows_are_exact(ctx, semantic_value, operation, operation_index,
                                               XR_TARGET_OWNERSHIP_OWNED))
        return false;
    *out_storage = XR_REP_TAGGED;
    *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
 ...
 * construction. */
static bool
oracle_imported_source_class_construction_is_exact(const VerifyAuthority *ctx,
                                                   uint32_t operation_index,
                                                   const XrSemanticOperationRecord *operation) {
    if (!ctx || !operation)
        return false;
    ...
    uint32_t source_class =
        target && target->dependency < xr_semantic_plan_dependency_count(ctx->semantic)
            ? xr_semantic_imported_class_construction_authority_source_class(
                  ...
                  xr_semantic_plan_dependency(ctx->semantic, target->dependency), source_export,
    ...
    const XrSemanticFunctionRecord *callee =
        constructor != XR_SEMANTIC_INDEX_NONE ? xr_semantic_plan_function(dependency, constructor)
                                              : NULL;
    XrStableId zero = {{0}};
    bool target_identity = target && source_export && source_class != XR_SEMANTIC_INDEX_NONE &&
...
static bool
oracle_source_dependency_owned_class_call_is_exact(const VerifyAuthority *ctx, uint32_t operation_index,
                                               const XrSemanticOperationRecord *operation) {
    if (!ctx || !operation)
        return false;
    const XrSemanticCallTargetRecord *target = NULL;
    ...
    const XrSemanticPlan *dependency =
        target ? verify_semantic_dependency(ctx, target->dependency) : NULL;
    uint32_t function = xr_semantic_source_dependency_call_function(ctx->semantic, target, dependency);
    const XrSemanticFunctionRecord *callee = xr_semantic_plan_function(dependency, function);
    bool instance_method = target && target->kind == XR_SEM_CALL_TARGET_SOURCE_METHOD_DEPENDENCY;
    bool static_method = target && target->kind == XR_SEM_CALL_TARGET_SOURCE_STATIC_METHOD_DEPENDENCY;
    uint32_t source_class = xr_semantic_source_dependency_owned_class_result_source_class(
    ...
    return target && callee && source_class != XR_SEMANTIC_INDEX_NONE && call &&
           binding &&
           verify_target_function_index(ctx, operation->function, &caller_function) &&
           ...
           call->calling_convention == (instance_method ? XR_TARGET_CALL_CONVENTION_SOURCE_METHOD
                : static_method ? XR_TARGET_CALL_CONVENTION_SOURCE_STATIC_METHOD
                                : XR_TARGET_CALL_CONVENTION_SOURCE_EXPORT) &&
           call->target_kind == (instance_method ? XR_TARGET_CALL_TARGET_SOURCE_METHOD
                : static_method ? XR_TARGET_CALL_TARGET_SOURCE_STATIC_METHOD
...
static bool oracle_dynamic_source_class_instance_storage(const VerifyAuthority *ctx,
    ...
    bool exact_source_export =
        ...
        oracle_source_dependency_owned_class_call_is_exact(ctx, operation_index, operation);
    if (!operation || operation->result_value != semantic_value ||
...
static bool aot_scalar_array_range_fill_binding_is_exact(const VerifyAuthority *ctx,
                                                         uint32_t operation_index,
                                                         AotScalarArrayRangeFillAuthority *out) {
    ...
    const XrTargetValueRepRecord *result = verify_target_value_rep(ctx, operation->result_value);
    const XrTargetMachineRepRecord *result_register =
        result ? xr_target_plan_machine_rep(ctx->target_plan, result->register_rep) : NULL;
    const XrTargetMachineRepRecord *result_memory =
        result ? xr_target_plan_machine_rep(ctx->target_plan, result->memory_rep) : NULL;
    ...
    const XrTargetSlotRecord *result_slot =
        result && slots && result->slot < slot_count ? &slots[result->slot] : NULL;
    const XrTargetLayoutRecord *layout =
        verify_target_layout(ctx, shape.receiver_type);
    ...
    if (!operation || !call || !receiver_type || !result || !result_register || !result_memory ||
        ...
        !aot_pair_identity("xray-target-array-member-scalar-v1", operation->id, receiver_type->id,
                           shape.element, &expected_identity) ||
        ...
        call->adapter_count != 0 || call->flags != 0 || call->result_mode != XR_TARGET_CALL_VALUE ||
        call->result_ownership != XR_TARGET_CALL_NONE ||
        call->calling_convention != XR_TARGET_CALL_CONVENTION_ARRAY_MEMBER_SCALAR ||
        call->target_kind != XR_TARGET_CALL_TARGET_ARRAY_MEMBER_SCALAR ||
...
static bool oracle_dynamic_scalar_array_range_fill_result_storage(const VerifyAuthority *ctx,
                                                                  uint32_t semantic_value,
                                                                  XrRep *out_storage,
                                                                  uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
 ...
 * and its ordered argument partition to the exact semantic values and machine
 * representations that the live Xi operation must materialize. */
static bool aot_source_class_array_fill_binding_is_exact(const VerifyAuthority *ctx,
                                                         uint32_t operation_index,
                                                         AotSourceClassArrayFillAuthority *out) {
    AotSourceClassArrayFillAuthority shape = {0};
    if (!ctx || operation_index >= ctx->operation_count ||
        !aot_source_class_array_fill_semantic_is_exact(ctx->semantic, operation_index, &shape))
        return false;
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, operation_index);
    uint32_t call_index = ctx->call_by_operation[operation_index];
    ...
    const XrTargetMachineRepRecord *result_memory =
        result ? xr_target_plan_machine_rep(ctx->target_plan, result->memory_rep) : NULL;
    if (!operation || !call || !arguments || !result || !result_register || !result_memory ||
        call->id != call_index || call->semantic_operation != operation_index ||
        call->result_value != operation->result_value || call->result_slot != result->slot ||
        call->result_register_rep != result->register_rep ||
        call->result_memory_rep != result->memory_rep || call->argument_count != 4 ||
        call->argument_begin > argument_count ||
        call->argument_count > argument_count - call->argument_begin || call->adapter_count != 0 ||
        call->calling_convention != XR_TARGET_CALL_CONVENTION_ARRAY_MEMBER_SCALAR ||
        call->target_kind != XR_TARGET_CALL_TARGET_ARRAY_MEMBER_SCALAR ||
    ...
    for (uint16_t ordinal = 0; ordinal < 4; ordinal++) {
        uint32_t semantic_operand = operation->operand_begin + ordinal;
        if (!semantic_operands || semantic_operand >= semantic_operand_count)
            return false;
        ...
        const XrTargetMachineRepRecord *caller_memory =
            caller ? xr_target_plan_machine_rep(ctx->target_plan, caller->memory_rep) : NULL;
        const XrTargetCallArgumentRecord *argument = &arguments[call->argument_begin + ordinal];
        XrRep storage = XR_REP_VOID;
        uint16_t machine = XR_MACHINE_REP_COUNT;
        bool exact_storage =
            ordinal == 0
                ? oracle_array_tagged_carrier_storage(ctx, operand->value, &storage, &machine)
            : ordinal == 1
                ? (oracle_reference_parameter_storage(ctx, operand->value, &storage, &machine) ||
                   oracle_dynamic_source_class_instance_storage(ctx, operand->value, &storage,
                                                                &machine))
                : oracle_machine_storage(ctx, operand->value, &storage, &machine);
        uint32_t expected_value = ordinal == 0   ? shape.receiver
                                  : ordinal == 1 ? shape.element
                                  : ordinal == 2 ? shape.start
                                                 : shape.end;
        uint16_t expected_machine = ordinal < 2 ? XR_MACHINE_REP_DYN_VALUE : XR_MACHINE_REP_I64;
        XrRep expected_storage = ordinal < 2 ? XR_REP_TAGGED : XR_REP_I64;
        if (!caller || !caller_register || !caller_memory || !exact_storage ||
            operand->value != expected_value || storage != expected_storage ||
            machine != expected_machine || argument->call != call_index ||
            ...
            argument->array_element_storage !=
                (ordinal == 1 ? XR_TARGET_ARRAY_STORAGE_TAGGED : XR_TARGET_ARRAY_STORAGE_NONE) ||
            caller_register->kind != expected_machine || caller_memory->kind != expected_machine)
            return false;
    }
    if (out)
        *out = shape;
...
static bool oracle_dynamic_source_class_array_fill_result_storage(const VerifyAuthority *ctx,
                                                                  uint32_t semantic_value,
                                                                  XrRep *out_storage,
                                                                  uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
...
/* The independent TargetPlan verifier has already admitted each view's
 * origin and borrow lifetime. AOT checks the same pointer/length carrier for
 * parameters, slices, and runtime-produced views, without a second roster of
 * allowed producers. */
static bool oracle_slice_view_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                      XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    uint32_t parameter_index = ctx->parameter_by_value[semantic_value];
    const XrSemanticParameterRecord *parameter =
        parameter_index < ctx->parameter_count
            ? xr_semantic_plan_parameter(ctx->semantic, parameter_index)
            : NULL;
    uint32_t operation_index =
        parameter ? XR_SEMANTIC_INDEX_NONE : ctx->operation_by_value[semantic_value];
    const XrSemanticOperationRecord *operation =
        operation_index < ctx->operation_count
            ? xr_semantic_plan_operation(ctx->semantic, operation_index)
            : NULL;
    ...
    uint32_t semantic_function = parameter   ? parameter->function
                                 : operation ? operation->function
                                             : XR_SEMANTIC_INDEX_NONE;
    uint8_t role = parameter                                  ? XR_TARGET_SLOT_PARAMETER
                   : operation && operation->opcode == XI_PHI ? XR_TARGET_SLOT_PHI
                                                              : XR_TARGET_SLOT_TEMPORARY;
    if ((!parameter && (!operation || operation->result_value != semantic_value)) ||
        (parameter &&
         (parameter->value != semantic_value || parameter->mode != XR_PARAM_READ ||
          (parameter->ownership != XI_OWN_BORROWED && parameter->ownership != XI_OWN_OWNED) ||
          parameter->transfer_mode != XR_TRANSFER_SHARE ||
          (parameter->flags & ~XR_SEM_PARAMETER_REQUIRED) != 0 || parameter->reserved != 0)) ||
        !xr_semantic_slice_view_type_is_exact(ctx->semantic, semantic_type, NULL))
        return false;
    ...
    const XrTargetSlotRecord *slot =
        binding && binding->slot < slot_count ? &slots[binding->slot] : NULL;
    const XrTargetLayoutRecord *layout = verify_target_layout(ctx, semantic_type);
    if (!binding || !register_rep || !memory_rep || !slot || !layout ||
        ...
        register_rep->detail != ctx->layout_by_type[semantic_type] ||
        memory_rep->detail != ctx->layout_by_type[semantic_type] || register_rep->lane_count != 0 ||
        memory_rep->lane_count != 0 || register_rep->reserved != 0 || memory_rep->reserved != 0 ||
        ...
        slot->register_rep != binding->register_rep || slot->memory_rep != binding->memory_rep ||
        slot->role != role || slot->root_kind != XR_TARGET_ROOT_VIEW_OWNER ||
...
static bool oracle_direct_local_tagged_ref_parameter_place_storage(const VerifyAuthority *ctx,
                                                                   uint32_t semantic_value,
                                                                   XrRep *out_storage,
                                                                   uint16_t *out_machine_kind);

/* An element read or write requires an exact Array type and proved carrier.
 * The index is an exact signed
...
static bool oracle_array_element_access_is_exact(const VerifyAuthority *ctx,
    ...
    if (container->role != XR_SEM_OPERAND_VALUE || container->parameter != -1 ||
        ...
        !aot_array_type_is_exact(ctx->semantic, container->type, false, NULL) ||
    ...
    if (!container_type || container_type->child_begin >= child_count)
        return false;
...
static bool oracle_nullable_scalar_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrTargetSlotRecord *slot =
        binding && binding->slot < slot_count ? &slots[binding->slot] : NULL;
    const XrTargetLayoutRecord *layout = verify_target_layout(ctx, type_index);
    if (!binding || !register_rep || !memory_rep || !slot || !layout ||
        binding->semantic_value != semantic_value ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        memory_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        ...
        layout->align != memory_rep->memory_align || slot->semantic_value != semantic_value ||
        slot->semantic_operation != (parameter ? XR_SEMANTIC_INDEX_NONE : operation_index) ||
        !verify_target_function_is(ctx, parameter ? parameter->function : operation->function,
                                   slot->function) ||
    ...
    return container->role == XR_SEM_OPERAND_VALUE && container->parameter == -1 &&
           ...
           (oracle_array_tagged_carrier_storage(ctx, container->value, &container_storage,
            ...
            oracle_range_tagged_carrier_storage(ctx, container->value, &container_storage,
                                                &container_kind) ||
            (xr_semantic_channel_type_row_is_exact(
                 ctx->semantic, xr_semantic_plan_type(ctx->semantic, container->type)) &&
             oracle_definition_storage(ctx, container->value, &container_storage, &container_kind) &&
             container_storage == XR_REP_TAGGED && container_kind == XR_MACHINE_REP_DYN_VALUE));
}
...
static bool oracle_dynamic_json_namespace_codec_storage(const VerifyAuthority *ctx,
                                                        uint32_t semantic_value, XrRep *out_storage,
                                                        uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    ...
    if (!aot_json_namespace_codec_is_exact(ctx->semantic, operation, &argument_value))
        return false;
    const XrSemanticTypeRecord *result_type =
        xr_semantic_plan_type(ctx->semantic, operation->result_type);
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, semantic_value);
    ...
    xr_stable_id_hex(result_type ? result_type->id : (XrStableId) {{0}}, second_hex);
    int written = snprintf(key, sizeof(key),
                           "xray-target-json-namespace-codec-v1:first=%s:second=%s:ordinal=%u",
...
/* Call identity, signature and ABI are admitted by TargetPlan independently.
 * A tagged result keeps its exact physical carrier even when a cold function
 * has no executable call row. All call forms use the frozen result ownership
 * and module-scoped storage; no receiver-method spelling selects this rule. */
static bool oracle_dynamic_call_result_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                               XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    uint32_t operation_index = ctx->operation_by_value[semantic_value];
    ...
    if (!operation || operation->result_value != semantic_value ||
        (operation->opcode != XI_CALL && operation->opcode != XI_TAIL_CALL &&
         operation->opcode != XI_CALL_BUILTIN && operation->opcode != XI_CALL_METHOD &&
...
static bool oracle_direct_local_tagged_ref_parameter_place_storage(const VerifyAuthority *ctx,
    ...
    if (!parameter || !binding || !register_rep || !memory_rep || !slot ||
        ...
        !verify_target_function_is(ctx, parameter->function, slot->function) ||
...
static bool tagged_value_parameter_rows_are_exact(const VerifyAuthority *ctx,
    ...
    return parameter && binding && register_rep && memory_rep && slot &&
           ...
           slot->role == XR_TARGET_SLOT_PARAMETER && slot->register_rep == binding->register_rep &&
           slot->memory_rep == binding->memory_rep && slot->root_kind == XR_TARGET_ROOT_DYNAMIC &&
           slot->ownership == callee_ownership;
}

/* TargetPlan independently admits the exact type, signature and physical rows.
 * A by-value reference parameter preserves that tagged carrier and declared
 * ownership, including imported and optional references. Reconstructing a local
 * class declaration here would lose the program-wide identity already proved. */
static bool oracle_reference_parameter_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                               XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    ...
    const XrSemanticParameterRecord *parameter =
        parameter_index != XR_SEMANTIC_INDEX_NONE
            ? xr_semantic_plan_parameter(ctx->semantic, parameter_index)
            : NULL;
    if (parameter && xr_semantic_task_type_is_exact(ctx->semantic, parameter->type) &&
        (parameter->mode != XR_PARAM_READ || parameter->ownership != XI_OWN_BORROWED))
        return false;
    bool callee_owns = false;
    bool exact_parameter = xr_semantic_direct_local_reference_parameter_is_exact(
        ctx->semantic, parameter, &callee_owns);
    if (!exact_parameter && parameter &&
        xr_semantic_class_method_receiver_source_class(ctx->semantic, parameter_index) !=
            XR_SEMANTIC_INDEX_NONE) {
        exact_parameter = true;
        callee_owns = parameter->ownership == XI_OWN_OWNED;
    }
    if (!exact_parameter ||
        (!xr_semantic_task_type_is_exact(ctx->semantic, parameter->type) &&
         ...
         !xr_semantic_tagged_tuple_type_is_exact(
             ctx->semantic, xr_semantic_plan_type(ctx->semantic, parameter->type))) ||
...
/* String consumers retain their exact type check while sharing parameter storage. */
static bool oracle_direct_local_string_value_parameter_storage(const VerifyAuthority *ctx,
    ...
    const XrSemanticParameterRecord *parameter =
        xr_semantic_plan_parameter(ctx->semantic, ctx->parameter_by_value[semantic_value]);
    return xr_semantic_direct_local_string_value_parameter_is_exact(ctx->semantic, parameter,
                                                                    NULL) &&
           oracle_reference_parameter_storage(ctx, semantic_value, out_storage, out_machine_kind);
}
...
static bool oracle_dynamic_direct_local_reference_result_storage(const VerifyAuthority *ctx,
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ...
            : NULL;
    if (!operation || operation->result_value != semantic_value ||
        !aot_direct_local_reference_result_is_exact(ctx->semantic, operation_index))
        return false;
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, semantic_value);
    const XrTargetMachineRepRecord *register_rep =
        binding ? xr_target_plan_machine_rep(ctx->target_plan, binding->register_rep) : NULL;
...
static bool oracle_dynamic_array_ref_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    bool place_load_value =
        ...
        (oracle_direct_local_tagged_ref_place_storage(ctx, place->value, &place_storage,
                                                      &place_kind) ||
         oracle_direct_local_tagged_ref_parameter_place_storage(ctx, place->value, &place_storage,
                                                                &place_kind)) &&
...
static bool oracle_static_direct_local_callee_storage(const VerifyAuthority *ctx,
    ...
    if (!operation || operation->result_value != semantic_value ||
        !xr_semantic_direct_local_callee_type_is_exact(ctx->semantic, operation, target_function))
        return false;
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, semantic_value);
    const XrTargetMachineRepRecord *register_rep =
        binding ? xr_target_plan_machine_rep(ctx->target_plan, binding->register_rep) : NULL;
...
static bool oracle_static_direct_local_go_callee_storage(const VerifyAuthority *ctx,
    ...
    const XrTargetLayoutRecord *layout =
        operation ? verify_target_layout(ctx, operation->result_type) : NULL;
    if (!operation || !binding || !register_rep || !memory_rep || !slot || !layout ||
        !xr_semantic_direct_local_callee_type_is_exact(ctx->semantic, operation, target) ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        memory_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        register_rep->root_kind != XR_TARGET_ROOT_DYNAMIC ||
        memory_rep->root_kind != XR_TARGET_ROOT_DYNAMIC ||
...
static bool oracle_direct_local_callee_use(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (!operation || !operands ||
        ...
        return false;
    /* The exact callee index has checked every frozen use and target identity.
     * A closed
     * program omits physical call rows for unreachable bodies, while
     * their materialized
     * resolution tokens must still match those semantics. */
    if (!aot_program_function_is_reachable(ctx, operation->function))
        return ctx->call_by_operation[operation_index] == XR_SEMANTIC_INDEX_NONE;
    uint32_t call_index = operation_index < ctx->operation_count
                              ? ctx->call_by_operation[operation_index]
                              : XR_SEMANTIC_INDEX_NONE;
    uint32_t call_count = 0;
    const XrTargetCallRecord *calls = xr_target_plan_calls(ctx->target_plan, &call_count);
    const XrTargetCallRecord *match = calls && call_index < call_count ? &calls[call_index] : NULL;
    uint32_t caller_function = XR_SEMANTIC_INDEX_NONE;
    uint32_t callee_function = XR_SEMANTIC_INDEX_NONE;
    bool exact_shape =
        operation && (operation->opcode == XI_CALL || operation->opcode == XI_TAIL_CALL) &&
        verify_target_function_index(ctx, operation->function, &caller_function) &&
...
static bool oracle_native_yieldable_callee_use(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    const XrSemanticOperationRecord *operation =
        ctx ? xr_semantic_plan_operation(ctx->semantic, operation_index) : NULL;
    uint32_t operand_count = 0u, metadata_count = 0u;
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    const char *const *metadata =
        ctx ? xr_semantic_plan_metadata(ctx->semantic, &metadata_count) : NULL;
    bool fresh_result = operation &&
        xr_semantic_native_yieldable_fresh_result_is_exact(ctx->semantic, operation, NULL);
    if (!ctx || !operation || !operands || !metadata || operand_index != 0u ||
        operation->opcode != XI_CALL || operation->operand_count == 0u ||
        operation->operand_begin >= operand_count ||
        operation->operand_count > operand_count - operation->operand_begin ||
        operands[operation->operand_begin].value != source_value ||
        operands[operation->operand_begin].role != XR_SEM_OPERAND_CALLEE ||
        operands[operation->operand_begin].parameter != -1 ||
        operands[operation->operand_begin].flags != 0u ||
        operands[operation->operand_begin].ownership_action != XR_SEM_OPERAND_BORROW ||
        (!fresh_result && !xr_semantic_native_module_boundary_type_is_exact(
            xr_semantic_plan_type(ctx->semantic, operation->result_type), true)))
        return false;

    uint32_t value = source_value;
    const XrSemanticOperationRecord *import = NULL;
    for (uint32_t depth = 0u; depth < ctx->operation_count; depth++) {
        uint32_t producer_index =
            value < ctx->value_count ? ctx->operation_by_value[value] : XR_SEMANTIC_INDEX_NONE;
        const XrSemanticOperationRecord *producer =
            producer_index < ctx->operation_count
                ? xr_semantic_plan_operation(ctx->semantic, producer_index)
                : NULL;
        if (!producer || producer->function != operation->function)
            return false;
        ...
        if (producer->opcode != XI_COPY || producer->semantic_immediate != XI_COPY_KIND_IDENTITY ||
            producer->operand_count != 1u || producer->operand_begin >= operand_count ||
            producer->result_alias_operand != 0)
            return false;
        value = operands[producer->operand_begin].value;
    }
    if (!import || import->operand_count != 0u || import->metadata_count != 2u ||
        ...
        (import->flags & XI_FLAG_MAY_SUSPEND) != 0u)
        return false;
    const char *module = metadata[import->metadata_begin];
    const char *member = metadata[import->metadata_begin + 1u];
    const XrStdlibDefEntry *entry = xr_stdlib_metadata_unique_func(module, member);
    if (!entry || !entry->signature || !entry->vm || !entry->vm_binding ||
        strcmp(entry->vm_binding, "yieldable") != 0 ||
        operation->operand_count != (uint16_t) (entry->argc + 1u) ||
        !xr_semantic_native_direct_signature_is_exact(
            ctx->semantic, entry, operation, &operands[operation->operand_begin + 1u]) ||
        !xr_semantic_native_direct_function_type_is_exact(
            ctx->semantic, xr_semantic_plan_type(ctx->semantic, import->result_type), operation,
            &operands[operation->operand_begin + 1u]))
        return false;
    for (uint16_t ordinal = 1u; ordinal < operation->operand_count; ordinal++) {
        const XrSemanticOperandRecord *argument = &operands[operation->operand_begin + ordinal];
        if (argument->role != XR_SEM_OPERAND_ARGUMENT ||
            argument->parameter != (int16_t) (ordinal - 1u) ||
            argument->flags != XR_SEM_OPERAND_CALL_CONTRACT ||
    ...
    XrStableId zero = {{0}};
    if (!target || target->kind != XR_SEM_CALL_TARGET_NATIVE_YIELDABLE ||
        target->function != XR_SEMANTIC_INDEX_NONE ||
        target->dependency != XR_SEMANTIC_INDEX_NONE ||
        target->source_export != XR_SEMANTIC_INDEX_NONE ||
        target->callable_type != XR_SEMANTIC_INDEX_NONE ||
    ...
    return verify_target_function_index(ctx, operation->function, &caller_function) &&
           ...
           aot_stable_id_is_zero(call->source_declaration_identity) &&
           aot_stable_id_is_zero(call->source_callee_identity) &&
           aot_stable_id_is_zero(call->native_callee_identity) && call->argument_count == 0u &&
...
static bool oracle_native_module_scalar_callee_use(const VerifyAuthority *ctx,
    ...
    const XrStdlibDefEntry *entry = NULL;
    bool exact_shape =
        ctx && operation && operands && operand_index == 0 && operation->operand_count != 0 &&
        operation->operand_begin < operand_count &&
        operands[operation->operand_begin].value == source_value &&
    ...
    return entry && result_type &&
           verify_target_function_index(ctx, operation->function, &caller_function) &&
           aot_pair_identity("xray-target-native-module-scalar-v1", operation->id, result_type->id,
                             entry->argc, &expected_identity) &&
           xr_stable_id_equal(call->identity, expected_identity) && call->id == call_index &&
           call->semantic_call_target == XR_SEMANTIC_INDEX_NONE &&
           call->semantic_operation == operation_index &&
           call->caller_function == caller_function &&
           ...
           call->source_export == XR_SEMANTIC_INDEX_NONE &&
           aot_stable_id_is_zero(call->source_declaration_identity) &&
           aot_stable_id_is_zero(call->source_callee_identity) && call->argument_count == 0u &&
           call->flags == 0u && call->target_kind == XR_TARGET_CALL_TARGET_NATIVE_MODULE_SCALAR &&
...
static bool oracle_native_direct_candidate(const VerifyAuthority *ctx, uint32_t operation_index) {
    const XrSemanticOperationRecord *operation =
        ctx ? xr_semantic_plan_operation(ctx->semantic, operation_index) : NULL;
    return operation &&
           xr_semantic_native_direct_call_shape_is_exact(ctx->semantic, operation, NULL, NULL);
}

/* The frozen import signature proves that this edge is a resolution token.
 * Target admission independently checks physical call identities and ABI rows;
 * a cold function still has its semantic signature after those rows are pruned. */
static bool oracle_native_direct_callee_use(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    return operation && operands && operand_index == 0 && operation->operand_count != 0 &&
           operation->operand_begin < operand_count &&
           operands[operation->operand_begin].value == source_value &&
           oracle_native_direct_candidate(ctx, operation_index);
}

/* Exact callees and static method receivers are resolution authority. Their
 * frozen identity is consumed by the call projection, so representation does
 * not allocate runtime storage or invent an adapter for these token edges. */
static bool oracle_resolution_only_use(const VerifyAuthority *ctx, uint32_t operation_index,
                                       uint16_t operand_index, uint32_t source_value) {
    const XrSemanticOperationRecord *operation =
        ctx ? xr_semantic_plan_operation(ctx->semantic, operation_index) : NULL;
    uint32_t operand_count = 0;
    ...
    return local ||
           oracle_native_module_scalar_callee_use(ctx, operation_index, operand_index,
                                                  source_value) ||
           oracle_native_yieldable_callee_use(ctx, operation_index, operand_index, source_value) ||
...
static bool oracle_cleanup_local_addr(const VerifyAuthority *ctx, uint32_t operation_index);
...
static bool oracle_raw_pointer_ref_place(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    bool local_place = !parameter && place_operation_index < ctx->operation_count &&
                       oracle_cleanup_local_addr(ctx, place_operation_index);
    if (!parameter_place && !local_place)
        return false;
    XrRep storage = XR_REP_TAGGED;
    uint16_t machine_kind = XR_MACHINE_REP_COUNT;
    if (!(local_place ? oracle_definition_storage(ctx, place->value, &storage, &machine_kind)
                      : oracle_machine_storage(ctx, place->value, &storage, &machine_kind)) ||
        storage != XR_REP_RAWPTR || machine_kind != XR_MACHINE_REP_RAW_PTR)
        return false;
    if (parameter_place) {
        const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, place->value);
        uint32_t slot_count = 0;
        const XrTargetSlotRecord *slots = xr_target_plan_slots(ctx->target_plan, &slot_count);
        const XrTargetSlotRecord *slot =
            binding && slots && binding->slot < slot_count ? &slots[binding->slot] : NULL;
        if (!binding || !slot || slot->id != binding->slot ||
            !verify_target_function_is(ctx, parameter->function, slot->function) ||
            slot->semantic_value != parameter->value ||
            slot->semantic_operation != XR_SEMANTIC_INDEX_NONE ||
            slot->role != XR_TARGET_SLOT_PARAMETER || slot->register_rep != binding->register_rep ||
    ...
    if (operation->opcode == XI_PLACE_LOAD) {
        ...
        if (!oracle_machine_storage(ctx, operation->result_value, &result_storage, &result_kind) &&
            !oracle_dynamic_value_storage(ctx, operation->result_value, &result_storage, &result_kind))
            return false;
    } else {
...
static bool oracle_resolve_identity_rename(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    for (;;) {
        ...
        bool owner_forward =
            operation && xr_semantic_owner_transfer_is_exact(ctx->semantic, operation, NULL);
...
/* Cleanup captures borrow a local's independently proved storage. The address
 * itself has a separate raw-pointer binding and never acquires ownership of
 * the subject, whether that subject is scalar or managed. */
static bool oracle_cleanup_local_addr(const VerifyAuthority *ctx, uint32_t operation_index) {
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx ? ctx->semantic : NULL, operation_index);
    ...
    if (!oracle_definition_storage(ctx, source->value, &source_storage, &source_kind))
        return false;
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, operation->result_value);
    const XrTargetMachineRepRecord *reg = binding
        ? xr_target_plan_machine_rep(ctx->target_plan, binding->register_rep) : NULL;
    const XrTargetMachineRepRecord *mem = binding
        ? xr_target_plan_machine_rep(ctx->target_plan, binding->memory_rep) : NULL;
    if (!reg || !mem || reg->kind != XR_MACHINE_REP_RAW_PTR ||
        mem->kind != XR_MACHINE_REP_RAW_PTR || reg->root_kind != XR_TARGET_ROOT_NONE ||
        mem->root_kind != XR_TARGET_ROOT_NONE || reg->ownership != XR_TARGET_OWNERSHIP_TRIVIAL ||
        mem->ownership != XR_TARGET_OWNERSHIP_TRIVIAL)
        return false;
    uint32_t slot_count = 0;
    const XrTargetSlotRecord *slots = xr_target_plan_slots(ctx->target_plan, &slot_count);
    const XrTargetSlotRecord *slot =
        binding && slots && binding->slot < slot_count ? &slots[binding->slot] : NULL;
    return binding && slot && slot->id == binding->slot &&
           verify_target_function_is(ctx, operation->function, slot->function) &&
...
static bool oracle_definition_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    if (ctx->parameter_by_value[semantic_value] != XR_SEMANTIC_INDEX_NONE) {
        const XrSemanticParameterRecord *parameter = xr_semantic_plan_parameter(
            ctx->semantic, ctx->parameter_by_value[semantic_value]);
        if (parameter && (parameter->flags & XR_SEM_PARAMETER_DEFAULT_SENTINEL) != 0) {
            *out_storage = XR_REP_TAGGED;
            *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
            return true;
        }
         ...
         * own. */
        if (oracle_direct_local_tagged_ref_parameter_place_storage(ctx, semantic_value, out_storage,
                                                                   out_machine_kind))
            return true;
        if (oracle_reference_parameter_storage(ctx, semantic_value, out_storage, out_machine_kind))
            return true;
    ...
    const char *native_module_path = NULL;
    bool native_namespace_candidate =
        (operation->opcode == XI_IMPORT_REF &&
         xr_semantic_native_module_import_is_exact(ctx->semantic, operation,
                                                   &native_module_path)) ||
        (operation->opcode == XI_GET_SHARED &&
         (native_module_path =
              xr_semantic_native_module_namespace_path(ctx->semantic, semantic_value)) != NULL);
    if (native_namespace_candidate)
        return ctx->exact_native_module_namespace_value &&
               ctx->exact_native_module_namespace_value[semantic_value] &&
               oracle_native_module_namespace_storage(ctx, semantic_value, out_storage,
                                                      out_machine_kind);
    if (oracle_dynamic_json_namespace_codec_storage(ctx, semantic_value, out_storage,
                                                    out_machine_kind))
        return true;
    if (oracle_dynamic_call_result_storage(ctx, semantic_value, out_storage, out_machine_kind))
        return true;
    ...
    switch (operation->opcode) {
        ...
        case XI_ERR_CATCH:
            ...
            break;
        case XI_LOAD_FIELD:
            if (oracle_source_class_field_read_storage(ctx, semantic_value, out_storage,
                                                       out_machine_kind) ||
        ...
        case XI_CHAN_TRY_RECV:
            ...
            {
                const XrTargetValueRepRecord *binding =
                    verify_target_value_rep(ctx, semantic_value);
                const XrTargetMachineRepRecord *machine =
        ...
        case XI_GET_BUILTIN:
            if (aot_json_namespace_global_is_exact(ctx->semantic, operation) ||
                xr_semantic_builtin_enum_namespace_is_exact(ctx->semantic, operation) ||
            ...
            break;
        case XI_INDEX_GET:
            if (xr_semantic_builtin_unit_enum_member_access_is_exact(ctx->semantic, operation,
                                                                      NULL, NULL, NULL))
                return oracle_machine_storage(ctx, semantic_value, out_storage, out_machine_kind);
        ...
        case XI_LOCAL_ADDR: {
            ...
            if (oracle_direct_local_tagged_ref_place_storage(ctx, semantic_value, out_storage,
                                                             out_machine_kind))
                return true;
            if (oracle_cleanup_local_addr(ctx, operation_index)) {
                *out_storage = XR_REP_RAWPTR;
                *out_machine_kind = XR_MACHINE_REP_RAW_PTR;
                return true;
            }
        ...
        case XI_VARIANT_PROJECT:
            /* A unit enum has one native ordinal carrier: projecting it out of
             * a nullable value reads the ordinal straight into that frozen
             * machine storage, the same carrier every other definition of the
             * enum uses. Any other payload is handed out as its stored XrValue
             * carrier, and scalar consumers unbox from this explicit boundary. */
            if (xr_semantic_unit_enum_type_is_exact(
                    xr_semantic_plan_type(ctx->semantic, operation->result_type)) &&
                oracle_machine_storage(ctx, semantic_value, out_storage, out_machine_kind))
                return *out_storage == XR_REP_I64 &&
                       *out_machine_kind == XR_MACHINE_REP_ENUM_ORDINAL;
            *out_storage = XR_REP_TAGGED;
            *out_machine_kind = XR_MACHINE_REP_DYN_VALUE;
            return true;
        case XI_OBJECT_NEW:
        ...
        case XI_CONST:
            ...
            break;
        case XI_CALL_METHOD:
            if (xr_semantic_builtin_runtime_method_has_result_class(
                    ctx->semantic, operation, XR_SEM_BUILTIN_RUNTIME_METHOD_RESULT_SCALAR))
                return oracle_machine_storage(ctx, semantic_value, out_storage, out_machine_kind);
            if (xr_semantic_builtin_runtime_method_has_result_class(
                    ctx->semantic, operation, XR_SEM_BUILTIN_RUNTIME_METHOD_RESULT_OWNED_DYNAMIC))
            ...
            if (xr_semantic_rune_to_uint32_is_exact(ctx->semantic, operation, NULL))
                return oracle_rune_to_uint32_call(ctx, semantic_value, out_storage,
                                                  out_machine_kind);
            if (xr_semantic_rune_to_string_is_exact(ctx->semantic, operation, NULL))
                return oracle_rune_to_string_call(ctx, semantic_value, out_storage,
                                                  out_machine_kind);
        ...
        case XI_TAIL_CALL:
            ...
            if (oracle_dynamic_direct_local_string_result_storage(ctx, semantic_value, out_storage,
                                                                  out_machine_kind))
                return true;
            if (oracle_dynamic_direct_local_reference_result_storage(ctx, semantic_value,
                                                                     out_storage, out_machine_kind))
                return true;
            if (oracle_adt_enum_operation_storage(ctx, semantic_value, out_storage,
                                                  out_machine_kind))
...
/* A local ref address is a storage fact of this semantic module, even when
 * executable call rows have been removed from a cold body. The independent
 * Target admission owns physical argument mapping; this proof joins the
 * frozen source operand to its declaration without scanning other modules. */
static bool oracle_direct_local_tagged_ref_place_use(const VerifyAuthority *ctx,
    ...
    if (!xr_semantic_ref_argument_local_addr_is_exact(
            ctx->semantic, place, place ? place->result_type : XR_SEMANTIC_INDEX_NONE, &source) ||
        source->value != source_value ||
        !aot_direct_ref_subject_type_is_exact(ctx->semantic, place->result_type, &storage))
        return false;
    XrRep caller_storage = XR_REP_VOID;
    uint16_t caller_kind = XR_MACHINE_REP_COUNT;
    if (!oracle_definition_storage(ctx, source_value, &caller_storage, &caller_kind) ||
        caller_storage != XR_REP_TAGGED || caller_kind != XR_MACHINE_REP_DYN_VALUE)
        return false;
    if (xr_semantic_native_direct_ref_address_is_exact(ctx->semantic, place))
        return true;
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx->semantic, &operand_count);
    uint32_t target_count = (uint32_t) xr_semantic_plan_call_target_count(ctx->semantic);
    for (uint32_t i = 0; operands && i < target_count; i++) {
        const XrSemanticCallTargetRecord *target = xr_semantic_plan_call_target(ctx->semantic, i);
        if (target && target->kind == XR_SEM_CALL_TARGET_SOURCE_EXPORT &&
            xr_semantic_source_export_ref_address_is_exact(
                ctx->semantic, verify_semantic_dependency(ctx, target->dependency), target, place))
            return true;
        const XrSemanticOperationRecord *call =
            target ? xr_semantic_plan_operation(ctx->semantic, target->operation) : NULL;
        const XrSemanticFunctionRecord *callee =
            target ? xr_semantic_plan_function(ctx->semantic, target->function) : NULL;
        uint32_t shift = xr_semantic_local_call_operand_shift(target);
        if (!target || !call || !callee ||
            call->function != place->function ||
            !xr_semantic_call_target_names_local_function(target, call, ctx->function_count) ||
        ...
        for (uint16_t ordinal = shift == 0u ? 1u : 0u;
            ...
            const XrSemanticParameterRecord *parameter =
                xr_semantic_plan_parameter(ctx->semantic, callee->parameter_begin + ordinal);
            const XrSemanticOperandRecord *argument = &operands[call->operand_begin + ordinal + shift];
            XrRep callee_storage = XR_REP_VOID;
            uint16_t callee_kind = XR_MACHINE_REP_COUNT;
            if (!parameter || parameter->function != target->function ||
                parameter->ordinal != ordinal || parameter->type != place->result_type ||
                argument->role != XR_SEM_OPERAND_ARGUMENT ||
                argument->parameter != (int16_t) (ordinal + shift - 1u) ||
                argument->value != place->result_value || argument->type != place->result_type ||
                argument->parameter_mode != XR_PARAM_REF || argument->access != XR_CALL_ARG_REF ||
                ...
                argument->flags != (XR_SEM_OPERAND_CALL_CONTRACT | XR_SEM_OPERAND_ADDRESSABLE) ||
                !oracle_direct_local_tagged_ref_parameter_place_storage(
...
/* Cold local bodies retain declaration and parameter-slot authority even when
 * their executable call rows are absent from the reachable TargetPlan. */
static bool oracle_local_tagged_ref_call_use(const VerifyAuthority *ctx,
                                             uint32_t operation_index, uint16_t operand_index,
    ...
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx->semantic, &operand_count);
    if (!call || !operands || operand_index >= call->operand_count ||
        call->operand_begin > operand_count ||
        call->operand_count > operand_count - call->operand_begin)
        return false;
    const XrSemanticOperandRecord *argument = &operands[call->operand_begin + operand_index];
    if (argument->role != XR_SEM_OPERAND_ARGUMENT || argument->parameter_mode != XR_PARAM_REF ||
        argument->access != XR_CALL_ARG_REF || argument->value != source_value)
        return false;
    uint32_t count = (uint32_t) xr_semantic_plan_call_target_count(ctx->semantic);
    for (uint32_t i = 0; i < count; i++) {
        const XrSemanticCallTargetRecord *target = xr_semantic_plan_call_target(ctx->semantic, i);
        if (!target || target->operation != operation_index ||
            !xr_semantic_call_target_names_local_function(target, call, ctx->function_count))
        ...
        uint32_t shift = xr_semantic_local_call_operand_shift(target);
        const XrSemanticFunctionRecord *callee =
            xr_semantic_plan_function(ctx->semantic, target->function);
        if (!callee || operand_index == 0 ||
            call->operand_count != (uint32_t) callee->parameter_count + shift)
            return false;
        uint32_t ordinal = operand_index - shift;
        const XrSemanticParameterRecord *parameter =
            xr_semantic_plan_parameter(ctx->semantic, callee->parameter_begin + ordinal);
        XrRep storage = XR_REP_VOID;
        uint16_t kind = XR_MACHINE_REP_COUNT;
        if (!parameter || parameter->function != target->function || parameter->ordinal != ordinal ||
            parameter->type != argument->type || argument->value != source_value ||
            argument->role != XR_SEM_OPERAND_ARGUMENT ||
            argument->parameter != (int16_t) (operand_index - 1u) ||
            argument->parameter_mode != XR_PARAM_REF || argument->access != XR_CALL_ARG_REF ||
            argument->origin == XI_PLACE_ORIGIN_NONE ||
            argument->lifetime != XI_PLACE_LIFETIME_CALL_BOUND ||
            argument->escape != XI_PLACE_ESCAPE_NONE ||
            ...
            argument->flags != (XR_SEM_OPERAND_CALL_CONTRACT | XR_SEM_OPERAND_ADDRESSABLE) ||
            !oracle_direct_local_tagged_ref_parameter_place_storage(
                ctx, parameter->value, &storage, &kind) ||
            storage != XR_REP_RAWPTR || kind != XR_MACHINE_REP_RAW_PTR ||
            (!oracle_direct_local_tagged_ref_place_storage(ctx, source_value, &storage, &kind) &&
             !oracle_direct_local_tagged_ref_parameter_place_storage(
                 ctx, source_value, &storage, &kind)) ||
            storage != XR_REP_RAWPTR || kind != XR_MACHINE_REP_RAW_PTR)
            return false;
        *out_storage = XR_REP_RAWPTR;
        return true;
...
/* Cross-module ref calls consume the same proven caller address as local
 * calls. Join the operand to its physical reference argument; a tagged value
 * or an ordinary READ argument cannot acquire address authority here. */
static bool oracle_source_export_ref_call_use(const VerifyAuthority *ctx,
                                               uint32_t operation_index, uint16_t operand_index,
                                               uint32_t source_value, XrRep *out_storage) {
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, operation_index);
    uint32_t operand_count = 0, call_count = 0, argument_count = 0;
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx->semantic, &operand_count);
    const XrTargetCallRecord *calls = xr_target_plan_calls(ctx->target_plan, &call_count);
    const XrTargetCallArgumentRecord *arguments =
    ...
    uint32_t call_index = operation_index < ctx->operation_count
        ? ctx->call_by_operation[operation_index] : XR_SEMANTIC_INDEX_NONE;
    const XrTargetCallRecord *call = calls && call_index < call_count ? &calls[call_index] : NULL;
    if (!operation || !operands || !arguments || !call || operand_index == 0 ||
        operand_index >= operation->operand_count || operation->operand_begin > operand_count ||
        operation->operand_count > operand_count - operation->operand_begin ||
        call->semantic_operation != operation_index ||
    ...
    const XrSemanticCallTargetRecord *target =
        xr_semantic_plan_call_target(ctx->semantic, call->semantic_call_target);
    const XrSemanticPlan *dependency = target ? verify_semantic_dependency(ctx, target->dependency) : NULL;
    uint32_t function = xr_semantic_source_dependency_call_function(ctx->semantic, target, dependency);
    const XrSemanticFunctionRecord *callee = xr_semantic_plan_function(dependency, function);
    const XrSemanticParameterRecord *parameter = callee && operand_index <= callee->parameter_count
        ? xr_semantic_plan_parameter(dependency, callee->parameter_begin + operand_index - 1u) : NULL;
    if (!target || !callee || !parameter || target->kind != XR_SEM_CALL_TARGET_SOURCE_EXPORT ||
        target->operation != operation_index || parameter->mode != XR_PARAM_REF ||
        call->source_dependency != target->dependency || call->source_export != target->source_export ||
        !xr_stable_id_equal(call->source_declaration_identity, target->export_identity) ||
        !xr_stable_id_equal(call->source_callee_identity, callee->id) ||
        !verify_target_function_is(ctx, operation->function, call->caller_function))
        return false;
    const XrSemanticOperandRecord *operand = &operands[operation->operand_begin + operand_index];
    const XrTargetCallArgumentRecord *argument =
        &arguments[call->argument_begin + operand_index - 1u];
    const XrTargetValueRepRecord *binding = verify_target_value_rep(ctx, source_value);
    ...
    uint16_t kind = XR_MACHINE_REP_COUNT;
    XrRep storage = XR_REP_VOID;
    if (!binding || !callee_register || !callee_memory ||
        operand->value != source_value || operand->role != XR_SEM_OPERAND_ARGUMENT ||
        operand->parameter != (int16_t) (operand_index - 1u) ||
        operand->parameter_mode != XR_PARAM_REF || operand->access != XR_CALL_ARG_REF ||
        operand->ownership_action != XR_SEM_OPERAND_BORROW ||
        operand->lifetime != XI_PLACE_LIFETIME_CALL_BOUND ||
        operand->escape != XI_PLACE_ESCAPE_NONE || operand->transfer_mode != XR_TRANSFER_SHARE ||
        operand->flags != (XR_SEM_OPERAND_CALL_CONTRACT | XR_SEM_OPERAND_ADDRESSABLE) ||
        argument->call != call->id || argument->ordinal != operand_index - 1u ||
        argument->semantic_operand != operation->operand_begin + operand_index ||
        argument->semantic_value != source_value || argument->caller_slot != binding->slot ||
        argument->register_rep != binding->register_rep ||
        argument->memory_rep != binding->memory_rep ||
        argument->mode != XR_TARGET_CALL_REFERENCE || argument->ownership != XR_TARGET_CALL_WRITEBACK ||
        callee_register->kind != XR_MACHINE_REP_RAW_PTR ||
        callee_memory->kind != XR_MACHINE_REP_RAW_PTR ||
        (!oracle_direct_local_tagged_ref_place_storage(ctx, source_value, &storage, &kind) &&
         !oracle_direct_local_tagged_ref_parameter_place_storage(ctx, source_value, &storage, &kind)) ||
        storage != XR_REP_RAWPTR || kind != XR_MACHINE_REP_RAW_PTR)
        return false;
    *out_storage = XR_REP_RAWPTR;
    return true;
...
/* A view retains its published pointer/length carrier at element accesses.
 * Element type and mutability are checked independently of whether the view
 * came from a parameter, a range, or a runtime operation. */
static bool oracle_slice_element_access_is_exact(const VerifyAuthority *ctx,
                                                 uint32_t operation_index) {
    const XrSemanticOperationRecord *operation =
        ctx ? xr_semantic_plan_operation(ctx->semantic, operation_index) : NULL;
    uint32_t operand_count = 0;
    ...
    if (view->role != XR_SEM_OPERAND_VALUE || view->parameter != -1 ||
        ...
        !oracle_slice_view_storage(ctx, view->value, &view_storage, &view_kind) ||
...
static bool oracle_array_like_receiver_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                               XrRep *out_storage, uint16_t *out_machine_kind) {
    return oracle_slice_view_storage(ctx, semantic_value, out_storage, out_machine_kind) ||
...
static bool oracle_leaf_direct_local_aggregate_argument_use(const VerifyAuthority *ctx,
                                                            uint32_t operation_index,
    ...
    const XrSemanticProgramCallBinding *program_call =
        operation ? xr_semantic_plan_program_call_for_operation(ctx->semantic, operation_index)
                  : NULL;
    const XrSemanticProgramFunctionBinding *caller_binding =
        operation ? xr_semantic_plan_program_function_for_semantic_function(ctx->semantic,
                                                                            operation->function)
                  : NULL;
    const XrSemanticProgramFunctionBinding *callee_binding =
        program_call ? xr_semantic_plan_program_function_for_semantic_function(
                           ctx->semantic, program_call->target_function)
                     : NULL;
    const XrSemanticFunctionRecord *callee =
        program_call ? xr_semantic_plan_function(ctx->semantic, program_call->target_function)
                     : NULL;
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    ...
    uint32_t call_index = ctx && operation_index < ctx->operation_count
                              ? ctx->call_by_operation[operation_index]
                              : XR_SEMANTIC_INDEX_NONE;
    if (!ctx || !out_storage || !provenance || !operation || !program_call || !caller_binding ||
        !callee_binding || !callee || !operands || !calls || !arguments ||
        provenance->program_family != XR_PROGRAM_SEMANTIC_FAMILY_LEAF_VALUE_AGGREGATE_DIRECT_CALL ||
        operation->opcode != XI_CALL || operand_index != 1 || operation->operand_count != 2 ||
        operation->operand_begin > operand_count ||
        operation->operand_count > operand_count - operation->operand_begin ||
        call_index == XR_SEMANTIC_INDEX_NONE || call_index >= call_count ||
        program_call->operation != operation_index ||
        program_call->target_function != callee_binding->semantic_function ||
        ...
        return false;
    const XrSemanticOperandRecord *operand = &operands[operation->operand_begin + operand_index];
    uint32_t parameter_index = callee->parameter_begin;
    const XrSemanticParameterRecord *parameter =
        xr_semantic_plan_parameter(ctx->semantic, parameter_index);
    const XrTargetCallRecord *call = &calls[call_index];
    if (!parameter || operand->value != source_value || operand->role != XR_SEM_OPERAND_ARGUMENT ||
        operand->parameter != 0 || operand->type != parameter->type ||
        ...
        operand->escape != XI_PLACE_ESCAPE_NONE || operand->flags != XR_SEM_OPERAND_CALL_CONTRACT ||
        parameter->function != program_call->target_function || parameter->ordinal != 0 ||
        parameter->mode != XR_PARAM_READ || parameter->ownership != XI_OWN_NONE ||
        parameter->transfer_mode != XR_TRANSFER_SHARE || call->id != call_index ||
        call->semantic_operation != operation_index ||
        !verify_target_function_is(ctx, operation->function, call->caller_function) ||
        call->callee_function != program_call->target_function ||
        call->target_kind != XR_TARGET_CALL_TARGET_DIRECT_LOCAL ||
        call->calling_convention != XR_TARGET_CALL_CONVENTION_DIRECT_LOCAL ||
        call->argument_count != 1 || call->adapter_count != 0 ||
        call->result_mode != XR_TARGET_CALL_CALLER_STORAGE ||
        call->result_ownership != XR_TARGET_CALL_NONE || call->argument_begin >= argument_count)
        return false;
    const XrTargetCallArgumentRecord *argument = &arguments[call->argument_begin];
    const XrTargetValueRepRecord *caller_value = verify_target_value_rep(ctx, source_value);
    const XrTargetValueRepRecord *callee_value = verify_target_value_rep(ctx, parameter->value);
    XrRep caller_storage = XR_REP_COUNT;
    XrRep callee_storage = XR_REP_COUNT;
    uint16_t caller_kind = XR_MACHINE_REP_COUNT;
    uint16_t callee_kind = XR_MACHINE_REP_COUNT;
    if (!caller_value || !callee_value ||
        !oracle_value_aggregate_storage(ctx, source_value, &caller_storage, &caller_kind) ||
        !oracle_value_aggregate_storage(ctx, parameter->value, &callee_storage, &callee_kind) ||
        caller_storage != XR_REP_TAGGED || callee_storage != caller_storage ||
        caller_kind != XR_MACHINE_REP_AGGREGATE || callee_kind != caller_kind ||
        argument->call != call->id ||
        argument->semantic_operand != operation->operand_begin + operand_index ||
        argument->semantic_value != source_value || argument->callee_parameter != parameter_index ||
        ...
        argument->memory_rep != argument->callee_memory_rep || argument->ordinal != 0 ||
...
/* Collector owner for the scalar index lane of an INDEX_SET.  The receiver and
 * stored element are proved by their existing authorities; this row answers
 * only whether the consumed index stays in the exact I64 storage already
 * frozen for that stable semantic value.  Consequently it does not infer
 * storage from the opcode or from an array-origin spelling. */
static bool collect_index_set_i64_identity_use_is_exact(const CollectContext *ctx,
                                                        uint32_t operation_index,
                                                        uint16_t operand_index,
                                                        uint32_t source_value) {
    const XrSemanticOperationRecord *operation =
    ...
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    if (!ctx || !operation || !operands || operation->opcode != XI_INDEX_SET ||
        operand_index != 1u || operation->operand_count != 3u ||
        operation->operand_begin > operand_count ||
        operation->operand_count > operand_count - operation->operand_begin ||
        operation->auxiliary_kind != 0 || operation->metadata_count != 0 ||
        (operation->semantic_immediate & ~((int64_t) XI_ACCESS_UNCHECKED | INT64_C(1))) != 0 ||
        operation->constant != XR_SEMANTIC_INDEX_NONE ||
        operation->intrinsic_kind != XR_SEM_INTRINSIC_NONE || operation->allocation_key != NULL ||
        operation->effects != xi_generated_op_effects(XI_INDEX_SET) ||
        operation->result_ownership != xi_generated_op_result_ownership(XI_INDEX_SET) ||
        operation->result_alias_operand != -1 || operation->return_parameter != -1)
        return false;
    const XrSemanticOperandRecord *operand = &operands[operation->operand_begin + operand_index];
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, operand->type);
    uint32_t definition = source_value < ctx->semantic_value_count
                              ? ctx->operation_by_value[source_value]
                              : XR_SEMANTIC_INDEX_NONE;
    uint32_t parameter = source_value < ctx->semantic_value_count
                             ? ctx->parameter_by_value[source_value]
                             : XR_SEMANTIC_INDEX_NONE;
    const XrSemanticOperationRecord *source_operation =
        definition != XR_SEMANTIC_INDEX_NONE ? xr_semantic_plan_operation(ctx->semantic, definition)
                                             : NULL;
    const XrSemanticParameterRecord *source_parameter =
        parameter != XR_SEMANTIC_INDEX_NONE ? xr_semantic_plan_parameter(ctx->semantic, parameter)
                                            : NULL;
    const XrSemanticFunctionRecord *function =
        xr_semantic_plan_function(ctx->semantic, operation->function);
    const XrTargetValueRepRecord *binding = collect_target_value_rep(ctx, source_value);
    ...
    const XrTargetMachineRepRecord *memory_rep =
        binding ? xr_target_plan_machine_rep(ctx->target_plan, binding->memory_rep) : NULL;
    XrStableId source_id = source_parameter   ? source_parameter->id
                           : source_operation ? source_operation->id
                                              : (XrStableId) {{0}};
    return operand->value == source_value && operand->role == XR_SEM_OPERAND_VALUE &&
           operand->parameter == -1 && operand->flags == 0 &&
           operand->ownership_action == XR_SEM_OPERAND_CONSUME && semantic_exact_i64_type(type) &&
           function && !aot_stable_id_is_zero(function->id) &&
           !aot_stable_id_is_zero(operation->id) && !aot_stable_id_is_zero(source_id) &&
           ((source_operation && source_operation->function == operation->function &&
             source_operation->result_value == source_value &&
             source_operation->result_type == operand->type) ||
            (source_parameter && source_parameter->function == operation->function &&
             source_parameter->value == source_value && source_parameter->type == operand->type)) &&
           binding && register_rep && memory_rep && binding->semantic_value == source_value &&
...
/* Independent replay of the same contract.  This verifier intentionally does
 * not call the collector predicate above (nor any array-origin oracle): it
 * rebuilds the semantic/source/function identities and TargetPlan join from
 * the immutable rows it is checking. */
static bool verify_index_set_i64_identity_use_is_exact(const VerifyAuthority *ctx,
                                                       uint32_t operation_index,
                                                       uint16_t operand_index,
                                                       uint32_t source_value) {
    const XrSemanticOperationRecord *operation =
    ...
    const XrSemanticOperandRecord *operand = &operands[operation->operand_begin + operand_index];
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, operand->type);
    uint32_t definition = source_value < ctx->value_count ? ctx->operation_by_value[source_value]
                                                          : XR_SEMANTIC_INDEX_NONE;
    uint32_t parameter = source_value < ctx->value_count ? ctx->parameter_by_value[source_value]
                                                         : XR_SEMANTIC_INDEX_NONE;
    const XrSemanticOperationRecord *source_operation =
        definition != XR_SEMANTIC_INDEX_NONE ? xr_semantic_plan_operation(ctx->semantic, definition)
                                             : NULL;
...
/* The exact generated native-direct signature accepts tagged value arguments.
 * Physical row identities, ownership and ABI mapping are checked by Target
 * admission once; representation asks only which carrier this edge consumes. */
static bool oracle_native_direct_argument_storage(const VerifyAuthority *ctx,
    ...
    const XrSemanticOperandRecord *operands =
        ctx ? xr_semantic_plan_operands(ctx->semantic, &operand_count) : NULL;
    if (!operation || !operands || !out_storage || operand_index == 0 ||
        operand_index >= operation->operand_count || operation->operand_begin > operand_count ||
        operation->operand_count > operand_count - operation->operand_begin ||
        operands[operation->operand_begin + operand_index].value != source_value ||
        !oracle_native_direct_candidate(ctx, operation_index))
        return false;
    *out_storage = operands[operation->operand_begin + operand_index].parameter_mode == XR_PARAM_REF
                       ? XR_REP_RAWPTR
                       : XR_REP_TAGGED;
    return true;
}

/* Imported class parameters keep their stable declaration identity instead of
 * an index into the
 * current module. Resolve that identity against the frozen
 * program modules; the store's operand
 * shape and parameter binding retain the
 * same borrow/consume obligations as a local class. Field
 * selection remains
 * the frontend's typed layout fact, independent of this carrier proof. */
static bool oracle_class_field_store_receiver_is_exact(const VerifyAuthority *ctx,
                                                       const XrSemanticOperationRecord *operation) {
    const XrSemanticOperandRecord *receiver =
        xr_semantic_class_field_store_receiver_is_exact(ctx->semantic, operation);
    if (!receiver)
        return false;
    if (xr_semantic_class_receiver_parameter_source_class(ctx->semantic, operation->function,
    ...
    const XrSemanticOperationRecord *definition = xr_semantic_plan_operation(
        ctx->semantic, ctx->operation_by_value[receiver->value]);
    XrRep storage = XR_REP_VOID;
    uint16_t machine_kind = XR_MACHINE_REP_COUNT;
    if (definition && definition->function == operation->function &&
        definition->result_value == receiver->value && definition->result_type == receiver->type &&
        xr_semantic_class_instance_type_source_class(
            ctx->semantic, xr_semantic_plan_type(ctx->semantic, receiver->type)) !=
            XR_SEMANTIC_INDEX_NONE &&
        oracle_definition_storage(ctx, receiver->value, &storage, &machine_kind) &&
        storage == XR_REP_TAGGED && machine_kind == XR_MACHINE_REP_DYN_VALUE)
        return true;
    uint32_t parameter_index = ctx->parameter_by_value[receiver->value];
    const XrSemanticParameterRecord *parameter =
        xr_semantic_plan_parameter(ctx->semantic, parameter_index);
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, receiver->type);
    if (!parameter || parameter->function != operation->function ||
        parameter->type != receiver->type ||
        !xr_semantic_class_argument_parameter_is_exact(ctx->semantic, parameter_index) ||
        !xr_semantic_external_class_instance_type_is_exact(type))
        return false;
    return oracle_external_class_type_is_declared(ctx, type);
}

/* Channel consumers read the exact declared Channel through its proven tagged
 * carrier, regardless of whether construction, a parameter or a field supplied it. */
static bool oracle_channel_operand_storage(const VerifyAuthority *ctx,
    const XrSemanticOperationRecord *operation, uint16_t operand_index,
    uint32_t source_value, XrRep *out_storage) {
    uint32_t count = 0u;
    const XrSemanticOperandRecord *operands = xr_semantic_plan_operands(ctx->semantic, &count);
    if (!operation || !operands || operand_index >= operation->operand_count ||
    ...
    return operand->value == source_value && operand->role == XR_SEM_OPERAND_VALUE &&
           operand->ownership_action == XR_SEM_OPERAND_BORROW &&
           xr_semantic_channel_type_row_is_exact(ctx->semantic,
               xr_semantic_plan_type(ctx->semantic, operand->type)) &&
           oracle_definition_storage(ctx, source_value, out_storage, &kind) &&
           *out_storage == XR_REP_TAGGED && kind == XR_MACHINE_REP_DYN_VALUE;
}
...
static bool oracle_use_storage(const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (overflow_match == AOT_I64_OVERFLOW_PREDICATE_EXACT) {
        ...
        uint32_t expected_slot =
            operand_index == 0 ? overflow->receiver_slot : overflow->argument_slot;
    ...
    if (oracle_slice_view_storage(ctx, source_value, out_storage, &ignored_kind))
        return true;
    switch (operation->opcode) {
        case XI_SUM_INJECT: {
            uint32_t operand_count = 0;
            ...
            if (!operands || operation->operand_begin >= operand_count)
                return false;
            const XrSemanticOperandRecord *operand = &operands[operation->operand_begin];
            if (operand->value != source_value)
                return false;
            const XrSemanticTypeRecord *result_type =
                xr_semantic_plan_type(ctx->semantic, operation->result_type);
            if (!aot_nullable_scalar_type_is_exact(result_type))
                break;
            const XrSemanticTypeRecord *payload_type =
                xr_semantic_plan_type(ctx->semantic, operand->type);
            XrRep result_storage = XR_REP_COUNT, payload_storage = XR_REP_COUNT;
            uint16_t result_kind = XR_MACHINE_REP_COUNT, payload_kind = XR_MACHINE_REP_COUNT;
            if (operand_index != 0 || operation->operand_count != 1 ||
                operation->semantic_immediate != 1 || operation->auxiliary_kind != XI_AUX_KIND_NONE ||
                operation->metadata_count != 0 || operation->constant != XR_SEMANTIC_INDEX_NONE ||
                operation->allocation_key || operation->result_ownership != xi_generated_op_result_ownership(XI_SUM_INJECT) ||
                operation->result_alias_operand != -1 || operation->return_parameter != -1 ||
                operation->effects != xi_generated_op_effects(XI_SUM_INJECT) ||
                operation->flags != xi_generated_op_default_flags(XI_SUM_INJECT) ||
                !payload_type || !result_type || !aot_nullable_scalar_type_is_exact(result_type) ||
                payload_type->flags != (result_type->flags & (uint8_t) ~XR_SEM_TYPE_NULLABLE) ||
                !aot_canonical_key_differs_only_by_nullable(payload_type->canonical_key,
                                                           result_type->canonical_key) ||
                !xr_semantic_type_same_structure(ctx->semantic, payload_type, result_type) ||
                operand->role != XR_SEM_OPERAND_VALUE || operand->parameter != -1 ||
                operand->flags != 0 || operand->access != XR_CALL_ARG_PLAIN ||
                !oracle_machine_storage(ctx, source_value, &payload_storage, &payload_kind) ||
                !oracle_nullable_scalar_storage(ctx, operation->result_value,
                                                 &result_storage, &result_kind) ||
                result_storage != XR_REP_TAGGED || result_kind != XR_MACHINE_REP_DYN_VALUE)
                return false;
            *out_storage = XR_REP_TAGGED;
            return true;
        }
        ...
        case XI_PLACE_LOAD:
            ...
            }
            {
                ...
                const XrSemanticParameterRecord *source_parameter =
                    source_parameter_index != XR_SEMANTIC_INDEX_NONE
                        ? xr_semantic_plan_parameter(ctx->semantic, source_parameter_index)
                        : NULL;
                const XrSemanticOperationRecord *source_operation =
                    source_operation_index != XR_SEMANTIC_INDEX_NONE
                        ? xr_semantic_plan_operation(ctx->semantic, source_operation_index)
                        : NULL;
                uint32_t source_type = source_parameter   ? source_parameter->type
                                       : source_operation ? source_operation->result_type
                                                          : XR_SEMANTIC_INDEX_NONE;
                XrRep source_storage = XR_REP_COUNT;
                XrRep result_storage = XR_REP_COUNT;
                uint16_t source_kind = XR_MACHINE_REP_COUNT;
                uint16_t result_kind = XR_MACHINE_REP_COUNT;
                if (source_type == operation->result_type &&
                    oracle_value_aggregate_storage(ctx, source_value, &source_storage,
                                                   &source_kind) &&
                    oracle_value_aggregate_storage(ctx, operation->result_value, &result_storage,
                                                   &result_kind) &&
                    source_storage == XR_REP_TAGGED && result_storage == source_storage &&
                    source_kind == XR_MACHINE_REP_AGGREGATE && result_kind == source_kind) {
                    *out_storage = XR_REP_TAGGED;
                    return true;
                }
            }
        ...
        case XI_CALL_BUILTIN: {
            ...
            if (operand_index == 0 &&
                ...
                /* Exact copy admission already owns the container type. Reuse the source's
                 * proved carrier so object and array roots obey the same storage rule. */
                if (oracle_tagged_reference_carrier_storage(ctx, source_value, out_storage,
                                                            &ignored_kind))
                    return true;
                return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind);
            }
        ...
        case XI_ERR_RETURN: {
            ...
            const XrSemanticOperandRecord *operands =
                xr_semantic_plan_operands(ctx->semantic, &operand_count);
            const XrSemanticOperandRecord *operand = operands &&
                operation->operand_begin < operand_count &&
                operand_index < operand_count - operation->operand_begin
                ? operands + operation->operand_begin + operand_index : NULL;
            if (operand && operand->value == source_value &&
        ...
        case XI_GO: {
            ...
            if (!operands || operation->operand_begin > operand_count ||
                ...
                !oracle_direct_local_go_callee_use(
                    ctx, operation_index, 0u, operands[operation->operand_begin].value))
                return false;
            /* The sealed spawn already joins each argument to its parameter.
             * Managed producers retain their proved tagged carrier at the
             * transfer boundary, including fields and explicit moves. */
            XrRep storage = XR_REP_VOID;
            if (!oracle_definition_storage(ctx, source_value, &storage, &ignored_kind) ||
                storage != XR_REP_TAGGED || ignored_kind != XR_MACHINE_REP_DYN_VALUE)
                return false;
            *out_storage = storage;
            return true;
        }
        ...
        case XI_CHAN_TIMER_DISPOSE:
            return operand_index == 0 && oracle_channel_operand_storage(
                ctx, operation, operand_index, source_value, out_storage);
        ...
        case XI_PHI:
            ...
            /* Incoming edges consume the merge's independently proved carrier.
             * Repeating a subset of producer families here rejects nullable
             * structural merges whose definition already has exact storage. */
        ...
        case XI_TAIL_CALL: {
            ...
            if (operand_index != 0 &&
                (oracle_direct_local_tagged_ref_place_storage(ctx, source_value, out_storage,
                                                               &ignored_kind) ||
                 oracle_direct_local_tagged_ref_parameter_place_storage(
                     ctx, source_value, out_storage, &ignored_kind)))
                return true;
            if (operand_index > 0 &&
                oracle_leaf_direct_local_aggregate_argument_use(ctx, operation_index, operand_index,
                                                                source_value, out_storage))
                return true;
            ...
            /* The native call boundary consumes scalars in their ABI representation,
             * even when a field read produced a tagged value. Reference arguments keep
             * their independently proved carrier. */
            if (oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind))
                return true;
            return oracle_tagged_reference_carrier_storage(ctx, source_value, out_storage,
                                                           &ignored_kind);
        }
        case XI_CALL_METHOD:
        case XI_CALL_METHOD_DIRECT:
            if (oracle_source_export_ref_call_use(ctx, operation_index, operand_index,
                                                   source_value, out_storage) ||
                oracle_local_tagged_ref_call_use(ctx, operation_index, operand_index,
                                                  source_value, out_storage))
                return true;
            ...
            if (operation->opcode == XI_CALL_METHOD) {
                ...
                if (xr_semantic_builtin_runtime_method_is_exact(ctx->semantic, operation,
                                                                &runtime_spec, NULL)) {
                    bool scalar_result = xr_semantic_builtin_runtime_method_result_class(
                        runtime_spec, xr_semantic_plan_type(ctx->semantic, operation->result_type)) == XR_SEM_BUILTIN_RUNTIME_METHOD_RESULT_SCALAR;
                    bool nullable_scalar = scalar_result && aot_nullable_scalar_type_is_exact(
                        xr_semantic_plan_type(ctx->semantic, operation->result_type));
                    bool result_exact = nullable_scalar
                        ? oracle_nullable_scalar_storage(ctx, operation->result_value, &result_storage,
                                                         &ignored_kind)
                        : scalar_result
                        ? oracle_machine_storage(ctx, operation->result_value, &result_storage,
                                                 &ignored_kind)
                        : oracle_dynamic_value_storage(ctx, operation->result_value,
                                                       &result_storage, &ignored_kind);
                    if (!result_exact ||
                        !xr_semantic_builtin_runtime_method_operand_type(
                            runtime_spec, operand_index, &operand_type) ||
                        !oracle_definition_storage(ctx, source_value, &source_storage, &source_kind))
                        return false;
                    if (operand_type == XA_BUILTIN_TYPE_RECEIVER_ELEM &&
                        runtime_spec->receiver == XA_BUILTIN_RECEIVER_CHANNEL) {
                        /* The frozen receiver/argument join already proves T.
                         * Preserve its independently verified machine carrier. */
                        if (source_storage != XR_REP_I64 && source_storage != XR_REP_F64 &&
                            source_storage != XR_REP_TAGGED)
                            return false;
                        *out_storage = source_storage;
                        return true;
                    }
                    if (operand_type == XA_BUILTIN_TYPE_INT) {
                        if (source_storage != XR_REP_I64 || source_kind != XR_MACHINE_REP_I64)
                            return false;
                        *out_storage = XR_REP_I64;
                        return true;
                    } else if ((operand_type != XA_BUILTIN_TYPE_STRING &&
                                operand_type != XA_BUILTIN_TYPE_RECEIVER) ||
                               source_storage != XR_REP_TAGGED ||
                               source_kind != XR_MACHINE_REP_DYN_VALUE) {
                        return false;
                    }
                    *out_storage = XR_REP_TAGGED;
                    return true;
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                uint16_t source_kind = XR_MACHINE_REP_COUNT;
                if (operand_index >= 3 ||
                    !oracle_dynamic_call_result_storage(ctx, operation->result_value,
                                                            &result_storage, &ignored_kind) ||
                    !oracle_definition_storage(ctx, source_value, &source_storage, &source_kind))
                    return false;
                if (operand_index == 0) {
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                if (operand_index >= 4 ||
                    !aot_source_class_array_fill_binding_is_exact(ctx, operation_index, &shape))
                    return false;
                uint32_t expected = operand_index == 0   ? shape.receiver
                                    : operand_index == 1 ? shape.element
                                    : operand_index == 2 ? shape.start
                                                         : shape.end;
                if (source_value != expected)
                    return false;
                *out_storage = operand_index < 2 ? XR_REP_TAGGED : XR_REP_I64;
                return true;
            }
            if (operation->opcode == XI_CALL_METHOD &&
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                uint16_t source_kind = XR_MACHINE_REP_COUNT;
                if (operand_index >= 2 ||
                    !oracle_dynamic_call_result_storage(
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                xr_semantic_rune_to_string_is_exact(ctx->semantic, operation, NULL)) {
                XrRep result_storage = XR_REP_TAGGED;
                if (operand_index != 0 ||
                    !oracle_rune_to_string_call(ctx, operation->result_value, &result_storage,
                                                &ignored_kind) ||
            ...
            /* Scalar arguments use the callee ABI even when optional payload
             * projection produced a tagged carrier. Reference arguments retain
             * their independently proved storage after scalar admission fails. */
            if (oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind))
                return true;
            return oracle_tagged_reference_carrier_storage(ctx, source_value, out_storage,
                                                           &ignored_kind);
        ...
        case XI_TUPLE_NEW: {
            ...
            if (!(xr_semantic_tuple_allocation_is_exact(ctx->semantic, operation) &&
                  oracle_dynamic_value_storage(ctx, operation->result_value, &aggregate_storage,
                                               &ignored_kind)) &&
                !oracle_value_aggregate_storage(ctx, operation->result_value, &aggregate_storage,
                                                &ignored_kind))
                return false;
            *out_storage = XR_REP_TAGGED;
            return true;
        ...
        case XI_TUPLE_GET:
            ...
            if (operand_index != 0 || !oracle_aggregate_field_access_is_exact(ctx, operation_index))
                return false;
            return oracle_tagged_tuple_storage(ctx, source_value, out_storage, &ignored_kind) ||
                   oracle_value_aggregate_storage(ctx, source_value, out_storage, &ignored_kind);
        case XI_OBJECT_GET_F:
            /* Structural objects keep one tagged root regardless of their field types. */
            if (operand_index != 0)
                return false;
        ...
        case XI_EQ:
        case XI_NE:
            /* A nullable unit enum compared with a member of the same enum
             * reads both sides in the tagged carrier that can hold null. */
            if (xr_semantic_nullable_unit_enum_equality_is_exact(ctx->semantic, operation)) {
                *out_storage = XR_REP_TAGGED;
                return true;
            }
        ...
        case XI_IS: {
            ...
            if (operand_index > 1 || !oracle_definition_storage(
                                         ctx, source_value, &definition_storage, &definition_kind))
                return false;
        ...
        case XI_AS: {
            ...
            XrRep definition_storage = XR_REP_VOID;
            uint16_t definition_kind = XR_MACHINE_REP_COUNT;
            bool exact_number_parse_error = xr_semantic_number_parse_error_catch_narrow_is_exact(
        ...
        case XI_SLICE_FROM_PTR:
            if (operand_index > 2 || !xr_semantic_raw_slice_is_exact(ctx->semantic, operation))
                return false;
            if (operand_index == 2)
                return oracle_definition_storage(ctx, source_value, out_storage, &ignored_kind);
            return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind);
        ...
        case XI_INDEX_GET:
            if (operation->opcode == XI_INDEX_GET) {
                ...
                uint32_t index_value = XR_SEMANTIC_INDEX_NONE;
                if (xr_semantic_builtin_unit_enum_member_access_is_exact(
            ...
            if (oracle_slice_element_access_is_exact(ctx, operation_index)) {
                if (operand_index == 0)
                    return oracle_slice_view_storage(ctx, source_value, out_storage,
                                                             &ignored_kind);
                return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind);
            }
            ...
            if (operand_index == 0)
                return oracle_array_tagged_carrier_storage(ctx, source_value, out_storage,
                       ...
                       oracle_direct_local_tagged_ref_parameter_place_storage(
                           ctx, source_value, out_storage, &ignored_kind);
            /* Element storage follows the exact Array element type. A scalar
             * Optional projection can still have a tagged definition carrier;
             * its native store therefore requires an explicit unbox. */
            return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind) ||
                   (operand_index == 2 && oracle_tagged_reference_carrier_storage(
                                              ctx, source_value, out_storage, &ignored_kind));
        case XI_LOAD_FIELD:
            /* Field identity and result type are admitted separately. Reading
             * a field consumes its receiver's proved tagged carrier, whether
             * it is a builtin namespace, parameter, allocation or call result. */
            return operand_index == 0 && oracle_tagged_reference_carrier_storage(
                                             ctx, source_value, out_storage, &ignored_kind);
        case XI_STORE_FIELD:
            /* The receiver retains its proved tagged instance carrier, whether
             * bound on entry or produced inside the function. The stored value
             * keeps its proved scalar or reference carrier.
             * Field selection and consuming ownership are separate obligations;
             * an unproved value still has no storage authority. */
            if (operand_index > 1 || !oracle_class_field_store_receiver_is_exact(ctx, operation))
                return false;
            if (operand_index == 0)
                return oracle_tagged_reference_carrier_storage(ctx, source_value, out_storage,
                                                               &ignored_kind);
            {
                /* A unit enum field holds the tagged enum value the object
                 * layout shares with every executor.  The ordinal's native
                 * machine storage reaches it through the box adapter it owes. */
                uint32_t operand_count = 0;
                const XrSemanticOperandRecord *operands =
                    xr_semantic_plan_operands(ctx->semantic, &operand_count);
                const XrSemanticOperandRecord *stored =
                    operands && operation->operand_begin < operand_count &&
                            operand_count - operation->operand_begin > 1u
                        ? operands + operation->operand_begin + 1u
                        : NULL;
                XrRep machine = XR_REP_TAGGED;
                if (stored && stored->value == source_value &&
                    xr_semantic_unit_enum_type_is_exact(
                        xr_semantic_plan_type(ctx->semantic, stored->type)) &&
                    oracle_machine_storage(ctx, source_value, &machine, &ignored_kind) &&
                    machine == XR_REP_I64) {
                    *out_storage = XR_REP_TAGGED;
                    return true;
                }
            ...
            return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind) ||
                   oracle_tagged_reference_carrier_storage(ctx, source_value, out_storage,
                                                          &ignored_kind);
        case XI_UNBOX:
    ...
    /* Explicit ABI and runtime boundaries above choose their required carrier.
     * Ordinary scalar consumers require the frozen native machine storage,
     * including an unbox when the definition deliberately holds a tagged value.
     * Other uses retain the definition's proved carrier without a second
     * producer list for nullable calls, variant tests and projections. */
    return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind) ||
           oracle_definition_storage(ctx, source_value, out_storage, &ignored_kind);
}

static bool authority_add_identity_obligation(CollectContext *ctx, const VerifyAuthority *oracle,
    ...
    XrAotRepresentationAdapterRequest request = {
        ...
        .source_partition = ctx->target_partition,
        .source_program_module_row = ctx->program_module_row,
        .use_operation = use_operation,
        .use_block = use_block,
        .use_operand = use_operand,
        .use_kind = XR_AOT_REP_USE_OPERATION,
        .adapter_kind = XR_AOT_REP_ADAPTER_IDENTITY,
        .input_rep_kind = XR_MACHINE_REP_I64,
        .output_rep_kind = XR_MACHINE_REP_I64,
        .layout = layout,
        .source_semantic_fingerprint = ctx->semantic_fingerprint,
        .source_module_identity = ctx->module_identity,
    ...
    if (!xr_aot_refinement_try_representation_adapter(
        ...
        rep_trace_refusal(oracle, "the INDEX_SET I64 identity authority",
                          "the stable source/use join does not match the "
                          "TargetPlan I64 value row",
                          source_value, use_operation, use_operand, XR_REP_I64, XR_REP_I64);
        set_diag(ctx->diag, XR_AOT_REFINEMENT_REPRESENTATION_SCHEMA_UNAVAILABLE, ctx->record_count,
                 source_value, use_operation);
        return false;
    }
...
/* A merge is the one producer whose opcode does not name what it hands back:
 * every other member of the two carrier lists below is an opcode that can only
 * ever produce one kind, so those members read the opcode and stop. A phi
 * carries whatever its edges carry, so its result type is checked here directly
 * and a merge of arrays is never admitted where a String is asked for. The
 * carrier itself is proved by the dynamic family, which binds a PHI-role slot
 * for exactly this shape -- the same proof the definition side already reads
 * for the merge. */
static bool oracle_dynamic_merge_carrier_storage(const VerifyAuthority *ctx,
                                                 uint32_t semantic_value, uint32_t expected_kind,
                                                 XrRep *out_storage, uint16_t *out_machine_kind) {
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ? xr_semantic_plan_operation(ctx->semantic, operation_index)
            : NULL;
    if (!operation || operation->opcode != XI_PHI || operation->result_value != semantic_value)
        return false;
    const XrSemanticTypeRecord *type = xr_semantic_plan_type(ctx->semantic, operation->result_type);
    if (!type || type->kind != expected_kind || (type->flags & XR_SEM_TYPE_NULLABLE) != 0)
        return false;
    return oracle_dynamic_value_storage(ctx, semantic_value, out_storage, out_machine_kind);
}

/* Array consumers need the exact container type and its proved carrier.
 * Whether an allocation, parameter, call, phi or Optional projection supplied
 * that value is already checked at its definition. Ref places stay separate. */
static bool oracle_array_tagged_carrier_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
                                                XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage || !out_machine_kind)
        return false;
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ? xr_semantic_plan_operation(ctx->semantic, operation_index)
            : NULL;
    uint32_t type = parameter ? parameter->type : operation ? operation->result_type
                                                          : XR_SEMANTIC_INDEX_NONE;
    XrRep storage = XR_REP_VOID;
    uint16_t kind = XR_MACHINE_REP_COUNT;
    if (!aot_array_type_is_exact(ctx->semantic, type, false, NULL) ||
        !oracle_definition_storage(ctx, semantic_value, &storage, &kind) ||
        storage != XR_REP_TAGGED || kind != XR_MACHINE_REP_DYN_VALUE)
        return false;
    *out_storage = storage;
    *out_machine_kind = kind;
    return true;
}

/* A String consumer checks the exact type and reuses the definition's carrier.
 * Keeping a second producer list here made field reads unusable by length and
 * equality even after their declaration and borrow authority were proved. */
static bool oracle_string_tagged_carrier_storage(const VerifyAuthority *ctx,
                                                 uint32_t semantic_value, XrRep *out_storage,
                                                 uint16_t *out_machine_kind) {
    if (!oracle_resolve_identity_rename(ctx, semantic_value, &semantic_value))
    ...
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, ctx->operation_by_value[semantic_value]);
    uint32_t type_index = parameter ? parameter->type
                                  : operation ? operation->result_type : XR_SEMANTIC_INDEX_NONE;
    return xr_semantic_tagged_string_type_is_exact(xr_semantic_plan_type(ctx->semantic, type_index)) &&
           oracle_tagged_reference_carrier_storage(ctx, semantic_value, out_storage, out_machine_kind);
}
...
/* Reference consumers use the definition's proven carrier. Repeating the
 * producer-family list here would make a legal definition unusable by cleanup
 * whenever the two lists drift. Native scalars and borrowed place pointers do
 * not become tagged references; aggregate boxing retains its exact layout
 * proof below. */
static bool oracle_tagged_reference_carrier_storage(const VerifyAuthority *ctx,
                                                    uint32_t semantic_value, XrRep *out_storage,
                                                    uint16_t *out_machine_kind) {
    ...
    XrRep storage = XR_REP_VOID;
    uint16_t kind = XR_MACHINE_REP_COUNT;
    if (oracle_definition_storage(ctx, semantic_value, &storage, &kind) &&
        storage == XR_REP_TAGGED && kind == XR_MACHINE_REP_DYN_VALUE) {
        *out_storage = storage;
        *out_machine_kind = kind;
        return true;
    }
    return oracle_fixed_array_backing_storage(ctx, semantic_value, out_storage, out_machine_kind) ||
           oracle_value_aggregate_storage(ctx, semantic_value, out_storage, out_machine_kind);
}

/* Native scalar returns use their ABI storage. Other results keep the
 * independently proved definition carrier; a second producer list at the
 * return edge would reject legal combinations already admitted by TargetPlan.
 * Function return type, ownership and physical ABI remain independently
 * verified before refinement consumes the immutable plan. */
static bool oracle_return_storage(const VerifyAuthority *ctx, uint32_t value, XrRep *out_storage,
    ...
    return oracle_machine_storage(ctx, value, out_storage, out_machine_kind) ||
           oracle_definition_storage(ctx, value, out_storage, out_machine_kind);
}
...
static bool authority_collect_obligations_indexed(CollectContext *ctx,
    ...
    for (uint32_t i = 0; i < operation_count; i++) {
        ...
        for (uint16_t a = 0; a < operation->operand_count; a++) {
            ...
            if (oracle_resolution_only_use(oracle, i, a, source_value))
                continue;
            bool has_definition =
            ...
            if (identity_use) {
                if (input_storage != XR_REP_I64 ||
                    !authority_add_identity_obligation(ctx, oracle, source_value, i,
                                                       operation->block, a))
                    return false;
            } else if (!authority_add_obligation(ctx, oracle, source_value, i, operation->block, a,
...
static bool authority_collect_obligations(CollectContext *ctx) {
    VerifyAuthority oracle = {
        ...
        .semantic_fingerprint = ctx->semantic_fingerprint,
        .module_identity = ctx->module_identity,
        .target_partition = ctx->target_partition,
        .program_module_row = ctx->program_module_row,
        .target_layout_begin = ctx->target_layout_begin,
        .target_layout_count = ctx->target_layout_count,
        .target_call_begin = ctx->target_call_begin,
        .target_call_count = ctx->target_call_count,
        .policy = ctx->policy,
        .diag = ctx->diag,
    ...
    bool valid =
        ...
        aot_index_native_module_namespace_values(&oracle) && aot_index_channel_values(&oracle) &&
    ...
    xr_target_program_reachability_dispose(&oracle.program_reachability);
    return valid;
}

bool xr_aot_representation_collect_authority(XrAotRefinementBuilder *builder,
                                             const XrTargetPlan *target_plan,
                                             const XrSemanticPlan *semantic,
                                             const XiRepPolicy *policy,
                                             XrAotRefinementDiagnostic *diag) {
    if (!builder || !target_plan || !semantic) {
...
static bool materialized_assertion_plan_matches(const XrSemanticOperationRecord *operation,
    ...
    if (!live || !xr_semantic_operation_assertion_plan(operation, &frozen) ||
        ...
        live->builtin_id != frozen.builtin_id || live->kind != frozen.kind || !live->source.file ||
        !live->source.file[0] || live->source.line != frozen.source.line ||
        live->source.column != frozen.source.column ||
        ...
        live->flags != frozen.flags || live->arity != frozen.arity || live->arity != value->nargs ||
        live->message_operand != frozen.message_operand ||
    ...
    for (uint16_t i = 0; i < value->nargs; i++) {
        ...
        XrRep expected =
            live->kind == XR_ASSERTION_KIND_CONDITION && i == 0 ? XR_REP_I64 : XR_REP_TAGGED;
...
static bool materialized_operation_shape_matches(const XrSemanticOperationRecord *operation,
    ...
    return operation && value &&
           ...
           value->backend_origin == XI_BACKEND_VALUE_NONE && value->id == local_value &&
...
static bool verify_exact_dynamic_storage_materialization(
    const XrAotRefinementPlanView *view, const XiFunc *root, const XrTargetPlan *target_plan,
    const XrSemanticPlan *semantic, const XiRepPolicy *policy, XrAotRefinementDiagnostic *diag) {
    VerifyAuthority ctx = {
        .target_plan = target_plan,
        .semantic = semantic,
        .policy = policy,
        ...
        if (array_range_fill_candidate) {
            ...
            for (uint16_t ordinal = 0; exact && ordinal < 4; ordinal++) {
                const XiValue *source = expected_values[ordinal] < ctx.value_count
                                            ...
                                            : NULL;
                const XrSemanticOperandRecord *operand =
                    &operands[operation->operand_begin + ordinal];
                exact = source && user->args[ordinal] == source &&
                        source_type_matches(source->type,
                                            xr_semantic_plan_type(ctx.semantic, operand->type)) &&
                        source->rep == ((source_class_exact && ordinal < 2) || ordinal == 0
                                            ? XR_REP_TAGGED
                                            : XR_REP_I64);
            }
            if (!exact) {
        ...
        for (uint16_t a = 0; valid && a < operation->operand_count; a++) {
            ...
            if (!user || !materialized_operation_shape_matches(operation, user, local_value) ||
                ...
                if (rep_trace_enabled())
                    fprintf(
                        stderr,
                        "[aot-refine] resolution use=%u operand=%u shape=%u actual=%u "
                        "expected=%u direct=%u go=%u namespace=%u\n",
                        i, a,
                        user && materialized_operation_shape_matches(operation, user, local_value),
                        user && user->args && a < user->nargs && user->args[a] ? user->args[a]->id
                                                                               : UINT32_MAX,
                        materialized_source ? materialized_source->id : UINT32_MAX,
                        ctx.exact_direct_callee_value[source_value],
                        ctx.exact_go_callee_value[source_value],
                        ctx.exact_source_namespace_value
                            ? ctx.exact_source_namespace_value[source_value]
...
static bool verify_no_extra_materialized_adapters(const VerifyAuthority *scope,
    ...
    if (!function)
        return false;
    bool reachable =
        aot_program_function_is_reachable(scope, function->semantic_plan_function_index);
    for (uint32_t b = 0; reachable && b < function->nblocks; b++) {
        const XiBlock *block = function->blocks[b];
        if (!block)
            return false;
        for (const XiPhi *phi = block->phis; phi; phi = phi->next) {
            ...
            if (value && value->backend_origin != XI_BACKEND_VALUE_NONE &&
                ...
                if (getenv("XRAY_AOT_REFINE_TRACE"))
                    fprintf(stderr,
                            ...
                            "op=%u:%s backend-origin=%u source=%u source-op=%s source-rep=%u\n",
                            ...
                            value->nargs == 1u && value->args && value->args[0]
                                ? xi_generated_op_name(value->args[0]->op) : "<none>",
                            value->nargs == 1u && value->args && value->args[0]
                                ? value->args[0]->rep : UINT32_MAX);
                if (getenv("XRAY_AOT_REFINE_TRACE")) {
    ...
    for (uint16_t i = 0; i < function->nchildren; i++) {
        if (!verify_no_extra_materialized_adapters(scope, function->children[i], matched,
                                                   matched_count, diag))
            return false;
    }
    return true;
...
static bool representation_materialization_verify_admitted(
    const XrAotRefinementPlanView *view, const XiFunc *root,
    const XrTargetPlan *target_plan, const XiRepPolicy *policy,
    XrAotRefinementDiagnostic *diag) {
    const XrSemanticPlan *semantic = root->semantic_plan;
    XiRepPolicy default_policy = xi_rep_policy_native_boundary();
    const XiRepPolicy *effective_policy = policy ? policy : &default_policy;
    if (!verify_immutable_authority_coverage(view, target_plan, semantic, effective_policy, diag))
        return false;
    if (!verify_exact_dynamic_storage_materialization(view, root, target_plan, semantic,
                                                      effective_policy, diag))
        return false;
    ...
    for (uint32_t i = 0; valid && i < view->record_count; i++) {
        ...
        if (!source || source_matches != 1 ||
            ...
            if (getenv("XRAY_AOT_REFINE_TRACE"))
                fprintf(stderr,
                        "[aot-refine] adapter source identity function=%s record=%u local=%u "
                        "matches=%u op=%s origin=%u resolved-function=%u resolved-value=%u\n",
                        function->name ? function->name : "<anonymous>", i, source_local,
                        source_matches, source ? xi_generated_op_name(source->op) : "<missing>",
                        source ? source->backend_origin : UINT32_MAX, source_function,
                        source_value);
            if (getenv("XRAY_AOT_REFINE_TRACE")) {
                for (uint32_t op = 0; op < xr_semantic_plan_operation_count(semantic); op++) {
                    const XrSemanticOperationRecord *definition =
                        xr_semantic_plan_operation(semantic, op);
                    if (definition && definition->result_value == record->source_value)
                        fprintf(stderr, "[aot-refine] missing source definition operation=%u "
                                "opcode=%s block=%u type=%u\n", op,
                                xi_generated_op_name(definition->opcode), definition->block,
        ...
        if (record->adapter_kind == XR_AOT_REP_ADAPTER_IDENTITY) {
            if (record->use_kind != XR_AOT_REP_USE_OPERATION || adapter != source ||
                source->rep != XR_REP_I64 || record->input_rep_kind != XR_MACHINE_REP_I64 ||
    ...
    if (valid) {
        VerifyAuthority scope = {.target_plan = target_plan, .semantic = semantic, .diag = diag};
        /* Adapter obligations have the same closed-program reachability as the
         * immutable
         * records. Frozen identity checks above still cover every body. */
        valid =
            verify_target_scope_init(&scope) && aot_program_reachability_init(&scope) &&
            verify_no_extra_materialized_adapters(&scope, root, matched, view->record_count, diag);
        verify_authority_dispose(&scope);
    }
    xr_free(matched);
    if (valid && diag)
        memset(diag, 0, sizeof(*diag));
...
bool xr_aot_representation_materialization_verify_modules(
    const XrAotRefinementPlanView *views, const XiFunc *const *roots,
    uint32_t module_count, const XrTargetPlan *target_plan, const XiRepPolicy *policy,
    uint32_t *failed_module, XrAotRefinementDiagnostic *diag) {
    if (diag)
        memset(diag, 0, sizeof(*diag));
    if (failed_module)
        *failed_module = 0;
    if (!views || !roots || !target_plan || module_count == 0 ||
        module_count > XR_TARGET_MAX_PROGRAM_MODULES) {
        set_diag(diag, XR_AOT_REFINEMENT_INVALID_ARGUMENT, 0, 0, 0);
        return false;
    }
    const XrSemanticPlan **semantics =
        (const XrSemanticPlan **) xr_calloc(module_count, sizeof(*semantics));
    if (!semantics) {
        set_diag(diag, XR_AOT_REFINEMENT_OUT_OF_MEMORY, 0, 0, 0);
        return false;
    }
    ...
    for (uint32_t i = 0; i < module_count; ++i) {
        semantics[i] = roots[i] ? roots[i]->semantic_plan : NULL;
        uint32_t partition = UINT32_MAX;
        if (!semantics[i] ||
            !xr_target_plan_partition_for_semantic(target_plan, semantics[i], &partition)) {
            if (failed_module)
                *failed_module = i;
    ...
    if (valid)
        valid = xr_aot_refinement_verify_modules(target_plan, semantics, views, module_count,
                                                  failed_module, diag);
    for (uint32_t i = 0; valid && i < module_count; ++i) {
        valid = representation_materialization_verify_admitted(
            &views[i], roots[i], target_plan, policy, diag);
...
bool xr_aot_representation_materialization_verify(
    ...
    const XrTargetPlan *target_plan, const XiRepPolicy *policy,
    XrAotRefinementDiagnostic *diag) {
    return xr_aot_representation_materialization_verify_modules(
        view, &root, 1, target_plan, policy, NULL, diag);
}
...
static bool verify_exact_record_authority(VerifyAuthority *ctx, uint32_t record_index,
    ...
    const XrSemanticFunctionRecord *function =
        xr_semantic_plan_function(ctx->semantic, source_function);
    if (!type || !function || record->source_function != source_function ||
        record->source_value != source_value || record->source_partition != ctx->target_partition ||
        record->source_program_module_row != ctx->program_module_row ||
        !xr_fingerprint_equal(record->source_semantic_fingerprint, ctx->semantic_fingerprint) ||
        !xr_stable_id_equal(record->source_module_identity, ctx->module_identity) ||
        record->source_operation != operation_index || record->source_type != source_type ||
        record->source_kind != source_kind || record->source_auxiliary_kind != source_auxiliary ||
        record->source_flags != source_flags ||
        record->source_semantic_immediate != source_immediate ||
        !xr_stable_id_equal(record->source_function_id, function->id) ||
    ...
    uint32_t layout_count = 0;
    const XrTargetLayoutRecord *layouts = xr_target_plan_layouts(ctx->target_plan, &layout_count);
    if (!layouts || record->layout >= layout_count || record->layout < ctx->target_layout_begin ||
        record->layout - ctx->target_layout_begin >= ctx->target_layout_count ||
...
static bool verify_exact_obligation(VerifyAuthority *ctx, uint32_t source_value,
    ...
    if (!verify_charge_work(ctx, 1))
        return false;
    if (input_storage == output_storage && !identity_use)
        return true;
    if (identity_use) {
        if (use_kind != XR_AOT_REP_USE_OPERATION || input_storage != XR_REP_I64 ||
            output_storage != XR_REP_I64 || machine_kind != XR_MACHINE_REP_I64) {
            rep_materialization_survey_row(ctx, source_value, use_operation, input_storage,
                                           output_storage, machine_kind);
            set_diag(ctx->diag, XR_AOT_REFINEMENT_REPRESENTATION, (uint32_t) ctx->work,
                     source_value, use_operation);
            return false;
        }
    } else if (!oracle_machine_storage(ctx, source_value, &native_storage, &machine_kind)) {
    ...
    if (!identity_use && ((input_storage != XR_REP_TAGGED && input_storage != native_storage) ||
                          (output_storage != XR_REP_TAGGED && output_storage != native_storage))) {
    ...
    if (!identity_use && input_storage != XR_REP_TAGGED && output_storage != XR_REP_TAGGED)
        return true;
    ...
    const XrAotRepresentationAdapterRecord *record =
        &ctx->view->records[record_index].representation_adapter;
    uint16_t expected_adapter = identity_use                     ? XR_AOT_REP_ADAPTER_IDENTITY
                                : input_storage == XR_REP_TAGGED ? XR_AOT_REP_ADAPTER_UNBOX
                                                                 : XR_AOT_REP_ADAPTER_BOX;
    uint16_t expected_input = identity_use                     ? XR_MACHINE_REP_I64
                              : input_storage == XR_REP_TAGGED ? XR_MACHINE_REP_DYN_VALUE
                                                               : machine_kind;
    uint16_t expected_output = identity_use                      ? XR_MACHINE_REP_I64
                               : output_storage == XR_REP_TAGGED ? XR_MACHINE_REP_DYN_VALUE
                                                                 : machine_kind;
    uint16_t expected_recipe = identity_use
                                   ? XR_AOT_REP_RECIPE_NONE
    ...
    if ((!identity_use && expected_recipe == XR_AOT_REP_RECIPE_NONE) ||
        record->adapter_kind != expected_adapter || record->recipe != expected_recipe ||
        record->input_rep_kind != expected_input || record->output_rep_kind != expected_output) {
        rep_materialization_survey_row(ctx, source_value, use_operation, input_storage,
                                       output_storage, machine_kind);
        set_diag(ctx->diag, XR_AOT_REFINEMENT_REPRESENTATION, record_index, source_value,
                 use_operation);
...
static bool verify_exact_semantic_coverage(VerifyAuthority *ctx) {
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        for (uint16_t a = 0; a < operation->operand_count; a++) {
            ...
            XrRep output_storage = XR_REP_TAGGED;
            if (source_value < ctx->value_count &&
                oracle_resolution_only_use(ctx, i, a, source_value))
                continue;
            bool identity_use = source_value < ctx->value_count &&
                                verify_index_set_i64_identity_use_is_exact(ctx, i, a, source_value);
            if (identity_use)
                output_storage = XR_REP_I64;
            if (source_value >= ctx->value_count ||
                (!identity_use && !oracle_use_storage(ctx, i, a, source_value, &output_storage))) {
            ...
            if (!verify_exact_obligation(ctx, source_value, i, operation->block, a,
                                         XR_AOT_REP_USE_OPERATION, output_storage, identity_use))
                return false;
        }
    }
    for (uint32_t i = 0; i < ctx->block_count; i++) {
        ...
        if (!verify_exact_obligation(ctx, block->control_value, XR_SEMANTIC_INDEX_NONE, i, 0,
                                     XR_AOT_REP_USE_BLOCK_CONTROL, output_storage, false))
            return false;
    }
