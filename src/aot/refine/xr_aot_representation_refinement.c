...
static bool aot_array_ref_type_is_exact(const XrSemanticPlan *semantic, uint32_t type_index,
                                        uint8_t *storage);
...
static bool aot_array_allocation_is_exact(const XrSemanticPlan *semantic,
    ...
    bool exact = element && capacity_type && function &&
                 ...
                 aot_array_intrinsic_storage_is_exact(element, operation->array_element_storage,
                                                      &semantic_storage) &&
                 aot_array_ref_type_is_exact(semantic, operation->result_type, &type_storage) &&
...
static bool aot_array_ref_type_is_exact(const XrSemanticPlan *semantic, uint32_t type_index,
    ...
    if (!element || element->builtin_type != XR_TID_NULL || element->child_count != 0 ||
        element->aggregate_extent != 0 || element->aggregate_align != 0 || element->flags != 0 ||
        !target_storage)
        return false;
    switch (element->kind) {
...
static bool oracle_array_element_access_is_exact(const VerifyAuthority *ctx,
    ...
    bool owned_array =
        ...
        oracle_dynamic_array_fill_scalar_storage(ctx, container->value, &container_storage,
                                                 &container_kind) ||
        oracle_array_hof_result_storage(ctx, container->value, &container_storage, &container_kind);
    bool borrowed_array =
    ...
    if (!container_type ||
        ...
        (borrowed_array &&
         !aot_array_ref_type_is_exact(ctx->semantic, container->type, &borrowed_storage)) ||
...
static bool oracle_dynamic_array_ref_storage(const VerifyAuthority *ctx, uint32_t semantic_value,
    ...
    const XrSemanticOperationRecord *operation =
        operation_index != XR_SEMANTIC_INDEX_NONE
            ...
            : NULL;
    uint8_t storage = XR_TARGET_ARRAY_STORAGE_NONE;
    bool shared_value = aot_array_ref_shared_value_is_exact(ctx->semantic, operation, &storage) &&
                        operation->result_value == semantic_value;
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx->semantic, &operand_count);
    ...
    bool place_load_value =
        ...
        aot_array_ref_type_is_exact(ctx->semantic, operation->result_type, &storage) &&
...
static bool oracle_direct_local_array_ref_place_use(const VerifyAuthority *ctx,
    ...
    if (!place || !operands || place->opcode != XI_LOCAL_ADDR || place->operand_count != 1 ||
        ...
        !aot_array_ref_type_is_exact(ctx->semantic, place->result_type, &storage))
        return false;
    ...
    switch (operation->opcode) {
        ...
        case XI_RELEASE: {
            ...
            if (!oracle_dynamic_string_literal_storage(ctx, source_value, &literal_storage,
                ...
                !oracle_dynamic_string_concat_result_storage(ctx, source_value, &literal_storage,
                                                             &ignored_kind) &&
                !oracle_dynamic_array_allocation_storage(ctx, source_value, &literal_storage,
                                                         &ignored_kind) &&
                ...
                !oracle_array_hof_result_storage(ctx, source_value, &literal_storage,
                                                 &ignored_kind) &&
                !oracle_dynamic_array_ref_storage(ctx, source_value, &literal_storage,
                                                  &ignored_kind) &&
        ...
        case XI_PRINT: {
            ...
            if (!oracle_machine_storage(ctx, source_value, &machine_storage, &ignored_kind) &&
                ...
                !oracle_dynamic_string_concat_result_storage(ctx, source_value, &machine_storage,
                                                             &ignored_kind) &&
                !oracle_dynamic_array_allocation_storage(ctx, source_value, &machine_storage,
                                                         &ignored_kind) &&
                ...
                !oracle_array_hof_result_storage(ctx, source_value, &machine_storage,
                                                 &ignored_kind) &&
                !oracle_dynamic_array_ref_storage(ctx, source_value, &machine_storage,
                                                  &ignored_kind) &&
        ...
        case XI_INDEX_GET:
            ...
            if (!oracle_array_element_access_is_exact(ctx, operation_index))
                return false;
            if (operand_index == 0)
                return oracle_dynamic_array_allocation_storage(ctx, source_value, out_storage,
                                                               &ignored_kind) ||
                       ...
                       oracle_direct_local_array_ref_parameter_place_storage(
                           ctx, source_value, out_storage, &ignored_kind) ||
                       oracle_dynamic_array_ref_storage(ctx, source_value, out_storage,
                                                        &ignored_kind);
            return oracle_machine_storage(ctx, source_value, out_storage, &ignored_kind);
        case XI_LOAD_FIELD:
