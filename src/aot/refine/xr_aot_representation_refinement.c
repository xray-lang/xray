...
static bool collect_indices_init(CollectContext *ctx) {
    ...
    size_t operation_count =
        xr_semantic_plan_operation_count(ctx->semantic);
    ...
    ctx->call_by_operation = (uint32_t *) xr_malloc(
        (ctx->operation_count ? ctx->operation_count : 1u) *
        sizeof(uint32_t));
    ...
    if (!ctx->operation_by_value || !ctx->parameter_by_value ||
        !ctx->use_count_by_value || !ctx->call_by_operation ||
        !ctx->layout_by_type) {
    ...
    for (uint32_t i = 0; i < call_count; i++) {
        if (!calls || calls[i].id != i ||
            calls[i].semantic_operation >= ctx->operation_count ||
            ctx->call_by_operation[calls[i].semantic_operation] !=
                XR_SEMANTIC_INDEX_NONE)
            goto invalid;
...
static bool oracle_dynamic_array_intrinsic_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind);
static bool oracle_dynamic_array_fill_scalar_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind);
static bool oracle_array_hof_result_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind);
    ...
    switch (element->kind) {
        case XR_KIND_INT:
            switch (element->scalar_rep) {
                case XR_NATIVE_I8: expected = XR_TARGET_ARRAY_STORAGE_I8; break;
                case XR_NATIVE_U8: expected = XR_TARGET_ARRAY_STORAGE_U8; break;
                case XR_NATIVE_I16: expected = XR_TARGET_ARRAY_STORAGE_I16; break;
                case XR_NATIVE_U16: expected = XR_TARGET_ARRAY_STORAGE_U16; break;
                case XR_NATIVE_I32: expected = XR_TARGET_ARRAY_STORAGE_I32; break;
                case XR_NATIVE_U32: expected = XR_TARGET_ARRAY_STORAGE_U32; break;
                case XR_NATIVE_I64: expected = XR_TARGET_ARRAY_STORAGE_I64; break;
                case XR_NATIVE_U64: expected = XR_TARGET_ARRAY_STORAGE_U64; break;
                default: return false;
            }
            break;
        case XR_KIND_FLOAT:
...
static bool aot_array_intrinsic_fill_type_is_exact(
...
static bool aot_array_ref_type_is_exact(const XrSemanticPlan *semantic,
    ...
    if (!children || !array || array->kind != XR_KIND_ARRAY ||
...
static bool aot_array_ref_parameter_is_exact(
    const XrSemanticPlan *semantic,
    ...
    return parameter &&
           ...
           aot_array_ref_type_is_exact(semantic, parameter->type, storage);
}

static bool aot_array_ref_shared_value_is_exact(
    const XrSemanticPlan *semantic,
    const XrSemanticOperationRecord *operation, uint8_t *storage) {
    return operation && operation->opcode == XI_GET_SHARED &&
           operation->operand_count == 0 &&
...
static bool aot_array_intrinsic_is_exact(
    const XrSemanticPlan *semantic, const XrSemanticOperationRecord *operation,
    ...
    if (!semantic || !operation || (!with_capacity && !filled) || !operands ||
        !children || operation->opcode != XI_CALL_BUILTIN ||
    ...
    if (!count_type || !semantic_exact_i64_type(count_type) ||
        ...
        !aot_array_intrinsic_storage_is_exact(
            element, operation->array_element_storage, &storage))
        return false;
    if (filled) {
...
static bool aot_array_fill_scalar_is_exact(
    const XrSemanticPlan *semantic,
...
static bool aot_pair_identity(const char *domain, XrStableId first,
    ...
    int written = snprintf(key, sizeof(key),
                           "%s:first=%s:second=%s:ordinal=%u", domain,
...
static bool aot_array_hof_array_type_is_exact(
    ...
    const XrSemanticTypeRecord *element =
        xr_semantic_plan_type(semantic, element_index);
    uint8_t storage = XR_TARGET_ARRAY_STORAGE_NONE;
    if (!aot_array_intrinsic_storage_is_exact(element, frozen_storage,
                                               &storage))
        return false;
    if (out_element)
...
static bool aot_array_hof_semantic_is_exact(
    const VerifyAuthority *ctx, uint32_t operation_index,
    AotArrayHofAuthority *out) {
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx ? ctx->semantic : NULL,
                                   operation_index);
    uint32_t operand_count = 0, child_count = 0, metadata_count = 0;
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx ? ctx->semantic : NULL,
                                  &operand_count);
    const uint32_t *children =
        xr_semantic_plan_type_children(ctx ? ctx->semantic : NULL,
                                       &child_count);
    if (ctx)
    ...
    if (!aot_array_hof_array_type_is_exact(
            ctx->semantic, rows[0].type, operation->array_element_storage,
            &source_element, &source_storage))
        return false;
    ...
    if (kind != XR_TARGET_ARRAY_HOF_REDUCE) {
        if (!aot_array_hof_array_type_is_exact(
                ctx->semantic, operation->result_type,
                operation->array_result_element_storage, &result_element,
                &result_storage) ||
            (kind == XR_TARGET_ARRAY_HOF_FILTER &&
        ...
        if (rows[2].type != operation->result_type ||
            !aot_array_intrinsic_storage_is_exact(
                result_type, operation->array_result_element_storage,
                &result_storage))
            return false;
    }
    const XrSemanticFunctionRecord *callee =
    ...
    uint16_t expected_parameters =
        kind == XR_TARGET_ARRAY_HOF_REDUCE ? 2u : 1u;
    if (!callee || callee->parent != operation->function ||
        ...
        callee->parameter_count >
            ctx->parameter_count - callee->parameter_begin)
        return false;
    ...
    const XrSemanticParameterRecord *second =
        expected_parameters == 2u
            ? xr_semantic_plan_parameter(ctx->semantic,
                                         callee->parameter_begin + 1u)
            : NULL;
    if (!first || first->function != operation->callable_function ||
        first->ordinal != 0 ||
        first->type != (kind == XR_TARGET_ARRAY_HOF_REDUCE
                           ? result_element
                           : source_element) ||
        (second &&
          (second->function != operation->callable_function ||
    ...
    if (kind == XR_TARGET_ARRAY_HOF_FILTER) {
        ...
        if (!return_type || return_type->kind != XR_KIND_BOOL ||
            ...
            return_type->aggregate_extent != 0 ||
            return_type->aggregate_align != 0 || return_type->flags != 0)
            return false;
    } else if (callee->return_type != result_element) {
        return false;
    }
    const XrSemanticTypeRecord *callback_type =
        xr_semantic_plan_type(ctx->semantic, rows[1].type);
    ...
    if (out) {
        *out = (AotArrayHofAuthority) {
            ...
            .initial = kind == XR_TARGET_ARRAY_HOF_REDUCE
                           ? rows[2].value
                           : XR_SEMANTIC_INDEX_NONE,
            .kind = kind,
...
static bool aot_native_module_yieldable_call_is_exact(
    const XrSemanticPlan *semantic, uint32_t operation_index,
...
static bool aot_native_module_namespace_value_is_exact(
    ...
    for (uint32_t i = 0; i < operation_count; i++) {
        ...
        for (uint16_t a = 0; a < use->operand_count; a++) {
            ...
            bool scalar = aot_native_module_call_shape_is_exact(
...
static bool aot_source_import_dependency_is_exact(
    const XrSemanticPlan *semantic,
    const XrSemanticOperationRecord *operation,
    ...
    if (!semantic || !operation || !metadata || !out_dependency ||
        !aot_source_namespace_operation_is_exact(
...
static bool aot_index_source_namespace_values(VerifyAuthority *ctx) {
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        if (!import || !load || receiver->type != load->result_type ||
            ...
            !aot_source_import_dependency_is_exact(
                ctx->semantic, import, metadata, metadata_count, named_export,
                &import_dependency) ||
    ...
    for (uint32_t i = 0; i < ctx->operation_count; i++) {
        ...
        if (candidate[import->result_value])
            continue;
        uint32_t import_dependency = XR_SEMANTIC_INDEX_NONE;
        if (!aot_source_import_dependency_is_exact(
                ctx->semantic, import, metadata, metadata_count, false,
                &import_dependency))
            goto invalid;
        uint32_t store_index = XR_SEMANTIC_INDEX_NONE;
        ...
        ctx->source_namespace_dependency_by_value[import->result_value] =
            import_dependency;
...
static bool verify_authority_init(VerifyAuthority *ctx) {
    ...
    if (!verify_charge_work(ctx, (uint64_t) ctx->function_count +
                                    ...
                                    ctx->parameter_count + ctx->type_count +
    ...
    if (!verify_alloc(ctx, ctx->value_count, sizeof(uint32_t),
        ...
        !verify_alloc(ctx, ctx->type_count, sizeof(uint32_t),
                      (void **) &ctx->layout_by_type) ||
    ...
    for (uint32_t i = 0; i < layout_count; i++) {
        if (!layouts || layouts[i].id != i ||
            layouts[i].semantic_type >= ctx->type_count ||
            ctx->layout_by_type[layouts[i].semantic_type] !=
                XR_SEMANTIC_INDEX_NONE) {
    ...
    uint32_t semantic_function =
        parameter ? parameter->function
                  : operation ? operation->function
                              : XR_SEMANTIC_INDEX_NONE;
    if (!exact_shape || !binding || !register_rep || !memory_rep || !layout ||
        !slot || register_rep->kind != XR_MACHINE_REP_AGGREGATE ||
        ...
        layout->semantic_type != semantic_type ||
...
static bool oracle_aggregate_field_access_is_exact(
    ...
    uint32_t child_ordinal = receiver_type &&
                                     receiver_type->kind == XR_KIND_FIXED_ARRAY
                                 ? 0u
                                 : field_index;
    if (!receiver_type || !children ||
        receiver_type->child_begin > child_count ||
    ...
    return stored->role == XR_SEM_OPERAND_VALUE && stored->parameter == -1 &&
           stored->flags == 0 && stored->type == field_type &&
           stored->value < ctx->value_count;
}

static bool aot_array_hof_target_is_exact(
    const VerifyAuthority *ctx, uint32_t operation_index,
    AotArrayHofAuthority *out) {
    ...
    const XrSemanticOperationRecord *operation =
        xr_semantic_plan_operation(ctx->semantic, operation_index);
    const XrSemanticFunctionRecord *callee = operation
        ? xr_semantic_plan_function(ctx->semantic,
                                    operation->callable_function)
        : NULL;
    ...
    uint32_t discriminator = ((uint32_t) shape.kind << 16) |
                             ((uint32_t) shape.source_storage << 8) |
                             shape.result_storage;
    if (!operation || !callee || !call || !result || !result_register ||
        !result_memory || !result_slot || !layout || !arguments ||
        !aot_pair_identity("xray-target-array-hof-v1", operation->id,
                           callee->id, discriminator, &expected_call) ||
        !xr_stable_id_equal(call->identity, expected_call) ||
        call->id != call_index ||
        call->semantic_call_target != XR_SEMANTIC_INDEX_NONE ||
        ...
        call->result_ownership !=
            (shape.kind == XR_TARGET_ARRAY_HOF_REDUCE
                 ? XR_TARGET_CALL_NONE
                 : XR_TARGET_CALL_RETURN_OWNED) ||
        call->array_intrinsic_kind != XR_TARGET_ARRAY_INTRINSIC_NONE ||
        call->array_element_storage != shape.source_storage ||
        call->array_hof_kind != shape.kind ||
        call->array_result_element_storage != shape.result_storage ||
        call->reserved8[0] != 0 || call->reserved8[1] != 0 ||
        call->reserved8[2] != 0 ||
        result->semantic_value != operation->result_value ||
        result_slot->id != result->slot ||
        ...
        layout->id != layout_index ||
        layout->semantic_type != operation->result_type ||
        layout->field_count != 0 || layout->root_field_count != 0 ||
        layout->fixed_prefix_size != result_memory->memory_size ||
    ...
    if (dynamic_result) {
        ...
        if (!oracle_machine_storage(ctx, operation->result_value,
                                    &scalar_storage, &scalar_kind) ||
            scalar_storage == XR_REP_TAGGED ||
            scalar_kind != result_register->kind ||
    ...
    for (uint16_t ordinal = 0; ordinal < call->argument_count; ordinal++) {
        ...
        if (!type || !caller || operand->value != expected_value ||
            !aot_pair_identity("xray-target-array-hof-argument-v1",
...
static bool oracle_dynamic_array_fill_scalar_storage(
    ...
    uint8_t storage = XR_TARGET_ARRAY_STORAGE_NONE;
    if (!aot_array_fill_scalar_is_exact(ctx->semantic, operation,
                                        &receiver_value, &fill_value,
                                        &storage))
        return false;
    const XrTargetValueRepRecord *binding =
        xr_target_plan_value_rep(ctx->target_plan, semantic_value);
    ...
    const XrTargetCallArgumentRecord *arguments =
        xr_target_plan_call_arguments(ctx->target_plan, &argument_count);
    const XrSemanticTypeRecord *receiver_type = operation
        ? xr_semantic_plan_type(ctx->semantic, operation->result_type)
        : NULL;
    XrStableId expected_call;
    ...
    if (!operation || !binding || !register_rep || !memory_rep || !slot ||
        !layout || !call || !operands || !arguments || !expected_call_valid ||
        !xr_stable_id_equal(call->identity, expected_call) ||
        call->semantic_call_target != XR_SEMANTIC_INDEX_NONE ||
        call->caller_function != operation->function ||
        call->callee_function != XR_SEMANTIC_INDEX_NONE ||
        ...
        call->result_ownership != XR_TARGET_CALL_NONE ||
        call->array_intrinsic_kind != XR_TARGET_ARRAY_INTRINSIC_NONE ||
        call->array_element_storage != storage ||
        call->reserved8[0] != 0 || call->reserved8[1] != 0 ||
        call->reserved8[2] != 0 ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
        memory_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
    ...
    for (uint16_t ordinal = 0; ordinal < 2; ordinal++) {
        ...
        if (!type || !caller ||
            operand->value !=
                (ordinal == 0 ? receiver_value : fill_value) ||
            !aot_pair_identity(
...
static bool oracle_string_slice_parameter_receiver_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
    ...
    if (!parameter || parameter->value != semantic_value ||
        !xr_semantic_string_slice_string_type_is_exact(type) ||
        ...
        parameter->reserved != 0)
        return false;
    uint8_t ownership = parameter->ownership == XI_OWN_OWNED
...
static bool oracle_string_slice_receiver_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    return oracle_string_slice_parameter_receiver_storage(
               ctx, semantic_value, out_storage, out_machine_kind) ||
           oracle_dynamic_string_literal_storage(
               ctx, semantic_value, out_storage, out_machine_kind);
}

static bool oracle_dynamic_string_slice_range_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
    ...
    if (!xr_semantic_string_slice_range_is_exact(
            ctx->semantic, operation, &receiver, &start, &end) ||
        operation->result_value != semantic_value)
        return false;
    ...
    if (!oracle_string_slice_receiver_storage(
            ctx, receiver, &receiver_storage, &receiver_kind) ||
        receiver_storage != XR_REP_TAGGED ||
        ...
        !oracle_machine_storage(ctx, end, &end_storage, &end_kind) ||
        end_storage != XR_REP_I64 || end_kind != XR_MACHINE_REP_I64)
        return false;
    const XrSemanticTypeRecord *result_type =
        xr_semantic_plan_type(ctx->semantic, operation->result_type);
    const XrTargetValueRepRecord *binding =
...
static bool oracle_iterator_rune_next_call(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
...
static bool oracle_rune_to_uint32_call(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
    ...
    XrRep receiver_storage = XR_REP_TAGGED;
    uint16_t receiver_kind = XR_MACHINE_REP_COUNT;
    if (!oracle_iterator_rune_next_call(ctx, receiver, &receiver_storage,
                                        &receiver_kind) ||
        receiver_storage != XR_REP_I64 ||
        receiver_kind != XR_MACHINE_REP_RUNE)
        return false;
    const XrSemanticTypeRecord *result_type =
        xr_semantic_plan_type(ctx->semantic, operation->result_type);
    const XrTargetValueRepRecord *binding =
    ...
    int written = snprintf(
        key, sizeof(key),
        "xray-target-rune-to-uint32-v1:first=%s:second=%s:ordinal=%u",
        first_hex, second_hex, receiver);
    if (!result_type || !binding || !call || written <= 0 ||
        (size_t) written >= sizeof(key) ||
        !xr_stable_id_from_key(key, &expected_call, &digest) ||
...
static bool oracle_rune_is_whitespace_call(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
...
static bool oracle_dynamic_source_class_instance_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind) {
    if (!ctx || semantic_value >= ctx->value_count || !out_storage ||
        !out_machine_kind)
...
static bool oracle_dynamic_array_ref_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind);
...
static bool oracle_array_element_access_is_exact(const VerifyAuthority *ctx,
    ...
    bool owned_array =
        ...
        oracle_dynamic_array_intrinsic_storage(ctx, container->value,
                                               &container_storage,
                                               &container_kind) ||
        oracle_dynamic_array_fill_scalar_storage(ctx, container->value,
                                                 &container_storage,
                                                 &container_kind) ||
        oracle_array_hof_result_storage(ctx, container->value,
                                        &container_storage,
                                        &container_kind);
    ...
    return container->role == XR_SEM_OPERAND_VALUE &&
           ...
           (oracle_dynamic_array_allocation_storage(
            ...
            oracle_dynamic_array_intrinsic_storage(
                ctx, container->value, &container_storage, &container_kind) ||
            oracle_dynamic_array_fill_scalar_storage(
                ctx, container->value, &container_storage, &container_kind) ||
            oracle_array_hof_result_storage(
                ctx, container->value, &container_storage, &container_kind) ||
...
static bool oracle_dynamic_array_intrinsic_storage(
    ...
    bool expected_call_valid =
        aot_pair_identity("xray-target-array-intrinsic-v1", operation->id,
                          operation->allocation_id, discriminator,
                          &expected_call);
    if (!operation || !binding || !register_rep || !memory_rep || !slot ||
        !layout || !call || !operands || !arguments ||
        !expected_call_valid ||
        !xr_stable_id_equal(call->identity, expected_call) ||
        call->semantic_call_target != XR_SEMANTIC_INDEX_NONE ||
        call->caller_function != operation->function ||
        ...
        call->result_mode != XR_TARGET_CALL_VALUE ||
        call->result_ownership != XR_TARGET_CALL_RETURN_OWNED ||
        call->array_intrinsic_kind != kind ||
        call->array_element_storage != storage ||
        call->reserved8[0] != 0 || call->reserved8[1] != 0 ||
        call->reserved8[2] != 0 ||
        register_rep->kind != XR_MACHINE_REP_DYN_VALUE ||
    ...
    for (uint16_t ordinal = 0; ordinal < call->argument_count; ordinal++) {
        ...
        XrStableId expected_argument;
        if (!type || !caller ||
            !aot_pair_identity("xray-target-array-intrinsic-argument-v1",
                               operation->id, type->id, ordinal,
                               &expected_argument) ||
            !xr_stable_id_equal(argument->identity, expected_argument) ||
            argument->call != call->id ||
...
static bool oracle_direct_local_array_ref_place_storage(
    const VerifyAuthority *ctx, uint32_t semantic_value,
    XrRep *out_storage, uint16_t *out_machine_kind);
...
static bool oracle_direct_local_array_ref_parameter_place_storage(
    ...
    if (!parameter || !binding || !register_rep || !memory_rep || !slot ||
        !aot_array_ref_parameter_is_exact(ctx->semantic, parameter,
                                          &storage) ||
...
static bool oracle_dynamic_array_ref_storage(
    ...
    uint8_t storage = XR_TARGET_ARRAY_STORAGE_NONE;
    bool shared_value =
        aot_array_ref_shared_value_is_exact(ctx->semantic, operation,
                                            &storage) &&
        operation->result_value == semantic_value;
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands =
    ...
    bool place_load_value =
        ...
        operation->callable_function == XR_SEMANTIC_INDEX_NONE &&
        operation->import_resolution == XR_SEM_IMPORT_RESOLUTION_NONE &&
        place->type == operation->result_type &&
        ...
        aot_array_ref_type_is_exact(ctx->semantic, operation->result_type,
                                    &storage) &&
        (oracle_direct_local_array_ref_place_storage(
             ctx, place->value, &place_storage, &place_kind) ||
         oracle_direct_local_array_ref_parameter_place_storage(
             ctx, place->value, &place_storage, &place_kind)) &&
        place_storage == XR_REP_RAWPTR &&
        place_kind == XR_MACHINE_REP_RAW_PTR;
...
static bool oracle_raw_pointer_local_addr(const VerifyAuthority *ctx,
                                          uint32_t operation_index);

static bool oracle_raw_pointer_ref_place(
    const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    const XrSemanticOperandRecord *place =
        &operands[operation->operand_begin];
    if (place->value >= ctx->value_count ||
        place->role != XR_SEM_OPERAND_VALUE || place->parameter != -1 ||
    ...
    bool parameter_place = parameter &&
        parameter->function == operation->function &&
        parameter->value == place->value && parameter->type == place->type &&
        parameter->mode == XR_PARAM_REF &&
        parameter->ownership == XI_OWN_NONE &&
        parameter->transfer_mode == XR_TRANSFER_SHARE &&
        (parameter->flags & ~(XR_SEM_PARAMETER_REQUIRED |
                              XR_SEM_PARAMETER_VARIADIC)) == 0;
    bool local_place = !parameter &&
        place_operation_index < ctx->operation_count &&
    ...
    if (operation->opcode == XI_PLACE_LOAD) {
        ...
        if (!oracle_machine_storage(ctx, operation->result_value,
            ...
            result_storage != XR_REP_RAWPTR ||
            result_kind != XR_MACHINE_REP_RAW_PTR)
            return false;
    } else {
        const XrSemanticOperandRecord *stored = place + 1;
        if (stored->value >= ctx->value_count ||
            stored->type != place->type ||
            stored->role != XR_SEM_OPERAND_VALUE || stored->parameter != -1 ||
            stored->transfer_mode != XR_TRANSFER_SHARE ||
            stored->ownership_action != XR_SEM_OPERAND_CONSUME ||
            stored->parameter_mode != XR_PARAM_READ ||
        ...
        if (!oracle_machine_storage(ctx, stored->value, &stored_storage,
                                    &stored_kind) ||
            stored_storage != XR_REP_RAWPTR ||
            stored_kind != XR_MACHINE_REP_RAW_PTR)
            return false;
        if (out_stored)
            *out_stored = stored;
    }
...
static bool oracle_raw_pointer_local_addr(const VerifyAuthority *ctx,
    ...
    const XrSemanticOperandRecord *source =
        &operands[operation->operand_begin];
    if (source->value >= ctx->value_count ||
        source->type != operation->result_type ||
    ...
    uint16_t result_kind = XR_MACHINE_REP_COUNT;
    if (!oracle_machine_storage(ctx, source->value, &source_storage,
                                &source_kind) ||
        ...
        source_kind != XR_MACHINE_REP_RAW_PTR ||
        result_kind != XR_MACHINE_REP_RAW_PTR)
        return false;
    const XrTargetValueRepRecord *binding =
        xr_target_plan_value_rep(ctx->target_plan, operation->result_value);
...
static bool oracle_definition_storage(const VerifyAuthority *ctx,
    ...
    if (ctx->parameter_by_value[semantic_value] != XR_SEMANTIC_INDEX_NONE) {
         ...
         * own. */
        if (oracle_direct_local_array_ref_parameter_place_storage(
                ctx, semantic_value, out_storage, out_machine_kind))
            return true;
    ...
    switch (operation->opcode) {
        ...
        case XI_CALL_BUILTIN:
            ...
            if (aot_array_intrinsic_is_exact(ctx->semantic, operation, NULL,
                                             NULL))
                return oracle_dynamic_array_intrinsic_storage(
                    ctx, semantic_value, out_storage, out_machine_kind);
            return false;
        case XI_GET_BUILTIN:
        ...
        case XI_ARRAY_NEW:
            return oracle_dynamic_array_allocation_storage(
                ctx, semantic_value, out_storage, out_machine_kind);
        case XI_LOCAL_ADDR:
            if (oracle_direct_local_array_ref_place_storage(
                    ctx, semantic_value, out_storage, out_machine_kind))
                return true;
            ...
            return oracle_machine_storage(ctx, semantic_value, out_storage,
                                          out_machine_kind);
        case XI_PLACE_LOAD:
            if (oracle_dynamic_array_ref_storage(
                    ctx, semantic_value, out_storage, out_machine_kind))
                return true;
            if (!oracle_raw_pointer_ref_place(ctx, operation_index, NULL,
                                              NULL))
                return false;
            return oracle_machine_storage(ctx, semantic_value, out_storage,
                                          out_machine_kind);
        ...
        case XI_GO:
            return oracle_dynamic_direct_local_go_task_storage(
                ctx, semantic_value, out_storage, out_machine_kind);
        case XI_GET_SHARED: {
            if (oracle_dynamic_array_ref_storage(ctx, semantic_value,
                                                 out_storage,
                                                 out_machine_kind))
                return true;
            ...
            if (ctx->exact_go_callee_value &&
                ...
                return oracle_static_direct_local_go_callee_storage(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (oracle_dynamic_source_class_instance_storage(
                    ctx, semantic_value, out_storage, out_machine_kind))
                return true;
        ...
        case XI_CALL_METHOD:
            if (operation->intrinsic_kind == XR_SEM_INTRINSIC_ARRAY_HOF)
                return oracle_array_hof_result_storage(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (aot_array_fill_scalar_is_exact(ctx->semantic, operation,
                                               NULL, NULL, NULL))
                return oracle_dynamic_array_fill_scalar_storage(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (xr_semantic_string_slice_range_is_exact(
                    ctx->semantic, operation, NULL, NULL, NULL))
                return oracle_dynamic_string_slice_range_storage(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (xr_semantic_iterator_rune_has_next_is_exact(
            ...
            if (xr_semantic_iterator_rune_next_is_exact(
                    ctx->semantic, operation, NULL))
                return oracle_iterator_rune_next_call(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (xr_semantic_rune_to_uint32_is_exact(
                    ctx->semantic, operation, NULL))
                return oracle_rune_to_uint32_call(
                    ctx, semantic_value, out_storage, out_machine_kind);
            if (xr_semantic_rune_is_whitespace_is_exact(
                    ctx->semantic, operation, NULL))
                return oracle_rune_is_whitespace_call(
                    ctx, semantic_value, out_storage, out_machine_kind);
...
static bool oracle_direct_local_array_ref_place_use(
    const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    const XrSemanticOperandRecord *operands =
        xr_semantic_plan_operands(ctx->semantic, &operand_count);
    uint8_t storage = XR_TARGET_ARRAY_STORAGE_NONE;
    if (!place || !operands || place->opcode != XI_LOCAL_ADDR ||
        place->operand_count != 1 || place->operand_begin >= operand_count ||
        ...
        place->result_alias_operand != -1 ||
        place->intrinsic_kind != XR_SEM_INTRINSIC_NONE ||
        place->metadata_count != 0 ||
        place->auxiliary_kind != XI_AUX_KIND_NONE ||
        place->semantic_immediate != 0 ||
        place->constant != XR_SEMANTIC_INDEX_NONE ||
        place->callable_function != XR_SEMANTIC_INDEX_NONE ||
        place->import_resolution != XR_SEM_IMPORT_RESOLUTION_NONE ||
        !aot_array_ref_type_is_exact(ctx->semantic, place->result_type,
                                     &storage))
        return false;
    const XrTargetValueRepRecord *caller =
    ...
    const XrSemanticOperationRecord *call_operation =
        call ? xr_semantic_plan_operation(ctx->semantic,
    ...
    const XrTargetValueRepRecord *callee =
        parameter ? xr_target_plan_value_rep(ctx->target_plan,
                                             parameter->value)
                  : NULL;
    XrStableId expected_identity;
    if (!caller || !argument || !call || !target || !parameter ||
        !call_operation || !call_operand || !callee ||
        !aot_array_ref_parameter_is_exact(ctx->semantic, parameter,
                                          &storage) ||
        target->kind != XR_SEM_CALL_TARGET_DIRECT_LOCAL ||
        ...
        call_operand->value != place->result_value ||
        call_operand->type != parameter->type ||
        ...
        !aot_pair_identity("xray-target-direct-array-ref-argument-v1",
                           target->id, parameter->id, argument->ordinal,
                           &expected_identity) ||
...
static bool oracle_direct_local_array_ref_place_storage(
    ...
    if (!operation || !operands || operation->opcode != XI_LOCAL_ADDR ||
        ...
        operation->operand_count != 1 ||
        operation->operand_begin >= operand_count ||
        !oracle_direct_local_array_ref_place_use(
            ctx, operation_index,
...
static bool oracle_array_hof_use_storage(
    const VerifyAuthority *ctx, uint32_t operation_index,
    ...
    if (operand_index == 0 && source_value == shape.receiver) {
        if (!oracle_definition_storage(ctx, source_value,
                                       &definition_storage, &machine_kind) ||
            definition_storage != XR_REP_TAGGED ||
            machine_kind != XR_MACHINE_REP_DYN_VALUE)
            return false;
        *out_storage = XR_REP_TAGGED;
        return true;
    }
    if (operand_index == 1 && source_value == shape.callback) {
        if (!oracle_dynamic_closure_storage(ctx, source_value,
                                            &definition_storage,
                                            &machine_kind) ||
            definition_storage != XR_REP_TAGGED ||
            machine_kind != XR_MACHINE_REP_DYN_VALUE)
    ...
    return shape.kind == XR_TARGET_ARRAY_HOF_REDUCE &&
           operand_index == 2 && source_value == shape.initial &&
           oracle_machine_storage(ctx, source_value, out_storage,
                                  &machine_kind);
}
...
static bool oracle_use_storage(const VerifyAuthority *ctx,
    ...
    switch (operation->opcode) {
        ...
        case XI_PLACE_STORE: {
            ...
            if (operand_index > 1 ||
                !oracle_raw_pointer_ref_place(ctx, operation_index, &place,
                                              &stored) ||
                source_value != (operand_index == 0 ? place->value
                                                    : stored->value))
                return false;
            return oracle_machine_storage(ctx, source_value, out_storage,
                                          &ignored_kind);
        ...
        case XI_LOCAL_ADDR:
            if (operand_index != 0)
                return false;
            if (oracle_direct_local_array_ref_place_use(
                    ctx, operation_index, source_value)) {
                *out_storage = XR_REP_TAGGED;
                return true;
            }
            if (!oracle_raw_pointer_local_addr(ctx, operation_index))
        ...
        case XI_CALL_BUILTIN: {
            ...
            if (aot_array_intrinsic_is_exact(ctx->semantic, operation, NULL,
                                             NULL)) {
                XrRep result_storage = XR_REP_TAGGED;
                if (!oracle_dynamic_array_intrinsic_storage(
        ...
        case XI_RELEASE: {
            ...
            if (!oracle_dynamic_string_literal_storage(
                ...
                !oracle_dynamic_string_runes_storage(
                    ctx, source_value, &literal_storage, &ignored_kind) &&
                !oracle_dynamic_string_slice_range_storage(
                    ctx, source_value, &literal_storage, &ignored_kind) &&
        ...
        case XI_CALL_METHOD_DIRECT:
            if (operation->opcode == XI_CALL_METHOD &&
                operation->intrinsic_kind == XR_SEM_INTRINSIC_ARRAY_HOF)
                return oracle_array_hof_use_storage(
                    ctx, operation_index, operand_index, source_value,
                    out_storage);
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                if (operand_index >= 3 ||
                    !oracle_dynamic_string_slice_range_storage(
                        ctx, operation->result_value, &result_storage,
                        &ignored_kind) ||
                    !oracle_definition_storage(ctx, source_value,
                                               &source_storage, &source_kind))
                    return false;
                if (operand_index == 0) {
                    if (source_storage != XR_REP_TAGGED ||
                        source_kind != XR_MACHINE_REP_DYN_VALUE)
                        return false;
                    *out_storage = XR_REP_TAGGED;
                    return true;
                }
                if (source_storage != XR_REP_I64 ||
                    source_kind != XR_MACHINE_REP_I64)
                    return false;
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                aot_array_fill_scalar_is_exact(ctx->semantic, operation,
                                               NULL, NULL, NULL)) {
                XrRep result_storage = XR_REP_TAGGED;
                if (!oracle_dynamic_array_fill_scalar_storage(
                        ctx, operation->result_value, &result_storage,
                        &ignored_kind))
                    return false;
                if (operand_index == 0) {
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                XrRep result_storage = XR_REP_TAGGED;
                if (operand_index != 0 ||
                    !oracle_iterator_rune_next_call(
                        ctx, operation->result_value, &result_storage,
                        &ignored_kind) ||
                    !oracle_dynamic_string_runes_storage(
                        ctx, source_value, out_storage, &ignored_kind))
            ...
            if (operation->opcode == XI_CALL_METHOD &&
                ...
                XrRep result_storage = XR_REP_TAGGED;
                if (operand_index != 0 ||
                    !oracle_rune_is_whitespace_call(
                        ctx, operation->result_value, &result_storage,
                        &ignored_kind) ||
                    !oracle_iterator_rune_next_call(
                        ctx, source_value, out_storage, &ignored_kind))
...
static bool oracle_return_storage(const VerifyAuthority *ctx, uint32_t value,
    ...
    return oracle_machine_storage(ctx, value, out_storage, out_machine_kind) ||
           ...
           oracle_dynamic_string_slice_range_storage(
               ctx, value, out_storage, out_machine_kind) ||
           oracle_dynamic_string_concat_result_storage(ctx, value, out_storage,
                                                       out_machine_kind) ||
           oracle_array_hof_result_storage(ctx, value, out_storage,
                                           out_machine_kind) ||
...
static bool authority_collect_obligations_indexed(
    ...
    for (uint32_t i = 0; i < operation_count; i++) {
        ...
        for (uint16_t a = 0; a < operation->operand_count; a++) {
            ...
            bool has_definition = oracle_definition_storage(
                oracle, source_value, &input_storage, &ignored_machine);
            bool has_use = has_definition && oracle_use_storage(
                oracle, i, a, source_value, &output_storage);
