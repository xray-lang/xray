...
typedef struct CgAotStdlibMethod {
     ...
     * wrapper to its provider storage, 'r' passes a borrowed tagged slot address,
     * and '.' passes the value through. NULL
...
cg_native_direct_emission_view(XiCgenCtx *ctx, const XiFunc *function, const XiValue *value,
    ...
    for (uint16_t ordinal = 0; ordinal < argc; ordinal++) {
        ...
        bool reference = operand->parameter_mode == XR_PARAM_REF;
        uint16_t machine_kind = reference ? XR_MACHINE_REP_RAW_PTR : XR_MACHINE_REP_DYN_VALUE;
        ...
        if (!cg_value_semantic_id(ctx, function, value->args[ordinal + 1u], &live_argument) ||
            ...
            argument->mode != (reference ? XR_TARGET_CALL_REFERENCE : XR_TARGET_CALL_VALUE) ||
            argument->ownership != (reference ? XR_TARGET_CALL_BORROW : XR_TARGET_CALL_READ) ||
            ...
            !callee_memory || callee_register->kind != machine_kind ||
            callee_memory->kind != machine_kind)
            goto invalid;
    }
    ...
    for (uint16_t ordinal = 0; ordinal < argc; ordinal++) {
        const XrSemanticOperandRecord *operand =
            &semantic_operands[operation->operand_begin + ordinal + 1u];
        if (!method->source_provider_arg_spec || (method->source_provider_arg_spec[ordinal] ==
                                                  'r') != (operand->parameter_mode == XR_PARAM_REF))
            goto invalid;
    }
    if (out) {
        out->entry = entry;
