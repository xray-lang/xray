...
#define XR_C_PROGRAM_FUNCTION_SYMBOL_CAPACITY (sizeof("xr_pf_") - 1u + XR_STABLE_ID_BYTES * 2u + 1u)
...
XR_FUNC bool xr_c_program_initializer_symbol_identity(XrStableId module_identity,
                                                      XrStableId semantic_function_identity,
                                                      XrStableId *out);
XR_FUNC bool xr_c_program_direct_i64_emission_bind(const XrTargetPlan *target_plan,
                                                   struct XiModule *const *modules,
                                                   uint32_t module_count,
                                                   XrCProgramDirectI64EmissionBinding *out,
                                                   char *error, size_t error_size);
XR_FUNC void xr_c_program_direct_i64_emission_release(XrCProgramDirectI64EmissionBinding *binding);
XR_FUNC bool xr_c_program_direct_i64_emission_verify(
    const XrCProgramDirectI64EmissionBinding *binding, const XrTargetPlan *target_plan,
    struct XiModule *const *modules, uint32_t module_count, char *error, size_t error_size);
XR_FUNC const XrCProgramXiFunctionBinding *
xr_c_program_direct_i64_function_binding(const XrCProgramDirectI64EmissionBinding *binding,
                                         const struct XiFunc *function);
XR_FUNC bool xr_c_program_direct_i64_value_view(const XrCProgramDirectI64EmissionBinding *binding,
                                                const struct XiFunc *function,
                                                const struct XiValue *value,
                                                XrCValueEmissionView *out);
XR_FUNC bool
xr_c_program_direct_i64_function_abi_view(const XrCProgramDirectI64EmissionBinding *binding,
                                          const struct XiFunc *function, uint16_t ordinal,
                                          XrCFunctionAbiEmissionView *out);
XR_FUNC bool
xr_c_program_direct_i64_call_is_exact(const XrCProgramDirectI64EmissionBinding *binding,
                                      const struct XiFunc *caller, const struct XiValue *call);
XR_FUNC bool
xr_c_program_direct_i64_callee_operand_is_elided(const XrCProgramDirectI64EmissionBinding *binding,
                                                 const struct XiFunc *caller,
                                                 const struct XiValue *value);
