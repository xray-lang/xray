...
static XiModule *module_for_partition(const XrCProgramEmissionContext *ctx, uint32_t partition) {
    if (!ctx || partition >= ctx->partition_count)
        return NULL;
    const XrTargetModulePartitionRecord *row = &ctx->partitions[partition];
    const XrSemanticPlan *semantic = xr_target_plan_semantic_module(ctx->target, partition);
    XiModule *match = NULL;
...
bool xr_c_program_initializer_symbol_identity(XrStableId module_identity,
    ...
    int written = snprintf(key, sizeof(key), "xray-program-c-initializer-v1:module=%s:function=%s",
...
static bool bind_function(const XrCProgramEmissionContext *ctx,
    ...
    if (!functions || expected->target_function >= function_count ||
        ...
        return false;
    const XrTargetModulePartitionRecord *partition = &ctx->partitions[expected->partition];
    ...
    if (!xr_target_plan_function_semantic_binding(ctx->target, expected->target_function, &semantic,
                                                  &semantic_function) ||
    ...
    const XrProgramSemanticFunctionRecord *source =
        program && module ? xr_program_semantic_closure_function(module->program_semantic_closure,
                                                                 program->program_row)
                          : NULL;
    const XiFunc *xi_function =
        program && module ? xi_program_semantic_function_for_row(module, program->program_row)
                          : NULL;
...
static bool program_graph_is_private_leaf(const XrCProgramEmissionContext *ctx) {
    return ctx && ctx->graph &&
           ctx->graph->family == XR_PROGRAM_SEMANTIC_FAMILY_SOURCE_MODULE_SCALAR_PRIVATE_LEAF_CALL;
}

static uint32_t program_graph_argument_count(const XrCProgramEmissionContext *ctx) {
    return program_graph_is_private_leaf(ctx) ? 0u : 1u;
}

static bool locator_is_exact(XiSourceLocator xi, XrProgramSemanticSourceLocator psc) {
    return xi.kind != 0u && xi.kind == psc.kind && xi.span.start_line != 0u &&
           xi.span.start_line == psc.start_line && xi.span.start_column != 0u &&
           xi.span.start_column == psc.start_column && xi.span.end_line != 0u &&
           xi.span.end_line == psc.end_line && xi.span.end_column != 0u &&
...
static bool builder_callee_carrier_is_exact(const XrCProgramEmissionContext *ctx,
    ...
    const XiImportRef *ref = xi_value_import_ref(binding->caller.xi_function, carrier);
    XiModule *source_module = module_for_partition(ctx, binding->caller.target_partition);
    XiModule *callee_module = module_for_partition(ctx, binding->callee.target_partition);
    const XrProgramSemanticDependencyRecord *dependency =
        ref && source_module && source_module->program_semantic_closure
            ? xr_program_semantic_closure_dependency(source_module->program_semantic_closure,
    ...
    if (!ref || !dependency || !source_module || !callee_module)
        return false;
    bool carrier_exact = private_leaf ? carrier->op == XI_IMPORT_REF && carrier->aux == ref
    ...
    return carrier_exact &&
           xr_stable_id_equal(dependency->source_module,
                              ctx->partitions[binding->caller.target_partition].module_identity) &&
           xr_stable_id_equal(dependency->dependency_module,
                              ctx->partitions[binding->callee.target_partition].module_identity) &&
           (private_leaf ? stable_id_is_zero(dependency->exported_function)
                         : xr_stable_id_equal(dependency->exported_function,
                                              ctx->graph->producer_function_identity)) &&
           xr_stable_id_equal(dependency->resolver_binding, ctx->graph->resolver_binding) &&
           xr_stable_id_equal(ref->psc_resolver_binding, program_call->resolver_binding) &&
           xr_stable_id_equal(ref->psc_resolver_binding, dependency->resolver_binding) &&
           locator_is_exact(ref->psc_import_locator, dependency->import_locator) &&
           ...
           ref->resolved_func == binding->callee.xi_function && ref->resolved_shared_slot >= 0 &&
           ref->resolved_shared_slot < callee_module->nslots && callee_module->slot_funcs &&
           callee_module->slot_funcs[ref->resolved_shared_slot] == binding->callee.xi_function &&
           ref->resolved_export_slot >= 0 && ref->resolved_export_slot < callee_module->nexports &&
...
static bool machine_rep_c_projection(const XrTargetMachineRepRecord *row, XrCValueRep *rep,
                                     const char **c_type) {
    if (!row || !rep || !c_type)
        return false;
    ...
    return row->register_bits != 0u && row->memory_size != 0u && row->memory_align != 0u;
}

static const XiValue *function_value_for_local_id(const XiFunc *function, uint32_t local_value) {
    const XiValue *match = NULL;
    for (uint32_t parameter = 0; function && parameter < function->nparams; parameter++) {
        const XiValue *candidate = function->params ? function->params[parameter] : NULL;
...
static bool bind_initializer_value_owner(const XrCProgramEmissionContext *ctx, uint32_t partition,
                                         XrCProgramXiFunctionBinding *out) {
    ...
    uint32_t semantic_function =
        initializer ? initializer->semantic_plan_function_index : XR_SEMANTIC_INDEX_NONE;
    ...
    const XrTargetModulePartitionRecord *partition_row = &ctx->partitions[partition];
    const XrSemanticPlan *bound_semantic = NULL;
    uint32_t bound_function = XR_SEMANTIC_INDEX_NONE;
    XrStableId symbol_identity = {{0}};
    if (!initializer || !semantic || !semantic_row || !semantic_row->is_module_initializer ||
        initializer->semantic_plan != semantic || !target_functions ||
        !xr_target_plan_find_function(ctx->target, semantic, semantic_function, &target_function) ||
        target_function >= target_function_count ||
        target_function < partition_row->functions_begin ||
        target_function - partition_row->functions_begin >= partition_row->functions_count ||
        target_functions[target_function].id != target_function ||
        target_functions[target_function].semantic_function != semantic_function ||
        !xr_target_plan_function_semantic_binding(ctx->target, target_function, &bound_semantic,
                                                  &bound_function) ||
        bound_semantic != semantic || bound_function != semantic_function ||
        !xr_c_program_initializer_symbol_identity(partition_row->module_identity, semantic_row->id,
                                                  &symbol_identity))
        return false;
    *out = (XrCProgramXiFunctionBinding) {
...
static bool project_value(const XrCProgramEmissionContext *ctx,
    ...
    const XrSemanticPlan *semantic =
        ctx && function ? xr_target_plan_semantic_module(ctx->target, function->target_partition)
                        : NULL;
    const XrSemanticFunctionRecord *semantic_function =
        semantic ? xr_semantic_plan_function(semantic, function->semantic_function) : NULL;
    if (!ctx || !function || !value || !out || !semantic_function || value->block == NULL ||
        value->block->func != function->xi_function ||
    ...
    if (!xr_aot_scalar_program_semantic_value_id(
            ctx->target, function->target_partition, function->target_function,
            function->xi_function, value, &semantic_function_index, &semantic_value, NULL, 0) ||
        semantic_function_index != function->semantic_function ||
        semantic_value != semantic_function->value_begin + value->id)
        return false;
    const XrTargetValueRepRecord *target_value = xr_target_plan_value_rep_for_module(
        ctx->target, function->target_partition, semantic_value);
    const XrTargetMachineRepRecord *register_rep =
        target_value ? xr_target_plan_machine_rep(ctx->target, target_value->register_rep) : NULL;
    const XrTargetMachineRepRecord *memory_rep =
        target_value ? xr_target_plan_machine_rep(ctx->target, target_value->memory_rep) : NULL;
    XrCValueRep c_rep = XR_C_VALUE_REP_COUNT;
    const char *c_type = NULL;
    if (!target_value || target_value->semantic_value != semantic_value || !register_rep ||
...
static bool bind_direct_call(const XrCProgramEmissionContext *ctx,
    ...
    uint32_t function_count = 0, call_count = 0, argument_count = 0, instruction_count = 0;
    ...
    if (!functions || out->caller.target_function >= function_count ||
        ...
        (expected_arguments == 0u ? ctx->graph->target_argument != XR_SEMANTIC_INDEX_NONE
    ...
    const XrSemanticProgramCallBinding *program_call =
        entry ? xr_semantic_plan_program_call_for_operation(entry, call->semantic_operation) : NULL;
    const XiValue *xi_call =
        program_call
            ? xi_program_semantic_call_for_row(out->caller.xi_function, program_call->program_row)
            : NULL;
    if (!program_call || !xi_call || xi_call->block == NULL ||
        xi_call->block->func != out->caller.xi_function || xi_call->op != XI_CALL ||
        xi_call->nargs != (uint16_t) (expected_arguments + 1u) || !xi_call->args ||
        !xi_call->args[0] ||
        (expected_arguments != 0u && (!xi_call->args[1] || xi_call->args[1]->block == NULL ||
        ...
        !xr_stable_id_equal(program_call->callee_program_function,
                            ctx->graph->producer_function_identity) ||
        !xr_stable_id_equal(program_call->resolver_binding, ctx->graph->resolver_binding) ||
        !builder_callee_carrier_is_exact(ctx, out, program_call, xi_call->args[0]))
        return false;
    ...
    for (uint32_t block_index = 0; block_index < out->caller.xi_function->nblocks; block_index++) {
        ...
        for (uint32_t value_index = 0; value_index < block->nvalues; value_index++) {
            ...
            for (uint16_t argument = 0; user && argument < user->nargs; argument++) {
    ...
    if (!caller_semantic ||
        (expected_arguments != 0u &&
         (xi_call->args[1]->id >= caller_semantic->value_count ||
          ...
          argument->semantic_value != caller_semantic->value_begin + xi_call->args[1]->id)) ||
    ...
    out->target_argument =
        expected_arguments ? ctx->graph->target_argument : XR_SEMANTIC_INDEX_NONE;
...
static bool verifier_callee_operand_has_one_use(const XiFunc *caller, const XiValue *call,
                                                const XiValue *operand);

static const XiValue *
bind_private_leaf_native_callee_operand(const XrCProgramEmissionContext *ctx,
                                        const XrCProgramDirectI64EmissionBinding *binding) {
    ...
    const XrSemanticPlan *semantic =
        xr_target_plan_semantic_module(ctx->target, binding->callee.target_partition);
    const XrSemanticProgramCallBinding *program_call =
        semantic && target_call
            ? xr_semantic_plan_program_call_for_operation(semantic, target_call->semantic_operation)
            : NULL;
    const XiValue *xi_call = program_call
                                 ? xi_program_semantic_call_for_row(binding->callee.xi_function,
                                                                    program_call->program_row)
                                 : NULL;
    const XiValue *carrier =
        xi_call && xi_call->nargs == 1u && xi_call->args ? xi_call->args[0] : NULL;
    const XiImportRef *ref =
        carrier ? xi_value_import_ref(binding->callee.xi_function, carrier) : NULL;
    uint32_t semantic_function = XR_SEMANTIC_INDEX_NONE;
    uint32_t semantic_value = XR_SEMANTIC_INDEX_NONE;
    if (!target_call || !program_call || !xi_call || !carrier || !ref || xi_call->op != XI_CALL ||
        carrier->op != XI_IMPORT_REF || carrier->aux != ref ||
        !xi_import_ref_is_grounded_native(ref) ||
        !verifier_callee_operand_has_one_use(binding->callee.xi_function, xi_call, carrier) ||
        ...
        !xr_aot_scalar_program_semantic_value_id(
            ctx->target, binding->callee.target_partition, binding->callee.target_function,
            binding->callee.xi_function, xi_call, &semantic_function, &semantic_value, NULL, 0) ||
...
static bool append_function_values(const XrCProgramEmissionContext *ctx,
                                   const XrCProgramXiFunctionBinding *function,
                                   const XiValue *elided, XrCProgramValueEmissionBinding *values,
                                   uint32_t capacity, uint32_t *count) {
    const XrSemanticPlan *semantic =
        xr_target_plan_semantic_module(ctx->target, function->target_partition);
    const XrSemanticFunctionRecord *semantic_function =
        semantic ? xr_semantic_plan_function(semantic, function->semantic_function) : NULL;
    if (!semantic_function || !values || !count ||
        semantic_function->value_count > capacity - *count)
        return false;
    for (uint32_t local_value = 0; local_value < semantic_function->value_count; local_value++) {
        const XiValue *value = function_value_for_local_id(function->xi_function, local_value);
...
static bool project_function_abi(const XrCProgramEmissionContext *ctx,
                                 const XrCProgramDirectI64EmissionBinding *binding,
    ...
    if (!semantic_function || !out || ordinal > semantic_function->parameter_count ||
        ...
        (semantic_function->semantic_effects & (XI_EFFECT_MAY_THROW | XI_EFFECT_MAY_SUSPEND)) != 0u)
        return false;
    ...
    if (ordinal != 0u) {
        const XrSemanticParameterRecord *parameter = xr_semantic_plan_parameter(
            semantic, semantic_function->parameter_begin + (uint32_t) ordinal - 1u);
        live_value = function->xi_function->params && ordinal <= function->xi_function->nparams
        ...
        uint32_t matches = 0;
        for (uint32_t block_index = 0; block_index < function->xi_function->nblocks;
             block_index++) {
            const XiBlock *block =
                function->xi_function->blocks ? function->xi_function->blocks[block_index] : NULL;
            ...
            const XrSemanticBlockRecord *semantic_block =
                xr_semantic_plan_block(semantic, semantic_function->block_begin + block->id);
            if (!semantic_block || semantic_block->function != function->semantic_function ||
                semantic_block->kind != XI_BLOCK_RETURN ||
                semantic_block->control_value == XR_SEMANTIC_INDEX_NONE ||
                (matches != 0u && semantic_value != semantic_block->control_value))
                return false;
    ...
    if (function == &binding->caller) {
        if (ordinal != 0u || !binding->call_row ||
            ...
            binding->call_row->result_register_rep != value.emission.target_register_rep ||
            binding->call_row->result_memory_rep != value.emission.target_memory_rep)
            return false;
        ...
        if (ordinal == 0u) {
            ...
            if (!binding->argument_row ||
                binding->argument_row->callee_parameter != semantic_function->parameter_begin ||
                binding->argument_row->callee_register_rep != value.emission.target_register_rep ||
                binding->argument_row->callee_memory_rep != value.emission.target_memory_rep)
                return false;
        } else {
            return false;
        }
...
static bool project_initializer_abi(const XrCProgramEmissionContext *ctx,
                                    const XrCProgramXiFunctionBinding *function,
                                    XrCProgramFunctionAbiEmissionBinding *out) {
    const XrSemanticPlan *semantic =
        ctx && function ? xr_target_plan_semantic_module(ctx->target, function->target_partition)
    ...
    const XrSemanticFunctionRecord *semantic_function =
        semantic ? xr_semantic_plan_function(semantic, function->semantic_function) : NULL;
    if (!semantic_function || !out || !semantic_function->is_module_initializer ||
        semantic_function->parameter_count != 0u || function->xi_function == NULL ||
        function->xi_function->semantic_plan != semantic ||
        function->xi_function->semantic_plan_function_index != function->semantic_function)
        return false;
    *out = (XrCProgramFunctionAbiEmissionBinding) {
        .xi_function = function->xi_function,
        .target_function = function->target_function,
...
static bool build_emission_rows(const XrCProgramEmissionContext *ctx,
                                XrCProgramDirectI64EmissionBinding *binding, char *error,
                                size_t error_size) {
    const XrSemanticPlan *caller_semantic =
        xr_target_plan_semantic_module(ctx->target, binding->caller.target_partition);
    const XrSemanticPlan *callee_semantic =
        xr_target_plan_semantic_module(ctx->target, binding->callee.target_partition);
    const XrSemanticFunctionRecord *caller_function =
        caller_semantic
            ? xr_semantic_plan_function(caller_semantic, binding->caller.semantic_function)
            : NULL;
    const XrSemanticFunctionRecord *callee_function =
        callee_semantic
            ? xr_semantic_plan_function(callee_semantic, binding->callee.semantic_function)
            : NULL;
    ...
    if (!caller_function || !callee_function || caller_function->parameter_count != 0u ||
        callee_function->parameter_count != callee_parameter_count ||
        (program_graph_is_private_leaf(ctx) && !native_leaf_callee_operand) ||
        caller_function->value_count > UINT32_MAX - callee_function->value_count)
    ...
    uint32_t value_count = caller_function->value_count + callee_function->value_count;
    XrCProgramXiFunctionBinding *initializers =
        (XrCProgramXiFunctionBinding *) xr_calloc(ctx->partition_count, sizeof(*initializers));
    if (!initializers)
        return emission_fail(error, error_size, "program initializer binding allocation failed");
    ...
    for (uint32_t partition = 0; partition < ctx->partition_count; partition++) {
        if (!bind_initializer_value_owner(ctx, partition, &initializers[partition]))
            return emission_fail(error, error_size,
        ...
        for (uint32_t prior = 0; prior < partition; prior++)
            if (memcmp(initializers[partition].c_symbol, initializers[prior].c_symbol,
                       sizeof(initializers[partition].c_symbol)) == 0)
                return emission_fail(error, error_size, "program initializer C symbols collide");
        const XrSemanticPlan *semantic =
            xr_target_plan_semantic_module(ctx->target, initializers[partition].target_partition);
        const XrSemanticFunctionRecord *initializer =
            semantic
                ? xr_semantic_plan_function(semantic, initializers[partition].semantic_function)
                : NULL;
    ...
    XrCProgramValueEmissionBinding *values =
        value_count ? (XrCProgramValueEmissionBinding *) xr_calloc(value_count, sizeof(*values))
                    : NULL;
    if (value_count && !values)
        return emission_fail(error, error_size, "program C-emission value allocation failed");
    uint32_t projected = 0;
    if (!append_function_values(ctx, &binding->caller, binding->xi_callee_operand, values,
                                value_count, &projected)) {
        xr_free(values);
        return emission_fail(error, error_size,
                             "caller values have no exact C-emission projection");
    }
    if (!append_function_values(ctx, &binding->callee, native_leaf_callee_operand, values,
                                value_count, &projected)) {
        xr_free(values);
        return emission_fail(error, error_size,
    ...
    for (uint32_t partition = 0; partition < ctx->partition_count; partition++) {
        if (!append_function_values(ctx, &initializers[partition], NULL, values, value_count,
                                    &projected)) {
            xr_free(values);
            return emission_fail(error, error_size,
    ...
    if (ctx->partition_count > UINT32_MAX - program_abi_count) {
        xr_free(values);
        return emission_fail(error, error_size, "program initializer ABI coverage overflowed");
    }
    uint32_t abi_count = program_abi_count + ctx->partition_count;
    XrCProgramFunctionAbiEmissionBinding *abis =
        (XrCProgramFunctionAbiEmissionBinding *) xr_calloc(abi_count, sizeof(*abis));
    if (!abis) {
        xr_free(values);
        return emission_fail(error, error_size, "program C-emission ABI allocation failed");
    }
    ...
    for (uint32_t i = 0; i < ctx->partition_count; i++) {
        if (project_initializer_abi(ctx, &initializers[i], &abis[program_abi_count + i]))
            continue;
        xr_free(abis);
        xr_free(values);
        return emission_fail(error, error_size,
...
static bool prepare_context(const XrTargetPlan *target_plan, XiModule *const *modules,
                            uint32_t module_count, XrCProgramEmissionContext *out, char *error,
                            size_t error_size) {
    if (error && error_size)
        error[0] = '\0';
    ...
    if (!graphs || graph_count != 1u || !partitions || partition_count != module_count ||
        ...
        !supported_family || graphs[0].module_count != module_count ||
    ...
    *out = (XrCProgramEmissionContext) {
    ...
    XiModule *entry_module = module_for_partition(out, graphs[0].entry_partition);
    ...
    if (entry_index == UINT32_MAX ||
        !xi_program_semantic_verify_module_set(modules, module_count, entry_index, NULL, error,
                                               error_size) ||
        !xi_program_semantic_plan_verify_module_set(modules, module_count, entry_index, error,
                                                    error_size))
        return emission_fail(error, error_size, "program Xi module set is not exact");
    return true;
}

static void graph_function_expectations(const XrTargetProgramGraphRecord *graph,
...
static bool value_binding_equal(const XrCProgramValueEmissionBinding *left,
                                const XrCProgramValueEmissionBinding *right) {
    if (!left || !right || left->xi_function != right->xi_function ||
        left->xi_value != right->xi_value || left->target_function != right->target_function ||
...
#define XR_C_STRING_FIELD_EQUAL(field)                                                             \
    ((!a->field && !b->field) || (a->field && b->field && strcmp(a->field, b->field) == 0))
    if (!XR_C_STRING_FIELD_EQUAL(c_type) || !XR_C_STRING_FIELD_EQUAL(backing_c_type) ||
        !XR_C_STRING_FIELD_EQUAL(literal_bytes) || !XR_C_STRING_FIELD_EQUAL(recipe_symbol) ||
        !XR_C_STRING_FIELD_EQUAL(recipe_type_name) ||
        !XR_C_STRING_FIELD_EQUAL(recipe_member_name) || a->recipe_arguments != b->recipe_arguments)
        return false;
#undef XR_C_STRING_FIELD_EQUAL
    return a->semantic_value == b->semantic_value &&
           ...
           a->target_memory_kind == b->target_memory_kind && a->register_bits == b->register_bits &&
           a->memory_align == b->memory_align && a->memory_size == b->memory_size &&
           a->rep == b->rep && a->materialization == b->materialization &&
           a->reserved == b->reserved && a->literal_byte_length == b->literal_byte_length &&
           ...
           a->recipe_hof_callback_parameter_reps[0] == b->recipe_hof_callback_parameter_reps[0] &&
           a->recipe_hof_callback_parameter_reps[1] == b->recipe_hof_callback_parameter_reps[1] &&
           a->recipe_hof_callback_return_rep == b->recipe_hof_callback_return_rep &&
...
static bool abi_binding_equal(const XrCProgramFunctionAbiEmissionBinding *left,
    ...
    if ((!a->c_type != !b->c_type) || (a->c_type && strcmp(a->c_type, b->c_type) != 0) ||
        (!a->pointee_c_type != !b->pointee_c_type) ||
        (a->pointee_c_type && strcmp(a->pointee_c_type, b->pointee_c_type) != 0))
        return false;
    return a->semantic_function == b->semantic_function && a->semantic_value == b->semantic_value &&
           a->ordinal == b->ordinal && a->parameter_count == b->parameter_count &&
           a->target_register_kind == b->target_register_kind &&
           a->target_memory_kind == b->target_memory_kind && a->slot_class == b->slot_class &&
           a->boundary_kind == b->boundary_kind && a->rep == b->rep &&
           a->pointee_rep == b->pointee_rep && a->aggregate_class == b->aggregate_class;
}

static bool verifier_symbol_is_exact(XrStableId identity,
                                     const char symbol[XR_C_PROGRAM_FUNCTION_SYMBOL_CAPACITY]) {
    static const char hex[] = "0123456789abcdef";
    static const char prefix[] = "xr_pf_";
    if (!symbol ||
        memchr(symbol, '\0', XR_C_PROGRAM_FUNCTION_SYMBOL_CAPACITY) !=
...
static bool verifier_initializer_symbol_identity(XrStableId module_identity,
                                                 XrStableId semantic_function_identity,
                                                 XrStableId *out) {
    char module_hex[XR_STABLE_ID_BYTES * 2u + 1u];
    char function_hex[XR_STABLE_ID_BYTES * 2u + 1u];
...
static bool verifier_function_binding_is_exact(const XrCProgramEmissionContext *ctx,
                                               const XrCProgramFunctionExpectation *expected,
...
static bool verifier_initializer_value_owner(const XrCProgramEmissionContext *ctx,
                                             uint32_t partition,
    ...
    if (!semantic || !module || !initializer || !semantic_row ||
        !semantic_row->is_module_initializer || initializer->semantic_plan != semantic ||
        !target_functions ||
        !xr_target_plan_find_function(ctx->target, semantic, semantic_function, &target_function) ||
        target_function >= target_function_count ||
        target_function < partition_row->functions_begin ||
        ...
        target_functions[target_function].id != target_function ||
        target_functions[target_function].semantic_function != semantic_function ||
        !xr_target_plan_function_semantic_binding(ctx->target, target_function, &target_semantic,
                                                  &target_semantic_function) ||
        target_semantic != semantic || target_semantic_function != semantic_function ||
        !verifier_initializer_symbol_identity(partition_row->module_identity, semantic_row->id,
                                              &symbol_identity))
        return false;
    return actual->xi_function == initializer && actual->target_function == target_function &&
...
static bool verifier_callee_carrier_is_exact(const XrCProgramEmissionContext *ctx,
                                             const XrCProgramDirectI64EmissionBinding *binding,
                                             const XrSemanticProgramCallBinding *program_call,
                                             const XiValue *carrier) {
    if (!ctx || !binding || !program_call || !carrier || !carrier->block ||
    ...
    if (!carrier_exact ||
        ...
        !locator_is_exact(ref->psc_import_locator, dependency->import_locator) ||
        !ref->resolution_attempted || callee_module_index == UINT32_MAX ||
        ref->resolved_mod_index < 0 || (uint32_t) ref->resolved_mod_index != callee_module_index ||
        ref->resolved_module != callee_module ||
        ref->resolved_func != binding->callee.xi_function || ref->resolved_shared_slot < 0 ||
        ref->resolved_shared_slot >= callee_module->nslots || !callee_module->slot_funcs ||
        callee_module->slot_funcs[ref->resolved_shared_slot] != binding->callee.xi_function ||
        ref->resolved_export_slot < 0 || ref->resolved_export_slot >= callee_module->nexports ||
        !callee_module->exports ||
        callee_module->exports[ref->resolved_export_slot].function != binding->callee.xi_function ||
        callee_module->exports[ref->resolved_export_slot].shared_slot != ref->resolved_shared_slot)
        return false;
    return true;
}

static bool verifier_call_binding_is_exact(const XrCProgramEmissionContext *ctx,
                                           const XrCProgramDirectI64EmissionBinding *binding) {
    uint32_t function_count = 0, call_count = 0, argument_count = 0, instruction_count = 0;
    uint32_t expected_arguments = program_graph_argument_count(ctx);
    const XrTargetFunctionRecord *functions =
    ...
    const XrTargetInstructionRecord *instructions =
        xr_target_plan_instructions(ctx->target, &instruction_count);
    if (!functions || !calls || (expected_arguments != 0u && !arguments) || !instructions ||
        ctx->graph->target_call >= call_count ||
        (expected_arguments == 0u ? ctx->graph->target_argument != XR_SEMANTIC_INDEX_NONE
                                  : ctx->graph->target_argument >= argument_count) ||
    ...
    const XrTargetMachineRepRecord *argument_register =
        argument ? xr_target_plan_machine_rep(ctx->target, argument->register_rep) : NULL;
    const XrTargetMachineRepRecord *argument_memory =
        argument ? xr_target_plan_machine_rep(ctx->target, argument->memory_rep) : NULL;
    const XrTargetMachineRepRecord *callee_register =
        argument ? xr_target_plan_machine_rep(ctx->target, argument->callee_register_rep) : NULL;
    const XrTargetMachineRepRecord *callee_memory =
        argument ? xr_target_plan_machine_rep(ctx->target, argument->callee_memory_rep) : NULL;
    if (call->id != ctx->graph->target_call ||
        call->caller_function != ctx->graph->entry_target_function ||
        call->callee_function != ctx->graph->producer_target_function ||
        call->calling_convention != XR_TARGET_CALL_CONVENTION_PROGRAM_DIRECT ||
        call->target_kind != XR_TARGET_CALL_TARGET_PROGRAM_DIRECT ||
        call->argument_count != expected_arguments || !result_register ||
        result_register->kind != XR_MACHINE_REP_I64 || !result_memory ||
        result_memory->kind != XR_MACHINE_REP_I64 ||
        (expected_arguments != 0u &&
         (call->argument_begin != ctx->graph->target_argument ||
          argument->call != ctx->graph->target_call ||
          argument->ordinal != ctx->graph->argument_ordinal ||
          argument->caller_slot != ctx->graph->caller_slot ||
          argument->callee_slot != ctx->graph->callee_slot || !argument_register ||
          argument_register->kind != XR_MACHINE_REP_I64 || !argument_memory ||
          argument_memory->kind != XR_MACHINE_REP_I64 || !callee_register ||
          callee_register->kind != XR_MACHINE_REP_I64 || !callee_memory ||
    ...
    const XrSemanticProgramCallBinding *program_call =
        entry ? xr_semantic_plan_program_call_for_operation(entry, call->semantic_operation) : NULL;
    const XiValue *xi_call = program_call
                                 ? xi_program_semantic_call_for_row(binding->caller.xi_function,
                                                                    program_call->program_row)
                                 : NULL;
    if (!program_call || !xi_call || xi_call->op != XI_CALL ||
        xi_call->nargs != (uint16_t) (expected_arguments + 1u) || !xi_call->args ||
        !xi_call->args[0] || (expected_arguments != 0u && !xi_call->args[1]) ||
        xi_call->block == NULL || xi_call->block->func != binding->caller.xi_function ||
        !xr_stable_id_equal(program_call->program_call, ctx->graph->call_identity) ||
        !xr_stable_id_equal(program_call->callsite, ctx->graph->callsite_identity) ||
        !xr_stable_id_equal(program_call->caller_program_function,
                            ctx->graph->entry_function_identity) ||
        !xr_stable_id_equal(program_call->callee_program_function,
                            ctx->graph->producer_function_identity) ||
        !xr_stable_id_equal(program_call->resolver_binding, ctx->graph->resolver_binding) ||
        !verifier_callee_carrier_is_exact(ctx, binding, program_call, xi_call->args[0]) ||
        program_call->operation != ctx->graph->entry_semantic_operation ||
        !verifier_callee_operand_has_one_use(binding->caller.xi_function, xi_call,
                                             xi_call->args[0]))
        return false;
    const XrTargetInstructionRecord *instruction = NULL;
    ...
    if (!instruction || instruction->id != instruction_index ||
        instruction->result_slot != call->result_slot ||
        !xr_aot_scalar_program_semantic_value_id(
            ctx->target, binding->caller.target_partition, binding->caller.target_function,
            binding->caller.xi_function, xi_call, &call_function, &call_value, NULL, 0) ||
        (expected_arguments != 0u &&
         !xr_aot_scalar_program_semantic_value_id(ctx->target, binding->caller.target_partition,
                                                  binding->caller.target_function,
                                                  binding->caller.xi_function, xi_call->args[1],
                                                  &argument_function, &argument_value, NULL, 0)) ||
        call_function != binding->caller.semantic_function ||
        (expected_arguments != 0u && argument_function != binding->caller.semantic_function) ||
        call->result_value != call_value ||
        (expected_arguments != 0u && argument->semantic_value != argument_value))
        return false;
    return binding->xi_call == xi_call &&
           binding->xi_argument == (expected_arguments ? xi_call->args[1] : NULL) &&
           binding->xi_callee_operand == xi_call->args[0] && binding->callee_operand_elided &&
           binding->caller_target_row == &functions[binding->caller.target_function] &&
           binding->callee_target_row == &functions[binding->callee.target_function] &&
...
static bool verifier_value_binding_is_exact(const XrCProgramEmissionContext *ctx,
                                            const XrCProgramXiFunctionBinding *function,
                                            const XiValue *value,
    ...
    if (!binding ||
        !xr_aot_scalar_program_semantic_value_id(
            ctx->target, function->target_partition, function->target_function,
            function->xi_function, value, &semantic_function, &semantic_value, NULL, 0) ||
        semantic_function != function->semantic_function)
        return false;
    const XrTargetValueRepRecord *target_value = xr_target_plan_value_rep_for_module(
        ctx->target, function->target_partition, semantic_value);
...
static const XrCProgramValueEmissionBinding *
verifier_find_value(const XrCProgramDirectI64EmissionBinding *binding, const XiFunc *function,
                    const XiValue *value) {
    const XrCProgramValueEmissionBinding *match = NULL;
    for (uint32_t i = 0; binding && binding->values && i < binding->value_count; i++) {
        if (binding->values[i].xi_function != function || binding->values[i].xi_value != value)
            continue;
        if (match)
            return NULL;
...
static bool verifier_abi_binding_is_exact(const XrCProgramEmissionContext *ctx,
                                          const XrCProgramDirectI64EmissionBinding *binding,
                                          const XrCProgramXiFunctionBinding *function,
                                          uint16_t ordinal,
    ...
    const XrSemanticFunctionRecord *semantic_function =
        semantic ? xr_semantic_plan_function(semantic, function->semantic_function) : NULL;
    if (!semantic_function || !actual || ordinal > semantic_function->parameter_count ||
        semantic_function->is_module_initializer || semantic_function->capture_count != 0u ||
        (semantic_function->semantic_effects & (XI_EFFECT_MAY_THROW | XI_EFFECT_MAY_SUSPEND)) != 0u)
        return false;
    const XiValue *live_value = NULL;
    ...
    if (ordinal == 0u) {
        ...
        const XrSemanticParameterRecord *parameter =
            xr_semantic_plan_parameter(semantic, semantic_function->parameter_begin + ordinal - 1u);
        live_value = function->xi_function->params && ordinal <= function->xi_function->nparams
                         ? function->xi_function->params[ordinal - 1u]
                         : NULL;
        if (!parameter || parameter->function != function->semantic_function ||
    ...
    const XrCProgramValueEmissionBinding *value =
        verifier_find_value(binding, function->xi_function, live_value);
    ...
    if (function == &binding->caller) {
        ...
        if (ordinal == 0u) {
            if (binding->call_row->result_register_rep != value->emission.target_register_rep ||
                binding->call_row->result_memory_rep != value->emission.target_memory_rep)
                return false;
        } else if (ordinal == 1u) {
            if (binding->argument_row->callee_parameter != semantic_function->parameter_begin ||
                binding->argument_row->callee_register_rep != value->emission.target_register_rep ||
                binding->argument_row->callee_memory_rep != value->emission.target_memory_rep)
                return false;
        } else {
...
static bool
verifier_initializer_abi_binding_is_exact(const XrCProgramEmissionContext *ctx,
                                          const XrCProgramXiFunctionBinding *function,
                                          const XrCProgramFunctionAbiEmissionBinding *actual) {
    const XrSemanticPlan *semantic =
        ctx && function ? xr_target_plan_semantic_module(ctx->target, function->target_partition)
...
void xr_c_program_direct_i64_emission_release(XrCProgramDirectI64EmissionBinding *binding) {
    if (!binding)
        return;
...
bool xr_c_program_direct_i64_emission_verify(const XrCProgramDirectI64EmissionBinding *binding,
                                             const XrTargetPlan *target_plan,
                                             XiModule *const *modules, uint32_t module_count,
                                             char *error, size_t error_size) {
    XrCProgramEmissionContext ctx = {0};
    if (!binding || binding->schema_version != XR_C_PROGRAM_DIRECT_I64_EMISSION_SCHEMA_VERSION ||
        !binding->verified ||
        !prepare_context(target_plan, modules, module_count, &ctx, error, error_size) ||
        !xr_fingerprint_equal(binding->target_fingerprint, xr_target_plan_fingerprint(target_plan)))
        return emission_fail(error, error_size, "program C-emission binding header is invalid");

    XrCProgramFunctionExpectation caller_expectation = {0};
    XrCProgramFunctionExpectation callee_expectation = {0};
    graph_function_expectations(ctx.graph, &caller_expectation, &callee_expectation);
    if (!verifier_function_binding_is_exact(&ctx, &caller_expectation, &binding->caller) ||
        !verifier_function_binding_is_exact(&ctx, &callee_expectation, &binding->callee) ||
        ...
        return emission_fail(error, error_size, "program function/call emission binding changed");
    ...
    if (!caller_function || !callee_function || !binding->initializers ||
        ...
        binding->xi_native_leaf_callee_operand != native_leaf_callee_operand ||
        (program_graph_is_private_leaf(&ctx) && !native_leaf_callee_operand) ||
        binding->function_abi_count != program_abi_count + binding->initializer_count)
        return emission_fail(error, error_size, "program C-emission row coverage is incomplete");
    ...
    for (uint32_t fi = 0; fi < 2u; fi++) {
        const XrSemanticPlan *semantic =
            xr_target_plan_semantic_module(ctx.target, functions[fi]->target_partition);
        const XrSemanticFunctionRecord *function =
            semantic ? xr_semantic_plan_function(semantic, functions[fi]->semantic_function) : NULL;
        for (uint32_t local = 0; function && local < function->value_count; local++) {
            const XiValue *xi_value =
                function_value_for_local_id(functions[fi]->xi_function, local);
            if (!xi_value || (fi == 0u && xi_value == binding->xi_callee_operand) ||
                (fi == 1u && xi_value == binding->xi_native_leaf_callee_operand))
                continue;
            if (!binding->values || value_index >= binding->value_count ||
                !verifier_value_binding_is_exact(&ctx, functions[fi], xi_value,
                                                 &binding->values[value_index]))
                return emission_fail(
                    error, error_size,
                    "program value C-emission row %u (function %u local %u) changed", value_index,
                    functions[fi]->target_function, local);
            value_index++;
        }
    }
    for (uint32_t partition = 0; partition < binding->initializer_count; partition++) {
        const XrCProgramXiFunctionBinding *initializer = &binding->initializers[partition];
        if (!verifier_initializer_value_owner(&ctx, partition, initializer))
            return emission_fail(error, error_size, "program initializer TargetPlan owner changed");
        ...
        for (uint32_t prior = 0; prior < partition; prior++)
            if (memcmp(initializer->c_symbol, binding->initializers[prior].c_symbol,
                       sizeof(initializer->c_symbol)) == 0)
                return emission_fail(error, error_size, "program initializer C symbols collide");
        const XrSemanticPlan *semantic =
            xr_target_plan_semantic_module(ctx.target, initializer->target_partition);
        const XrSemanticFunctionRecord *function =
            semantic ? xr_semantic_plan_function(semantic, initializer->semantic_function) : NULL;
        for (uint32_t local = 0; function && local < function->value_count; local++) {
            const XiValue *xi_value = function_value_for_local_id(initializer->xi_function, local);
            if (!xi_value)
                continue;
            if (value_index >= binding->value_count ||
                !verifier_value_binding_is_exact(&ctx, initializer, xi_value,
                                                 &binding->values[value_index]))
                return emission_fail(
                    error, error_size,
    ...
    if (value_index != binding->value_count)
        return emission_fail(error, error_size, "program value C-emission coverage is incomplete");

    if (!verifier_abi_binding_is_exact(&ctx, binding, &binding->caller, 0u,
                                       &binding->function_abis[0]) ||
        !verifier_abi_binding_is_exact(&ctx, binding, &binding->callee, 0u,
                                       &binding->function_abis[1]) ||
        (callee_parameter_count != 0u &&
         !verifier_abi_binding_is_exact(&ctx, binding, &binding->callee, 1u,
                                        &binding->function_abis[2])))
        return emission_fail(error, error_size, "program function ABI emission row changed");
    for (uint32_t i = 0; i < binding->initializer_count; i++) {
        if (!verifier_initializer_abi_binding_is_exact(
                &ctx, &binding->initializers[i], &binding->function_abis[program_abi_count + i]))
            return emission_fail(error, error_size, "program initializer ABI emission row changed");
    }
    return true;
}

bool xr_c_program_direct_i64_emission_bind(const XrTargetPlan *target_plan,
                                           XiModule *const *modules, uint32_t module_count,
                                           XrCProgramDirectI64EmissionBinding *out, char *error,
                                           size_t error_size) {
    if (!out)
        return emission_fail(error, error_size, "program C-emission output is missing");
    memset(out, 0, sizeof(*out));
    XrCProgramEmissionContext ctx = {0};
    if (!prepare_context(target_plan, modules, module_count, &ctx, error, error_size))
        return false;
    ...
    if (!bind_function(&ctx, &caller, &candidate.caller))
        return emission_fail(error, error_size, "entry function has no exact Xi emission binding");
    ...
    if (candidate.caller.xi_function == candidate.callee.xi_function ||
        ...
        return emission_fail(error, error_size, "program C function identities collide");
    if (!bind_direct_call(&ctx, &candidate))
        return emission_fail(error, error_size, "program call has no exact Xi emission binding");
    if (!build_emission_rows(&ctx, &candidate, error, error_size)) {
        xr_c_program_direct_i64_emission_release(&candidate);
        return false;
    }
    candidate.schema_version = XR_C_PROGRAM_DIRECT_I64_EMISSION_SCHEMA_VERSION;
    candidate.target_fingerprint = xr_target_plan_fingerprint(target_plan);
    candidate.verified = true;
    if (!xr_c_program_direct_i64_emission_verify(&candidate, target_plan, modules, module_count,
                                                 error, error_size)) {
        xr_c_program_direct_i64_emission_release(&candidate);
        return false;
    }
...
xr_c_program_direct_i64_function_binding(const XrCProgramDirectI64EmissionBinding *binding,
    ...
    for (uint32_t i = 0; binding->initializers && i < binding->initializer_count; i++) {
...
bool xr_c_program_direct_i64_value_view(const XrCProgramDirectI64EmissionBinding *binding,
...
bool xr_c_program_direct_i64_function_abi_view(const XrCProgramDirectI64EmissionBinding *binding,
    ...
    for (uint32_t i = 0; i < binding->function_abi_count; i++) {
        const XrCProgramFunctionAbiEmissionBinding *candidate = &binding->function_abis[i];
        if (candidate->xi_function != function || candidate->emission.ordinal != ordinal)
            continue;
        if (match)
            return false;
        match = candidate;
...
bool xr_c_program_direct_i64_call_is_exact(const XrCProgramDirectI64EmissionBinding *binding,
                                           const XiFunc *caller, const XiValue *call) {
    bool nullary = binding && binding->call_row && binding->call_row->argument_count == 0u;
    return binding && binding->verified && caller && call &&
           binding->caller.xi_function == caller && binding->xi_call == call &&
           call->op == XI_CALL && call->args && call->nargs == (uint16_t) (nullary ? 1u : 2u) &&
           (nullary ? binding->xi_argument == NULL && binding->argument_row == NULL &&
                          binding->target_argument == XR_SEMANTIC_INDEX_NONE
                    : call->args[1] == binding->xi_argument && binding->argument_row != NULL) &&
           binding->caller_target_row && binding->callee_target_row && binding->call_row &&
           binding->instruction_row;
}

bool xr_c_program_direct_i64_callee_operand_is_elided(
    const XrCProgramDirectI64EmissionBinding *binding, const XiFunc *caller, const XiValue *value) {
    if (!binding || !binding->verified || !binding->callee_operand_elided || !caller || !value)
        return false;
    if (binding->caller.xi_function == caller && binding->xi_callee_operand == value &&
        binding->xi_call && binding->xi_call->args &&
        (binding->xi_call->nargs == 1u || binding->xi_call->nargs == 2u) &&
        binding->xi_call->args[0] == value)
        return true;
    return binding->callee.xi_function == caller && binding->xi_native_leaf_callee_operand == value;
}
