 ...
 *   - Supported C storage and spelling come from the immutable C emission
...
struct XiCgenCtx {
    ...
    CgValueEmissionRegistryEntry *value_emission_registry; /* owned index; plans borrowed */
    uint32_t value_emission_registry_count;
    const CgValueEmissionRegistryEntry *value_emission_registry_last;
...
static const XiModule *cg_import_ref_resolved_module(const XiCgenCtx *ctx, const XiImportRef *ref) {
    ...
    if (ctx->program_direct_i64_required) {
        ...
        if (!ctx->program_direct_i64_bound || !bundle || !ref->resolution_attempted ||
            !ref->resolved_module ||
            !xaot_bundle_program_partition_for_xi_module(bundle, ref->resolved_module,
                                                         &target_partition))
            return NULL;
        uint32_t matches = 0;
        for (int module_index = 0; module_index < ctx->all_nmodules; module_index++) {
            uint32_t candidate_partition = UINT32_MAX;
            const XiModule *candidate = ctx->all_modules[module_index];
            if (!candidate ||
                !xaot_bundle_program_partition_for_xi_module(bundle, candidate,
                                                             &candidate_partition) ||
...
cg_program_direct_i64_function_binding(const XiCgenCtx *ctx, const XiFunc *function) {
    if (!ctx || !ctx->program_direct_i64_required || !ctx->program_direct_i64_bound || !function)
        return NULL;
    return xr_c_program_direct_i64_function_binding(&ctx->program_direct_i64, function);
}

static bool cg_program_direct_i64_function_is_initializer(const XiCgenCtx *ctx,
                                                          const XiFunc *function) {
    if (!ctx || !function || !ctx->program_direct_i64_required || !ctx->program_direct_i64_bound ||
        !ctx->program_direct_i64.verified || !ctx->program_direct_i64.initializers)
        return false;
    for (uint32_t i = 0; i < ctx->program_direct_i64.initializer_count; i++)
        if (ctx->program_direct_i64.initializers[i].xi_function == function)
            return true;
    return false;
}

static const char *cg_program_direct_i64_symbol(const XiCgenCtx *ctx, const XiFunc *function) {
    const XrCProgramXiFunctionBinding *binding =
        cg_program_direct_i64_function_binding(ctx, function);
    return binding && binding->c_symbol[0] ? binding->c_symbol : NULL;
}

static bool cg_program_direct_i64_callee_operand_is_elided(const XiCgenCtx *ctx,
                                                           const XiFunc *function,
                                                           const XiValue *value) {
    return ctx && ctx->program_direct_i64_required && ctx->program_direct_i64_bound &&
           xr_c_program_direct_i64_callee_operand_is_elided(&ctx->program_direct_i64, function,
                                                            value);
}
...
static bool emit_canonical_extern_c_name(XiCgenCtx *ctx, FILE *out, const XaotExternDecl *decl) {
...
static bool cg_func_needs_external_linkage(const XiCgenCtx *ctx, const XiFunc *f,
    ...
    if (!ctx || !ctx->extern_linkage || !f)
        return false;
    if (ctx->program_direct_i64_required) {
        const XrCProgramXiFunctionBinding *binding = cg_program_direct_i64_function_binding(ctx, f);
        return binding && binding->c_symbol[0] != '\0' &&
               memchr(binding->c_symbol, '\0', sizeof(binding->c_symbol)) != NULL;
    }
    if (!prefix || !prefix[0])
        return true;
...
static bool cg_func_c_name(XiCgenCtx *ctx, const char *prefix, const XiFunc *f, char *buf,
    ...
    if (ctx->program_direct_i64_required) {
        ...
        const void *terminator = memchr(binding->c_symbol, '\0', sizeof(binding->c_symbol));
        if (!binding->c_symbol[0] || !terminator) {
            ctx->error = true;
            return false;
        }
        int written = snprintf(buf, bufsz, "%s", binding->c_symbol);
...
cg_leaf_aggregate_direct_callee_get_status(XiCgenCtx *ctx, const XiFunc *current,
                ...
                switch ((XiOp) user->op) {
                    case XI_CALL: {
                        ...
                        XaotLeafAggregateTargetStatus leaf_status =
                            cg_leaf_aggregate_direct_callee_get_status(ctx, owner, value, error,
                                                                       sizeof(error));
...
cg_leaf_aggregate_direct_callee_get_status(XiCgenCtx *ctx, const XiFunc *current,
    ...
    if (status != XAOT_LEAF_AGGREGATE_TARGET_FOUND)
        return status;
    const XiFunc *slot_target = cg_shared_function_slot_target(ctx, current, (int) value->aux_int);
    if (!view.target_plan || !view.call_instruction || !view.callee || view.callee != slot_target ||
        view.argument_value != call->args[1]) {
        if (error && error_size)
...
static bool cg_shared_static_function_get_is_elided(XiCgenCtx *ctx, const XiFunc *current,
    ...
    char error[256] = {0};
    XaotLeafAggregateTargetStatus leaf_status =
        cg_leaf_aggregate_direct_callee_get_status(ctx, current, v, error, sizeof(error));
    if (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_FOUND)
        return true;
    if (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_INVALID) {
        if (!ctx->error)
...
typedef enum CgValueEmissionStatus {
    CG_VALUE_EMISSION_NOT_CONFIGURED = 0,
    CG_VALUE_EMISSION_BACKEND_ONLY,
    CG_VALUE_EMISSION_NOT_COVERED,
    CG_VALUE_EMISSION_FOUND,
    CG_VALUE_EMISSION_ERROR,
} CgValueEmissionStatus;

static CgValueEmissionStatus cg_value_emission_fail(XiCgenCtx *ctx, const char *detail) {
    if (ctx && !ctx->error) {
        const char *prefix = detail && strncmp(detail, "XR_", 3) == 0 ? "" : "XR_TARGET_1001: ";
        fprintf(stderr, "[xi_cgen] ERROR: %s%s\n", prefix,
                detail ? detail : "C value emission failed");
    }
    if (ctx)
        ctx->error = true;
    return CG_VALUE_EMISSION_ERROR;
}

static CgValueEmissionStatus cg_program_direct_i64_value_emission(XiCgenCtx *ctx,
                                                                  const XiFunc *function,
                                                                  const XiValue *value,
                                                                  XrCValueEmissionView *out) {
    if (!ctx || !function || !value || !out)
        return cg_value_emission_fail(ctx, "program direct-i64 value consumer input is missing");
    if (xr_c_program_direct_i64_value_view(&ctx->program_direct_i64, function, value, out))
        return CG_VALUE_EMISSION_FOUND;

    return cg_value_emission_fail(ctx, "program direct-i64 C value binding is missing");
}

/* Resolve every program-owned value from the verified program binding. A
 * missing row is an authority failure: neither Xi-id shape nor a legacy Xaot
 * representation adapter may synthesize a second C answer. */
static CgValueEmissionStatus cg_value_emission_view(XiCgenCtx *ctx, const XiFunc *function,
    ...
    if (ctx->program_direct_i64_required && !ctx->program_direct_i64_bound)
        return cg_value_emission_fail(ctx, "program direct-i64 C-emission binding is missing");
    if (!function && value->block)
        function = value->block->func;
    if (ctx->program_direct_i64_required && !function)
        return cg_value_emission_fail(ctx, "program direct-i64 value has no owning Xi function");
    if (!function)
        return CG_VALUE_EMISSION_BACKEND_ONLY;
    if (ctx->program_direct_i64_required)
        return cg_program_direct_i64_value_emission(ctx, function, value, out);
    if (!ctx->value_emission_registry && ctx->value_emission_registry_count == 0)
    ...
    if (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_FOUND) {
        for (uint32_t module_index = 0; module_index < ctx->value_emission_registry_count;
            ...
            if (ctx->value_emission_registry[module_index].target_plan != leaf_target)
                continue;
            if (entry)
                return cg_value_emission_fail(ctx, "leaf-aggregate C value authority is ambiguous");
    ...
    if (!entry)
        return cg_value_emission_fail(
            ctx, "C value SemanticPlan authority is missing from the module registry");
    ctx->value_emission_registry_last = entry;
    const XrTargetPlan *target_plan = entry->target_plan;
    const XrCEmissionPlan *emission_plan = entry->emission_plan;

    const XrSemanticPlan *semantic_plan = entry->semantic_plan;
    if (!semantic_plan || (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_FOUND
    ...
    const XrSemanticFunctionRecord *semantic_function =
        xr_semantic_plan_function(semantic_plan, leaf_status == XAOT_LEAF_AGGREGATE_TARGET_FOUND
...
static bool cg_value_semantic_id(XiCgenCtx *ctx, const XiFunc *function, const XiValue *value,
    ...
    if (ctx->program_direct_i64_required) {
        if (!ctx->program_direct_i64_bound) {
            (void) cg_value_emission_fail(ctx, "program direct-i64 C-emission binding is missing");
            return false;
        }
        XrCValueEmissionView view = {0};
        if (xr_c_program_direct_i64_value_view(&ctx->program_direct_i64, function, value, &view)) {
        ...
        if (cg_program_direct_i64_callee_operand_is_elided(ctx, function, value)) {
            ...
            if (!ctx->program_direct_i64.verified || !function_binding || !target ||
                ...
                !xr_aot_scalar_program_semantic_value_id(
                    target, function_binding->target_partition, function_binding->target_function,
                    function, value, &semantic_function, &semantic_value, error, sizeof(error)) ||
                semantic_function != function_binding->semantic_function) {
                (void) cg_value_emission_fail(
                    ctx, error[0] ? error : "elided program callee semantic identity is invalid");
                return false;
            }
            *out = semantic_value;
            return true;
        }
        (void) cg_value_emission_fail(ctx, "program direct-i64 semantic value binding is missing");
        return false;
    }
    if (!function->semantic_plan)
        return false;
    const CgValueEmissionRegistryEntry *entry = NULL;
    ...
    if (!entry)
        return false;
    bool exact = xr_aot_scalar_semantic_value_id(entry->target_plan, function, value,
    ...
    if (!adapter || !xaot_value_plan_is_exact_rep_adapter(ctx->aot_bundle, adapter) ||
        ...
        return false;
    exact = xr_aot_scalar_semantic_value_id(entry->target_plan, function, value->args[0],
                                            &semantic_function, out, error, sizeof(error));
    return exact && semantic_function == function->semantic_plan_function_index;
}
...
static bool cg_number_parse_error_catch_narrow_is_exact(XiCgenCtx *ctx, const XiFunc *function,
                                                        const XiValue *value) {
    return function && function->semantic_plan &&
...
static bool cg_direct_local_scalar_ref_v1_argument_emission(XiCgenCtx *ctx, const XiFunc *function,
                                                            const XiValue *call, uint16_t ordinal,
                                                            const XiValue *argument,
                                                            XrCCallArgumentEmissionView *out) {
    if (out)
...
static bool cg_i64_overflow_predicate_emission_view(XiCgenCtx *ctx, const XiFunc *function,
                                                    const XiValue *value,
                                                    XrCValueEmissionView *out) {
    if (out)
        memset(out, 0, sizeof(*out));
    if (!ctx || !function || !value || !out || !cg_i64_overflow_predicate_required(function))
        return false;
    uint32_t receiver = XR_SEMANTIC_INDEX_NONE;
    ...
    if (status != CG_VALUE_EMISSION_FOUND || value->op != XI_CALL_METHOD || value->nargs != 2 ||
        ...
        (void) cg_value_emission_fail(ctx,
                                      "i64 overflow predicate has no exact immutable C recipe");
        return false;
    }
    return true;
}
...
static bool cg_iterator_rune_nth_emission_view(XiCgenCtx *ctx, const XiFunc *function,
                                               const XiValue *value, XrCValueEmissionView *out) {
    if (out)
        memset(out, 0, sizeof(*out));
    if (!ctx || !function || !value || !out)
    ...
    CgValueEmissionStatus status = cg_value_emission_view(ctx, function, value, out);
    if (status != CG_VALUE_EMISSION_FOUND ||
        out->materialization != XR_C_VALUE_MATERIALIZATION_ITERATOR_RUNE_NTH)
        return false;
    uint32_t receiver = XR_SEMANTIC_INDEX_NONE;
    uint32_t index = XR_SEMANTIC_INDEX_NONE;
    if (value->nargs != 2 || !value->args || !value->args[0] || !value->args[1] ||
        out->rep != XR_C_VALUE_REP_RUNE || !out->recipe_symbol || !out->recipe_symbol[0] ||
        !cg_value_semantic_id(ctx, function, value->args[0], &receiver) ||
        !cg_value_semantic_id(ctx, function, value->args[1], &index) ||
        receiver != out->recipe_operand_value || index != out->recipe_argument_value) {
        (void) cg_value_emission_fail(ctx, "Iterator<rune>.nth has no exact immutable C recipe");
        return false;
    }
    return true;
}
...
static bool cg_rune_to_string_emission_view(XiCgenCtx *ctx, const XiFunc *function,
    ...
    if (status != CG_VALUE_EMISSION_FOUND ||
        ...
        return false;
    uint32_t receiver = XR_SEMANTIC_INDEX_NONE;
    if (value->nargs != 1 || !value->args || !value->args[0] || out->rep != XR_C_VALUE_REP_TAGGED ||
        !out->recipe_symbol || !out->recipe_symbol[0] ||
        !cg_value_semantic_id(ctx, function, value->args[0], &receiver) ||
        receiver != out->recipe_operand_value) {
...
static const char *cg_byte_array_append_adapter_name(XiCgenCtx *ctx);
...
static bool cg_value_narrow_int_rep(XiCgenCtx *ctx, const XiFunc *f, const XiValue *v,
    ...
    if (emission_status == CG_VALUE_EMISSION_FOUND) {
        if (!cg_value_emission_xaot_rep(ctx, &emission, &rep))
            return false;
    } else if (emission_status == CG_VALUE_EMISSION_ERROR ||
...
static const XrTargetPlan *cg_function_target_plan(XiCgenCtx *ctx, const XiFunc *function) {
    const XrSemanticPlan *semantic =
        ctx && function ? xaot_bundle_program_semantic_for_func(ctx->aot_bundle, function, NULL)
                        : NULL;
    const XrTargetPlan *target = semantic ? xaot_bundle_program_target_plan(ctx->aot_bundle) : NULL;
...
static bool cg_freestanding_assertion_authorized(XiCgenCtx *ctx, const XiFunc *function,
                                                 const XrAssertionPlan *assertion) {
    if (!assertion || (assertion->kind != XR_ASSERTION_KIND_CONDITION &&
                       assertion->kind != XR_ASSERTION_KIND_EQUAL))
        return false;
    const XrTargetPlan *target_plan = cg_function_target_plan(ctx, function);
    uint64_t capability_mask = 0;
    if (!target_plan || !xr_target_plan_capability_mask(target_plan, &capability_mask))
        return false;
...
static const char *cg_codegen_opaque_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_CODEGEN_OPAQUE_HI,
                                        XR_SEM_OWNER_ID_SHARED_CODEGEN_OPAQUE_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: codegen-opaque owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_codegen_fence_adapter_name(XiCgenCtx *ctx) {
    ...
    const char *adapter =
        xr_semantic_owner_cgen_adapter(XR_SEM_OWNER_ID_SHARED_CODEGEN_COMPILER_FENCE_HI,
                                       XR_SEM_OWNER_ID_SHARED_CODEGEN_COMPILER_FENCE_LO);
...
static const char *cg_reference_count_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_REFERENCE_COUNT_HI,
                                        XR_SEM_OWNER_ID_SHARED_REFERENCE_COUNT_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: reference-count owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_type_identity_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_PRIMITIVE_TYPE_IDENTITY_HI,
                                        XR_SEM_OWNER_ID_PRIMITIVE_TYPE_IDENTITY_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: type-identity owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_byte_slice_scalar_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_SCALAR_HI,
                                        XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_SCALAR_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: byte-slice scalar owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
    const char *adapter = xr_semantic_owner_cgen_adapter(
...
static const char *cg_byte_slice_compare_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_COMPARE_HI,
                                        XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_COMPARE_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: byte-slice compare owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
    const char *adapter = xr_semantic_owner_cgen_adapter(
...
static const char *cg_byte_slice_fill_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_FILL_HI,
                                        XR_SEM_OWNER_ID_SHARED_BYTE_SLICE_FILL_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: byte-slice fill owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_byte_array_copy_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_BYTE_ARRAY_COPY_HI,
                                        XR_SEM_OWNER_ID_SHARED_BYTE_ARRAY_COPY_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: byte-array copy owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_byte_array_append_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_BYTE_ARRAY_APPEND_HI,
                                        XR_SEM_OWNER_ID_SHARED_BYTE_ARRAY_APPEND_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: byte-array append owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
    const char *adapter = xr_semantic_owner_cgen_adapter(
...
static const char *cg_pod_slice_fill_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_POD_SLICE_FILL_HI,
                                        XR_SEM_OWNER_ID_SHARED_POD_SLICE_FILL_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: POD-slice fill owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_pod_slice_view_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_POD_SLICE_VIEW_HI,
                                        XR_SEM_OWNER_ID_SHARED_POD_SLICE_VIEW_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: POD-slice view owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_cell_access_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_CELL_ACCESS_HI,
                                        XR_SEM_OWNER_ID_SHARED_CELL_ACCESS_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: cell-access owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static const char *cg_null_test_adapter_name(XiCgenCtx *ctx) {
    if (!xr_semantic_owner_has_consumer(XR_SEM_OWNER_ID_SHARED_NULL_TEST_HI,
                                        XR_SEM_OWNER_ID_SHARED_NULL_TEST_LO,
                                        XR_SEM_CONSUMER_CGEN)) {
        fprintf(stderr, "[xi_cgen] ERROR: null-test owner has no CGen consumer\n");
        cg_ctx_set_error(ctx);
        return NULL;
    }
...
static bool cg_func_body_gets_shared_slot(XiCgenCtx *ctx, const XiFunc *f, int slot) {
    if (!ctx || !f || slot < 0)
        return false;
    for (uint32_t bi = 0; bi < f->nblocks; bi++) {
        const XiBlock *blk = f->blocks[bi];
        ...
        for (uint32_t vi = 0; vi < blk->nvalues; vi++) {
            const XiValue *v = blk->values[vi];
            if (v && v->op == XI_GET_SHARED && (int) v->aux_int == slot &&
                !cg_program_direct_i64_callee_operand_is_elided(ctx, f, v))
                return true;
        }
    }
    return false;
...
static bool cg_reachable_func_tree_gets_shared_slot(XiCgenCtx *ctx, const XiModule *owner_mod,
    ...
    if (!source_reachable)
        return false;
    if (cg_module_for_func(ctx, source) == owner_mod &&
        cg_func_body_gets_shared_slot(ctx, source, slot))
        return true;
...
static bool cg_import_ref_value_use_requires_runtime_value(XiCgenCtx *ctx, const XiFunc *owner,
    ...
    if (depth > 16)
        return true;
    if (cg_program_direct_i64_callee_operand_is_elided(ctx, owner, ref))
        return false;
    const XiModule *owner_mod = cg_module_for_func(ctx, owner);
    for (uint32_t bi = 0; bi < owner->nblocks; bi++) {
        const XiBlock *blk = owner->blocks[bi];
...
static bool cg_rep_alias_source_relation_is_exact(XiCgenCtx *ctx, const XiValue *alias,
    ...
    bool backend_inverse = (alias->op == XI_UNBOX && adapter->op == XI_BOX &&
    ...
    if (!f || alias->op != XI_UNBOX || alias->backend_origin != XI_BACKEND_VALUE_REP_UNBOX ||
        adapter->op != XI_BOX || adapter->backend_origin != XI_BACKEND_VALUE_NONE ||
        adapter->type != source->type || alias->type != source->type ||
    ...
    if (!alias_plan || !xaot_value_plan_is_exact_rep_adapter(ctx->aot_bundle, alias_plan))
        return false;
    const XrSemanticOperationRecord *operation = cg_semantic_operation_for_value(ctx, f, adapter);
    uint32_t operand_count = 0;
    const XrSemanticOperandRecord *operands =
...
static bool cg_coalesced_semantic_box_is_elided(XiCgenCtx *ctx, const XiFunc *f,
                                                const XiValue *box) {
    if (!ctx || !f || !box || box->op != XI_BOX || box->backend_origin != XI_BACKEND_VALUE_NONE ||
        box->nargs != 1 || !box->args || !box->args[0] || !f->phi_coalesce ||
...
static bool cg_c_export_xray_func_signature_supported(XiCgenCtx *ctx, const XiFunc *f) {
    ...
    for (uint16_t i = 0; i < f->nparams; i++) {
        ...
        if (cg_cfn_value_type_supported(pt, false))
            continue;
        if (!cg_c_export_slot_is_struct_aggregate(ctx, f, (uint16_t) (i + 1u)))
            return false;
    }
    return true;
}
...
static void emit_c_export_return_c_type(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    XrCFunctionAbiEmissionView return_view;
    if (cg_c_export_slot_is_struct_aggregate(ctx, f, 0) &&
        cg_func_return_abi_emission(ctx, f, &return_view) && return_view.c_type) {
        fprintf(out, "%s", return_view.c_type);
        return;
    }
...
static void emit_c_export_param_c_type(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    XrCFunctionAbiEmissionView param_view;
    if (cg_c_export_slot_is_struct_aggregate(ctx, f, (uint16_t) (index + 1u)) &&
        cg_func_param_abi_emission(ctx, f, index, &param_view) && param_view.c_type) {
        fprintf(out, "%s", param_view.c_type);
        return;
    }
...
static const XiFunc *cg_value_static_func_target(XiCgenCtx *ctx, const XiFunc *owner,
    ...
    if (v->op == XI_IMPORT_REF && v->aux) {
        ...
        const XiModule *mod = cg_import_ref_target_module(ctx, ref, &resolved_slot);
        if (mod && mod->slot_funcs && resolved_slot >= 0 && resolved_slot < (int64_t) mod->nslots &&
...
static bool cg_func_has_forced_body_root(XiCgenCtx *ctx, const XiFunc *f) {
    if (!f)
        return false;
    /* The verified program binding owns the entire bounded function graph.
     * Only lifecycle initializers are implicit roots; its ordinary caller is
     * seeded explicitly below and no export/class/link metadata may add a
     * parallel product root. */
    if (ctx->program_direct_i64_required) {
        if (!cg_program_direct_i64_function_binding(ctx, f))
            return false;
        return cg_program_direct_i64_function_is_initializer(ctx, f) &&
...
static void cg_func_reach_mark_root(XiCgenCtx *ctx, const XiFunc *f) {
    ...
    if (ctx->program_direct_i64_required) {
        if (!cg_program_direct_i64_function_binding(ctx, f))
            return;
    } else if (!xaot_callable_func_has_executable_body_plan(cg_ctx_aot_bundle(ctx), f)) {
        return;
    }
...
static bool cg_func_reach_mark_edge(XiCgenCtx *ctx, const XiFunc *target) {
    if (!ctx || !target)
        return false;
    if (ctx->program_direct_i64_required) {
        if (!cg_program_direct_i64_function_binding(ctx, target))
            return false;
...
static bool cg_func_reach_mark_value_edges(XiCgenCtx *ctx, const XiFunc *owner, const XiValue *v) {
    ...
    /* Program reachability is the closure of the verified direct edge, not a
     * union with the legacy class/name/shared/parallel resolvers below. Any
     * other call in the frozen bounded family is an authority mismatch and is
     * rejected by the same program call consumer used by emission. */
    if (ctx->program_direct_i64_required) {
        const XrCProgramDirectI64EmissionBinding *binding = &ctx->program_direct_i64;
        if (!ctx->program_direct_i64_bound || !binding->verified) {
            ctx->error = true;
            return false;
        }
        if (v->op != XI_CALL && v->op != XI_TAIL_CALL && v->op != XI_CALL_METHOD &&
            v->op != XI_CALL_METHOD_DIRECT)
            return false;
        XaotDirectI64TargetView direct_i64 = {0};
        XaotDirectI64TargetStatus status = cg_direct_i64_call_view(ctx, owner, v, &direct_i64);
        if (status != XAOT_DIRECT_I64_TARGET_FOUND)
            return false;
        return owner == binding->caller.xi_function && v == binding->xi_call &&
               direct_i64.callee == binding->callee.xi_function &&
               cg_func_reach_mark_edge(ctx, binding->callee.xi_function);
    }
    bool changed = false;
    const XaotBundle *bundle = cg_ctx_aot_bundle(ctx);
    ...
    const XiFunc *ref_target = cg_program_direct_i64_callee_operand_is_elided(ctx, owner, v)
                                   ? NULL
                                   : cg_value_static_func_target(ctx, owner, v);
...
static bool cg_func_reach_mark_body_edges(XiCgenCtx *ctx, const XiFunc *source) {
    ...
    for (uint32_t bi = 0; bi < source->nblocks; bi++) {
        ...
        if (!ctx->program_direct_i64_required)
            changed |= cg_func_reach_mark_edge(
                ctx, cg_value_static_func_target(ctx, source, block->control));
        for (uint32_t vi = 0; vi < block->nvalues; vi++)
...
static bool cg_func_reach_mark_body_edges_in_owner_module(XiCgenCtx *ctx, const XiFunc *source) {
    if (!ctx || !source)
        return false;
    /* Program edges are fully described by stable pointers in the verified
     * binding. They do not require switching module scan state or registering
     * imported class/name metadata. */
    if (ctx->program_direct_i64_required)
...
static void cg_func_reachability_compute(XiCgenCtx *ctx) {
    ...
    /* A program graph has one product entry root. The callee must become live
     * only through the verified direct-call edge; seeding it here as a second
     * root would hide a missing or corrupt call binding. */
    if (ctx->program_direct_i64_required) {
        if (!ctx->program_direct_i64_bound || !ctx->program_direct_i64.verified ||
            !ctx->program_direct_i64.caller.xi_function) {
            fprintf(stderr, "[xi_cgen] ERROR: XR_TARGET_1001: verified program graph "
                            "reachability binding is missing or stale\n");
            ctx->error = true;
        } else {
            cg_func_reach_mark_root(ctx, ctx->program_direct_i64.caller.xi_function);
        }
    }
    if (!ctx->program_direct_i64_required) {
...
static void cg_report_mandatory_contract_failure(XiCgenCtx *ctx, const XiFunc *func,
    ...
    fprintf(stderr,
            ...
            contract_kind, func && func->name ? func->name : "?", value ? value->id : 0, stable_id,
...
static bool cg_json_codec_summary_matches_site(const XiValue *value,
                                               ...
                                               uint8_t expected_kind) {
    if (!value || !summary || summary->source_node_id == 0 || summary->codec_kind != expected_kind)
        return false;
    if (expected_kind == XG_JSON_CODEC_PARSE) {
        const bool typed_parse = value->op == XI_JSON_DECODE &&
...
static bool cg_mandatory_plans_preflight_value(XiCgenCtx *ctx, const XiFunc *func,
    ...
    if (json_codec_site) {
        if (value->xg_json_codec_id == XG_NO_ID) {
            ...
            cg_report_mandatory_contract_failure(ctx, func, value, "json-codec evidence", XG_NO_ID,
                                                 issue);
            return false;
        }
        const XgGlobalEvidence *evidence = bundle ? bundle->global_evidence_plan.evidence : NULL;
