...
static bool cg_emit_program_direct_i64_callee_decl(XiCgenCtx *ctx, FILE *out, int mod_index) {
    ...
    if (!callee->xi_function || !callee->c_symbol[0] || !symbol_end ||
        ...
        return_view.parameter_count > 1u || return_view.rep != (uint8_t) XR_C_VALUE_REP_I64 ||
        !return_view.c_type) {
        ctx->error = true;
        return false;
    }
    ...
    if (!xr_c_program_direct_i64_function_abi_view(&ctx->program_direct_i64, callee->xi_function,
                                                   1u, &parameter_view) ||
        parameter_view.boundary_kind != (uint8_t) XR_C_ABI_BOUNDARY_NATIVE ||
        parameter_view.parameter_count != 1u ||
        parameter_view.rep != (uint8_t) XR_C_VALUE_REP_I64 || !parameter_view.c_type) {
        ctx->error = true;
        return false;
    }
