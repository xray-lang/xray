 ...
 * emits solely from that opaque binding and schema-58 TargetPlan rows. No
 * mutable binding escapes this call. The output contains no test oracle or
 * process entry point and is released with xr_free(). */
XR_FUNC bool xr_c_leaf_value_product_program_emit(const XrTargetPlan *target_plan,
                                                  const struct XiModule *module, char **out_source,
