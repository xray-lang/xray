...
static bool verify_program_function_is_excluded(
    const XrCProgramDirectI64EmissionBinding *program_scope,
    const XiFunc *func) {
    return program_scope && func &&
           !xr_c_program_direct_i64_function_binding(program_scope, func);
}

static bool verify_program_callee_operand_elision(
    const XaotBundle *bundle, const XiFunc *func, const XiValue *value,
    bool *out_elided, char *errbuf, size_t errbuf_len) {
    if (out_elided)
        *out_elided = false;
    if (!bundle || !func || !value || !out_elided)
        return set_error(errbuf, errbuf_len,
                         "AOT callee elision verifier input is incomplete");
...
static bool verify_func_has_plan_recursive(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope, char *errbuf,
    size_t errbuf_len) {
    uint16_t ci;
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        ...
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_has_plan_recursive(
                    bundle, func->children[ci], program_scope, errbuf,
                    errbuf_len))
                return false;
        }
        return true;
    ...
    for (ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_has_plan_recursive(bundle, func->children[ci],
                                            program_scope, errbuf, errbuf_len))
            return false;
    }
    return true;
}
...
static bool verify_value_plan(const XaotBundle *bundle, const XaotValuePlan *plan, char *errbuf,
    ...
    if (!xaot_bundle_find_func_plan(bundle, plan->func))
        return set_error(errbuf, errbuf_len,
                         "AOT value plan has no owning function plan");
...
static bool verify_func_values_have_plans_recursive(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope, char *errbuf,
    size_t errbuf_len) {
    uint32_t bi;
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_values_have_plans_recursive(
                    bundle, func->children[ci], program_scope, errbuf,
                    errbuf_len))
                return false;
        }
    ...
    for (bi = 0; bi < func->nblocks; bi++) {
        ...
        for (phi = blk->phis; phi; phi = phi->next) {
            ...
            if (!binding && !legacy) {
                bool elided = false;
                if (!verify_program_callee_operand_elision(
                        bundle, func, &phi->value, &elided, errbuf,
                        errbuf_len))
                    return false;
                if (!elided)
                    return set_error(
        ...
        for (vi = 0; vi < blk->nvalues; vi++) {
            ...
            if (!binding && !legacy) {
                bool elided = false;
                if (!verify_program_callee_operand_elision(
                        bundle, func, blk->values[vi], &elided, errbuf,
                        errbuf_len))
                    return false;
                if (!elided)
                    return set_error(
    ...
    for (ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_values_have_plans_recursive(
                bundle, func->children[ci], program_scope, errbuf, errbuf_len))
            return false;
    }
    return true;
}
...
static bool verify_func_extern_calls(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope, char *errbuf,
    size_t errbuf_len) {
    if (!func)
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (uint16_t child = 0; child < func->nchildren; child++) {
            if (!verify_func_extern_calls(bundle, func->children[child],
                                          program_scope, errbuf, errbuf_len))
                return false;
        }
        return true;
    ...
    for (uint16_t ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_extern_calls(bundle, func->children[ci], program_scope,
                                      errbuf, errbuf_len))
            return false;
    }
    return true;
...
static bool verify_func_boundaries_recursive(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope, char *errbuf,
    size_t errbuf_len) {
    uint32_t bi;
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_boundaries_recursive(
                    bundle, func->children[ci], program_scope, errbuf,
                    errbuf_len))
                return false;
        }
    ...
    for (ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_boundaries_recursive(bundle, func->children[ci],
                                              program_scope, errbuf, errbuf_len))
            return false;
    }
    return true;
...
static bool verify_func_closure_plans_recursive(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope,
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_closure_plans_recursive(
                    bundle, func->children[ci], program_scope, out_count,
                    errbuf, errbuf_len))
                return false;
        }
        return true;
    ...
    for (ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_closure_plans_recursive(
                bundle, func->children[ci], program_scope, out_count, errbuf,
                errbuf_len))
            return false;
    }
    return true;
...
static bool verify_func_stack_allocations_recursive(const XaotBundle *bundle,
                                                    const XiFunc *func,
                                                    const XrCProgramDirectI64EmissionBinding *program_scope,
                                                    char *errbuf,
                                                    size_t errbuf_len) {
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_stack_allocations_recursive(
                    bundle, func->children[ci], program_scope, errbuf,
                    errbuf_len))
                return false;
        }
...
static bool verify_func_transfer_plans_recursive(
    const XaotBundle *bundle, const XiFunc *func,
    const XrCProgramDirectI64EmissionBinding *program_scope,
    uint32_t *out_count, char *errbuf, size_t errbuf_len) {
    uint32_t bi;
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_transfer_plans_recursive(
                    bundle, func->children[ci], program_scope, out_count,
                    errbuf, errbuf_len))
                return false;
        }
...
static bool verify_bundle_function_plans(const XaotBundle *bundle,
    ...
    if (graph_count != 0u) {
        ...
        scope = &program_scope;
        for (uint32_t plan_index = 0; plan_index < bundle->nfunc_plans;
             plan_index++) {
    ...
    for (uint32_t module_index = 0;
         exact && module_index < bundle->nmodules; module_index++) {
        const XiModule *module = bundle->modules[module_index];
        if (!module || !module->init) {
            exact = set_error(errbuf, errbuf_len,
                              "AOT bundle contains an invalid module");
            break;
        }
        exact = verify_func_has_plan_recursive(
                    bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_values_have_plans_recursive(
                    bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_boundaries_recursive(
                    bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_stack_allocations_recursive(
                    bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_closure_plans_recursive(
                    bundle, module->init, scope, closure_count, errbuf,
                    errbuf_len) &&
                verify_func_transfer_plans_recursive(
                    bundle, module->init, scope, transfer_count, errbuf,
                    errbuf_len) &&
                verify_func_extern_calls(bundle, module->init, scope, errbuf,
                                         errbuf_len);
    }
    xr_c_program_direct_i64_emission_release(&program_scope);
    return exact;
...
XR_FUNC bool xaot_verify_bundle(const XaotBundle *bundle, char *errbuf, size_t errbuf_len) {
    ...
    if (!verify_bundle_function_plans(bundle, &closure_count, &transfer_count,
                                      errbuf, errbuf_len))
        return false;
