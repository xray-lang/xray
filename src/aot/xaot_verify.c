...
static bool
verify_program_function_is_excluded(const XrCProgramDirectI64EmissionBinding *program_scope,
                                    const XiFunc *func) {
    return program_scope && func && !xr_c_program_direct_i64_function_binding(program_scope, func);
}

static bool verify_program_callee_operand_elision(const XaotBundle *bundle, const XiFunc *func,
                                                  const XiValue *value, bool *out_elided,
                                                  char *errbuf, size_t errbuf_len) {
    if (out_elided)
        *out_elided = false;
    if (!bundle || !func || !value || !out_elided)
        return set_error(errbuf, errbuf_len, "AOT callee elision verifier input is incomplete");
    ...
    char error[256] = {0};
    if (!xr_c_program_direct_i64_emission_bind(target, bundle->modules, bundle->nmodules, &binding,
                                               error, sizeof(error)))
        return set_error(errbuf, errbuf_len,
                         "AOT callee elision has no exact program C-emission binding");
    *out_elided = xr_c_program_direct_i64_callee_operand_is_elided(&binding, func, value);
...
static bool verify_target_value_binding(const XaotBundle *bundle, const XiFunc *func,
    ...
    if (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_INVALID)
        return set_error(errbuf, errbuf_len, "AOT leaf-aggregate value lacks exact PSC identity");
    ...
    *out_binding = xr_target_plan_value_rep_for_module(target_plan, partition, semantic_value);
    return true;
}
...
static const char *verify_exact_raw_pointer_c_type(const XaotBundle *bundle, const XiFunc *func,
                                                   const XrTargetValueRepRecord *binding) {
    XrCValueEmissionView view = {0};
    char error[256] = {0};
    const XrCEmissionPlan *emission_plan = xaot_bundle_emission_plan_for_func(bundle, func);
...
static bool verify_effective_value_rep(const XaotBundle *bundle, const XiFunc *func,
    ...
    if (!verify_target_value_binding(bundle, func, value, &binding, &rep_adapter, errbuf,
        ...
        return false;
    if (binding) {
        const XrTargetPlan *target_plan = xaot_bundle_program_semantic_for_func(bundle, func, NULL)
...
static bool verify_func_has_plan_recursive(const XaotBundle *bundle, const XiFunc *func,
    ...
    if (verify_program_function_is_excluded(program_scope, func)) {
        if (xaot_bundle_find_func_plan(bundle, func))
            return set_error(errbuf, errbuf_len,
                             "excluded Xi function retains an AOT function plan");
        for (ci = 0; ci < func->nchildren; ci++) {
            if (!verify_func_has_plan_recursive(bundle, func->children[ci], program_scope, errbuf,
                                                errbuf_len))
                return false;
        }
        return true;
...
static bool verify_value_plan(const XaotBundle *bundle, const XaotValuePlan *plan, char *errbuf,
    ...
    if (!xaot_bundle_find_func_plan(bundle, plan->func))
        return set_error(errbuf, errbuf_len, "AOT value plan has no owning function plan");
...
static bool verify_transfer_value_authority(const XaotBundle *bundle, const XiFunc *func,
    ...
    if (binding) {
        ...
        if (rep_adapter || !verify_target_machine_value_rep(
                               bundle, func,
...
static bool verify_body_summary_anchor(const XaotBundle *bundle, const XiFunc *func,
    ...
    if (func && func->name && body->kind != XG_BODY_MODULE_INIT &&
        body->name_id != xg_name_id("<anonymous>") && body->name_id != xg_name_id(func->name)) {
        if (errbuf && errbuf_len > 0)
...
static const XgModuleSummary *verify_find_evidence_module(const XgGlobalEvidence *ev,
                                                          XgModuleId module_id) {
    if (!ev || module_id == XG_NO_ID)
        return NULL;
    for (uint32_t i = 0; i < ev->nmodules; i++) {
        if (ev->modules[i].module_id == module_id)
            return &ev->modules[i];
    }
    return NULL;
}
...
static bool verify_generic_specialization_identity_shape(
    const XgGlobalEvidence *ev, uint8_t kind, uint64_t origin_nominal_key,
    XgClassId receiver_class_id, uint64_t receiver_type_key, uint64_t receiver_type_arg_key_start,
    ...
    if (((kind == XG_GENERIC_INST_CLASS || kind == XG_GENERIC_INST_METHOD) !=
         (origin_nominal_key != 0))) {
        if (errbuf && errbuf_len > 0)
            snprintf(errbuf, errbuf_len, "AOT %s specialization identity is invalid", row_name);
        return false;
    }
    ...
    if (kind == XG_GENERIC_INST_CLASS) {
        if (receiver_class_id != XG_NO_ID || receiver_type_key != 0 ||
            ...
            declaration_type_key != xg_generic_nominal_type_key(origin_nominal_key,
                                                                declaration_type_arg_key_start,
                                                                declaration_type_arg_count, kind)) {
            if (errbuf && errbuf_len > 0)
                snprintf(errbuf, errbuf_len, "AOT %s aggregate identity does not re-derive",
                         row_name);
            return false;
        }
        return true;
    ...
    if (receiver_type_key !=
        (receiver_type_arg_count > 0u
             ...
             : origin_nominal_key)) {
        if (errbuf && errbuf_len > 0)
            snprintf(errbuf, errbuf_len, "AOT %s receiver identity does not re-derive", row_name);
        return false;
    }
...
static bool verify_generic_inst_anchors_monomorphized_class(const XgGenericInstSummary *inst,
    ...
    return inst->origin_class_id == cls->generic_origin_class_id &&
           inst->origin_nominal_key == cls->generic_origin_nominal_key &&
...
static bool verify_generic_method_receiver_matches_source(const XgGlobalEvidence *ev,
    ...
    return receiver && (receiver->flags & XG_CLASS_MONOMORPHIZED) != 0u &&
           ...
           receiver->generic_origin_nominal_key == inst->origin_nominal_key &&
           receiver->generic_type_key == inst->receiver_type_key &&
           receiver->generic_type_key ==
               xg_generic_nominal_type_key(
                   inst->origin_nominal_key, receiver->generic_type_arg_key_start,
                   receiver->generic_type_arg_count, XG_GENERIC_INST_CLASS) &&
...
static bool verify_generic_inst_rows(const XgGlobalEvidence *ev, char *errbuf, size_t errbuf_len) {
    ...
    for (uint32_t i = 0; i < ev->ngeneric_insts; i++) {
        ...
        if (!verify_generic_specialization_identity_shape(
                ev, inst->kind, inst->origin_nominal_key, inst->receiver_class_id,
                inst->receiver_type_key, inst->receiver_type_arg_key_start,
                inst->receiver_type_arg_count, inst->declaration_type_key,
                inst->declaration_type_arg_key_start, inst->declaration_type_arg_count,
                inst->specialization_effect, "generic inst", errbuf, errbuf_len))
            return false;
        if (inst->kind == XG_GENERIC_INST_CLASS || inst->kind == XG_GENERIC_INST_METHOD) {
            const XgDeclSummary *origin_decl = verify_find_evidence_decl(ev, inst->origin_decl_id);
            const XgModuleSummary *origin_module =
                origin_decl ? verify_find_evidence_module(ev, origin_decl->module_id) : NULL;
            if (!origin_decl || !origin_module ||
                ...
                return set_error(errbuf, errbuf_len,
                                 "AOT declaration nominal identity does not re-derive");
            if (inst->origin_nominal_key != origin_decl->nominal_key)
                return set_error(errbuf, errbuf_len,
                                 "AOT generic inst origin nominal identity does not re-derive");
            if (inst->kind == XG_GENERIC_INST_CLASS) {
                const XgClassSummary *origin_class =
                    verify_find_evidence_class(ev, inst->origin_class_id);
                if (!origin_class || origin_class->decl_id != origin_decl->decl_id ||
                    origin_class->name_id != inst->name_id ||
                    (origin_class->flags & XG_CLASS_GENERIC_SKELETON) == 0u ||
                    (origin_class->flags & XG_CLASS_MONOMORPHIZED) != 0u)
                    return set_error(errbuf, errbuf_len,
                                     "AOT generic class source owner identity is stale");
            }
        }
        ...
        if (inst->specialized_class_id != XG_NO_ID) {
            ...
            if (specialized_class->generic_origin_name_id != inst->name_id ||
                specialized_class->generic_origin_nominal_key != inst->origin_nominal_key ||
                specialized_class->generic_type_key != inst->declaration_type_key ||
                specialized_class->generic_type_key !=
                    xg_generic_nominal_type_key(specialized_class->generic_origin_nominal_key,
                                                specialized_class->generic_type_arg_key_start,
                                                specialized_class->generic_type_arg_count,
        ...
        if (inst->root_callsite_id != XG_NO_ID) {
            const XgCallsiteSummary *root =
                verify_find_evidence_callsite(ev, inst->root_callsite_id);
            const XgBodySummary *owner =
                root ? verify_find_evidence_body_by_func(ev, root->owner_func_id) : NULL;
            if (!root || !owner || owner->module_id != inst->module_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT generic inst caller module does not re-derive");
        }
...
static bool verify_generic_deepen_rows(const XgGlobalEvidence *ev, char *errbuf,
    ...
    for (uint32_t i = 0; i < ev->ngeneric_body_uses; i++) {
        ...
        if (!verify_generic_specialization_identity_shape(
                ev, inst->kind, use->origin_nominal_key, use->receiver_class_id,
                use->receiver_type_key, use->receiver_type_arg_key_start,
                use->receiver_type_arg_count, use->declaration_type_key,
                use->declaration_type_arg_key_start, use->declaration_type_arg_count,
                use->specialization_effect, "generic body-use", errbuf, errbuf_len))
            return false;
        if (use->module_id != inst->module_id ||
            use->origin_nominal_key != inst->origin_nominal_key ||
                             ...
                             "AOT generic body-use identity does not match inst");
        {
            ...
            if (!root || use->root_callsite_id != inst->root_callsite_id ||
                use->owner_func_id != root->owner_func_id ||
                use->origin_body_func_id != inst->origin_func_id ||
                use->specialized_body_func_id != inst->specialized_func_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT generic body-use call identity does not match inst");
        }
...
static bool verify_derive_rows(const XgGlobalEvidence *ev, char *errbuf, size_t errbuf_len) {
    ...
    for (uint32_t i = 0; i < ev->nderived_fields; i++) {
        ...
        derive = verify_find_derive_by_id(ev, field->derive_id);
        if (!derive)
            return set_error(errbuf, errbuf_len, "AOT derived field owner derive is missing");
        source = xg_global_evidence_find_class_field(ev, field->source_field_id);
        if (field->source_field_id == XG_NO_ID || !source)
            return set_error(errbuf, errbuf_len, "AOT derived source field is missing");
        owner = verify_find_evidence_class(ev, source->owner_class_id);
        ...
        if (source->name_id != field->name_id || source->type_key != field->type_key)
            return set_error(errbuf, errbuf_len,
                             "AOT derived source field identity does not re-derive");
        expected_flags = (source->flags & XG_CLASS_FIELD_PRIVATE) != 0 ? XG_DERIVED_FIELD_PRIVATE
                         : (source->flags & XG_CLASS_FIELD_PROTECTED) != 0
                             ? XG_DERIVED_FIELD_PROTECTED
                             : XG_DERIVED_FIELD_PUBLIC;
        if ((source->flags & XG_CLASS_FIELD_CONST) != 0)
            expected_flags |= XG_DERIVED_FIELD_READONLY;
        if (field->flags != expected_flags)
            return set_error(errbuf, errbuf_len, "AOT derived source field flags do not re-derive");
    }
...
static bool verify_json_codec_shape_contract(const XgGlobalEvidence *ev,
                                             const XgJsonCodecSummary *codec, char *errbuf,
                                             size_t errbuf_len) {
    const XgObjectShapeSummary *input_shape = NULL;
    const XgObjectShapeSummary *output_shape = NULL;
    bool has_input_shape;
    bool has_output_shape;

    if (!ev || !codec)
        return set_error(errbuf, errbuf_len, "AOT Json codec shape verifier has incomplete input");
    has_input_shape = (codec->flags & XG_JSON_CODEC_HAS_INPUT_SHAPE) != 0;
    has_output_shape = (codec->flags & XG_JSON_CODEC_HAS_OUTPUT_SHAPE) != 0;
    bool target_object_shape = (codec->flags & XG_JSON_CODEC_TARGET_OBJECT_SHAPE) != 0;
    if (has_input_shape != (codec->input_shape_id != XG_NO_ID))
        return set_error(errbuf, errbuf_len, "AOT Json codec input shape flag does not re-derive");
    if (has_output_shape != (codec->output_shape_id != XG_NO_ID))
        return set_error(errbuf, errbuf_len, "AOT Json codec output shape flag does not re-derive");
    if (has_input_shape) {
        input_shape = xg_global_evidence_find_object_shape(ev, codec->input_shape_id);
        if (!input_shape)
            return set_error(errbuf, errbuf_len,
                             "AOT Json codec references missing input Object shape");
    }
    if (has_output_shape) {
        output_shape = xg_global_evidence_find_object_shape(ev, codec->output_shape_id);
        if (!output_shape)
            return set_error(errbuf, errbuf_len,
                             "AOT Json codec references missing output Object shape");
    }
    switch ((XgJsonCodecKind) codec->codec_kind) {
        ...
        case XG_JSON_CODEC_STRINGIFY:
            if (output_shape)
                return set_error(errbuf, errbuf_len,
                                 "AOT Json stringify unexpectedly has an output shape");
            break;
        default:
            return set_error(errbuf, errbuf_len, "AOT Json codec evidence has invalid kind");
    }
    if (codec->field_count !=
        (output_shape ? output_shape->field_count : (input_shape ? input_shape->field_count : 0)))
        return set_error(errbuf, errbuf_len,
                         "AOT Json codec field count does not re-derive from Object shape");
    return true;
}
...
static bool verify_object_shape_kind_valid(uint8_t kind) {
...
static bool verify_object_access_kind_valid(uint8_t kind) {
...
static bool verify_object_shape_identity(const XgGlobalEvidence *ev,
    ...
    if (!ev || !shape)
        return set_error(errbuf, errbuf_len,
                         "AOT Object shape identity verifier has incomplete input");
    if (shape->field_count != 0) {
        fields = (XgObjectFieldSummary *) xr_calloc(shape->field_count, sizeof(*fields));
        if (!fields)
            return set_error(errbuf, errbuf_len,
                             "AOT Object shape identity verifier ran out of memory");
        for (uint32_t i = 0; i < ev->nobject_fields; i++) {
            ...
            if (field->field_ordinal >= shape->field_count) {
                set_error(errbuf, errbuf_len, "AOT Object field ordinal is stale");
                goto done;
            }
            if (fields[field->field_ordinal].shape_id != XG_NO_ID) {
                set_error(errbuf, errbuf_len, "AOT Object field ordinal is duplicated");
                goto done;
            }
            fields[field->field_ordinal] = *field;
            found++;
        }
        if (found != shape->field_count) {
            set_error(errbuf, errbuf_len, "AOT Object field ordinals have a gap");
            goto done;
        }
        for (uint16_t ordinal = 0; ordinal < shape->field_count; ordinal++) {
            if (fields[ordinal].stable_name_key == 0) {
                set_error(errbuf, errbuf_len, "AOT Object field stable name key is missing");
                goto done;
            }
            if (shape->domain == XG_OBJECT_DOMAIN_STRUCT &&
                (shape->flags & XG_OBJECT_SHAPE_HAS_OPTIONS) == 0 && ordinal > 0 &&
                (fields[ordinal - 1].stable_name_key > fields[ordinal].stable_name_key ||
                 (fields[ordinal - 1].stable_name_key == fields[ordinal].stable_name_key &&
                  fields[ordinal - 1].name_id > fields[ordinal].name_id))) {
                set_error(errbuf, errbuf_len, "AOT structural Object field table is not canonical");
                goto done;
            }
        }
    }
    if (!xg_object_shape_stable_key(shape->domain, fields, shape->field_count, &expected_key) ||
        expected_key != shape->stable_shape_key) {
        set_error(errbuf, errbuf_len,
                  "AOT Object shape stable key does not re-derive from its field table");
        goto done;
    }
    if (shape->stable_type_key !=
        (shape->type_key ? (uint64_t) shape->type_key : shape->stable_shape_key)) {
        set_error(errbuf, errbuf_len, "AOT Object shape stable type key is stale");
        goto done;
    }
    ok = true;
...
static bool verify_object_rows(const XgGlobalEvidence *ev, char *errbuf, size_t errbuf_len) {
    ...
    for (uint32_t i = 0; i < ev->nobject_shapes; i++) {
        ...
        for (uint32_t j = i + 1; j < ev->nobject_shapes; j++) {
            if (ev->object_shapes[j].object_shape_id == shape->object_shape_id)
                return set_error(errbuf, errbuf_len, "AOT Object shape evidence id is duplicated");
        }
    }
    for (uint32_t i = 0; i < ev->nobject_fields; i++) {
        const XgObjectFieldSummary *field = &ev->object_fields[i];
        ...
        for (uint32_t j = i + 1; j < ev->nobject_fields; j++) {
            if (ev->object_fields[j].field_id == field->field_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT structural object field evidence id is duplicated");
        }
        shape = xg_global_evidence_find_object_shape(ev, field->shape_id);
        if (!shape)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object field references missing shape");
        if (field->field_ordinal >= shape->field_count)
        ...
        for (uint32_t j = i + 1; j < ev->nobject_fields; j++) {
            ...
            if (other->shape_id != field->shape_id)
                continue;
            if (other->field_ordinal == field->field_ordinal)
                return set_error(errbuf, errbuf_len, "AOT Object field ordinal is duplicated");
            if (other->name_id == field->name_id)
                return set_error(errbuf, errbuf_len, "AOT Object field name is duplicated");
        }
    }
    for (uint32_t i = 0; i < ev->nobject_accesses; i++) {
        ...
        if (!verify_object_access_kind_valid(access->access_kind))
            return set_error(errbuf, errbuf_len,
                             "AOT structural object access evidence has invalid kind");
        for (uint32_t j = i + 1; j < ev->nobject_accesses; j++) {
            if (ev->object_accesses[j].object_access_id == access->object_access_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT structural object access evidence id is duplicated");
        }
        if (access->receiver_shape_count == 0 || access->receiver_shape_set_id == 0)
            return set_error(errbuf, errbuf_len, "AOT Object access receiver shape set is missing");
        for (uint32_t j = 0; j < ev->nobject_access_cases; j++) {
            const XgObjectAccessCaseSummary *access_case = &ev->object_access_cases[j];
            const XgObjectShapeSummary *case_shape;
            const XgObjectFieldSummary *case_field = NULL;
            if (access_case->object_access_id != access->object_access_id)
                continue;
            if (access_case->case_id == XG_NO_ID ||
                access_case->receiver_shape_set_id != access->receiver_shape_set_id ||
                access_case->domain != access->domain)
                return set_error(errbuf, errbuf_len, "AOT Object access case identity is stale");
            for (uint32_t k = j + 1; k < ev->nobject_access_cases; k++) {
                const XgObjectAccessCaseSummary *other = &ev->object_access_cases[k];
                if (other->case_id == access_case->case_id)
                    return set_error(errbuf, errbuf_len, "AOT Object access case id is duplicated");
                if (other->object_access_id == access->object_access_id &&
                    other->receiver_shape_id == access_case->receiver_shape_id)
                    return set_error(errbuf, errbuf_len,
                                     "AOT Object access receiver shape is duplicated");
            }
            case_shape = xg_global_evidence_find_object_shape(ev, access_case->receiver_shape_id);
            if (!case_shape || case_shape->domain != access_case->domain ||
                case_shape->stable_shape_key != access_case->stable_shape_key ||
                case_shape->mutation_epoch != access_case->mutation_epoch)
                return set_error(errbuf, errbuf_len,
                                 "AOT Object access case shape identity is stale");
            for (uint32_t k = 0; k < ev->nobject_fields; k++) {
                const XgObjectFieldSummary *candidate = &ev->object_fields[k];
                if (candidate->shape_id == case_shape->object_shape_id &&
                    candidate->field_ordinal == access_case->field_ordinal) {
                    case_field = candidate;
                    break;
                }
            }
            if (!case_field || case_field->name_id != access->field_name_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT Object access case ordinal does not match its field");
            if (access_case->receiver_shape_id == access->receiver_shape_id) {
                if (found_representative || access_case->field_ordinal != access->field_ordinal)
                    return set_error(errbuf, errbuf_len,
                                     "AOT Object access representative shape is stale");
        ...
        if (!shape)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object access references missing shape");
        ...
        if ((access->flags & XG_OBJECT_ACCESS_STATIC_FIELD) != 0 && access->field_name_id != 0 &&
            ...
            return set_error(errbuf, errbuf_len,
                             "AOT structural object access field ordinal is stale");
    }
    for (uint32_t i = 0; i < ev->nobject_merges; i++) {
        ...
        for (uint32_t j = i + 1; j < ev->nobject_merges; j++) {
            if (ev->object_merges[j].merge_id == merge->merge_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT structural object merge evidence id is duplicated");
        ...
        if (!base || !patch || !result)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object merge references missing shape");
        if (base->field_count != merge->base_field_count ||
            ...
            return set_error(errbuf, errbuf_len,
                             "AOT structural object merge field count is stale");
        if (merge->overwrite_count > merge->patch_field_count)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object merge overwrite count is stale");
    }
    return true;
}
        ...
        switch ((XgCallsiteKind) call->kind) {
            ...
            case XG_CALL_CLOSURE:
                ...
                if (call->static_target_func_id != XG_NO_ID &&
                    !verify_find_evidence_body_by_func(ev, call->static_target_func_id))
                    return set_error(errbuf, errbuf_len,
                                     "AOT global evidence closure target body is missing");
                break;
            case XG_CALL_NATIVE:
                if (call->static_target_func_id != XG_NO_ID ||
                    call->receiver_static_class_id != XG_NO_ID ||
    ...
    for (uint32_t i = 0; i < ev->nbodies; i++) {
        ...
        switch ((XgBodyKind) body->kind) {
            ...
            case XG_BODY_FUNCTION:
                if (body->source_node_id == 0 || body->owner_method_id != XG_NO_ID ||
                ...
                if (body->owner_decl_id != XG_NO_ID) {
                    ...
                    if (owner_decl->kind != XG_DECL_FUNC ||
                ...
                } else if (body->owner_class_id != XG_NO_ID &&
                           !verify_find_evidence_class(ev, body->owner_class_id)) {
                    return set_error(errbuf, errbuf_len,
                                     "AOT global evidence function body owner class is missing");
                }
                break;
            case XG_BODY_METHOD:
                if (body->lexical_parent_func_id != XG_NO_ID || body->source_node_id == 0 ||
...
static bool verify_derived_clone_field_range_valid(const XgGlobalEvidence *ev,
    ...
    return end >= clone->field_start && end <= ev->nderived_fields;
}

static const XgClassSummary *verify_derived_clone_class_by_id(const XgGlobalEvidence *ev,
                                                              XgClassId class_id) {
    if (!ev || class_id == XG_NO_ID)
        return NULL;
    for (uint32_t i = 0; i < ev->nclasses; i++) {
...
static bool verify_derived_clone_class_has_clone(const XgGlobalEvidence *ev, XgClassId class_id) {
    ...
    if (!cls)
        return false;
    for (uint32_t i = 0; i < ev->nderives; i++) {
        if (ev->derives[i].owner_decl_id == cls->decl_id &&
            ev->derives[i].derive_kind == XG_DERIVE_CLONE)
            return true;
    }
    return false;
}
    ...
    for (uint32_t i = 0; i < clone->field_count; i++) {
        ...
        switch ((XgClassFieldTypeKind) field->semantic_kind) {
            ...
            case XG_CLASS_FIELD_TYPE_CLASS:
                if (!verify_derived_clone_class_has_clone(ev, field->target_class_id))
                    return XAOT_DERIVED_CLONE_REJECT;
                needs_deep_copy = true;
                break;
...
static uint8_t verify_derived_clone_action_for(const XgGlobalEvidence *ev,
                                               const XgDeriveSummary *clone) {
    if (!clone || clone->derive_kind != XG_DERIVE_CLONE)
        return XAOT_DERIVED_CLONE_REJECT;
    if (clone->method_count != 0)
        return XAOT_DERIVED_CLONE_DIRECT_GENERATED_CALL;
    return verify_derived_clone_source_action(ev, clone);
}

static uint8_t verify_derived_clone_reason_for(const XgGlobalEvidence *ev,
                                               const XgDeriveSummary *clone) {
    if (!clone || clone->derive_kind != XG_DERIVE_CLONE)
        return XAOT_CLONE_UNPROVEN_MISSING_CLONE;
    if (verify_derived_clone_action_for(ev, clone) == XAOT_DERIVED_CLONE_REJECT)
...
static bool verify_derived_clone_plan_rederives(const XgGlobalEvidence *ev,
    ...
    if (plan->action != verify_derived_clone_action_for(ev, clone) ||
        plan->unproven_reason != verify_derived_clone_reason_for(ev, clone))
...
static uint32_t verify_object_shape_evidence_for(const XgObjectShapeSummary *shape) {
...
static bool verify_object_shape_plan_rederives(const XaotObjectShapePlan *plan,
                                               const XgObjectShapeSummary *shape, char *errbuf,
                                               size_t errbuf_len) {
    if (!plan || !shape)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object shape verifier has incomplete input");
    if (plan->object_shape_id != shape->object_shape_id || plan->module_id != shape->module_id ||
        ...
        plan->domain != shape->domain || plan->provenance != shape->provenance ||
        plan->concrete_exact != shape->concrete_exact || plan->fresh != shape->fresh ||
        plan->escaped != shape->escaped || plan->mutation_epoch != shape->mutation_epoch ||
        plan->stable_type_key != shape->stable_type_key ||
        plan->stable_shape_key != shape->stable_shape_key || plan->shape_hash != shape->shape_hash)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object shape plan identity does not re-derive");
    if (plan->action != verify_object_shape_action_for(shape) ||
        plan->unproven_reason != verify_object_shape_reason_for(shape))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object shape plan action does not re-derive");
    if (plan->evidence != verify_object_shape_evidence_for(shape))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object shape plan evidence does not re-derive");
    return true;
}
...
static bool verify_object_access_case_set(const XgGlobalEvidence *ev,
    ...
    for (uint32_t i = 0; i < ev->nobject_access_cases; i++) {
        ...
        const XgObjectShapeSummary *shape;
        bool ordinal_matches_name = false;
        if (access_case->object_access_id != access->object_access_id)
            continue;
        if (access_case->receiver_shape_set_id != access->receiver_shape_set_id ||
            access_case->domain != access->domain)
            return false;
        shape = xg_global_evidence_find_object_shape(ev, access_case->receiver_shape_id);
        ...
        for (uint32_t j = 0; j < ev->nobject_fields; j++) {
            ...
            if (field->shape_id == access_case->receiver_shape_id &&
                field->field_ordinal == access_case->field_ordinal &&
                field->name_id == access->field_name_id) {
                ordinal_matches_name = true;
                break;
            }
        }
...
static uint32_t verify_object_access_evidence_for(const XgGlobalEvidence *ev,
                                                  const XgObjectAccessSummary *access) {
    const XgObjectShapeSummary *shape;
    uint32_t evidence = XAOT_OBJECT_EV_GLOBAL_ROW;
    if (!access)
    ...
    if ((access->flags & XG_OBJECT_ACCESS_STATIC_FIELD) != 0 && access->field_name_id != 0)
        evidence |= XAOT_OBJECT_EV_STATIC_FIELD;
    if (access->receiver_shape_id != XG_NO_ID)
        evidence |= XAOT_OBJECT_EV_RECEIVER_SHAPE;
    shape = xg_global_evidence_find_object_shape(ev, access->receiver_shape_id);
    if (shape && access->field_ordinal < shape->field_count)
        evidence |= XAOT_OBJECT_EV_FIELD_INDEX;
    return evidence;
}

static bool verify_object_access_plan_rederives(const XgGlobalEvidence *ev,
                                                const XaotBundle *bundle,
    ...
    if (!ev || !bundle || !plan || !access)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object access verifier has incomplete input");
    if (plan->object_access_id != access->object_access_id ||
        ...
        plan->domain != access->domain || plan->syntax != access->syntax ||
        ...
        return set_error(errbuf, errbuf_len,
                         "AOT structural object access plan identity does not re-derive");
    if (plan->dispatch_case_start > bundle->nobject_access_case_plans ||
        ...
        return set_error(errbuf, errbuf_len, "AOT Object access dispatch case range is stale");
    uint32_t matched = 0;
    for (uint32_t i = 0; i < ev->nobject_access_cases; i++) {
        const XgObjectAccessCaseSummary *source = &ev->object_access_cases[i];
        const XaotObjectAccessCasePlan *target;
        if (source->object_access_id != access->object_access_id)
            continue;
        if (matched >= access->receiver_shape_count)
            return set_error(errbuf, errbuf_len, "AOT Object access dispatch case count is stale");
        target = &bundle->object_access_case_plans[plan->dispatch_case_start + matched];
        if (target->case_id != source->case_id ||
            ...
            target->field_ordinal != source->field_ordinal || target->domain != source->domain)
            return set_error(errbuf, errbuf_len,
                             "AOT Object access dispatch case does not re-derive");
        matched++;
    }
    if (matched != access->receiver_shape_count)
        return set_error(errbuf, errbuf_len, "AOT Object access dispatch case set is incomplete");
    if (plan->action != verify_object_access_action_for(ev, access) ||
        plan->unproven_reason != verify_object_access_reason_for(ev, access))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object access plan action does not re-derive");
    if (plan->evidence != verify_object_access_evidence_for(ev, access))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object access plan evidence does not re-derive");
    return true;
}

static uint8_t verify_object_merge_reason_for(const XgGlobalEvidence *ev,
...
static uint8_t verify_object_merge_action_for(const XgGlobalEvidence *ev,
                                              const XgObjectMergeSummary *merge) {
...
static uint32_t verify_object_merge_evidence_for(const XgGlobalEvidence *ev,
                                                 const XgObjectMergeSummary *merge) {
    uint32_t evidence = XAOT_OBJECT_EV_GLOBAL_ROW;
    if (!ev || !merge)
        return evidence;
    if (xg_global_evidence_find_object_shape(ev, merge->base_shape_id))
        evidence |= XAOT_OBJECT_EV_BASE_SHAPE;
    if (xg_global_evidence_find_object_shape(ev, merge->patch_shape_id))
        evidence |= XAOT_OBJECT_EV_PATCH_SHAPE;
    if (xg_global_evidence_find_object_shape(ev, merge->result_shape_id))
...
static bool verify_object_merge_plan_rederives(const XgGlobalEvidence *ev,
                                               const XaotObjectMergePlan *plan,
                                               const XgObjectMergeSummary *merge, char *errbuf,
                                               size_t errbuf_len) {
    if (!ev || !plan || !merge)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object merge verifier has incomplete input");
    if (plan->merge_id != merge->merge_id || plan->module_id != merge->module_id ||
        ...
        return set_error(errbuf, errbuf_len,
                         "AOT structural object merge plan identity does not re-derive");
    if (plan->action != verify_object_merge_action_for(ev, merge) ||
        plan->unproven_reason != verify_object_merge_reason_for(ev, merge))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object merge plan action does not re-derive");
    if (plan->evidence != verify_object_merge_evidence_for(ev, merge))
        return set_error(errbuf, errbuf_len,
                         "AOT structural object merge plan evidence does not re-derive");
    return true;
}
...
static uint8_t verify_options_row_unproven_reason(const XgGlobalEvidence *ev,
    ...
    param_shape = xg_global_evidence_find_object_shape(ev, options->param_shape_id);
    ...
    if (options->supplied_shape_id != XG_NO_ID) {
        supplied_shape = xg_global_evidence_find_object_shape(ev, options->supplied_shape_id);
...
static uint32_t verify_options_evidence_for(const XgGlobalEvidence *ev,
    ...
    if (xg_global_evidence_find_object_shape(ev, options->param_shape_id))
        evidence |= XAOT_OPTIONS_EV_PARAM_SHAPE;
    if (options->supplied_shape_id != XG_NO_ID &&
        xg_global_evidence_find_object_shape(ev, options->supplied_shape_id))
...
XR_FUNC bool xaot_verify_global_evidence_plan(const XaotBundle *bundle, char *errbuf,
    ...
    if (!verify_json_rows(ev, errbuf, errbuf_len))
        return false;
    if (!verify_object_rows(ev, errbuf, errbuf_len))
        return false;
    ...
    if (bundle->nobject_shape_plans != expected_object_shape_plans)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object shape plan count mismatches evidence");
    ...
    for (uint32_t i = 0; i < ev->nobject_accesses; i++) {
        ...
        if (!plan)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object access evidence has no access plan");
    ...
    if (bundle->nobject_access_plans != expected_object_access_plans)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object access plan count mismatches evidence");
    ...
    for (uint32_t i = 0; i < ev->nobject_merges; i++) {
        ...
        if (!plan)
            return set_error(errbuf, errbuf_len,
                             "AOT structural object merge evidence has no merge plan");
    ...
    if (bundle->nobject_merge_plans != expected_object_merge_plans)
        return set_error(errbuf, errbuf_len,
                         "AOT structural object merge plan count mismatches evidence");
...
verify_func_values_have_plans_recursive(const XaotBundle *bundle, const XiFunc *func,
    ...
    for (bi = 0; bi < func->nblocks; bi++) {
        ...
        for (phi = blk->phis; phi; phi = phi->next) {
            ...
            if (!binding && !legacy) {
                bool elided = false;
                if (!verify_program_callee_operand_elision(bundle, func, &phi->value, &elided,
                                                           errbuf, errbuf_len))
                    return false;
                if (!elided)
        ...
        for (vi = 0; vi < blk->nvalues; vi++) {
            ...
            if (!binding && !legacy) {
                bool elided = false;
                if (!verify_program_callee_operand_elision(bundle, func, blk->values[vi], &elided,
                                                           errbuf, errbuf_len))
                    return false;
                if (!elided)
                    return set_error(errbuf, errbuf_len,
...
static bool verify_zero_value_rep(XaotValueRep rep) {
    return rep.kind == (XaotValueKind) 0 && rep.rep == (XaotRep) 0 && rep.type == NULL &&
           rep.c_type == NULL && rep.flags == 0u && rep.vector_native_type == 0u &&
           rep.vector_lanes == 0u && rep.vector_width_bytes == 0u;
}

static bool verify_target_owned_abi_is_empty(const XaotFuncAbi *abi) {
    return abi && abi->kind == XAOT_ABI_NATIVE && abi->ret.cls == XAOT_ARG_VOID &&
           verify_zero_value_rep(abi->ret.rep) && verify_zero_value_rep(abi->ret.pointee_rep) &&
           abi->ret.c_type == NULL && abi->ret.flags == 0u && abi->params == NULL &&
           abi->nparams == 0u && abi->boundary_reason == XAOT_BOUNDARY_NONE;
}
...
static bool verify_abi_plan(const XaotBundle *bundle, const XaotFuncPlan *plan, char *errbuf,
    ...
    if (plan->abi_authority == XAOT_FUNC_ABI_AUTHORITY_TARGET_PLAN) {
        ...
        XaotLeafAggregateTargetStatus leaf_status =
            xaot_boundary_leaf_aggregate_abi_status(bundle, plan->func, errbuf, errbuf_len);
        if (leaf_status == XAOT_LEAF_AGGREGATE_TARGET_INVALID)
            return false;
        if (leaf_status != XAOT_LEAF_AGGREGATE_TARGET_FOUND &&
            xaot_boundary_direct_i64_abi_status(bundle, plan->func, NULL, errbuf, errbuf_len) !=
                XAOT_DIRECT_I64_TARGET_FOUND)
            return set_error(errbuf, errbuf_len, "TargetPlan-owned function authority is invalid");
        return true;
    }
...
static bool verify_extern_registry(const XaotBundle *bundle, char *errbuf, size_t errbuf_len) {
    ...
    for (uint32_t i = 0; i < bundle->nextern_decls; i++) {
        ...
        if (decl->symbol_qualified != decl->representative_func->extern_symbol_qualified ||
            ...
            return set_error(errbuf, errbuf_len, "AOT extern source/native qualification is stale");
...
static bool verify_func_extern_calls(const XaotBundle *bundle, const XiFunc *func,
                                     const XrCProgramDirectI64EmissionBinding *program_scope,
                                     char *errbuf, size_t errbuf_len) {
    if (!func)
        return true;
    ...
    for (uint16_t ci = 0; ci < func->nchildren; ci++) {
        if (!verify_func_extern_calls(bundle, func->children[ci], program_scope, errbuf,
                                      errbuf_len))
            return false;
    }
    return true;
...
static bool verify_direct_call_boundaries(const XaotBundle *bundle, const XiFunc *func,
    ...
    XaotDirectI64TargetStatus direct_i64_status =
        xaot_boundary_direct_i64_call_view(bundle, func, call, &direct_i64, errbuf, errbuf_len);
...
static bool
verify_func_boundaries_recursive(const XaotBundle *bundle, const XiFunc *func,
                                 const XrCProgramDirectI64EmissionBinding *program_scope,
                                 char *errbuf, size_t errbuf_len) {
    uint32_t bi;
    uint16_t ci;
...
static bool
verify_func_closure_plans_recursive(const XaotBundle *bundle, const XiFunc *func,
                                    const XrCProgramDirectI64EmissionBinding *program_scope,
                                    uint32_t *out_count, char *errbuf, size_t errbuf_len) {
    uint32_t bi;
    uint16_t ci;
...
static bool
verify_func_stack_allocations_recursive(const XaotBundle *bundle, const XiFunc *func,
                                        const XrCProgramDirectI64EmissionBinding *program_scope,
                                        char *errbuf, size_t errbuf_len) {
    uint32_t bi;
    uint16_t ci;
    ...
    for (bi = 0; bi < func->nblocks; bi++) {
        ...
        for (vi = 0; vi < blk->nvalues; vi++) {
            ...
            if (value->escape != (uint8_t) XI_ESC_NONE)
                return set_error(errbuf, errbuf_len, "AOT stack allocation is escaping");
        }
    }
...
static bool
verify_func_transfer_plans_recursive(const XaotBundle *bundle, const XiFunc *func,
                                     const XrCProgramDirectI64EmissionBinding *program_scope,
                                     uint32_t *out_count, char *errbuf, size_t errbuf_len) {
    uint32_t bi;
    uint16_t ci;
...
static bool verify_bundle_function_plans(const XaotBundle *bundle, uint32_t *closure_count,
                                         uint32_t *transfer_count, char *errbuf,
                                         size_t errbuf_len) {
    const XrTargetPlan *target = xaot_bundle_program_target_plan(bundle);
    uint32_t graph_count = 0;
    ...
    if (graph_count != 0u) {
        if (!graphs || graph_count != 1u ||
            !xr_c_program_direct_i64_emission_bind(target, bundle->modules, bundle->nmodules,
                                                   &program_scope, error, sizeof(error))) {
            xr_c_program_direct_i64_emission_release(&program_scope);
            return set_error(errbuf, errbuf_len,
                             error[0] ? error
                                      : "AOT verifier has no exact program C-emission scope");
        }
        scope = &program_scope;
        for (uint32_t plan_index = 0; plan_index < bundle->nfunc_plans; plan_index++) {
            const XiFunc *function = bundle->func_plans[plan_index].func;
            if (function && xr_c_program_direct_i64_function_binding(scope, function))
                continue;
            xr_c_program_direct_i64_emission_release(&program_scope);
            return set_error(errbuf, errbuf_len,
                             "excluded Xi function retains an AOT function plan");
    ...
    bool exact = true;
    for (uint32_t module_index = 0; exact && module_index < bundle->nmodules; module_index++) {
        const XiModule *module = bundle->modules[module_index];
        if (!module || !module->init) {
            exact = set_error(errbuf, errbuf_len, "AOT bundle contains an invalid module");
            break;
        }
        exact = verify_func_has_plan_recursive(bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_values_have_plans_recursive(bundle, module->init, scope, errbuf,
                                                        errbuf_len) &&
                verify_func_boundaries_recursive(bundle, module->init, scope, errbuf, errbuf_len) &&
                verify_func_stack_allocations_recursive(bundle, module->init, scope, errbuf,
                                                        errbuf_len) &&
                verify_func_closure_plans_recursive(bundle, module->init, scope, closure_count,
                                                    errbuf, errbuf_len) &&
                verify_func_transfer_plans_recursive(bundle, module->init, scope, transfer_count,
                                                     errbuf, errbuf_len) &&
                verify_func_extern_calls(bundle, module->init, scope, errbuf, errbuf_len);
    }
    xr_c_program_direct_i64_emission_release(&program_scope);
    return exact;
}
...
XR_FUNC bool xaot_verify_bundle(const XaotBundle *bundle, char *errbuf, size_t errbuf_len) {
    ...
    if (!verify_bundle_function_plans(bundle, &closure_count, &transfer_count, errbuf, errbuf_len))
        return false;
