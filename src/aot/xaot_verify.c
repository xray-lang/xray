...
static bool verify_json_codec_shape_contract(const XgGlobalEvidence *ev,
    ...
    if (has_output_shape != (codec->output_shape_id != XG_NO_ID))
        return set_error(errbuf, errbuf_len,
                         "AOT Json codec output shape flag does not re-derive");
    if (has_input_shape) {
    ...
    switch ((XgJsonCodecKind) codec->codec_kind) {
        ...
        default:
            return set_error(errbuf, errbuf_len,
                             "AOT Json codec evidence has invalid kind");
    }
    if (codec->field_count !=
        (output_shape ? output_shape->field_count
                      : (input_shape ? input_shape->field_count : 0)))
        return set_error(errbuf, errbuf_len,
...
static bool verify_json_rows(const XgGlobalEvidence *ev, char *errbuf, size_t errbuf_len) {
    if (!ev)
        return set_error(errbuf, errbuf_len,
                         "AOT global evidence Json verifier has no evidence");
    for (uint32_t i = 0; i < ev->njson_dynamic_accesses; i++) {
        ...
        if (access->json_dynamic_access_id == XG_NO_ID)
            return set_error(errbuf, errbuf_len,
                             "AOT Json dynamic access evidence has no id");
        ...
        if ((access->flags & XG_JSON_DYNAMIC_ACCESS_COMPUTED_KEY) != 0) {
            ...
            return set_error(errbuf, errbuf_len,
                             "AOT Json dynamic access field ordinal is stale");
        }
    }
    for (uint32_t i = 0; i < ev->njson_codecs; i++) {
        ...
        if (codec->codec_id == XG_NO_ID)
            return set_error(errbuf, errbuf_len,
                             "AOT Json codec evidence has no id");
        if (codec->owner_func_id == XG_NO_ID || codec->source_node_id == 0)
            return set_error(errbuf, errbuf_len,
                             "AOT Json codec source identity is missing");
        ...
        for (uint32_t j = i + 1; j < ev->njson_codecs; j++) {
            if (ev->json_codecs[j].codec_id == codec->codec_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT Json codec evidence id is duplicated");
        ...
        for (uint16_t ordinal = 0; ordinal < shape->field_count; ordinal++) {
            ...
            if (shape->domain == XG_OBJECT_DOMAIN_STRUCT &&
                ...
                set_error(errbuf, errbuf_len,
                          "AOT structural Object field table is not canonical");
                goto done;
            }
        }
    }
...
static bool verify_object_shape_flows(const XgGlobalEvidence *ev, char *errbuf,
                                      size_t errbuf_len) {
    if (!ev)
        return false;
    for (uint32_t i = 0; i < ev->nobject_shape_flows; i++) {
        ...
        const XgCallsiteSummary *callsite =
            verify_find_object_flow_callsite(ev, flow->callsite_id);
        const uint32_t kind = flow->flags &
                              (XG_OBJECT_SHAPE_FLOW_CONCRETE |
                               XG_OBJECT_SHAPE_FLOW_FORWARDED);
        ...
        if (kind == 0 || kind == (XG_OBJECT_SHAPE_FLOW_CONCRETE |
                                  XG_OBJECT_SHAPE_FLOW_FORWARDED) ||
            (flow->flags & ~(XG_OBJECT_SHAPE_FLOW_CONCRETE |
                             XG_OBJECT_SHAPE_FLOW_FORWARDED)) != 0)
        ...
        for (uint32_t j = i + 1; j < ev->nobject_shape_flows; j++) {
            ...
            if (other->flow_id == flow->flow_id)
                return set_error(errbuf, errbuf_len,
                                 "AOT Object shape flow id is duplicated");
...
static bool verify_collect_object_flow_shapes(const XgGlobalEvidence *ev,
    ...
    for (uint32_t i = 0; i < *visit_count; i++) {
        if (visits[i].func_id == target_func_id &&
            visits[i].param_ordinal == target_param_ordinal)
            return true;
    }
...
static bool verify_open_object_access_shape_set(const XgGlobalEvidence *ev,
    ...
    if (!constraint || (constraint->flags & XG_OBJECT_SHAPE_OPEN_ROW) == 0 ||
        ...
        return set_error(errbuf, errbuf_len,
                         "AOT open Object access constraint shape is stale");
    for (uint32_t i = 0; i < ev->nobject_fields; i++) {
    ...
    if (!verify_collect_object_flow_shapes(
            ev, access->owner_func_id, access->receiver_param_ordinal, shape_ids,
            ev->nobject_shape_flows, &shape_count, visits, ev->nobject_shape_flows + 1,
            &visit_count) ||
...
static bool verify_object_rows(const XgGlobalEvidence *ev, char *errbuf, size_t errbuf_len) {
    ...
    for (uint32_t i = 0; i < ev->nobject_accesses; i++) {
        ...
        if (access->receiver_shape_count == 0 || access->receiver_shape_set_id == 0)
            return set_error(errbuf, errbuf_len,
                             "AOT Object access receiver shape set is missing");
        for (uint32_t j = 0; j < ev->nobject_access_cases; j++) {
            const XgObjectAccessCaseSummary *access_case = &ev->object_access_cases[j];
            ...
            if (access_case->case_id == XG_NO_ID ||
                ...
                return set_error(errbuf, errbuf_len,
                                 "AOT Object access case identity is stale");
            for (uint32_t k = j + 1; k < ev->nobject_access_cases; k++) {
                ...
                if (other->case_id == access_case->case_id)
                    return set_error(errbuf, errbuf_len,
                                     "AOT Object access case id is duplicated");
            ...
            case_shape =
                xg_global_evidence_find_object_shape(ev, access_case->receiver_shape_id);
...
static bool verify_json_dynamic_access_plan_rederives(const XgGlobalEvidence *ev,
                                              ...
                                              const XgJsonDynamicAccessSummary *access, char *errbuf,
                                              size_t errbuf_len) {
    if (!ev || !plan || !access)
        return set_error(errbuf, errbuf_len, "AOT Json access verifier has incomplete input");
    if (plan->json_dynamic_access_id != access->json_dynamic_access_id || plan->module_id != access->module_id ||
...
static bool verify_object_access_case_set(const XgGlobalEvidence *ev,
    ...
    if (!ev || !access || access->receiver_shape_count == 0 ||
...
static bool verify_object_access_plan_rederives(const XgGlobalEvidence *ev,
    ...
    if (plan->dispatch_case_start > bundle->nobject_access_case_plans ||
        ...
        return set_error(errbuf, errbuf_len,
                         "AOT Object access dispatch case range is stale");
    uint32_t matched = 0;
    for (uint32_t i = 0; i < ev->nobject_access_cases; i++) {
        ...
        if (matched >= access->receiver_shape_count)
            return set_error(errbuf, errbuf_len,
                             "AOT Object access dispatch case count is stale");
