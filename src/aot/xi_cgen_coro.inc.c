...
static const XiValue *cg_coro_box_scalar_source(XiCgenCtx *ctx, const XiValue *v) {
    if (!v || v->op != XI_BOX || v->nargs < 1)
        return NULL;
    XrRep rep = cg_value_plan_storage_rep(ctx, v->args[0]);
    return (rep == XR_REP_I64 || rep == XR_REP_F64) ? v->args[0] : NULL;
}

static bool cg_coro_is_typed_send_use(XiCgenCtx *ctx, const XiValue *user, const XiValue *target,
                                      uint16_t arg_idx) {
    if (!user || !target || !cg_coro_box_scalar_source(ctx, target) || arg_idx != 1)
        return false;
...
static bool cg_coro_box_only_feeds_typed_send(XiCgenCtx *ctx, const XiFunc *f,
                                              const XiValue *target) {
    if (!f || !cg_coro_box_scalar_source(ctx, target))
        return false;
    bool seen_use = false;
        ...
        for (uint32_t vi = 0; vi < blk->nvalues; vi++) {
            ...
            for (uint16_t a = 0; a < user->nargs; a++) {
                if (user->args[a] != target)
                    continue;
                if (!cg_coro_is_typed_send_use(ctx, user, target, a))
                    return false;
                seen_use = true;
            }
        }
...
static bool cg_coro_value_has_storage(XiCgenCtx *ctx, const XiFunc *f, const XiValue *v) {
    ...
    if (xi_coro_typed_recv_unbox_user(f, v))
        return false;
    if (cg_coro_box_only_feeds_typed_send(ctx, f, v))
        return false;
...
static const XiValue *cg_coro_unboxed_scalar_value(XiCgenCtx *ctx, const XiValue *v) {
    if (!v)
        return NULL;
    const XiValue *boxed = cg_coro_box_scalar_source(ctx, v);
    if (boxed)
        return boxed;
...
static const char *cg_coro_typed_send_helper(XiCgenCtx *ctx, const char *base, const XiValue *v,
                                             const XiValue **send_arg) {
    const XiValue *arg = cg_coro_unboxed_scalar_value(ctx, v);
    if (!arg)
        return base;
...
static const char *cg_coro_typed_recv_pair_helper(XiCgenCtx *ctx, const XiFunc *f,
                                                  const XiValue *v) {
    const XiValue *slot_value = xi_coro_typed_recv_unbox_user(f, v);
    XrRep rep = cg_value_plan_storage_rep(ctx, slot_value ? slot_value : v);
    if (rep == XR_REP_I64)
...
static void emit_int64_arg(XiCgenCtx *ctx, FILE *out, const XiValue *v) {
    XrRep rep = cg_value_plan_storage_rep(ctx, v);
    if (rep == XR_REP_I64) {
        emit_vref(out, v);
...
static void emit_assign_from_xrvalue_temp_rep(FILE *out, const XiValue *dst, const char *temp_name,
                                              XrRep rep) {
    fprintf(out, "    ");
    emit_vref(out, dst);
    fprintf(out, " = ");
...
static void emit_assign_from_xrvalue_temp(XiCgenCtx *ctx, FILE *out, const XiValue *dst,
                                          const char *temp_name) {
    if (cg_unit_enum_scalar_plan(ctx, dst->type)) {
        fprintf(out, "    ");
        emit_vref(out, dst);
        fprintf(out, " = ");
        const char *suffix = emit_load_conversion_prefix(ctx, out, dst, XR_REP_TAGGED);
        fprintf(out, "%s", temp_name);
...
static void emit_aot_coro_op_stmt(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const XiValue *v) {
    ...
    if (cg_coro_value_has_storage(ctx, f, v))
        emit_assign_from_xrvalue_temp(ctx, out, v, tmp_name);
}

static void emit_bridge_stored_tagged_value(XiCgenCtx *ctx, FILE *out, const XiValue *value) {
    if (!value || cg_value_plan_storage_rep(ctx, value) != XR_REP_TAGGED)
        return;
    fprintf(out, "    ");
    emit_vref(out, value);
...
static void emit_bridge_stored_owned_tagged_value(XiCgenCtx *ctx, FILE *out, const XiValue *value) {
    if (!value || cg_value_plan_storage_rep(ctx, value) != XR_REP_TAGGED)
        return;
    fprintf(out, "    ");
    emit_vref(out, value);
...
static int cg_coro_claim_state(XiCgenCtx *ctx, const XiFunc *f, const XiValue *op,
                               int *legacy_cursor) {
    const XiCoroPlan *plan = cg_coro_plan(ctx, f);
    const XiCoroSuspendPoint *point = xi_coro_plan_find_point(plan, op);
    if (!ctx || !plan || ctx->coro_emit_plan != plan || !point || point->state_id == 0 ||
        point->state_id > plan->nstates || point->state_id >= ctx->coro_emit_seen_cap ||
    ...
    ctx->coro_emit_seen[point->state_id] = 1;
    ctx->coro_emit_seen_count++;
    if (legacy_cursor)
        *legacy_cursor = (int) point->state_id;
    return (int) point->state_id;
}
...
/* A static declaration has no receiver parameter. Prove the namespace operand
 * from the frozen binding before omitting it from coroutine frame arguments. */
static bool cg_coro_static_namespace_operand_is_exact(
    XiCgenCtx *ctx, const XiFunc *caller, const XiValue *value, const XiFunc *callee) {
    if (!caller || !callee || !caller->semantic_plan || !callee->semantic_plan)
        return false;
    const XrSemanticOperationRecord *operation = cg_semantic_operation_for_value(ctx, caller, value);
    const XrSemanticFunctionRecord *function = xr_semantic_plan_function(
        callee->semantic_plan, callee->semantic_plan_function_index);
    if (!operation || !function || function->source_kind != XR_SEM_SOURCE_FUNCTION_STATIC_METHOD)
        return false;
    for (uint32_t i = 0; i < xr_semantic_plan_call_target_count(caller->semantic_plan); i++) {
        const XrSemanticCallTargetRecord *target = xr_semantic_plan_call_target(caller->semantic_plan, i);
        if (xr_semantic_plan_operation(caller->semantic_plan, target->operation) != operation ||
            !xr_stable_id_equal(target->callee_function, function->id))
            continue;
        if (target->kind == XR_SEM_CALL_TARGET_SOURCE_STATIC_METHOD_DEPENDENCY)
            return xr_semantic_dependency_static_method_is_exact(
                       caller->semantic_plan, target, callee->semantic_plan) ==
                   callee->semantic_plan_function_index;
        return target->kind == XR_SEM_CALL_TARGET_SOURCE_STATIC_METHOD_LOCAL &&
               caller->semantic_plan == callee->semantic_plan &&
               target->function == callee->semantic_plan_function_index;
    }
    return false;
}
...
static CgCoroSuspendCallSite cg_coro_direct_call_site_info(XiCgenCtx *ctx, const XiFunc *current,
    ...
    if (v->op == XI_CALL) {
        ...
        if (site.call.func && cg_coro_static_namespace_operand_is_exact(ctx, current, v, site.call.func))
            site.arg_start = 1;
        site.cl_source = NULL;
    }
...
static void cg_collect_sync_go_targets_from_func(XiCgenCtx *ctx, const XiFunc *f) {
    ...
    /* Heartbeat/wrapper requirements are executable-code requirements, not
     * whole-package inventory facts.  Imported libraries can contain dead
     * coroutine callers of an otherwise live synchronous function.  Scanning
     * those dead callers used to instrument the live function while the entry
     * plan (correctly) omitted the coroutine runtime, leaving an undefined
     * xr_aot_sync_backedge_heartbeat at link time.  Use the exact same
     * reachability predicate that controls function-body emission. */
...
static void cg_collect_sync_heartbeat_targets_from_func(XiCgenCtx *ctx, const XiFunc *f) {
    ...
    if (cg_func_body_is_reachable_from_roots(ctx, f, 0) && cg_func_needs_aot_coro_ctx(ctx, f)) {
        for (uint32_t bi = 0; bi < f->nblocks; bi++) {
            const XiBlock *blk = f->blocks[bi];
            if (!blk)
                continue;
...
static void emit_aot_frame_new_params(XiCgenCtx *ctx, FILE *out, const XiFunc *f, bool typed_params,
                                      bool empty_as_void) {
    ...
    for (uint16_t i = 0; i < total_params; i++) {
        if (need_comma)
            fprintf(out, ", ");
        fprintf(out, "%s p%u",
                typed_params ? ctype_str(cg_value_plan_storage_rep(ctx, f->params[i])) : "XrValue",
                i);
        need_comma = true;
    }
...
static void emit_aot_frame_raw_cl_arg(XiCgenCtx *ctx, FILE *out, const XiFunc *current,
    ...
    if (callee) {
        fprintf(out, "(xrt_closure_t *)");
        emit_vref(out, callee);
        if (cg_value_plan_storage_rep(ctx, callee) == XR_REP_TAGGED)
            fprintf(out, ".ptr");
        return;
    }
    fprintf(out, "NULL");
...
static bool emit_aot_frame_transfer_cl_arg(XiCgenCtx *ctx, FILE *out, const XiFunc *current,
    ...
    emit_aot_frame_raw_cl_arg(ctx, out, current, callee, target);
    fprintf(out,
...
static void emit_sync_go_frame_factory(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    fprintf(out, "(");
    emit_aot_frame_new_params(ctx, out, f, true, true);
    fprintf(out, ") {\n");
    fprintf(out, "    ");
    emit_fname_suffix(ctx, out, prefix, f, "_aot_frame");
    fprintf(out, " *f = (");
...
static bool cg_sync_go_param_is_consumed(XiCgenCtx *ctx, const XiFunc *f, uint16_t index) {
    if (!cg_sync_go_param_needs_release(f, index))
        return false;
    const XrSemanticFunctionRecord *function =
        f->semantic_plan ? xr_semantic_plan_function(f->semantic_plan,
                                                     f->semantic_plan_function_index) : NULL;
    const XrSemanticParameterRecord *parameter = function && index < function->parameter_count
        ? xr_semantic_plan_parameter(f->semantic_plan, function->parameter_begin + index) : NULL;
    if (!parameter || parameter->function != f->semantic_plan_function_index ||
        (parameter->ownership != XI_OWN_OWNED && parameter->ownership != XI_OWN_BORROWED)) {
        cg_value_emission_fail(ctx, "sync spawn parameter lacks frozen ownership");
        return false;
    }
...
static size_t estimate_sync_go_frame_size(XiCgenCtx *ctx, const XiFunc *f) {
    size_t size = 0;
    size_t max_align = 1;
    if (cg_func_frame_needs_cl(f))
        cg_coro_layout_add(&size, &max_align, sizeof(void *), _Alignof(void *));
    for (uint16_t i = 0; i < cg_coro_param_count(f); i++)
        cg_coro_layout_add_rep(&size, &max_align, cg_value_plan_storage_rep(ctx, f->params[i]));
    if (size == 0)
        cg_coro_layout_add(&size, &max_align, sizeof(uint8_t), _Alignof(uint8_t));
    return cg_coro_align_up(size, max_align);
}

static void emit_sync_go_frame_param_as_xrvalue(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
                                                uint16_t index) {
    XrRep rep = cg_value_plan_storage_rep(ctx, f->params[index]);
    if (rep == XR_REP_I64) {
...
static void emit_sync_go_frame_param_for_func_abi(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    if (param_rep == XR_REP_TAGGED) {
        emit_sync_go_frame_param_as_xrvalue(ctx, out, f, index);
    } else if (param_rep == XR_REP_I64 && frame_rep == XR_REP_I64 &&
               cg_value_type_is_bool(f->params[index])) {
        fprintf(out, "(uint8_t)(f->p%u != 0)", index);
    } else {
        const char *suffix =
...
static void emit_sync_go_wrapper(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const char *prefix) {
    ...
    record_coro_frame_stats(ctx, estimate_sync_go_frame_size(ctx, f), root_count, release_count);
    ...
    for (uint16_t i = 0; i < cg_coro_param_count(f); ++i) {
        if (!cg_sync_go_param_is_consumed(ctx, f, i))
            continue;
        XrRep argument_rep = use_boxed_adapter ? XR_REP_TAGGED : cg_func_param_abi_rep(ctx, f, i);
        fprintf(out, "    %s _owned_arg%u = ", ctype_str(argument_rep), i);
        if (use_boxed_adapter)
            emit_sync_go_frame_param_as_xrvalue(ctx, out, f, i);
        else
            emit_sync_go_frame_param_for_func_abi(ctx, out, f, i);
        fprintf(out, ";\n    f->p%u = %s;\n", i,
                cg_value_plan_storage_rep(ctx, f->params[i]) == XR_REP_TAGGED ? "XR_NULL_VAL" : "NULL");
    }
    if (use_boxed_adapter) {
    ...
    fprintf(out, "    }\n");
    for (uint16_t i = 0; i < cg_coro_param_count(f); i++) {
        if (!cg_sync_go_param_needs_release(f, i) || cg_sync_go_param_is_consumed(ctx, f, i))
            continue;
    ...
    fprintf(out, "(void) {\n    static const XrAotCoroDesc descriptor = {\n");
    fprintf(out, "    .name = \"%s\",\n", f->name ? f->name : "aot");
...
static void emit_coro_frame_factory(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    emit_fname_suffix(ctx, out, prefix, f, "_aot_frame_new");
    fprintf(out, "(");
    emit_aot_frame_new_params(ctx, out, f, false, true);
    fprintf(out, ") {\n");
    fprintf(out, "    ");
    emit_fname_suffix(ctx, out, prefix, f, "_aot_frame");
    fprintf(out, " *f = (");
...
static void emit_coro_callable_closure_ptr(XiCgenCtx *ctx, FILE *out, const XiValue *callee) {
    fprintf(out, "(xrt_closure_t *)");
    emit_vref(out, callee);
    if (cg_value_plan_storage_rep(ctx, callee) == XR_REP_TAGGED)
        fprintf(out, ".ptr");
...
static bool emit_coro_callable_target_switch(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    emit_coro_callable_closure_ptr(ctx, out, v->args[0]);
    fprintf(out, ";\n");
    fprintf(out,
    ...
    XrRep result_storage_rep = cg_value_plan_storage_rep(ctx, v);
    if ((result_storage_rep == XR_REP_TAGGED || result_storage_rep == XR_REP_PTR) &&
        !cg_value_plan_is_aggregate(ctx, v))
        fprintf(out,
...
static CgCoroNetCallKind cg_coro_net_call_kind(XiCgenCtx *ctx, const XiFunc *f, const XiValue *v) {
    ...
    if (!resolver.call_is_module_member)
        return CG_CORO_NET_NONE;
#define CG_NET_MEMBER(name_, kind_)                                                                \
    if (resolver.call_is_module_member(resolver.ud, f, v, "net", (name_)))                         \
    return (kind_)
    CG_NET_MEMBER("__accept", CG_CORO_NET_ACCEPT);
    CG_NET_MEMBER("__readInto", CG_CORO_NET_READ_INTO);
    CG_NET_MEMBER("__udpRecvInto", CG_CORO_NET_UDP_RECV_INTO);
...
static void emit_coro_net_timeout_result(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
                                         const XiValue *v, CgCoroNetCallKind kind,
    ...
    if (kind == CG_CORO_NET_WRITE_BYTES) {
        fprintf(out,
                ...
                v->id, v->id, v->id);
    } else if (kind == CG_CORO_NET_UDP_RECV_INTO || kind == CG_CORO_NET_READ_INTO) {
        fprintf(out, "        XrValue _net_timeout_value_%u = XR_FROM_INT(-1);\n", v->id);
    ...
    if (kind == CG_CORO_NET_WRITE_BYTES)
        fprintf(out, "        f->net_progress_%u = 0;\n", v->id);
    fprintf(out, "        f->state = 0;\n        goto %s;\n    }\n", done_label);
}
...
static bool emit_coro_net_io_call_stmt(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const XiValue *v,
    ...
    bool arity_ok = (kind == CG_CORO_NET_ACCEPT && argc == 1) ||
                    ((kind == CG_CORO_NET_READ_INTO || kind == CG_CORO_NET_UDP_RECV_INTO) && argc == 3) ||
    ...
    char wait_name[48];
    char resume_name[48];
    snprintf(retry_label, sizeof(retry_label), "N%u_RETRY", v->id);
    snprintf(done_label, sizeof(done_label), "N%u_DONE", v->id);
    snprintf(wait_name, sizeof(wait_name), "_net_wait_%u", v->id);
    ...
    if (kind == CG_CORO_NET_ACCEPT) {
        ...
        fprintf(out, ");\n");
    } else if (kind == CG_CORO_NET_READ_INTO || kind == CG_CORO_NET_UDP_RECV_INTO) {
        fprintf(out, "        xrt_net_try_result_t _net_%u = %s(", v->id,
                kind == CG_CORO_NET_UDP_RECV_INTO ? "xrt_net_udp_recv_into_try" : "xrt_net_read_into_try");
        emit_coro_net_storage_arg(ctx, out, v, 1);
        fprintf(out, ", ");
    ...
    fprintf(out, "            f->state = %d;\n", sid);
    fprintf(out,
            "            XrAotResult %s = xr_aot_io_wait(ctx, _net_%u.fd, "
...
static bool emit_coro_test_yield_call_stmt(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
    ...
    if (cg_coro_value_has_storage(ctx, f, v) && strcmp(method, "add") == 0) {
        ...
        emit_assign_from_i64_temp(ctx, out, v, temp);
...
static bool cg_coro_owner_forward_keeps_frame_source(const XiBlock *blk, uint32_t value_index,
                                                     const XiValue *forward) {
    if (!blk || !forward || forward->op != XI_OWNER_FORWARD || forward->nargs < 1 ||
        !forward->args[0] || value_index == 0)
        return false;

    const XiValue *prev = blk->values[value_index - 1];
    if (!prev || prev->op != XI_RETAIN || prev->nargs < 1 || !prev->args[0])
        return false;

    const XiValue *source = forward->args[0];
    for (uint8_t depth = 0; source && depth < 16; depth++) {
        if (source == prev->args[0])
            return true;
        if ((source->op == XI_BOX || source->op == XI_UNBOX || xi_copy_is_identity_alias(source)) &&
            source->nargs >= 1) {
...
static const XiValue *cg_coro_owner_forward_consumed_frame_source(XiCgenCtx *ctx, const XiFunc *f,
                                                                  const XiBlock *blk,
                                                                  uint32_t value_index,
                                                                  const XiValue *forward) {
    if (!forward || forward->op != XI_OWNER_FORWARD || forward->nargs < 1 || !forward->args[0])
        return NULL;

    if (cg_coro_owner_forward_keeps_frame_source(blk, value_index, forward))
        return NULL;

    /* Representation lowering can put an UNBOX/COPY alias in the physical
     * frame while ARC retains its tagged origin. Walk from the forwarded value
     * toward that origin and clear the first physical owning slot. */
    const XiValue *source = forward->args[0];
    for (uint8_t depth = 0; source && depth < 16; depth++) {
        if (cg_coro_value_needs_frame(ctx, f, source) &&
            cg_coro_value_live_across_suspend(ctx, f, source) &&
            cg_coro_value_needs_frame_arc_release(ctx, f, source))
            return source;
        if ((source->op == XI_BOX || source->op == XI_UNBOX || xi_copy_is_identity_alias(source)) &&
            source->nargs >= 1) {
            source = source->args[0];
...
static void emit_coro_clear_value_slot(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
                                       const XiValue *value) {
    fprintf(out, "    ");
    emit_vref(out, value);
    fprintf(out, " = ");
    if (cg_value_plan_is_aggregate(ctx, value) || cg_value_plan_is_vector(ctx, value))
        emit_value_plan_zero_expr(ctx, out, value);
    else {
        XrRep rep = cg_coro_decl_rep(ctx, f, value);
        fprintf(out, "%s",
                rep == XR_REP_PTR ? "NULL" : (rep == XR_REP_TAGGED ? "XR_NULL_VAL" : "0"));
    }
    fprintf(out, ";\n");
}

static void emit_coro_value_stmt(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const XiBlock *blk,
                                 uint32_t value_index, const XiValue *v, const char *prefix,
                                 int *state_id) {
        ...
        if (await_all) {
            ...
            if (deferred_task_array) {
                ...
                if (deferred_task_index)
                    emit_int64_arg(ctx, out, deferred_task_index);
                else
                    fprintf(out, "-1");
                fprintf(out, ", ");
            } else {
    ...
    if (cg_native_timer_call_is_exact(ctx, f, v)) {
        emit_value_generated_line_reset(ctx, out, v);
        int sid = cg_coro_claim_state(ctx, f, v, state_id);
        fprintf(out, "    f->state = %d;\n", sid);
        emit_value_source_line(ctx, out, v);
    ...
    if (v->op == XI_TIME_AFTER) {
        ...
        emit_int64_arg(ctx, out, v->args[0]);
        fprintf(out, ");\n");
        if (cg_coro_value_has_storage(ctx, f, v)) {
            char tmp[32];
    ...
    if (v->op == XI_CHAN_TRY_SEND) {
        ...
        const char *helper =
            cg_coro_typed_send_helper(ctx, "xr_aot_chan_try_send_ready", v->args[1], &send_arg);
    ...
    if (xi_value_is_task_method_call(v, "cancel", 0)) {
        if (cg_coro_value_has_storage(ctx, f, v))
            ...
            fprintf(out, "    (void)xr_aot_task_cancel(ctx, ");
        emit_vref(out, v->args[0]);
        fprintf(out, ");\n");
        if (cg_coro_value_has_storage(ctx, f, v)) {
            char tmp[32];
            ...
            emit_assign_from_xrvalue_temp(ctx, out, v, tmp);
            emit_bridge_stored_tagged_value(ctx, out, v);
        }
        return;
    }
    ...
    if (xi_value_is_channel_method_call(v, "sendTimeout", 2)) {
        ...
        const XiValue *send_arg = NULL;
        const char *helper =
            cg_coro_typed_send_helper(ctx, "xr_aot_chan_send_timeout", v->args[1], &send_arg);
    ...
    if (xi_value_is_channel_method_call(v, "recvTimeout", 1)) {
        ...
        if (result_observed && cg_coro_value_has_storage(ctx, f, v))
            emit_bridge_stored_owned_tagged_value(ctx, out, v);
        if (result_observed)
            emit_coro_debug_result_source_var_sync(ctx, out, f, v);
        return;
    }
    ...
    if (v->op == XI_CHAN_SEND || xi_value_is_channel_method_call(v, "send", 1)) {
        ...
        const XiValue *send_arg = NULL;
        const char *helper =
            cg_coro_typed_send_helper(ctx, "xr_aot_chan_send", send_value, &send_arg);
    ...
    if (v->op == XI_CHAN_RECV) {
        ...
        fprintf(out, ";\n");
        fprintf(out, "    f->state = %d;\n", sid);
        const char *helper = cg_coro_typed_recv_pair_helper(ctx, f, v);
        emit_value_source_line(ctx, out, v);
    ...
    if (v->op == XI_CALL_METHOD && xi_value_type_is_channel(v->args[0])) {
        if (xi_value_is_channel_method_call(v, "trySend", 1)) {
            ...
            const XiValue *send_arg = NULL;
            const char *helper =
                cg_coro_typed_send_helper(ctx, "xr_aot_chan_try_send", v->args[1], &send_arg);
            bool transfer_helper = send_arg == NULL && strcmp(helper, "xr_aot_chan_try_send") == 0;
            const XaotTransferPlan *transfer_plan = NULL;
            if (transfer_helper) {
                helper = "xr_aot_chan_try_send_xrt";
                transfer_plan = cg_required_transfer_plan(ctx, v, 0, v->args[1], "channel method");
                if (!transfer_plan)
                    return;
            }
            emit_value_source_line(ctx, out, v);
            ...
            emit_coro_send_value(ctx, out, v->args[1], send_arg);
            if (transfer_helper)
                fprintf(out, ", %u", (unsigned) transfer_plan->mode);
            fprintf(out, ");\n");
            emit_value_generated_line_reset(ctx, out, v);
            if (cg_coro_value_has_storage(ctx, f, v)) {
                char tmp[32];
        ...
        if (xi_value_is_channel_method_call(v, "close", 0)) {
            if (cg_coro_value_has_storage(ctx, f, v))
                fprintf(out, "    XrValue _chan_method_%u = ", v->id);
            else
                fprintf(out, "    (void)");
            fprintf(out, "xr_aot_chan_close(ctx, ");
            emit_vref(out, v->args[0]);
            fprintf(out, ");\n");
            if (cg_coro_value_has_storage(ctx, f, v)) {
                char tmp[32];
    ...
    emit_value_rhs(ctx, out, f, v, prefix);
    fprintf(out, ";\n");
    const XiValue *moved_frame_source =
        cg_coro_owner_forward_consumed_frame_source(ctx, f, blk, value_index, v);
    if (moved_frame_source)
        emit_coro_clear_value_slot(ctx, out, f, moved_frame_source);
    emit_value_generated_line_reset(ctx, out, v);
    emit_debug_source_var_sync(ctx, out, f, v);
}
...
static void emit_coro_block(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const XiBlock *blk,
    ...
    for (uint32_t i = 0; i < blk->nvalues; i++) {
        ...
        if (v) {
            emit_coro_value_stmt(ctx, out, f, blk, i, v, prefix, state_id);
...
static void xi_cgen_coro_func(XiCgenCtx *ctx, FILE *out, XiFunc *f, const char *prefix) {
    ...
    uint32_t seen_required = plan->nstates + 1u;
    if (seen_required > ctx->coro_emit_seen_cap) {
        uint8_t *seen = (uint8_t *) xr_realloc(ctx->coro_emit_seen, seen_required);
        if (!seen) {
            ctx->error = true;
            return;
        }
        ctx->coro_emit_seen = seen;
        ctx->coro_emit_seen_cap = seen_required;
    }
    memset(ctx->coro_emit_seen, 0, seen_required);
    ctx->coro_emit_plan = plan;
    ctx->coro_emit_seen_count = 0;
    ...
    if (state_count > 0) {
        ...
        for (uint32_t i = 1; i < plan->ndispatch; i++) {
            const XiCoroDispatchEntry *entry = &plan->dispatch[i];
            fprintf(out, "        case %u: goto S%u;\n", entry->state_id, entry->state_id);
        }
        fprintf(out, "        default: return xr_aot_error(XR_NULL_VAL, false);\n");
        fprintf(out, "    }\n");
    }
