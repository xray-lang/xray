...
static void xicgen_assert(XiCgenCtx *ctx, FILE *out, const XiFunc *f, const XiValue *v,
    ...
    if (ctx && ctx->freestanding_profile) {
        fprintf(out,
                "xrt_freestanding_trap(\"Assertion failed%s: %s\"), XR_NULL_VAL))",
...
static void emit_vararg_rest_expr_values(XiCgenCtx *ctx, FILE *out, uint32_t site_id,
    ...
    if (rest_typed) {
        ...
        fprintf(out, "xrt_array_new_typed_copy(%" PRId64 ", %s, ", rest_count,
                rest_elem.elem_name);
        if (rest_count == 0) {
            fprintf(out, "NULL");
        } else {
...
static bool xicgen_emit_slice_stmt(XiCgenCtx *ctx, FILE *out, const XiFunc *f,
                                   const XiValue *v, const char *prefix) {
    (void) prefix;
    ...
    bool static_source =
        cg_static_fixed_array_value_ex(ctx, source, &fixed, &static_slot, &static_module);
    bool full_fixed =
        source && (static_source || cg_fixed_array_lane_info_from_value(source, &fixed)) &&
        fixed.ctype && cg_const_int_value(v->args[1], &start) &&
        cg_const_int_value(v->args[2], &end) && start == 0 &&
        (end == INT64_MAX || end == (int64_t) fixed.count);
...
static bool xicgen_emit_planned_type_switch_method_stmt(XiCgenCtx *ctx, FILE *out,
    ...
    for (uint16_t i = 0; i < dispatch_plan->target_count; i++) {
        ...
        fprintf(out, "    %s (xrt_instanceof(_xr_ts_recv_%u, (uint16_t)",
                i == 0 ? "if" : "else if", v->id);
        if (!emit_class_native_type_id_expr(ctx, out, target_class)) {
            ctx->error = true;
            fprintf(stderr,
...
static bool emit_portable_map_class_ctor_value_stmt(XiCgenCtx *ctx, FILE *out,
                                                    const XiFunc *f, const char *prefix,
                                                    const XiValue *v) {
    if (!ctx || !out || !f || !v ||
        (v->op != XI_CALL && v->op != XI_CALL_METHOD) || v->nargs == 0)
        return false;
...
static void emit_class_native_type_register_helpers(XiCgenCtx *ctx, FILE *out, XiModule *module,
    ...
    for (uint16_t ci = 0; ci < module->nclasses; ci++) {
        ...
        if (module->classes[ci] &&
            (!module->classes[ci]->needs_runtime_type ||
             !cg_func_needs_class_native_type_register_helper(ctx, module->init,
                                                              module->classes[ci])))
            continue;
