...
static const XgObjectFieldSummary *xi_lower_object_shape_field_at(
    const XgGlobalEvidence *evidence, XgObjectShapeId shape_id, uint16_t ordinal) {
...
static bool xi_lower_fill_object_field_names_from_evidence(
    XiLower *l, const XrType *type, const char **names, int count, uint32_t source_span_id) {
    if (!l || !l->global_evidence || !l->func ||
        l->func->xg_body_func_id == XG_NO_ID || !type || !type->object.field_names ||
        count <= 0 || count > (int) UINT16_MAX || source_span_id == 0)
        return false;
    const XgGlobalEvidence *evidence = l->global_evidence;
    uint8_t domain = type->kind == XR_KIND_STRUCT_OBJECT ? XG_OBJECT_DOMAIN_STRUCT
...
static XiValue *xi_lower_object_literal_spread(XiLower *l, AstNode *node,
    ...
    if (XR_TYPE_HAS_OBJECT_SHAPE(result_type) && result_type->object.field_count > 0 &&
        ...
        if (!key_names)
            return NULL;
        if (!xi_lower_fill_canonical_object_field_names(
                l, result_type, key_names, static_count, (uint32_t) node->line))
            return NULL;
    }
