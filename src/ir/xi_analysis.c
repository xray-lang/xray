...
static void rpo_dfs(XiBlock *blk, XiBlock **post_order,
                     uint32_t *post_idx) {
...
static XiBlock **build_rpo_order(XiFunc *f, uint32_t reachable) {
    XiBlock **rpo_order = (XiBlock **)xr_calloc(reachable + 1,
                                                  sizeof(XiBlock *));
    if (!rpo_order) return NULL;
...
XR_FUNC void xi_compute_dominators(XiFunc *f) {
    ...
    while (changed) {
        ...
        for (uint32_t r = 2; r <= reachable; r++) {
            ...
            XR_DCHECK(new_idom != NULL,
                       "xi_compute_dominators: no processed pred");
                ...
                for (XiPhi *phi = succ->phis; phi; phi = phi->next) {
                    ...
                    for (uint16_t p = 0; p < succ->npreds; p++) {
                        if (succ->preds[p] == blk && p < phi->value.nargs) {
                            ...
                            if (arg) bitset_set(new_in, arg->id);
                            break;
                        }
                    }
                }
...
XR_FUNC bool xi_is_live_in(const XiLiveness *l, const XiBlock *blk,
    ...
    if (blk->rpo == 0 || blk->rpo > l->nblocks) return false;
