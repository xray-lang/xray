...
bool xi_semantic_intrinsic_verify_value(const XiValue *value, XiStage stage, char *error,
    ...
    if (desc->family == XA_INTRINSIC_FAMILY_TARGET) {
        ...
        if (desc->id == XA_INTRINSIC_STRING_BYTE_SLICE_VIEW) {
            ...
            if (value->op != XI_CALL_BUILTIN || value->nargs != 2 || !value->args ||
                !value->args[0] || !value->args[0]->type ||
                !XR_TYPE_IS_ARRAY(value->args[0]->type) || !value->args[1] ||
                !value->args[1]->type || !XR_TYPE_IS_INT(value->args[1]->type) ||
                value->type != value->args[0]->type || value->result_alias_operand != 0 ||
                value->aux || value->aux_int != 0 || value->aux_kind != XI_AUX_KIND_NONE)
                return set_error(error, error_size,
