...
static XiValue *lower_private_leaf_source_import_callee(XiLower *l, CallExprNode *call,
    ...
    const MemberAccessNode *member =
        call && call->callee && call->callee->type == AST_MEMBER_ACCESS
            ? &call->callee->as.member_access
            : NULL;
    const XaSelection *selection =
        l && l->analyzer && call && call->callee
            ? xa_analyzer_get_selection(l->analyzer, call->callee)
            : NULL;
    const XiImportRef *namespace_ref =
        namespace_member && namespace_member->op == XI_LOAD_FIELD &&
                namespace_member->nargs == 1
...
static XiValue *lower_program_semantic_call(XiLower *l, AstNode *node, CallExprNode *call) {
    ...
    if (!l || !l->program_semantics || !node || !call || !call->callee ||
        ...
        ((product || private_leaf) ? (call->arg_count != 0)
    ...
    bool source_private_leaf_call = private_leaf && program_call &&
                                    !lower_stable_id_is_zero(program_call->resolver_binding);
    bool native_private_leaf_call = private_leaf && program_call &&
                                    lower_stable_id_is_zero(program_call->resolver_binding);
    XiValue *callee = xi_lower_expr(l, call->callee);
    ...
    XiValue *argument = !product && !private_leaf && callee
                            ? xi_lower_expr(l, call->arguments[0])
                            : NULL;
    XiImportRef *import_ref = callee ? lower_import_ref_from_value(l, callee) : NULL;
    XrType *result_type = xi_lower_node_type(l, node);
    if (!callee || (!product && !private_leaf && (!argument || !argument->type ||
