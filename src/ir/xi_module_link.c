...
static XiModuleExport *find_export(XiModule **modules, int nmodules, const XiModuleImport *imp) {
    ...
    for (int m = 0; m < nmodules; m++) {
        ...
        if (strncmp(mod_name, imp_name, mod_len) != 0)
            continue;
        if (imp_name[mod_len] != '\0' && imp_name[mod_len] != '.')
            continue;
        ...
        for (uint16_t ei = 0; ei < mod->nexports; ei++) {
            if (mod->exports[ei].name && strcmp(mod->exports[ei].name, imp->member_name) == 0) {
...
static int find_module_by_path(XiModule **modules, int nmodules, const char *import_path) {
    if (!import_path)
        return -1;
...
static void tarjan_strongconnect(TarjanCtx *tc, int v) {
    ...
    if (mod) {
        for (uint16_t i = 0; i < mod->nimports; i++) {
            int w = find_module_by_path(tc->modules, tc->nmodules, mod->imports[i].module_path);
            if (w < 0)
                continue;
        ...
        for (uint16_t i = 0; i < mod->nimports; i++) {
            ...
            if (!imp->resolved && imp->member_name) {
                ...
                fprintf(stderr, "[xi_module_link] unresolved: %s.%s from %s\n",
                        ...
                        imp->member_name ? imp->member_name : "*", mod->name ? mod->name : "?");
            }
        }
    }
