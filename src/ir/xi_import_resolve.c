...
static int resolve_reexport_source_topo(const XrModuleGraph *graph, const XiModule *module,
    ...
    if (specifier[0] == '.') {
        ...
        spec_index = xr_module_graph_find_named_dependency(graph, module->path, specifier);
    }
    ...
    for (uint32_t b = 0; b < f->nblocks; b++) {
        ...
        for (uint32_t i = 0; i < blk->nvalues; i++) {
            ...
            if (ref->module_path[0] == '.') {
                ...
                target_spec_idx = xr_module_graph_find_named_dependency(
                    graph, importer_path, ref->module_path);
            }
