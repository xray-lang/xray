...
XR_FUNC void xi_lower_class_decl(XiLower *l, AstNode *node) {
    ...
    if (declared_instance_fields > 0) {
        ...
        size_t types_size =
            (size_t) declared_instance_fields * sizeof(*data->instance_field_types);
        ...
        if (cls_sym) {
            ...
            if (class_info && data->instance_field_types) {
                for (uint16_t field_index = 0;
                     field_index < data->instance_field_count; field_index++) {
                    ...
                    XrType *field_type =
                        field_links && (field_links->type || field_links->declared_type)
                            ? (field_links->type ? field_links->type
                                                 : field_links->declared_type)
                            : NULL;
                    if (field_type)
